...
const AnnotationResolvedObject& require_single_resolved_object(
    const std::vector<AnnotationResolvedObject>& resolved_objects) {
    assert(resolved_objects.size() == 1U);
...
void assert_single_resolved_bbox_and_mask(const std::vector<AnnotationResolvedObject>& resolved_objects,
                                          const AnnotationBox& bbox, const std::string_view mask_rle) {
...
void test_persistent_capture_space_model_mask_survives_live_mode() {
    const AnnotationFrame cropped = make_capture_space_frame();
    const std::vector<AnnotationResolvedObject> preview =
        make_capture_space_preview_result(cropped, 0U, std::nullopt, true);
    assert_single_resolved_bbox_and_mask(preview, AnnotationBox{0, 0, 1, 1}, "0:1");
}
...
void test_live_model_mask_requires_matching_live_frame_identity() {
    ...
    const std::vector<AnnotationResolvedObject> preview = make_capture_space_preview_result(
