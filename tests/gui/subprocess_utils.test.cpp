...
void test_append_console_output_normalizes_terminal_sequences() {
    std::string tail;
    mmltk::gui::console_output::append_console_output(
        tail,
        "hello\rworld\nabc\b!\033[31m?\n",
        128);
    assert(tail == "world\nab!?\n");
...
void test_child_setup_failure_round_trip() {
    ...
    const auto failure =
        mmltk::gui::subprocess::read_child_setup_failure<TestFailureStage>(pipe_fds[0]);
...
void test_spawn_captured_child_process_returns_open_fds() {
    auto child = mmltk::gui::subprocess::spawn_captured_child_process(
        "test child",
        [](const int stdout_fd, const int) {
            if (::dup2(stdout_fd, STDOUT_FILENO) < 0 || ::dup2(stdout_fd, STDERR_FILENO) < 0) {
                std::_Exit(127);
            }
            ::close(stdout_fd);
            constexpr std::array<char, 16> kPayload{
                's', 'p', 'a', 'w', 'n', 'e', 'd', ' ', 'o', 'u', 't', 'p', 'u', 't', '\n', '\0'};
            (void)::write(STDOUT_FILENO, kPayload.data(), kPayload.size() - 1U);
            std::_Exit(0);
        });
    ...
    const std::string output =
        mmltk::gui::console_output::read_fd(
            child.stdout_fd, "failed to read spawned child output: ", true);
    const auto setup_failure =
        mmltk::gui::subprocess::read_child_setup_failure<TestFailureStage>(child.setup_error_fd);
...
void test_run_captured_child_process_captures_output() {
    const auto result = mmltk::gui::subprocess::run_captured_child_process<TestFailureStage>(
        "test child",
        "failed to read test child output: ",
        [](const int stdout_fd, const int setup_error_fd) {
            ...
            ::close(stdout_fd);
            constexpr std::array<char, 17> kPayload{
                'c', 'a', 'p', 't', 'u', 'r', 'e', 'd', ' ', 'o', 'u', 't', 'p', 'u', 't', '\n', '\0'};
            (void)::write(STDOUT_FILENO, kPayload.data(), kPayload.size() - 1U);
            std::_Exit(0);
        });
...
void test_run_captured_child_process_reports_setup_failure() {
    const auto result = mmltk::gui::subprocess::run_captured_child_process<TestFailureStage>(
        "test child",
        "failed to read test child output: ",
        [](const int, const int setup_error_fd) {
            errno = ENOENT;
            (void)mmltk::gui::subprocess::write_child_setup_failure(setup_error_fd, TestFailureStage::kExec);
            std::_Exit(127);
        });
...
void test_drain_nonblocking_fd_reads_available_output() {
    ...
    assert(::write(pipe_fds[1], kPayload.data(), kPayload.size() - 1U) ==
           static_cast<ssize_t>(kPayload.size() - 1U));

    const std::string output =
        mmltk::gui::console_output::read_fd(
            pipe_fds[0], "failed to read test output pipe: ", true);
    assert(output == "ready");
...
void test_read_fd_to_string_reads_until_eof() {
    ...
    const std::string output =
        mmltk::gui::console_output::read_fd(
            pipe_fds[0], "failed to read test blocking output pipe: ");
...
MMLTK_REGISTER_TEST_CASE("[gui][subprocess_utils]",
