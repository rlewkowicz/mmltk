...
mmltk::gui::GuiSettingsSnapshot make_gui_snapshot(mmltk::gui::View current_view, mmltk::gui::UiSettingsState& ui,
    ...
    return mmltk::gui::GuiSettingsSnapshot{current_view, &ui, &train, &validate, &predict, &annotate, &export_state};
}
...
struct GuiSnapshotFixture {
    ...
    GuiSnapshotFixture() {
        predict.preset_name = "rf-detr-seg-small";
    }

    [[nodiscard]] mmltk::gui::GuiSettingsSnapshot make_snapshot() {
        return make_gui_snapshot(current_view, ui, train, validate, predict, annotate, export_state);
    }
};
...
void set_firefox_runtime_capabilities(BrowserRuntimeCapabilities& capabilities) {
    capabilities.host_backend = BrowserHostBackend::Firefox;
    capabilities.navigator_gpu = BrowserRuntimeCapabilityStatus::Available;
    capabilities.workspace_surface_bridge = BrowserRuntimeCapabilityStatus::Unavailable;
...
BrowserHostIntentContext make_fixture_intent_context(BrowserHostSettingsFixture& fixture,
                                                     const bool bind_view = false) {
    auto& gui = fixture.gui;
    return BrowserHostIntentContext{
        bind_view ? &gui.current_view : nullptr,
        &fixture.snapshot,
        &gui.predict,
        &gui.annotate,
    };
...
void apply_fixture_routed_intent(BrowserHostSettingsFixture& fixture, const IntentMessage& intent,
                                 const bool bind_view = false) {
    apply_routed_intent(route_intent(intent), make_fixture_intent_context(fixture, bind_view));
}
...
BrowserHostIntentContext make_annotation_intent_context(
    ...
    return BrowserHostIntentContext{
        current_view, nullptr,     nullptr, nullptr,    &document,
        &session,     &controller, frame,   categories, std::move(ensure_frame_pixels),
    };
}
...
void test_state_snapshot_round_trips_through_json() {
    ...
    set_firefox_runtime_capabilities(snapshot.runtime_capabilities);
    snapshot.workspace_surface = WorkspaceSurfaceInfo{
        "89", 640U, 360U, 4U, "rgba8unorm", "upright", "live", true,
    };
    ...
    assert(json.at("runtime_capabilities").at("host_backend") == "firefox");
    assert(json.at("runtime_capabilities").at("navigator_gpu") == "available");
    assert(json.at("runtime_capabilities").at("workspace_surface_bridge") == "unavailable");
    assert(json.at("runtime_capabilities").at("workspace_surface_zero_copy") == "unavailable");
    assert(json.at("runtime_capabilities").size() == 4U);
    assert(json.at("workspace_surface").at("generation") == "89");
    assert(json.at("workspace_surface").at("capacityWidth") == 640U);
    assert(json.at("workspace_surface").at("capacityHeight") == 360U);
    assert(json.at("workspace_surface").at("slotCount") == 4U);
    assert(json.at("workspace_surface").at("format") == "rgba8unorm");
    assert(json.at("workspace_surface").at("orientation") == "upright");
    assert(json.at("workspace_surface").at("sourceKind") == "live");
    assert(json.at("workspace_surface").at("ready") == true);
    ...
    assert(round_trip.workspace_surface == snapshot.workspace_surface);
    ...
    assert(round_trip.runtime_capabilities.host_backend == BrowserHostBackend::Firefox);
    assert(round_trip.runtime_capabilities.navigator_gpu == BrowserRuntimeCapabilityStatus::Available);
    assert(round_trip.runtime_capabilities.workspace_surface_bridge == BrowserRuntimeCapabilityStatus::Unavailable);
    assert(round_trip.runtime_capabilities.workspace_surface_zero_copy == BrowserRuntimeCapabilityStatus::Unavailable);
    assert(round_trip.workspace_surface == snapshot.workspace_surface);
}

void test_state_snapshot_round_trips_live_runtime_metadata_without_workspace_surface() {
    StateSnapshot snapshot;
    snapshot.protocol_version = kHostApiProtocolVersion;
    ...
    assert(json.at("workspace_surface").is_null());
...
void test_browser_runtime_capabilities_round_trip_accepts_firefox_backend() {
    ...
    assert(json.at("host_backend") == "firefox");
    ...
    assert(round_trip.host_backend == BrowserHostBackend::Firefox);
...
void test_route_predict_start_intent_preserves_request_metadata_and_options() {
    IntentMessage intent = make_intent_message(Workflow::Predict, "predict.start",
                                               nlohmann::json{{"backend", "onnx"}, {"threshold", 0.2}});
...
void test_predict_start_settings_patch_updates_predict_workflow_state() {
    ...
    intent.options = nlohmann::json{
        ...
        {"model_artifacts",
         {{"preset_name", "rf-detr-seg-medium"},
          ...
          {"source", static_cast<int>(ModelSelectionSource::Custom)},
          {"input", static_cast<int>(ModelInputMode::Onnx)}}},
    ...
    assert(fixture.gui.predict.preset_name == "rf-detr-seg-medium");
...
void test_train_details_settings_patch_updates_train_workflow_state() {
    ...
    apply_settings_update(
        SettingsUpdateIntent{nlohmann::json{{"workflows",
                                             {{"train",
                                               {{"model_artifacts",
                                                 {{"preset_name", "rf-detr-seg-medium"},
                                                  {"weights_path", "/tmp/base.pt"},
                                                  {"resolution", 512},
                                                  {"source", static_cast<int>(ModelSelectionSource::Custom)},
                                                  {"input", static_cast<int>(ModelInputMode::Weights)}}},
                                                {"dataset_paths",
                                                 {{"source_dir", "/tmp/dataset"},
                                                  {"compiled_directory", "/tmp/compiled"},
                                                  {"overwrite", true},
                                                  {"compile_dimensions", true},
    ...
    assert(fixture.gui.train.preset_name == "rf-detr-seg-medium");
    ...
    assert(fixture.snapshot.workflows.train->dataset_source_dir == "/tmp/dataset");
    assert(fixture.snapshot.workflows.train->compiled_dataset_dir == "/tmp/compiled");
    assert(fixture.snapshot.workflows.train->overwrite_compiled_dataset);
    assert(fixture.snapshot.workflows.train->compile_dimensions);
    assert(fixture.snapshot.workflows.train->model_resolution == 512);
    ...
    const IntentMessage intent =
        make_intent_message(Workflow::Predict, "settings.update",
                            nlohmann::json{
                                ...
                                {"workflows",
                                 {{"predict",
                                   {{"model_artifacts", {{"preset_name", "rf-detr-seg-medium"}}},
                                    {"predict", {{"output_path", "/tmp/updated.json"}, {"threshold", 0.4}}}}}}},
                            });
         ...
         {{"annotate",
           {{"source",
             {{"kind", static_cast<int>(mmltk::gui::SourceKind::VideoStream)},
              ...
              {"single_image_path", ""},
              {"image_directory", ""}}},
            {"model_artifacts", {{"input", static_cast<int>(ModelInputMode::None)}}}}}}},
    };
...
void test_route_annotate_and_live_native_shell_control_intents() {
    ...
    const IntentMessage ui_log_intent = make_intent_message(
        Workflow::Live, "ui.log",
        nlohmann::json{{"level", "debug"}, {"event", "transport.connected"}, {"fields", {{"attempt", 1}}}});
    const RoutedIntent ui_log_routed = route_intent(ui_log_intent);
    assert(std::holds_alternative<UiLogIntent>(ui_log_routed.payload));
    assert(routed_intent_name(ui_log_routed) == "ui.log");
    const auto& ui_log = std::get<UiLogIntent>(ui_log_routed.payload);
    assert(ui_log.event == "transport.connected");
    assert(ui_log.fields.at("attempt") == 1);
...
void test_generated_browser_contract_dialog_ids_are_bound() {
    ...
    for (const BrowserNativeFileDialogContractSpec& dialog : kBrowserNativeFileDialogsById) {
        ...
        assert(*browser_file_dialog_target(binding, access).path == picked_path);
    }
}
...
void test_native_browser_contract_metadata_covers_dtos_and_dispatch() {
    ...
    const nlohmann::json dataset_schema = api::schema_for<DatasetArtifactState>();
    assert(dataset_schema.at("properties").at("compile_phase").at("enum").size() == 6U);
    assert(dataset_schema.at("properties").contains("elapsed_ms"));
    assert(dataset_schema.at("properties").contains("remaining_ms"));
    assert(dataset_schema.at("properties").contains("dropped_instances"));
    DatasetArtifactState dataset_state;
    dataset_state.phase = ArtifactOperationPhase::Running;
    dataset_state.compile_phase = DatasetCompilePhase::Pixels;
    dataset_state.generation = 7U;
    dataset_state.done = 41U;
    dataset_state.total = 100U;
    dataset_state.elapsed_ms = 2'500U;
    dataset_state.remaining_ms = 3'750U;
    dataset_state.dropped_instances = 2U;
    dataset_state.eta_ready = true;
    const nlohmann::json encoded_dataset_state = dataset_state;
    const DatasetArtifactState decoded_dataset_state = encoded_dataset_state.get<DatasetArtifactState>();
    assert(decoded_dataset_state.compile_phase == DatasetCompilePhase::Pixels);
    assert(decoded_dataset_state.generation == dataset_state.generation);
    assert(decoded_dataset_state.elapsed_ms == dataset_state.elapsed_ms);
    assert(decoded_dataset_state.remaining_ms == dataset_state.remaining_ms);
    assert(decoded_dataset_state.dropped_instances == dataset_state.dropped_instances);
    assert(decoded_dataset_state.eta_ready);
...
[[nodiscard]] nlohmann::json representative_file_dialog_payload(const Workflow workflow) {
    ...
    return nlohmann::json{
        {"token", 1},
        {"dialog_id", found->id},
        {"target_field", found->field},
        {"mode", file_dialog_mode_name(found->mode)},
        {"title", found->title},
...
[[nodiscard]] nlohmann::json representative_payload(const std::string_view intent_id, const Workflow workflow) {
    if constexpr (std::is_same_v<Payload, TrainStartIntent> || std::is_same_v<Payload, TrainRemoteStartIntent> ||
    ...
    } else if constexpr (std::is_same_v<Payload, DatasetCompileStartIntent>) {
        return nlohmann::json{{"source_dir", "/workspace/dataset"},
                              {"output_dir", "/workspace/build/dataset"},
                              {"preset_name", "rf-detr-seg-medium"},
                              {"resolution", 512},
                              {"overwrite", false}};
    } else if constexpr (std::is_same_v<Payload, CustomModelSelectIntent>) {
        return nlohmann::json{{"dialog_token", 1},
                              {"artifact_kind", "weights"},
                              {"preset_name", "rf-detr-seg-nano"},
                              {"resolution", 312},
                              {"path", "/workspace/weights/custom.pth"}};
    } else if constexpr (std::is_same_v<Payload, SettingsResetIntent>) {
        return nlohmann::json{{"confirmed", true}};
    } else if constexpr (std::is_same_v<Payload, PredictStartIntent>) {
        return nlohmann::json{{"backend", "auto"}};
        ...
        return nlohmann::json{{"phase", "begin"}, {"pointer_id", 1},   {"button", 0},          {"buttons", 1},
                              {"canvas_x", 10.0}, {"canvas_y", 11.0},  {"capture_x", 12},      {"capture_y", 14},
                              {"tool", "box"},    {"brush_radius", 3}, {"drag_kind", "create"}};
    ...
    } else if constexpr (std::is_same_v<Payload, UiLogIntent>) {
        return nlohmann::json{{"level", "trace"}, {"event", "contract.representative"}, {"fields", {}}};
...
void test_route_intent_validation_rejects_invalid_workflow_and_payloads() {
    ...
    try {
        ...
        invalid_settings_preset.payload = nlohmann::json{
            {"patch", {{"workflows", {{"train", {{"model_artifacts", {{"preset_name", "rf-detr-missing"}}}}}}}}},
        };
...
void test_live_runtime_state_to_json_surfaces_preview_and_pipeline_status() {
    ...
    set_firefox_runtime_capabilities(state.runtime_capabilities);
    ...
    state.compositor.source_acquire_waits = 8U;
    state.compositor.source_leases_acquired = 9U;
    state.compositor.source_leases_released = 7U;
    state.compositor.source_stale_releases = 1U;
    state.compositor.source_skipped_stale_frames = 3U;
    state.compositor.source_slot_pressure = 4U;
    state.compositor.source_release_latency_ns = 55'000U;
    ...
    state.single_buffer_diagnostic.failure_stage = "frame.presentation_unavailable";
    ...
    assert(runtime.at("runtime_capabilities").at("host_backend") == "firefox");
    assert(runtime.at("runtime_capabilities").at("navigator_gpu") == "available");
    assert(runtime.at("runtime_capabilities").at("workspace_surface_bridge") == "unavailable");
    assert(runtime.at("runtime_capabilities").at("workspace_surface_zero_copy") == "unavailable");
    ...
    assert(runtime.at("preview")
               .at("last_failure_detail")
               .get<std::string>()
    ...
    assert(runtime.at("preview")
               .at("last_failure_result_detail")
               .get<std::string>()
               .find("stage=compositor.publish_workspace") != std::string::npos);
    assert(!runtime.at("preview").contains("publication_counters"));
    ...
    assert(runtime.at("compositor").at("source_acquire_waits") == 8U);
    assert(runtime.at("compositor").at("source_leases_acquired") == 9U);
    assert(runtime.at("compositor").at("source_leases_released") == 7U);
    assert(runtime.at("compositor").at("source_stale_releases") == 1U);
    assert(runtime.at("compositor").at("source_skipped_stale_frames") == 3U);
    assert(runtime.at("compositor").at("source_slot_pressure") == 4U);
    assert(runtime.at("compositor").at("source_release_latency_ns") == 55'000U);
    ...
    assert(runtime.at("single_buffer_diagnostic").at("failure_stage") == "frame.presentation_unavailable");
...
void test_browser_live_display_startup_state_requires_current_drawn_revision() {
    ...
    assert(std::string_view(browser_live_display_startup_state(
               true, current_revision, imported_revision, submitted_revision, drawn_revision)) == "surface published");

    imported_revision = current_revision;
    assert(std::string_view(browser_live_display_startup_state(
               true, current_revision, imported_revision, submitted_revision, drawn_revision)) == "texture imported");
    ...
    assert(std::string_view(browser_live_display_startup_state(
               true, current_revision, imported_revision, submitted_revision, drawn_revision)) == "draw submitted");
...
void test_apply_routed_settings_update_updates_bound_gui_state() {
    ...
    const IntentMessage intent = make_intent_message(
        ...
        nlohmann::json{
            ...
            {"workflows",
             {{"predict",
               {{"model_artifacts", {{"preset_name", "rf-detr-seg-medium"}}}, {"predict", {{"threshold", 0.4}}}}}}},
        });

    apply_fixture_routed_intent(fixture, intent, true);

    assert(gui.current_view == mmltk::gui::View::Annotate);
    assert(gui.predict.preset_name == "rf-detr-seg-medium");
...
void test_structured_linux_import_contract_accepts_camelcase_nested_metadata() {
    ...
    const nlohmann::json json = {
        ...
        {"linuxImport",
         {{"image",
          ...
          {"metadata",
           {{"runtime", "firefox"}, {"textureFormat", "rgba8unorm"}, {"releaseBehavior", "transport_message"}}}}}};
    ...
    assert(native_import_metadata.runtime == "firefox");
    assert(linux_import_metadata.runtime == "firefox");
    assert(frame_slot_supports_imported_acquire(descriptor));

    const nlohmann::json normalized = descriptor;
    assert(normalized.at("linux_import").at("metadata").at("runtime") == "firefox");
...
void test_gui_settings_snapshot_builds_browser_state_snapshot() {
    ...
    gui.predict.preset_name = "rf-detr-seg-medium";
    ...
    assert(snapshot.workflow_state.at("workflows").at("predict").at("model_artifacts").at("preset_name") ==
           "rf-detr-seg-medium");
...
void test_live_view_snapshot_infers_source_and_attaches_annotation_without_workspace_surface() {
    BrowserHostSettingsFixture fixture;
    auto& gui = fixture.gui;
...
void test_browser_host_snapshot_cache_tracks_snapshot_delivery_revisions() {
    ...
    assert(first.workflow_state.at("marker") == "rf-detr-seg-small");
...
void test_browser_host_helper_resolves_packaged_browser_root() {
    const mmltk::gui::BrowserHostAssetPaths paths = mmltk::gui::resolve_browser_host_asset_paths();
    const std::filesystem::path packaged_root = mmltk::runtime_paths::browser_app_root();
    assert(paths.browser_root == packaged_root);
    assert(paths.index_html == packaged_root / "index.html");
}
...
MMLTK_REGISTER_TEST_CASE("[gui][browser_host_api]",
                         test_browser_runtime_capabilities_round_trip_accepts_firefox_backend);
MMLTK_REGISTER_TEST_CASE("[gui][browser_host_api]",
...
MMLTK_REGISTER_TEST_CASE("[gui][browser_host_api]",
                         test_state_snapshot_round_trips_live_runtime_metadata_without_workspace_surface);
MMLTK_REGISTER_TEST_CASE("[gui][browser_host_api]",
...
MMLTK_REGISTER_TEST_CASE("[gui][browser_host_api]",
                         test_live_view_snapshot_infers_source_and_attaches_annotation_without_workspace_surface);
MMLTK_REGISTER_TEST_CASE("[gui][browser_host_api]",
                         test_browser_host_snapshot_cache_tracks_snapshot_delivery_revisions);
MMLTK_REGISTER_TEST_CASE("[gui][browser_host_api]", test_browser_host_helper_resolves_packaged_browser_root);
