...
void test_create_drag_defers_commit_until_release() {
    ...
    start_preview_rect_drag(session,
                            RectDragKind::Create,
                            10.0f,
                            10.0f,
                            AnnotationBox{10, 10, 11, 11},
                            6);

    const PreviewRectDragResult drag =
        update_preview_rect_drag(session, true, 18.0f, 19.0f, viewport, 100, 100, 1);
    assert(drag.active);
    assert(drag.changed);
    assert(!drag.commit);
    assert(!drag.cancel);
    ...
    const PreviewRectDragResult release =
        update_preview_rect_drag(session, false, 18.0f, 19.0f, viewport, 100, 100, 1);
...
void test_create_drag_cancels_when_box_is_too_small() {
    ...
    const PreviewRectDragResult drag =
        update_preview_rect_drag(session, true, 13.0f, 13.0f, viewport, 100, 100, 1);
    assert(drag.changed);
    assert(!drag.commit);

    const PreviewRectDragResult release =
        update_preview_rect_drag(session, false, 13.0f, 13.0f, viewport, 100, 100, 1);
    assert(!release.active);
    assert(release.changed);
    assert(!release.commit);
    assert(release.cancel);
...
void test_move_drag_commits_only_on_release() {
    ...
    start_preview_rect_drag(session,
                            ...
                            25.0f,
                            25.0f,
                            AnnotationBox{20, 20, 40, 40},
                            1);

    const PreviewRectDragResult drag =
        update_preview_rect_drag(session, true, 35.0f, 30.0f, viewport, 100, 100, 1);
    assert(drag.active);
    assert(drag.changed);
    assert(!drag.commit);
    ...
    const PreviewRectDragResult release =
        update_preview_rect_drag(session, false, 35.0f, 30.0f, viewport, 100, 100, 1);
    assert(!release.active);
    assert(release.changed);
    assert(release.commit);
    assert(!release.cancel);
...
void test_resize_drag_clamps_to_bounds_before_commit() {
    ...
    PreviewRectDragSession session{};
    start_preview_rect_drag(session,
                            RectDragKind::ResizeBottomRight,
                            40.0f,
                            40.0f,
                            AnnotationBox{20, 20, 40, 40},
                            1);
    ...
    const PreviewRectDragResult drag =
        update_preview_rect_drag(session, true, 120.0f, 130.0f, viewport, 100, 100, 1);
    assert(drag.active);
    assert(drag.changed);
    assert(!drag.commit);
