...
void test_renderer_manual_overlay_snapshot_tracks_document_and_session_revisions() {
    ...
    const auto& brush_preview =
        require_optional_ref(snapshot.brush_preview, "expected brush preview in manual overlay snapshot");
    assert(brush_preview.capture_x == 21);
    assert(brush_preview.capture_y == 17);
    assert(brush_preview.radius == 9);
    assert(!brush_preview.erase);
}
