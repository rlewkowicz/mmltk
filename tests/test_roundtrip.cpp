...
struct TestOptions {
    std::string test_dir = "/tmp/fastloader_test";
...
static TestOptions parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--keep-artifacts") == 0) {
            ...
            fprintf(stderr,
                    "Usage: %s [--keep-artifacts] [--test-dir PATH] [--width N] [--height N] [--num-images N]\n",
                    argv[0]);
            std::exit(1);
        }
    }
    if (opts.width <= 0 || opts.height <= 0 || opts.num_images <= 0) {
        fprintf(stderr, "width, height, and num-images must be positive\n");
        std::exit(1);
    }
    return opts;
}
