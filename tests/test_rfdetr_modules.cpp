...
void test_detection_head_shapes() {
    fastloader::rfdetr::DetectionHead head(/*hidden_dim=*/256, /*num_classes=*/91);
...
void test_position_embedding_shape() {
    fastloader::rfdetr::PositionEmbeddingSine embedding(/*num_pos_feats=*/64, /*temperature=*/10000.0, /*normalize=*/true);
    fastloader::rfdetr::NestedTensor tensor{
...
void test_segmentation_head_shapes() {
    fastloader::rfdetr::SegmentationHead head(/*in_dim=*/256, /*num_blocks=*/3);
...
void test_cuda_ms_deform_attn_matches_reference() {
    ...
    const auto actual = fastloader::rfdetr::ms_deform_attn_cuda_autograd(
        value,
        spatial_shapes,
        level_start_index,
        sampling_locations,
...
int main() {
    ...
    test_position_embedding_shape();
    test_segmentation_head_shapes();
    test_cuda_ms_deform_attn_matches_reference();
    return 0;
}
