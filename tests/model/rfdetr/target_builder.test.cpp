...
void test_target_builder_with_masks(int first_class_id) {
    ...
    mmltk::CompilerConfig compile_config;
    compile_config.source_dir = mmltk::testsupport::dataset_dir(fixture);
    compile_config.output_dir = mmltk::testsupport::compiled_dir(fixture);
    ...
    const mmltk::DatasetCompilePlan compile_plan =
        mmltk::DatasetCompiler::prepare(compile_config, {compile_config.split});
    mmltk::DatasetCompiler::compile(compile_plan, 0U);
    ...
    const auto device_index = mmltk::rfdetr::cuda_device_index(kDeviceId);
    ...
    if (!prepared.packed_masks.has_value()) {
        throw std::runtime_error("expected packed masks from target builder");
    }
    const auto& packed_masks = prepared.packed_masks.value();
    assert(packed_masks.bits.defined());
    assert(packed_masks.bits.size(0) == 2);
    assert(packed_masks.height == fixture.height);
    assert(packed_masks.width == fixture.width);
    assert(packed_masks.bits.size(1) ==
    ...
    const auto dense_masks = unpack_packed_masks(packed_masks);
...
MMLTK_REGISTER_TEST_CASE("[model][rfdetr][target_builder]", test_target_builder_with_zero_based_classes);
