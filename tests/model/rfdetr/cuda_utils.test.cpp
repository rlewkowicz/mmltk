...
void test_vertical_flip_in_place_reverses_rows() {
    ...
    CUDA_ASSERT_OK(
        cudaMallocPitch(reinterpret_cast<void**>(&device), &pitch_bytes, static_cast<std::size_t>(width) * 3U, height));
    CUDA_ASSERT_OK(cudaMemcpy2DAsync(device, pitch_bytes, input.data(), static_cast<std::size_t>(width) * 3U,
    ...
    CUDA_ASSERT_OK(cudaMemcpy2D(output.data(), static_cast<std::size_t>(width) * 3U, device, pitch_bytes,
...
void test_split_to_planar_preserves_bottom_row_orientation() {
    ...
    CUDA_ASSERT_OK(cudaMallocPitch(reinterpret_cast<void**>(&src_device), &src_pitch_bytes,
                                   static_cast<std::size_t>(width) * 3U, height));
    CUDA_ASSERT_OK(cudaMemcpy2DAsync(src_device, src_pitch_bytes, input.data(), static_cast<std::size_t>(width) * 3U,
                                     static_cast<std::size_t>(width) * 3U, height, cudaMemcpyHostToDevice, stream));
    ...
    CUDA_ASSERT_OK(launch_bgr_split_to_planar_float(src_device, src_pitch_bytes, width, height, dst_device, width,
                                                    height, stream));
    CUDA_ASSERT_OK(cudaStreamSynchronize(stream));
