...
inline torch::Tensor make_fixture_query_feat(const ParityFixtureCase& fixture) {
    return torch::arange(fixture.query_rows * kParityFixtureHiddenDim, torch::TensorOptions().dtype(torch::kFloat32))
...
inline torch::Tensor make_fixture_refpoint_embed(const ParityFixtureCase& fixture) {
    return torch::linspace(-1.0f + fixture.offset, 1.0f + fixture.offset, fixture.query_rows * 4,
                           torch::TensorOptions().dtype(torch::kFloat32))
        .reshape({fixture.query_rows, 4});
}

inline torch::Tensor make_fixture_class_weight(const ParityFixtureCase& fixture) {
    return torch::arange(kParityFixtureNumClasses * kParityFixtureHiddenDim,
                         torch::TensorOptions().dtype(torch::kFloat32))
...
inline torch::Tensor make_fixture_class_bias(const ParityFixtureCase& fixture) {
    return torch::linspace(-1.0f + fixture.offset, 1.0f + fixture.offset, kParityFixtureNumClasses,
                           torch::TensorOptions().dtype(torch::kFloat32));
}
...
inline void assert_matches_native_parity_fixture(const NativeCheckpoint& checkpoint, const ParityFixtureCase& fixture) {
    ...
    parity_fixture_require(checkpoint.metadata.source_path == fixture.upstream_filename,
                           "fixture source path mismatch");
    parity_fixture_require(checkpoint.metadata.num_classes == kParityFixtureNumClasses, "fixture class count mismatch");
    ...
    for (const auto& entry : checkpoint.state_dict) {
        if (entry.name == "query_feat.weight") {
            ...
            found_refpoint = true;
            parity_fixture_require(torch::equal(entry.tensor, make_fixture_refpoint_embed(fixture)),
                                   "refpoint_embed mismatch");
        } else if (entry.name == "class_embed.weight") {
            found_class_weight = true;
            parity_fixture_require(torch::equal(entry.tensor, make_fixture_class_weight(fixture)),
                                   "class_embed.weight mismatch");
        } else if (entry.name == "class_embed.bias") {
            found_class_bias = true;
            parity_fixture_require(torch::equal(entry.tensor, make_fixture_class_bias(fixture)),
