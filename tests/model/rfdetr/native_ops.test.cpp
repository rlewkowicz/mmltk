...
mmltk::rfdetr::ModelOutputs make_distinct_layer_outputs() {
    ...
    auto& enc_outputs =
        require_optional_ref(outputs.enc_outputs, "expected encoder outputs in distinct layer outputs");
...
void test_matcher_and_losses() {
    ...
    const auto loss_dict =
        mmltk::rfdetr::detection_loss_dict(outputs, targets, config, true, false);
    ...
    const auto weighted =
        mmltk::rfdetr::weighted_detection_loss(loss_dict, config, torch::Device(torch::kCPU));
...
void test_batched_matcher_transfer_matches_single_layer_losses() {
    ...
    const auto main_loss =
        mmltk::rfdetr::detection_loss_dict(main_only, targets, single_config, true, false);
    ...
    for (size_t aux_index = 0; aux_index < outputs.aux_outputs.size(); ++aux_index) {
        ...
        const auto aux_loss =
            mmltk::rfdetr::detection_loss_dict(aux_only, targets, single_config, true, false);
    ...
    const auto enc_loss =
        mmltk::rfdetr::detection_loss_dict(enc_only, targets, single_config, true, false);
...
void test_sparse_mask_loss_matches_dense_reference() {
    ...
    const auto dense_loss =
        mmltk::rfdetr::detection_loss_dict(dense_outputs, targets, config, true, false);
    torch::manual_seed(1234);
    const auto sparse_loss =
        mmltk::rfdetr::detection_loss_dict(sparse_outputs, targets, config, true, false);
...
void test_packed_mask_sampling_matches_grid_sample_nearest_boundaries() {
    ...
    const auto sampled = mmltk::rfdetr::sample_packed_masks_cuda(
        packed_bits,
        packed.height,
        packed.width,
        mask_indices,
