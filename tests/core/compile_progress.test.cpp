...
void assert_progress_batch_contract() {
    constexpr size_t kImageCount = 64U;
    constexpr size_t kPublishBatch = 32U;
    CompileTelemetry telemetry(kImageCount);
    compiler_internal::ProgressCounter counter{
        &telemetry,
        compiler_internal::ProgressCounter::Stage::kPixels,
    };

    {
        compiler_internal::ProgressBatch batch(&counter);
        CompileProgress progress = telemetry.snapshot();
        assert(progress.active_workers == 1U);
        assert(progress.pixel_done == 0U);

        for (size_t index = 0; index + 1U < kPublishBatch; ++index) {
            batch.increment();
        }
        ...
        progress = telemetry.snapshot();
        assert(progress.active_workers == 1U);
        assert(progress.pixel_done == kPublishBatch);
    ...
    const CompileProgress progress = telemetry.snapshot();
    assert(progress.active_workers == 0U);
    assert(progress.pixel_done == kPublishBatch + 1U);
}
...
void test_partial_mask_vanish_keeps_instance() {
    ...
    assert(instance.mask_rle_pairs == 2);
    assert(instance.bbox_x1 == 0);
    assert(instance.bbox_y1 == 0);
    assert(instance.bbox_x2 == 3);
    assert(instance.bbox_y2 == 3);
}

void test_compile_default_workers_use_full_cpuset() {
    const std::vector<int> allowed = allowed_cpu_set();
    assert(!allowed.empty());
    assert(compiler_internal::resolve_num_workers(0, std::span<const int>(allowed)) ==
           static_cast<int>(allowed.size()));
    assert(compiler_internal::resolve_num_workers(-1, std::span<const int>(allowed)) ==
           static_cast<int>(allowed.size()));
}
...
void test_format_compile_postfix_uses_expected_spinner_and_metrics() {
    ...
    progress.active_workers = 2;

    assert(spdmon::format_compile_postfix(progress, 0) == "pixels 2 active |");
    assert(spdmon::format_compile_postfix(progress, 1) == "pixels 2 active /");
    assert(spdmon::format_compile_postfix(progress, 2) == "pixels 2 active -");
    assert(spdmon::format_compile_postfix(progress, 3) == "pixels 2 active \\");
}
...
void test_compile_progress_reports_monotonic_updates() {
    ...
    assert(plan.splits.size() == 1U);
    assert(plan.splits.front().image_count == static_cast<uint32_t>(fixture.num_images));
    assert(plan.total_steps() == expected_total);
    compile_with_progress(plan, 0U, [&](const CompileProgress& progress) {
        observed_done.push_back(progress.done);
        observed_totals.push_back(progress.total);
        observed_phases.push_back(progress.phase);
        observed_active.push_back(progress.active_workers);
        observed_label_done.push_back(progress.label_done);
        observed_pixel_done.push_back(progress.pixel_done);
        observed_dropped_instances.push_back(progress.dropped_instances);
    ...
    assert(
        std::all_of(observed_done.begin(), observed_done.end(), [&](size_t done) { return done <= expected_total; }));
    assert(observed_done.back() == expected_total);
    assert(observed_phases.back() == CompileProgressPhase::kSyncing);
    assert(observed_label_done.back() == expected_images);
    assert(observed_pixel_done.back() == expected_images);
    assert(observed_active.back() == 0U);
    assert(observed_dropped_instances.back() == 0U);
    for (size_t index = 1; index < observed_done.size(); ++index) {
        assert(observed_label_done[index] >= observed_label_done[index - 1]);
        assert(observed_pixel_done[index] >= observed_pixel_done[index - 1]);
    }
    ...
    assert_progress_batch_contract();
