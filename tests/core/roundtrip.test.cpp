...
void test_roundtrip_end_to_end() {
    ...
    compile_with_progress(resized_plan, 0U,
                          [&](const CompileProgress& progress) { resized_progress.push_back(progress); });
    const size_t resized_total_steps = resized_plan.total_steps();
    assert(!resized_progress.empty());
    assert(std::is_sorted(resized_progress.begin(), resized_progress.end(),
                          [](const CompileProgress& lhs, const CompileProgress& rhs) { return lhs.done < rhs.done; }));
    assert(std::all_of(resized_progress.begin(), resized_progress.end(), [&](const CompileProgress& progress) {
        return progress.done <= resized_total_steps && progress.total == resized_total_steps;
    }));
    assert(resized_progress.back().done == resized_total_steps);
    assert(resized_progress.back().phase == CompileProgressPhase::kSyncing);
    assert(resized_progress.back().active_workers == 0U);
