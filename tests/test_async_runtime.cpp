...
using namespace fastloader::runtime;
...
void test_success_callback_runs_on_ui_drain() {
    ...
    (void)submit_background_task(
        ...
        [&](const std::string&) {
            assert(false && "unexpected error callback");
        });
...
void test_error_callback_propagates_message() {
    ...
    (void)submit_background_task(
        ...
        [&](int) {
            assert(false && "unexpected success callback");
        },
...
int main() {
    test_success_callback_runs_on_ui_drain();
    test_error_callback_propagates_message();
    test_cancellation_drops_callbacks();
    return 0;
}
