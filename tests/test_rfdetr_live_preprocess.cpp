...
using fastloader::rfdetr::launch_bgr_split_to_planar_float;
using fastloader::rfdetr::launch_bgr_vertical_flip_in_place_pitched;
...
void test_split_to_planar_preserves_bottom_row_orientation() {
    ...
    std::vector<float> output(3U * width * height, 0.0f);
...
int main() {
    if (!has_cuda_device()) {
        std::fprintf(stderr, "Skipping live preprocess CUDA test: no CUDA device available.\n");
        return 0;
    }
