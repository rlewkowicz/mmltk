...
using fastloader::rfdetr::testsupport::assert_matches_native_parity_fixture;
...
using fastloader::rfdetr::testsupport::make_native_parity_fixture;
...
fs::path fixture_root() {
    return fs::temp_directory_path() / "fastloader_rfdetr_checkpoint_fixture";
}
...
const fastloader::rfdetr::StateDictEntry* find_entry(const fastloader::rfdetr::NativeCheckpoint& checkpoint,
                                                     const char* name) {
...
void test_upstream_checkpoint_parse(const ParityFixtureCase& fixture, const fs::path& upstream_path) {
    const auto checkpoint = fastloader::rfdetr::load_checkpoint(upstream_path);
    ...
    for (const auto& entry : checkpoint.state_dict) {
        ...
        if (entry.name == "class_embed.bias") {
            ...
            assert(entry.tensor.index({0}).item<float>() == -1.0f);
        }
    }
...
void test_native_checkpoint_roundtrip(const ParityFixtureCase& fixture, const fs::path& upstream_path) {
    const auto upstream = fastloader::rfdetr::load_checkpoint(upstream_path);
    const fs::path output_path = native_roundtrip_path(fixture);
    fs::create_directories(output_path.parent_path());

    const auto normalized = fastloader::rfdetr::normalize_checkpoint_to_native(upstream_path, output_path);
    const bool output_exists = fs::exists(output_path);
    assert(output_exists);
    assert(normalized.metadata.preset_name == fixture.preset_name);
    assert(fastloader::rfdetr::is_native_checkpoint_file(output_path));

    const auto native = fastloader::rfdetr::load_checkpoint(output_path);
...
void test_native_golden_fixture_roundtrip(const ParityFixtureCase& fixture) {
    ...
    fastloader::rfdetr::save_native_checkpoint(output_path, expected);
    const bool output_exists = fs::exists(output_path);
    assert(output_exists);
    assert(fastloader::rfdetr::is_native_checkpoint_file(output_path));

    const auto loaded = fastloader::rfdetr::load_checkpoint(output_path);
...
void test_native_checkpoint_tensor_preparation() {
    ...
    fastloader::rfdetr::save_native_checkpoint(output_path, checkpoint);
    const auto loaded = fastloader::rfdetr::load_checkpoint(output_path);
...
int main() {
    try {
        ...
        std::fprintf(stderr, "test_rfdetr_checkpoint error: %s\n", error.what());
    } catch (...) {
        std::fputs("test_rfdetr_checkpoint error: unknown exception\n", stderr);
    }
    fs::remove_all(fixture_root());
    return 1;
}
