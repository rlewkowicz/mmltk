...
using mmltk::DatasetCompilePlan;
...
void print_iteration_line(const char* phase, int index, int total, const TrainRun& run) {
    if (run.mask_ap.has_value()) {
        ...
        std::printf("%s=rfdetr.train %d/%d total=%.3fs train_loss=%.4f bbox=%.4f mask=null\n", phase, index, total,
                    static_cast<double>(run.elapsed_ns) / 1.0e9, run.train_loss, run.bbox_ap);
    }
    std::fflush(stdout);
}
