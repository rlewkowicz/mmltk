...
fastloader::rfdetr::PackedTargetMasks pack_dense_masks(const torch::Tensor& dense_masks) {
    ...
    auto bits = torch::zeros(
        {dense.size(0), words_per_mask},
    ...
    return fastloader::rfdetr::PackedTargetMasks{bits, height, width};
}

fastloader::rfdetr::PreparedTargets make_targets() {
...
fastloader::rfdetr::PreparedTargets make_multi_targets() {
    fastloader::rfdetr::PreparedTargets targets;
    targets.orig_sizes = torch::tensor({{100, 100}}, torch::TensorOptions().dtype(torch::kInt64));
    targets.nested_mask = torch::zeros({1, 100, 100}, torch::TensorOptions().dtype(torch::kBool));

    fastloader::rfdetr::PreparedTarget image_targets;
    ...
    image_targets.boxes = torch::tensor(
        {{0.5f, 0.5f, 0.2f, 0.2f}, {0.2f, 0.2f, 0.1f, 0.12f}},
...
fastloader::rfdetr::DetectionConfig make_config() {
    fastloader::rfdetr::DetectionConfig config;
    ...
    fastloader::rfdetr::populate_default_detection_weight_dict(config);
    return config;
}

fastloader::rfdetr::ModelOutputs make_outputs() {
    fastloader::rfdetr::ModelOutputs outputs;
    outputs.main.pred_logits = torch::tensor(
        {{{-5.0f, 8.0f, -5.0f}, {6.0f, -5.0f, -5.0f}}},
        torch::TensorOptions().dtype(torch::kFloat32));
    outputs.main.pred_boxes = torch::tensor(
        {{{0.5f, 0.5f, 0.2f, 0.2f}, {0.1f, 0.1f, 0.1f, 0.1f}}},
        torch::TensorOptions().dtype(torch::kFloat32));
...
fastloader::rfdetr::ModelOutputs make_distinct_layer_outputs() {
    ...
    auto& enc_outputs = outputs.enc_outputs.value();
    enc_outputs.pred_logits = enc_outputs.pred_logits + 0.1f;
    enc_outputs.pred_boxes = enc_outputs.pred_boxes + 0.015f;
    return outputs;
}

fastloader::rfdetr::ModelOutputs make_multi_match_outputs() {
    fastloader::rfdetr::ModelOutputs outputs;
    outputs.main.pred_logits = torch::tensor(
        {{{-5.0f, 8.0f, -5.0f}, {-5.0f, -5.0f, 8.0f}, {7.0f, -5.0f, -5.0f}}},
        torch::TensorOptions().dtype(torch::kFloat32));
    outputs.main.pred_boxes = torch::tensor(
        {{{0.52f, 0.48f, 0.22f, 0.18f},
          {0.18f, 0.22f, 0.12f, 0.08f},
          {0.9f, 0.9f, 0.05f, 0.05f}}},
        torch::TensorOptions().dtype(torch::kFloat32));
    return outputs;
}

fastloader::rfdetr::ModelOutputs make_mask_outputs(int64_t height, int64_t width) {
    auto outputs = make_outputs();
    outputs.aux_outputs.clear();
    outputs.enc_outputs.reset();
    outputs.main.pred_masks = torch::zeros(
        {1, 2, height, width},
        torch::TensorOptions().dtype(torch::kFloat32));
...
void test_matcher_and_losses() {
    ...
    const auto indices = fastloader::rfdetr::matcher_indices(outputs, targets, config, true);
    ...
    const auto loss_dict =
        fastloader::rfdetr::detection_loss_dict(outputs, targets, config, true, false);
    ...
    const auto weighted =
        fastloader::rfdetr::weighted_detection_loss(loss_dict, config, torch::Device(torch::kCPU));
...
void test_batched_matcher_transfer_matches_single_layer_losses() {
    ...
    fastloader::rfdetr::DetectionConfig single_config = config;
    single_config.aux_loss = false;
    single_config.two_stage = false;
    fastloader::rfdetr::populate_default_detection_weight_dict(single_config);

    auto main_only = outputs;
    main_only.aux_outputs.clear();
    main_only.enc_outputs.reset();
    const auto main_loss =
        fastloader::rfdetr::detection_loss_dict(main_only, targets, single_config, true, false);
    ...
    for (size_t aux_index = 0; aux_index < outputs.aux_outputs.size(); ++aux_index) {
        fastloader::rfdetr::ModelOutputs aux_only;
        aux_only.main = outputs.aux_outputs[aux_index];
        const auto aux_loss =
            fastloader::rfdetr::detection_loss_dict(aux_only, targets, single_config, true, false);
    ...
    fastloader::rfdetr::ModelOutputs enc_only;
    assert(outputs.enc_outputs.has_value());
    enc_only.main = outputs.enc_outputs.value();
    const auto enc_loss =
        fastloader::rfdetr::detection_loss_dict(enc_only, targets, single_config, true, false);
...
void test_multi_match_box_losses() {
    ...
    for (int64_t match = 0; match < row_index.size(0); ++match) {
        const int64_t row = row_index[match].item<int64_t>();
        const int64_t col = col_index[match].item<int64_t>();
    ...
    const float actual_bbox = loss_dict.at("loss_bbox").item<float>();
    const float actual_giou = loss_dict.at("loss_giou").item<float>();
...
void test_sparse_mask_loss_matches_dense_reference() {
    ...
    auto spatial_features = torch::linspace(
        0.05f,
        1.28f,
        kChannels * kHeight * kWidth,
        torch::TensorOptions().dtype(torch::kFloat32))
                                .view({1, kChannels, kHeight, kWidth});
    auto query_features = torch::tensor(
        {{{0.35f, -0.15f}, {-0.2f, 0.4f}}},
        torch::TensorOptions().dtype(torch::kFloat32));
    ...
    fastloader::rfdetr::ModelOutputs dense_outputs;
    dense_outputs.main.pred_logits = torch::tensor(
        {{{-5.0f, 8.0f, -5.0f}, {6.0f, -5.0f, -5.0f}}},
        torch::TensorOptions().dtype(torch::kFloat32));
    dense_outputs.main.pred_boxes = torch::tensor(
        {{{0.5f, 0.5f, 0.2f, 0.2f}, {0.1f, 0.1f, 0.1f, 0.1f}}},
        torch::TensorOptions().dtype(torch::kFloat32));
    dense_outputs.main.pred_masks = dense_masks;

    fastloader::rfdetr::ModelOutputs sparse_outputs;
    ...
    sparse_outputs.main.sparse_pred_masks = fastloader::rfdetr::OutputLayer::SparsePredMasks{
        spatial_features,
        query_features,
        bias,
    };
    ...
    const auto dense_loss =
        fastloader::rfdetr::detection_loss_dict(dense_outputs, targets, config, true, false);
    torch::manual_seed(1234);
    const auto sparse_loss =
        fastloader::rfdetr::detection_loss_dict(sparse_outputs, targets, config, true, false);

    assert(torch::allclose(
        dense_loss.at("loss_mask_ce"),
        sparse_loss.at("loss_mask_ce"),
        1.0e-5,
        1.0e-5));
    assert(torch::allclose(
        dense_loss.at("loss_mask_dice"),
        sparse_loss.at("loss_mask_dice"),
        1.0e-5,
        1.0e-5));
}
...
void test_matcher_mask_cost_handles_zero_point_sampling_on_cuda() {
    ...
    auto targets = make_targets();
    targets.packed_masks = pack_dense_masks(
        torch::ones({1, 1, 1}, torch::TensorOptions().dtype(torch::kFloat32)));
    const auto& packed_masks = targets.packed_masks.value();
    targets.packed_masks = fastloader::rfdetr::PackedTargetMasks{
    ...
    assert(outputs.main.pred_masks.has_value());
    outputs.main.pred_masks = outputs.main.pred_masks.value().to(torch::kCUDA);
...
void test_packed_mask_sampling_matches_grid_sample_nearest_boundaries() {
    ...
    const auto point_coords = torch::tensor(
        {{{0.0f, 0.5f},
          {0.125f, 0.5f},
          {0.25f, 0.5f},
          {0.375f, 0.5f},
    ...
    const auto sampled = fastloader::rfdetr::sample_packed_masks_cuda(
        packed_bits,
        packed.height,
        packed.width,
        mask_indices,
    ...
    const auto expected = F::grid_sample(
                              dense_cuda,
                              2.0f * point_coords.unsqueeze(2) - 1.0f,
...
void test_postprocess() {
    ...
    const auto results = fastloader::rfdetr::postprocess_outputs(outputs, target_sizes, 2);
...
int main() {
    test_weight_dict_population();
    test_matcher_and_losses();
    test_batched_matcher_transfer_matches_single_layer_losses();
    test_multi_match_box_losses();
    test_matcher_sanitizes_nonfinite_costs();
    test_cpu_mask_loss_uses_upstream_keys();
    test_sparse_mask_loss_matches_dense_reference();
