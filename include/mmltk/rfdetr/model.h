...
inline NestedTensor nested_tensor_from_tensor_list(const std::vector<torch::Tensor>& tensor_list) {
    ...
    for (size_t index = 1; index < tensor_list.size(); ++index) {
        ...
        for (int64_t dim = 0; dim < tensor.dim(); ++dim) {
            max_size[static_cast<size_t>(dim)] = std::max(max_size[static_cast<size_t>(dim)], tensor.size(dim));
        }
    }

    const auto batch = static_cast<int64_t>(tensor_list.size());
    const auto options = tensor_list.front().options();
    auto padded = torch::zeros({batch, max_size[0], max_size[1], max_size[2]}, options);
    auto mask = torch::ones({batch, max_size[1], max_size[2]},
...
class NativeRfDetrModel : public torch::nn::Module {
   public:
    ...
    bool is_compiled(bool for_training) const {
        return for_training ? is_compiled_train_ : is_compiled_eval_;
    }

    void export_onnx(const std::filesystem::path& output_path, int opset_version = 19, int batch_size = 1,
                     bool simplify = false);
    void set_force_pytorch_deformable_attn(bool value);

    const NativeRfDetrConfig& config() const {
        return config_;
    }
    const ResolvedModelArtifacts& artifacts() const {
        return artifacts_;
    }
...
void export_weights_to_onnx(const ModelArtifactRequest& input, const std::filesystem::path& output_path,
                            int device_id = 0, int opset_version = 19, bool simplify = false);
