...
MMLTK_DRAW_CUDA_DECLARE_LAUNCHER(launch_draw_manual_mask_runs_rgba_pitched,
                                 draw_launch::ManualMaskRunsRgbaPitchedLaunch)

inline void launch_draw_manual_mask_runs_rgba_pitched(std::uint8_t* overlay_region, std::size_t pitch_bytes, int width,
                                                      int height, const std::uint32_t* run_pairs,
                                                      std::uint32_t run_count, std::uint8_t r, std::uint8_t g,
                                                      std::uint8_t b, std::uint8_t alpha, cudaStream_t stream) {
    launch_draw_manual_mask_runs_rgba_pitched(draw_launch::ManualMaskRunsRgbaPitchedLaunch{
        draw_launch::make_pitched_surface(overlay_region, pitch_bytes, width, height), run_pairs, run_count,
        draw_launch::RgbaColorU8{r, g, b, alpha}, stream});
}
...
inline void launch_draw_selection_handles_rgba_pitched(std::uint8_t* overlay_out, std::size_t pitch_bytes,
                                                       ...
                                                       int box_x2, int box_y2, int handle_radius, std::uint8_t r,
                                                       std::uint8_t g, std::uint8_t b, std::uint8_t alpha,
                                                       cudaStream_t stream) {
    launch_draw_selection_handles_rgba_pitched(draw_launch::SelectionHandlesRgbaPitchedLaunch{
        draw_launch::make_pitched_surface(overlay_out, pitch_bytes, image_width, image_height),
        draw_launch::IntRect{box_x1, box_y1, box_x2, box_y2}, handle_radius, draw_launch::RgbaColorU8{r, g, b, alpha},
        stream});
}
