...
// Draws an analysis overlay into a pitched RGBA image.
// The output surface is expected to be cleared to transparent before launch.
void launch_draw_analysis_overlay_rgba_pitched(
    ...
    const int* labels,
    int num_instances,
    std::uint8_t mask_alpha,
    int box_thickness,
    cudaStream_t stream
);

// Blends a pitched RGBA overlay over a pitched BGR image in place.
...
// Draws a single manual mask into a pitched RGBA surface.
...
// Draws a rectangular outline into a pitched RGBA surface.
...
// Draws selection handles into a pitched RGBA surface.
...
// Draws a polyline into a pitched RGBA surface. `points_xy` is packed as
...
// Draws point markers into a pitched RGBA surface. `points_xy` is packed as
// [x0, y0, x1, y1, ...] in image-space pixels.
...
// Draws skeleton edges into a pitched RGBA surface. `points_xy` is packed as
// [x0, y0, x1, y1, ...] and `edge_indices` is packed as
// [source_index0, target_index0, source_index1, target_index1, ...].
