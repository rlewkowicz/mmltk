...
enum class LivePipelineStage : std::uint8_t {
    ...
    BrowserRetainSurface,
    CefExportSharedImage,
    CefPresentFrontSlot,
    RendererReceiveSurface,
    RendererImportTexture,
    WebGpuDrawSurface,
    RendererReleaseSurface,
    BrowserReleaseSurface,
};

inline constexpr std::size_t kLivePipelineStageCount = 15U;
...
struct LivePipelineTraceSnapshot {
    ...
    std::uint64_t diagnostic_frame_budget_us = 0;
    std::uint32_t diagnostic_frame_limit = 0;
    std::uint64_t diagnostic_drawn_frames = 0;
    std::uint64_t diagnostic_consecutive_misses = 0;
    bool diagnostic_completed = false;
    LivePipelineTraceEvent diagnostic_failure_event{};
};
...
class LivePipelineTrace {
   public:
    ...
    void reset() noexcept;
    void configure_single_buffer_diagnostic(std::uint64_t frame_budget_ns, std::uint32_t frame_limit,
                                            std::uint32_t consecutive_miss_limit) noexcept;
   ...
   private:
    ...
    std::atomic<bool> diagnostic_failed_{false};
    std::atomic<std::uint64_t> diagnostic_frame_budget_us_{0};
    std::atomic<std::uint32_t> diagnostic_frame_limit_{0};
    std::atomic<std::uint64_t> diagnostic_drawn_frames_{0};
    std::atomic<std::uint32_t> diagnostic_consecutive_miss_limit_{1};
    std::array<std::atomic<std::uint64_t>, kLivePipelineStageCount> diagnostic_stage_consecutive_misses_{};
    std::atomic<bool> diagnostic_completed_{false};
    std::atomic<std::uint32_t> diagnostic_failure_stage_index_{0};
    std::atomic<std::uint64_t> diagnostic_failure_session_nonce_{0};
    std::atomic<std::uint64_t> diagnostic_failure_sequence_{0};
    std::atomic<std::uint64_t> diagnostic_failure_event_count_{0};
    std::atomic<std::uint64_t> diagnostic_failure_timestamp_ns_{0};
    std::atomic<std::uint64_t> diagnostic_failure_latency_us_{0};
    std::atomic<std::uint32_t> diagnostic_failure_cuda_device_index_{kLivePipelineUnknownCudaDevice};
    std::atomic<std::uint64_t> diagnostic_failure_surface_revision_{0};
};
