...
std::vector<std::pair<std::string, Metric>> sorted_metric_items(const std::unordered_map<std::string, Metric>& metrics) {
    ...
    for (const auto& entry : metrics) {
        if (has_metric_data(entry.second)) { items.emplace_back(entry.first, entry.second); }
    }
...
void write_aggregate_block(FILE* out, const std::string& run_label, const std::vector<RunSnapshot>& runs, std::uint64_t process_total_ns) {
    ...
    for (const auto& entry : items) {
        ...
        if (metric.has_duration) {
            const double total_ms_avg_per_run = static_cast<double>(metric.total_ns_sum) / (1.0e6 * static_cast<double>(metric.runs));
            const double avg_ms_per_call = static_cast<double>(metric.total_ns_sum) / (1.0e6 * static_cast<double>(metric.calls_total));
...
void profile_set_process_label(const char*) {}
void profile_set_run_label(const char*) {}
void profile_reset_iteration() {}
