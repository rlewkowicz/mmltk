...
[[nodiscard]] consteval bool claim_enum_mapping_member(std::array<bool, SourceSize>& sources,
                                                       std::array<bool, DestinationSize>& destinations, const std::size_t source_index,
                                                       const std::size_t destination_index) noexcept {
    if (source_index >= SourceSize || destination_index >= DestinationSize || sources[source_index] || destinations[destination_index]) {
        return false;
    }
...
[[nodiscard]] consteval bool complete_counted_enum_mapping(
    const std::array<std::pair<Source, Destination>, Size>& mapping,
