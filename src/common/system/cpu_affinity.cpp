...
std::vector<int> allowed_cpu_set() {
 for (std::size_t count = configured_cpu_count();;) {
  ...
  if (::sched_getaffinity(0, mask.bytes(), mask.get()) != 0) {
   if (errno == EINVAL && count <= std::numeric_limits<std::size_t>::max() / 2U) {
    count *= 2U;
    continue;
   }
  ...
  for (size_t cpu = 0; cpu < mask.cpu_count(); ++cpu)
   if (CPU_ISSET_S(cpu, mask.bytes(), mask.get())) cpus.push_back(static_cast<int>(cpu));
  if (cpus.empty()) throw std::runtime_error("current affinity mask does not allow any CPUs");
  return cpus;
 }
}
std::vector<int> parse_cpu_list(const std::string& spec) {
 ...
 while (cursor <= spec_view.size()) {
  ...
  if (next == std::string::npos) { next = spec_view.size(); }
  ...
  if (!token.empty()) {
   ...
   if (dash == std::string::npos) {
    ...
    if (begin > end) { throw std::runtime_error("cpu affinity range start exceeds end: " + std::string(token)); }
    for (int cpu = begin;; ++cpu) {
     cpus.push_back(cpu);
     if (cpu == end) break;
    }
   }
