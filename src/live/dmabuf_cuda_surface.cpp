...
constexpr const char* kImplicitTextureProfileLabel = "implicit-texture";
#else
...
constexpr const char* kImplicitTextureProfileLabel = "implicit-rendering";
#endif
...
[[nodiscard]] std::string modifier_mode_name(const DmaBufModifierMode mode) noexcept {
    switch (mode) {
        case DmaBufModifierMode::Implicit:
            return "implicit";
        case DmaBufModifierMode::Explicit:
            return "explicit";
        case DmaBufModifierMode::Unknown:
            return "unknown";
    }
    return "unknown";
}
...
void log_dmabuf_capabilities(const LinuxGpuInteropDevice& device) {
    mmltk::logging::logger("live.dmabuf")
        ->info(
            ...
            "modifier_query={} modifier_count={} modifiers={} implicit_modifier_attrs={}",
            ...
            device.omit_modifier_attrs_for_implicit_imports() ? "omitted" : "included");
}
...
[[nodiscard]] std::string allocation_probe_label(const LinuxGpuInteropDevice& device,
    ...
    message << context << ": cuda_device=" << device.cuda_device_index()
            ...
            << " modifier=" << hex_u64(profile.modifier)
            << " modifier_mode=" << modifier_mode_name(profile.modifier_mode)
            << " usage_flags=" << hex_u32(profile.usage_flags);
    return message.str();
}
...
[[nodiscard]] std::vector<GpuInteropAllocationProfile> build_allocation_profiles(
    const LinuxGpuInteropDevice& interop_device) {
    std::vector<GpuInteropAllocationProfile> profiles;
    profiles.reserve(interop_device.dma_buf_modifiers().size() * 2U + 2U);
    for (const std::uint64_t modifier : interop_device.dma_buf_modifiers()) {
        profiles.push_back(GpuInteropAllocationProfile{
            GpuInteropAllocationProfile::Mode::ExplicitModifier,
            kGbmTextureUsage,
            modifier,
            DmaBufModifierMode::Explicit,
    ...
    profiles.push_back(GpuInteropAllocationProfile{
        GpuInteropAllocationProfile::Mode::ImplicitModifier,
        0U,
        DRM_FORMAT_MOD_INVALID,
        DmaBufModifierMode::Implicit,
        "implicit-zero-usage",
    });
    profiles.push_back(GpuInteropAllocationProfile{
        GpuInteropAllocationProfile::Mode::ImplicitModifier,
...
[[nodiscard]] gbm_bo* allocate_gbm_bo(gbm_device* device, const GpuInteropAllocationProfile& profile,
    ...
    errno = 0;
    if (profile.mode == GpuInteropAllocationProfile::Mode::ExplicitModifier) {
    ...
    return gbm_bo_create(device, width, height, kDmaBufRgbaFormat, profile.usage_flags);
}
...
[[nodiscard]] EGLImageKHR create_dma_buf_egl_image(const LinuxGpuInteropDevice& interop_device,
    ...
    if (profile.modifier_mode == DmaBufModifierMode::Explicit) {
...
LinuxGpuInteropDevice::LinuxGpuInteropDevice(LinuxGpuInteropDevice&& other) noexcept
    : fd_(other.fd_),
      ...
      omit_modifier_attrs_for_implicit_imports_(other.omit_modifier_attrs_for_implicit_imports_),
...
void LinuxGpuInteropDevice::clear_after_move() noexcept {
    ...
    omit_modifier_attrs_for_implicit_imports_ = true;
...
DmaBufCudaRgbaSurface::DmaBufCudaRgbaSurface(DmaBufCudaRgbaSurface&& other) noexcept
    : bo_(other.bo_),
      ...
      modifier_mode_(other.modifier_mode_),
...
void DmaBufCudaRgbaSurface::clear_after_move() noexcept {
    ...
    drm_format_ = 0;
    drm_modifier_ = 0;
    modifier_mode_ = DmaBufModifierMode::Unknown;
...
void DmaBufCudaRgbaSurface::ensure_dimensions(LinuxGpuInteropDevice& interop_device, const int cuda_device_index,
    ...
    if (cuda_resource_ != nullptr && width_ >= width && height_ >= height) {
        return;
    }
    ...
    if (interop_device.cached_allocation_profile().has_value()) {
        DmaBufCudaRgbaSurface next;
        next.initialize(interop_device, *interop_device.cached_allocation_profile(), width, height, context);
        *this = std::move(next);
        return;
    }

    std::string probe_errors;
    for (const GpuInteropAllocationProfile& profile : build_allocation_profiles(interop_device)) {
        DmaBufCudaRgbaSurface next;
        try {
...
void DmaBufCudaRgbaSurface::initialize(LinuxGpuInteropDevice& interop_device,
    ...
    try {
        ...
        if (profile.modifier_mode == DmaBufModifierMode::Explicit) {
            ...
            if (drm_modifier_ == DRM_FORMAT_MOD_INVALID) {
                throw std::runtime_error(allocation_probe_label(interop_device, profile, context) +
                                         " explicit-modifier allocation returned DRM_FORMAT_MOD_INVALID");
            }
            modifier_mode_ = DmaBufModifierMode::Explicit;
        } else {
            drm_modifier_ = DRM_FORMAT_MOD_INVALID;
            modifier_mode_ = DmaBufModifierMode::Implicit;
        }
...
WorkspaceDmaBufImage DmaBufCudaRgbaSurface::dmabuf_image(const std::uint32_t published_width,
    ...
    return WorkspaceDmaBufImage{
        fd_,         published_width, published_height, stride_bytes, offset_, allocation_size_,
        drm_format_, drm_modifier_,   modifier_mode_,
    };
}
