...
void log_model_info(const std::filesystem::path& model_path) {
    ...
    mmltk::common::logging::info("rfdetr.onnx_info", [&info](auto& logger) {
        logger.info(
            ...
            info.backend, info.model_path, info.input.name, rfdetr::format_shape(info.input.shape), info.input.dtype, info.outputs.size(),
            info.num_queries, info.num_classes);
        for (const auto& output : info.outputs) {
            logger.info("  output: {} {} {}", output.name, rfdetr::format_shape(output.shape), output.dtype);
        }
    });
}
