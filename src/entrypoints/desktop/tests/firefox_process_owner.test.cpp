...
class FirefoxProcessOwnerTestAccess final {
   public:
    static void PreferRetainedPidWait(mmltk::controller::services::FirefoxProcessOwner& owner) noexcept {
        owner.prefer_retained_pid_wait_for_test();
    }
};
...
void await_child_readiness(const int descriptor) {
    ...
    ssize_t received = -1;
    do {
        received = ::read(descriptor, &fact, sizeof(fact));
