...
inline constexpr std::array kCompileOptions{
    reflection::option<data::CompilerConfig, &data::CompilerConfig::source_dir>("--source-dir", "Source dataset directory", "Dataset",
                                                                                "source_dir", {}, true),
    reflection::option<data::CompilerConfig, &data::CompilerConfig::output_dir>("--output-dir", "Compiled binary output directory",
                                                                                "Dataset", "output_dir", {}, true),
    ...
    reflection::option<data::CompilerConfig, &data::CompilerConfig::cuda_mask_batch_size>("--cuda-mask-batch-size", "CUDA mask batch size",
                                                                                          "Execution"),
...
inline constexpr std::array kBenchOptions{
    reflection::negative_flag<BenchCommandRequest, &BenchCommandRequest::h2d_dataloader>("--gdrcopy", "Use GDRCopy image loading",
                                                                                         "Execution"),
    reflection::option<BenchCommandRequest, &BenchCommandRequest::numa_node>("--numa-node", "GPU-local NUMA node (-1 automatic)",
                                                                             "Execution"),
    reflection::option<BenchCommandRequest, &BenchCommandRequest::compiled_path>("--compiled", "Compiled dataset binary", "Dataset",
                                                                                 "compiled", {}, true),
    reflection::option<BenchCommandRequest, &BenchCommandRequest::batch_size>("--batch-size", "Streaming batch size", "Execution",
                                                                              "batch_size"),
    reflection::option<BenchCommandRequest, &BenchCommandRequest::epochs>("--epochs", "Number of epochs", "Execution", "num_epochs")};
inline constexpr std::array kInfoOptions{reflection::option<InfoCommandRequest, &InfoCommandRequest::compiled_path>(
    "--compiled", "Compiled dataset binary", "Dataset", "compiled", {}, true)};
...
void run_info(const std::string& compiled_path) {
    ...
    std::printf("Stride: %zu bytes/image (%.2f KB)\n", static_cast<std::size_t>(header.image_stride),
...
void run_bench(const BenchCommandRequest& request) {
    ...
    std::printf("Benchmarking: %zu images, batch=%zu, stride=%zu, epochs=%d\n", loader.num_images(), config.batch_size,
                loader.image_stride(), request.epochs);
...
void print_root_help() {
    std::fputs(
        ...
        "benchmarks\nUsage: mmltk <command>\nCommands:",
        stdout);
    for (const auto& command : kRootCommands)
