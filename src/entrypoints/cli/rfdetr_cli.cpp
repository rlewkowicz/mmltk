...
inline constexpr std::array kExportOnnxOptions{
    reflection::option<rfdetr::ExportOnnxRequest, &rfdetr::ExportOnnxRequest::class_layout_path>("--class-layout", "Digest-bound class descriptor",
                                                                                                 "Model input"),
...
inline constexpr std::array kEvaluateOptions{
    ...
    reflection::option<rfdetr::EvaluateRequest, &rfdetr::EvaluateRequest::num_queries>("--num-queries", "Native query count", "Model input"),
    ...
    while (start <= text.size()) {
        ...
        if (item.empty() || error != std::errc{} || end != item.data() + item.size() || id < (Unique ? 0 : -1) ||
            ...
            return std::unexpected(
                reflection::ParseError{reflection::ParseErrorCode::InvalidValue, text,
