...
[[nodiscard]] inline bool transition_slot_state(std::atomic<std::uint32_t>& state, const SlotState expected,
                                                const SlotState desired) noexcept {
...
template <class Slot>
[[nodiscard]] ReusableLiveSlot<Slot> reserve_live_slot(Slot* const slots, const std::uint32_t count,
                                                       const std::atomic<int>* const latest = nullptr) noexcept {
...
template <class Scrub>
void publish_latest_live_owner_slot(std::atomic<int>& latest, const std::uint32_t index, std::atomic<std::uint32_t>& state,
                                    const SlotState published, Scrub&& scrub) noexcept {
