...
struct DeviceFrameMetadata {
    ...
    [[nodiscard]] DeviceFrameView View(std::uint32_t slot, CUdeviceptr pixels, std::size_t pitch_bytes, cudaEvent_t ready,
...
struct DeviceFrameView final : DeviceFrameMetadata {
    ...
    [[nodiscard]] inline bool valid() const noexcept {
        return frame.valid() && capture_identity.valid() && pixels != 0U && pitch_bytes != 0U && width != 0U && height != 0U &&
               ready != nullptr;
    }
};
...
inline DeviceFrameMetadata DeviceFrameMetadata::From(const DeviceFrameView& view) noexcept {
    return static_cast<const DeviceFrameMetadata&>(view);
}

inline DeviceFrameView DeviceFrameMetadata::View(const std::uint32_t slot, const CUdeviceptr pixels, const std::size_t pitch_bytes,
                                                 const cudaEvent_t ready, const cudaStream_t stream) const noexcept {
