...
[[nodiscard]] std::optional<ManualOverlayDeferredMaskProjection> validate_manual_overlay_deferred_mask(
    const ManualOverlayDeferredMaskMapping& mapping, const ManualOverlayMaskRegion& region, std::span<const ManualOverlayMaskRun> runs,
...
struct ManualOverlayDocumentSnapshot {
    ...
    mmltk::backend::imaging::annotation::SemanticRenderer renderer_mode =
        mmltk::backend::imaging::annotation::SemanticRenderer::NativeImageView;

    [[nodiscard]] bool same_content(const ManualOverlayDocumentSnapshot& other) const {
        return document_generation == other.document_generation && session_revision == other.session_revision &&
               interaction_ack == other.interaction_ack && capture_width == other.capture_width && capture_height == other.capture_height &&
               instances == other.instances && interaction_instances == other.interaction_instances &&
               selected_instance == other.selected_instance && brush_preview == other.brush_preview && renderer_mode == other.renderer_mode;
    }
};
