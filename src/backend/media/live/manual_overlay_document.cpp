...
bool manual_overlay_mask_region_contained(const ManualOverlayMaskRegion& region, const std::uint32_t target_width,
                                          const std::uint32_t target_height) noexcept {
    return region.width > 0U && region.height > 0U && target_width > 0U && target_height > 0U &&
           static_cast<std::uint64_t>(region.capture_x) + region.width <= target_width &&
           static_cast<std::uint64_t>(region.capture_y) + region.height <= target_height;
}

std::optional<ManualOverlayDeferredMaskProjection> validate_manual_overlay_deferred_mask(const ManualOverlayDeferredMaskMapping& mapping,
                                                                                         const ManualOverlayMaskRegion& region,
                                                                                         const std::span<const ManualOverlayMaskRun> runs,
                                                                                         const std::uint32_t target_width,
                                                                                         const std::uint32_t target_height) noexcept {
    constexpr std::uint32_t kIntMaximum = static_cast<std::uint32_t>(std::numeric_limits<int>::max());
    if (mapping.source_width == 0U || mapping.source_height == 0U || mapping.source_crop_width == 0U || mapping.source_crop_height == 0U ||
        mapping.output_width == 0U || mapping.output_height == 0U || region.width == 0U || region.height == 0U || target_width == 0U ||
        target_height == 0U || runs.empty() || mapping.source_width > kIntMaximum || mapping.source_height > kIntMaximum ||
        mapping.source_crop_width > kIntMaximum || mapping.source_crop_height > kIntMaximum || mapping.output_width > kIntMaximum ||
        mapping.output_height > kIntMaximum || region.width > kIntMaximum || region.height > kIntMaximum ||
        runs.size() > std::numeric_limits<std::uint32_t>::max() || runs.size() > std::numeric_limits<std::size_t>::max() / 2U)
        return std::nullopt;
    const auto contained = [](const std::uint32_t origin, const std::uint32_t extent, const std::uint32_t bound) noexcept {
        return static_cast<std::uint64_t>(origin) + extent <= bound;
    };
    if (!contained(mapping.source_crop_x, mapping.source_crop_width, mapping.source_width) ||
        !contained(mapping.source_crop_y, mapping.source_crop_height, mapping.source_height) ||
        !contained(mapping.view_x, region.capture_x, mapping.output_width) ||
        !contained(mapping.view_y, region.capture_y, mapping.output_height) ||
        static_cast<std::uint64_t>(mapping.view_x) + region.capture_x + region.width > mapping.output_width ||
        static_cast<std::uint64_t>(mapping.view_y) + region.capture_y + region.height > mapping.output_height ||
        !manual_overlay_mask_region_contained(region, target_width, target_height))
        return std::nullopt;
...
void ManualOverlayDocument::publish_snapshot(ManualOverlayDocumentSnapshot snapshot) {
    publish_snapshot(std::make_shared<ManualOverlayDocumentSnapshot>(std::move(snapshot)));
}
