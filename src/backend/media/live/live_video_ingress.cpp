...
LiveVideoIngress::LiveVideoIngress(capture::CaptureConfig config, const std::uint32_t count, LivePhysicalCudaContext cuda)
    ...
    try {
        ...
        for (std::uint32_t index = 0; index < count; ++index) {
            ...
            if (scope.Record(cudaMallocPitch(reinterpret_cast<void**>(&slot.pixels), &slot.pitch_bytes, static_cast<std::size_t>(config_.width) * 3U,
                                             config_.height)) != cudaSuccess)
...
void LiveVideoIngress::settle() noexcept {
    ...
    auto scope = cuda_.scope();
    retire_live_slots(scope, slots_.get(), slot_count_, [this](DeviceSlot& slot, const SlotState published) noexcept { publish_slot(slot, published); });
}
...
bool LiveVideoIngress::ingest_next() {
    ...
    cudaError_t failure = scope.Record(cudaMemcpy2DAsync(reinterpret_cast<void*>(slot->pixels), slot->pitch_bytes, source, slot->lease.stride_bytes(),
