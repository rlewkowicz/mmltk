...
int CaptureSession::Impl::DeviceIoctl(const unsigned long request, void* const argument) noexcept {
 return config.device_api.ioctl != nullptr ? config.device_api.ioctl(config.device_api.context, fd_, request, argument) : Xioctl(fd_, request, argument);
}
...
Status CaptureSession::Impl::DestroyHostSlots() {
 ...
 for (auto& slot : host_slots_) {
  ...
  if (released.ok() && config.device_api.host_storage_released != nullptr) config.device_api.host_storage_released(config.device_api.context, slot->slot_index);
 }
 host_slots_.clear();
 return status;
}
...
Status CaptureSession::Impl::QueueV4l2Buffer(const std::uint32_t slot_index, const bool record_requeue_failure) {
 ...
 host_slots_[slot_index]->phase.store(capture_internal::CaptureSlotPhaseValue(CaptureSlotPhase::kHardwareQueued), std::memory_order_release);
 return Status::Ok();
}
}  // namespace mmltk::backend::media::capture
