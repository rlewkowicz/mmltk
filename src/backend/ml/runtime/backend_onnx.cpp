...
[[nodiscard]] RuntimeElementType runtime_element_type(const ONNXTensorElementDataType value) {
    switch (value) {
        case ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT: return RuntimeElementType::Float32;
        case ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16: return RuntimeElementType::Float16;
        case ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32: return RuntimeElementType::Int32;
        case ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64: return RuntimeElementType::Int64;
        case ONNX_TENSOR_ELEMENT_DATA_TYPE_BOOL: return RuntimeElementType::Bool;
        default: throw std::runtime_error("unsupported ONNX Runtime tensor element type");
    }
}
[[nodiscard]] ONNXTensorElementDataType onnx_element_type(const RuntimeElementType value) {
    switch (value) {
        case RuntimeElementType::Float16: return ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16;
        case RuntimeElementType::Float32: return ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT;
        case RuntimeElementType::Int32: return ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32;
        case RuntimeElementType::Int64: return ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64;
        case RuntimeElementType::Bool: return ONNX_TENSOR_ELEMENT_DATA_TYPE_BOOL;
    }
...
class OnnxRuntimeBackend final : public RuntimeBackend {
   public:
    explicit OnnxRuntimeBackend(RuntimeBackendOptions options)
        : RuntimeBackend(options.device, options.command_stream), environment_(ORT_LOGGING_LEVEL_WARNING, "mmltk_runtime"), options_(std::move(options)) {
    ...
    [[nodiscard]] RawSubmission Submit(const RuntimeTensorBuffer& input, const std::span<RuntimeTensorBuffer> outputs,
        ...
        Ort::Value input_value = Ort::Value::CreateTensor(memory, input.device_data, input.capacity_bytes, input_shape.data(), input_shape.size(),
                                                          onnx_element_type(input.element_type));
