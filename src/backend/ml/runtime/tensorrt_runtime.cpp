...
void configure_optimization_profiles(nvinfer1::IBuilder& builder, nvinfer1::INetworkDefinition& network, nvinfer1::IBuilderConfig& config,
    ...
    for (const TensorRtOptimizationProfile& definition : options.optimization_profiles) {
        ...
        if (input.isShapeTensor()) {
            throw std::invalid_argument(options.context + " does not support value profiles for shape tensor " + definition.input_name);
        }
...
[[nodiscard]] std::string parser_errors(const nvonnxparser::IParser& parser) {
    ...
    for (std::int32_t index = 0; index < parser.getNbErrors(); ++index) {
        ...
        output << '[' << index << "] code=" << static_cast<int>(error->code())
               << " op=" << (error->nodeOperator() != nullptr ? error->nodeOperator() : "<unknown>")
