...
RuntimeSubmission RuntimeBackend::Run(const RuntimeTensorBuffer& input, std::span<RuntimeTensorBuffer> outputs,
                                      const RuntimeOutputBinding output_binding, const RuntimeContinuation continuation,
    ...
    try {
        ...
        if (submitted.device != device() || submitted.output_count != outputs.size() || !submitted.stream ||
            submitted.stream != command_stream() || submitted.completion_event == 0U) {
        ...
        active_submission_.emplace(ActiveSubmission{.receipt = submitted, .retained_storage = std::move(retained_storage)});
        state_ = LaneState::Active;
        return RuntimeSubmission{std::move(owner),      generation, submitted.device, submitted.stream, submitted.completion_event,
                                 submitted.output_count};
    } catch (...) {
...
void RuntimeBackend::CudaLane::Record(const RuntimeContinuation continuation) const {
    ...
    try {
        ...
        require_cuda(cudaEventRecord(reinterpret_cast<cudaEvent_t>(completion_), reinterpret_cast<cudaStream_t>(stream_)),
                     "cudaEventRecord");
...
RuntimeStatus RuntimeBackend::CudaLane::Observe() const noexcept {
    ...
    return selected == kRuntimeSuccess ? static_cast<RuntimeStatus>(cudaEventSynchronize(reinterpret_cast<cudaEvent_t>(completion_)))
                                       : selected;
}
