...
class RuntimeBackend : public std::enable_shared_from_this<RuntimeBackend> {
   public:
    ...
    [[nodiscard]] RuntimeSubmission Run(const RuntimeTensorBuffer& input, std::span<RuntimeTensorBuffer> outputs,
   ...
   protected:
    ...
    [[nodiscard]] virtual RawSubmission Submit(const RuntimeTensorBuffer& input, std::span<RuntimeTensorBuffer> outputs,
                                               RuntimeContinuation continuation) = 0;
   ...
   private:
    ...
    static void ValidateRuntimeTensorSet(const RuntimeModelInfo& model, const RuntimeTensorBuffer& input,
                                         std::span<RuntimeTensorBuffer> outputs);
...
inline void RuntimeBackend::ValidateRuntimeTensorSet(const RuntimeModelInfo& model, const RuntimeTensorBuffer& input,
    ...
    for (std::size_t index = 0U; index < outputs.size(); ++index) {
        static_cast<void>(validate_runtime_tensor_buffer(model.outputs[index], outputs[index]));
    }
}
