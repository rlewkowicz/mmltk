...
private:
 ...
 [[nodiscard]] static bool ValidateResult(const AnalysisRequest& request, const ProviderWorkResult& result) noexcept {
  ...
  if (request.cancellation.stop_requested() || result.completed_ns == 0U || result.output_count != request.annotations.size() || !result.completion.valid() ||
      result.completion.device != request.source.device) {
   return false;
  }
  for (const AnalysisAnnotationStorage& output : request.annotations) {
