...
std::size_t validate_runtime_tensor_buffer(const RuntimeTensorDescriptor& descriptor, const RuntimeTensorBuffer& buffer,
                                           const RuntimeShape* resolved_shape) {
    if (descriptor.name.empty() || buffer.device_data == nullptr || descriptor.element_type != buffer.element_type ||
        descriptor.shape.rank > kMaximumRuntimeRank || buffer.shape.rank > kMaximumRuntimeRank ||
        descriptor.shape.rank != buffer.shape.rank) {
    ...
    for (std::size_t axis = 0U; axis < actual.rank; ++axis) {
        ...
        if (declared == 0 || declared < -1 || supplied <= 0 || resolved <= 0 || (declared > 0 && supplied != declared) ||
            (declared > 0 && resolved != declared) || (resolved_shape == nullptr && declared == -1 && supplied != resolved)) {
        ...
        if (extent > std::numeric_limits<std::size_t>::max() / elements) {
            throw std::overflow_error("runtime tensor element count overflow");
        }
        elements *= extent;
    }
