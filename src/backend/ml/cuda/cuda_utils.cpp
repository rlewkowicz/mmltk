...
const char* validate_bgr_split_to_planar_float_args(const std::size_t src_pitch_bytes, const std::uint32_t src_width,
                                                    const std::uint32_t src_height, const std::uint32_t dst_width,
                                                    const std::uint32_t dst_height, const std::uintptr_t stream) {
...
std::int32_t launch_bgr_split_to_planar_float(const std::uint8_t* src, const std::size_t src_pitch_bytes, const std::uint32_t src_width,
                                              const std::uint32_t src_height, float* dst, const std::uint32_t dst_width,
                                              const std::uint32_t dst_height, const std::uintptr_t stream) {
    return static_cast<std::int32_t>(detail::launch_bgr_split_to_planar_float(src, src_pitch_bytes, src_width, src_height, dst, dst_width,
                                                                              dst_height, reinterpret_cast<cudaStream_t>(stream)));
}
