...
std::size_t tensor_bytes(const at::Tensor& tensor) {
    if (!tensor.defined() || tensor.layout() != at::kStrided || tensor.is_quantized() ||
        (!tensor.is_cpu() && !tensor.is_cuda())) throw std::invalid_argument("unsupported tensor readback storage");
    ...
    if (item && elements > std::numeric_limits<std::size_t>::max() / item)
        throw std::overflow_error("tensor readback byte extent overflows");
...
class LocalMemory final {
   public:
    ...
    ~LocalMemory() noexcept { try { sys::restore_memory_policy(previous_); } catch (...) { std::terminate(); } }
   private:
    sys::MemoryPolicy previous_;
};
struct SourceDevice final {
    explicit SourceDevice(c10::DeviceIndex index)
        : device(index), stream(c10::cuda::getCurrentCUDAStream(index)), producer(stream) {
        c10::cuda::CUDAGuard guard(index);
        CUcontext current{};
        if (cuCtxGetCurrent(&current) != CUDA_SUCCESS ||
            cuDevicePrimaryCtxRetain(&context, index) != CUDA_SUCCESS) throw std::runtime_error("retain tensor readback context");
        if (current != context) {
        ...
        try {
            numa_node = gpu::resolve_device_execution(index, sys::NumaTopology::Capture(),
                sys::bound_memory_node(sys::capture_memory_policy())).placement.numa_node;
...
struct TensorReadbackBuffers::Impl final {
    ...
    SourceDevice& device(c10::DeviceIndex index) {
        for (auto& value : storage->devices) if (value->device == index) return *value;
    ...
    void complete() {
        ...
        for (auto& device : storage->devices) {
            ...
            try {
                ...
                if (status == cudaSuccess) { device->pending = false; device->producer_dependency = false; }
                else if (failure == cudaSuccess) failure = status;
            } catch (...) {
    ...
    void release_capacity() {
        release();
        for (auto& slot : storage->slots) {
            if (slot.host && slot.host->ReleaseSettled() != CUDA_SUCCESS)
                throw std::runtime_error("release settled tensor readback pages");
            slot.host.reset();
        }
        storage->slots.clear();
...
void TensorReadbackBuffers::Reserve(std::span<const at::Tensor> sources, std::size_t first_slot) {
    ...
    for (std::size_t index = 0; index < sources.size(); ++index) {
        ...
        if (sources[index].is_cuda() && bytes) {
            const auto capacity = first_slot + index < slots.size() && slots[first_slot + index].host ?
                slots[first_slot + index].host->capacity_bytes() : 0;
    ...
    for (auto& device : impl_->storage->devices) { device->requested_slots.clear(); device->requested_scratch = 0; }
    for (std::size_t index = 0; index < sources.size(); ++index) {
        ...
        if (!slot.source.is_contiguous() || slot.source.is_conj() || slot.source.is_neg())
            device.requested_scratch = std::max(device.requested_scratch, slot.bytes);
    }
    ...
    for (auto& retained : impl_->storage->devices) {
        ...
        if (device.requested_scratch > device.scratch_bytes) {
            ...
            device.scratch = at::empty({static_cast<std::int64_t>(device.requested_scratch)},
                at::TensorOptions().device(at::Device(at::kCUDA, device.device)).dtype(at::kByte));
...
at::Tensor TensorReadbackBuffers::Stage(std::size_t index) {
    ...
    if (!slot.source.is_cuda()) {
        ...
        try {
            ...
            check(cudaMemcpyAsync(slot.view.mutable_data_ptr(), source, slot.bytes, cudaMemcpyDeviceToHost, device.stream.stream()),
                  "submit tensor readback");
        } catch (...) {
            const auto failure = std::current_exception();
            try { impl_->complete(); } catch (...) {}
...
std::size_t TensorReadbackBuffers::capacity_bytes() const noexcept {
    ...
    std::size_t total = 0;
    for (const auto& slot : impl_->storage->slots) if (slot.host) total += slot.host->capacity_bytes();
    return total;
}
}  // namespace mmltk::backend::ml::cuda
