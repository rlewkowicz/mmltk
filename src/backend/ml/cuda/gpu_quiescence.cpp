...
GpuBackendGeneration next_gpu_backend_generation() noexcept {
 ...
 while (candidate != 0U && candidate != std::numeric_limits<std::uint64_t>::max()) {
  if (next_generation.compare_exchange_weak(candidate, candidate + 1U, std::memory_order_relaxed, std::memory_order_relaxed)) return GpuBackendGeneration{candidate};
 }
 return {};
}
}  // namespace mmltk::backend::ml::cuda
