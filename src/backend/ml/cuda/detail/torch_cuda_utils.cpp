...
TorchDeviceIndex checked_device_index(const int device_id) {
 if (device_id < std::numeric_limits<TorchDeviceIndex>::min() || device_id > std::numeric_limits<TorchDeviceIndex>::max()) { throw std::runtime_error("device id exceeds c10::DeviceIndex range"); }
...
TorchCudaStream get_priority_cuda_stream(const int device_id, const int priority) { return get_priority_cuda_stream(checked_device_index(device_id), priority); }
