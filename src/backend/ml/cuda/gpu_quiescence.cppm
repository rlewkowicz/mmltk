...
struct GpuBackendQuiescenceRequirement {
    ...
    [[nodiscard]] bool valid() const noexcept {
        ...
        switch (strategy) {
            case GpuBackendQuiescenceStrategy::TransitiveFence:
                return user_compute_stream_owned && provider_copy_streams_ordered && auxiliary_streams_ordered;
            case GpuBackendQuiescenceStrategy::TargetDeviceBarrier:
                return true;
            case GpuBackendQuiescenceStrategy::Count:
                return false;
        }
        return false;
    }
