...
ValidatedDeformAttn validate_deform_attn_inputs(const torch::Tensor& value, const torch::Tensor& spatial_shapes,
    ...
    TORCH_CHECK(sampling_loc.dim() == 6 && sampling_loc.size(5) == 2,
                "sampling_loc must have shape [batch, queries, heads, levels, points, 2]");
    ...
    TORCH_CHECK(value.device() == sampling_loc.device() && value.device() == attn_weight.device(),
                "deformable attention tensors must share one CUDA device");
    ...
    TORCH_CHECK(sampling_loc.size(0) == batch && attn_weight.size(0) == batch && sampling_loc.size(1) == attn_weight.size(1) &&
                    sampling_loc.size(2) == attn_weight.size(2) && sampling_loc.size(3) == attn_weight.size(3) &&
                    sampling_loc.size(4) == attn_weight.size(4),
                "sampling locations and attention weights must have compatible shapes");
    TORCH_CHECK(sampling_loc.size(2) == num_heads && sampling_loc.size(3) == num_levels,
                "sampling locations must match value heads and spatial levels");
...
[[nodiscard]] ms_deform_attn_launch::CommonLaunch make_tensor_launch_common(const torch::Tensor& value,
                                                                            const ms_deform_attn_launch::DeviceLayout& layout,
                                                                            const torch::Tensor& sampling_loc,
                                                                            const torch::Tensor& attn_weight, const DeformAttnDims& dims,
                                                                            cudaStream_t stream) {
    return ms_deform_attn_launch::make_common_launch(
        value.data_ptr<float>(), layout, sampling_loc.data_ptr<float>(), attn_weight.data_ptr<float>(), dims.batch, dims.spatial_size,
        dims.num_heads, dims.channels, dims.num_levels, dims.num_query, dims.num_point, dims.im2col_step, stream);
}  // namespace
...
torch::Tensor ms_deform_attn_cuda_forward(const torch::Tensor& value, const torch::Tensor& spatial_shapes,
    ...
    const auto validated = validate_deform_attn_inputs(value, spatial_shapes, level_start_index, sampling_loc, attn_weight, im2col_step,
...
std::vector<torch::Tensor> ms_deform_attn_cuda_backward(const torch::Tensor& value, const torch::Tensor& spatial_shapes,
                                                        const torch::Tensor& level_start_index, const torch::Tensor& sampling_loc,
                                                        const torch::Tensor& attn_weight, const torch::Tensor& grad_output,
                                                        int64_t im2col_step) {
    const auto validated = validate_deform_attn_inputs(value, spatial_shapes, level_start_index, sampling_loc, attn_weight, im2col_step,
