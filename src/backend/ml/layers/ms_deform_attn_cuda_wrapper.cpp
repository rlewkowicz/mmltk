...
ValidatedDeformAttn validate_deform_attn_inputs(const torch::Tensor& value, const torch::Tensor& spatial_shapes, const torch::Tensor& level_start_index, const torch::Tensor& sampling_loc,
 ...
 TORCH_CHECK(spatial_size > 0 && num_heads > 0 && channels > 0 && num_levels > 0 && num_query > 0 && num_point > 0, "deformable attention dimensions must be positive");
 TORCH_CHECK(spatial_shapes.size(0) == level_start_index.size(0), "spatial shape and level-start counts must agree");
 TORCH_CHECK(sampling_loc.size(0) == batch && attn_weight.size(0) == batch && sampling_loc.size(1) == attn_weight.size(1) && sampling_loc.size(2) == attn_weight.size(2) &&
              sampling_loc.size(3) == attn_weight.size(3) && sampling_loc.size(4) == attn_weight.size(4),
 ...
 for (int level = 0; level < num_levels; ++level) {
  ...
  TORCH_CHECK(static_cast<std::int64_t>(height) <= (std::numeric_limits<int>::max() - expected_start) / width, "deformable attention spatial layout exceeds CUDA integer range");
 ...
 TORCH_CHECK(product_fits({spatial_size, num_heads, channels}, std::numeric_limits<int>::max()), "deformable attention value stride exceeds CUDA integer range");
 TORCH_CHECK(product_fits({num_query, num_heads, num_levels, num_point, 2}, std::numeric_limits<int>::max()), "deformable attention sampling stride exceeds CUDA integer range");
 TORCH_CHECK(product_fits({num_query, num_heads, channels}, std::numeric_limits<int>::max()), "deformable attention head/channel stride exceeds CUDA integer range");
...
torch::Tensor ms_deform_attn_cuda_forward(
 const torch::Tensor& value, const torch::Tensor& spatial_shapes, const torch::Tensor& level_start_index, const torch::Tensor& sampling_loc, const torch::Tensor& attn_weight, int64_t im2col_step) {
...
std::vector<torch::Tensor> ms_deform_attn_cuda_backward(const torch::Tensor& value, const torch::Tensor& spatial_shapes, const torch::Tensor& level_start_index, const torch::Tensor& sampling_loc,
 const torch::Tensor& attn_weight, const torch::Tensor& grad_output, int64_t im2col_step) {
 const auto validated = validate_deform_attn_inputs(value, spatial_shapes, level_start_index, sampling_loc, attn_weight, im2col_step, "ms_deform_attn_cuda_backward");
 const auto& dims = validated.dims;
 ...
 TORCH_CHECK(grad_output.dim() == 3 && grad_output.size(0) == dims.batch && grad_output.size(1) == dims.num_query && grad_output.size(2) == static_cast<int64_t>(dims.num_heads) * dims.channels,
