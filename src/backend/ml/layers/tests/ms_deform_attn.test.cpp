...
void test_cuda_ms_deform_attn_matches_reference() {
    ...
    auto sampling_locations =
        torch::rand({2, 3, 2, 2, 4, 2}, torch::TensorOptions().dtype(torch::kFloat32).device(device)).requires_grad_(true);
    ...
    const auto actual = mmltk::backend::ml::layers::ms_deform_attn_cuda_autograd(value, spatial_shapes, level_start_index,
