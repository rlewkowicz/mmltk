...
[[nodiscard]] ms_deform_attn_launch::LaunchPlan make_plan_for_launch(const ms_deform_attn_launch::CommonLaunch& launch) {
 return ms_deform_attn_launch::make_launch_plan(launch.batch, launch.spatial_size, launch.num_heads, launch.channels, launch.num_levels, launch.num_query, launch.num_point, launch.im2col_step);
}
...
void launch_ms_deform_attn_cuda_forward(const ms_deform_attn_launch::ForwardLaunch& launch) {
 ...
 for (int chunk = 0; chunk < plan.chunk_count; ++chunk) {
  ...
  ms_deformable_im2col_cuda(launch.stream, args.value, args.layout, args.sampling_loc, args.attn_weight, plan.batch_n, plan.spatial_size, plan.num_heads, plan.channels, plan.num_levels,
   plan.num_query, plan.num_point, args.output);
...
void launch_ms_deform_attn_cuda_backward(const ms_deform_attn_launch::BackwardLaunch& launch) {
 ...
 for (int chunk = 0; chunk < plan.chunk_count; ++chunk) {
  ...
  ms_deformable_col2im_cuda(launch.stream, args.grad_output, args.value, args.layout, args.sampling_loc, args.attn_weight, plan.batch_n, plan.spatial_size, plan.num_heads, plan.channels,
   plan.num_levels, plan.num_query, plan.num_point, args.grad_value, args.grad_sampling_loc, args.grad_attn_weight);
