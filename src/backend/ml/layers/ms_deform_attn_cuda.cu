...
void launch_ms_deform_attn_cuda_backward(const ms_deform_attn_launch::BackwardLaunch& launch) {
 ...
 for (int chunk = 0; chunk < plan.chunk_count; ++chunk) {
  ...
  ms_deformable_col2im_cuda(launch.stream, args.grad_output, args.value, args.layout, args.sampling_loc, args.attn_weight, plan.batch_n, plan.spatial_size,
                            plan.num_heads, plan.channels, plan.num_levels, plan.num_query, plan.num_point, args.grad_value, args.grad_sampling_loc,
