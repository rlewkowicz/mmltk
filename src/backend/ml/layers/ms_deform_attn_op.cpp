...
torch::Tensor ms_deform_attn_reference(const torch::Tensor& value, const torch::Tensor& spatial_shapes, const torch::Tensor& sampling_locations,
 ...
 for (int64_t level = 0; level < num_levels; ++level) {
  ...
  sampled_values.push_back(torch::nn::functional::grid_sample(
   value_level, sampling_grid_level, torch::nn::functional::GridSampleFuncOptions().mode(torch::kBilinear).padding_mode(torch::kZeros).align_corners(false)));
  start += height * width;
 }
