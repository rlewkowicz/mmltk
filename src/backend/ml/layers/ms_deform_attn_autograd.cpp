...
class MsDeformAttnAutograd : public torch::autograd::Function<MsDeformAttnAutograd> {
   public:
    static torch::autograd::variable_list forward(torch::autograd::AutogradContext* ctx, const torch::Tensor& value,
                                                  const torch::Tensor& spatial_shapes, const torch::Tensor& level_start_index,
        ...
        TORCH_CHECK(spatial_shapes.device().is_cpu() && level_start_index.device().is_cpu(),
                    "deformable attention layout metadata must remain on CPU");
        ...
        auto output = ms_deform_attn_cuda_forward(value_fp32, spatial_shapes, level_start_index, sampling_locations_fp32,
                                                  attention_weights_fp32, im2col_step);
...
void ms_deform_attn_cuda_autograd_abi(const void* const value, const void* const spatial_shapes, const void* const level_start_index,
                                      const void* const sampling_locations, const void* const attention_weights,
                                      const std::int64_t im2col_step, void* const output) {
    auto outputs = MsDeformAttnAutograd::apply(
        *static_cast<const torch::Tensor*>(value), *static_cast<const torch::Tensor*>(spatial_shapes),
