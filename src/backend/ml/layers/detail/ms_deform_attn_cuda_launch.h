...
[[nodiscard]] inline LaunchPlan make_launch_plan(const int batch, const int spatial_size, const int num_heads, const int channels,
                                                 const int num_levels, const int num_query, const int num_point, const int im2col_step) {
    return LaunchPlan{
        batch,
        im2col_step,
        batch / im2col_step,
...
[[nodiscard]] inline std::int64_t batch_offset(const LaunchPlan& plan, const int batch_index, const std::int64_t stride) {
...
[[nodiscard]] inline CommonLaunch make_common_launch(const float* value, const DeviceLayout& layout, const float* sampling_loc,
                                                     const float* attn_weight, const int batch, const int spatial_size, const int num_heads,
                                                     const int channels, const int num_levels, const int num_query, const int num_point,
                                                     const int im2col_step, cudaStream_t stream) {
    return CommonLaunch{
        value,    layout,     sampling_loc, attn_weight, batch,       spatial_size, num_heads,
        channels, num_levels, num_query,    num_point,   im2col_step, stream,
    };
}
...
[[nodiscard]] inline BackwardBatchArgs make_backward_batch_args(const BackwardLaunch& launch, const LaunchPlan& plan,
