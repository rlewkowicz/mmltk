...
[[nodiscard]] bool rebuild_explore_order(std::span<const ExploreImageSummary> summaries, const ExploreSampleFilter& filter, bool shuffled,
                                         std::uint64_t shuffle_seed, std::vector<std::uint32_t>& current,
                                         std::vector<std::uint32_t>& scratch, const std::atomic<bool>* cancel_requested = nullptr,
                                         std::uint64_t expected_generation = 0U,
                                         const std::atomic<std::uint64_t>* current_generation = nullptr,
                                         mmltk::common::concurrency::WorkerPool* workers = nullptr);

[[nodiscard]] std::optional<std::uint32_t> adjacent_explore_index(std::span<const std::uint32_t> order, std::uint32_t selected,
                                                                  bool next) noexcept;

[[nodiscard]] std::optional<ExploreAtlasLayout> make_explore_atlas_layout(std::size_t item_count, ExploreViewport viewport,
                                                                          std::uint32_t card_extent) noexcept;
...
[[nodiscard]] std::size_t prioritize_explore_work(std::span<const std::uint32_t> order, ExploreViewport viewport,
                                                  std::optional<std::uint32_t> focused_index, std::uint32_t background_cursor,
