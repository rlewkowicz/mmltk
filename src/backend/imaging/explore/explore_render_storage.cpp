...
struct ExploreHighWaterBuffer::Owner final {
    ...
    [[nodiscard]] bool ensure_bytes(const std::size_t required) noexcept {
        ...
        const auto allocated =
            allocation.AllocateCandidate([this, next](void*& replacement) noexcept { return allocate(&replacement, next); });
    ...
    [[nodiscard]] cudaError_t release(void* value) noexcept {
        return cuda_status(memory == ExploreBufferMemory::PinnedHost ? api.release_pinned(api.context, value)
                                                                     : api.release_device(api.context, value));
    }
