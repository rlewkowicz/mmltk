...
[[nodiscard]] consteval bool rle_abi_layout_matches() {
 ...
 return std::is_trivially_copyable_v<Public> && std::is_trivially_copyable_v<Abi> && std::is_standard_layout_v<Public> && std::is_standard_layout_v<Abi> &&
        sizeof(Public) == sizeof(Abi) && alignof(Public) == alignof(Abi) && offsetof(Public, start) == offsetof(Abi, start) &&
        offsetof(Public, length) == offsetof(Abi, length);
}
...
ExploreStorageStatus render_explore_atlas_tiles(const ExploreRenderAtlasView& view, const ExploreRenderTileBatchView& tiles,
 ...
 return static_cast<ExploreStorageStatus>(
  detail::render_explore_atlas_tiles_cuda(view, tiles, semantics, to_cuda_scratch_abi(scratch), target, reinterpret_cast<cudaStream_t>(stream), demand));
}
ExploreStorageStatus render_explore_detail(const ExploreRenderDetailView& view, const ExploreRenderSemanticView& semantics,
                                           const ExploreRenderScratchView& scratch, const ExploreRenderTargetView& target, const std::uintptr_t stream,
                                           const detail::ExploreRenderDemand demand) noexcept {
 if (!demand.valid()) return kExploreStorageSuccess;
 if (stream == 0U || !target.valid() || !scratch.valid() || view.pixels == nullptr || view.source_width == 0U || view.source_height == 0U ||
     view.crop_width == 0U || view.crop_height == 0U || view.crop_x > view.source_width - 1U || view.crop_y > view.source_height - 1U ||
     view.crop_width > view.source_width - view.crop_x || view.crop_height > view.source_height - view.crop_y || !semantic_storage_valid(semantics, scratch)) {
  return static_cast<ExploreStorageStatus>(cudaErrorInvalidValue);
 }
 return static_cast<ExploreStorageStatus>(
...
ExploreStorageStatus probe_explore_rendered_card(const ExploreRenderTargetView& clean, const ExploreRenderTargetView& semantic,
 ...
 const bool content_valid = probe.reference.valid() && probe.reference.width == clean.width && probe.reference.height == clean.height &&
                            probe.content_width != 0U && probe.content_height != 0U && probe.content_x < clean.width && probe.content_y < clean.height &&
                            probe.content_width <= clean.width - probe.content_x && probe.content_height <= clean.height - probe.content_y;
 ...
 if (stream == 0U || device_counts == nullptr || !clean.valid() || !semantic.valid() || clean.width != semantic.width || clean.height != semantic.height ||
     !content_valid || !box_valid)
  return static_cast<ExploreStorageStatus>(cudaErrorInvalidValue);
 return static_cast<ExploreStorageStatus>(
