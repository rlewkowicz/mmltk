...
__global__ void atlas_tile_base_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderCardDescriptor* cards,
 ...
 if (card.source_width == 0U || card.source_height == 0U || card.crop_width == 0U || card.crop_height == 0U || card.crop_x >= card.source_width ||
     card.crop_y >= card.source_height || card.crop_width > card.source_width - card.crop_x || card.crop_height > card.source_height - card.crop_y) {
  store(target, static_cast<int>(x), static_cast<int>(y), output);
  return;
 }
 ...
 const float source_x =
  static_cast<float>(card.crop_x) + (static_cast<float>(x - image_x) + 0.5F) * static_cast<float>(card.crop_width) / static_cast<float>(card.image_width);
 const float source_y =
  static_cast<float>(card.crop_y) + (static_cast<float>(y - image_y) + 0.5F) * static_cast<float>(card.crop_height) / static_cast<float>(card.image_height);
 ...
 if (semantics.show_masks != 0U && card.annotation_count != 0U &&
  ...
  const bool mask_applied =
   apply_masks(red, green, blue, alpha, annotations, pairs, classes, card.annotation_offset, card.annotation_count, annotation_capacity, semantics.rle_count,
...
__global__ void atlas_tile_box_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderCardDescriptor* cards,
 ...
 for (std::uint32_t local = 0U; local < card.annotation_count; ++local) {
  ...
  draw_box_edges(
   target, annotation, image_x, image_y, static_cast<float>(card.image_width), static_cast<float>(card.image_height),
   ...
   static_cast<float>(card.crop_width) / static_cast<float>(card.source_width), static_cast<float>(card.crop_height) / static_cast<float>(card.source_height),
   image_x, image_y, min(card.image_width, tile.destination_width - min(card.image_x, tile.destination_width)),
...
__global__ void detail_box_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderAnnotationDescriptor* annotations,
 ...
 draw_box_edges(target, annotations[annotation_index], 0U, 0U, static_cast<float>(target.width), static_cast<float>(target.height),
                ...
                static_cast<float>(view.crop_width) / source_width, static_cast<float>(view.crop_height) / source_height, 0U, 0U, target.width, target.height,
...
cudaError_t render_explore_detail_cuda(const ExploreRenderDetailView& view, const ExploreRenderSemanticView& semantics, const ExploreRenderScratchView& scratch,
 ...
 if (!demand.valid()) return cudaSuccess;
 detail_base_kernel<<<static_cast<unsigned int>((pixels + threads - 1U) / threads), threads, 0U, stream>>>(target, view, annotations, pairs, classes, semantics,
                                                                                                           scratch.capacities.annotations);
 if (demand.valid() && semantics.show_boxes != 0U && semantics.annotation_count != 0U) {
...
cudaError_t checksum_explore_pixels_cuda(const ExploreRenderTargetView& target, std::uint64_t* const checksum, const cudaStream_t stream) noexcept {
 constexpr std::uint32_t threads = 256U;
 const std::uint64_t pixels = static_cast<std::uint64_t>(target.width) * target.height;
 checksum_pixels_kernel<<<static_cast<unsigned int>((pixels + threads - 1U) / threads), threads, 0U, stream>>>(target,
