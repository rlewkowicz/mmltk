...
[[nodiscard]] __device__ __forceinline__ uchar4 class_colour(const ExploreRenderClassDescriptor* classes,
...
[[nodiscard]] __device__ __forceinline__ float read_nchw(const float* pixels, const std::uint32_t width, const std::uint32_t height,
    ...
    return pixels[static_cast<std::size_t>(channel) * plane + static_cast<std::size_t>(clamped_y) * width +
...
[[nodiscard]] __device__ __forceinline__ bool rle_contains(const ExploreRenderRlePair* pairs,
                                                           const ExploreRenderAnnotationDescriptor& annotation,
                                                           const std::uint32_t rle_capacity, const std::uint32_t width,
...
[[nodiscard]] __device__ __forceinline__ bool apply_masks(float& red, float& green, float& blue, float& alpha,
                                                          const ExploreRenderAnnotationDescriptor* annotations,
                                                          const ExploreRenderRlePair* pairs, const ExploreRenderClassDescriptor* classes,
                                                          const std::uint32_t annotation_offset, const std::uint32_t annotation_count,
                                                          const std::uint32_t annotation_capacity, const std::uint32_t rle_capacity,
                                                          const std::uint32_t class_count, const std::uint32_t width,
                                                          const std::uint32_t height, const float x, const float y) noexcept {
    if (annotations == nullptr || annotation_offset > annotation_capacity || annotation_count > annotation_capacity - annotation_offset) {
        return false;
    }
    bool applied = false;
    for (std::uint32_t local = 0U; local < annotation_count; ++local) {
        const auto& annotation = annotations[annotation_offset + local];
        if (!class_visible(classes, class_count, annotation.class_id) ||
            !rle_contains(pairs, annotation, rle_capacity, width, height, x, y)) {
            continue;
        }
...
__global__ void atlas_tile_base_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderCardDescriptor* cards,
                                       ...
                                       const ExploreRenderClassDescriptor* classes, const ExploreRenderTileDescriptor* tiles,
                                       const ExploreRenderAtlasView view, const ExploreRenderTileBatchView batch,
    ...
    uchar4 output = view.draw_base != 0U ? make_uchar4(24U, 18U, 35U, 255U) : make_uchar4(0U, 0U, 0U, 0U);
    ...
    if (semantics.show_masks != 0U && card.annotation_count != 0U &&
        !mmltk::backend::models::rfdetr::augment_math::erases_sample(card.erasure, normalized_x, normalized_y, card.source_width,
                                                                     card.source_height)) {
        ...
        const bool mask_applied = apply_masks(red, green, blue, alpha, annotations, pairs, classes, card.annotation_offset,
                                              card.annotation_count, annotation_capacity, semantics.rle_count, semantics.class_count,
                                              card.source_width, card.source_height, normalized_x, normalized_y);
        if (mask_applied && alpha > 0.0F) {
            output = make_uchar4(to_byte(red / alpha), to_byte(green / alpha), to_byte(blue / alpha), to_byte(alpha));
        }
    }
    store(target, static_cast<int>(x), static_cast<int>(y), output);
}
...
__global__ void detail_base_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderDetailView view,
    ...
    if (semantics.show_masks != 0U && semantics.annotation_count != 0U &&
        ...
        const bool mask_applied =
            apply_masks(red, green, blue, alpha, annotations, pairs, classes, 0U, semantics.annotation_count, annotation_capacity,
...
__device__ __forceinline__ void draw_box_edges(const Target target, const ExploreRenderAnnotationDescriptor annotation,
                                               const std::uint32_t offset_x, const std::uint32_t offset_y, const float scale_x,
                                               const float scale_y, const float source_origin_x, const float source_origin_y,
                                               const float source_extent_x, const float source_extent_y, const std::uint32_t clip_x,
                                               const std::uint32_t clip_y, const std::uint32_t clip_width, const std::uint32_t clip_height,
                                               const ExploreRenderClassDescriptor* classes, const std::uint32_t class_count) noexcept {
...
__global__ void atlas_tile_box_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderCardDescriptor* cards,
                                      ...
                                      const ExploreRenderTileDescriptor* tiles, const ExploreRenderAtlasView view,
    ...
    for (std::uint32_t local = 0U; local < card.annotation_count; ++local) {
        ...
        draw_box_edges(
            target, annotation, image_x, image_y, static_cast<float>(card.image_width), static_cast<float>(card.image_height), 0.0F, 0.0F,
            1.0F, 1.0F, image_x, image_y, min(card.image_width, tile.destination_width - min(card.image_x, tile.destination_width)),
...
__global__ void detail_box_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderAnnotationDescriptor* annotations,
                                  const ExploreRenderClassDescriptor* classes, const ExploreRenderDetailView view,
    ...
    draw_box_edges(target, annotations[annotation_index], 0U, 0U, static_cast<float>(target.width), static_cast<float>(target.height),
                   ...
                   static_cast<float>(view.crop_width) / source_width, static_cast<float>(view.crop_height) / source_height, 0U, 0U,
                   target.width, target.height, classes, semantics.class_count);
}
...
__global__ void probe_rendered_card_kernel(const ExploreRenderTargetView clean_view, const ExploreRenderTargetView semantic_view,
    ...
    const bool on_edge =
        x == probe.box_x || y == probe.box_y || x + 1U == probe.box_x + probe.box_width || y + 1U == probe.box_y + probe.box_height;
...
cudaError_t count_explore_nonzero_alpha_cuda(const ExploreRenderTargetView& target, std::uint64_t* const count,
...
cudaError_t checksum_explore_pixels_cuda(const ExploreRenderTargetView& target, std::uint64_t* const checksum,
                                         const cudaStream_t stream) noexcept {
    constexpr std::uint32_t threads = 256U;
    const std::uint64_t pixels = static_cast<std::uint64_t>(target.width) * target.height;
...
cudaError_t probe_explore_rendered_card_cuda(const ExploreRenderTargetView& clean, const ExploreRenderTargetView& semantic,
                                             const ExploreRenderedCardProbeAbi& probe, std::uint64_t* const counts,
                                             const cudaStream_t stream) noexcept {
    constexpr std::uint32_t threads = 256U;
