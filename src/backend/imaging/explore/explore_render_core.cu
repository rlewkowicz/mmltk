...
[[nodiscard]] __device__ __forceinline__ Target target_view(const ExploreRenderTargetView view) noexcept { return {view.data, view.pitch_bytes, view.width, view.height}; }
...
[[nodiscard]] __device__ __forceinline__ bool class_visible(const ExploreRenderClassDescriptor* classes, const std::uint32_t class_count, const std::uint16_t class_id) noexcept {
 return classes != nullptr && class_id < class_count && classes[class_id].visible != 0U;
}
[[nodiscard]] __device__ __forceinline__ unsigned char to_byte(const float value) noexcept { return static_cast<unsigned char>(__float2int_rn(fminf(1.0F, fmaxf(0.0F, value)) * 255.0F)); }
[[nodiscard]] __device__ __forceinline__ float read_nchw(const float* pixels, const std::uint32_t width, const std::uint32_t height, const std::uint32_t channel, const int x, const int y) noexcept {
...
[[nodiscard]] __device__ __forceinline__ float sample_nchw(
 const float* pixels, const std::uint32_t width, const std::uint32_t height, const std::uint32_t channel, const float x, const float y) noexcept {
...
[[nodiscard]] __device__ __forceinline__ bool rle_contains(const ExploreRenderRlePair* pairs, const ExploreRenderAnnotationDescriptor& annotation, const std::uint32_t rle_capacity,
 const std::uint32_t width, const std::uint32_t height, const float x, const float y) noexcept {
...
[[nodiscard]] __device__ __forceinline__ bool apply_masks(float& red, float& green, float& blue, float& alpha, const ExploreRenderAnnotationDescriptor* annotations, const ExploreRenderRlePair* pairs,
 const ExploreRenderClassDescriptor* classes, const std::uint32_t annotation_offset, const std::uint32_t annotation_count, const std::uint32_t annotation_capacity, const std::uint32_t rle_capacity,
 const std::uint32_t class_count, const std::uint32_t width, const std::uint32_t height, const float x, const float y) noexcept {
...
__global__ void atlas_tile_base_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderCardDescriptor* cards, const ExploreRenderAnnotationDescriptor* annotations,
 const ExploreRenderRlePair* pairs, const ExploreRenderClassDescriptor* classes, const ExploreRenderTileDescriptor* tiles, const ExploreRenderAtlasView view, const ExploreRenderTileBatchView batch,
 ...
 if (tile.generation.viewport != batch.viewport_generation || tile.destination_width == 0U || tile.destination_height == 0U || tile.destination_x >= target.width ||
     tile.destination_y >= target.height || tile.destination_width > target.width - tile.destination_x || tile.destination_height > target.height - tile.destination_y) {
  return;
 }
 ...
 if (card.source_width == 0U || card.source_height == 0U || card.crop_width == 0U || card.crop_height == 0U || card.crop_x >= card.source_width || card.crop_y >= card.source_height ||
     card.crop_width > card.source_width - card.crop_x || card.crop_height > card.source_height - card.crop_y) {
  store(target, static_cast<int>(x), static_cast<int>(y), output);
  return;
 }
 ...
 const float source_x = static_cast<float>(card.crop_x) + (static_cast<float>(x - image_x) + 0.5F) * static_cast<float>(card.crop_width) / static_cast<float>(card.image_width);
 const float source_y = static_cast<float>(card.crop_y) + (static_cast<float>(y - image_y) + 0.5F) * static_cast<float>(card.crop_height) / static_cast<float>(card.image_height);
 ...
 if (view.draw_base != 0U && card.pixels != nullptr) {
  ...
  output = make_uchar4(to_byte(sample_nchw(card.pixels, card.source_width, card.source_height, 0U, sample_x, sample_y)),
   to_byte(sample_nchw(card.pixels, card.source_width, card.source_height, 1U, sample_x, sample_y)), to_byte(sample_nchw(card.pixels, card.source_width, card.source_height, 2U, sample_x, sample_y)),
   255U);
 }
 if (semantics.show_masks != 0U && card.annotation_count != 0U &&
  ...
  const bool mask_applied = apply_masks(red, green, blue, alpha, annotations, pairs, classes, card.annotation_offset, card.annotation_count, annotation_capacity, semantics.rle_count,
   semantics.class_count, card.source_width, card.source_height, normalized_x, normalized_y);
  if (mask_applied && alpha > 0.0F) { output = make_uchar4(to_byte(red / alpha), to_byte(green / alpha), to_byte(blue / alpha), to_byte(alpha)); }
 }
 store(target, static_cast<int>(x), static_cast<int>(y), output);
}
__global__ void detail_base_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderDetailView view, const ExploreRenderAnnotationDescriptor* annotations,
 const ExploreRenderRlePair* pairs, const ExploreRenderClassDescriptor* classes, const ExploreRenderSemanticView semantics, const std::uint32_t annotation_capacity) {
 const Target target = target_view(target_view_value);
 ...
 if (view.draw_base != 0U && view.pixels != nullptr) {
  output = make_uchar4(to_byte(sample_nchw(view.pixels, view.source_width, view.source_height, 0U, source_x, source_y)),
   to_byte(sample_nchw(view.pixels, view.source_width, view.source_height, 1U, source_x, source_y)), to_byte(sample_nchw(view.pixels, view.source_width, view.source_height, 2U, source_x, source_y)),
   255U);
 }
 ...
 if (semantics.show_masks != 0U && semantics.annotation_count != 0U &&
  ...
  float blue = static_cast<float>(output.z) / 255.0F;
  float alpha = static_cast<float>(output.w) / 255.0F;
  const bool mask_applied = apply_masks(red, green, blue, alpha, annotations, pairs, classes, 0U, semantics.annotation_count, annotation_capacity, semantics.rle_count, semantics.class_count,
   view.source_width, view.source_height, mask_x, mask_y);
  if (mask_applied && alpha > 0.0F) { output = make_uchar4(to_byte(red / alpha), to_byte(green / alpha), to_byte(blue / alpha), to_byte(alpha)); }
 }
 store(target, static_cast<int>(x), static_cast<int>(y), output);
}
__device__ __forceinline__ void draw_box_edges(const Target target, const ExploreRenderAnnotationDescriptor annotation, const std::uint32_t offset_x, const std::uint32_t offset_y, const float scale_x,
 const float scale_y, const float source_origin_x, const float source_origin_y, const float source_extent_x, const float source_extent_y, const std::uint32_t clip_x, const std::uint32_t clip_y,
 const std::uint32_t clip_width, const std::uint32_t clip_height, const ExploreRenderClassDescriptor* classes, const std::uint32_t class_count) noexcept {
 ...
 for (std::uint32_t edge_index = threadIdx.x; edge_index < edge_pixels; edge_index += blockDim.x) {
  ...
  if (x >= static_cast<int>(clip_x) && y >= static_cast<int>(clip_y) && x < static_cast<int>(clip_x + clip_width) && y < static_cast<int>(clip_y + clip_height) && x < static_cast<int>(target.width) &&
      y < static_cast<int>(target.height)) {
   store(target, x, y, class_colour(classes, annotation.class_id));
  }
 }
}
__global__ void atlas_tile_box_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderCardDescriptor* cards, const ExploreRenderAnnotationDescriptor* annotations,
 const ExploreRenderClassDescriptor* classes, const ExploreRenderTileDescriptor* tiles, const ExploreRenderAtlasView view, const ExploreRenderTileBatchView batch,
 const ExploreRenderSemanticView semantics, const std::uint32_t annotation_capacity) {
 const Target target = target_view(target_view_value);
 const std::uint32_t tile_index = blockIdx.z;
 ...
 auto tile = tiles[tile_index];
 if (view.draw_base == 0U) tile.destination_y = static_cast<std::uint32_t>(static_cast<std::int64_t>(tile.destination_y) + tile.semantic_y_offset);
 if (tile.generation.viewport != batch.viewport_generation || tile.placeholder != 0U || tile.card_index >= view.card_count || tile.destination_width == 0U || tile.destination_height == 0U ||
     tile.destination_x >= target.width || tile.destination_y >= target.height || tile.destination_width > target.width - tile.destination_x ||
     tile.destination_height > target.height - tile.destination_y || cards[tile.card_index].placeholder != 0U) {
  return;
 }
 const auto card = cards[tile.card_index];
 if (card.source_width == 0U || card.source_height == 0U || card.crop_width == 0U || card.crop_height == 0U || card.annotation_offset > annotation_capacity ||
     card.annotation_count > annotation_capacity - card.annotation_offset) {
  return;
 }
 const std::uint32_t image_x = tile.destination_x + card.image_x;
 const std::uint32_t image_y = tile.destination_y + card.image_y;
 for (std::uint32_t local = 0U; local < card.annotation_count; ++local) {
  ...
  draw_box_edges(target, annotation, image_x, image_y, static_cast<float>(card.image_width), static_cast<float>(card.image_height),
   static_cast<float>(card.crop_x) / static_cast<float>(card.source_width), static_cast<float>(card.crop_y) / static_cast<float>(card.source_height),
   static_cast<float>(card.crop_width) / static_cast<float>(card.source_width), static_cast<float>(card.crop_height) / static_cast<float>(card.source_height), image_x, image_y,
   min(card.image_width, tile.destination_width - min(card.image_x, tile.destination_width)), min(card.image_height, tile.destination_height - min(card.image_y, tile.destination_height)), classes,
   semantics.class_count);
 }
}
__global__ void detail_box_kernel(const ExploreRenderTargetView target_view_value, const ExploreRenderAnnotationDescriptor* annotations, const ExploreRenderClassDescriptor* classes,
 const ExploreRenderDetailView view, const ExploreRenderSemanticView semantics) {
 const Target target = target_view(target_view_value);
...
__global__ void checksum_pixels_kernel(const ExploreRenderTargetView target_view_value, unsigned long long* checksum) {
 ...
 const unsigned long long packed =
  static_cast<unsigned long long>(pixel.x) | (static_cast<unsigned long long>(pixel.y) << 8U) | (static_cast<unsigned long long>(pixel.z) << 16U) | (static_cast<unsigned long long>(pixel.w) << 24U);
 atomicAdd(checksum, packed * (index + 1ULL));
}
__global__ void probe_rendered_card_kernel(const ExploreRenderTargetView clean_view, const ExploreRenderTargetView semantic_view, const ExploreRenderedCardProbeAbi probe, unsigned long long* counts) {
 ...
 const std::uint32_t padding_count = (probe.content_y != 0U && content_column && y == (probe.content_y - 1U) / 2U ? 1U : 0U) +
 ...
 if (in_content && on_content_edge) {
  ...
  if (matches_reference(x, y)) {
   const std::uint32_t matching_transitions =
    (probe.content_y != 0U && y == probe.content_y && matches_reference(x, y - 1U) ? 1U : 0U) + (content_bottom < clean.height && y + 1U == content_bottom && matches_reference(x, y + 1U) ? 1U : 0U) +
    (probe.content_x != 0U && x == probe.content_x && matches_reference(x - 1U, y) ? 1U : 0U) + (content_right < clean.width && x + 1U == content_right && matches_reference(x + 1U, y) ? 1U : 0U);
...
__global__ void sample_rendered_card_kernel(
 const ExploreRenderTargetView clean, const ExploreRenderTargetView semantic, const ExploreRenderTargetView reference, const ExploreRenderedCardSampleGridAbi grid, std::uint64_t* samples) {
 ...
 const auto sample = [&](const ExploreRenderTargetView target) -> std::uint64_t {
  ...
  return static_cast<std::uint64_t>(pixel.x) | (static_cast<std::uint64_t>(pixel.y) << 8U) | (static_cast<std::uint64_t>(pixel.z) << 16U) | (static_cast<std::uint64_t>(pixel.w) << 24U);
 };
...
cudaError_t render_explore_atlas_tiles_cuda(const ExploreRenderAtlasView& view, const ExploreRenderTileBatchView& tiles, const ExploreRenderSemanticView& semantics,
 const ExploreRenderScratchView& scratch, const ExploreRenderTargetView& target, const cudaStream_t stream, const ExploreRenderDemand demand) noexcept {
 ...
 const dim3 tile_grid{(tiles.max_tile_width + tile_block.x - 1U) / tile_block.x, (tiles.max_tile_height + tile_block.y - 1U) / tile_block.y, tiles.tile_count};
 const dim3 tile_box_grid{1U, 1U, tiles.tile_count};
 if (!demand.valid()) return cudaSuccess;
 atlas_tile_base_kernel<<<tile_grid, tile_block, 0U, stream>>>(target, cards, annotations, pairs, classes, tile_descriptors, view, tiles, semantics, scratch.capacities.annotations);
 if (demand.valid() && semantics.show_boxes != 0U && semantics.annotation_count != 0U) {
  atlas_tile_box_kernel<<<tile_box_grid, threads, 0U, stream>>>(target, cards, annotations, classes, tile_descriptors, view, tiles, semantics, scratch.capacities.annotations);
 }
 return cudaPeekAtLastError();
}
cudaError_t render_explore_detail_cuda(const ExploreRenderDetailView& view, const ExploreRenderSemanticView& semantics, const ExploreRenderScratchView& scratch, const ExploreRenderTargetView& target,
 ...
 if (!demand.valid()) return cudaSuccess;
 detail_base_kernel<<<static_cast<unsigned int>((pixels + threads - 1U) / threads), threads, 0U, stream>>>(target, view, annotations, pairs, classes, semantics, scratch.capacities.annotations);
 if (demand.valid() && semantics.show_boxes != 0U && semantics.annotation_count != 0U) {
...
cudaError_t count_explore_nonzero_alpha_cuda(const ExploreRenderTargetView& target, std::uint64_t* const count, const cudaStream_t stream) noexcept {
 constexpr std::uint32_t threads = 256U;
 const std::uint64_t pixels = static_cast<std::uint64_t>(target.width) * target.height;
 count_nonzero_alpha_kernel<<<static_cast<unsigned int>((pixels + threads - 1U) / threads), threads, 0U, stream>>>(target, reinterpret_cast<unsigned long long*>(count));
 return cudaPeekAtLastError();
}
cudaError_t checksum_explore_pixels_cuda(const ExploreRenderTargetView& target, std::uint64_t* const checksum, const cudaStream_t stream) noexcept {
 constexpr std::uint32_t threads = 256U;
 const std::uint64_t pixels = static_cast<std::uint64_t>(target.width) * target.height;
 checksum_pixels_kernel<<<static_cast<unsigned int>((pixels + threads - 1U) / threads), threads, 0U, stream>>>(target, reinterpret_cast<unsigned long long*>(checksum));
 return cudaPeekAtLastError();
}
cudaError_t probe_explore_rendered_card_cuda(
 const ExploreRenderTargetView& clean, const ExploreRenderTargetView& semantic, const ExploreRenderedCardProbeAbi& probe, std::uint64_t* const counts, const cudaStream_t stream) noexcept {
 constexpr std::uint32_t threads = 256U;
 const std::uint64_t pixels = static_cast<std::uint64_t>(clean.width) * clean.height;
 probe_rendered_card_kernel<<<static_cast<unsigned int>((pixels + threads - 1U) / threads), threads, 0U, stream>>>(clean, semantic, probe, reinterpret_cast<unsigned long long*>(counts));
 return cudaPeekAtLastError();
}
cudaError_t sample_explore_rendered_card_cuda(
 const ExploreRenderTargetView& clean, const ExploreRenderTargetView& semantic, const ExploreRenderTargetView& reference, std::uint64_t* const samples, const cudaStream_t stream) noexcept {
