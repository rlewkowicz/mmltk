...
std::filesystem::path compile_explore_fixture(const std::filesystem::path& temporary_root, const std::string_view fixture_name, const int num_images,
 ...
 if (annotations.objects != 0U) {
  ...
  for (int image = 11; image <= num_images; ++image) {
   ...
   for (std::size_t object = 0U; object < annotations.objects; ++object) {
    output << "{\"iscrowd\":" << ((annotations.crowd_only || (annotations.mixed_crowd && object % 2U == 0U)) ? "true" : "false") << ",\"class\":\"person\"";
 ...
 if (annotations.ring_and_dots) {
  if (num_images != 2 || dimensions.source_width != 8 || dimensions.source_height != 8) throw std::invalid_argument("ring fixture requires two 8 by 8 images");
 ...
 if (annotations.independent_masks) {
  ...
  for (unsigned object = 0; object < 3; ++object) {
   ...
   if (object != 2) output << R"(,"mask_rle_encoding":"row_major_start_length","mask_rle":")" << (object == 0 ? "0:1" : "") << '"';
   output << "}\n";
  }
  if (!output) throw std::runtime_error("Explore independent mask fixture write failed");
 }
