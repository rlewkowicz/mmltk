...
[[nodiscard]] __host__ __device__ inline bool sample_annotation_mask(const ExploreRenderRlePairAbi* pairs,
                                                                     const ExploreRenderAnnotationDescriptorAbi& annotation,
                                                                     const std::uint32_t capacity, const std::uint32_t width,
                                                                     const std::uint32_t height, const float x, const float y) noexcept {
    if (!pairs || width == 0U || height == 0U || annotation.rle_offset > capacity ||
        annotation.rle_count > capacity - annotation.rle_offset || annotation.rle_count == 0U || x < annotation.box_xyxy[0] ||
        x > annotation.box_xyxy[2] || y < annotation.box_xyxy[1] || y > annotation.box_xyxy[3])
        return false;
...
[[nodiscard]] __host__ __device__ inline bool sample_annotation_support(const ExploreRenderRlePairAbi* pairs,
    ...
    if (annotation.rle_count != 0) return sample_annotation_mask(pairs, annotation, capacity, width, height, x, y);
