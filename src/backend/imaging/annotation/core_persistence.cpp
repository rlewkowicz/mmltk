...
AnnotationShapeType annotation_shape_type_from_name(const std::string_view shape_type_name) {
 ...
 if (shape_type_name == "spline") { return AnnotationShapeType::Spline; }
 if (shape_type_name == "point") { return AnnotationShapeType::Point; }
...
void validate_annotation_categories_meta(const json& meta) {
 const auto require_string = [&](const char* key, const std::string_view expected) {
  ...
  if (field == meta.end() || !field->is_string() || field->get<std::string>() != expected) { throw std::runtime_error("annotation categories meta: unexpected `" + std::string(key) + "`"); }
 };
 const auto require_shape_types = [&]() {
  const auto field = meta.find("shape_types");
  if (field == meta.end() || !field->is_array() || field->size() != kAnnotationShapeTypes.size()) { throw std::runtime_error("annotation categories meta: unexpected `shape_types`"); }
  for (std::size_t index = 0; index < kAnnotationShapeTypes.size(); ++index) {
   if (!(*field)[index].is_string() || (*field)[index].get<std::string>() != kAnnotationShapeTypes[index]) { throw std::runtime_error("annotation categories meta: unexpected `shape_types`"); }
  }
 };
...
json category_json(const AnnotationCategory& category) {
 ...
 if (!category.keypoints.empty()) { payload["keypoints"] = category.keypoints; }
 if (!category.skeleton_edges.empty()) {
  payload["skeleton_edges"] = json::array();
  for (const AnnotationCategorySkeletonEdge& edge : category.skeleton_edges) { payload["skeleton_edges"].push_back({edge.source_index, edge.target_index}); }
 }
 return payload;
}
json category_split_stats(const std::filesystem::path& split_dir) {
 ...
 if (std::filesystem::exists(split_dir)) {
  for (const auto& entry : std::filesystem::directory_iterator(split_dir)) {
   if (entry.is_regular_file() && entry.path().extension() == ".png") { ++total; }
  }
 }
 return json{
  {"total", total},
...
void append_jsonl(const std::filesystem::path& path, const json& entry) { mmltk::common::io::throw_on_json_write_failure(mmltk::common::io::append_json_line(path, entry), path, "JSONL manifest"); }
...
std::optional<AnnotationBox> frame_box_to_capture_box(
 const AnnotationBox& frame_box, const std::uint32_t view_x, const std::uint32_t view_y, const std::uint32_t capture_width, const std::uint32_t capture_height) {
...
std::vector<std::uint8_t> trim_dense_mask_to_bbox(const std::vector<std::uint8_t>& dense_mask, const std::uint32_t image_width, const AnnotationBox& bbox) {
 ...
 for (std::uint32_t row = 0; row < trimmed_height; ++row) {
  const std::size_t src_offset = static_cast<std::size_t>(bbox.y1 + static_cast<int>(row)) * static_cast<std::size_t>(image_width) + static_cast<std::size_t>(bbox.x1);
...
AnnotationObject annotation_object_from_scene_record(const json& record, AnnotationCategories* categories) {
 ...
 switch (shape_type) {
  case AnnotationShapeType::Point: {
   ...
   if (xy == shape_payload.end()) { fail("point record is missing `shape.xy`"); }
   const std::optional<AnnotationPoint> point = annotation_point_from_json(*xy);
   if (!point.has_value()) { fail("point record has invalid `shape.xy`"); }
  ...
  case AnnotationShapeType::Spline: {
   ...
   if (knots == shape_payload.end() || !knots->is_array()) { fail("spline record is missing array `shape.knots`"); }
   ...
   for (const auto& knot_entry : *knots) {
    if (!knot_entry.is_object()) { fail("spline record contains a non-object knot"); }
    const auto knot_xy = knot_entry.find("xy");
    if (knot_xy == knot_entry.end()) { fail("spline knot is missing `xy`"); }
    ...
    if (const auto in_handle = knot_entry.find("in_handle_xy"); in_handle != knot_entry.end()) {
     const std::optional<AnnotationPoint> handle_point = annotation_point_from_json(*in_handle);
     if (!handle_point.has_value()) { fail("spline knot has invalid `in_handle_xy`"); }
     const AnnotationPoint handle_point_value = handle_point.value_or(AnnotationPoint{});
     knot.in_handle.position = annotation_frame_point_to_capture_unclipped(handle_point_value, view_x, view_y);
     knot.in_handle.enabled = true;
    }
    if (const auto out_handle = knot_entry.find("out_handle_xy"); out_handle != knot_entry.end()) {
     const std::optional<AnnotationPoint> handle_point = annotation_point_from_json(*out_handle);
     if (!handle_point.has_value()) { fail("spline knot has invalid `out_handle_xy`"); }
     const AnnotationPoint handle_point_value = handle_point.value_or(AnnotationPoint{});
     knot.out_handle.position = annotation_frame_point_to_capture_unclipped(handle_point_value, view_x, view_y);
  ...
  case AnnotationShapeType::Skeleton: {
   ...
   if (nodes == shape_payload.end() || !nodes->is_array()) { fail("skeleton record is missing array `shape.nodes`"); }
   const auto edges = shape_payload.find("edges");
   if (edges == shape_payload.end() || !edges->is_array()) { fail("skeleton record is missing array `shape.edges`"); }
   AnnotationSkeletonShape skeleton;
   skeleton.nodes.reserve(nodes->size());
   for (const auto& node_entry : *nodes) {
    if (!node_entry.is_object()) { fail("skeleton record contains a non-object node"); }
    const auto node_xy = node_entry.find("xy");
    if (node_xy == node_entry.end()) { fail("skeleton node is missing `xy`"); }
    const std::optional<AnnotationPoint> node_point = annotation_point_from_json(*node_xy);
    if (!node_point.has_value()) { fail("skeleton node has invalid `xy`"); }
    const AnnotationPoint node_point_value = node_point.value_or(AnnotationPoint{});
    const auto key = node_entry.find("key");
    if (key == node_entry.end() || !key->is_string()) { fail("skeleton node is missing string `key`"); }
   ...
   for (const auto& edge_entry : *edges) {
    if (!edge_entry.is_array() || edge_entry.size() != 2U) { fail("skeleton edge must contain exactly two indices"); }
  ...
  case AnnotationShapeType::Mask: {
   ...
   if (shape_box_xyxy == shape_payload.end()) { fail("mask record is missing `shape.box_xyxy`"); }
   const std::optional<AnnotationBox> shape_frame_box = annotation_box_from_json(*shape_box_xyxy);
   if (!shape_frame_box.has_value()) { fail("mask record has invalid `shape.box_xyxy`"); }
   const AnnotationBox shape_frame_box_value = shape_frame_box.value_or(AnnotationBox{});
   const std::optional<AnnotationBox> shape_capture_box = frame_box_to_capture_box(shape_frame_box_value, view_x, view_y, capture_width, capture_height);
   if (!shape_capture_box.has_value()) { fail("mask shape box falls outside the declared capture bounds"); }
   const AnnotationBox shape_capture_box_value = shape_capture_box.value_or(AnnotationBox{});
  ...
  case AnnotationShapeType::Box: {
   const json& shape_payload = required_shape_object();
   const auto shape_box_xyxy = shape_payload.find("box_xyxy");
   if (shape_box_xyxy == shape_payload.end()) { fail("box record is missing `shape.box_xyxy`"); }
   const std::optional<AnnotationBox> shape_frame_box = annotation_box_from_json(*shape_box_xyxy);
   if (!shape_frame_box.has_value()) { fail("box record has invalid `shape.box_xyxy`"); }
   const AnnotationBox shape_frame_box_value = shape_frame_box.value_or(AnnotationBox{});
   const std::optional<AnnotationBox> shape_capture_box = frame_box_to_capture_box(shape_frame_box_value, view_x, view_y, capture_width, capture_height);
   if (!shape_capture_box.has_value()) { fail("box shape box falls outside the declared capture bounds"); }
   const AnnotationBox shape_capture_box_value = shape_capture_box.value_or(AnnotationBox{});
...
AnnotationCategories load_annotation_categories(const std::filesystem::path& output_root) {
 ...
 if (!std::filesystem::exists(categories_path)) { return categories; }
 ...
 if (meta != parsed.end()) {
  ...
  if (const auto dataset_name = meta->find("dataset_name"); dataset_name != meta->end() && dataset_name->is_string()) categories.dataset_name = dataset_name->get<std::string>();
 }
 ...
 for (const auto& entry : *classes) {
  ...
  if (const auto keypoints = entry.find("keypoints"); keypoints != entry.end() && keypoints->is_array()) { category.keypoints = keypoints->get<std::vector<std::string>>(); }
  if (const auto edges = entry.find("skeleton_edges"); edges != entry.end() && edges->is_array()) {
   for (const auto& edge : *edges) {
    if (!edge.is_array() || edge.size() != 2U) { continue; }
    category.skeleton_edges.push_back(AnnotationCategorySkeletonEdge{
...
void write_annotation_categories(const std::filesystem::path& output_root, const AnnotationCategories& categories) {
 ...
 for (const char* split_name : {"train", "val", "test"}) {
  ...
  if (!std::filesystem::exists(split_dir)) { continue; }
...
std::optional<std::vector<AnnotationObject>> load_saved_annotation_scene_for_frame(const std::filesystem::path& output_root, const AnnotationFrame& frame, AnnotationCategories* categories) {
 ...
 while (std::getline(stream, line)) {
  ...
  if (frame_id != frame.frame_id) { continue; }
  if (!target_source_path.empty() && normalized_path_string(source_path) != target_source_path) { continue; }
  const std::uint32_t scene_index = entry.value("scene_index", 0U);
  if (scene_index < best_scene_index) { continue; }
  const std::filesystem::path relative_scene_jsonl = entry.value("scene_jsonl", std::string{});
  if (relative_scene_jsonl.empty()) { continue; }
...
AnnotationSaveResult save_annotation_scene(const AnnotationSaveConfig& config, const AnnotationFrame& frame, AnnotationCategories& categories, const std::vector<AnnotationObject>& objects,
 ...
 for (std::size_t index = 0; index < resolved_objects.size(); ++index) {
  ...
  if (resolved.crop_width > 0U && resolved.crop_height > 0U && !resolved.crop_rgba.empty()) {
   ...
   write_annotation_png(entity_path, static_cast<int>(resolved.crop_width), static_cast<int>(resolved.crop_height), 4, resolved.crop_rgba.data(), static_cast<int>(resolved.crop_width * 4U));
