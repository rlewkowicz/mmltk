...
AnnotationBox clamp_box(AnnotationBox box, const int max_width, const int max_height) {
    return normalize_annotation_box(box, static_cast<std::uint32_t>(std::max(0, max_width)), static_cast<std::uint32_t>(std::max(0, max_height)));
}
...
AnnotationBox apply_rectangle_drag(const RectDragState& state, const CanvasPointerState& pointer, const CanvasViewport& viewport, const int max_width,
...
RectDragKind rectangle_hover_kind_with_options(const CanvasPointerState& pointer, const CanvasViewport& viewport, const AnnotationBox& box, bool edge_only_move,
                                               float edge_hit_half_width, float corner_hit_size);
...
RectDragKind rectangle_hover_kind_with_options(const CanvasPointerState& pointer, const CanvasViewport& viewport, const AnnotationBox& box,
    ...
    const bool near_top_left = near_top && near_left && pointer.screen_x < screen_box.x1 + corner_size && pointer.screen_y < screen_box.y1 + corner_size;
    const bool near_top_right = near_top && near_right && pointer.screen_x > screen_box.x2 - corner_size && pointer.screen_y < screen_box.y1 + corner_size;
    const bool near_bottom_left = near_bottom && near_left && pointer.screen_x < screen_box.x1 + corner_size && pointer.screen_y > screen_box.y2 - corner_size;
...
AnnotationBox apply_rect_drag(const RectDragState& state, const CanvasPointerState& pointer, const CanvasViewport& viewport, const int max_width,
                              const int max_height, const int min_size) {
    return apply_rectangle_drag(state, pointer, viewport, max_width, max_height, min_size);
}
RectLayerFrameResult update_rect_layers(RectLayerState& state, const RectLayerSpec* layers, const std::size_t layer_count, const CanvasViewport& viewport,
