...
[[nodiscard]] std::optional<AnnotationPoint> default_spline_handle_capture_point(
 const AnnotationFrame& frame, const AnnotationSplineShape& spline, const std::size_t knot_index, const AnnotationHandleRole role) {
 if ((role != AnnotationHandleRole::SplineInHandle && role != AnnotationHandleRole::SplineOutHandle) || knot_index >= spline.knots.size() || spline.knots.size() < 2U) { return std::nullopt; }
 ...
 const std::optional<std::size_t> previous = knot_index > 0U ? std::optional<std::size_t>{knot_index - 1U} : spline.closed ? std::optional<std::size_t>{spline.knots.size() - 1U} : std::nullopt;
 const std::optional<std::size_t> next = knot_index + 1U < spline.knots.size() ? std::optional<std::size_t>{knot_index + 1U} : spline.closed ? std::optional<std::size_t>{0U} : std::nullopt;
 if ((role == AnnotationHandleRole::SplineInHandle && !previous.has_value()) || (role == AnnotationHandleRole::SplineOutHandle && !next.has_value())) { return std::nullopt; }
 ...
 if (previous.has_value() && next.has_value()) {
  ...
  handle_length = std::min(std::hypot(knot.position.x - before.x, knot.position.y - before.y), std::hypot(after.x - knot.position.x, after.y - knot.position.y)) / 3.0F;
 ...
 return clamp_capture_point_to_bounds(frame, {knot.position.x + tangent_x / tangent_length * handle_length * direction, knot.position.y + tangent_y / tangent_length * handle_length * direction});
}
void push_handle(AnnotationProjectedScene& scene, const std::size_t selected_object_index, const std::size_t index, const AnnotationHandleRole role, const std::size_t category_index,
 const AnnotationPoint& capture_point, const AnnotationPoint& frame_point, const AnnotationPoint& tether_frame_point = {}, const bool has_tether = false, const bool materialized = true) {
...
AnnotationProjectedScene refresh_annotation_projected_scene_selection(
 const AnnotationFrame& frame, const AnnotationSceneView& document, AnnotationProjectedScene scene, std::optional<std::size_t> selected_object_index) {
 if (selected_object_index.has_value() && *selected_object_index >= document.size()) { selected_object_index.reset(); }
