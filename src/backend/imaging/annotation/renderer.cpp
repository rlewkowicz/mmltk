...
[[nodiscard]] AnnotationPoint clamp_capture_point_to_bounds(const AnnotationFrame& frame, const AnnotationPoint& point) {
    ...
    const float max_y =
        annotation_frame_capture_height(frame) == 0U ? 0.0F : static_cast<float>(annotation_frame_capture_height(frame) - 1U);
    return {
...
[[nodiscard]] std::optional<AnnotationPoint> default_spline_handle_capture_point(const AnnotationFrame& frame,
    ...
    const std::optional<std::size_t> next = knot_index + 1U < spline.knots.size() ? std::optional<std::size_t>{knot_index + 1U}
                                            : spline.closed                       ? std::optional<std::size_t>{0U}
                                                                                  : std::nullopt;
    if ((role == AnnotationHandleRole::SplineInHandle && !previous.has_value()) ||
        (role == AnnotationHandleRole::SplineOutHandle && !next.has_value())) {
        return std::nullopt;
    }
    ...
    if (previous.has_value() && next.has_value()) {
        ...
        handle_length = std::min(std::hypot(knot.position.x - before.x, knot.position.y - before.y),
                                 std::hypot(after.x - knot.position.x, after.y - knot.position.y)) /
                        3.0F;
    ...
    return clamp_capture_point_to_bounds(frame, {knot.position.x + tangent_x / tangent_length * handle_length * direction,
                                                 knot.position.y + tangent_y / tangent_length * handle_length * direction});
}

void push_handle(AnnotationProjectedScene& scene, const std::size_t selected_object_index, const std::size_t index,
                 const AnnotationHandleRole role, const std::size_t category_index, const AnnotationPoint& capture_point,
                 const AnnotationPoint& frame_point, const AnnotationPoint& tether_frame_point = {}, const bool has_tether = false,
                 const bool materialized = true) {
    ...
    std::visit(
        [&](const auto& shape) {
            ...
            if constexpr (std::is_same_v<Shape, AnnotationPointShape>) {
                ...
                for (AnnotationPoint& point : geometry.frame_points) {
                    point = annotation_capture_point_to_frame_unclipped(frame, point);
                }
...
void project_selected_handles(const AnnotationFrame& frame, const AnnotationSceneView& document,
                              const std::optional<std::size_t> selected_object_index, AnnotationProjectedScene& scene) {
            ...
            if constexpr (std::is_same_v<Shape, AnnotationPointShape>) {
                ...
                for (std::size_t index = 0; index < shape.knots.size(); ++index) {
                    ...
                    const auto push_spline_handle = [&](const AnnotationSplineHandle& handle, const AnnotationHandleRole role) {
                        ...
                        push_handle(scene, selected_index, index, role, object->category_index, *point,
                                    annotation_capture_point_to_frame_unclipped(frame, *point),
