...
TEST_CASE("test_hover_prefers_higher_priority_layer", "[backend][imaging][annotation]") {
    ...
    const RectLayerFrameResult frame = update_rect_layers(state, layers.data(), layers.size(), viewport, make_pointer(12.0f, 12.0f, true, false, false));
...
TEST_CASE("test_active_layer_keeps_pointer_capture_until_release", "[backend][imaging][annotation]") {
    ...
    RectLayerState state{};
    RectLayerFrameResult frame = update_rect_layers(state, layers.data(), layers.size(), viewport, make_pointer(25.0f, 25.0f, true, true, true));
...
TEST_CASE("test_custom_white_crop_hit_metrics_support_larger_edges_and_corners", "[backend][imaging][annotation]") {
    ...
    RectDragKind hover =
        rectangle_hover_kind_with_options(make_pointer(50.0f, 27.0f, true, false, false), viewport, box, true, edge_hit_half_width, corner_hit_size);
    REQUIRE(hover == RectDragKind::Move);
    hover = rectangle_hover_kind_with_options(make_pointer(50.0f, 29.0f, true, false, false), viewport, box, true, edge_hit_half_width, corner_hit_size);
    REQUIRE(hover == RectDragKind::None);
    hover = rectangle_hover_kind_with_options(make_pointer(27.0f, 27.0f, true, false, false), viewport, box, true, edge_hit_half_width, corner_hit_size);
