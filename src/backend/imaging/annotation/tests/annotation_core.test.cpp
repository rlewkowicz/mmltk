...
AnnotationFrame make_large_frame() {
    ...
    set_annotation_frame_pixels(
        frame, std::vector<std::uint8_t>(static_cast<std::size_t>(frame.width) * static_cast<std::size_t>(frame.height) * 3U, 192U));
    return frame;
}
...
AnnotationObject make_capture_space_mask_preview_object(const std::uint64_t seed_frame_id,
                                                        const std::optional<ContentIdentity>& seed_live_frame_id) {
    AnnotationObject instance = make_mask_object(AnnotationBox{1, 1, 3, 3}, AnnotationMaskRegion{0, 0, 4, 4}, make_single_pixel_seed_mask(),
                                                 seed_frame_id, seed_live_frame_id);
...
std::vector<AnnotationResolvedObject> make_capture_space_preview_result(const AnnotationFrame& cropped, const std::uint64_t seed_frame_id,
    ...
    return resolve_annotation_objects(cropped, make_single_category("reticle"),
                                      {make_capture_space_mask_preview_object(seed_frame_id, seed_live_frame_id)}, live_mode);
}
...
std::vector<AnnotationResolvedObject> build_reticle_preview(const AnnotationFrame& frame, AnnotationObject object,
...
ReloadedScene save_and_reload_single_object(const char* temp_name, const char* split, const AnnotationFrame& frame,
                                            AnnotationCategories categories, const AnnotationObject& object,
                                            const char* expected_category_name) {
...
ANNOTATION_TEST_CASE(test_deferred_model_mask_stays_compact_during_initial_preview_resolution) {
    ...
    AnnotationObject object =
        make_mask_object(AnnotationBox{0, 0, 2, 2}, AnnotationMaskRegion{0, 0, 2, 2}, {}, cropped.frame_id, std::nullopt);
...
ANNOTATION_TEST_CASE(test_scene_round_trip_preserves_skeleton_topology) {
    ...
    const ReloadedScene scene =
        save_and_reload_single_object("mmltk-test-gui-annotation-skeleton", "val", make_large_frame(), categories, skeleton, "pose");
