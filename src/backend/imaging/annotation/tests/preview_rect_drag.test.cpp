...
ANNOTATION_TEST_CASE(test_release_rules_match_between_pointer_and_draft_paths) {
    ...
    const PreviewRectDragResult create_cancel = resolve_preview_rect_release(RectDragKind::Create, original, AnnotationBox{10, 10, 13, 13}, 100, 100, 6);
    REQUIRE(!create_cancel.commit);
    REQUIRE(create_cancel.cancel);
    const PreviewRectDragResult move_clip = resolve_preview_rect_release(RectDragKind::Move, original, AnnotationBox{-5, 10, 15, 30}, 100, 100, 1);
    ...
    const PreviewRectDragResult move_delete = resolve_preview_rect_release(RectDragKind::Move, original, AnnotationBox{-40, 10, -20, 30}, 100, 100, 1);
