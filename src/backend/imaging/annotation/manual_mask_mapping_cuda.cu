...
__global__ void deferred_manual_mask_runs_kernel(const DeferredManualMaskRunsLaunchAbi launch) {
    ...
    const std::uint32_t source_x =
        mapping.source_crop_x + project_crop_coordinate(output_x, mapping.source_crop_width, mapping.output_width);
    const std::uint32_t source_y =
        mapping.source_crop_y + project_crop_coordinate(output_y, mapping.source_crop_height, mapping.output_height);
...
cudaError_t launch_deferred_manual_mask_runs_cuda(const DeferredManualMaskRunsLaunchAbi& launch) noexcept {
    const ManualMaskMappingAbi& mapping = launch.mapping;
    if (launch.stream == nullptr || launch.overlay_region.pixels == nullptr || launch.overlay_region.width <= 0 ||
        launch.overlay_region.height <= 0 ||
        ...
        static_cast<std::uint64_t>(mapping.view_x) + launch.region_capture_x + static_cast<std::uint32_t>(launch.overlay_region.width) >
