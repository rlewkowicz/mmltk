...
[[nodiscard]] AnnotationLaunchStatus draw_deferred_manual_mask_runs_rgba_pitched(const ManualMaskMapping& mapping, std::uint8_t* overlay_region, std::size_t pitch_bytes, int width, int height,
 const std::uint32_t* run_pairs, std::uint32_t run_count, std::uint32_t region_capture_x, std::uint32_t region_capture_y, std::uint8_t red, std::uint8_t green, std::uint8_t blue, std::uint8_t alpha,
