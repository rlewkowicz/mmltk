...
void scale_point_to_box(AnnotationPoint* point, const AnnotationBox& source_box, const AnnotationBox& target_box,
    ...
    point->x = scale_axis(point->x, static_cast<float>(source_box.x1), source_width, static_cast<float>(target_box.x1), target_width,
                          capture_width);
    point->y = scale_axis(point->y, static_cast<float>(source_box.y1), source_height, static_cast<float>(target_box.y1), target_height,
                          capture_height);
}
...
bool translate_annotation_object(AnnotationObject* object, const int dx, const int dy, const std::uint32_t capture_width,
                                 const std::uint32_t capture_height, const bool clip_to_bounds) {
    ...
    if (auto* shape = std::get_if<AnnotationMaskShape>(&object->shape)) {
        ...
        if (clip_to_bounds) {
            ...
            const bool valid_mask =
                shape->mask.size() == static_cast<std::size_t>(shape->region.width) * static_cast<std::size_t>(shape->region.height);
    ...
    std::visit(
        [&](auto& shape) {
            ...
            if constexpr (std::is_same_v<T, AnnotationBoxShape>) {
                ...
                shape.mask =
                    resize_mask_nearest(shape.mask, shape.region.width, shape.region.height, next_region.width, next_region.height);
...
std::optional<AnnotationPoint> annotation_spline_segment_point(const AnnotationSplineShape& spline, const std::size_t segment_index,
                                                               const float t) {
...
bool annotation_deferred_mask_valid(const AnnotationMaskShape& shape) noexcept {
    ...
    const auto contained = [](const std::uint64_t origin, const std::uint32_t extent, const std::uint32_t limit) noexcept {
        return origin + extent <= limit;
    };
...
std::optional<std::vector<std::uint8_t>> project_annotation_mask_pixels(const AnnotationMaskShape& shape) {
    ...
    for (std::uint32_t y = 0U; y < shape.region.height; ++y) {
        ...
        const std::uint32_t source_y = std::min(
            deferred->source_height - 1U,
            deferred->source_crop_y + detail::project_crop_coordinate(output_y, deferred->source_crop_height, deferred->output_height));
        for (std::uint32_t x = 0U; x < shape.region.width; ++x) {
            ...
            const std::uint32_t source_x = std::min(
                deferred->source_width - 1U,
                deferred->source_crop_x + detail::project_crop_coordinate(output_x, deferred->source_crop_width, deferred->output_width));
            const std::uint64_t flattened = static_cast<std::uint64_t>(source_y) * deferred->source_width + source_x;
            while (run_index < deferred->runs.size() &&
                   static_cast<std::uint64_t>(deferred->runs[run_index].offset) + deferred->runs[run_index].length <= flattened) {
                ++run_index;
            }
