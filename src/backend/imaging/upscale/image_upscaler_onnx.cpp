...
class OnnxImageUpscalerRuntime final : public TiledImageUpscalerRuntimeAdapter<OnnxImageUpscalerRuntime, ImageUpscalerBackend::OnnxRuntime> {
    ...
    ImageUpscalerOutcome ActivateBackend(ImageUpscalerCurrent current) {
        ...
        try {
            ...
            mmltk::common::logging::trace([&](auto& logger) {
                logger.trace("event=image_upscaler_onnx_graph_unavailable device={} model={} detail={}", device_id, descriptor.label, error.what());
            });
            if (!current()) return ImageUpscalerOutcome::Cancelled;
        ...
        output_value_ = std::make_unique<Ort::Value>(
            Ort::Value::CreateTensor<float>(*device_memory_, output_.data(), output_elements_, kOutputShape.data(), kOutputShape.size()));
        bind_tensors();
   ...
   private:
        ...
        switch (static_cast<cudaError_t>(code)) {
            ...
            case cudaErrorNotSupported:
                return detail.find("expr=cudaStreamBeginCapture(") != std::string_view::npos ||
                       detail.find("expr=cudaGraphInstantiate(") != std::string_view::npos || detail.find("expr=cudaGraphLaunch(") != std::string_view::npos;
    ...
    [[nodiscard]] cudaError_t ReleaseBackend() noexcept {
        ...
        if (abandoned_capture_ != nullptr && cleanup_.Record(cudaGraphDestroy(abandoned_capture_), "destroy abandoned ONNX capture"))
            abandoned_capture_ = nullptr;
        if (abandoned_capture_ == nullptr) {
        ...
        if (timed) {
            ...
            mmltk::common::logging::trace([&](auto& logger) {
                logger.trace(
                    ...
                    reinterpret_cast<std::uintptr_t>(request.target_pixels), request.target_pitch * restored_height,
