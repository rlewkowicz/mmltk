    ...
    // Explicitly destroy all ORT bindings/session before the physical owner.
    {
        ...
        for (const auto geometry : std::array<std::array<std::uint32_t, 2>, 4>{{{1, 1}, {5, 3}, {197, 193}, {5, 3}}}) {
            ...
            for (int pattern = 0; pattern < 2; ++pattern) {
                ...
                REQUIRE(cudaMemcpy2DAsync(source.as<void>(), source_pitch, pixels.data(), width * 4U, width * 4U, height, cudaMemcpyHostToDevice,
                ...
                for (std::uint32_t y = 0; y < height; y += 192U) {
                    for (std::uint32_t x = 0; x < width; x += 192U) {
                        const tiles::Tile tile{
                            .origin_x = x, .origin_y = y, .core_width = std::min(192U, width - x), .core_height = std::min(192U, height - y)};
                        tiles::prepare_tile(source.as<std::uint8_t>(), source_pitch, width, height, 0, 0, width, height, tile, true, 32U, input.as<float>(),
                                            stream.get());
