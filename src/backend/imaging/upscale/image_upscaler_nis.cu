...
[[nodiscard]] bool valid_configuration(const Configuration& config) noexcept {
    if (config.source_width == 0U || config.source_height == 0U || config.crop_width == 0U || config.crop_height == 0U ||
        config.output_width == 0U || config.output_height == 0U || config.crop_x >= config.source_width ||
        config.crop_y >= config.source_height || config.crop_width > config.source_width - config.crop_x ||
...
__global__ void horizontal_kernel(const void* source, const std::size_t source_pitch, HalfRgba* horizontal, const Configuration config) {
    ...
    const float source_x =
        (static_cast<float>(output_x) + 0.5F) * static_cast<float>(config.crop_width) / static_cast<float>(config.output_width) - 0.5F;
...
__global__ void vertical_kernel(const HalfRgba* horizontal, HalfRgba* scaled, const Configuration config) {
    ...
    const float source_y =
        (static_cast<float>(output_y) + 0.5F) * static_cast<float>(config.crop_height) / static_cast<float>(config.output_height) - 0.5F;
    const int base = static_cast<int>(floorf(source_y)) - 2;
    const std::uint32_t source_phase = device::phase(source_y);
    float3 color = make_float3(0.0F, 0.0F, 0.0F);
#pragma unroll
    for (std::uint32_t tap = 0U; tap < device::kFilterTaps; ++tap) {
        const std::uint32_t source_y_index =
            static_cast<std::uint32_t>(max(0, min(static_cast<int>(config.crop_height) - 1, base + static_cast<int>(tap))));
...
__global__ void sharpen_kernel(const void* source, const std::size_t source_pitch, const HalfRgba* scaled, std::uint8_t* target,
    ...
    const float source_x =
        (static_cast<float>(x) + 0.5F) * static_cast<float>(config.crop_width) / static_cast<float>(config.output_width) - 0.5F;
    const float source_y =
        (static_cast<float>(y) + 0.5F) * static_cast<float>(config.crop_height) / static_cast<float>(config.output_height) - 0.5F;
...
#pragma unroll
    ...
    const float horizontal_gradient = fabsf(0.2126F * (__half2float(left.red) - __half2float(right.red)) +
                                            0.7152F * (__half2float(left.green) - __half2float(right.green)) +
                                            0.0722F * (__half2float(left.blue) - __half2float(right.blue)));
    const float vertical_gradient = fabsf(0.2126F * (__half2float(top.red) - __half2float(bottom.red)) +
                                          0.7152F * (__half2float(top.green) - __half2float(bottom.green)) +
    ...
    const float4 color =
        make_float4(fmaf(strength, detail.x, __half2float(center.red)), fmaf(strength, detail.y, __half2float(center.green)),
...
std::optional<ScratchRequirements> scratch_requirements(const Configuration& config) noexcept {
    ...
    if (!checked_elements(config.output_width, config.crop_height, horizontal_elements) ||
        !checked_elements(config.output_width, config.output_height, output_elements))
        return std::nullopt;
    return ScratchRequirements{.horizontal_bytes = horizontal_elements * sizeof(HalfRgba),
                               .scaled_bytes = output_elements * sizeof(HalfRgba)};
}
...
cudaError_t launch_scale(const void* source, const std::size_t source_pitch, void* horizontal, void* scaled, const Configuration& config,
    ...
    vertical_kernel<<<static_cast<unsigned int>((output_elements + kThreads - 1U) / kThreads), kThreads, 0U, stream>>>(
        static_cast<const HalfRgba*>(horizontal), static_cast<HalfRgba*>(scaled), config);
    return cudaPeekAtLastError();
}

cudaError_t launch_sharpen(const void* source, const std::size_t source_pitch, const void* scaled, std::uint8_t* target,
