...
[[nodiscard]] inline Tile prepare_request_tile(const ImageUpscalerRequest& request, const std::uint32_t x, const std::uint32_t y,
    ...
    prepare_tile(request.device_pixels, request.source_width, request.source_height, request.crop_x, request.crop_y, request.crop_width,
...
inline void stitch_request_tile(const float* output, const Tile tile, const ImageUpscalerKind kind, const std::uint32_t halo,
                                float* restored, const std::uint32_t restored_width, const std::uint32_t restored_height,
                                const cudaStream_t stream) {
    stitch_tile(output, tile, kind == ImageUpscalerKind::ShiftLUT, halo, restored, restored_width, restored_height, stream);
}
