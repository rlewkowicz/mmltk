...
public:
 explicit FileLock(const std::filesystem::path& path) : descriptor_(::open(path.c_str(), O_CREAT | O_RDWR | O_CLOEXEC, 0664)) {
  if (descriptor_.get() < 0) { throw std::runtime_error("failed to open Image upscaler cache lock " + path.string() + ": " + std::strerror(errno)); }
 }
 [[nodiscard]] ImageUpscalerOutcome Acquire(ImageUpscalerCurrent current, const ImageUpscalerExecutionCheckpoint& checkpoint) {
  while (current()) {
   ...
   if (error != EWOULDBLOCK && error != EINTR) throw std::runtime_error(std::string("failed to lock Image upscaler cache: ") + std::strerror(error));
   if (!image_upscaler_admitted(checkpoint, ImageUpscalerExecutionStage::CacheLockWaiting, current)) return ImageUpscalerOutcome::Cancelled;
...
struct ImageUpscaler::Impl {
 ...
 struct DefaultModel final {
  ...
  ~DefaultModel() noexcept {
   if (stream != nullptr || terminal_event != nullptr || horizontal != nullptr || scaled != nullptr || orphaned != nullptr) std::terminate();
  }
  ...
  [[nodiscard]] ImageUpscalerOutcome run(const std::uint8_t* source, const std::size_t source_pitch, const std::uint32_t width, const std::uint32_t height, std::uint8_t* target,
   ...
   try {
    ...
    if (!image_upscaler_admitted(checkpoint, ImageUpscalerExecutionStage::BasicAllocationAdmitted, current)) return ImageUpscalerOutcome::Cancelled;
    ...
    if (!image_upscaler_admitted(checkpoint, ImageUpscalerExecutionStage::BasicLaunchAdmitted, current)) return ImageUpscalerOutcome::Cancelled;
    ensure_cuda_ok(image_upscaler_nis::launch_sharpen(source, source_pitch, scaled, target, target_pitch, config, stream), "launch Basic sharpening");
  ...
  [[nodiscard]] cudaError_t Stop() noexcept {
   ...
   for (void** allocation : {&horizontal, &scaled, &orphaned}) {
    if (*allocation != nullptr && cleanup.Record(cudaFreeAsync(*allocation, stream), "release Basic scratch")) *allocation = nullptr;
   }
 ...
 private:
  [[nodiscard]] cudaError_t select_device() const noexcept { return owned_device >= 0 ? cudaSetDevice(owned_device) : cudaErrorInvalidDevice; }
 ...
 explicit Impl(const std::uint64_t generation_in, const std::int32_t device_id_in, const std::uint64_t device_generation_in, const ImageUpscalerAggregateConfig& config)
  ...
  if (config.models[0U].device_id != device_id || config.models[0U].backend_generation != device_generation) throw ImageUpscalerStartError::DefaultNis;
  if (config.models[1U].device_id != device_id || config.models[1U].backend_generation != device_generation) throw ImageUpscalerStartError::ShiftLut;
  if (config.models[2U].device_id != device_id || config.models[2U].backend_generation != device_generation) throw ImageUpscalerStartError::RealPlksr;
 }
 ~Impl() {
 ...
 [[nodiscard]] cudaError_t Stop() noexcept {
  ...
  for (const ImageUpscalerKind kind : {ImageUpscalerKind::ShiftLUT, ImageUpscalerKind::RealPLKSR}) {
   ...
   if (target.runtime != nullptr) {
    ...
    runtime_cleanup_failure = mmltk::frameworks::gpu::combine_image_failures(runtime_cleanup_failure, target.runtime->cleanup_failure());
   }
 ...
 ImageUpscalerOutcome verify_model(const ImageUpscalerDescriptor& descriptor, ImageUpscalerCurrent current) {
  ...
  if (!image_upscaler_admitted(checkpoint, ImageUpscalerExecutionStage::ChecksumAdmitted, current)) return ImageUpscalerOutcome::Cancelled;
  const std::filesystem::path path = image_upscaler_model_path(descriptor);
  const auto digest = mmltk::common::io::try_sha256_file(path, [&] { return cancellation.cancelled(); });
 ...
 ImageUpscalerOutcome initialize_tensorrt(Slot& target, ImageUpscalerCurrent current) {
  ...
  if (!image_upscaler_admitted(checkpoint, ImageUpscalerExecutionStage::CacheLockAdmitted, current)) return ImageUpscalerOutcome::Cancelled;
  ...
  if (std::filesystem::is_regular_file(engine_path)) {
   ...
   if (!std::filesystem::remove(engine_path, removal_error) || removal_error) {
    throw std::runtime_error("failed to remove invalid Image upscaler engine cache " + engine_path.string() + ": " + removal_error.message());
   }
  }
  ...
  if (!image_upscaler_admitted(checkpoint, ImageUpscalerExecutionStage::BuildAdmitted, current)) return ImageUpscalerOutcome::Cancelled;
  try {
...
struct ImageUpscaler::Resolver final {
 ...
 [[nodiscard]] static std::optional<Coordinate> register_owner(const std::shared_ptr<Impl>& owner, const std::uint64_t core_generation) noexcept {
  ...
  for (std::size_t index = 0U; index < resolver.slots.size(); ++index) {
   ...
   if (registered == owner) return Coordinate{.slot = static_cast<std::uint8_t>(index), .service_generation = slot.service_generation};
  }
  for (std::size_t index = 0U; index < resolver.slots.size(); ++index) {
 ...
 [[nodiscard]] static bool seal(const std::uint8_t service_slot, const std::uint64_t service_generation, const std::uint64_t core_generation, const std::shared_ptr<Impl>& expected) noexcept {
  ...
  if (!slot.live || slot.service_generation != service_generation || slot.core_generation != core_generation || owner != expected) return false;
...
std::expected<std::unique_ptr<ImageUpscaler>, ImageUpscalerStartError> ImageUpscaler::Create(
 ...
 if (generation == 0U || generation == std::numeric_limits<std::uint64_t>::max()) return std::unexpected(ImageUpscalerStartError::Registration);
 try {
...
std::expected<void, ImageUpscalerStartError> ImageUpscaler::Activate() noexcept {
 ...
 client_ = ImageUpscalerClient{coordinate->slot, coordinate->service_generation, generation_, owner_->device_id, owner_->device_generation};
 owner_->in_flight->open();
 return {};
}
ImageUpscaler::~ImageUpscaler() = default;
bool ImageUpscalerClient::valid() const noexcept {
 if (service_slot_ == kInvalidService || service_generation_ == 0U || core_generation_ == 0U || device_id_ < 0 || device_generation_ == 0U) return false;
...
ImageUpscalerProcessOwner ImageUpscalerClient::ClaimOperation() const noexcept {
 if (service_slot_ == kInvalidService || service_generation_ == 0U || core_generation_ == 0U || device_id_ < 0 || device_generation_ == 0U) return {};
 std::shared_ptr<ImageUpscaler::Impl> owner = ImageUpscaler::Resolver::resolve(service_slot_, service_generation_, core_generation_);
 if (owner == nullptr || owner->device_id != device_id_ || owner->device_generation != device_generation_ || !owner->in_flight->acquire()) return {};
...
ImageUpscalerOutcome ImageUpscalerProcessOwner::run_rgba8(const ImageUpscalerModelHandle handle, const ImageUpscalerMode mode, const std::uint8_t* source, const std::size_t source_pitch,
 ...
 try {
  if (slot.runtime == nullptr) {
   ...
   const auto initialized = kind == ImageUpscalerKind::ShiftLUT ? owner->initialize_onnx(slot, current) : owner->initialize_tensorrt(slot, current);
  ...
  if (output.device_pixels == nullptr || output.width != width * 4U || output.height != height * 4U) throw std::runtime_error("invalid Upscale runtime output");
  slot.runtime->mark_consumed(stream);
...
std::uintptr_t ImageUpscalerProcessOwner::operation_stream(const ImageUpscalerMode mode, const int device_id, ImageUpscalerCurrent current) {
...
ImageUpscalerClient ImageUpscaler::client() const noexcept { return owner_ != nullptr && owner_->generation == generation_ ? client_ : ImageUpscalerClient{}; }
ImageUpscalerStatus ImageUpscaler::Stop() noexcept {
 ...
 if (activated) {
  if (!Resolver::seal(client_.service_slot_, client_.service_generation_, client_.core_generation_, owner_)) return cudaErrorInvalidResourceHandle;
