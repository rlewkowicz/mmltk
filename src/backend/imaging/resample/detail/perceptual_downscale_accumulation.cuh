...
__device__ inline void accumulate_sample(RgbConstImageView source, const TransferTable& transfer, Footprint fx, Footprint fy,
                                         std::uint32_t x, std::uint32_t y, MomentAccumulator& sum, float& alpha, float& alpha_error) {
...
template <RgbPixelFormat Format, bool Integer>
__device__ inline void accumulate_strided(RgbConstImageView source, const TransferTable& transfer, Footprint fx, Footprint fy, std::size_t offset, std::size_t step,
...
template <RgbPixelFormat Format, bool Integer>
__device__ inline void accumulate_serial(RgbConstImageView source, const TransferTable& transfer, Footprint fx, Footprint fy,
                                         MomentAccumulator& sum, float& alpha) {
    float alpha_error = 0;
    for (auto y = fy.first; y < fy.end; ++y)
        for (auto x = fx.first; x < fx.end; ++x)
            accumulate_sample<Format, Integer>(source, transfer, fx, fy, x, y, sum, alpha, alpha_error);
    if constexpr (Format == RgbPixelFormat::RGBA8) alpha = sum.weight > 0 ? (alpha - alpha_error) / sum.weight : 0;
}
}  // namespace mmltk::backend::imaging::resample::perceptual
