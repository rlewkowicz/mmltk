...
__device__ inline void accumulate_sample(RgbConstImageView source, const TransferTable& transfer, Footprint fx, Footprint fy, std::uint32_t x, std::uint32_t y,
                                         MomentAccumulator& sum, float& alpha, float& alpha_error) {
...
template <RgbPixelFormat Format, bool Integer>
__device__ inline void accumulate_strided(RgbConstImageView source, const TransferTable& transfer, Footprint fx, Footprint fy, std::size_t offset,
                                          std::size_t step, MomentAccumulator& sum, float& alpha) {
...
template <RgbPixelFormat Format, bool Integer>
__device__ inline void accumulate_serial(RgbConstImageView source, const TransferTable& transfer, Footprint fx, Footprint fy, MomentAccumulator& sum,
                                         float& alpha) {
 float alpha_error = 0;
