...
struct DriverAdmission {
 ...
 bool caller_rejection() const noexcept {
  ...
  return (query == AdmissionQuery::Range && status == CUDA_ERROR_NOT_FOUND) || (query == AdmissionQuery::Stream && status == CUDA_ERROR_INVALID_HANDLE);
 }
...
public:
 explicit CompletionAllocationFailure(cudaError_t status) : std::runtime_error("perceptual CUDA completion allocation failed"), status_(status) {}
...
public:
 ...
 Submission& reserve(cudaStream_t stream, std::shared_ptr<const void> source, std::shared_ptr<const void> destination) {
  ...
  for (auto& slot : slots_) {
   ...
   slot.destination = std::move(destination);
   slot.stream = stream;
   slot.pending = true;
   slot.recorded = false;
   return slot;
 ...
 cudaError_t order(const Submission& slot) noexcept { return latest_ && latest_stream_ != slot.stream ? api_.order(slot.stream, latest_, 0) : cudaSuccess; }
 ...
 cudaError_t settle(bool wait) noexcept {
  for (auto& slot : slots_) {
   if (!slot.pending) continue;
   const auto status = slot.recorded ? (wait ? api_.wait(slot.event) : api_.query(slot.event)) : (wait ? api_.stream_wait(slot.stream) : cudaErrorNotReady);
   if (status == cudaErrorNotReady && !wait) continue;
   if (status != cudaSuccess) return status;
   slot.pending = false;
   slot.recorded = false;
   slot.source.reset();
   slot.destination.reset();
  }
  return cudaSuccess;
 }
 void forget_order() noexcept {
  latest_ = nullptr;
  latest_stream_ = nullptr;
 }
