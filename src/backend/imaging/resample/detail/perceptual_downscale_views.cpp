...
IdentityCopyGeometry identity_geometry(const RgbImageLayout& layout) {
 switch (layout.format) {
  ...
  case RgbPixelFormat::RGBA8: return {common::math::checked_multiply<std::size_t>(layout.width, 4, "perceptual image extent overflow"), 1};
  case RgbPixelFormat::PlanarUnitSrgbF32: return {common::math::checked_multiply<std::size_t>(layout.width, sizeof(float), "perceptual image extent overflow"), 3};
 }
...
std::size_t validate_view(const void* pointer, const RgbImageLayout& l) {
 ...
 const auto preceding_rows = common::math::checked_multiply<std::size_t>(l.height - 1, l.row_stride_bytes, "perceptual image extent overflow");
 ...
 if (l.format == RgbPixelFormat::PlanarUnitSrgbF32) {
  if (reinterpret_cast<std::uintptr_t>(pointer) % alignof(float) || l.row_stride_bytes % sizeof(float) || l.plane_stride_bytes % sizeof(float) || l.plane_stride_bytes < extent)
   throw std::invalid_argument("perceptual float planes overlap or are unaligned");
  const auto preceding_planes = common::math::checked_multiply<std::size_t>(2, l.plane_stride_bytes, "perceptual image extent overflow");
  extent = common::math::checked_add(preceding_planes, extent, "perceptual image offset overflow");
 } else if (l.plane_stride_bytes)
  throw std::invalid_argument("packed perceptual image has a plane stride");
 if (extent > l.capacity_bytes) throw std::invalid_argument("perceptual image storage is too small");
 if (extent > std::numeric_limits<std::uintptr_t>::max() - reinterpret_cast<std::uintptr_t>(pointer)) throw std::overflow_error("perceptual image address overflow");
 return extent;
}
ValidatedResize validate_pair(RgbConstImageView source, RgbMutableImageView destination, const bool quantized_planar) {
 ...
 if (quantized_planar ? (source.layout.format != RgbPixelFormat::RGB8 || destination.layout.format != RgbPixelFormat::PlanarUnitSrgbF32) : source.layout.format != destination.layout.format)
  throw std::invalid_argument("perceptual image formats must match");
 if (source.layout.width < destination.layout.width || source.layout.height < destination.layout.height) throw std::invalid_argument("perceptual resampling cannot enlarge images");
 ...
 if (a < b + destination_bytes && b < a + source_bytes &&
     !(identity && a == b && source.layout.row_stride_bytes == destination.layout.row_stride_bytes && source.layout.plane_stride_bytes == destination.layout.plane_stride_bytes))
