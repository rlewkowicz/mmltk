...
struct ForeignStream {
 ...
 explicit ForeignStream(const gpu::DeviceExecution& execution) : context(0, gpu::cuda_image_copy_backend(), gpu::DeviceContextMode::Isolated, -1, execution) {
  context.Bind();
...
TEST_CASE("CUDA perceptual admission proves device intervals and stream identity", "[backend][data][image_resize][perceptual][cuda]") {
 ...
 const auto reject = [&](RgbConstImageView source_view, RgbMutableImageView output_view, cudaStream_t stream, const std::shared_ptr<const void>& source_owner) {
...
TEST_CASE("perceptual event allocation preserves status and existing custody before new admission", "[backend][data][image_resize][perceptual]") {
 for (const auto status : {cudaErrorMemoryAllocation, cudaErrorContextIsDestroyed, cudaErrorCudartUnloading, cudaErrorIllegalAddress, cudaErrorLaunchFailure}) {
  FakeCompletion fake;
  perceptual::CudaDownscaleCompletion completion(fake.api());
