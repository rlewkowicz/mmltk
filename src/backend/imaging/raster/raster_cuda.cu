...
__global__ void scale_rgba_kernel(const draw_launch::ScaleRgbaLaunch launch) {
 ...
 if (launch.bilinear) {
  const float sx = fmaxf(0.0F, (static_cast<float>(x) + 0.5F) * static_cast<float>(launch.source.width) / static_cast<float>(launch.target.width) - 0.5F);
  const float sy = fmaxf(0.0F, (static_cast<float>(y) + 0.5F) * static_cast<float>(launch.source.height) / static_cast<float>(launch.target.height) - 0.5F);
  ...
  auto* target = launch.target.pixels + static_cast<std::size_t>(y) * launch.target.pitch_bytes + static_cast<std::size_t>(x) * 4U;
  const auto* first = launch.source.pixels + static_cast<std::size_t>(y0) * launch.source.pitch_bytes;
  const auto* second = launch.source.pixels + static_cast<std::size_t>(y1) * launch.source.pitch_bytes;
  const auto left = static_cast<std::size_t>(x0) * 4U;
  const auto right = static_cast<std::size_t>(x1) * 4U;
  for (std::size_t channel = 0U; channel < 4U; ++channel) {
   const float top = static_cast<float>(first[left + channel]) * (1.0F - fx) + static_cast<float>(first[right + channel]) * fx;
   const float bottom = static_cast<float>(second[left + channel]) * (1.0F - fx) + static_cast<float>(second[right + channel]) * fx;
   target[channel] = static_cast<std::uint8_t>(roundf(top * (1.0F - fy) + bottom * fy));
  }
  return;
 }
...
MMLTK_DRAW_CUDA_DEFINE_LAUNCHER(launch_composite_rgba, draw_launch::CompositeRgbaLaunchAbi) {
 ...
 if (launch.base_rgba.kind == draw_launch::RgbaTargetKindAbi::Pitched) {
  return launch_composite_rgba_over_rgba_pitched(draw_launch::CompositeRgbaOverRgbaPitchedLaunch{launch.base_rgba.pitched, launch.overlay_rgba, launch.stream});
 }
 const dim3 block = draw_kernel_block();
 const dim3 grid = draw_kernel_grid(launch.base_rgba.width, launch.base_rgba.height);
...
MMLTK_DRAW_CUDA_DEFINE_LAUNCHER(launch_copy_bgr_to_rgba, draw_launch::CopyBgrToRgbaLaunchAbi) {
 ...
 if (launch.target_rgba.kind == draw_launch::RgbaTargetKindAbi::Pitched) {
  return launch_copy_bgr_to_rgba_pitched(draw_launch::CopyBgrToRgbaPitchedLaunch{launch.source_bgr, launch.target_rgba.pitched, launch.alpha, launch.stream});
 }
 const dim3 block = draw_kernel_block();
 const dim3 grid = draw_kernel_grid(launch.source_bgr.width, launch.source_bgr.height);
