...
__global__ void pack_bool_masks_kernel(const bool* masks, std::uint8_t* packed_masks, std::int64_t mask_count, std::int64_t pixels_per_mask, std::int64_t bytes_per_mask) {
 ...
 if (output_index >= total_bytes) { return; }
...
#pragma unroll
 for (int bit = 0; bit < 8; ++bit) {
  ...
  if (pixel_index < pixels_per_mask && source[pixel_index]) { packed |= static_cast<std::uint8_t>(1U << bit); }
 }
 packed_masks[output_index] = packed;
}
}  // namespace
cudaError_t launch_pack_bool_masks_cuda(const PackBoolMasksLaunch& launch) noexcept {
 ...
 pack_bool_masks_kernel<<<blocks, kCudaThreads, 0, launch.stream>>>(launch.masks, launch.packed_masks, launch.mask_count, launch.pixels_per_mask, launch.bytes_per_mask);
