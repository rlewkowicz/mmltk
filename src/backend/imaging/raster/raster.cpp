...
[[nodiscard]] bool drawable_points(const MutableBytes& overlay, const PointBuffer& points, const int minimum_count, const int extent,
 ...
 return overlay.valid(4U) && points.points_xy != nullptr && points.point_count >= minimum_count && points.point_count <= std::numeric_limits<int>::max() / 2 &&
        squared_extent_fits(extent) && static_cast<bool>(stream);
}
}  // namespace
...
std::int32_t raster_mask_boxes_bgr(const MaskBoxLabelBgrWork& work) noexcept {
 if (work.instances.instance_count == 0) return cudaSuccess;
 if (!work.image.valid(3U) || !mask_inputs_valid(work.instances) || work.mask_alpha < 0.0F || work.mask_alpha > 1.0F || work.box_thickness <= 0 ||
     !work.stream) {
  return cudaErrorInvalidValue;
 }
...
std::int32_t raster_instance_overlay_rgba(const InstanceOverlayRgbaWork& work) noexcept {
 ...
 const MaskBoxLabelInputs launch_instances{
  work.masks, work.instances.boxes, work.instances.colors, work.instances.labels, work.instances.instance_count, work.instances.device_instance_count};
...
std::int32_t finalize_rgba(const FinalizeRgbaWork& work) noexcept {
 ...
 const auto submit = [&](IntRect region) {
  ...
  return detail::launch_finalize_rgba(
   {as_launch_surface(work.clean), as_launch_surface(work.semantic), as_launch_surface(work.destination), region, as_stream(work.stream)});
 };
...
std::int32_t raster_mask_rgba(const MaskRgbaWork& work) noexcept {
 if (!writable_rgba(work.overlay, work.stream) || work.mask == nullptr || !indexed_instances_fit(1, work.overlay.width, work.overlay.height)) {
  return cudaErrorInvalidValue;
 }
...
std::int32_t raster_mask_runs_rgba(const MaskRunsRgbaWork& work) noexcept {
 ...
 if ((work.scale_x != 1 || work.scale_y != 1 || work.source_x != work.target_x || work.source_y != work.target_y) &&
  ...
  return cudaErrorInvalidValue;
 return detail::launch_draw_manual_mask_runs_rgba_pitched({as_launch_surface(work.overlay), work.run_pairs, work.run_count, work.color, as_stream(work.stream),
                                                           work.clip, work.source_x, work.source_y, work.target_x, work.target_y, work.scale_x, work.scale_y});
}
