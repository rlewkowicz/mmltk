...
__global__ void convert_rgb8(const std::uint8_t* source, std::uint32_t width, std::uint32_t height, std::uint8_t* destination, std::size_t pitch) {
 ...
 auto* pixel = destination + y * pitch + static_cast<std::size_t>(x) * 4U;
 for (unsigned channel = 0U; channel < 3U; ++channel) pixel[channel] = source[offset + channel];
 pixel[3] = 255U;
}
}  // namespace
int chw_float_to_rgba(const float* source, std::uint32_t width, std::uint32_t height, std::uint8_t* destination, std::size_t pitch, cudaStream_t stream) noexcept {
...
int rgb8_to_rgba(const std::uint8_t* source, std::uint32_t width, std::uint32_t height, std::uint8_t* destination, std::size_t pitch, cudaStream_t stream) noexcept {
 if (!source || !destination || !width || !height || pitch < static_cast<std::size_t>(width) * 4U) return cudaErrorInvalidValue;
 convert_rgb8<<<dim3((width + 15U) / 16U, (height + 15U) / 16U), dim3(16U, 16U), 0U, stream>>>(source, width, height, destination, pitch);
 return cudaGetLastError();
}
}  // namespace mmltk::backend::imaging::raster
