   ...
   public:
    InferenceBatchPreprocessor(const std::int64_t capacity, const int height, const int width, const int device,
        ...
        : capacity_(capacity),
          ...
          width_(width),
          device_(device),
          output_(type == at::kFloat
                      ? torch::Tensor{}
                      : torch::empty(
                            {capacity, 3, height, width},
                            torch::TensorOptions().dtype(type).device(
                                torch::kCUDA, static_cast<c10::DeviceIndex>(device)))) {}
    [[nodiscard]] torch::Tensor Run(const mmltk::backend::data::Batch& batch) {
        ...
        const auto input = torch::from_blob(
            const_cast<float*>(batch.device_images), at::IntArrayRef{shape},
            torch::TensorOptions()
                .dtype(at::kFloat)
                .device(torch::kCUDA, static_cast<c10::DeviceIndex>(device_)));
