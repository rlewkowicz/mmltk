...
struct PredictionCapacity final {
    ...
    [[nodiscard]] static PredictionCapacity Resolve(std::size_t requested, std::size_t batch, std::int64_t queries,
                                                    std::int64_t classes, std::uint32_t width, std::uint32_t height, bool masks, std::optional<std::size_t> eligible_classes = std::nullopt) {
        ...
        const auto eligible_candidates = eligible == 0 ? 0U : checked_prediction_extent(static_cast<std::size_t>(queries), eligible,
            static_cast<std::size_t>(std::numeric_limits<std::int64_t>::max()));
