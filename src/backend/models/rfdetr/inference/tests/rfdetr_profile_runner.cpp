...
void print_iteration_line(const char* phase, int index, int total, const EvaluateRun& run) {
    std::printf("%s=rfdetr.evaluate.checkpoint %d/%d pt=%.3fs bbox=%.4f mask=%s\n", phase, index, total, seconds_from_ns(run.elapsed_ns),
                run.bbox_ap, optional_metric_text(run.mask_ap).c_str());
    std::fflush(stdout);
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        run_profile_phases(
            "rfdetr.evaluate.checkpoint", options.warmup_runs, options.repetitions,
            [&options](bool, int) { return run_checkpoint_backend(options); }, record_evaluate_metrics, print_iteration_line);
