...
class PredictionRawPreparation final {
   public:
    ...
    PredictionRawPreparation(bool requested,
        mmltk::backend::ml::runtime::AnalysisAnnotationStorage& annotation, std::string& failure,
        cudaStream_t stream, Settle settle = &cudaStreamSynchronize) noexcept
    ...
    void Execute(Operation&& operation) {
        ...
        try { std::forward<Operation>(operation)(); }
        catch (...) {
            ...
            if (status != cudaSuccess)
                throw mmltk::backend::ml::runtime::CudaOperationError{status, "prediction raw preparation settlement"};
