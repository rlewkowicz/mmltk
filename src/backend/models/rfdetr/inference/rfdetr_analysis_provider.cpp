...
struct RfdetrAnalysisProvider::Impl final {
    explicit Impl(const RfdetrAnalysisOptions& options)
        ...
        if (backend->input_element_type() == element_type::Float16) {
            input_runtime = torch::empty_like(input_float, input_float.options().dtype(at::kHalf));
        }
    }

    [[nodiscard]] c10::cuda::CUDAStream cuda_stream() const noexcept {
        return c10::cuda::getStreamFromExternal(stream, c10::DeviceIndex(device));
    }
...
RfdetrAnalysisProvider::ProviderWorkResult RfdetrAnalysisProvider::DoAnalyze(const runtime::AnalysisRequest& request) noexcept {
    try {
        if (request.source.device != impl_->device || request.source.channels != 3U ||
            request.regions.size() > runtime::kMaximumAnalysisRegions || impl_->active_submission.has_value()) {
        ...
        mmltk::frameworks::gpu::ensure_cuda_ok(
            cudaStreamWaitEvent(impl_->stream, reinterpret_cast<cudaEvent_t>(request.source_ready.event), 0U),
        ...
        for (std::size_t index = 0U; index < count; ++index) {
            ...
            const auto* source = reinterpret_cast<const std::uint8_t*>(request.source.pixels.address) +
                                 static_cast<std::size_t>(region.y) * request.source.pitch_bytes +
                                 static_cast<std::size_t>(region.x) * request.source.channels;
            mmltk::frameworks::gpu::ensure_cuda_ok(
                static_cast<cudaError_t>(mmltk::backend::ml::cuda::launch_bgr_split_to_planar_float(
        ...
        for (std::size_t axis = 0U; axis < 4U; ++axis) {
            shape.extents[axis] = input.size(static_cast<std::int64_t>(axis));
        }
        const auto element_type =
            input.scalar_type() == at::kHalf ? runtime::RuntimeElementType::Float16 : runtime::RuntimeElementType::Float32;
        impl_->active_submission.emplace(
            impl_->backend->Run({.device_data = input.data_ptr(),
        ...
        return {
            ...
            .completion = {.device = submission.device(),
                           .event = submission.completion_event(),
                           .producer_stream = submission.stream().native_handle},
        };
    } catch (...) {
        impl_->active_submission.reset();
        for (auto& annotation : request.annotations) {
