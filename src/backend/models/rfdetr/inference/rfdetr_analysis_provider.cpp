...
struct RfdetrAnalysisProvider::Impl final {
 explicit Impl(const RfdetrAnalysisOptions& options)
  ...
  preprocessor =
   std::make_unique<GpuBatchPreprocessor>(runtime::kMaximumAnalysisRegions, resolution, resolution, device, backend->input_element_type() == element_type::Float16 ? at::kHalf : at::kFloat);
 }
...
RfdetrAnalysisProvider::ProviderWorkResult RfdetrAnalysisProvider::DoAnalyze(const runtime::AnalysisRequest& request) noexcept {
 try {
  ...
  mmltk::frameworks::gpu::ensure_cuda_ok(cudaStreamWaitEvent(impl_->stream, reinterpret_cast<cudaEvent_t>(request.source_ready.event), 0U), "RF-DETR analysis source readiness");
  ...
  for (std::size_t index = 0U; index < count; ++index) {
   ...
   mmltk::frameworks::gpu::ensure_cuda_ok(static_cast<cudaError_t>(mmltk::backend::imaging::raster::launch_bgr_split_to_planar_float(source, request.source.pitch_bytes, region.width, region.height,
                                           input_float[index].data_ptr<float>(), impl_->resolution, impl_->resolution, reinterpret_cast<std::uintptr_t>(impl_->stream))),
  ...
  impl_->active_submission.emplace(
   impl_->backend->Run({.device_data = input.data_ptr(), .capacity_bytes = static_cast<std::size_t>(input.numel() * input.element_size()), .shape = shape, .element_type = element_type},
...
bool RfdetrAnalysisProvider::ObserveCompletion(const runtime::AnalysisCompletion& completion) noexcept {
 ...
 const auto& submission = *impl_->active_submission;
 if (submission.device() != completion.device || submission.stream().native_handle != completion.producer_stream || submission.completion_event() != completion.event) { return false; }
 try {
