...
class MatcherAccess final {
   public:
    explicit MatcherAccess(const torch::Device& device) {
        ...
        const auto placement = device.is_cuda()
...
torch::Tensor matcher_point_sample(const torch::Tensor& input, const torch::Tensor& point_coords, F::GridSampleFuncOptions::mode_t mode) {
    if (input.is_cuda() && input.scalar_type() == torch::kFloat32 && point_coords.scalar_type() == torch::kFloat32) {
        return matcher_point_sample_cuda_forward(input.contiguous(), point_coords.contiguous(), std::holds_alternative<torch::enumtype::kNearest>(mode));
    }
...
torch::Tensor position_supervised_loss(const torch::Tensor& inputs, const torch::Tensor& targets, double num_boxes, double alpha, double gamma,
                                       bool use_jit_traced_loss_ops) {
    return run_parametric_traced_or_direct(&TracedLossOpCache::position_supervised, "__torch__.NativeRfDetrPositionSupervisedLoss", inputs, targets, alpha,
                                           gamma, use_jit_traced_loss_ops,
                                           [alpha, gamma](const torch::Tensor& a, const torch::Tensor& b) {
                                               const auto prob = a.sigmoid();
...
torch::Tensor ia_bce_loss(const torch::Tensor& inputs, const torch::Tensor& pos_weights, const torch::Tensor& neg_weights, double num_boxes,
    ...
    return run_ternary_traced_or_direct(
               ...
               [](const torch::Tensor& a, const torch::Tensor& b, const torch::Tensor& c) { return (c * a - F::logsigmoid(a) * (b + c)).sum(); }, inputs,
...
torch::Tensor matched_pred_masks(const OutputLayer& layer, const MatcherLayerIndices& indices) {
    ...
    if (!layer.sparse_pred_masks.has_value()) { throw std::runtime_error("native RF-DETR mask projection requires pred_masks in the model outputs"); }
    ...
    while (begin < batch_indices_cpu.numel()) {
        ...
        int64_t end = begin + 1;
        while (end < batch_indices_cpu.numel() && batch_indices[end] == batch_index) { ++end; }
...
MatchIndices empty_matcher_indices(const PreparedTargets& targets) {
    ...
    for (size_t index = 0; index < targets.targets.size(); ++index) { empty.emplace_back(torch::empty({0}, options), torch::empty({0}, options)); }
    return empty;
}
...
torch::Tensor build_dense_matcher_cost(const OutputLayer& layer, const PreparedTargets& targets, const DetectionConfig& config) {
    ...
    torch::Tensor cost_giou;
    {
        ...
        cost_giou =
            -pairwise_generalized_box_iou(box_cxcywh_to_xyxy(out_bbox, BoxExtentPolicy::Preserve), box_cxcywh_to_xyxy(tgt_bbox, BoxExtentPolicy::Preserve));
    }
...
void solve_matcher_indices_for_batch_cpu(const torch::Tensor& batch_cost_cpu, int64_t group_detr, LsapScratch& scratch, MatchIndices::value_type& result) {
    ...
    scratch.batch_cols.clear();
    {
        ...
        for (int64_t group_index = 0; group_index < group_detr; ++group_index) {
            ...
            for (const int64_t row : scratch.row_indices) { scratch.batch_rows.push_back(row + group_index * group_queries); }
...
std::vector<MatchIndices> compute_matcher_indices_for_layers(const std::vector<const OutputLayer*>& layers, const PreparedTargets& targets,
    ...
    for (const auto& layer : layers) {
        ...
        if (layer->pred_logits.size(0) != bs) { throw std::runtime_error("native RF-DETR batched matcher requires identical layer batch sizes"); }
        if (layer->pred_logits.device() != device) { throw std::runtime_error("native RF-DETR batched matcher requires all layers on the same device"); }
        if (layer_queries % group_detr != 0) { throw std::runtime_error("num_queries must be divisible by group_detr for every matcher layer"); }
        ...
        for (const int64_t target_count : targets.counts) {
            ...
            if (active_extent > std::numeric_limits<int64_t>::max() - assignment_extent) throw std::overflow_error("matcher assignment extent overflows");
            assignment_extent += active_extent;
        }
        layer_query_counts.push_back(layer_queries);
        max_queries = std::max(max_queries, layer_queries);
    }
    for (const auto count : targets.counts) { max_targets_per_image = std::max(max_targets_per_image, count); }
...
MatchIndices compute_matcher_indices(const OutputLayer& layer, const PreparedTargets& targets, const DetectionConfig& config, int64_t group_detr) {
...
TensorMap loss_labels(const OutputLayer& layer, const PreparedTargets& targets, const MatcherLayerIndices& indices, const DetectionConfig& config,
    ...
    if (config.ia_bce_loss) {
        ...
        loss_ce = position_supervised_loss(src_logits, cls_targets, num_boxes, config.focal_alpha, 2.0, config.use_jit_traced_loss_ops) * src_logits.size(1);
        ...
        loss_ce = sigmoid_varifocal_loss(src_logits, cls_targets, num_boxes, config.focal_alpha, 2.0, config.use_jit_traced_loss_ops) * src_logits.size(1);
    } else {
        auto target_classes =
            torch::full({src_logits.size(0), src_logits.size(1)}, config.num_classes, torch::TensorOptions().dtype(torch::kInt64).device(device));
        ...
        target_classes_onehot.scatter_(2, target_classes.unsqueeze(-1), torch::ones(target_classes.unsqueeze(-1).sizes(), target_classes_onehot.options()));
        target_classes_onehot = target_classes_onehot.index({Slice(), Slice(), Slice(None, -1)});
        loss_ce =
            sigmoid_focal_loss(src_logits, target_classes_onehot, num_boxes, config.focal_alpha, 2.0, config.use_jit_traced_loss_ops) * src_logits.size(1);
    }
...
void update_losses(TensorMap& destination, TensorMap source, const std::string& suffix = std::string()) {
    for (auto& item : source) { destination[item.first + suffix] = std::move(item.second); }
}
}  // namespace
...
torch::Tensor weighted_detection_loss(const TensorMap& loss_dict, const DetectionConfig& config, const torch::Device& device,
    ...
    for (const auto& item : config.weight_dict) {
        ...
        if (auxiliary_weighted && (item.first.ends_with("_enc") || (!item.first.empty() && std::isdigit(static_cast<unsigned char>(item.first.back()))))) {
