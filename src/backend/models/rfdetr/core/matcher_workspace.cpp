...
std::vector<MatcherLayerIndices> MatcherWorkspace::pack(const std::vector<std::vector<std::pair<at::Tensor, at::Tensor>>>& indices,
    ...
    for (const auto& layer : indices) {
        ...
        for (const auto& pair : layer) {
            if (!pair.first.device().is_cpu() || !pair.second.device().is_cpu() || pair.first.scalar_type() != at::kLong ||
                pair.second.scalar_type() != at::kLong || pair.first.dim() != 1 || pair.second.dim() != 1 || !pair.first.is_contiguous() ||
                !pair.second.is_contiguous() || pair.first.numel() != pair.second.numel() ||
