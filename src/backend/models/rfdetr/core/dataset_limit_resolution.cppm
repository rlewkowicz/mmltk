...
[[nodiscard]] inline ResolvedDatasetLimit resolve_dataset_limit(const std::uint32_t max_instances_per_image, const std::size_t requested) {
 ...
 const std::uint64_t value =
  automatic ? static_cast<std::uint64_t>(max_instances_per_image) * 2U + 1U : mmltk::common::math::checked_cast<std::uint64_t>(requested, "RF-DETR dataset limit exceeds uint64_t");
...
[[nodiscard]] inline ResolvedDatasetLimit resolve_dataset_query_limit(const std::uint32_t max_instances_per_image, const std::size_t requested, const std::size_t automatic_num_queries_cap) {
