...
[[nodiscard]] inline ResolvedDatasetLimit resolve_dataset_limit(const std::uint32_t max_instances_per_image, const std::size_t requested) {
    ...
    const std::uint64_t value = automatic
                                    ? static_cast<std::uint64_t>(max_instances_per_image) * 2U + 1U
...
[[nodiscard]] inline ResolvedDatasetLimit resolve_dataset_query_limit(const std::uint32_t max_instances_per_image,
    ...
    if (automatic_num_queries_cap == 0U) {
        throw std::invalid_argument("RF-DETR automatic query resolution requires a positive model query cap");
    }
