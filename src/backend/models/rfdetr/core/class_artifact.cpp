...
ClassArtifactSnapshot capture(const std::filesystem::path& artifact, const std::filesystem::path& descriptor) {
 ClassArtifactSnapshot result{.artifact_path = normalized(artifact), .descriptor_path = normalized(descriptor), .artifact = io::FileSnapshot::Read(artifact)};
...
ClassArtifactAdmission::ClassArtifactAdmission(const std::filesystem::path& artifact, const std::filesystem::path& descriptor,
 ...
 if (snapshot_ != capture(snapshot_.artifact_path, snapshot_.descriptor_path)) throw std::runtime_error("class artifact changed during descriptor admission");
 check_stop(stop);
}
