...
at::Tensor make_fixture_class_bias(const ParityFixtureCase& fixture) {
 return torch::linspace(-1.0F + fixture.offset, 1.0F + fixture.offset, kParityFixtureNumClasses, torch::TensorOptions().dtype(torch::kFloat32));
}
DecodedNativeModelState make_native_parity_fixture(const ParityFixtureCase& fixture) {
 ...
 set_synthetic_model_state(checkpoint, std::move(entries));
 return checkpoint;
}
void assert_matches_native_parity_fixture(const DecodedNativeModelState& checkpoint, const ParityFixtureCase& fixture) {
 ...
 for (const auto& entry : entries) {
  if (entry.name == "query_feat.weight") {
   found[0] = true;
   require(torch::equal(entry.tensor, make_fixture_query_feat(fixture)), "query_feat mismatch");
  } else if (entry.name == "refpoint_embed.weight") {
   found[1] = true;
   require(torch::equal(entry.tensor, make_fixture_refpoint_embed(fixture)), "refpoint mismatch");
  } else if (entry.name == "class_embed.weight") {
   found[2] = true;
   require(torch::equal(entry.tensor, make_fixture_class_weight(fixture)), "class weight mismatch");
  } else if (entry.name == "class_embed.bias") {
   found[3] = true;
   require(torch::equal(entry.tensor, make_fixture_class_bias(fixture)), "class bias mismatch");
  }
 }
