...
void save_upstream_python_checkpoint(const std::filesystem::path& path, const std::vector<NormalizedModelStateEntry>& state) {
    ...
    detail::model_state_owner(checkpoint).entries = state;
...
void log_fixture_phase(const char* test_name, const std::size_t index, const std::size_t total, const char* phase,
