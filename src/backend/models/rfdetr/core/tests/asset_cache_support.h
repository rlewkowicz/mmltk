    ...
    regenerate_file_atomically(
        ...
        [&asset](const fs::path& temp_path) {
            return std::vector<std::string>{
                ...
                "--retry", "3",  "--output", temp_path.string(), std::string(asset.download_url),
            };
        },
...
inline void ensure_native_checkpoint(const fs::path& upstream_weights_path, const fs::path& native_checkpoint_path) {
    ...
    regenerate_file_atomically(native_checkpoint_path, "RF-DETR native checkpoint export",
                               [&upstream_weights_path](const fs::path& temp_path) {
                                   return std::vector<std::string>{
                                       mmltk::testsupport::mmltk_cli_path(), "rfdetr",   "normalize-weights", "--input",
                                       upstream_weights_path.string(),       "--output", temp_path.string(),
                                   };
                               });
}
...
inline void ensure_exported_onnx(const fs::path& native_checkpoint_path, const fs::path& onnx_path) {
    ...
    regenerate_file_atomically(
        ...
        [](const fs::path& temp_path) {
            if (!validate_onnx_model(temp_path)) {
                throw std::runtime_error("RF-DETR ONNX export does not expose canonical prediction outputs");
            }
        });
}
