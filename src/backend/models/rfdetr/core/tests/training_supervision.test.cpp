...
rfdetr::NativeRfDetrConfig production_denoising_config(const rfdetr::TrainAssignmentKind assignment, const bool bbox_reparam, const bool segmentation = false) {
...
void install_oracle_projections(rfdetr::TrainingSupervisionImpl& supervision) {
 set_parameter(supervision, "ground_truth_mlp.linear1.weight", torch::tensor({{1.0F, -0.5F, 0.2F, 0.3F, -0.1F, 0.4F}, {-0.25F, 0.75F, -0.3F, 0.1F, 0.5F, 0.2F}}));
 set_parameter(supervision, "ground_truth_mlp.linear1.bias", torch::tensor({0.1F, 0.2F}));
 set_parameter(supervision, "ground_truth_mlp.linear2.weight", torch::tensor({{0.8F, -0.2F}, {0.3F, 0.7F}}));
 set_parameter(supervision, "ground_truth_mlp.linear2.bias", torch::tensor({0.05F, -0.1F}));
 set_parameter(supervision, "query_mlp.linear1.weight", torch::eye(2));
 set_parameter(supervision, "query_mlp.linear1.bias", torch::zeros({2}));
 set_parameter(supervision, "query_mlp.linear2.weight", torch::eye(2));
 set_parameter(supervision, "query_mlp.linear2.bias", torch::zeros({2}));
...
rfdetr::PreparedTargets one_image_targets(const torch::Tensor& boxes, const torch::Tensor& labels) {
 ...
 const auto integer_options = torch::TensorOptions().dtype(torch::kInt64).device(labels.device());
 targets.target_offsets = torch::tensor({0}, integer_options);
 targets.target_counts = torch::tensor({labels.size(0)}, integer_options);
 return targets;
}
rfdetr::PreparedTargets batched_targets(const std::vector<torch::Tensor>& boxes, const std::vector<torch::Tensor>& labels, int64_t resolved_queries = 4) {
 ...
 targets.all_boxes = offset == 0 ? torch::zeros({0, 4}, float_options) : torch::cat(boxes, 0);
 targets.all_labels = offset == 0 ? torch::zeros({0}, integer_options) : torch::cat(labels, 0);
 targets.target_offsets = torch::tensor(targets.offsets, integer_options);
 targets.target_counts = torch::tensor(targets.counts, integer_options);
 return targets;
}
...
void require_layer_equal(const rfdetr::OutputLayer& lhs, const rfdetr::OutputLayer& rhs) {
 REQUIRE(torch::equal(lhs.pred_logits, rhs.pred_logits));
 REQUIRE(torch::equal(lhs.pred_boxes, rhs.pred_boxes));
 REQUIRE(lhs.pred_masks.has_value() == rhs.pred_masks.has_value());
 if (lhs.pred_masks) { REQUIRE(torch::equal(*lhs.pred_masks, *rhs.pred_masks)); }
}
void require_float32(const std::initializer_list<torch::Tensor> tensors) {
 for (const auto& tensor : tensors) { REQUIRE(tensor.scalar_type() == torch::kFloat32); }
}
...
void retain_captured_output_gradients(rfdetr::ModelOutputs& outputs) {
 ...
 for (auto& auxiliary : outputs.aux_outputs) { retain_captured_layer_gradients(auxiliary); }
 if (outputs.enc_outputs) { retain_captured_layer_gradients(*outputs.enc_outputs); }
}
void append_layer_gradient_evidence(const rfdetr::OutputLayer& layer, const std::string& prefix, const bool require_zero, std::vector<std::string>& names) {
...
std::vector<std::string> output_gradient_evidence(const rfdetr::ModelOutputs& outputs, const bool require_zero) {
 ...
 for (std::size_t index = 0; index < outputs.aux_outputs.size(); ++index) { append_layer_gradient_evidence(outputs.aux_outputs[index], "aux." + std::to_string(index), require_zero, names); }
...
bool selected_model_gradient_family(const std::string& name, const bool include_encoder) {
 return name.starts_with("training_supervision.") || name.starts_with("class_embed.") || name.starts_with("bbox_embed.") ||
        (include_encoder && (name.starts_with("transformer.enc_out_class_embed.") || name.starts_with("transformer.enc_out_bbox_embed.")));
}
std::vector<std::string> selected_model_gradient_evidence(rfdetr::NativeRfDetrModel& module, const bool include_encoder, const bool require_zero) {
...
std::vector<std::string> ordered_defined_trainable_gradients(rfdetr::NativeRfDetrModel& module) {
 std::vector<std::string> names;
 for (const auto& parameter : module.named_parameters(true)) {
...
rfdetr::OutputLayer oracle_layer(const torch::Tensor& features) {
 ...
 layer.pred_logits = torch::tensor({{{0.4F, -0.7F, 0.2F}, {-0.1F, 0.8F, -0.3F}, {0.6F, 0.1F, -0.5F}}}, torch::TensorOptions().dtype(torch::kFloat32)).set_requires_grad(true);
 layer.pred_boxes = torch::tensor({{{0.45F, 0.55F, 0.2F, 0.3F}, {0.2F, 0.25F, 0.15F, 0.1F}, {0.7F, 0.65F, 0.25F, 0.2F}}}, torch::TensorOptions().dtype(torch::kFloat32)).set_requires_grad(true);
...
void test_geometry_preserves_consumer_policies_and_batch_isolation() {
 ...
 REQUIRE(torch::allclose(giou, torch::ones_like(giou)));
...
void test_gathered_ground_truth_affinity_matches_explicit_one_hot_and_sqrt_d() {
 ...
 const auto boxes = torch::tensor({{{0.5F, 0.5F, 0.2F, 0.3F}, {0.2F, 0.3F, 0.1F, 0.15F}}});
 const auto valid = torch::ones({1, 2}, torch::TensorOptions().dtype(torch::kBool));
 const auto features = torch::tensor({{{{0.8F, 0.1F}, {0.2F, 0.9F}, {0.4F, 0.3F}}}});
 const auto result = supervision.correspondence(labels, boxes, valid, features);
 const auto one_hot = torch::eye(2).index({labels});
 const auto input = torch::cat({one_hot, boxes}, -1);
 const auto parameters = supervision.named_parameters(true);
 const auto gt_hidden = torch::relu(torch::matmul(input, parameters["ground_truth_mlp.linear1.weight"].transpose(0, 1)) + parameters["ground_truth_mlp.linear1.bias"]);
 const auto gt = torch::matmul(gt_hidden, parameters["ground_truth_mlp.linear2.weight"].transpose(0, 1)) + parameters["ground_truth_mlp.linear2.bias"];
 ...
 REQUIRE(torch::allclose(result.dense, expected, 1.0e-6, 1.0e-6));
}
void test_rectangular_supervision_retains_404_targets_with_300_queries() {
 ...
 const auto targets = batched_targets({torch::full({404, 4}, 0.2F), torch::zeros({0, 4})}, {torch::zeros({404}, integers), torch::zeros({0}, integers)}, config.num_queries);
 ...
 layer.pred_boxes = torch::full({2, 300, 4}, 0.3F).set_requires_grad(true);
 layer.query_features = torch::ones({2, 1, 300, config.hidden_dim}).set_requires_grad(true);
 layer.query_layout = rfdetr::SupervisedQueryLayout{1, 300};
 const auto labels = torch::zeros({2, 404}, integers);
 const auto boxes = torch::full({2, 404, 4}, 0.2F);
 auto valid = torch::ones({2, 404}, integers.dtype(torch::kBool));
 ...
 REQUIRE(torch::allclose(correspondence.dense.index({0}).sum(-1), torch::ones({1, 404})));
 REQUIRE(correspondence.sparse_normalized.index({1}).abs().sum().item<float>() == 0.0F);
 const auto loss = supervision.loss(outputs, targets, {torch::tensor(404.0F)}, true);
 ...
 REQUIRE(torch::equal(dn->target_indices.index({0, 1}), torch::arange(404, integers)));
 const auto retained_content = dn->content.clone();
 const auto smaller = batched_targets({torch::full({1, 4}, 0.2F)}, {torch::zeros({1}, integers)}, 300);
 REQUIRE(supervision.prepare_denoising(smaller, {404, 0, 0, 1}, torch::Device(torch::kCPU), torch::kFloat32).has_value());
 REQUIRE(torch::equal(dn->content, retained_content));
}
void test_scg_excludes_inactive_columns_handles_ties_and_keeps_selected_gradients_live() {
 auto dense = torch::tensor({{{{0.60F, 0.40F, 0.0F}, {0.30F, 0.70F, 0.0F}, {0.25F, 0.34F, 0.0F}}}}).set_requires_grad(true);
 const auto valid = torch::ones({1, 3}, torch::TensorOptions().dtype(torch::kBool));
 ...
 (sparse.index({0, 0, 0}) * torch::tensor({1.0F, 2.0F, 3.0F})).sum().backward();
 REQUIRE(dense.grad().index({0, 0, 0, 0}).abs().item<float>() > 0.0F);
 const auto tied = torch::tensor({{{{0.5F, 0.5F, 0.0F}}}});
 const auto tied_sparse = rfdetr::sparse_match_free_correspondence(tied, torch::ones({1, 1}, torch::TensorOptions().dtype(torch::kBool)), 0.99F);
...
void test_focal_broadcast_cost_sums_every_channel_and_places_coefficients_once() {
 ...
 const auto labels = torch::tensor({{1}}, torch::TensorOptions().dtype(torch::kInt64));
 const auto boxes = torch::tensor({{{0.5F, 0.5F, 0.2F, 0.25F}}});
 const auto valid = torch::ones({1, 1}, torch::TensorOptions().dtype(torch::kBool));
 ...
 REQUIRE(torch::allclose(costs.classification.index({0, 0, 0, 0}), expected_classification));
 ...
 const auto actual = supervision.loss(outputs, targets, {torch::tensor(1.0F)}, true);
 REQUIRE(torch::allclose(actual.total, expected, 1.0e-5, 1.0e-5));
}
void test_layer_group_and_device_target_reductions_follow_declared_gating() {
 ...
 auto final_only = one_group.loss(one_outputs, targets, {torch::tensor(1.0F)}, true);
 ...
 const auto layered_result = layered.loss(layered_outputs, targets, {torch::tensor(1.0F)}, true);
 const auto layered_loss = layered_result.total.detach();
 REQUIRE(torch::allclose(layered_loss, baseline * 4.0F, 1.0e-5, 1.0e-5));
 ...
 const auto grouped_loss = grouped.loss(grouped_outputs, targets, {torch::tensor(1.0F)}, true).total.detach();
 REQUIRE(torch::allclose(grouped_loss, baseline, 1.0e-5, 1.0e-5));
 ...
 const auto summed_group_loss = summed_groups.loss(grouped_outputs, targets, {torch::tensor(1.0F)}, true).total.detach();
 REQUIRE(torch::allclose(summed_group_loss, baseline * 2.0F, 1.0e-5, 1.0e-5));
 rfdetr::ModelOutputs validation_outputs;
 validation_outputs.main = oracle_layer(features);
 const auto validation_loss = grouped.loss(validation_outputs, targets, {torch::tensor(1.0F)}, false).total.detach();
 REQUIRE(torch::allclose(validation_loss, baseline, 1.0e-5, 1.0e-5));
}
void test_complete_match_free_loss_is_fp32_inside_cuda_autocast() {
 ...
 const auto labels = torch::tensor({{1}}, torch::TensorOptions().dtype(torch::kInt64).device(torch::kCUDA));
 const auto boxes = torch::tensor({{{0.5F, 0.5F, 0.2F, 0.25F}}}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
 const auto valid = torch::ones({1, 1}, torch::TensorOptions().dtype(torch::kBool).device(torch::kCUDA));
 const auto features = torch::ones({1, 1, 3, 2}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCUDA)).set_requires_grad(true);
 auto layer = oracle_layer(features);
 ...
 outputs.main = layer;
 const auto targets = one_image_targets(boxes.squeeze(0), labels.squeeze(0));
 const auto loss = supervision.loss(outputs, targets, {torch::tensor(1.0F, boxes.options())}, true);
 ...
 const auto expected = alpha * (correspondence.dense * costs.total).sum() + beta * (correspondence.sparse_normalized * costs.total).sum();
...
void test_timing_leases_are_explicit_bounded_and_harvested_once() {
 ...
 const auto disabled_features = torch::ones({1, 1, 3, 2});
 disabled_outputs.main = oracle_layer(disabled_features);
 const auto disabled_targets = one_image_targets(torch::tensor({{0.5F, 0.5F, 0.2F, 0.25F}}), torch::tensor({1}, torch::TensorOptions().dtype(torch::kInt64)));
 static_cast<void>(disabled.loss(disabled_outputs, disabled_targets, {torch::tensor(1.0F)}, true));
 ...
 install_oracle_projections(supervision);
 supervision.to(torch::Device(torch::kCUDA));
 const auto features = torch::ones({1, 1, 3, 2}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
 rfdetr::ModelOutputs outputs;
 outputs.main = oracle_layer(features);
 ...
 const auto normalizer = rfdetr::DeviceLossNormalizer{torch::tensor(1.0F, boxes.options())};
 ...
 auto& dn_owner = (dn_model);
 dn_owner.initialize_training_supervision(46);
 dn_owner.to(torch::Device(torch::kCUDA));
 dn_owner.train(true);
 ...
 const auto dn_targets = batched_targets({torch::tensor({{0.5F, 0.5F, 0.2F, 0.25F}}, dn_float_options)}, {torch::tensor({1}, dn_integer_options)}, dn_config.num_queries);
 ...
 const auto record_dn_step = [&](const std::uint64_t batch_sequence) {
  ...
  const auto dn_outputs = dn_owner.forward_with_denoising(rfdetr::nested_tensor_from_tensor_list({dn_image}), dn_targets, {46, 1, 0, batch_sequence});
  ...
  static_cast<void>(dn_owner.supervision_loss(dn_outputs, dn_targets, {torch::tensor(1.0F, dn_float_options)}, true));
 ...
 auto& combined_owner = (combined_model);
 combined_owner.initialize_training_supervision(47);
 combined_owner.to(torch::Device(torch::kCUDA));
 combined_owner.train(true);
 ...
 const auto combined_outputs = combined_owner.forward_with_denoising(rfdetr::nested_tensor_from_tensor_list({dn_image.clone()}), dn_targets, {47, 1, 0, 1});
 combined_owner.begin_criterion_timing();
 static_cast<void>(combined_owner.supervision_loss(combined_outputs, dn_targets, {torch::tensor(1.0F, dn_float_options)}, true));
 ...
 const auto untimed_outputs = dn_owner.forward_with_denoising(rfdetr::nested_tensor_from_tensor_list({dn_image}), dn_targets, {46, 1, 0, 3});
 static_cast<void>(dn_owner.supervision_loss(untimed_outputs, dn_targets, {torch::tensor(1.0F, dn_float_options)}, true));
 ...
 REQUIRE_THROWS(dn_owner.forward_with_denoising(rfdetr::nested_tensor_from_tensor_list({dn_image}), invalid_device_targets, {46, 1, 0, 4}));
 static_cast<void>(cudaDeviceSynchronize());
...
void test_empty_and_retained_graphs_are_finite_and_independent() {
 ...
 auto second_loss = supervision.loss(second, targets, {torch::tensor(1.0F)}, true).total;
...
void test_production_match_free_boundary_captures_layers_and_empty_gradient_anchors() {
 const auto run_case = [](const bool include_auxiliary_and_encoder) {
  ...
  config.training_supervision.assignment = rfdetr::TrainAssignmentKind::MatchFree;
  torch::manual_seed(include_auxiliary_and_encoder ? 71 : 67);
  ...
  const auto nonempty_model_gradients = selected_model_gradient_evidence(owner, include_auxiliary_and_encoder, false);
  const auto nonempty_optimizer_inventory = ordered_defined_trainable_gradients(owner);
  ...
  const auto empty_optimizer_inventory = ordered_defined_trainable_gradients(owner);
...
void test_mixed_empty_images_use_safe_internal_padding_without_loss_contribution() {
 ...
 const auto single_features = torch::tensor({{{{0.8F, 0.1F}, {0.2F, 0.9F}, {0.4F, 0.3F}}}});
 ...
 auto mixed_targets = batched_targets({torch::tensor({{0.5F, 0.5F, 0.2F, 0.25F}}), torch::tensor({{std::numeric_limits<float>::quiet_NaN(), std::numeric_limits<float>::infinity(), -1.0F, -2.0F}})},
 ...
 const auto actual = supervision.loss(mixed, mixed_targets, {torch::tensor(1.0F)}, true).total;
 REQUIRE(torch::isfinite(actual).item<bool>());
 REQUIRE(torch::allclose(actual.detach(), expected, 1.0e-5, 1.0e-5));
 actual.backward();
...
void test_conditional_model_construction_preserves_rng_and_default_state_topology() {
 ...
 REQUIRE(torch::equal(inactive_rng, active_rng));
 const auto inactive_parameters = inactive.named_parameters(true);
 const auto active_parameters = active.named_parameters(true);
 ...
 for (const auto& parameter : inactive_parameters) {
  ...
  REQUIRE(torch::equal(parameter.value(), *enabled_parameter));
 }
 ...
 const auto inactive_buffers = inactive.named_buffers(true);
 const auto active_buffers = active.named_buffers(true);
 ...
 auto& inactive_owner = (inactive);
 auto& active_owner = (active);
 inactive_owner.eval();
 active_owner.eval();
 torch::NoGradGuard no_grad;
 ...
 for (std::size_t index = 0; index < inactive_matches.size(); ++index) {
  REQUIRE(torch::equal(inactive_matches[index].first, active_matches[index].first));
  REQUIRE(torch::equal(inactive_matches[index].second, active_matches[index].second));
 }
 ...
 for (const auto& [name, value] : inactive_losses) {
  ...
  REQUIRE(torch::equal(value, active_losses.at(name)));
 }
}
void test_dn_endpoint_transform_mapping_padding_and_one_class_behavior() {
 ...
 const auto boxes = std::vector<torch::Tensor>{torch::tensor({{0.02F, 0.98F, 0.2F, 0.4F}, {0.5F, 0.5F, 0.1F, 0.2F}}), torch::zeros({0, 4})};
 const auto labels = std::vector<torch::Tensor>{torch::tensor({0, 2}, torch::TensorOptions().dtype(torch::kInt64)), torch::zeros({0}, torch::TensorOptions().dtype(torch::kInt64))};
 ...
 variates.center = torch::tensor({0.0F, 1.0F}).view({1, 1, 1, 2}).expand({2, 2, 2, 2}).clone();
 variates.size = torch::tensor({0.0F, 1.0F}).view({1, 1, 1, 2}).expand({2, 2, 2, 2}).clone();
 variates.label_flip = torch::zeros(slot_shape);
 variates.other_label = torch::tensor({0, 1, 1, 0, 0, 1, 1, 0}, torch::TensorOptions().dtype(torch::kInt64)).view(slot_shape);
 ...
 const auto transformed =
  rfdetr::transform_denoising_targets(prepared->original_labels, prepared->original_boxes, prepared->valid_slots, config.num_classes - 1, config.training_supervision.denoising, variates);
 ...
 REQUIRE(transformed.extent_scales.max().item<float>() <= std::nextafter(1.0F + config.training_supervision.denoising.size_noise_scale, std::numeric_limits<float>::infinity()));
 ...
 const auto expected_extent =
  boxes[0].index({1, Slice(2, 4)}) * torch::tensor({1.0F - config.training_supervision.denoising.size_noise_scale, 1.0F + config.training_supervision.denoising.size_noise_scale});
 ...
 REQUIRE(torch::allclose(prepared->original_boxes.index({0, 0}), boxes[0]));
 REQUIRE(torch::equal(prepared->original_labels.index({0, 0}), labels[0]));
 ...
 open_upper.label_flip = torch::ones(slot_shape);
 const auto open_upper_unflipped =
  rfdetr::transform_denoising_targets(prepared->original_labels, prepared->original_boxes, prepared->valid_slots, config.num_classes - 1, config.training_supervision.denoising, open_upper);
 REQUIRE(torch::equal(open_upper_unflipped.labels, prepared->original_labels));
 const auto distinct_labels = torch::tensor({{{0, 2}}}, torch::TensorOptions().dtype(torch::kInt64));
 const auto distinct_boxes = torch::tensor({{{{0.4F, 0.6F, 0.2F, 0.4F}, {0.7F, 0.3F, 0.1F, 0.2F}}}});
 const auto distinct_valid = torch::ones({1, 1, 2}, torch::TensorOptions().dtype(torch::kBool));
 rfdetr::DenoisingVariates distinct;
 distinct.center = torch::tensor({0.125F, 0.875F, 0.25F, 0.75F}).view({1, 1, 2, 2});
 distinct.size = torch::tensor({0.2F, 0.8F, 0.3F, 0.7F}).view({1, 1, 2, 2});
 distinct.label_flip = torch::tensor({{{0.49F, 1.0F}}});
 distinct.other_label = torch::zeros({1, 1, 2}, torch::TensorOptions().dtype(torch::kInt64));
 const auto independently_noised = rfdetr::transform_denoising_targets(distinct_labels, distinct_boxes, distinct_valid, 3, config.training_supervision.denoising, distinct);
 const auto expected_center_offsets = (distinct.center * 2.0F - 1.0F) * distinct_boxes.slice(-1, 2, 4) * (0.5F * config.training_supervision.denoising.center_noise_scale);
 const auto expected_extent_scales = 1.0F - config.training_supervision.denoising.size_noise_scale + 2.0F * config.training_supervision.denoising.size_noise_scale * distinct.size;
 REQUIRE(torch::allclose(independently_noised.center_offsets, expected_center_offsets));
 REQUIRE(torch::allclose(independently_noised.extent_scales, expected_extent_scales));
 ...
 one_variates.label_flip = torch::zeros({1, 2, 1});
 const auto one_prepared = one_class.prepare_denoising(one_targets, {1, 2, 3, 4}, torch::Device(torch::kCPU), torch::kFloat32, &one_variates);
 REQUIRE(one_prepared.has_value());
 REQUIRE((one_prepared->original_labels == 0).all().item<bool>());
 const auto one_transformed =
  rfdetr::transform_denoising_targets(one_prepared->original_labels, one_prepared->original_boxes, one_prepared->valid_slots, 1, one_class_config.training_supervision.denoising, one_variates);
 ...
 const auto zero_ratio_unflipped = rfdetr::transform_denoising_targets(
  prepared->original_labels, prepared->original_boxes, prepared->valid_slots, zero_flip_config.num_classes - 1, zero_flip_config.training_supervision.denoising, variates);
 REQUIRE(torch::equal(zero_ratio_unflipped.labels, prepared->original_labels));
 const auto all_empty =
  batched_targets({torch::zeros({0, 4}), torch::zeros({0, 4})}, {torch::zeros({0}, torch::TensorOptions().dtype(torch::kInt64)), torch::zeros({0}, torch::TensorOptions().dtype(torch::kInt64))});
...
void test_dn_center_underflow_other_label_bijection_and_step_determinism() {
 ...
 const auto targets =
  batched_targets({torch::tensor({{0.5F, 0.5F, 0.2F, 0.3F}, {0.4F, 0.6F, 0.1F, 0.15F}, {0.7F, 0.3F, 0.25F, 0.2F}})}, {torch::tensor({0, 1, 2}, torch::TensorOptions().dtype(torch::kInt64))});
 rfdetr::DenoisingVariates variates;
 variates.center = torch::zeros({1, 2, 3, 2});
 variates.size = torch::full({1, 2, 3, 2}, 0.5F);
 variates.label_flip = torch::zeros({1, 2, 3});
 variates.other_label = torch::tensor({0, 0, 1, 1, 1, 0}, torch::TensorOptions().dtype(torch::kInt64)).view({1, 2, 3});
 ...
 REQUIRE(torch::equal(transformed_centers, source_centers));
 ...
 REQUIRE(torch::equal(first->content, repeated->content));
 REQUIRE(torch::equal(first->normalized_references, repeated->normalized_references));
 REQUIRE(torch::equal(first->target_indices, repeated->target_indices));
 REQUIRE_FALSE(torch::equal(first->normalized_references, intervening->normalized_references));
}
torch::Tensor dense_group_attention(
 torch::nn::MultiheadAttention& attention, const torch::Tensor& target, const torch::Tensor& position, const std::vector<int64_t>& group_widths, bool allow_dn_to_ordinary) {
 const int64_t total = target.size(1);
 auto mask = torch::full({total, total}, -std::numeric_limits<float>::infinity(), target.options());
 int64_t offset = 0;
...
void test_split_self_attention_matches_equation_seven_and_symmetric_group_layout() {
 ...
 const auto target = torch::rand({1, 6, 4}).set_requires_grad(true);
 const auto position = torch::rand({1, 6, 4});
 ...
 equation_layout.denoising_key_padding = torch::zeros({1, 2, 2}, torch::TensorOptions().dtype(torch::kBool));
 equation_layout.denoising_valid_slots = torch::ones({1, 2, 2}, torch::TensorOptions().dtype(torch::kBool));
 ...
 REQUIRE(torch::allclose(split.narrow(1, 0, 2), equation_seven.narrow(1, 0, 2), 1.0e-6, 1.0e-6));
 const auto grouped_target = torch::rand({1, 10, 4}).set_requires_grad(true);
 const auto grouped_position = torch::rand({1, 10, 4});
 ...
 REQUIRE(torch::allclose(grouped_split, symmetric, 1.0e-6, 1.0e-6));
 ...
 REQUIRE(torch::allclose(grouped_split.narrow(1, 0, 6), mutated_output.narrow(1, 0, 6), 1.0e-6, 1.0e-6));
 REQUIRE(torch::allclose(grouped_split.narrow(1, 8, 2), mutated_output.narrow(1, 8, 2), 1.0e-6, 1.0e-6));
 ...
 const auto ordinary_mutated_output = rfdetr::isolated_group_self_attention(attention, ordinary_mutated, grouped_position, grouped_layout);
 REQUIRE(torch::allclose(grouped_split.narrow(1, 3, 7), ordinary_mutated_output.narrow(1, 3, 7), 1.0e-6, 1.0e-6));
...
void test_dn_direct_loss_auxiliary_gating_padding_and_retained_graphs() {
 ...
 const auto targets = batched_targets(
  {torch::tensor({{0.5F, 0.5F, 0.2F, 0.2F}}), torch::zeros({0, 4})}, {torch::tensor({1}, torch::TensorOptions().dtype(torch::kInt64)), torch::zeros({0}, torch::TensorOptions().dtype(torch::kInt64))});
 ...
 auto make_outputs = [&](const rfdetr::DenoisingQueryBatch& prepared_batch, float shift) {
  ...
  denoising.main.pred_logits = torch::full({batch, groups, queries, 4}, shift).set_requires_grad(true);
  denoising.main.pred_boxes = (prepared_batch.original_boxes + shift).clone().set_requires_grad(true);
  denoising.aux_outputs.push_back({torch::full({batch, groups, queries, 4}, shift + 0.1F).set_requires_grad(true), (prepared_batch.original_boxes + shift + 0.1F).clone().set_requires_grad(true)});
  outputs.denoising = std::move(denoising);
  outputs.main.pred_logits = torch::zeros({batch, 4, 4}).set_requires_grad(true);
  outputs.main.pred_boxes = torch::zeros({batch, 4, 4}).set_requires_grad(true);
  outputs.main.pred_masks = torch::zeros({batch, 4, 2, 2}).set_requires_grad(true);
  rfdetr::OutputLayer encoder;
  encoder.pred_logits = torch::zeros({batch, 4, 4}).set_requires_grad(true);
  encoder.pred_boxes = torch::zeros({batch, 4, 4}).set_requires_grad(true);
  outputs.enc_outputs = std::move(encoder);
  return outputs;
 };
 auto first_outputs = make_outputs(*prepared, 0.05F);
 const auto first = supervision.loss(first_outputs, targets, {torch::tensor(1.0F)}, true);
 REQUIRE(torch::allclose(first.total, first.classification + first.box + first.giou));
 REQUIRE(torch::equal(first.total, first.denoising));
 ...
 const auto auxiliary_loss = auxiliary_supervision.loss(auxiliary_outputs, targets, {torch::tensor(1.0F)}, true);
 ...
 const auto retained_second_loss = supervision.loss(retained_second, other_targets, {torch::tensor(2.0F)}, true).total;
...
void test_production_dn_forward_preserves_reference_and_output_boundaries() {
 const auto run_case = [](const rfdetr::TrainAssignmentKind assignment, const bool bbox_reparam, const bool segmentation, const std::uint64_t seed) {
  ...
  const auto boxes = torch::tensor({{0.25F, 0.75F, 0.2F, 0.4F}, {0.7F, 0.35F, 0.15F, 0.1F}});
  const auto labels = torch::tensor({0, 1}, torch::TensorOptions().dtype(torch::kInt64));
  ...
  REQUIRE(torch::equal(denoising.original_labels.index({0, 0}), labels));
  REQUIRE(torch::allclose(denoising.original_boxes.index({0, 0}), boxes));
  ...
  REQUIRE(torch::allclose(denoising.main.pred_boxes, prepared->normalized_references.view_as(denoising.main.pred_boxes), 1.0e-6, 1.0e-6));
  const auto changed_identity = rfdetr::TrainingStepIdentity{seed, 7, 2, 20};
  const auto changed = owner.forward_with_denoising(rfdetr::nested_tensor_from_tensor_list({image.clone()}), targets, changed_identity);
...
void test_production_dn_retained_graph_scratch_padding_and_gradients() {
 ...
 const auto image = torch::rand({3, 64, 64});
 const auto first_targets = batched_targets({torch::tensor({{0.4F, 0.6F, 0.2F, 0.25F}})}, {torch::tensor({0}, torch::TensorOptions().dtype(torch::kInt64))}, config.num_queries);
 ...
 const auto second_targets = batched_targets({torch::tensor({{0.2F, 0.3F, 0.1F, 0.15F}, {0.75F, 0.7F, 0.2F, 0.2F}}), torch::zeros({0, 4})},
  {torch::tensor({1, 2}, torch::TensorOptions().dtype(torch::kInt64)), torch::zeros({0}, torch::TensorOptions().dtype(torch::kInt64))}, config.num_queries);
 ...
 REQUIRE(torch::isfinite(second.denoising->main.pred_logits.index({1})).all().item<bool>());
 REQUIRE(torch::isfinite(second.denoising->main.pred_boxes.index({1})).all().item<bool>());
 second.denoising->main.pred_logits.retain_grad();
 second.denoising->main.pred_boxes.retain_grad();
 const auto second_loss = owner.supervision_loss(second, second_targets, {torch::tensor(2.0F)}, true).total;
 const auto high_water_targets = batched_targets(
  {torch::tensor({{0.2F, 0.2F, 0.1F, 0.1F}, {0.5F, 0.5F, 0.2F, 0.2F}, {0.8F, 0.8F, 0.1F, 0.1F}})}, {torch::tensor({0, 1, 2}, torch::TensorOptions().dtype(torch::kInt64))}, config.num_queries);
 ...
 REQUIRE(torch::equal(*empty.main.query_features, *ordinary_empty.main.query_features));
 ...
 const auto over_capacity_targets =
  batched_targets({torch::full({config.num_queries + 1, 4}, 0.2F)}, {torch::zeros({config.num_queries + 1}, torch::TensorOptions().dtype(torch::kInt64))}, config.num_queries);
 ...
 const auto over_capacity_loss = owner.supervision_loss(over_capacity, over_capacity_targets, {torch::tensor(static_cast<float>(config.num_queries + 1))}, true);
 REQUIRE(torch::isfinite(over_capacity_loss.total).item<bool>());
 over_capacity_loss.total.backward();
 REQUIRE_FALSE(rfdetr::training_supervision_query_layout_valid(config.training_supervision, std::numeric_limits<std::size_t>::max(), 2U, 0U));
 REQUIRE_FALSE(rfdetr::training_supervision_query_layout_valid(config.training_supervision, 0U, 1U, std::numeric_limits<std::size_t>::max()));
 REQUIRE_FALSE(rfdetr::training_supervision_query_layout_valid(config.training_supervision, std::numeric_limits<std::size_t>::max() - 1U, 1U, 1U));
}
void test_dn_reference_conventions_inference_removal_and_disabled_exact_path() {
 ...
 variates.label_flip = torch::full({1, 2, 1}, 1.0F);
 variates.other_label = torch::zeros({1, 2, 1}, torch::TensorOptions().dtype(torch::kInt64));
 ...
 REQUIRE(torch::allclose(additive_reference.sigmoid(), reparameterized_reference, 1.0e-6, 1.0e-6));
 const auto zero_delta = torch::zeros_like(reparameterized_reference);
 const auto reparameterized_result = torch::cat(
  {zero_delta.slice(-1, 0, 2) * reparameterized_reference.slice(-1, 2, 4) + reparameterized_reference.slice(-1, 0, 2), zero_delta.slice(-1, 2, 4).exp() * reparameterized_reference.slice(-1, 2, 4)},
  -1);
 ...
 for (const auto& parameter : inactive_owner.named_parameters(true)) { REQUIRE_FALSE(parameter.key().starts_with("training_supervision.")); }
 const auto active_parameters = active_owner.named_parameters(true);
...
void test_dn_all_empty_loss_is_finite_and_parameter_anchored() {
 ...
 rfdetr::ModelOutputs outputs;
 outputs.main.pred_logits = torch::zeros({2, 4, 4}).set_requires_grad(true);
 ...
 const auto targets = batched_targets({torch::zeros({0, 4}), torch::zeros({0, 4})}, {torch::zeros({0}, torch::TensorOptions().dtype(torch::kInt64)),
...
void test_active_empty_loss_anchors_every_selected_mask_operand() {
 const auto run_case = [](const bool auxiliary_and_encoder) {
  ...
  const auto make_layer = [](const bool dense) {
   rfdetr::OutputLayer layer;
   layer.pred_logits = torch::zeros({1, 4, 4}).set_requires_grad(true);
   layer.pred_boxes = torch::zeros({1, 4, 4}).set_requires_grad(true);
   if (dense) {
  ...
  const auto targets = batched_targets({torch::zeros({0, 4})}, {torch::zeros({0}, torch::TensorOptions().dtype(torch::kInt64))});
  const auto loss = supervision.loss(outputs, targets, {torch::tensor(0.0F)}, true);
  REQUIRE(loss.total.item<float>() == 0.0F);
  loss.total.backward();
  ...
  for (const auto& layer : outputs.aux_outputs) { require_layer_masks(layer); }
...
void test_dn_preparation_and_objective_remain_fp32_under_cuda_autocast() {
 ...
 denoising.main.pred_logits = torch::zeros({1, 2, 1, 4}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCUDA)).set_requires_grad(true);
 denoising.main.pred_boxes = prepared->original_boxes.to(torch::kFloat16).clone().set_requires_grad(true);
 outputs.denoising = std::move(denoising);
 outputs.main.pred_logits = torch::zeros({1, 4, 4}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCUDA)).set_requires_grad(true);
...
void test_feature_initialization_is_reproducible_and_independent_of_dn_toggle() {
 ...
 REQUIRE(torch::equal(rng_before, at::detail::getDefaultCPUGenerator().get_state()));
 ...
 REQUIRE(torch::equal(combined_parameters["denoising_label_embedding.weight"], dn_parameters["denoising_label_embedding.weight"]));
 REQUIRE(torch::equal(combined_parameters["denoising_task_embedding.weight"], dn_parameters["denoising_task_embedding.weight"]));
...
void test_sparse_masks_share_erasure_after_geometry_and_donor_composition() {
 ...
 const rfdetr::PackedTargetMasks masks{torch::full({4, 1}, 0xFFFF, integers), 4, 4, transforms, torch::tensor({1, -1, -1, -1}, integers), occluder_transforms, bytes.to(torch::kCUDA)};
 ...
 const auto expected = torch::tensor({0.F, 0.F, 0.F, 0.F, 0.F, 0.F, 0.F, 1.F, 1.F, 1.F, 0.F, 0.F, 0.F, 0.F, 0.F, 0.F, 0.F, 1.F, 0.F, 1.F}, floats).view({4, 5});
