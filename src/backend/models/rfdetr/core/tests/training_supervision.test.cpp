...
rfdetr::OutputLayer oracle_layer(const torch::Tensor& features) {
    ...
    layer.pred_logits =
        torch_api::tensor({{{0.4F, -0.7F, 0.2F}, {-0.1F, 0.8F, -0.3F}, {0.6F, 0.1F, -0.5F}}}, torch_api::TensorOptions().dtype(torch_api::kFloat32))
...
void test_geometry_preserves_consumer_policies_and_batch_isolation() {
    ...
    auto criterion_targets =
        one_image_targets(torch_api::tensor({{0.8F, 0.5F, 0.2F, 0.4F}}), torch_api::tensor({0}, torch_api::TensorOptions().dtype(torch_api::kInt64)));
...
void test_scg_excludes_inactive_columns_handles_ties_and_keeps_selected_gradients_live() {
    ...
    const auto tied_sparse = rfdetr::sparse_match_free_correspondence(tied, torch_api::ones({1, 1}, torch_api::TensorOptions().dtype(torch_api::kBool)), 0.99F);
...
void test_complete_focal_cost_and_both_objectives_keep_prediction_and_probe_gradients() {
    ...
    const auto targets =
        one_image_targets(torch_api::tensor({{0.5F, 0.5F, 0.2F, 0.25F}}), torch_api::tensor({1}, torch_api::TensorOptions().dtype(torch_api::kInt64)));
...
void test_complete_match_free_loss_is_fp32_inside_cuda_autocast() {
    ...
    const auto boxes = torch_api::tensor({{{0.5F, 0.5F, 0.2F, 0.25F}}}, torch_api::TensorOptions().dtype(torch_api::kFloat32).device(torch::kCUDA));
    const auto valid = torch_api::ones({1, 1}, torch_api::TensorOptions().dtype(torch_api::kBool).device(torch::kCUDA));
    const auto features = torch_api::ones({1, 1, 3, 2}, torch_api::TensorOptions().dtype(torch::kFloat16).device(torch::kCUDA)).set_requires_grad(true);
    auto layer = oracle_layer(features);
    ...
    REQUIRE(torch_api::allclose(loss.total - loss.correspondence, beta * (correspondence.sparse_normalized * costs.total).sum(), 1.0e-5, 1.0e-5));
    loss.total.backward();
...
void test_timing_leases_are_explicit_bounded_and_harvested_once() {
    ...
    const auto boxes = torch_api::tensor({{0.5F, 0.5F, 0.2F, 0.25F}}, torch_api::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
    ...
    const auto dn_targets = batched_targets({torch_api::tensor({{0.5F, 0.5F, 0.2F, 0.25F}}, dn_float_options)}, {torch_api::tensor({1}, dn_integer_options)},
...
void test_empty_and_retained_graphs_are_finite_and_independent() {
    ...
    auto targets =
        one_image_targets(torch_api::tensor({{0.5F, 0.5F, 0.2F, 0.2F}}), torch_api::tensor({0}, torch_api::TensorOptions().dtype(torch_api::kInt64)));
    ...
    const auto empty_targets = batched_targets({torch_api::zeros({0, 4})}, {torch_api::zeros({0}, torch_api::TensorOptions().dtype(torch_api::kInt64))});
...
void test_production_match_free_boundary_captures_layers_and_empty_gradient_anchors() {
    const auto run_case = [](const bool include_auxiliary_and_encoder) {
        ...
        const auto targets =
            one_image_targets(torch_api::tensor({{0.5F, 0.5F, 0.25F, 0.25F}}), torch_api::tensor({0}, torch_api::TensorOptions().dtype(torch_api::kInt64)));
        ...
        for (auto& parameter : owner.module().parameters(true)) { parameter.mutable_grad() = torch::Tensor(); }
        ...
        const auto empty_targets =
            batched_targets({torch_api::zeros({0, 4})}, {torch_api::zeros({0}, torch_api::TensorOptions().dtype(torch_api::kInt64))}, config.num_queries);
...
void test_mixed_empty_images_use_safe_internal_padding_without_loss_contribution() {
    ...
    auto mixed_targets = batched_targets({torch_api::tensor({{0.5F, 0.5F, 0.2F, 0.25F}}),
...
void test_dn_endpoint_transform_mapping_padding_and_one_class_behavior() {
    ...
    const auto boxes = std::vector<torch::Tensor>{torch_api::tensor({{0.02F, 0.98F, 0.2F, 0.4F}, {0.5F, 0.5F, 0.1F, 0.2F}}), torch_api::zeros({0, 4})};
    ...
    variates.other_label = torch_api::tensor({0, 1, 1, 0, 0, 1, 1, 0}, torch_api::TensorOptions().dtype(torch_api::kInt64)).view(slot_shape);
    ...
    const auto expected_extent = boxes[0].index({1, Slice(2, 4)}) * torch_api::tensor({1.0F - config.training_supervision.denoising.size_noise_scale,
    ...
    const auto one_targets =
        batched_targets({torch_api::tensor({{0.5F, 0.5F, 0.2F, 0.2F}})}, {torch_api::tensor({0}, torch_api::TensorOptions().dtype(torch_api::kInt64))});
    ...
    const auto all_empty =
        batched_targets({torch_api::zeros({0, 4}), torch_api::zeros({0, 4})}, {torch_api::zeros({0}, torch_api::TensorOptions().dtype(torch_api::kInt64)),
                                                                               torch_api::zeros({0}, torch_api::TensorOptions().dtype(torch_api::kInt64))});
...
void test_dn_center_underflow_other_label_bijection_and_step_determinism() {
    ...
    const auto targets = batched_targets({torch_api::tensor({{0.5F, 0.5F, 0.2F, 0.3F}, {0.4F, 0.6F, 0.1F, 0.15F}, {0.7F, 0.3F, 0.25F, 0.2F}})},
...
void test_dn_direct_loss_auxiliary_gating_padding_and_retained_graphs() {
    ...
    const auto other_targets = batched_targets({torch_api::tensor({{0.4F, 0.4F, 0.1F, 0.1F}, {0.7F, 0.7F, 0.2F, 0.2F}}), torch_api::zeros({0, 4})},
...
void test_production_dn_forward_preserves_reference_and_output_boundaries() {
    const auto run_case = [](const rfdetr::TrainAssignmentKind assignment, const bool bbox_reparam, const bool segmentation, const std::uint64_t seed) {
        ...
        REQUIRE(torch_api::allclose(denoising.main.pred_boxes, prepared->normalized_references.view_as(denoising.main.pred_boxes), 1.0e-6, 1.0e-6));
...
void test_production_dn_retained_graph_scratch_padding_and_gradients() {
    ...
    const auto image = torch_api::rand({3, 64, 64});
    const auto first_targets = batched_targets({torch_api::tensor({{0.4F, 0.6F, 0.2F, 0.25F}})},
    ...
    const auto second_targets = batched_targets({torch_api::tensor({{0.2F, 0.3F, 0.1F, 0.15F}, {0.75F, 0.7F, 0.2F, 0.2F}}), torch_api::zeros({0, 4})},
    ...
    const auto high_water_targets = batched_targets({torch_api::tensor({{0.2F, 0.2F, 0.1F, 0.1F}, {0.5F, 0.5F, 0.2F, 0.2F}, {0.8F, 0.8F, 0.1F, 0.1F}})},
    ...
    const auto over_capacity_targets =
        batched_targets({torch_api::full({config.num_queries + 1, 4}, 0.2F)},
...
void test_dn_reference_conventions_inference_removal_and_disabled_exact_path() {
    ...
    const auto targets =
        batched_targets({torch_api::tensor({{0.25F, 0.75F, 0.2F, 0.4F}})}, {torch_api::tensor({1}, torch_api::TensorOptions().dtype(torch_api::kInt64))});
    rfdetr::DenoisingVariates variates;
    ...
    for (const auto& parameter : inactive_owner.module().named_parameters(true)) { REQUIRE_FALSE(parameter.key().starts_with("training_supervision.")); }
...
void test_active_empty_loss_anchors_every_selected_mask_operand() {
    const auto run_case = [](const bool auxiliary_and_encoder) {
        ...
        const auto targets = batched_targets({torch_api::zeros({0, 4})}, {torch_api::zeros({0}, torch_api::TensorOptions().dtype(torch_api::kInt64))});
        const auto loss = supervision.loss(outputs, targets, {torch_api::tensor(0.0F)}, true);
        REQUIRE(loss.total.item<float>() == 0.0F);
        loss.total.backward();
...
void test_dn_preparation_and_objective_remain_fp32_under_cuda_autocast() {
    ...
    const auto targets = batched_targets({torch_api::tensor({{0.5F, 0.5F, 0.2F, 0.2F}}, float_options)}, {torch_api::tensor({1}, integer_options)});
    // CLEANUP-ON
    mmltk::backend::ml::cuda::TorchAutocastScope autocast(true, torch::kFloat16);
    ...
    denoising.main.pred_logits = torch_api::zeros({1, 2, 1, 4}, torch_api::TensorOptions().dtype(torch::kFloat16).device(torch::kCUDA)).set_requires_grad(true);
    denoising.main.pred_boxes = prepared->original_boxes.to(torch::kFloat16).clone().set_requires_grad(true);
    outputs.denoising = std::move(denoising);
    outputs.main.pred_logits = torch_api::zeros({1, 4, 4}, torch_api::TensorOptions().dtype(torch::kFloat16).device(torch::kCUDA)).set_requires_grad(true);
    outputs.main.pred_boxes = torch_api::zeros({1, 4, 4}, torch_api::TensorOptions().dtype(torch::kFloat16).device(torch::kCUDA)).set_requires_grad(true);
