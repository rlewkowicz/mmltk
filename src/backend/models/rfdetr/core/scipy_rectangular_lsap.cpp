...
void argsort_iter(const std::pmr::vector<intptr_t>& values, std::pmr::vector<intptr_t>& index) {
    ...
    std::ranges::sort(
        index, [&values](intptr_t lhs, intptr_t rhs) { return values[static_cast<size_t>(lhs)] < values[static_cast<size_t>(rhs)]; });
}

intptr_t augmenting_path(intptr_t num_cols, double* cost, std::pmr::vector<double>& u, std::pmr::vector<double>& v,
                         std::pmr::vector<intptr_t>& path, std::pmr::vector<intptr_t>& row_for_col,
                         std::pmr::vector<double>& shortest_path_costs, intptr_t row, std::pmr::vector<bool>& visited_rows,
                         std::pmr::vector<bool>& visited_cols, std::pmr::vector<intptr_t>& remaining, double* min_value) {
    ...
    while (sink == -1) {
        ...
        current_min = lowest;
        if (current_min == std::numeric_limits<double>::infinity()) { return -1; }
...
RectangularLsApStatus solve_impl(intptr_t num_rows, intptr_t num_cols, double* input_cost, bool maximize, int64_t* row_indices,
                                 int64_t* col_indices, RectangularLsApWorkspace& workspace) {
        ...
        if (transpose) {
            for (intptr_t row = 0; row < num_rows; ++row) {
                for (intptr_t col = 0; col < num_cols; ++col) {
                    temp[static_cast<size_t>(col * num_rows + row)] = input_cost[static_cast<size_t>(row * num_cols + col)];
                }
            }
    ...
    for (intptr_t i = 0; i < num_rows * num_cols; ++i) {
        ...
        if (value != value || value == -std::numeric_limits<double>::infinity()) { return RectangularLsApStatus::kInvalid; }
    }
    ...
    for (intptr_t row = 0; row < num_rows; ++row) {
        ...
        const intptr_t sink = augmenting_path(num_cols, input_cost, u, v, path, row_for_col, shortest_path_costs, row, visited_rows,
                                              visited_cols, remaining, &min_value);
...
RectangularLsApStatus solve_rectangular_linear_sum_assignment(int64_t num_rows, int64_t num_cols, const double* cost_matrix, bool maximize,
    ...
    return solve_impl(static_cast<intptr_t>(num_rows), static_cast<intptr_t>(num_cols), const_cast<double*>(cost_matrix), maximize,
                      row_indices, col_indices, workspace);
}
