...
intptr_t augmenting_path(intptr_t num_cols, double* cost, std::pmr::vector<double>& u, std::pmr::vector<double>& v, std::pmr::vector<intptr_t>& path,
 ...
 while (sink == -1) {
  ...
  for (intptr_t i = 0; i < num_remaining; ++i) {
   ...
   const double reduced_cost = current_min + cost[row * num_cols + col] - u[static_cast<size_t>(row)] - v[static_cast<size_t>(col)];
 ...
 if (transpose || maximize) {
  ...
  if (transpose) {
   for (intptr_t row = 0; row < num_rows; ++row) {
    for (intptr_t col = 0; col < num_cols; ++col) { temp[static_cast<size_t>(col * num_rows + row)] = input_cost[static_cast<size_t>(row * num_cols + col)]; }
   }
  ...
  if (maximize) {
   for (double& value : temp) { value = -value; }
  }
