...
PositionEmbeddingSineImpl::~PositionEmbeddingSineImpl() {
 ...
 for (auto& entry : cache.entries) {
  if (entry.ready_event) { entry.ready_event->wait(reinterpret_cast<std::uintptr_t>(settlement_stream.stream()), "settle position embedding cache readiness"); }
 }
...
torch::Tensor PositionEmbeddingSineImpl::build_full_valid_base(int64_t height, int64_t width, const torch::Device& device, bool align_dim_orders) const {
 auto y_embed = torch::arange(1, height + 1, torch::TensorOptions().dtype(torch::kFloat32).device(device)).view({1, height, 1}).expand({1, height, width});
 auto x_embed = torch::arange(1, width + 1, torch::TensorOptions().dtype(torch::kFloat32).device(device)).view({1, 1, width}).expand({1, height, width});
 if (normalize_) {
  constexpr double kEps = 1e-6;
...
SegmentationHeadImpl::SegmentationHeadImpl(int64_t in_dim, int64_t num_blocks, c10::optional<int64_t> bottleneck_ratio, int64_t downsample_ratio)
