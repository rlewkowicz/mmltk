...
torch::Tensor reparam_box_refine(const torch::Tensor& base, const torch::Tensor& delta) {
    auto cxcy =
        delta.index({Slice(), Slice(), Slice(None, 2)}) * base.index({Slice(), Slice(), Slice(2, None)}) + base.index({Slice(), Slice(), Slice(None, 2)});
   ...
   public:
    ConvXImpl(int64_t in_planes, int64_t out_planes, int64_t kernel = 3, int64_t stride = 1, int64_t groups = 1, bool layer_norm = false)
        : conv(register_module(
              "conv",
              torch::nn::Conv2d(torch::nn::Conv2dOptions(in_planes, out_planes, kernel).stride(stride).padding(kernel / 2).groups(groups).bias(false)))),
   ...
   public:
    explicit WindowedDinov2EmbeddingsImpl(const NativeRfDetrConfig& config)
        : cls_token(register_parameter("cls_token", torch::randn({1, 1, kDinoHiddenSize}, torch::kFloat32))),
          ...
          position_embeddings(
              register_parameter("position_embeddings",
    ...
    std::vector<torch::Tensor> forward(const torch::Tensor& pixel_values) {
        ...
        for (const auto stage : kOutFeatureStages) {
            ...
            if (num_windows_ > 1) {
                ...
                hidden_state =
                    hidden_state.reshape({batch_size * num_windows_, num_windows_, num_h_patches_per_window, num_w_patches_per_window, kDinoHiddenSize});
   ...
   public:
    ...
    torch::Tensor forward(const torch::Tensor& query, const torch::Tensor& reference_points, const torch::Tensor& input_flatten,
        ...
        if (value.is_cuda() && !force_pytorch_deformable_attn_) {
            attended = ms_deform_attn_cuda_autograd(value, feature_layout.spatial_shapes(), feature_layout.level_start_index(), sampling_locations, attention,
                                                    im2col_step_);
        } else {
   ...
   public:
    ...
    std::pair<torch::Tensor, torch::Tensor> forward(const torch::Tensor& tgt, const torch::Tensor& memory, const torch::Tensor& memory_key_padding_mask,
        ...
        for (int64_t layer_id = 0; layer_id < num_layers_; ++layer_id) {
            if (!lite_refpoint_refine_) { std::tie(refpoints_input, query_pos) = get_reference(bbox_reparam_ ? refpoints : refpoints.sigmoid()); }
            output = layers[layer_id]->as<NativeDecoderLayer>()->forward(output, memory, memory_key_padding_mask, query_pos, refpoints_input, feature_layout,
                                                                         query_layout);
...
class NativeTransformerImpl : public torch::nn::Module {
   public:
    ...
    NativeTransformerOutput forward(const std::vector<torch::Tensor>& srcs, const std::vector<torch::Tensor>& masks,
                                    const std::vector<torch::Tensor>& pos_embeds, const torch::Tensor& refpoint_embed, const torch::Tensor& query_feat,
                                    bool training_mode, const DenoisingQueryBatch* denoising, TrainingSupervisionImpl* supervision) {
        ...
        if (config_.dec_layers > 0) {
            ...
            try {
                std::tie(hidden_states, refs_unsigmoid) = decoder->forward(tgt, memory, mask_flatten_tensor, lvl_pos_embed_flatten, decoder_refpoints, layout,
...
NativeRfDetrModel::Impl::Impl(const NativeRfDetrConfig& config, ModelClassLayout layout)
    ...
    if (training_supervision_enabled(config_.training_supervision)) {
        ...
        try {
            training_supervision_ = register_module(std::string(detail::kTrainingSupervisionModule),
                                                    std::make_shared<TrainingSupervisionImpl>(config_, layout_->catalog()->size()));
        } catch (...) {
...
ModelOutputs NativeRfDetrModel::Impl::forward_impl(const NestedTensor& batch, const bool include_masks, const bool capture_match_free_features,
    ...
    poss.reserve(features.size());
    {
        ...
        for (const auto& feature : features) {
            ...
            poss.push_back(backbone_->at<PositionEmbeddingSineImpl>(1).forward_full_valid(feature.tensors.size(0), feature.tensors.size(2),
                                                                                          feature.tensors.size(3), feature.tensors.device()));
        }
    }
    ...
    if (include_masks && segmentation_head_ && !decoder_query_features.empty()) {
        ...
        if (is_training()) {
            sparse_masks =
                segmentation_head->sparse_forward(features.front().tensors, decoder_query_features, {samples.tensors.size(2), samples.tensors.size(3)});
        } else {
    ...
    if (!decoder_query_features.empty()) {
        ...
        for (size_t index = 0; index < decoder_query_features.size(); ++index) {
            ...
            if (outputs.denoising) {
                ...
                dn_layer.pred_logits = class_embed->forward(dn_hs).view(
                    {dn_hs.size(0), denoising->layout.denoising_groups, denoising->layout.denoising_queries_per_group, config_.num_classes});
    ...
    if (config_.two_stage && transformed.enc_memory.defined() && transformed.enc_boxes.defined()) {
        ...
        if (include_masks && segmentation_head_) {
            if (!segmentation_head) { throw std::runtime_error("RF-DETR segmentation head is not initialized"); }
            if (is_training()) {
                enc_output.sparse_pred_masks = segmentation_head->sparse_forward(features.front().tensors, {transformed.enc_memory},
            ...
            if (const auto found = axis_by_name.find(entry.name); found != axis_by_name.end()) {
                ...
                if (compatible) {
                    ...
                    if (known_axis && source_layout->semantic()) {
                        ...
                        if (boxes)
                            adapted.narrow(axis.dimension, layout_->catalog()->size(), 4)
                                .copy_(converted.narrow(axis.dimension, source_layout->catalog()->size(), 4));
                    }
