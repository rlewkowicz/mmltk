...
struct TrainingSupervisionImpl::DenoisingScratch {
    void ensure(const torch::Device& requested_device, const int64_t batch, const int64_t maximum_count) {
        static_cast<void>(mmltk::common::math::checked_multiply(static_cast<std::uint64_t>(batch), static_cast<std::uint64_t>(maximum_count),
                                                                "RF-DETR DN padding extent overflow"));
...
struct TrainingSupervisionImpl::TimingState {
    ...
    static const char* metric_name(const Stage stage) {
        switch (stage) {
            ...
            case Stage::GroundTruthProjection: return kGtProjectionMetric;
            case Stage::Affinity: return kAffinityMetric;
            case Stage::Sparse: return kSparseMetric;
            case Stage::BroadcastCost: return kBroadcastCostMetric;
            case Stage::Objective: return kObjectiveMetric;
            case Stage::DenoisingPrepare: return kDenoisingPrepareMetric;
            case Stage::DenoisingDecoder: return kDenoisingDecoderMetric;
            case Stage::DenoisingObjective: return kDenoisingObjectiveMetric;
            case Stage::SupervisedStep: return kSupervisedStepMetric;
            case Stage::Criterion: return kCriterionMetric;
        }
...
TrainingSupervisionImpl::TrainingSupervisionImpl(const NativeRfDetrConfig& config, const std::int64_t foreground_count)
    ...
    if (config.training_supervision.denoising.enabled) {
        ...
        denoising_label_embedding_ =
            register_module(std::string(detail::kDenoisingClassAxis.module_name()), torch::nn::Embedding(object_classes, config.hidden_dim));
...
void TrainingSupervisionImpl::install_replicated_initialized_runtime(const TrainingSupervisionConfig& source_config) {
    ...
    if (config_.training_supervision != source_config) { throw std::runtime_error("RF-DETR training supervision replicas have incompatible topology"); }
    initialized_ = true;
}
...
std::optional<DenoisingQueryBatch> TrainingSupervisionImpl::prepare_denoising(const PreparedTargets& targets, const TrainingStepIdentity& identity,
    ...
    if (decoder_dtype != torch::kFloat16 && decoder_dtype != torch::kBFloat16 && decoder_dtype != torch::kFloat32 && decoder_dtype != torch::kFloat64) {
    ...
    auto operation = [&]() -> DenoisingQueryBatch {
        ...
        if (!denoising_generator_ || denoising_generator_->device() != device) {
            denoising_generator_ =
                device.is_cuda()
                    ? at::cuda::detail::createCUDAGenerator(static_cast<c10::DeviceIndex>(device.has_index() ? device.index() : c10::cuda::current_device()))
        ...
        const int64_t object_classes = foreground_count_;
        auto transformed = transform_denoising_targets(labels, boxes, valid, object_classes, config_.training_supervision.denoising, *variates);
        ...
        const auto target_indices = torch::where(valid, padded.indices.unsqueeze(1).expand(slot_shape), torch::full(slot_shape, -1, integer_options));
    ...
    if (!device.is_cuda() || timing_->device_id != device.index()) { throw std::runtime_error("DN timing leases must match the decoder CUDA device"); }
...
void TrainingSupervisionImpl::configure_timing(const SupervisionTimingSetup& setup) {
    if (!setup.profiling_enabled) {
        if (timing_ && timing_->outstanding() != 0) { throw std::runtime_error("RF-DETR supervision timing cannot be disabled with outstanding leases"); }
        timing_.reset();
        return;
    }
    ...
    if (match_free_enabled()) {
        scopes_per_loss = mmltk::common::math::checked_add(
            ...
            mmltk::common::math::checked_add(
                std::uint64_t{1},
                mmltk::common::math::checked_multiply(std::uint64_t{4}, supervised_layers, "RF-DETR supervision timing scope capacity overflow"),
...
MatchFreeCost TrainingSupervisionImpl::broadcast_cost(const torch::Tensor& padded_labels, const torch::Tensor& padded_boxes, const torch::Tensor& valid_rows,
    ...
    if (layer.pred_logits.dim() != 3 || layer.pred_boxes.dim() != 3 || layer.pred_logits.size(2) != config_.num_classes || layer.pred_boxes.size(2) != 4 ||
        layer.pred_logits.size(1) != layout.total_queries() || layer.pred_boxes.size(1) != layout.total_queries()) {
    ...
    const auto label_index = padded_labels.unsqueeze(1).unsqueeze(2).expand({batch, layout.groups, layout.queries_per_group, padded_labels.size(1)});
...
TrainingLoss TrainingSupervisionImpl::empty_loss(const ModelOutputs& outputs) const {
    ...
    auto add_layer = [&](const OutputLayer& layer) {
        ...
        if (layer.query_layout) {
            const auto layout = *layer.query_layout;
            layer_zero = layer.pred_logits.view({layer.pred_logits.size(0), layout.groups, layout.queries_per_group, layer.pred_logits.size(-1)})
                             .index({0, Slice(), 0, 0})
                             .sum() *
                         0.0F;
            layer_zero = layer_zero +
                         layer.pred_boxes.view({layer.pred_boxes.size(0), layout.groups, layout.queries_per_group, 4}).index({0, Slice(), 0, 0}).sum() * 0.0F;
        } else {
    ...
    if (config_.aux_loss) {
        for (const auto& layer : outputs.aux_outputs) { add_layer(layer); }
...
TrainingLoss TrainingSupervisionImpl::loss(const ModelOutputs& outputs, const PreparedTargets& targets, const DeviceLossNormalizer& normalizer,
    ...
    if (mmltk::common::logging::profile_enabled()) {
        ...
        for (const auto count : targets.counts) {
            for (const auto* layer : layers) {
                const auto group_pairs = mmltk::common::math::checked_multiply(static_cast<std::uint64_t>(layer->query_layout->groups),
                                                                               static_cast<std::uint64_t>(layer->query_layout->queries_per_group),
                                                                               "Match-Free valid-pair group product overflow");
