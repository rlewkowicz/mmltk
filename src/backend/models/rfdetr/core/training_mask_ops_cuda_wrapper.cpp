...
torch::Tensor matcher_point_sample_cuda_forward(const torch::Tensor& input, const torch::Tensor& point_coords, bool nearest) {
    ...
    TORCH_CHECK(point_coords.dim() == 3 && coord_sizes[2] == 2, "matcher_point_sample_cuda_forward expects point_coords shaped [N,P,2] or [1,P,2]");
    TORCH_CHECK(coord_sizes[0] == 1 || coord_sizes[0] == input_sizes[0], "matcher_point_sample_cuda_forward point_coords batch must match input or be shared");
    ...
    launch_matcher_point_sample_cuda(training_mask_ops_launch::MatcherPointSampleLaunch{
        .input = input.data_ptr<float>(),
        .coords = point_coords.data_ptr<float>(),
        .output = output.data_ptr<float>(),
        .batch_size = input_sizes[0],
        .coord_batches = coord_sizes[0],
        .channels = input_sizes[1],
        .height = input_sizes[2],
        .width = input_sizes[3],
        .point_count = coord_sizes[1],
        .nearest = nearest,
...
torch::Tensor sample_packed_masks_cuda(const torch::Tensor& packed_mask_bits, int64_t height, int64_t width, const torch::Tensor& mask_indices,
    ...
    TORCH_CHECK(point_coords.dim() == 3 && coord_sizes[2] == 2, "sample_packed_masks_cuda expects point_coords shaped [N,P,2] or [1,P,2]");
    TORCH_CHECK(coord_sizes[0] == 1 || coord_sizes[0] == index_sizes[0], "sample_packed_masks_cuda point_coords batch must match mask_indices or be shared");
    ...
    if (erasure.defined()) {
        ...
        TORCH_CHECK(erasure.device() == packed_mask_bits.device() && erasure.scalar_type() == torch::kUInt8 && erasure.dim() == 2 &&
                        erasure.size(0) == packed_sizes[0] && erasure.size(1) == static_cast<int64_t>(sizeof(AugmentationSpatialErasure)) &&
                        erasure.is_contiguous(),
    ...
    if (transformed) {
        ...
        TORCH_CHECK(inverse_transforms.scalar_type() == torch::kFloat32 && inverse_transforms.dim() == 2 && inverse_transforms.size(1) == 6 &&
                        inverse_transforms.stride(1) == 1,
    ...
    if (occluded) {
        ...
        TORCH_CHECK(
            occluder_mask_indices.scalar_type() == torch::kInt64 && occluder_mask_indices.dim() == 1 && occluder_mask_indices.size(0) == packed_sizes[0],
        ...
        TORCH_CHECK(occluder_inverse_transforms.scalar_type() == torch::kFloat32 && occluder_inverse_transforms.dim() == 2 &&
                        occluder_inverse_transforms.size(0) == packed_sizes[0] && occluder_inverse_transforms.size(1) == 6 &&
                        occluder_inverse_transforms.stride(1) == 1,
    ...
    launch_sample_packed_masks_cuda(training_mask_ops_launch::PackedMaskSampleLaunch{
        .packed_bits = packed_mask_bits.data_ptr<int64_t>(),
        .mask_indices = mask_indices.data_ptr<int64_t>(),
        .coords = point_coords.data_ptr<float>(),
        .inverse_transforms = transformed ? inverse_transforms.data_ptr<float>() : nullptr,
        .occluder_mask_indices = occluded ? occluder_mask_indices.data_ptr<int64_t>() : nullptr,
        .occluder_inverse_transforms = occluded ? occluder_inverse_transforms.data_ptr<float>() : nullptr,
        .erasure = erasure.defined() ? reinterpret_cast<const AugmentationSpatialErasure*>(erasure.data_ptr<std::uint8_t>()) : nullptr,
        .output = output.data_ptr<float>(),
        .mask_count = index_sizes[0],
        .coord_batches = coord_sizes[0],
        .words_per_mask = packed_sizes[1],
        .height = height,
        .width = width,
        .point_count = coord_sizes[1],
        .transform_stride = transformed ? inverse_transforms.stride(0) : 0,
        .stream = at::cuda::getCurrentCUDAStream(),
    });
    return output;
}
