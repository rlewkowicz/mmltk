...
__global__ void matcher_cost_kernel(float* output, const Logit* pred_logits, const Box* pred_boxes, const int64_t* target_labels,
                                    const float* target_boxes, const int64_t* target_offsets, const int64_t* target_counts,
                                    int64_t batch_size, int64_t query_count, int64_t class_count, int64_t output_query_stride,
                                    int64_t max_targets, int64_t total_targets, float class_cost, float bbox_cost, float giou_cost) {
    ...
    const int64_t target_index = target_offset + target_slot;
...
__global__ void matcher_mask_cost_kernel(float* output, const Logit* pred_logits, const float* target_masks, const int64_t* target_offsets,
                                         const int64_t* target_counts, int64_t batch_size, int64_t query_count, int64_t output_query_stride,
                                         int64_t max_targets, int64_t total_targets, int64_t point_count, float ce_cost, float dice_cost) {
    ...
    if (warp_index >= pair_count) { return; }
...
int64_t checked_extent(const std::initializer_list<int64_t> factors, const int64_t limit, const char* context) {
    int64_t product = 1;
    for (const int64_t factor : factors) {
        TORCH_CHECK(factor >= 0 && (product == 0 || factor <= limit / product), context,
                    " dimensions exceed the checked pointer/launch range");
        product *= factor;
    }
    return product;
}

float checked_cost_coefficient(const double value, const char* name) {
    TORCH_CHECK(std::isfinite(value) && std::abs(value) <= static_cast<double>(std::numeric_limits<float>::max()), name,
...
void pairwise_detection_cost_cuda_out(const torch::Tensor& output, const torch::Tensor& pred_logits, const torch::Tensor& pred_boxes,
                                      const torch::Tensor& target_labels, const torch::Tensor& target_boxes,
                                      const torch::Tensor& target_offsets, const torch::Tensor& target_counts, double class_cost,
                                      double bbox_cost, double giou_cost) {
    ...
    TORCH_CHECK(pred_boxes.size(0) == pred_logits.size(0) && pred_boxes.size(1) == pred_logits.size(1) && pred_boxes.size(2) == 4,
    ...
    TORCH_CHECK(checked_extent({batch, queries, classes}, std::numeric_limits<int64_t>::max(), "matcher logits pointer extent") ==
                    pred_logits.numel(),
                "matcher logits storage does not match its shape");
    TORCH_CHECK(checked_extent({batch, queries, 4}, std::numeric_limits<int64_t>::max(), "matcher prediction-box pointer extent") ==
                    pred_boxes.numel(),
                "matcher prediction-box storage does not match its shape");
    TORCH_CHECK(checked_extent({total_targets, 4}, std::numeric_limits<int64_t>::max(), "matcher target-box pointer extent") ==
                    target_boxes.numel(),
                "matcher target-box storage does not match its shape");
    const int64_t output_extent =
        checked_extent({batch, output.size(1), max_targets}, std::numeric_limits<int64_t>::max(), "matcher output pointer extent");
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::ScalarType::Half, at::ScalarType::BFloat16, pred_logits.scalar_type(), "pairwise_detection_cost_logits", [&] {
            ...
            AT_DISPATCH_FLOATING_TYPES_AND2(
                at::ScalarType::Half, at::ScalarType::BFloat16, pred_boxes.scalar_type(), "pairwise_detection_cost_boxes", [&] {
                    using Box = scalar_t;
                    matcher_cost_kernel<Logit, Box><<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                        ...
                        target_labels.data_ptr<int64_t>(), target_boxes.data_ptr<float>(), target_offsets.data_ptr<int64_t>(),
                        target_counts.data_ptr<int64_t>(), batch, queries, classes, output.size(1), max_targets, total_targets,
                        checked_class_cost, checked_bbox_cost, checked_giou_cost);
                });
        });
    TORCH_CHECK(cudaGetLastError() == cudaSuccess, "matcher cost CUDA kernel launch failed");
}

void pairwise_mask_cost_cuda_add_(const torch::Tensor& output, const torch::Tensor& pred_mask_logits, const torch::Tensor& target_masks,
                                  const torch::Tensor& target_offsets, const torch::Tensor& target_counts, double ce_cost,
                                  double dice_cost) {
    ...
    TORCH_CHECK(output.size(0) >= pred_mask_logits.size(0) && target_offsets.size(0) == pred_mask_logits.size(0) &&
                    target_counts.size(0) == pred_mask_logits.size(0),
    ...
    TORCH_CHECK(checked_extent({batch, queries, points}, std::numeric_limits<int64_t>::max(), "matcher mask-logit pointer extent") ==
                    pred_mask_logits.numel(),
                "matcher mask-logit storage does not match its shape");
    TORCH_CHECK(checked_extent({total_targets, points}, std::numeric_limits<int64_t>::max(), "matcher target-mask pointer extent") ==
                    target_masks.numel(),
                "matcher target-mask storage does not match its shape");
    const int64_t output_extent =
        checked_extent({batch, output.size(1), max_targets}, std::numeric_limits<int64_t>::max(), "matcher mask output pointer extent");
    ...
    const int64_t pair_count = checked_extent({batch, queries, max_targets},
                                              static_cast<int64_t>(std::numeric_limits<int>::max()) * warps_per_block, "matcher mask cost");
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, pred_mask_logits.scalar_type(), "pairwise_mask_cost",
                                    [&] {
                                        matcher_mask_cost_kernel<scalar_t><<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                                            ...
                                            target_offsets.data_ptr<int64_t>(), target_counts.data_ptr<int64_t>(), batch, queries,
                                            output.size(1), max_targets, total_targets, points, checked_ce_cost, checked_dice_cost);
                                    });
