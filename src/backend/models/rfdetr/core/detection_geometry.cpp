...
void require_box_tensor(const torch::Tensor& boxes, const char* what) {
    if (!boxes.defined() || boxes.dim() < 2 || boxes.size(-1) != 4) {
        throw std::runtime_error(std::string(what) + " expects a [...,N,4] box tensor");
    }
}
...
torch::Tensor span_area(const torch::Tensor& lhs_lower, const torch::Tensor& rhs_lower, const torch::Tensor& lhs_upper,
                        const torch::Tensor& rhs_upper, const BoxSpan span) {
...
void require_pairwise_prefix(const torch::Tensor& boxes1, const torch::Tensor& boxes2, const char* what) {
    ...
    if (boxes1.dim() != boxes2.dim() ||
        boxes1.sizes().slice(0, boxes1.dim() - 2).vec() != boxes2.sizes().slice(0, boxes2.dim() - 2).vec()) {
...
torch::Tensor pairwise_box_iou(const torch::Tensor& boxes1, const torch::Tensor& boxes2) {
    ...
    if (boxes1.dim() == 2 && boxes1.is_cuda() && boxes2.is_cuda() && boxes1.scalar_type() == torch::kFloat32 &&
...
torch::Tensor pairwise_generalized_box_iou(const torch::Tensor& boxes1, const torch::Tensor& boxes2) {
    ...
    const auto enclosure = span_area(boxes1.slice(-1, 0, 2).unsqueeze(-2), boxes2.slice(-1, 0, 2).unsqueeze(-3),
                                     boxes1.slice(-1, 2, 4).unsqueeze(-2), boxes2.slice(-1, 2, 4).unsqueeze(-3), BoxSpan::Enclosure);
...
torch::Tensor aligned_box_iou(const torch::Tensor& boxes1, const torch::Tensor& boxes2) {
    ...
    const auto intersection =
        span_area(boxes1.slice(-1, 0, 2), boxes2.slice(-1, 0, 2), boxes1.slice(-1, 2, 4), boxes2.slice(-1, 2, 4), BoxSpan::Intersection);
...
torch::Tensor aligned_generalized_box_iou(const torch::Tensor& boxes1, const torch::Tensor& boxes2) {
    ...
    const auto enclosure =
        span_area(boxes1.slice(-1, 0, 2), boxes2.slice(-1, 0, 2), boxes1.slice(-1, 2, 4), boxes2.slice(-1, 2, 4), BoxSpan::Enclosure);
...
torch::Tensor batched_pairwise_generalized_box_iou(const torch::Tensor& boxes1, const torch::Tensor& boxes2) {
    if (boxes1.dim() < 3 || boxes2.dim() < 3) {
        throw std::runtime_error("batched_pairwise_generalized_box_iou expects [...,M,4] and [...,N,4]");
    }
