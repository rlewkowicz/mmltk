...
RuntimeConfig resolve_runtime_config(int requested_workers, int requested_lanes, int loader_prefetch_factor,
                                     const std::string& cpu_affinity_value, int device, int numa_node) {
...
RuntimeSplit split_runtime_workers(const RuntimeConfig& config) {
    ...
    if (split.loader_threads != requested_loader_threads || split.lane_threads != requested_lanes) {
        mmltk::common::logging::warn([&](auto& logger) {
            logger.warn("rfdetr.runtime worker split clamped loader={}->{} lanes={}->{} cpu={} total={}", requested_loader_threads,
...
RuntimeContext::RuntimeContext(const RuntimeConfig& config)
    ...
    for (int worker = 0; worker < split_.cpu_threads; ++worker)
        solver_workspaces_.push_back(std::make_unique<LsapScratch>(&solver_memory_));
    ...
    std::call_once(initialized, [&] {
        ...
        try {
            ...
            at::parallel_for(0, std::max(2, at::get_num_threads()) * 2, 1, [&](std::int64_t, std::int64_t) {
                try {
                    if (at::get_num_threads() > 1 && config_.library_cpus.size() > 1 &&
                        mmltk::common::system::allowed_cpu_set().size() == 1)
    ...
    cpu_pool_ = std::make_shared<mmltk::common::concurrency::WorkerPool>(static_cast<size_t>(split_.cpu_threads), cpu_cpus_, "rfdetrcpu",
                                                                         0U, &config_.execution.placement, false);
}
