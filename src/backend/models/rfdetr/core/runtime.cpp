...
MatcherWorkspace* active_matcher_workspace() { return g_matcher_workspace ? g_matcher_workspace : (g_runtime_context ? &g_runtime_context->matcher_workspace() : nullptr); }
MatcherWorkspace& RuntimeContext::matcher_workspace() { return *matcher_workspaces_.at(g_runtime_lane); }
RuntimeConfig resolve_runtime_config(int requested_workers, int requested_lanes, int loader_prefetch_factor, const std::string& cpu_affinity_value, int device, int numa_node) {
 if (loader_prefetch_factor < 0 || loader_prefetch_factor == std::numeric_limits<int>::max()) { throw std::runtime_error("RF-DETR runtime loader prefetch factor is out of range"); }
 ...
 if (available_workers >= 3) {
  ...
  mmltk::common::logging::warn(
   [&](auto& logger) { logger.warn("rfdetr.runtime cpuset={} has fewer than 3 CPUs; runtime helper threads will overlap", mmltk::common::system::format_cpu_list(config.cpu_affinity)); });
 }
...
RuntimeSplit split_runtime_workers(const RuntimeConfig& config) {
 ...
 if (split.loader_threads != requested_loader_threads || split.lane_threads != requested_lanes) {
  mmltk::common::logging::warn([&](auto& logger) {
   logger.warn(
    "rfdetr.runtime worker split clamped loader={}->{} lanes={}->{} cpu={} total={}", requested_loader_threads, split.loader_threads, requested_lanes, split.lane_threads, split.cpu_threads, total);
  });
 }
 return split;
}
RuntimeContext::RuntimeContext(const RuntimeConfig& config) : config_(config), split_(split_runtime_workers(config)), solver_memory_(config.execution.placement.numa_node) {
 for (int lane = 0; lane <= split_.lane_threads; ++lane) matcher_workspaces_.push_back(std::make_unique<MatcherWorkspace>(config.execution.placement.numa_node, config.h2d_dataloader));
 ...
 std::call_once(initialized, [&] {
  ...
  try {
   ...
   at::parallel_for(0, std::max(2, at::get_num_threads()) * 2, 1, [&](std::int64_t, std::int64_t) {
    try {
     if (at::get_num_threads() > 1 && config_.library_cpus.size() > 1 && mmltk::common::system::allowed_cpu_set().size() == 1) bad_affinity.store(true);
 ...
 cpu_pool_ = std::make_shared<mmltk::common::concurrency::WorkerPool>(static_cast<size_t>(split_.cpu_threads), cpu_cpus_, "rfdetrcpu", 0U, &config_.execution.placement, false);
}
