...
class ClassArtifactAdmission final {
 public:
    explicit ClassArtifactAdmission(const std::filesystem::path& artifact,
    ...
    [[nodiscard]] ModelClassLayout Resolve(std::size_t output_width,
        const std::optional<ModelClassLayout>& embedded, std::stop_token stop = {}) const;
