...
[[nodiscard]] DenoisingTransform transform_denoising_targets(const torch::Tensor& original_labels, const torch::Tensor& original_boxes,
                                                             const torch::Tensor& valid_slots, int64_t object_classes,
...
class TrainingSupervisionImpl final : public torch::nn::Module {
   public:
    ...
    [[nodiscard]] MatchFreeCorrespondence correspondence(const torch::Tensor& padded_labels, const torch::Tensor& padded_boxes,
                                                         const torch::Tensor& valid_rows, const torch::Tensor& query_features);
    [[nodiscard]] MatchFreeCost broadcast_cost(const torch::Tensor& padded_labels, const torch::Tensor& padded_boxes,
                                               const torch::Tensor& valid_rows, const OutputLayer& layer) const;
    [[nodiscard]] TrainingLoss loss(const ModelOutputs& outputs, const PreparedTargets& targets, const DeviceLossNormalizer& normalizer,
                                    bool training_mode);
   ...
   private:
    ...
    [[nodiscard]] PaddedTargets pad_targets(const PreparedTargets& targets, const torch::Device& device, int64_t batch,
                                            bool reuse_construction_scratch);
