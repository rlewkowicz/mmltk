...
class ClassTensorFamily final {
 public:
    constexpr ClassTensorFamily(std::string_view owner, std::string_view module, std::string_view tensor,
        bool indexed, std::int64_t dimension, ClassTensorCoordinates coordinates)
    ...
    [[nodiscard]] std::string prefix() const {
        return (owner_.empty() ? std::string{} : std::string(owner_) + ".") + std::string(module_) + ".";
    }
    [[nodiscard]] constexpr std::optional<ClassTensorShape> Match(std::string_view name) const noexcept {
        ...
        if (indexed_) {
            ...
            for (std::size_t index = 0; index < end; ++index) if (name[index] < '0' || name[index] > '9') return std::nullopt;
            name.remove_prefix(end + 1);
        }
        if (!tensor_.empty()) {
            if (name != tensor_) return std::nullopt;
        } else if (name != "weight" && name != "bias") return std::nullopt;
