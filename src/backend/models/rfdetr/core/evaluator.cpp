...
ImageMatchingScratch& image_matching_scratch(const size_t category_count) {
    ...
    for (auto& predictions : scratch.predictions_by_category) { predictions.clear(); }
    return scratch;
}
std::uint16_t eligible_threshold_bits(const double iou) {
    ...
    for (size_t threshold_index = 0; threshold_index < std::size(kIouThresholds); ++threshold_index) {
        if (iou >= kIouThresholds[threshold_index]) { bits |= static_cast<std::uint16_t>(std::uint16_t{1} << threshold_index); }
    }
    return bits;
}
    ...
    for (const std::uint32_t prediction_index : prediction_indices) {
        ...
        for (size_t ground_truth_index = 0; ground_truth_index < ground_truth_ordinals.size(); ++ground_truth_index) {
            ...
            scratch.candidates.push_back(
                {iou, static_cast<std::uint32_t>(ground_truth_index), ground_truth_ordinals[ground_truth_index], eligible_threshold_bits(iou)});
        }
...
CocoDataset CocoDataset::load_from_loader(const mmltk::backend::data::DatasetLoader& loader, const EvaluationMetricSet metric_set) {
    ...
    if (image_count > std::numeric_limits<std::uint32_t>::max() || image_count > static_cast<size_t>(std::numeric_limits<int>::max()) || num_classes == 0 ||
        num_classes > std::numeric_limits<std::uint16_t>::max() || label_count > std::numeric_limits<std::uint32_t>::max() ||
    ...
    for (size_t image_index = 0; image_index < image_count; ++image_index) {
        ...
        for (uint16_t annotation_ordinal = 0; annotation_ordinal < entry.num_instances; ++annotation_ordinal) {
            ...
            if (packed.class_id >= num_classes || !std::ranges::all_of(bbox, [](const float coordinate) { return std::isfinite(coordinate); }) ||
    ...
    if (next_label_index != label_count) { throw std::runtime_error("compiled evaluation annotation cardinality does not match the label index"); }
    ...
    for (const GroundTruthSpan& span : out.ground_truth_spans_) { span_cursors.push_back(span.offset); }
    ...
    for (size_t span_index = 0; span_index < out.ground_truth_spans_.size(); ++span_index) {
        ...
        if (span_cursors[span_index] != span.offset + span.count) { throw std::runtime_error("compiled evaluation dense-span fill mismatch"); }
    }
...
void CocoDataset::reset_match_storage() {
    ...
    for (auto& category_matches : bbox_matches_by_category_) { category_matches.clear(); }
    if (metric_set_ == EvaluationMetricSet::BBoxAndMask) {
        ...
        for (auto& category_matches : *mask_matches_by_category_) { category_matches.clear(); }
    } else {
...
void CocoDataset::limit_images(const size_t limit) {
    ...
    if (ground_truth_masks_) {
        ...
        for (const GroundTruthMask& mask : masks) { retained_run_count = std::max(retained_run_count, static_cast<size_t>(mask.run_offset) + mask.run_count); }
        mask_runs.resize(retained_run_count);
    }
    image_ids_.resize(limit);
    image_id_to_index_.clear();
    for (size_t index = 0; index < image_ids_.size(); ++index) { image_id_to_index_.emplace(image_ids_[index], index); }
    ...
    if (mask_matches_by_category_) {
        for (auto& category_matches : *mask_matches_by_category_) { std::erase_if(category_matches, remove_outside_limit); }
    }
...
ImageEvaluationMatches CocoDataset::match_staged_predictions(const std::int64_t dataset_index, const BBoxPredictionView& bbox,
    ...
    if (bbox.image_id != image_ids_[image_index]) { throw std::invalid_argument("staged prediction image ID does not match the image matcher index"); }
    ...
    if (mask && ((bbox.count != 0U && mask->data == nullptr) || mask->prediction_stride <= 0 ||
                 static_cast<std::uint64_t>(mask->prediction_stride) < packed_mask_bytes || mask->height != ground_truth_mask_height_ ||
                 mask->width != ground_truth_mask_width_)) {
    ...
    if (mask_mode) {
        ...
        const auto prediction_mask_at = [&](size_t index) -> const EncodedMask& { return mask ? scratch.staged_masks[index] : encoded_masks[index].mask; };
...
void CocoDataset::merge_matches(ImageEvaluationMatches&& matches) {
    ...
    if (mask_records != nullptr) {
        for (const CompactImageMatchRecord& record : *mask_records) {
            if (record.category_index >= mask_matches_by_category->size()) { throw std::out_of_range("mask match category is outside the evaluation dataset"); }
        }
    }
    matched_max_dets_per_image_ = matches.max_dets_per_image;
    for (CompactImageMatchRecord& record : matches.bbox) { bbox_matches_by_category_[record.category_index].push_back(record); }
    if (mask_records != nullptr) {
        for (const CompactImageMatchRecord& record : *mask_records) { (*mask_matches_by_category)[record.category_index].push_back(record); }
    }
...
EvalSummary CocoDataset::evaluate(const size_t max_dets_per_image, EvaluationDetailRetention retention,
    ...
    const auto reduce_metric = [this, worker_pool, &caps, retain_details](auto& matches_by_category, MetricScratch& scratch, EvaluationMetricKind kind) {
        ...
        for (size_t area = 0; area < kEvaluationAreaCount; ++area) {
            ...
            if (aggregate.available) {
                for (size_t iou = 0; iou < kIouThresholds.size(); ++iou) {
                    ...
                    if (aggregate_detail)
                        for (double& precision : aggregate_detail->precision_curve[iou]) precision /= static_cast<double>(available_categories);
                }
            }
            if (aggregate_detail) {
            ...
            if (area == 0U) {
                ...
                if (aggregate.available)
                    for (size_t cap = 0; cap < caps.size(); ++cap) summary.average_recall[cap] = ar[cap];
...
struct EvaluationDatasetOwner::Impl final {
    Impl(mmltk::backend::data::DatasetLoader& loader, const EvaluationMetricSet metric_set) : dataset(CocoDataset::load_from_loader(loader, metric_set)) {}
