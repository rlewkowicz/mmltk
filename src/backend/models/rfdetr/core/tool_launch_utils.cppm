...
export module mmltk.backend.models.rfdetr.core.tool_launch_utils;
...
inline std::filesystem::path resolve_sibling_tool_path(std::string_view tool_name, std::string_view env_override = {}) {
    if (!env_override.empty()) {
        ...
        if (const char* override_path = std::getenv(env_name.c_str()); override_path != nullptr && override_path[0] != '\0') {
