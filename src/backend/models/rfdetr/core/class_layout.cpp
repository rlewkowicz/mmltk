...
ResolvedClassLayout::ResolvedClassLayout(ModelClassLayout record) : record_(std::move(record)) {
    ...
    if (!record_.provenance.artifact_sha256.empty() && (record_.provenance.artifact_sha256.size() != 64 ||
    ...
    if (record_.provenance.origin > ClassLayoutOrigin::VerifiedAsset)
        throw std::invalid_argument("unknown RF-DETR class provenance");
    if (record_.class_name_evidence.names.size() > catalog::kClassCatalogCapacity)
        throw std::invalid_argument("oversized class-name evidence");
    ...
    if (record_.supervision_in_foreground_order && !semantic_)
        throw std::invalid_argument("unresolved class layout cannot declare foreground supervision");
...
ModelClassLayoutSummary ResolvedClassLayout::summary() const {
    ModelClassLayoutSummary result{.domain=domain(), .foreground_count=static_cast<std::uint32_t>(catalog_->size()),
        .output_count=static_cast<std::uint32_t>(output_width()), .scores=record_.scores, .no_object=record_.no_object,
...
void ResolvedClassLayout::require_execution(bool require_semantic) const {
    if (record_.scores != ClassScoreEncoding::SigmoidLogits)
        throw std::invalid_argument("RF-DETR execution requires declared sigmoid logits");
...
ModelClassLayout native_training_class_layout(const catalog::ClassCatalog& catalog) {
    ...
    for (std::size_t index = 0; index < catalog.size(); ++index)
        result.slots.push_back({ClassSlotRole::Foreground, static_cast<std::uint32_t>(index)});
...
std::string encode_record(const Record& record) {
    ...
    return nlohmann::json::from_cbor(begin, begin + bytes.size()).dump();
}
template<class Record>
Record decode_record(std::string_view text) {
    ...
    const auto bytes = nlohmann::json::to_cbor(nlohmann::json::parse(text));
    auto decoded = mmltk::frameworks::serialization::decode<Record>(
        {.first = {reinterpret_cast<const std::byte*>(bytes.data()), bytes.size()}},
        {.max_bytes=kClassLayoutByteBudget, .max_items=kClassLayoutByteBudget});
...
void validate_descriptor(const ModelClassDescriptor& descriptor) {
    ...
    for (std::size_t index = 0; index < descriptor.output_roles.size(); ++index) {
        ...
        if (output.name.empty() || output.name.size() > 1024 || output.name.find('\0') != std::string::npos ||
            role == 0 || role >= roles.size() || roles[role]) throw std::invalid_argument("invalid descriptor output role");
        roles[role] = true;
...
std::vector<RfdetrNamedOutputRole> class_descriptor_output_roles(std::span<const ModelClassDescriptor> descriptors) {
    ...
    for (const auto& descriptor : descriptors) for (const auto& declared : descriptor.output_roles) {
        ...
        if (previous == roles.end()) roles.push_back(declared);
        else if (previous->role != declared.role) throw std::invalid_argument("authoritative output roles disagree");
    }
...
ModelClassLayout admit_artifact_class_layout(std::size_t output_width, const std::optional<ModelClassLayout>& embedded,
    ...
    for (const auto& descriptor : descriptors) {
        ...
        if (selected) {
            ...
            if (previous.semantic() && *selected != next)
                throw std::invalid_argument("authoritative class layouts disagree");
            if (!previous.semantic() && next.class_name_evidence.names.empty())
                next.class_name_evidence = selected->class_name_evidence;
        }
