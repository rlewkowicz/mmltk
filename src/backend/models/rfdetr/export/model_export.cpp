...
void export_model_onnx(NativeRfDetrModel& model, const std::filesystem::path& output_path, const int opset_version, const int batch_size, const bool simplify,
    ...
    try {
        ...
        for (const auto& tensor : export_module.named_parameters(true)) { collect(tensor.name, tensor.value); }
        for (const auto& tensor : export_module.named_buffers(true)) { collect(tensor.name, tensor.value); }
        ...
        auto exported = torch::jit::export_onnx(graph, {}, static_cast<std::int64_t>(opset_version), dynamic_axes, false,
