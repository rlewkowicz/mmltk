...
[[nodiscard]] std::string onnx_tensor_dtype_name(const int elem_type) {
 ...
 switch (static_cast<DataType>(elem_type)) {
  case DataType::TensorProto_DataType_FLOAT: return "float32";
  case DataType::TensorProto_DataType_FLOAT16: return "float16";
  case DataType::TensorProto_DataType_DOUBLE: return "float64";
  case DataType::TensorProto_DataType_INT8: return "int8";
  case DataType::TensorProto_DataType_INT16: return "int16";
  case DataType::TensorProto_DataType_INT32: return "int32";
  case DataType::TensorProto_DataType_INT64: return "int64";
  case DataType::TensorProto_DataType_UINT8: return "uint8";
  case DataType::TensorProto_DataType_UINT16: return "uint16";
  case DataType::TensorProto_DataType_BOOL: return "bool";
  case DataType::TensorProto_DataType_BFLOAT16: return "bfloat16";
...
[[nodiscard]] TensorInfo tensor_info_from_value_info(const ONNX_NAMESPACE::ValueInfoProto& value_info) {
 ...
 for (const auto& dimension : tensor_type.shape().dim()) { info.shape.push_back(dimension.has_dim_value() ? dimension.dim_value() : -1); }
 return info;
}
...
ModelInfo load_onnx_model_info(const std::filesystem::path& model_path, std::span<const RfdetrNamedOutputRole> roles) {
 ...
 for (const auto& initializer : graph.initializer()) { initializer_names.insert(initializer.name()); }
 ...
 for (const auto& output : graph.output()) { info.outputs.push_back(tensor_info_from_value_info(output)); }
