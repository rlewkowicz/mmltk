...
void launch_gpu_rgba8_to_planar_float(const std::uint8_t* input, float* output, std::int64_t batch_size, int height, int width,
                                      cudaStream_t stream);

void launch_gpu_batch_normalization(const float* input, void* output, std::int64_t active_batch_size, std::int64_t output_batch_size,
                                    int height, int width, GpuPreprocessOutputType output_type, cudaStream_t stream);
...
void launch_gpu_augmentation_images(const float* input, float* output, const float* parameters, const float* copy_paste_parameters,
                                    ...
                                    std::uint64_t sequence, bool remap, GpuAugmentationOutputDomain output_domain, cudaStream_t stream,
                                    const float* const* input_slots = nullptr, const float* const* donor_slots = nullptr);
...
void launch_gpu_augmentation_images_explicit(const float* input, float* output, const float* parameters, const float* copy_paste_parameters,
                                             const float* donor_images, const std::int64_t* donor_masks, const float* donor_boxes,
                                             std::int64_t donor_mask_words, const std::uint64_t* image_keys, std::int64_t batch_size,
                                             int height, int width, const GpuAugmentationLaunchConfig& config, bool remap,
                                             GpuAugmentationOutputDomain output_domain, cudaStream_t stream,
                                             const float* const* input_slots = nullptr, const float* const* donor_slots = nullptr);
