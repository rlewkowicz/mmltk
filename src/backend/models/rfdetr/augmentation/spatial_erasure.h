...
[[nodiscard]] __host__ __device__ __forceinline__ bool erases_pixel(
 const AugmentationSpatialErasure& erasure, const std::int64_t x, const std::int64_t y, const std::int64_t width, const std::int64_t height) {
 if (erasure.dropout_probability > 0.0F && uniform01(erasure.key, static_cast<std::uint64_t>(y * width + x) * 32ULL + 0x100DULL) < erasure.dropout_probability) { return true; }
...
[[nodiscard]] __host__ __device__ __forceinline__ bool erases_sample(const AugmentationSpatialErasure& erasure, const float x, const float y, const std::int64_t width, const std::int64_t height) {
