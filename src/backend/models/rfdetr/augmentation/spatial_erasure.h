...
[[nodiscard]] __host__ __device__ __forceinline__ std::uint64_t mix64(std::uint64_t value) {
    value += kGoldenRatio;
...
[[nodiscard]] __host__ __device__ __forceinline__ bool erases_pixel(const AugmentationSpatialErasure& erasure, const std::int64_t x,
                                                                    const std::int64_t y, const std::int64_t width,
...
[[nodiscard]] __host__ __device__ __forceinline__ bool erases_sample(const AugmentationSpatialErasure& erasure, const float x,
                                                                     const float y, const std::int64_t width, const std::int64_t height) {
    if (erasure.dropout_probability <= 0.0F && erasure.rectangular == 0U) { return false; }
    const auto pixel_x = support_pixel_index(x, width);
    const auto pixel_y = support_pixel_index(y, height);
