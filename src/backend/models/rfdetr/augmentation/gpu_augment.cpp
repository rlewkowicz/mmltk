...
struct GpuAugmentationExecutor::Impl final {
    ...
    bool prepare_reductions(const GpuAugmentationBatchView& batch, const GpuAugmentationDonorBatchView& donors, const float* input,
        ...
        for (std::size_t image = 0; image != count; ++image) {
            ...
            require(donors.image_capacity_bytes >= image_bytes * (donors.image_slots.empty() ? donor + 1U : 1U),
                    "augmentation donor logical extent is too small");
...
const AugmentationBatchPlan& GpuAugmentationExecutor::RunTraining(const GpuAugmentationBatchView& batch, const std::uint64_t seed, const int epoch,
    ...
    return RunImpl(batch, std::span{impl_->training_keys}.first(batch.image_indices.size()), donors, donor_batch, stream, staging_slot, false, seed, epoch,
                   rank, sequence);
}
const AugmentationBatchPlan& GpuAugmentationExecutor::RunImpl(const GpuAugmentationBatchView& batch, const std::span<const std::uint64_t> image_keys,
    ...
    try {
        ...
        const bool has_prepared = impl_->prepare_reductions(
            batch, donor_batch, augmentation_input,
        ...
        if (explicit_keys) {
            launch_gpu_augmentation_images_explicit(augmentation_input, batch.output, impl_->parameters.data(), paste_parameters, donor_images, donor_masks,
                                                    donor_boxes, donor_mask_words, impl_->keys.data(), batch_size, impl_->height, impl_->width, launch,
                                                    impl_->remap, batch.output_domain, stream, input_slots, donor_slots, prepared);
        } else {
