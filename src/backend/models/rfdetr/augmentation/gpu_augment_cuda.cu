...
__device__ __forceinline__ void apply_effects(float& red, float& green, float& blue, const float* values, const std::uint64_t key,
                                              const std::int64_t pixel_index, const int output_x, const int output_y, const int width,
                                              const int height, const GpuAugmentationOutputDomain output_domain) {
    const float input_red = red;
    const float input_green = green;
    const float input_blue = blue;
    red = values[kColorMatrix + 0] * input_red + values[kColorMatrix + 1] * input_green + values[kColorMatrix + 2] * input_blue +
          values[kColorOffset + 0];
    green = values[kColorMatrix + 3] * input_red + values[kColorMatrix + 4] * input_green + values[kColorMatrix + 5] * input_blue +
            values[kColorOffset + 1];
    blue = values[kColorMatrix + 6] * input_red + values[kColorMatrix + 7] * input_green + values[kColorMatrix + 8] * input_blue +
           values[kColorOffset + 2];
...
__global__ void pointwise_images_kernel(const float* input, float* output, const float* parameters, const std::uint64_t* image_keys,
                                        const std::int64_t batch_size, const int height, const int width, const std::uint64_t seed,
                                        const int epoch, const int rank, const std::uint64_t sequence,
                                        const GpuAugmentationOutputDomain output_domain, const float* const* input_slots) {
    const int groups_per_row = static_cast<int>(ceil_div(width, 4));
    const std::int64_t group_index = static_cast<std::int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
...
#pragma unroll
        for (int channel = 0; channel < 3; ++channel) {
            const std::int64_t source_offset =
                ((source_image * 3 + channel) * height + source_y) * static_cast<std::int64_t>(width) + source_x;
...
#pragma unroll
    for (int lane = 0; lane < 4; ++lane) {
        ...
        apply_effects(red_values[lane], green_values[lane], blue_values[lane], values, key, pixel_index, x, output_y, width, height,
                      output_domain);
    }
...
__device__ __forceinline__ bool packed_mask_contains(const std::int64_t* packed_masks, const std::int64_t words_per_mask, const int slot,
                                                     const int height, const int width, const float source_x, const float source_y) {
...
template <bool ExplicitKeys>
__global__ void remap_images_kernel(const float* input, float* output, const float* parameters, const float* copy_paste_parameters,
...
__global__ void update_donor_images_kernel(const float* source_images, float* donor_images, const std::int64_t* source_ordinals,
...
__global__ void rgba8_to_planar_float_kernel(const std::uint8_t* input, float* output, const std::int64_t image_pixels,
...
void launch_gpu_rgba8_to_planar_float(const std::uint8_t* input, float* output, const std::int64_t batch_size, const int height,
                                      const int width, cudaStream_t stream) {
...
void launch_gpu_batch_normalization(const float* input, void* output, const std::int64_t active_batch_size,
    ...
    switch (output_type) {
        case GpuPreprocessOutputType::Float32:
            normalize_images_kernel<<<blocks, kThreads, 0, stream>>>(input, static_cast<float*>(output), active_batch_size,
                                                                     output_batch_size, height, width);
            break;
        case GpuPreprocessOutputType::Float16:
            normalize_images_kernel<<<blocks, kThreads, 0, stream>>>(input, static_cast<__half*>(output), active_batch_size,
                                                                     output_batch_size, height, width);
            break;
        case GpuPreprocessOutputType::BFloat16:
            normalize_images_kernel<<<blocks, kThreads, 0, stream>>>(input, static_cast<__nv_bfloat16*>(output), active_batch_size,
                                                                     output_batch_size, height, width);
            break;
    }
...
void launch_gpu_augmentation_images(const float* input, float* output, const float* parameters, const float* copy_paste_parameters,
                                    const float* donor_images, const std::int64_t* donor_masks, const float* donor_boxes,
...
void launch_gpu_augmentation_images_explicit(const float* input, float* output, const float* parameters, const float* copy_paste_parameters,
                                             const float* donor_images, const std::int64_t* donor_masks, const float* donor_boxes,
                                             const std::int64_t donor_mask_words, const std::uint64_t* image_keys,
...
void update_gpu_augmentation_donor_cache(const float* source_images, float* donor_images, const std::int64_t* source_ordinals,
                                         const std::int64_t batch_size, const std::int64_t pixels_per_image, cudaStream_t stream) {
    if (batch_size == 0) { return; }
