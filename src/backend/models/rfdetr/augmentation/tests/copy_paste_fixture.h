...
// ..####..
// ........
inline constexpr std::array ring_runs{mmltk::backend::data::RLEPair{10, 4}, mmltk::backend::data::RLEPair{17, 2}, mmltk::backend::data::RLEPair{21, 2}, mmltk::backend::data::RLEPair{25, 1},
 mmltk::backend::data::RLEPair{30, 1}, mmltk::backend::data::RLEPair{33, 1}, mmltk::backend::data::RLEPair{38, 1}, mmltk::backend::data::RLEPair{41, 2}, mmltk::backend::data::RLEPair{45, 2},
...
inline constexpr std::array<std::uint64_t, 7> identity_survivors{1ULL << 27, 3ULL << 19, 1ULL << 9, 0, 1, (1ULL << 24) | (15ULL << 26) | (1ULL << 31), 1ULL << 14};
...
[[nodiscard]] inline bool ring_paste_contains(const std::array<float, 6>& inverse, bool masked, int x, int y) {
 ...
 return dx >= 0 && dx <= 1 && dy >= 0 && dy <= 1 &&
        fixture_contains(ring_runs, std::clamp(static_cast<int>(std::nearbyint(dx * 8 - .5F)), 0, 7), std::clamp(static_cast<int>(std::nearbyint(dy * 8 - .5F)), 0, 7));
}
}  // namespace mmltk::backend::models::rfdetr::test_support
