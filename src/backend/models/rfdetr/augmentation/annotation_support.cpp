...
AugmentationAnnotationSupport mask_extent(std::span<const mmltk::backend::data::RLEPair> runs, int width, int height) {
    ...
    for (const auto& run : runs) {
        if (run.length == 0 || run.start < end ||
            static_cast<std::uint64_t>(run.start) + run.length > static_cast<std::uint64_t>(width) * height)
...
bool contains(const std::array<float, 4>& box, std::span<const mmltk::backend::data::RLEPair> runs, const std::array<float, 6>& inverse,
    ...
    const auto pixel = augment_math::support_pixel_index(sy, height) * width + augment_math::support_pixel_index(sx, width);
    return augment_math::rle_support_contains(runs.data(), runs.size(), pixel);
}
}  // namespace

bool augmentation_changes_support(const AugmentationImagePlan* plan) noexcept {
    return plan != nullptr && (plan->forward != identity || plan->paste_donor_slot >= 0 || plan->erasure.dropout_probability > 0 ||
                               plan->erasure.rectangular != 0);
}
...
AugmentationAnnotationSupport resolve_augmentation_annotation_support(const std::array<float, 4>& source_box,
                                                                      ...
                                                                      int height, const AugmentationImagePlan* plan, bool donor) {
    ...
    const auto original = source_mask.empty()
                              ? AugmentationAnnotationSupport{transform_augmentation_box_xyxy(source_box, identity),
    ...
    auto candidate = donor ? plan->paste_output_box
                           : transform_augmentation_box_xyxy(source_mask.empty() ? source_box : original.box_xyxy, plan->forward);
    ...
    if (has_paste && plan->paste_masked && (plan->paste_support == nullptr || plan->paste_support_count == 0))
        throw std::invalid_argument("masked augmentation paste requires original host donor support");
    const std::span<const mmltk::backend::data::RLEPair> donor_mask = has_paste && plan->paste_masked
                                                                          ? std::span{plan->paste_support, plan->paste_support_count}
    ...
    for (int y = y0; y < y1; ++y)
        for (int x = x0; x < x1; ++x) {
            ...
            if (augment_math::erases_pixel(plan->erasure, x, y, width, height) ||
                !contains(source_box, source_mask, inverse, nx, ny, width, height) ||
    ...
    if (result.present)
        result.box_xyxy = {static_cast<float>(min_x) / image_width, static_cast<float>(min_y) / image_height,
                           static_cast<float>(max_x + 1) / image_width, static_cast<float>(max_y + 1) / image_height};
    return result;
}
}  // namespace mmltk::backend::models::rfdetr
