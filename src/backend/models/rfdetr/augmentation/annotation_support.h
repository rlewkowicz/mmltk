...
[[nodiscard]] AugmentationAnnotationSupport resolve_augmentation_annotation_support(
    const std::array<float, 4>& source_box, std::span<const mmltk::backend::data::RLEPair> source_mask, int width, int height,
    const AugmentationImagePlan* plan, bool donor = false);
[[nodiscard]] inline std::array<float, 4> transform_augmentation_box_xyxy(
    const std::array<float, 4>& box, const std::array<float, kAugmentationTransformSize>& transform) noexcept {
