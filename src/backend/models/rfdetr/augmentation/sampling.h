...
[[nodiscard]] __host__ __device__ __forceinline__ std::uint64_t training_augmentation_image_key(
 const std::uint64_t seed, const int epoch, const int rank, const std::uint64_t sequence, const std::size_t image) noexcept {
...
[[nodiscard]] inline std::uint64_t augmentation_preview_image_key(const std::uint64_t dataset_identity, const std::uint64_t preview_seed, const std::uint32_t compiled_index) noexcept {
