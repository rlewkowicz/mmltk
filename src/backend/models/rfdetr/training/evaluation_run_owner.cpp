...
void TrainingEvaluationRunOwner::load_dataset(mmltk::backend::data::DatasetLoader& loader) {
 if (!impl_->lane_futures_.empty() || !impl_->encoding_queue_.empty()) { throw std::logic_error("evaluation dataset cannot change while work is in flight"); }
  ...
  if (calculate_loss) {
   ...
   if (match_free_loss) {
    ...
    TensorMap loss_dict;
    {
     ...
     runtime.matcher_workspace().complete_assignments(torch_cuda::getCurrentCUDAStream(torch_cuda::checked_device_index(options.device_id)).stream());
    }
