...
ResumeState load_resume_checkpoint_state(const std::filesystem::path&, DecodedNativeModelState&, const detail::TrainingContinuation&, NativeOptimizer&, const TrainRequest&, const std::vector<std::string>&, const std::vector<torch::Tensor>&, bool);
void save_collected_checkpoint(const std::filesystem::path&, const NativeCheckpointMetadata&, const NativeRfDetrModel&, const std::unordered_map<std::string, torch::Tensor>*, const char*, const char*, const std::filesystem::path&);
...
class TrainingSnapshot final {
 public:
    ...
    void save_resume(const std::filesystem::path&, const NativeCheckpointMetadata&, const NativeOptimizer&, const GradScaler&, const TrainRequest&, int epoch, double best_regular, double best_ema, int64_t ema_completed_updates, std::string_view attempt_id, const std::filesystem::path& descriptor);
    void release();
 private:
