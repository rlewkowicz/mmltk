...
inline constexpr auto members = std::define_static_array(
    std::meta::nonstatic_data_members_of(^^TrainingScalars, std::meta::access_context::current()));
inline constexpr std::size_t size = members.size();
using Tensors = std::array<mmltk::backend::ml::torch_api::Tensor, size>;
template <std::meta::info Member> consteval std::size_t index() {
    for (std::size_t i = 0; i < size; ++i) if (members[i] == Member) return i;
    throw "unknown training scalar";
}
template <std::meta::info Member> void set(Tensors& values, const mmltk::backend::ml::torch_api::Tensor& tensor) {
