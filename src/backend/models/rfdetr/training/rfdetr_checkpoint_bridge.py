...
def _extract_checkpoint_metadata(checkpoint) -> dict:
    ...
    for name, coercer in field_specs.items():
        raw_value = _checkpoint_arg_value(args_obj, name)
        value = coercer(raw_value)
        if name in {"num_queries", "num_select"} and raw_value is not None and (value is None or value <= 0):
            raise RuntimeError(f"RF-DETR checkpoint {name} must be a positive integer")
        if value is not None:
            metadata[name] = value
    if (
        "num_queries" in metadata
        and "num_select" in metadata
        and metadata["num_select"] > metadata["num_queries"]
    ):
        raise RuntimeError("RF-DETR checkpoint num_select must not exceed num_queries")
    return metadata
