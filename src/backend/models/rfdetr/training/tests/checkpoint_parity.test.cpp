...
void assert_outputs_bitwise_equal(const ModelOutputs& actual, const ModelOutputs& expected, const char* preset_name) {
    ...
    if (actual.enc_outputs.has_value()) {
        assert_output_layer_bitwise_equal(*actual.enc_outputs, *expected.enc_outputs, std::string(preset_name) + ".enc");
    }
}
...
void run_checkpoint_parity_case(const ParityFixtureCase& fixture, size_t index, size_t total) {
    ...
    const auto upstream_outputs =
        model_detail::native_model_owner(upstream_model).forward(nested_tensor_from_tensor_list({image.clone()}), true);
    const auto native_outputs =
        model_detail::native_model_owner(native_model).forward(nested_tensor_from_tensor_list({image.clone()}), true);
