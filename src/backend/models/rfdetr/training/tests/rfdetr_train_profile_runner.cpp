...
Options parse_options(int argc, char** argv) {
    ...
    if (options.width <= 0 || options.height <= 0 || options.num_images <= 0 || options.batch_size <= 0 || options.epochs <= 0 ||
        options.device_id < 0 || options.workers < 0 || options.lanes < 0 || options.prefetch_factor <= 0 || options.repetitions <= 0 ||
    ...
    if (options.num_images != kProfileFixtureImages) {
        throw std::runtime_error("RF-DETR matcher-route profile requires the complete 24-image fixture");
    }
...
void reset_cuda_peak(const Options& options) {
    c10::cuda::CUDACachingAllocator::resetPeakStats(static_cast<c10::DeviceIndex>(options.device_id));
}
...
[[nodiscard]] std::uint64_t metric_field(const std::string_view line, const std::string_view field) {
    ...
    if (error != std::errc{} || end != value.data() + value.size()) {
        throw std::runtime_error("profile metric contains malformed " + key + " field");
    }
    return parsed;
}
...
void verify_profile_route(const std::string& log_path, const std::string& run_label, const Options& options) {
    ...
    while (std::getline(input, line)) {
        ...
        if (view.starts_with("=== mmltk profile ")) {
            current = nullptr;
            if (view.starts_with("=== mmltk profile aggregate ") || view.find(" run=" + run_label + " ") == std::string_view::npos) {
                continue;
            }
    ...
    if (measured_blocks != repetitions.size()) {
        throw std::runtime_error("profile log does not contain exactly the requested measured blocks");
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (profile_log == nullptr || profile_log[0] == '\0') {
            throw std::runtime_error("RF-DETR train profile runner requires nonempty MMLTK_PROFILE_LOG");
        }
        ...
        run_profile_phases(
            ...
            [&options](const char* phase, const int index, const int total, const TrainRun& run) {
