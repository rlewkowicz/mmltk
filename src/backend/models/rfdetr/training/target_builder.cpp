...
TargetStagingSlot& TargetScratch::acquire_staging_slot(const std::size_t batch_size, const int64_t instances, const bool include_masks, const int height,
 ...
 for (std::size_t offset = 0; offset < staging_slots_.size(); ++offset) {
  ...
  const bool replace_masks =
   include_masks && (slot.mask_height != height || slot.mask_width != width || !has_matrix_capacity(slot.packed_masks, required_instances, mask_words));
...
void TargetScratch::retire_consumer_on_stream(const std::uintptr_t stream) {
 ...
 const auto torch_stream =
  consumer_stream == nullptr ? torch_cuda::current_torch_cuda_stream_object(device_index) : torch_cuda::getStreamFromExternal(consumer_stream, device_index);
...
PreparedTargets build_targets(const mmltk::backend::data::Batch& batch, int image_height, int image_width, bool include_masks, bool require_masks,
 ...
 const std::array<float, 6> identity{1.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F};
 {
  ...
  for (size_t image_pos = 0; image_pos < batch.num_images; ++image_pos) {
   ...
   const auto donor_support = paste_plan != nullptr ? resolve_augmentation_annotation_support(
                                                       paste_plan->paste_source_box, std::span{paste_plan->paste_support, paste_plan->paste_support_count},
 ...
 torch::Tensor image_ids_gpu;
 {
  ...
  if (total_instances > 0) {
   ...
   if (include_masks) {
    const bool transformed = augmentation_plan != nullptr &&
                             (augmentation_plan->transforms_geometry || augmentation_plan->copy_paste_enabled || augmentation_plan->erases_spatial_support);
