...
void TargetScratch::ensure_batch(size_t batch_size, int height, int width, int target_device_id) {
    ...
    const auto metadata_tensor_matches = [&](const torch_types::Tensor& tensor) {
        return tensor.defined() && tensor.device() == target_device && tensor.scalar_type() == torch_types::kInt64 && tensor.dim() == 1 &&
               tensor.size(0) >= capacity;
    };
    if (!metadata_tensor_matches(offsets_gpu) || !metadata_tensor_matches(counts_gpu)) {
        const auto device_int64 = torch_types::TensorOptions().dtype(torch_types::kInt64).device(target_device);
        auto replacement_offsets = torch_types::empty({capacity}, device_int64);
        auto replacement_counts = torch_types::empty({capacity}, device_int64);
        static_assert(std::is_nothrow_move_assignable_v<torch_types::Tensor>);
...
TargetStagingSlot& TargetScratch::acquire_staging_slot(const std::size_t batch_size, const int64_t instances, const bool include_masks,
    ...
    for (std::size_t offset = 0; offset < staging_slots_.size(); ++offset) {
        ...
        const auto has_vector_capacity = [](const torch_types::Tensor& tensor, const int64_t capacity) {
            return tensor.defined() && tensor.dim() == 1 && tensor.size(0) >= capacity;
        };
        const auto has_matrix_capacity = [](const torch_types::Tensor& tensor, const int64_t capacity, const int64_t columns) {
        ...
        if (replace_batch || replace_instances || replace_masks || create_completion_event) {
            ...
            if (replace_batch) {
                ...
                replacement.counts = allocate_staging({batch_capacity}, torch_types::kInt64);
                replacement.batch_capacity = batch_capacity;
            }
            if (replace_instances) {
                replacement.boxes = allocate_staging({slot_instance_capacity, 4}, torch_types::kFloat32);
                replacement.labels = allocate_staging({slot_instance_capacity}, torch_types::kInt64);
                replacement.area = allocate_staging({slot_instance_capacity}, torch_types::kFloat32);
                replacement.iscrowd = allocate_staging({slot_instance_capacity}, torch_types::kInt64);
                replacement.inverse_transforms = allocate_staging({slot_instance_capacity, 6}, torch_types::kFloat32);
            ...
            if (replace_masks) {
                replacement.packed_masks = allocate_staging({required_instances, mask_words}, torch_types::kInt64).zero_();
...
PreparedTargets build_targets(const mmltk::backend::data::Batch& batch, int image_height, int image_width, bool include_masks,
    ...
    const auto gpu_float = torch_types::TensorOptions().dtype(torch_types::kFloat32).device(device);
    const auto gpu_int64 = torch_types::TensorOptions().dtype(torch_types::kInt64).device(device);
    ...
    torch_types::Tensor packed_masks_cpu;
    ...
    torch_types::Tensor image_ids_gpu;
    {
        ...
        if (total_instances > 0) {
            boxes_gpu = staging.boxes.narrow(0, 0, total_instances).to(device, torch_types::kFloat32, true, false);
            labels_gpu = staging.labels.narrow(0, 0, total_instances).to(device, torch_types::kInt64, true, false);
            ...
            if (include_masks) {
                ...
                packed_masks_gpu = PackedTargetMasks{
                    ...
                    image_height,
                    image_width,
                    transformed ? staging.inverse_transforms.narrow(0, 0, total_instances).to(device, torch_types::kFloat32, true, false)
                                : torch_types::Tensor{},
