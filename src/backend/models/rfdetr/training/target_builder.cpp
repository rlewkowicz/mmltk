...
void BatchStaticTensors::ensure(int64_t batch_size, int height, int width, int target_device_id) {
 ...
 const bool sizes_match = sizes.defined() && sizes.device() == requested_device && sizes.dim() == 2 && sizes.size(0) >= requested_capacity && sizes.size(1) == 2;
 const bool mask_matches = nested_mask.defined() && nested_mask.device() == requested_device && nested_mask.dim() == 3 && nested_mask.size(0) >= requested_capacity && nested_mask.size(1) == height &&
                           nested_mask.size(2) == width;
...
void TargetScratch::ensure_instance_capacity(int64_t instances) {
 ...
 const bool tensor_matches = target_indices_gpu.defined() && target_indices_gpu.device() == target_device && target_indices_gpu.dim() == 1 && target_indices_gpu.size(0) >= requested_capacity;
...
TargetStagingSlot& TargetScratch::acquire_staging_slot(const std::size_t batch_size, const int64_t instances, const bool include_masks, const int height, const int width) {
...
void TargetScratch::record_staging_copy_on_stream(const std::uintptr_t stream) {
 ...
 ensure_cuda_ok(cudaEventRecord(reinterpret_cast<cudaEvent_t>(slot.copy_complete_event), reinterpret_cast<cudaStream_t>(stream)), "cudaEventRecord for target staging copy");
...
void TargetScratch::ensure_copy_resources(int target_device_id) {
 ...
 const torch_cuda::TorchCudaStream copy_stream = torch_cuda::get_priority_cuda_stream(device_index, mmltk::frameworks::gpu::current_cuda_highest_stream_priority());
...
void TargetScratch::wait_for_pending_copy_on_stream(const std::uintptr_t stream) const {
 ...
 torch_cuda::TorchCudaDeviceGuard device_guard(mmltk::backend::ml::cuda::checked_device_index(copy_stream_device_id_));
 ensure_cuda_ok(cudaStreamWaitEvent(reinterpret_cast<cudaStream_t>(stream), reinterpret_cast<cudaEvent_t>(copy_complete_event_), 0), "cudaStreamWaitEvent for target copy");
}
void TargetScratch::record_pending_copy_on_stream(const std::uintptr_t stream) {
 ...
 torch_cuda::TorchCudaDeviceGuard device_guard(mmltk::backend::ml::cuda::checked_device_index(copy_stream_device_id_));
 ensure_cuda_ok(cudaEventRecord(reinterpret_cast<cudaEvent_t>(copy_complete_event_), reinterpret_cast<cudaStream_t>(stream)), "cudaEventRecord for target copy");
 copy_pending_ = true;
}
...
void TargetScratch::retire_consumer_on_stream(const std::uintptr_t stream) {
 ...
 if (consumers_pending_) { ensure_cuda_ok(cudaStreamWaitEvent(consumer_stream, reinterpret_cast<cudaEvent_t>(consumers_retired_event_), 0), "cudaStreamWaitEvent for prior target consumer"); }
 const auto device_index = torch_cuda::checked_device_index(copy_stream_device_id_);
 const auto torch_stream = consumer_stream == nullptr ? torch_cuda::current_torch_cuda_stream_object(device_index) : torch_cuda::getStreamFromExternal(consumer_stream, device_index);
...
void validate_feature_active_target(const int64_t label, const std::span<const float, 4> normalized_cxcywh, const int64_t object_classes) {
 if (object_classes < 1 || label < 0 || label >= object_classes) { throw std::runtime_error("feature-active RF-DETR target label is outside the object-class catalog"); }
 const bool finite_and_normalized = std::isfinite(normalized_cxcywh[0]) && normalized_cxcywh[0] >= 0.0F && normalized_cxcywh[0] <= 1.0F && std::isfinite(normalized_cxcywh[1]) &&
                                    normalized_cxcywh[1] >= 0.0F && normalized_cxcywh[1] <= 1.0F && std::isfinite(normalized_cxcywh[2]) && normalized_cxcywh[2] > 0.0F &&
                                    normalized_cxcywh[2] <= 1.0F && std::isfinite(normalized_cxcywh[3]) && normalized_cxcywh[3] > 0.0F && normalized_cxcywh[3] <= 1.0F;
...
PreparedTargets build_targets(const mmltk::backend::data::Batch& batch, int image_height, int image_width, bool include_masks, bool require_masks, int device_id, TargetScratch& scratch,
 const std::string_view split, const int64_t resolved_query_count, const TrainingSupervisionConfig& supervision, const int64_t object_classes, AugmentationBatchPlan* augmentation_plan) {
 mmltk::common::logging::ScopedProfile profile_rfdetr_targets_total{"rfdetr.targets.total"};
 if (split.empty() || resolved_query_count <= 0) { throw std::invalid_argument("RF-DETR target preparation requires a split and positive resolved query count"); }
 if (augmentation_plan != nullptr && augmentation_plan->active_size != batch.num_images) { throw std::runtime_error("augmentation plan batch size does not match target batch"); }
 if (training_supervision_enabled(supervision) && object_classes < 1) { throw std::runtime_error("feature-active RF-DETR supervision requires at least one object class"); }
 ...
 if (include_masks && maximum_instances > 0) {
  ...
  std::memset(packed_masks_cpu.data_ptr<int64_t>(), 0, static_cast<size_t>(maximum_instances) * static_cast<size_t>(staging.mask_words_per_instance) * sizeof(int64_t));
 ...
 const std::array<float, 6> identity{1.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F};
 {
  ...
  for (size_t image_pos = 0; image_pos < batch.num_images; ++image_pos) {
   ...
   for (int64_t instance_index = 0; instance_index < static_cast<int64_t>(entry.num_instances); ++instance_index) {
    ...
    const auto runs = instance.mask_rle_pairs != 0 ? std::span{batch.rle_pairs + instance.mask_rle_offset / sizeof(mmltk::backend::data::RLEPair), static_cast<std::size_t>(instance.mask_rle_pairs)}
   ...
   const auto donor_support = paste_plan != nullptr ? resolve_augmentation_annotation_support(
                                                       paste_plan->paste_source_box, std::span{paste_plan->paste_support, paste_plan->paste_support_count}, image_width, image_height, paste_plan, true)
 ...
 if (training_supervision_enabled(supervision)) {
  for (int64_t index = 0; index < total_instances; ++index) { validate_feature_active_target(labels[index], std::span<const float, 4>(&boxes[index][0], 4), object_classes); }
 }
