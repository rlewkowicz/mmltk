...
class TrainingValidationRuntime {
   public:
    ...
    torch_api::Tensor preprocess(const mmltk::backend::data::Batch& batch) { return preprocessor_->run(batch, static_cast<int64_t>(batch_size_)); }
...
class SupervisionTimingLease final {
   public:
    ...
    SupervisionTimingLease(detail::NativeModelTechnicalOwner& owner, const bool active, const Kind kind) : owner_(active ? &owner : nullptr), kind_(kind) {
...
int64_t prepared_target_count(const PreparedTargets& targets) { return std::accumulate(targets.counts.begin(), targets.counts.end(), int64_t{0}); }
   ...
   public:
    explicit TrainingMetricHandoff(int device_id)
        : device_id_(device_id),
          event_pool_(mmltk::frameworks::gpu::make_cuda_device_owner<TrainingMetricHandoff, &TrainingMetricHandoff::record_failure>(this, device_id), 1U,
                      retirement_owner_) {
...
std::optional<mmltk::backend::ml::cuda::CudaEventPool::Lease> record_current_stream_event(mmltk::backend::ml::cuda::CudaEventPool& event_pool,
                                                                                          const int device_id, const char* context) {
    return event_pool.record(
        reinterpret_cast<std::uintptr_t>(torch_cuda::current_torch_cuda_stream_object(torch_cuda::checked_device_index(device_id)).stream()), context);
}
void wait_for_lane_result(TrainLaneResult& lane_result, int device_id) {
    ...
    lane_result.ready_event->wait(
        reinterpret_cast<std::uintptr_t>(torch_cuda::current_torch_cuda_stream_object(torch_cuda::checked_device_index(device_id)).stream()),
...
scalar_packet::Tensors ordinary_scalar_tensors(const TensorMap& losses, const torch_api::Tensor& total, const torch_api::Tensor& auxiliary) {
...
std::vector<NormalizedModelStateEntry> collect_module_state(const NativeRfDetrModel& model,
                                                            const std::unordered_map<std::string, torch_api::Tensor>* parameter_overrides = nullptr) {
...
ModelStateLoadSummary load_training_model_weights(NativeRfDetrModel& model, const DecodedNativeModelState& checkpoint, const TrainingSupervisionRoute route) {
    ...
    auto candidate = detail::native_model_owner(model).stage_normalized_state(
        detail::model_state_owner(checkpoint).entries,
...
ResumeState load_resume_checkpoint_state(const std::filesystem::path& checkpoint_path, DecodedNativeModelState& admitted,
                                         ...
                                         const std::vector<std::string>& parameter_names, const std::vector<torch_api::Tensor>& parameters,
...
std::future<TrainLaneResult> enqueue_train_lane(mmltk::common::concurrency::WorkerPool& lane_pool, RuntimeContext* runtime,
                                                mmltk::backend::data::DatasetLoader& loader, TrainLaneContext& lane, const mmltk::backend::data::Batch& batch,
                                                ...
                                                torch_api::ScalarType autocast_dtype, TrainingSupervisionRoute route,
    ...
    return lane_pool.enqueue([runtime, &loader, &lane, batch, params_ready, &event_pool, scaled_loss_factor, parameter_version, &detection_config, &model,
                              device_id, image_height, image_width, seed, epoch, rank, augmentation_sequence, amp_enabled, autocast_dtype, route,
                              wave_normalizer = std::move(wave_normalizer), lane_index]() mutable {
        try {
            ...
            PreparedTargets prepared;
            {
                ...
                prepared = build_targets(batch, image_height, image_width, detection_config.include_masks, detection_config.include_masks, device_id,
                                         lane.target_scratch, "train", model.config().num_queries, model.config().training_supervision,
            ...
            std::vector<torch_api::Tensor> gradients;
            {
                ...
                if (route_is_active(route) || wave_normalizer) {
                    ...
                    const double group_divisor = detection_config.sum_group_losses ? 1.0 : static_cast<double>(detection_config.group_detr);
                    const double num_boxes_value = std::max(
                        static_cast<double>(target_count) * group_divisor / static_cast<double>(std::max<int64_t>(1, detection_config.world_size)), 1.0);
...
EvalPassResult evaluate_model(const TrainRequest& options, TrainingValidationRuntime& validation, NativeRfDetrModel& model, TrainingEventOwner& event_owner,
    ...
    while (loader.next_batch(batch)) {
        ...
        ModelOutputs outputs;
        {
            ...
            {
                mmltk::backend::ml::cuda::TorchAutocastScope autocast_guard(amp_enabled, autocast_dtype);
                outputs = match_free_loss ? detail::native_model_owner(model).forward_for_match_free(NestedTensor{inference_input, validation.nested_mask()})
        ...
        if (calculate_loss) {
            ...
            if (match_free_loss) {
                const auto target_count = torch_api::tensor({static_cast<float>(prepared_target_count(*prepared))},
        ...
        evaluation_run.submit(
            ...
            [processed = std::move(processed), processed_ready = std::move(*processed_ready), metadata = std::move(prediction_metadata),
             category_count = loader.num_classes(), max_dets = validation.detection_limit().as_size, device_id = options.device_id](auto& lane) mutable {
                ...
                torch_cuda::TorchCudaStreamGuard stream_guard(lane.stream);
                return stage_prediction_batch(std::move(metadata), std::move(processed), category_count, max_dets, std::move(lease), device_id,
...
TrainRunResult TrainingRuntimeOwner::Impl::run() {
    ...
    detail::native_model_owner(model).configure_supervision_timing(SupervisionTimingSetup{
        cuda_device(options.device_id), static_cast<std::size_t>(std::max(1, options.grad_accum_steps)), mmltk::common::logging::profile_enabled()});
    ...
    if (!options.resume_path.empty()) {
        resume_model_candidate = detail::native_model_owner(model).stage_normalized_state(detail::model_state_owner(admitted.model_state).entries,
    ...
    if (!options.resume_path.empty()) {
        ...
        if (resume_model_candidate.has_value()) { detail::native_model_owner(model).commit_normalized_state(std::move(*resume_model_candidate)); }
    ...
    if (train_lane_count > 1) {
        train_lane_pool = std::make_unique<mmltk::common::concurrency::WorkerPool>(static_cast<size_t>(train_lane_count), train_runtime.lane_cpus(),
                                                                                   "rfdtrtlane", 0U, &train_runtime.execution().placement, false);
        for (int lane_index = 0; lane_index < train_lane_count; ++lane_index) {
            train_lanes.emplace_back(torch_cuda::get_priority_cuda_stream(options.device_id, mmltk::frameworks::gpu::current_cuda_highest_stream_priority()),
    ...
    for (int epoch = start_epoch; epoch < options.epochs; ++epoch) {
        ...
        if (train_lane_count <= 1) {
            while (true) {
                ...
                if (route_is_active(training_route)) {
                    auto target_count = torch_api::tensor({static_cast<float>(prepared_target_count(prepared))},
                ...
                if (route_is_active(training_route)) {
                    ...
                    loss_dict = detection_loss_dict(
                        ...
                        distributed.enabled ? AllReduceTensorFn([&distributed](torch_api::Tensor& value) { distributed_all_reduce_tensor(distributed, value); })
                                            : AllReduceTensorFn{});
                }
                if (!route_is_active(training_route)) {
            ...
            while (local_full_batches < usable_full_batches) {
                ...
                for (int lane_index = 0; lane_index < train_lane_count; ++lane_index) {
                    ...
                    wave.add(enqueue_train_lane(*train_lane_pool, &train_runtime, train_loader, lane, *batch, params_ready ? &*params_ready : nullptr,
                ...
                if (local_waves % options.grad_accum_steps == 0) {
                    ...
                    for (auto& lane : train_lanes) { static_cast<void>(detail::native_model_owner(*lane.model).harvest_supervision_timing()); }
                }
                flush_progress(false);
            }
        }
        ...
        if (train_lane_count <= 1) {
            ...
            for (auto& lane : train_lanes) { lane.target_scratch.wait_for_pending_copy(); }
        }
        flush_progress(true);
        if (progress) { progress->close(); }
        ...
        if (main_process) {
            ...
            save_snapshot_checkpoint(options.output_dir / std::format("checkpoint_epoch_{}.pt", epoch + 1), metadata, ordinary_snapshot, {},
                                     checkpoint_readback, options.class_layout_path);
            // Preparing later artifacts must not undo a successfully published epoch.
            const auto ema_snapshot = ema ? ema_state_entries(all_param_names, *ema) : std::vector<NormalizedModelStateEntry>{};
            ...
            if (selected_metric > best_metric) {
                save_snapshot_checkpoint(selected_path, metadata, ordinary_snapshot, ema_snapshot, checkpoint_readback, options.class_layout_path);
