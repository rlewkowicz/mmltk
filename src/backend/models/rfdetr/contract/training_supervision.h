...
[[nodiscard]] constexpr std::string_view cli_enum_spelling(const TrainAssignmentKind assignment) noexcept {
    switch (assignment) {
        case TrainAssignmentKind::Hungarian:
            return "hungarian";
        case TrainAssignmentKind::MatchFree:
            return "match-free";
    }
    return {};
}
...
struct MatchFreeSupervisionConfig {
    [[= mmltk::frameworks::reflection::Minimum<float>{kSupervisionOpenUnitMinimum}]][[= mmltk::frameworks::reflection::Maximum<float>{
        kSupervisionOpenUnitMaximum}]][[= mmltk::frameworks::reflection::Finite{}]] float rho = 0.5F;
    [[= mmltk::frameworks::reflection::Minimum<float>{0.0F}]][[= mmltk::frameworks::reflection::Finite{}]] float correspondence_weight =
        1.0F;
...
struct DenoisingSupervisionConfig {
    ...
    [[= mmltk::frameworks::reflection::Minimum<float>{0.0F}]][[= mmltk::frameworks::reflection::Maximum<float>{1.0F}]]
                                                             [[= mmltk::frameworks::reflection::Finite{}]] float label_noise_ratio = 0.2F;
    [[= mmltk::frameworks::reflection::Minimum<float>{kSupervisionOpenUnitMinimum}]][[= mmltk::frameworks::reflection::Maximum<float>{
        kSupervisionOpenUnitMaximum}]][[= mmltk::frameworks::reflection::Finite{}]] float center_noise_scale = 0.4F;
    [[= mmltk::frameworks::reflection::Minimum<float>{kSupervisionOpenUnitMinimum}]][[= mmltk::frameworks::reflection::Maximum<float>{
        kSupervisionOpenUnitMaximum}]][[= mmltk::frameworks::reflection::Finite{}]] float size_noise_scale = 0.4F;
...
[[nodiscard]] constexpr bool training_supervision_config_valid(const TrainingSupervisionConfig& config) noexcept {
    const bool selected_objective = config.assignment != TrainAssignmentKind::MatchFree || config.match_free.correspondence_weight > 0.0F ||
                                    config.match_free.query_weight > 0.0F;
    return !mmltk::frameworks::reflection::validate_reflected_fields(config).has_value() &&
           mmltk::frameworks::reflection::enum_contains(config.assignment) && selected_objective;
}
...
[[nodiscard]] constexpr bool training_supervision_query_layout_valid(const TrainingSupervisionConfig& config,
                                                                     const std::size_t ordinary_queries, const std::size_t ordinary_groups,
