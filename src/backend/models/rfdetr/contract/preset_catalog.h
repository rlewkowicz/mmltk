...
inline constexpr std::array<PresetCatalogEntry, 10U> kPresetCatalog{{
 {"rf-detr-nano", "RF-DETR Nano", "N", ModelTask::Detection, 384U, "dinov2_windowed_small", "rf-detr-nano.pth", "https://storage.googleapis.com/rfdetr/nano_coco/checkpoint_best_regular.pth",
  "fb6504cce7fbdc783f7a46991f07639f", 16U, 2U, 24U, 2U, 300U, 300U, 91U, 256U, 13U, true, 1.0, 5.0, 2.0, 1.0, 1.0},
 {"rf-detr-small", "RF-DETR Small", "S", ModelTask::Detection, 512U, "dinov2_windowed_small", "rf-detr-small.pth", "https://storage.googleapis.com/rfdetr/small_coco/checkpoint_best_regular.pth",
  "fb37061c1af7bace359c91b723a8d5c1", 16U, 2U, 32U, 3U, 300U, 300U, 91U, 256U, 13U, true, 1.0, 5.0, 2.0, 1.0, 1.0},
 {"rf-detr-medium", "RF-DETR Medium", "M", ModelTask::Detection, 576U, "dinov2_windowed_small", "rf-detr-medium.pth", "https://storage.googleapis.com/rfdetr/medium_coco/checkpoint_best_regular.pth",
  "7223f764a87b863f02eb8d52bf0ce2ee", 16U, 2U, 36U, 4U, 300U, 300U, 91U, 256U, 13U, true, 1.0, 5.0, 2.0, 1.0, 1.0},
 {"rf-detr-large", "RF-DETR Large", "L", ModelTask::Detection, 704U, "dinov2_windowed_small", "rf-detr-large-2026.pth", "https://storage.googleapis.com/rfdetr/rf-detr-large-2026.pth",
  "5cb72153541cbcb9aa6efa26222acc75", 16U, 2U, 44U,
...
[[nodiscard]] consteval bool preset_catalog_is_valid() {
 for (std::size_t index = 0U; index < kPresetCatalog.size(); ++index) {
  ...
  if (preset.preset_name.empty() || preset.display_name.empty() || preset.size_label.empty() || preset.resolution == 0U || !model_task_name(preset.task).has_value() ||
      preset.resolution > static_cast<std::uint32_t>(std::numeric_limits<int>::max()) || preset.encoder.empty() || preset.canonical_weight_filename.empty() || preset.canonical_weight_url.empty() ||
      preset.canonical_weight_md5.size() != 32U || preset.patch_size == 0U || preset.window_count == 0U || preset.positional_encoding_size == 0U || preset.decoder_layer_count == 0U ||
      preset.query_count == 0U || preset.selected_query_count == 0U || preset.class_count == 0U || preset.hidden_dimension == 0U || preset.group_count == 0U) {
   return false;
  }
  for (std::size_t sibling = index + 1U; sibling < kPresetCatalog.size(); ++sibling) {
   if (preset.preset_name == kPresetCatalog[sibling].preset_name || preset.canonical_weight_filename == kPresetCatalog[sibling].canonical_weight_filename) { return false; }
  }
 }
 return true;
}
