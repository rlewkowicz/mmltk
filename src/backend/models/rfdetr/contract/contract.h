...
[[nodiscard]] consteval bool reflected_contract_shape_is_valid() {
    constexpr auto members =
        std::define_static_array(std::meta::nonstatic_data_members_of(^^RfdetrContractContribution, std::meta::access_context::current()));
    return members.size() == 5U && !kRfdetrContract.model_id.empty() && !kRfdetrContract.display_name.empty() &&
           !kRfdetrContract.presets.empty();
}
