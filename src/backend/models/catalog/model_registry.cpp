...
[[nodiscard]] consteval bool catalog_is_valid() {
    ...
    for (std::size_t model_index = 0U; model_index < kModels.size(); ++model_index) {
        ...
        for (std::size_t preset_index = 0U; preset_index < model.presets.size(); ++preset_index) {
            ...
            if (preset.model_id != model.model_id || preset.preset_name.empty() || preset.display_name.empty() || preset.size_label.empty() ||
                preset.task.empty() || preset.canonical_weight_filename.empty() || preset.resolution == 0U) {
                return false;
            }
