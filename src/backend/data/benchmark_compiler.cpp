...
class ProgressReporter {
   public:
    ...
    void pixel_attempt(const std::uint64_t completed, const std::uint64_t total, const std::string_view split,
                       const std::uint64_t split_total) {
    ...
    void pixel_completed() {
        ...
        if (trace_ != nullptr && *trace_) {
            ...
            const double eta_seconds =
                images_per_second > 0.0 ? static_cast<double>(pixel_split_total_ - pixel_completed_) / images_per_second : 0.0;
    ...
    void source_bytes(const BenchmarkDatasetSource source, const std::uint64_t completed, const std::uint64_t total,
   ...
   private:
    ...
    [[nodiscard]] static std::string default_phase_activity(const DatasetCompilePhase phase) {
        switch (phase) {
            case DatasetCompilePhase::Idle:
                return "Compiling benchmark dataset";
            case DatasetCompilePhase::Planning:
                return "Planning benchmark dataset";
            case DatasetCompilePhase::Downloading:
                return "Acquiring benchmark metadata";
            case DatasetCompilePhase::Indexing:
                return "Indexing benchmark annotations";
            case DatasetCompilePhase::Extracting:
                return "Acquiring and extracting source images";
            case DatasetCompilePhase::Labels:
                return "Preparing compiled labels";
            case DatasetCompilePhase::Pixels:
                return "Compiling image pixels";
            case DatasetCompilePhase::Syncing:
                return "Validating and syncing compiled output";
            case DatasetCompilePhase::Publishing:
                return "Publishing benchmark dataset";
        }
        std::unreachable();
    }
...
void require_storage(const std::filesystem::path& path, const std::uint64_t required, const char* description,
    ...
    trace_benchmark_event(trace, "benchmark.storage.preflight", [&] {
        return nlohmann::json{
            {"target", description}, {"path", path.string()}, {"required_bytes", required}, {"available_bytes", available}};
    });
    if (available < required) {
        throw std::runtime_error(std::string("insufficient storage for ") + description + ": requires " + std::to_string(required) +
   ...
   public:
    ...
    [[nodiscard]] Reservation reserve(const std::uint64_t required, const std::string_view description) {
        ...
        if (reserved_ > available || required > available - reserved_) {
            throw std::runtime_error("insufficient storage for " + std::string(description) + ": requires " + std::to_string(required) +
                                     " bytes with " + std::to_string(reserved_) + " bytes already reserved, available " +
                                     std::to_string(available));
        }
        reserved_ += required;
        trace_benchmark_event(trace_, "benchmark.storage.reserved", [&] {
            return nlohmann::json{{"target", description},
                                  {"path", path_.string()},
                                  {"required_bytes", required},
                                  {"reserved_bytes", reserved_},
...
[[nodiscard]] DownloadRequest make_download_request(const BenchmarkCacheLayout& cache, const std::string_view source,
...
struct ArtifactProgressTotals {
    ...
    void update(const DownloadProgress& update, ProgressReporter* reporter) {
        ...
        reporter->source_bytes(source, source_completed, source_total,
                               common_math::checked_cast<std::uint32_t>(source_retries, "source retry count overflow"), source_cache_hit,
                               source_resumed);
    }
...
[[nodiscard]] std::string extract_archive_member(const std::filesystem::path& archive_path, const std::string_view member_suffix,
    ...
    if (std::filesystem::is_regular_file(output_path) && std::filesystem::is_regular_file(completion)) {
        try {
            ...
            if (metadata.value("schema_version", 0U) == kBenchmarkCacheSchemaVersion && metadata.value("complete", false) &&
                metadata.value("archive_identity", std::string{}) == archive_identity &&
                metadata.value("member", std::string{}) == member_suffix &&
    ...
    while (true) {
        ...
        for (std::uint32_t retry = 0U; status == ARCHIVE_RETRY && retry < 8U; ++retry) {
        ...
        if (!name.ends_with(member_suffix)) {
            ...
            for (std::uint32_t retry = 0U; skip_status == ARCHIVE_RETRY && retry < 8U; ++retry) {
        ...
        staging = common_io::FileHandle::create_unique_output(
            staging_text, common_math::checked_cast<std::size_t>(expected_size, "extracted annotation size overflow"));
        ...
        while (offset < expected_size) {
            ...
            const std::size_t remaining =
                common_math::checked_cast<std::size_t>(expected_size - offset, "extraction remaining size overflow");
...
[[nodiscard]] std::vector<std::uint64_t> image_ids(const NormalizedAnnotationIndex& index,
                                                   const std::optional<std::uint16_t> shard = std::nullopt) {
...
class JpegValidator {
   private:
    class MappedJpeg {
       public:
        explicit MappedJpeg(const std::filesystem::path& path)
            : file_(common_io::FileHandle::open_readonly(path.string())), size_(file_.size()) {
   ...
   public:
    [[nodiscard]] std::pair<std::uint32_t, std::uint32_t> read_header(const std::span<const std::uint8_t> encoded,
                                                                      const std::uint32_t expected_width = 0U,
    ...
    [[nodiscard]] std::pair<std::uint32_t, std::uint32_t> validate_file(const std::filesystem::path& path,
                                                                        const std::uint32_t expected_width = 0U,
                                                                        const std::uint32_t expected_height = 0U) {
    ...
    void validate_decodable(const std::span<const std::uint8_t> encoded, const std::uint32_t expected_width = 0U,
                            const std::uint32_t expected_height = 0U) {
        try {
            const BenchmarkJpegHeader header = decoder_.read_header(encoded, expected_width, expected_height);
            decoder_.decode_rgb(encoded, header, &decoded_, &cmyk_);
        } catch (const BenchmarkJpegError& error) {
            throw InvalidJpegError(std::string("cannot fully decode benchmark JPEG: ") + error.what());
        }
    }

    void validate_decodable_file(const std::filesystem::path& path, const std::uint32_t expected_width = 0U,
                                 const std::uint32_t expected_height = 0U) {
        const MappedJpeg mapped = map_file(path);
...
struct OpenImagesTransfer {
    ...
    OpenImagesTransfer(const std::uint64_t id, const std::uint32_t attempt_value,
                       mmltk::common::concurrency::CancellationObservation cancel, std::vector<std::uint8_t> reusable_buffer)
    ...
    static std::size_t write_callback(char* data, const std::size_t size, const std::size_t count, void* opaque) {
        return curl_run_data_callback<OpenImagesTransfer>(
            size, count, opaque, [data](OpenImagesTransfer& transfer, const std::size_t bytes) {
   ...
   public:
    ...
    OpenImageCacheWorkers(const std::size_t worker_count, std::filesystem::path image_root,
        ...
        for (std::size_t worker = 0U; worker < worker_count; ++worker) {
            validators.push_back(std::make_unique<JpegValidator>());
        }
...
void download_open_images(const std::span<const std::uint64_t> expected_ids, const std::filesystem::path& image_root,
                          ...
                          std::uint64_t* completed_images, const std::uint64_t total_images, std::uint64_t* cached_image_bytes,
                          const std::size_t transfer_concurrency, const std::size_t cache_workers, const BenchmarkTraceSink& trace) {
    ensure_curl_global();
    ...
    for (const std::uint64_t id : expected_ids) {
        pending.push_back(Pending{id, 1U, Clock::now()});
    }
    ...
    for (std::vector<std::uint8_t>& buffer : reusable_buffers) {
        buffer.reserve(kEstimatedJpegBytes);
    }
    ...
    const auto retry_or_quarantine = [&](const std::uint64_t image_id, const std::uint32_t attempt, std::string reason,
                                         const bool throttled = false) {
        ...
        progress->source_activity(BenchmarkDatasetSource::kOpenImagesV7,
                                  "Quarantined unavailable Open Images training image " + std::to_string(image_id));
    };
        ...
        const auto drain_cache_results = [&] {
            ...
            while (cache_writer.try_pop(&result)) {
                ...
                if (!result.retry_reason.empty()) {
                    ...
                    *cached_image_bytes =
                        common_math::checked_add(*cached_image_bytes, result.encoded.size(), "Open Images cached byte total overflow");
                    ++*completed_images;
                    report_image_progress();
                }
        ...
        while (!pending.empty() || !active.empty() || cache_writer.outstanding() != 0U) {
            ...
            while (std::optional completion = active.next_completed()) {
                ...
                if (active_limit < transfer_concurrency && successes_since_throttle >= 1024U) {
                    ...
                    successes_since_throttle = 0U;
                    progress->source_activity(BenchmarkDatasetSource::kOpenImagesV7,
                                              "Open Images concurrency recovered to " + std::to_string(active_limit));
                }
...
[[nodiscard]] CachedImageDirectory acquire_archive_images(
    const BenchmarkCacheLayout& cache, const BenchmarkDatasetSource source, std::string shard,
    ...
    std::atomic<std::uint64_t>* source_extracted_images, const std::uint64_t source_total_images, const std::size_t decompression_workers,
    const std::size_t cache_write_workers, const std::size_t download_connections, const BenchmarkTraceSink& trace,
    ...
    const std::string source_identity =
        source_name + ":" + shard + ":" + artifact.artifact_id + ":" + std::string(kBenchmarkCatalogRevision);
    ...
    if (validate_cached_image_group(image_root, image_root / ".complete.json", source_identity, expected_ids, &cached_image_bytes,
                                    cancel_requested, trace, quarantine_unavailable ? &cached_quarantine : nullptr)) {
        ...
        for (const std::uint64_t image_id : expected_ids) {
            if (!std::ranges::binary_search(cached_quarantine, image_id, {}, &CachedImageRejection::image_id)) {
                available_ids.push_back(image_id);
            }
        }
        const std::uint64_t source_completed =
            source_extracted_images->fetch_add(expected_ids.size(), std::memory_order_relaxed) + expected_ids.size();
    ...
    StorageReservationPool::Reservation storage_reservation =
        storage_reservations->reserve(acquisition_storage, source_name + " archive and extracted images");
    ...
    for (std::uint32_t attempt = 1U; attempt <= 3U; ++attempt) {
        ...
        try {
            progress->source_activity(source, "Downloading " + archive_name + " image archive with " +
                                                  std::to_string(download_connections) + " download connections");
            const std::vector<DownloadResult> downloads = download_artifacts(
                {request}, download_connections, cancel_requested,
                [&](const DownloadProgress& update) { transfer_progress->update(update, progress); }, trace);
            ...
            CachedImageDirectory extracted = extract_selected_archive_images(ArchiveExtractionRequest{
                ...
                .activity =
                    [&](const std::string_view activity) {
                        progress->source_activity(source, archive_name + ": " + std::string(activity));
                    },
            });
            ...
            if (attempt_reported != 0U) {
                const std::uint64_t source_completed =
                    source_extracted_images->fetch_sub(attempt_reported, std::memory_order_relaxed) - attempt_reported;
                progress->source_images(source, source_completed, source_total_images);
            }
            throw_if_benchmark_cancelled(cancel_requested);
...
[[nodiscard]] std::string open_images_group_name(const std::size_t group_index) {
    ...
    if (length <= 0 || static_cast<std::size_t>(length) >= shard_buffer.size()) {
        throw std::runtime_error("cannot format Open Images image group");
    }
    return std::string(shard_buffer.data(), static_cast<std::size_t>(length));
}

[[nodiscard]] CachedOpenImagesGroup discover_cached_open_images_group(const std::filesystem::path& image_root,
                                                                      const std::filesystem::path& completion,
                                                                      const std::string_view identity,
                                                                      const std::span<const std::uint64_t> requested_image_ids,
                                                                      mmltk::common::concurrency::CancellationObservation cancel_requested,
                                                                      const BenchmarkTraceSink& trace) {
    ...
    try {
        ...
        if (const auto iterator = manifest.find("quarantined"); iterator != manifest.end()) {
            ...
            const bool parsed =
                parse_quarantined_manifest_records(*iterator, requested_image_ids, [&](const std::uint64_t image_id, std::string reason) {
            ...
            if (std::ranges::adjacent_find(result.quarantined, {}, &QuarantinedImage::image_id) != result.quarantined.end()) {
                return CachedOpenImagesGroup{};
            }
        }
        if (result.quarantined.size() > requested_image_ids.size() ||
            available_count != requested_image_ids.size() - result.quarantined.size()) {
            return CachedOpenImagesGroup{};
        }
        ...
        if (available_count != result.available_image_ids.size() ||
            available_digest != cached_image_selection_digest(result.available_image_ids)) {
            return CachedOpenImagesGroup{};
        }
        const auto dimensions = manifest.find("dimensions");
        if (dimensions == manifest.end() || !dimensions->is_array() || dimensions->size() != available_count * 3U) {
            return CachedOpenImagesGroup{};
        }
        ...
        if (!result.available_image_ids.empty() &&
            !validate_cached_image_group(image_root, completion, identity, result.available_image_ids, &result.image_bytes,
...
void complete_open_images_group(const std::filesystem::path& image_root, const std::filesystem::path& completion,
                                const std::string_view identity, const std::span<const std::uint64_t> requested_image_ids,
                                const std::span<const std::uint64_t> available_image_ids,
                                const std::span<const QuarantinedImage> quarantined, const NormalizedAnnotationIndex& index,
    ...
    for (const QuarantinedImage& image : quarantined) {
        quarantine_records.push_back({{"image_id", image.image_id}, {"reason", image.reason}});
    }
    ...
    trace_benchmark_event(trace, "benchmark.images.complete", [&] {
        return nlohmann::json{{"root", image_root.string()},
                              {"identity", identity},
                              {"images", available_image_ids.size()},
...
[[nodiscard]] AcquiredOpenImages acquire_open_images(const BenchmarkCacheLayout& cache, NormalizedAnnotationIndex& index,
                                                     std::vector<QuarantinedImage>* quarantined,
                                                     mmltk::common::concurrency::CancellationObservation cancel_requested,
                                                     ProgressReporter* progress, const int num_workers, const std::size_t cache_workers,
                                                     const BenchmarkTraceSink& trace,
                                                     const std::optional<JpegDecodeProbe> decode_probe = std::nullopt) {
    ...
    const std::size_t configured_workers =
        common_math::checked_cast<std::size_t>(num_workers, "Open Images configured worker count overflow");
    if (configured_workers > std::numeric_limits<std::size_t>::max() / 10U) {
        throw std::overflow_error("Open Images transfer concurrency overflow");
    }
    ...
    for (std::size_t begin = 0U; begin < ids.size(); begin += kOpenImagesGroupImages) {
        ...
        CachedOpenImagesGroup cached_group =
            discover_cached_open_images_group(image_root, completion, identity, group, cancel_requested, trace);
        ...
        if (group_cache_hit) {
            ...
            cached_image_bytes =
                common_math::checked_add(cached_image_bytes, cached_group.image_bytes, "Open Images cached byte total overflow");
            ...
            if (regular && !error && bytes > 0U) {
                try {
                    ...
                    trace_benchmark_event(trace, "benchmark.images.cache_invalid", [&] {
                        return nlohmann::json{
                            {"source", "open-images"}, {"shard", shard}, {"image_id", image_id}, {"error", jpeg_error.what()}};
                    });
                }
            }
        ...
        if (!missing_downloads.empty()) {
            ...
            download_open_images(missing_downloads, image_root, &index, quarantined, cancel_requested, progress, &completed_images,
                                 ids.size(), &cached_image_bytes, transfer_concurrency, cache_workers, trace);
        }
        ...
        if (decode_probe && std::ranges::binary_search(group, decode_probe->image_id) &&
            std::ranges::binary_search(group_available, decode_probe->image_id)) {
        ...
        const std::span<const QuarantinedImage> group_quarantined(quarantined->data() + quarantine_begin,
                                                                  quarantined->size() - quarantine_begin);
        ...
        complete_open_images_group(image_root, completion, identity, group, group_available, group_quarantined, index,
                                   cached_image_bytes - group_bytes_begin, cancel_requested, trace);
...
[[nodiscard]] std::optional<NormalizedAnnotationIndex> discover_cached_index(
    const std::filesystem::path& path, const BenchmarkDatasetSource source, const std::string_view split,
...
[[nodiscard]] NormalizedAnnotationIndex load_or_build_index(const BenchmarkCacheLayout& cache, const std::filesystem::path& path,
    ...
    ArtifactLease lease = ArtifactLease::acquire(
        cache.locks / (std::string(benchmark_source_name(source)) + "-" + std::string(split) + ".index.lock"), cancel_requested);
    if (auto cached = load_normalized_annotation_index(path, source, split, annotation_sha256, cancel_requested, trace)) {
...
[[nodiscard]] constexpr auto planned_label_identity(const PlannedBenchmarkInstance& instance) noexcept {
    return std::tuple{instance.label.class_id, instance.label.bbox_x1, instance.label.bbox_y1, instance.label.bbox_x2,
                      instance.label.bbox_y2};
}
...
SourceCompileCount append_source_plan(const NormalizedAnnotationIndex& index, const std::vector<CachedImageDirectory>& directories,
                                      ...
                                      const bool require_every_image, PreparedBenchmarkSplit* split, std::uint64_t* dropped_boxes,
                                      mmltk::common::concurrency::CancellationObservation cancel_requested) {
    if (directories.empty()) { throw std::runtime_error("benchmark source has no cached image directories"); }
    if (unavailable_image_ids != nullptr && (!std::ranges::is_sorted(*unavailable_image_ids) ||
                                             std::ranges::adjacent_find(*unavailable_image_ids) != unavailable_image_ids->end())) {
    ...
    for (std::size_t image_index = 0U; image_index < index.images.size(); ++image_index) {
        ...
        if (index.source == BenchmarkDatasetSource::kObjects365V2) {
            ...
            if (local_source == std::numeric_limits<std::size_t>::max()) {
                throw std::runtime_error("Objects365 image references an unacquired shard");
            }
        }
        ...
        for (std::uint64_t box_index = image.first_box; box_index < image.first_box + image.box_count; ++box_index) {
            ...
            if (box.mask_rle_pairs != 0U) {
                if (box.mask_rle_offset > index.mask_rle_pairs.size() ||
                    box.mask_rle_pairs > index.mask_rle_pairs.size() - box.mask_rle_offset) {
                    throw std::runtime_error("benchmark source mask range is invalid");
                }
                const auto source_mask =
                    std::span(index.mask_rle_pairs).subspan(static_cast<std::size_t>(box.mask_rle_offset), box.mask_rle_pairs);
        ...
        *dropped_boxes += static_cast<std::uint64_t>(image_labels.end() - unique_end);
        ...
        for (PlannedBenchmarkInstance& instance : image_labels) {
            instance.label.mask_rle_offset = common_math::checked_cast<std::uint32_t>(
                common_math::checked_multiply(split->rle_pairs.size(), sizeof(RLEPair), "benchmark mask offset overflow"),
                "benchmark mask offset overflow");
            instance.label.mask_rle_pairs =
                common_math::checked_cast<std::uint16_t>(instance.mask_rle.size(), "benchmark instance mask run count overflow");
        ...
        counts.compiled_boxes =
            common_math::checked_add(counts.compiled_boxes, image_labels.size(), "benchmark compiled box count overflow");
    }
    return counts;
}
...
[[nodiscard]] PreparedBenchmarkSplit make_split(const std::string& name) {
    ...
    for (const std::string_view class_name : coco80_class_names()) {
        split.class_names.emplace_back(class_name);
    }
    return split;
}
...
[[nodiscard]] std::uint64_t estimate_output_bytes(const std::uint64_t images, const std::uint64_t boxes, const std::uint32_t resolution,
    ...
    const std::uint64_t pixels = common_math::checked_multiply(
        images,
        common_math::checked_multiply(
            common_math::checked_multiply<std::uint64_t>(resolution, resolution, "benchmark output resolution overflow"),
    ...
    return common_math::checked_add(common_math::checked_add(common_math::checked_add(pixels, labels, "benchmark output size overflow"),
                                                             masks, "benchmark output size overflow"),
                                    common_math::checked_add(index, 2U * HUGE_PAGE_SIZE, "benchmark output metadata overflow"),
                                    "benchmark output size overflow");
}
...
void publish_dataset_directory(const std::filesystem::path& staging, const std::filesystem::path& output, const bool overwrite,
    ...
    trace_benchmark_event(trace, "benchmark.publication.old_generation_cleanup", [&] {
        return nlohmann::json{
            {"path", staging.string()}, {"removed", removed}, {"error", cleanup_error ? cleanup_error.message() : std::string{}}};
    });
}
...
[[nodiscard]] nlohmann::json source_catalog_manifest() {
    ...
    const auto append = [&](const CatalogArtifact& artifact) {
        nlohmann::json record{{"artifact_id", artifact.artifact_id},
                              {"url", artifact.url},
    ...
    for (const CatalogArtifact& artifact : objects365_train_image_artifacts()) {
        append(artifact);
    }
    append(open_images_boxes_artifact());
    append(open_images_classes_artifact());
    return nlohmann::json{{"artifacts", std::move(artifacts)},
                          {"open_images_train_image_url_template", open_images_train_image_url_template()}};
}
...
void compile_benchmark_dataset(BenchmarkCompilerConfig config) {
    ...
    if (config.output_dir == config.output_dir.root_path()) {
        throw std::runtime_error("benchmark output directory must not be the filesystem root");
    }
    const std::filesystem::path output_parent =
        config.output_dir.parent_path().empty() ? std::filesystem::path{"."} : config.output_dir.parent_path();
    ...
    const std::filesystem::path cache_root =
        config.cache_dir.empty() ? std::filesystem::path{"./.cache/benchmark-dataset/v1"} : config.cache_dir;
    ...
    ArtifactLease output_lease = ArtifactLease::acquire(normalized_output.parent_path() / ".cache" / "benchmark-dataset" / "v1" / "locks" /
                                                            ("output-" + output_lock_identity(normalized_output) + ".lock"),
                                                        cancel_requested);
    ...
    const std::size_t effective_num_workers =
        std::min<std::size_t>(common_math::checked_cast<std::size_t>(config.num_workers, "benchmark effective worker count overflow"),
                              common_system::allowed_cpu_set().size());
    const int effective_num_workers_int =
        common_math::checked_cast<int>(effective_num_workers, "benchmark effective worker count overflow");
    ...
    ArtifactLease objects_annotation_lifecycle =
        ArtifactLease::acquire(cache.locks / "objects365-annotations.lifecycle.lock", cancel_requested);
    ArtifactLease open_images_annotation_lifecycle =
        ArtifactLease::acquire(cache.locks / "open-images-annotations.lifecycle.lock", cancel_requested);
    ...
    std::uint64_t completed_indexes = static_cast<std::uint64_t>(coco_train.has_value()) +
                                      static_cast<std::uint64_t>(coco_val.has_value()) + static_cast<std::uint64_t>(objects.has_value()) +
                                      static_cast<std::uint64_t>(open_images.has_value());
    ...
    const auto annotation_parse_options = [&](const BenchmarkDatasetSource source, std::string split,
                                              const std::uint32_t expected_image_count, const bool keep_images_without_mapped_boxes) {
    ...
    const auto fetch_annotation_artifacts = [&](const std::vector<DownloadRequest>& requests, const std::size_t workers,
                                                ArtifactProgressTotals* totals) {
        const std::vector<DownloadResult> downloads = download_artifacts(
            requests, workers, cancel_requested, [&](const DownloadProgress& update) { totals->update(update, &progress); }, trace);
    ...
    if (!annotation_requests.empty()) {
        ...
        for (const DownloadRequest& request : annotation_requests) {
            annotation_storage =
                common_math::checked_add(annotation_storage, request.expected_size, "benchmark annotation storage estimate overflow");
        ...
        if (needs_archive_scratch) {
            annotation_storage =
                common_math::checked_add(annotation_storage, kArchiveScratchBytes, "benchmark annotation storage estimate overflow");
        }
        ...
        if (!coco_train || !coco_val) {
            progress.source_activity(BenchmarkDatasetSource::kCoco2017, "Downloading COCO annotation metadata");
        }
        if (!objects) { progress.source_activity(BenchmarkDatasetSource::kObjects365V2, "Downloading Objects365 annotation metadata"); }
        if (!open_images) {
            progress.source_activity(BenchmarkDatasetSource::kOpenImagesV7, "Downloading Open Images annotation metadata");
        }
    ...
    const auto repair_annotation_artifacts = [&](const BenchmarkDatasetSource source, const std::vector<DownloadRequest>& requests,
        ...
        progress.source_activity(source,
                                 ...
                                 "validation failure");
        fetch_annotation_artifacts(requests,
                                   requests.size() == 1U ? std::min<std::size_t>(8U, effective_num_workers)
                                                         : std::min<std::size_t>({3U, requests.size(), effective_num_workers}),
            ...
            [&] {
                ...
                if (!coco_train) {
                    ...
                    const std::string digest =
                        extract_archive_member(archive.path, "annotations/instances_train2017.json", json_path, archive.identity,
                                               cache.locks / "coco-train-json.extract.lock", cancel_requested, trace);
                    coco_train = load_or_build_index(
                        cache, coco_train_index_path, BenchmarkDatasetSource::kCoco2017, "train2017", digest, cancel_requested, trace, [&] {
                            ...
                            return parse_coco_style_annotations(
                                json_path, digest, coco_category_mappings(),
                ...
                if (!coco_val) {
                    ...
                    const std::string digest =
                        extract_archive_member(archive.path, "annotations/instances_val2017.json", json_path, archive.identity,
                                               cache.locks / "coco-val-json.extract.lock", cancel_requested, trace);
                    coco_val = load_or_build_index(
                        cache, coco_val_index_path, BenchmarkDatasetSource::kCoco2017, "val2017", digest, cancel_requested, trace, [&] {
    ...
    if (!objects) {
        retry_annotation_indexing(
            ...
            [&] {
                ...
                objects = load_or_build_index(cache, objects_index_path, BenchmarkDatasetSource::kObjects365V2, "train", annotation_digest,
                                              cancel_requested, trace, [&] {
                                                  progress.source_activity(BenchmarkDatasetSource::kObjects365V2,
                                                                           "Parsing and indexing Objects365 annotations");
                                                  return parse_coco_style_annotations(
                                                      json_path, annotation_digest, objects365_category_mappings(),
        ...
        retry_annotation_indexing(
            ...
            [&] {
                ...
                open_images = load_or_build_index(
                    cache, open_images_index_path, BenchmarkDatasetSource::kOpenImagesV7, "train", annotation_identity, cancel_requested,
                    trace, [&] {
                        ...
                        return parse_open_images_annotations(
                            boxes.path, classes.path, annotation_identity, open_images_category_mappings(),
            ...
            [&](const std::exception& error) {
                ...
                repair_annotation_artifacts(BenchmarkDatasetSource::kOpenImagesV7,
                                            {*open_images_boxes_request, *open_images_classes_request}, error.what());
            });
    }
    if (!coco_train || !coco_val || !objects || !open_images) {
        throw std::runtime_error("benchmark normalized annotation indexing did not complete");
    }
    if (coco_val->images.size() != 5000U) { throw std::runtime_error("COCO val2017 normalized index must contain exactly 5,000 images"); }
    {
        ...
        for (const NormalizedImage& image : coco_val->images) {
            validation_ids.emplace(image.source_image_id);
        }
        for (const NormalizedImage& image : coco_train->images) {
            if (validation_ids.contains(image.source_image_id)) {
                throw std::runtime_error("COCO train and validation metadata reuse an image ID");
            }
        }
        trace_benchmark_event(trace, "benchmark.split_reuse.metadata_check", [&] {
            return nlohmann::json{
                {"coco_train_images", coco_train->images.size()}, {"coco_val_images", coco_val->images.size()}, {"overlap", 0}};
        });
    }
    ...
    for (const CatalogArtifact& artifact : sampling_object_artifacts) {
        object_shard_bytes.push_back(artifact.expected_size);
    }
    ...
    *open_images = std::move(open_images_sampling.index);
    ...
    const std::uint64_t selected_train_images = common_math::checked_add(
        common_math::checked_add(coco_train->images.size(), objects->images.size(), "benchmark selected image count overflow"),
    ...
    const std::uint64_t selected_train_labels = common_math::checked_add(
        common_math::checked_add(coco_train->boxes.size(), objects->boxes.size(), "benchmark selected label count overflow"),
        open_images->boxes.size(), "benchmark selected label count overflow");
    const std::uint64_t projected_output_upper_bound = common_math::checked_add(
        estimate_output_bytes(selected_train_images, selected_train_labels, config.resolution, coco_train->mask_rle_pairs.size()),
    ...
    for (const std::uint16_t shard : combined_sampling.objects365_shards) {
        archive_sizes.push_back(sampling_object_artifacts.at(shard).expected_size);
    }
    std::uint64_t retained_archive_bytes = 0U;
    for (const std::uint64_t archive_size : archive_sizes) {
        retained_archive_bytes =
            common_math::checked_add(retained_archive_bytes, archive_size, "benchmark archive cache estimate overflow");
    }
    const std::uint64_t projected_cache = common_math::checked_add(
        common_math::checked_multiply(selected_total_images, kEstimatedJpegBytes, "benchmark cache estimate overflow"),
    ...
    for (std::size_t shard = 0U; shard < object_artifacts.size(); ++shard) {
        ...
        if (!ids.empty()) {
            archive_tasks.push_back(ArchiveTask{BenchmarkDatasetSource::kObjects365V2, "patch-" + std::to_string(shard), std::move(ids),
                                                object_artifacts[shard]});
        }
    }
    ...
    if (overlap_open_images) {
        open_images_thread = std::thread([&] {
            try {
                open_image_result = acquire_open_images(cache, *open_images, &quarantined, cancel_requested, &progress,
                                                        acquisition_num_workers, open_cache_workers, trace);
            } catch (...) { record_pipeline_error(); }
        });
    }
    try {
        std::atomic<std::size_t> next_archive_task{0U};
        common_concurrency::parallel_for_range_indexed<int>(
            0, archive_workers, archive_workers, [&](const int, const int begin, const int end) {
                for (int worker = begin; worker < end; ++worker) {
                    ...
                    while (true) {
                        ...
                        std::atomic<std::uint64_t>* source_completed =
                            task.source == BenchmarkDatasetSource::kCoco2017 ? &coco_extracted : &objects_completed;
                        const std::uint64_t source_total =
                            task.source == BenchmarkDatasetSource::kCoco2017 ? coco_selected_images : objects->images.size();
                        try {
                            archive_results[task_index] = acquire_archive_images(
                                cache, task.source, task.shard, task.image_ids, task.artifact, cancel_requested, &progress,
                                &image_transfer_progress, &cache_storage_reservations, source_completed, source_total,
                                decompression_workers, archive_cache_workers, archive_download_connections, trace);
                        } catch (...) {
    ...
    if (!coco_train_result.has_value() || !coco_val_result.has_value()) {
        throw std::runtime_error("COCO image extraction did not complete");
    }
    ...
    const auto refresh_archive_availability = [&] {
        ...
        for (const CachedImageDirectory& directory : object_images) {
            unavailable_object_count += directory.quarantined.size();
        }
    ...
    progress.source_complete(BenchmarkDatasetSource::kObjects365V2,
                             objects_index_cache_hit && std::ranges::all_of(object_images, [](const CachedImageDirectory& directory) {
                                 return directory.cache_hit;
                             }));
    ...
    const SourceCompileCount validation_count = append_source_plan(*coco_val, coco_val_images, nullptr, config.resolution, true,
                                                                   &validation, &validation_target_dropped_boxes, cancel_requested);
    progress.phase(DatasetCompilePhase::Labels, ++completed_label_plans, kLabelPlanCount);
    ...
    common_io::StagingDirectory staging(config.output_dir, ".", ".benchmark.tmp.XXXXXX",
                                        "cannot create benchmark dataset staging directory");
    ...
    const auto repair_cached_image = [&](const PreparedBenchmarkSplit& split, const BenchmarkImageReadError& error) {
        ...
        progress.source_activity(task->source,
                                 "Repairing failed cached JPEG " + std::to_string(error.source_image_id()) + " from " + task->shard);
        std::atomic<std::uint64_t> repaired_images{0U};
        CachedImageDirectory repaired =
    ...
    const auto add_annotation_drops = [&](const NormalizedAnnotationIndex& index) {
        annotation_drops =
            common_math::checked_add(annotation_drops, index.rejected.degenerate_boxes, "benchmark rejected annotation count overflow");
        annotation_drops =
            common_math::checked_add(annotation_drops, index.rejected.duplicate_boxes, "benchmark rejected annotation count overflow");
    };
    add_annotation_drops(*coco_train);
    add_annotation_drops(*coco_val);
    add_annotation_drops(*objects);
    add_annotation_drops(*open_images);
    const std::uint64_t target_dropped_boxes = common_math::checked_add(training_target_dropped_boxes, validation_target_dropped_boxes,
                                                                        "benchmark target annotation count overflow");
    ...
    append_source_manifest(*open_images, source_counts[2], open_images_boxes_artifact().url, open_images_index_path,
    ...
    const auto append_image_cache = [&](const std::vector<CachedImageDirectory>& directories) {
        for (const CachedImageDirectory& directory : directories) {
            ...
            cached_image_bytes =
                common_math::checked_add(cached_image_bytes, directory.image_bytes, "benchmark cached image byte total overflow");
        }
    };
    ...
    manifest["compiled_bytes"] =
        common_math::checked_add(manifest["train"]["bytes"].get<std::uint64_t>(), manifest["val"]["bytes"].get<std::uint64_t>(),
