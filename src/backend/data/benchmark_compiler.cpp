...
void write_split_with_progress(BenchmarkSplitWriter& writer, const BenchmarkWriteRequest& request, ProgressReporter* progress, const BenchmarkTraceSink& trace) {
   ...
   try {
    ...
    const auto append_pixel_members = [&](const NormalizedAnnotationIndex& index, bool validation_split, std::optional<CoconutImageNamespace> physical_source = {},
     ...
     for (const auto& task : archive_tasks) {
      if (physical_source ? task.image_namespace != *physical_source : task.source != index.source || (index.source == BenchmarkDatasetSource::kCoco2017 && task.shard != index.split + "2017"))
       continue;
    ...
    const auto prepare_source_labels = [&](std::size_t slot, const NormalizedAnnotationIndex& index, std::vector<CachedImageDirectory> directories) {
     ...
     if (slot == 2)
      for (const auto& image : quarantined)
       if (image.source == BenchmarkDatasetSource::kOpenImagesV7) unavailable.push_back(image.image_id);
    ...
    const auto archive_labels_ready = [&](std::size_t task) {
     if (coconut) return;
     if (task < 2)
      prepare_source_labels(task == 0 ? 0 : 3, task == 0 ? *coco_train : *coco_val, {*archive_results[task]});
    ...
    if (pipeline_error) {
     try {
      pipeline.drain();
     } catch (...) { /* Every reader has retired; propagate the initiating failure. */
     }
    ...
    for (const QuarantinedImage& image : quarantined) {
     trace_benchmark_event(
      trace, "benchmark.quarantine.image", [&] { return nlohmann::json{{"source", benchmark_source_name(image.source)}, {"image_id", image.image_id}, {"reason", image.reason}}; });
    }
