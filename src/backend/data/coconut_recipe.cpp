...
CoconutRecipePreparation prepare_coconut_recipe(const BenchmarkCompilerConfig& config, const BenchmarkCacheLayout& cache,
    const CoconutRecipeCatalog& catalog, std::span<const AdmittedRecipeArchive> acquired, const CoconutPhysicalMembership& physical, ProgressReporter& progress, std::size_t workers,
    mmltk::common::concurrency::CancellationObservation cancellation, const BenchmarkTraceSink& trace, std::span<const CoconutImageNamespace> refreshed_sources) {
    ...
    const auto acquire = [&](const CatalogArtifact& artifact, std::string_view owner, bool redownload = false) {
        ...
        const auto reservation = reservations.reserve(additional_download_bytes(request.destination,artifact.expected_size), "additional COCONut annotation download bytes");
    ...
    prepared.manifest = {{"dataset", "coconut"}, {"validation", config.selection.validation},
                         {"components", nlohmann::json::array()}, {"artifacts", nlohmann::json::array()}};
    for (const auto& release : catalog.releases) {
        ...
        bool complete = std::ranges::none_of(sources, [&](const auto source) { return std::ranges::find(refreshed_sources,source)!=refreshed_sources.end(); });
        for (auto source : sources) {
            if (!complete) break;
            auto cached = load_coconut_component(path_for(source), release.edition, source, identity, cancellation);
            if (!cached) { complete = false; break; }
        ...
        if (!complete) {
            ...
            else progress.phase(DatasetCompilePhase::Indexing, 0, release.expected_rows);
            for (const auto& artifact : downloads) {
                if (artifact.path.extension() == ".parquet") request.parquet_shards.push_back(artifact.path);
                else if (artifact.path.extension() == ".json") request.annotation_json = artifact.path;
                else request.mask_archive = artifact.path;
            }
        ...
        for (auto& component : components) {
            ...
            prepared.manifest["components"].push_back({{"edition",release.edition},{"revision",release.revision},
                {"physical_source",coconut_namespace_name(component.source)},{"input_identity",component.input_identity},
                {"offered_images",component.index.images.size()},{"annotation_source",coconut_annotation_source(component.source)},
    ...
    const auto split_key = [](const CoconutPhysicalImage& image) {
        const auto source = image.source == CoconutImageNamespace::CocoUnlabeled || image.source == CoconutImageNamespace::CocoValidation
            ? CoconutImageNamespace::CocoTrain : image.source;
    ...
    if (prepared.stock_validation) for (const auto& image : prepared.stock_validation->images)
    ...
    for (const auto& component : prepared.components) {
        if (coconut_validation_component(component.edition)) for (const auto& image : component.inventory)
    ...
    for (const auto& admitted : acquired) {
        ...
        prepared.manifest["artifacts"].push_back({{"artifact_id",archive.artifact.artifact_id},
            {"url",archive.artifact.url},{"filename",archive.artifact.filename},{"expected_size",archive.artifact.expected_size},
            {"expected_sha256",archive.artifact.expected_sha256},{"physical_source",coconut_namespace_name(archive.source)},
            {"identity",admitted.download.identity},{"bytes",admitted.download.size}});
    }
    if (config.selection.validation == CoconutValidation::Stock) prepared.manifest["artifacts"].push_back({
        {"artifact_id",catalog.stock_annotations.artifact_id},{"url",catalog.stock_annotations.url},
        {"expected_size",catalog.stock_annotations.expected_size},{"expected_sha256",catalog.stock_annotations.expected_sha256}});
