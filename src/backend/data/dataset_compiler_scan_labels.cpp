...
struct MaskResizeLookup {
    ...
    void prepare(const ImageDimensions& source, const uint32_t target_w, const uint32_t target_h) {
        ...
        if (source_dims.width == source.width && source_dims.height == source.height && target_width == target_w &&
...
ParsedRle parse_rle(const std::string_view rle) {
    ...
    while (cursor != end) {
        ...
        if (start_result.ec != std::errc{} || start_result.ptr == cursor) {
            throw std::runtime_error("mask_rle contains an invalid or overflowing run start");
        }
        ...
        if (cursor != end && !std::isspace(static_cast<unsigned char>(*cursor))) {
            throw std::runtime_error("mask_rle runs must be separated by whitespace");
        }
        ...
        if (length > std::numeric_limits<size_t>::max() - parsed.foreground) {
            throw std::runtime_error("mask_rle foreground count overflow");
        }
...
[[nodiscard]] CompileDiagnostic make_resize_diagnostic(const CompileDiagnosticKind kind, const std::filesystem::path& annotation_file,
                                                       const std::string& class_name, const size_t line_number,
                                                       const ImageDimensions& source_dims, const uint32_t target_width,
                                                       const uint32_t target_height, const size_t source_foreground) {
...
void append_source_bbox_mismatch(std::vector<CompileDiagnostic>* diagnostics, const json& record,
                                 const std::filesystem::path& annotation_file, const std::string& class_name, const size_t line_number,
                                 const ImageDimensions& source_dims, const uint32_t target_width, const uint32_t target_height,
                                 const MaskBounds& source_bounds, const size_t source_foreground) {
    if (diagnostics == nullptr) { return; }
    ...
    CompileDiagnostic diagnostic = make_resize_diagnostic(CompileDiagnosticKind::kSourceBoundingBoxMismatch, annotation_file, class_name,
...
void materialize_mask_row_major(const std::vector<RLEPair>& input_pairs, const ImageDimensions& source_dims,
                                std::vector<uint8_t>& source_mask_scratch, MaskBounds* source_bounds = nullptr) {
...
void clear_mask_letterbox_padding(std::vector<uint8_t>& target_mask, const uint32_t target_width, const uint32_t target_height,
    ...
    if (bottom_pixels != 0U) {
        std::fill_n(target_mask.data() + static_cast<size_t>(content_end_y) * target_width, bottom_pixels, uint8_t{0});
    }
}

ResizedMask resize_mask_row_major(const std::vector<RLEPair>& input_pairs, const ImageDimensions& source_dims, uint32_t target_width,
                                  uint32_t target_height, const RgbLetterbox& letterbox, std::vector<uint8_t>& source_mask_scratch,
    ...
    for (uint32_t y = 0; y < letterbox.resized_height; ++y) {
        ...
        for (uint32_t x = 0; x < letterbox.resized_width; ++x) {
            dst_row[x] = src_row[resize_lookup.source_x[x]] != 0 ? 1u : 0u;
        }
    }
...
ParsedLabels parse_jsonl(const std::filesystem::path& annotation_file, const std::filesystem::path& image_file,
                         const std::unordered_map<std::string, uint8_t>& class_map, const CompilerConfig& config,
                         ResizeObservation* resize_observation, std::vector<uint8_t>& source_mask_scratch,
                         std::vector<uint8_t>& target_mask_scratch, MaskResizeLookup& resize_lookup,
    ...
    const RgbLetterbox letterbox = exact_target ? RgbLetterbox{target_width, target_height, 0U, 0U}
                                                : compute_rgb_letterbox(source_dims.width, source_dims.height, target_width, target_height);
    ...
    while (std::getline(file, line)) {
        ...
        try {
            ...
            if (needs_resize) {
                ResizedMask resized = resize_mask_row_major(instance.rle_pairs, source_dims, target_width, target_height, letterbox,
                                                            source_mask_scratch, target_mask_scratch, resize_lookup, source_bounds_out);
                if (source_bounds_out != nullptr) {
                    append_source_bbox_mismatch(diagnostics, record, annotation_file, class_name, line_number, source_dims, target_width,
                                                target_height, source_bounds, source_foreground);
                }
                if (!resized.has_foreground) {
                    if (diagnostics != nullptr) {
                        append_diagnostic(diagnostics, make_resize_diagnostic(CompileDiagnosticKind::kDroppedInstanceAfterResize,
                                                                              annotation_file, class_name, line_number, source_dims,
                                                                              target_width, target_height, source_foreground));
                    }
            ...
            throw std::runtime_error(std::string(error.what()) + " in " + annotation_file.string() + " at line " +
                                     std::to_string(line_number) + " (source " + std::to_string(source_dims.width) + "x" +
                                     std::to_string(source_dims.height) + ", target " + std::to_string(target_width) + "x" +
...
LabelBlocks build_label_blocks(const std::filesystem::path& split_dir, uint32_t num_images, const CompilerConfig& config,
                               const std::unordered_map<std::string, uint8_t>& class_map, int num_workers,
                               const std::span<const int> worker_cpus, ProgressCounter* completed_images,
    ...
    parallel_for_range_indexed<uint32_t>(
        0, num_images, num_workers, worker_cpus,
        [&](int worker_id, uint32_t start, uint32_t end) {  // NOLINT(bugprone-easily-swappable-parameters)
            ...
            std::vector<CompileDiagnostic>* diagnostics =
                worker_diagnostics.empty() ? nullptr : &worker_diagnostics[static_cast<size_t>(worker_id)];
            for (uint32_t image_index = start; image_index < end; ++image_index) {
                ...
                if (worker_cancelled.load(std::memory_order_relaxed) ||
                    (failure_requested != nullptr && failure_requested->load(std::memory_order_relaxed))) {
                    break;
                }
                try {
                    ResizeObservation resize_observation;
                    ParsedLabels parsed_labels =
                        parse_jsonl(annotation_path(split_dir, image_index), image_path(split_dir, image_index), class_map, config,
            ...
            stats.active_ns = checked_cast<std::uint64_t>(
                std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - worker_start).count(),
    ...
    if (config.diagnostics != nullptr) {
        for (std::vector<CompileDiagnostic>& diagnostics : worker_diagnostics) {
            for (CompileDiagnostic& diagnostic : diagnostics) {
                append_diagnostic(config.diagnostics, std::move(diagnostic));
            }
        }
    }
