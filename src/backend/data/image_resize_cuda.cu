...
constexpr std::size_t workspace_limit = 256U * 1024U * 1024U;
...
__global__ void prepare_kernel(Workspace work, std::uint32_t sw, std::uint32_t sh, std::uint32_t dw, std::uint32_t dh) {
    for (std::size_t i=blockIdx.x*std::size_t(blockDim.x)+threadIdx.x;
         i<static_cast<std::size_t>(max(max(dw,dh),256U)); i+=std::size_t(blockDim.x)*gridDim.x) {
...
__device__ void accumulate(RgbConstImageView source, const Workspace& work, Footprint fx, Footprint fy,
                           std::size_t offset, std::size_t step, MomentAccumulator& sum, float& alpha) {
...
__global__ void large_moments(RgbConstImageView source, Workspace work, std::uint32_t width, std::size_t count) {
    ...
    for (std::size_t i=blockIdx.x;i<count;i+=gridDim.x) {
        ...
        for (int k=0;k<3;++k) { partial[k][threadIdx.x]=sum.mean[k]; partial[k+3][threadIdx.x]=sum.variance[k]; }
        ...
        for (unsigned stride=threads/2;stride;stride/=2) {
            if (threadIdx.x<stride) {
                ...
                Moment a,b;
                for(int k=0;k<3;++k) {
                    a.mean[k]=partial[k][lane];a.variance[k]=partial[k+3][lane];
                    b.mean[k]=partial[k][other];b.variance[k]=partial[k+3][other];
                }
                const float aw=partial[6][lane],bw=partial[6][other];
                const auto combined=merge(a,aw,b,bw);
                for(int k=0;k<3;++k) {partial[k][lane]=combined.mean[k];partial[k+3][lane]=combined.variance[k];}
        ...
        if (threadIdx.x==0) {
            for (int k=0;k<3;++k) { sum.mean[k]=partial[k][0]; sum.variance[k]=partial[k+3][0]; }
...
__global__ void output_kernel(Workspace work, RgbMutableImageView output, std::size_t count) {
    ...
    for (std::size_t i=blockIdx.x*std::size_t(blockDim.x)+threadIdx.x;i<count;i+=std::size_t(blockDim.x)*gridDim.x) {
        ...
        store<Format>(output,x,y,reconstruct(work.moments[i],work.coefficients[i],work.coefficients[left],
                                             work.coefficients[up],work.coefficients[corner]),alpha,*work.transfer);
    }
}
template<RgbPixelFormat Format, bool Integer>
void launch(RgbConstImageView source, RgbMutableImageView output, Workspace work, cudaStream_t stream) {
    ...
    if (fw<=64 && fh<=64/fw) small_moments<Format,Integer><<<blocks(count),threads,0,stream>>>(source,work,output.layout.width,count);
    else large_moments<Format,Integer><<<static_cast<unsigned>(std::min<std::size_t>(count,65535)),threads,0,stream>>>(source,work,output.layout.width,count);
...
void dispatch(RgbConstImageView source, RgbMutableImageView output, Workspace work, cudaStream_t stream) {
    ...
    else launch<Format,false>(source,output,work,stream);
}
}
struct GpuPerceptualDownscaler::Impl {
    ...
    Impl(frameworks::gpu::DeviceContext owner, frameworks::gpu::TerminalCudaRetirementAuthority& authority)
        ...
        try {
            context.Bind();
            if(cuCtxGetCurrent(&native_context)!=CUDA_SUCCESS || !native_context)
                throw std::runtime_error("cannot inspect perceptual CUDA owning context");
        } catch(...) { (void)cuCtxSetCurrent(previous);throw; }
    ...
    DriverAdmission check_span(const void* pointer,std::size_t extent) const {
        ...
        void* results[]{&allocation_context,&memory_type};
        ...
        return {range_status,range_status==CUDA_SUCCESS && address>=base && address-base<=bytes &&
                extent<=bytes-static_cast<std::size_t>(address-base),AdmissionQuery::Range};
    }
...
void GpuPerceptualDownscaler::retire(cudaError_t failure) noexcept {
    Impl::retire_owner(impl_,failure);
}
GpuPerceptualDownscaler::~GpuPerceptualDownscaler() noexcept {
    if (!impl_) return;
    if(cuCtxPushCurrent(impl_->native_context)!=CUDA_SUCCESS) {retire(cudaErrorContextIsDestroyed);return;}
...
void GpuPerceptualDownscaler::finish() {
    ...
    if (status!=cudaSuccess) { retire(status); require_cuda(status); }
}
void GpuPerceptualDownscaler::downscale(RgbConstImageView source, RgbMutableImageView destination, cudaStream_t stream,
    ...
    if (settled!=cudaSuccess) { retire(settled); require_cuda(settled); }
    ...
    auto& slot=[&]() -> perceptual::CudaDownscaleCompletion::Submission& {
        try {return impl_->completion.reserve(stream,std::move(source_custody),std::move(destination_custody));}
    ...
    try {
        ...
        if (identity) {
            ...
            if (grow || impl_->sw!=source.layout.width || impl_->sh!=source.layout.height ||
                impl_->dw!=destination.layout.width || impl_->dh!=destination.layout.height || impl_->alpha!=alpha) {
                ...
                require_cuda(cudaGetLastError());
                impl_->sw=source.layout.width;impl_->sh=source.layout.height;impl_->dw=destination.layout.width;impl_->dh=destination.layout.height;impl_->alpha=alpha;
            }
        ...
        const auto status=impl_->completion.settle(true);
        if (status!=cudaSuccess) retire(status);
        else {impl_->sw=0;impl_->completion.forget_order();}
