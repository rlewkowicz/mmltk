...
std::filesystem::path BenchmarkCacheLayout::source_downloads(const std::string_view source) const {
    return ensured_source_subdirectory(downloads, source);
}

std::filesystem::path BenchmarkCacheLayout::source_images(const std::string_view source) const {
    return ensured_source_subdirectory(images, source);
}

std::filesystem::path BenchmarkCacheLayout::source_indexes(const std::string_view source) const {
    return ensured_source_subdirectory(indexes, source);
}
...
ArtifactLease ArtifactLease::acquire(const std::filesystem::path& lock_path,
...
nlohmann::json read_json_file(const std::filesystem::path& path) {
    ...
    try {
        ...
        throw std::runtime_error("invalid benchmark cache metadata " + path.string() + ": " + error.what());
    }
}
