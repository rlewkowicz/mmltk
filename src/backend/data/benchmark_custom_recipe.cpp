...
CustomRecipePreparation prepare_custom_recipe(const BenchmarkCompilerConfig& config, const BenchmarkCacheLayout& cache, const CustomRecipeCatalog& catalog,
 ...
 if (!annotation_requests.empty()) {
  ...
  for (const DownloadRequest& request : annotation_requests) {
   ...
   needs_archive_scratch =
    needs_archive_scratch || request.artifact_id == catalog.coco_annotations.artifact_id || request.artifact_id == catalog.objects_annotations.artifact_id;
  }
 ...
 if (coco_cache.pending_download()) {
  coco_cache.settle(std::move(annotation_downloads.at(catalog.coco_annotations.artifact_id)), progress, effective_num_workers, completed_indexes, kIndexCount);
 }
 ...
 if (!open_images) {
  retry_annotation_indexing(
   ...
   [&] {
    ...
    open_images =
     load_or_build_index(cache, open_images_index_path, BenchmarkDatasetSource::kOpenImagesV7, "train", annotation_identity, cancel_requested, trace, [&] {
