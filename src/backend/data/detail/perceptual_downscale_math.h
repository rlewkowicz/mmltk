...
MMLTK_PERCEPTUAL_HD inline float unit(float value) {
    return !(value > 0.0F) ? 0.0F : (value < 1.0F ? value : 1.0F);
}
MMLTK_PERCEPTUAL_HD inline float decode(float v) {
    return v > 0.04045F ? ::powf((v + 0.055F) / 1.055F, 2.4F) : v / 12.92F;
}
MMLTK_PERCEPTUAL_HD inline float encode(float v) {
    return v > 0.0031308F ? 1.055F * ::powf(v, 1.0F / 2.4F) - 0.055F : 12.92F * v;
}

struct Color { float y, cb, cr; };
...
MMLTK_PERCEPTUAL_HD inline Color inverse(Color v) {
    return {v.y + 1.402F*v.cr,v.y - 0.344136F*v.cb - 0.714136F*v.cr,v.y + 1.772F*v.cb};
}
...
MMLTK_PERCEPTUAL_HD inline Footprint footprint(std::uint32_t src, std::uint32_t dst, std::uint32_t i) {
    ...
    return {first, last + 1,
            first == last ? float(double(end - begin) / dst) : float(double(dst - begin % dst) / dst),
            float(double(end - std::uint64_t(last) * dst) / dst)};
}

struct Moment { float mean[3]{}, variance[3]{}; };
struct Coefficient { float ratio[3]{}, center[3]{}; };
...
MMLTK_PERCEPTUAL_HD inline Color reconstruct(const Moment& m, const Coefficient& a, const Coefficient& b,
                                            const Coefficient& c, const Coefficient& d) {
    float v[3];
    for (int k = 0; k < 3; ++k) {
        // Centered reconstruction avoids subtracting large r*m terms.
        v[k] = (::fmaf(a.ratio[k],m.mean[k]-a.center[k],a.center[k])+
                ::fmaf(b.ratio[k],m.mean[k]-b.center[k],b.center[k])+
                ::fmaf(c.ratio[k],m.mean[k]-c.center[k],c.center[k])+
                ::fmaf(d.ratio[k],m.mean[k]-d.center[k],d.center[k]))*0.25F;
    }
    return {v[0], v[1], v[2]};
}
...
MMLTK_PERCEPTUAL_HD inline Color load(RgbConstImageView view, std::uint32_t x, std::uint32_t y,
                                      const TransferTable& table, float& alpha) {
    ...
    if constexpr (Format == RgbPixelFormat::PlanarUnitSrgbF32) {
        ...
        r = table.linear[pixel[0]]; g = table.linear[pixel[1]]; b = table.linear[pixel[2]];
...
template<RgbPixelFormat Format>
MMLTK_PERCEPTUAL_HD inline void store(RgbMutableImageView view, std::uint32_t x, std::uint32_t y,
                                     Color value, float alpha, const TransferTable& table) {
    ...
    for (int k = 0; k < 3; ++k) {
        ...
        if constexpr (Format == RgbPixelFormat::RGBA8) linear = alpha > 0.0F ? unit(linear / alpha) : 0.0F;
        else linear = unit(linear);
        if constexpr (Format == RgbPixelFormat::PlanarUnitSrgbF32) {
    ...
    if constexpr (Format == RgbPixelFormat::RGBA8) row[std::size_t(x) * 4 + 3] = static_cast<std::uint8_t>(unit(alpha) * 255.0F + 0.5F);
}
} // namespace mmltk::backend::data::perceptual
