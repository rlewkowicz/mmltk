#pragma once  // backend.data private CUDA boundary
#include <cstddef>
#include <cstdint>
#include <vector>
...
class CudaBinaryMaskResizer {
   public:
    ...
    void resize_batch(const std::vector<uint8_t>& source_masks, const std::vector<uint32_t>& source_widths, const std::vector<uint32_t>& source_heights,
                      const std::vector<size_t>& source_offsets, uint32_t target_width, uint32_t target_height, std::vector<uint8_t>& output_masks);
