...
[[nodiscard]] std::vector<ClassMembership> build_memberships(const NormalizedAnnotationIndex& source, SupplementalSamplingStats* stats,
    ...
    for (std::size_t image_index = 0U; image_index < source.images.size(); ++image_index) {
        ...
        if (membership.low == 0U && membership.high == 0U) {
            throw std::runtime_error("supplemental sampler received an image without mapped classes");
        }
...
[[nodiscard]] std::array<std::uint64_t, kClassCount> coco_class_counts(
    const NormalizedAnnotationIndex& coco_train, mmltk::common::concurrency::CancellationObservation cancel_requested) {
...
[[nodiscard]] std::vector<std::uint16_t> choose_objects365_shards(const NormalizedAnnotationIndex& objects365,
                                                                  ...
                                                                  const std::span<const std::uint64_t> shard_bytes,
                                                                  const std::uint64_t required_images,
    ...
    *selected_archive_bytes = 0U;
    ...
    while (selected_images < headroom_images ||
           !std::ranges::equal(
               selected_coverage, coverage_targets,
               [](const std::uint64_t available, const std::uint64_t target) { return available >= target; })) {
        ...
        for (std::size_t shard = 0U; shard < summaries.size(); ++shard) {
            ...
            for (std::size_t class_id = 0U; class_id < kClassCount; ++class_id) {
                const std::uint64_t deficit = coverage_targets[class_id] > selected_coverage[class_id]
                                                  ? coverage_targets[class_id] - selected_coverage[class_id]
                                                  : 0U;
                coverage_gain += std::min(deficit, summaries[shard].class_images[class_id]);
            }
            const std::uint64_t useful_images =
                selected_images < headroom_images ? std::min(summaries[shard].images, headroom_images - selected_images) : 0U;
            ...
            if (score > best_score || (score == best_score && (best == summaries.size() || bytes < shard_bytes[best] ||
                                                               (bytes == shard_bytes[best] && shard < best)))) {
                best = shard;
                best_score = score;
            }
        }
        ...
        *selected_archive_bytes =
            checked_cast<std::uint64_t>(*selected_archive_bytes + shard_bytes[best], "Objects365 selected archive byte total overflow");
        for (std::size_t class_id = 0U; class_id < kClassCount; ++class_id) {
            selected_coverage[class_id] += summaries[best].class_images[class_id];
        }
    }
    for (std::size_t position = result.size(); position > 0U; --position) {
        ...
        *selected_archive_bytes -= shard_bytes[shard];
        for (std::size_t class_id = 0U; class_id < kClassCount; ++class_id) {
            selected_coverage[class_id] -= summary.class_images[class_id];
        }
...
[[nodiscard]] SourceSelection make_source_selection(const NormalizedAnnotationIndex& source, std::vector<ClassMembership> memberships,
    ...
    for (const ClassMembership membership : result.memberships) {
        ClassMembershipCursor classes(membership);
        while (const auto class_id = classes.next()) {
            ++result.stats.available_class_images[*class_id];
        }
    }
    ...
    for (const HashedImage& image : result.hash_order) {
        ...
        while (const auto class_id = classes.next()) {
            result.candidates[*class_id].push_back(image.image_index);
        }
    }
    return result;
}
...
[[nodiscard]] bool class_candidate_available(SourceSelection* source, const std::size_t class_id) {
    ...
    while (cursor < candidates.size() && source->selected[candidates[cursor]] != 0U) {
        ++cursor;
    }
...
[[nodiscard]] std::size_t rarest_available_class(SourceSelection* objects, SourceSelection* open_images,
                                                 const std::array<std::uint64_t, kClassCount>& combined_counts, const bool open_only,
                                                 const bool open_allowed) {
...
[[nodiscard]] NormalizedAnnotationIndex materialize_selection(const SourceSelection& source) {
    ...
    for (std::size_t image_index = 0U; image_index < source.source->images.size(); ++image_index) {
        ...
        const auto first =
            source.source->boxes.begin() + checked_cast<BoxDifference>(input.first_box, "supplemental box offset exceeds iterator range");
        ...
        for (auto box = first; box != last; ++box) {
            ...
            const auto mask =
                std::span(source.source->mask_rle_pairs).subspan(static_cast<std::size_t>(box->mask_rle_offset), box->mask_rle_pairs);
...
CombinedSupplementalSamplingResult sample_combined_supplemental_indices(
    ...
    const std::span<const std::uint64_t> objects365_shard_bytes, mmltk::common::concurrency::CancellationObservation cancel_requested) {
    ...
    result.target_images =
        objects365.images.size() / kSupplementalBudgetDenominator + open_images.images.size() / kSupplementalBudgetDenominator;
    ...
    result.objects365_shards = choose_objects365_shards(objects365, object_memberships, objects365_shard_bytes, required_objects,
                                                        &result.objects365_archive_bytes, cancel_requested);
    std::vector<std::uint8_t> eligible_shards(objects365_shard_bytes.size(), std::uint8_t{0U});
    for (const std::uint16_t shard : result.objects365_shards) {
        eligible_shards[shard] = 1U;
    }
