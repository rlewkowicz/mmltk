...
ClassCatalog::ClassCatalog(std::vector<std::string> names, std::size_t name_capacity) : names_(std::move(names)) {
    if (names_.size() > kClassCatalogCapacity || name_capacity > kClassNameCapacity)
        throw std::invalid_argument("class catalog exceeds supported capacity");
    ...
    for (std::size_t index = 0; index < names_.size(); ++index) {
        ...
        if (!lookup_.emplace(name, static_cast<std::uint32_t>(index)).second)
            throw std::invalid_argument("class catalog contains duplicate name: " + name);
    }
}
ClassCatalog::ClassCatalog(const OrderedClassCatalog& record) : ClassCatalog([&] {
