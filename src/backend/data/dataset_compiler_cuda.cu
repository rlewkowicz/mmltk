...
namespace {
__global__ void resize_binary_masks_kernel(const uint8_t* input_masks, uint8_t* output_masks, const uint32_t* source_widths, const uint32_t* source_heights,
                                           const uint64_t* source_offsets, uint32_t target_width, uint32_t target_height, uint32_t batch_size,
    ...
    const uint32_t source_y =
        min(source_height - 1, static_cast<uint32_t>(((static_cast<uint64_t>(2) * y + 1) * source_height) / (static_cast<uint64_t>(2) * target_height)));
    const uint32_t source_x =
        min(source_width - 1, static_cast<uint32_t>(((static_cast<uint64_t>(2) * x + 1) * source_width) / (static_cast<uint64_t>(2) * target_width)));
...
struct CudaBinaryMaskResizer::Impl {
    ...
    std::unique_ptr<mmltk::frameworks::gpu::PinnedHostBuffer> input_storage, output_storage, widths_storage, heights_storage, offsets_storage;
...
CudaBinaryMaskResizer::CudaBinaryMaskResizer(int device_id) {
    ...
    candidate->execution = mmltk::frameworks::gpu::resolve_device_execution(
        device_id, mmltk::common::system::NumaTopology::Capture(), mmltk::common::system::bound_memory_node(mmltk::common::system::capture_memory_policy()));
...
void CudaBinaryMaskResizer::resize_batch(const std::vector<uint8_t>& source_masks, const std::vector<uint32_t>& source_widths,
                                         const std::vector<uint32_t>& source_heights, const std::vector<size_t>& source_offsets, uint32_t target_width,
    ...
    if (batch_size > static_cast<size_t>(std::numeric_limits<int>::max())) { throw std::runtime_error("CUDA mask resize batch is too large"); }
    ...
    if (target_pixels > std::numeric_limits<size_t>::max() / batch_size) { throw std::runtime_error("CUDA mask resize output buffer size overflow"); }
    ...
    auto ensure_host_buffer = [](std::unique_ptr<mmltk::frameworks::gpu::PinnedHostBuffer>& storage, void** pointer, size_t& capacity, size_t required) {
    ...
    ensure_host_buffer(impl_->widths_storage, reinterpret_cast<void**>(&impl_->host_widths), impl_->host_widths_capacity, batch_size * sizeof(uint32_t));
    ensure_host_buffer(impl_->heights_storage, reinterpret_cast<void**>(&impl_->host_heights), impl_->host_heights_capacity, batch_size * sizeof(uint32_t));
    ensure_host_buffer(impl_->offsets_storage, reinterpret_cast<void**>(&impl_->host_offsets), impl_->host_offsets_capacity, batch_size * sizeof(uint64_t));
    ensure_device_buffer(reinterpret_cast<void**>(&impl_->device_input), impl_->device_input_capacity, source_masks.size(), "cudaMalloc for mask resize input");
    ensure_device_buffer(reinterpret_cast<void**>(&impl_->device_output), impl_->device_output_capacity, output_bytes, "cudaMalloc for mask resize output");
    ...
    for (size_t index = 0; index < batch_size; ++index) { impl_->host_offsets[index] = static_cast<uint64_t>(source_offsets[index]); }
    cuda_bridge::require_cuda_success(cudaMemcpyAsync(impl_->device_input, impl_->host_input, source_masks.size(), cudaMemcpyHostToDevice, impl_->stream),
    ...
    resize_binary_masks_kernel<<<grid, block, 0, impl_->stream>>>(impl_->device_input, impl_->device_output, impl_->device_widths, impl_->device_heights,
                                                                  impl_->device_offsets, target_width, target_height, static_cast<uint32_t>(batch_size),
                                                                  static_cast<uint64_t>(target_pixels));
    cuda_bridge::require_cuda_success(cudaGetLastError(), "resize_binary_masks_kernel launch");
    cuda_bridge::require_cuda_success(cudaMemcpyAsync(impl_->host_output, impl_->device_output, output_bytes, cudaMemcpyDeviceToHost, impl_->stream),
