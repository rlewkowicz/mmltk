 ...
 for (const std::vector<double>& polygon : polygons) {
  ...
  for (std::uint32_t y = first_row; y < final_row; ++y) {
   ...
   for (std::size_t pair = 0U; pair + 1U < intersections.size(); pair += 2U) {
    ...
    std::fill(dense->data() + static_cast<std::size_t>(y) * dimensions.width + x1, dense->data() + static_cast<std::size_t>(y) * dimensions.width + x2,
              std::uint8_t{1U});
   }
  }
 }
 ...
 for (std::size_t image_index = 0U; image_index < index.images.size(); ++image_index) {
  ...
  if ((!first && image.source_image_id <= previous_id) || image.first_box != expected_first || image.first_box > index.boxes.size() ||
   ...
   throw std::runtime_error("normalized benchmark image record is invalid at index " + std::to_string(image_index) +
                            ...
                            ", first_box=" + std::to_string(image.first_box) + ", expected_first_box=" + std::to_string(expected_first) + ", box_count=" +
                            std::to_string(image.box_count) + ", width=" + std::to_string(image.width) + ", height=" + std::to_string(image.height) + ")");
  }
  first = false;
  ...
  for (std::uint64_t box_index = image.first_box; box_index < image.first_box + image.box_count; ++box_index) {
   ...
   if (box.class_id >= coco80_class_names().size() || !std::isfinite(box.x1) || !std::isfinite(box.y1) || !std::isfinite(box.x2) || !std::isfinite(box.y2) ||
       box.x2 <= box.x1 || box.y2 <= box.y1 || !std::isfinite(box.original_area) || box.original_area < 0.0 || (box.flags & ~kAnnotationFlags) != 0U ||
       (box.flags & kAnnotationCategory) == 0U || ((box.flags & kAnnotationMask) == 0U && box.mask_rle_pairs != 0U) ||
       ((box.flags & kAnnotationId) == 0U && box.annotation_id != 0U) || ((box.flags & kAnnotationCategory) == 0U && box.source_category_id != 0U) ||
 ...
 parallel_object_array(
  file, annotations_array, workers, options.cancel_requested, [&](const int worker, simdjson::ondemand::object object, std::uint64_t ordinal) {
   ...
   try {
    ...
    candidate =
     normalize_coco_box(annotation, images, image_lookup, category_lookup, &rejected, file, segmentation_parsers[static_cast<std::size_t>(worker)], scratch);
