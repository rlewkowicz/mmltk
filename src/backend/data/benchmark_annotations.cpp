...
void for_each_object(const PaddedMappedFile& file, const ByteRange range, simdjson::ondemand::parser& parser, const std::function<void(simdjson::ondemand::object, std::uint64_t)>& callback) {
 bool in_string = false;
 bool escaped = false;
 int depth = 0;
...
void validate_numeric_categories(const PaddedMappedFile& file, const ByteRange categories, const CategoryLookup& lookup, mmltk::common::concurrency::CancellationObservation cancel_requested) try {
 ...
 for (const std::vector<double>& polygon : polygons) {
  ...
  for (std::uint32_t y = first_row; y < final_row; ++y) {
   ...
   for (std::size_t pair = 0U; pair + 1U < intersections.size(); pair += 2U) {
    ...
    std::fill(dense->data() + static_cast<std::size_t>(y) * dimensions.width + x1, dense->data() + static_cast<std::size_t>(y) * dimensions.width + x2, std::uint8_t{1U});
   }
  }
 }
}
...
[[nodiscard]] std::vector<RLEPair> materialize_segmentation(
...
[[nodiscard]] NormalizedAnnotationIndex begin_normalized_index_result(
...
void compact_annotations(NormalizedAnnotationIndex* index, const std::vector<ParsedImage>& parsed_images, const std::vector<std::uint64_t>& offsets, const std::span<const std::uint64_t> accepted_ends,
 ...
 if (offsets.size() != parsed_images.size() + 1U || accepted_ends.size() != parsed_images.size()) throw std::runtime_error("accepted annotation image ranges are inconsistent");
 const auto accepted_range = [&](std::size_t image_index) {
  if ((image_index & 4095U) == 0U) { throw_if_benchmark_cancelled(cancel_requested); }
  return std::pair{checked_cast<std::size_t>(offsets[image_index], "box offset overflow"), checked_cast<std::size_t>(accepted_ends[image_index], "box offset overflow")};
 };
 ...
 for (std::size_t image_index = 0U; image_index < parsed_images.size(); ++image_index) {
  ...
  if (end < begin || end > offsets[image_index + 1U] || offsets[image_index + 1U] > boxes.size()) throw std::runtime_error("accepted annotations exceed image capacity");
...
void validate_open_images_classes(const std::filesystem::path& path, const std::span<const StringCategoryMapping> mappings) {
 ...
 for_each_csv_line(classes, ByteRange{0U, classes.size()}, {}, [&](const std::string_view line) {
  ...
  if (found != expected.end()) {
   if (fields[1] != found->second || !matched.emplace(found->first).second) { throw std::runtime_error("Open Images class metadata disagrees with its fixed mapping"); }
  }
 });
...
void validate_normalized_records(const NormalizedAnnotationIndex& index, mmltk::common::concurrency::CancellationObservation cancel_requested) {
 ...
 for (std::size_t image_index = 0U; image_index < index.images.size(); ++image_index) {
  ...
  if ((!first && image.source_image_id <= previous_id) || image.first_box != expected_first || image.first_box > index.boxes.size() || image.box_count > index.boxes.size() - image.first_box ||
  ...
  for (std::uint64_t box_index = image.first_box; box_index < image.first_box + image.box_count; ++box_index) {
   ...
   if (box.class_id >= coco80_class_names().size() || !std::isfinite(box.x1) || !std::isfinite(box.y1) || !std::isfinite(box.x2) || !std::isfinite(box.y2) || box.x2 <= box.x1 || box.y2 <= box.y1 ||
       ...
       ((box.flags & kAnnotationCategory) == 0U && box.source_category_id != 0U) || box.mask_rle_offset != expected_mask_offset || expected_mask_offset > index.mask_rle_pairs.size() ||
       box.mask_rle_pairs > index.mask_rle_pairs.size() - expected_mask_offset || !std::ranges::all_of(box.reserved, [](const std::uint8_t value) { return value == 0U; })) {
   ...
   for (std::uint64_t pair_index = box.mask_rle_offset; pair_index < box.mask_rle_offset + box.mask_rle_pairs; ++pair_index) {
    ...
    if (pair.length == 0U || pair.start < previous_end || end > mask_pixels) { throw std::runtime_error("normalized benchmark mask record is invalid"); }
    previous_end = end;
   }
...
std::size_t slice_run_count(const NormalizedAnnotationIndex& index, std::span<const NormalizedBox> boxes, mmltk::common::concurrency::CancellationObservation cancellation) {
 std::size_t count = 0;
...
void transfer_image_slice(NormalizedAnnotationIndex& destination, const NormalizedAnnotationIndex& source, std::size_t position, std::size_t image_destination, std::size_t box_destination,
...
void append_normalized_image_slice(
 NormalizedAnnotationIndex& destination, const NormalizedAnnotationIndex& source, std::size_t position, mmltk::common::concurrency::CancellationObservation cancellation) {
 throw_if_benchmark_cancelled(cancellation);
...
NormalizedAnnotationIndex parse_open_images_annotations(const std::filesystem::path& boxes_csv_path, const std::filesystem::path& classes_csv_path, std::string annotation_sha256,
 ...
 for (std::size_t index = 0U; index < merged.size(); ++index) {
  ...
  if (merged[index].count > std::numeric_limits<std::uint64_t>::max() - offsets[index]) { throw std::overflow_error("Open Images annotation offset overflow"); }
 ...
 NormalizedAnnotationIndex result = begin_normalized_index_result(BenchmarkDatasetSource::kOpenImagesV7, options.split, std::move(annotation_sha256), worker_rejected);
...
std::optional<NormalizedAnnotationIndex> load_normalized_annotation_index(const std::filesystem::path& path, const BenchmarkDatasetSource expected_source, const std::string_view expected_split,
 const std::string_view expected_annotation_sha256, mmltk::common::concurrency::CancellationObservation cancel_requested, const BenchmarkTraceSink& trace) {
 try {
  throw_if_benchmark_cancelled(cancel_requested);
  ...
  if (header.source != static_cast<std::uint8_t>(expected_source) || std::string_view(header.split.data(), ::strnlen(header.split.data(), header.split.size())) != expected_split ||
      ...
      header.annotation_sha256 != mmltk::common::io::parse_sha256_hex(std::string(expected_annotation_sha256)) || manifest.value("size", 0ULL) != file.size() ||
