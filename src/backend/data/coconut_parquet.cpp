...
template<class T> const T& as(const std::shared_ptr<arrow::Array>& array, arrow::Type::type type, std::string_view field) {
...
const arrow::StructArray& structure(const std::shared_ptr<arrow::Array>& array, std::string_view name) {
    return as<arrow::StructArray>(array, arrow::Type::STRUCT, name);
}
...
const arrow::StructType& struct_type(const std::shared_ptr<arrow::Field>& field, std::string_view name) {
    if (!field || field->type()->id() != arrow::Type::STRUCT) malformed(name);
    return static_cast<const arrow::StructType&>(*field->type());
}
...
void validate_schema(const arrow::Schema& schema) {
    ...
    field_type(mask, "bytes", arrow::Type::BINARY); field_type(mask, "path", arrow::Type::STRING);
    ...
    field_type(annotations, "file_name", arrow::Type::STRING); field_type(annotations, "image_id", arrow::Type::INT64);
    ...
    field_type(segment, "iscrowd", arrow::Type::INT64, true); field_type(segment, "ignore", arrow::Type::INT64, true);
...
void read_batch(const arrow::RecordBatch& batch, const CoconutImportLimits& limits,
    ...
    for (std::int64_t row = 0; row < batch.num_rows(); ++row) {
        ...
        present(masks, row, "mask"); present(bytes, row, "mask.bytes"); present(annotations, row, "segments_info"); present(images, row, "image_info"); present(lists, row, "segments list");
        ...
        (void)text(images, "coco_url", row); (void)text(images, "date_captured", row); (void)integer(images, "license", row);
        const auto width = integer(images, "width", row), height = integer(images, "height", row);
        if (width == 0 || height == 0 || width > limits.max_dimension || height > limits.max_dimension || width > limits.max_pixels / height || width * height > UINT32_MAX) malformed("dimensions exceed admission");
        record.width = static_cast<std::uint32_t>(width); record.height = static_cast<std::uint32_t>(height);
        record.source_ordinal = row_ordinal; record.first_segment_ordinal = segment_ordinal;
        record.segments.clear(); record.segments.reserve(static_cast<std::size_t>(length));
        for (std::int64_t offset = 0; offset < length; ++offset) {
            ...
            segment.id = static_cast<std::uint32_t>(id); segment.category_id = integer(segments, "category_id", index);
            segment.isthing = boolean(segments, "isthing", index); segment.crowd = boolean(segments, "iscrowd", index, true);
        ...
        ++row_ordinal; segment_ordinal += record.segments.size();
    }
}
}
