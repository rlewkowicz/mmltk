...
void fill_center_scale_lookup(const std::span<std::uint32_t> lookup, const std::uint32_t target_extent, const std::uint32_t source_extent,
    ...
    for (std::uint32_t index = 0U; index < target_extent; ++index) {
        lookup[index] = std::min<std::uint32_t>(
            source_extent - 1U,
            checked_cast<std::uint32_t>(((static_cast<std::uint64_t>(2U) * index + 1U) * source_extent) / target_twice, overflow_context));
    }
}
...
[[nodiscard]] std::size_t checked_pixel_count(const MaskDimensions dimensions) {
    if (dimensions.width == 0U || dimensions.height == 0U ||
        dimensions.width > std::numeric_limits<std::size_t>::max() / dimensions.height) {
...
EncodedRowMajorMask encode_dense_row_major_mask(const std::span<const std::uint8_t> dense, const MaskDimensions dimensions) {
    ...
    while (cursor < pixels) {
        ...
        while (cursor < pixels && dense[cursor] == 0U) {
            ++cursor;
        }
...
void materialize_row_major_mask(const std::span<const RLEPair> pairs, const MaskDimensions dimensions, std::vector<std::uint8_t>* dense,
    ...
    for (const RLEPair pair : pairs) {
        ...
        if (length == 0U || begin < previous_end || begin > pixels || length > pixels - begin) {
...
EncodedRowMajorMask resize_row_major_mask(const std::span<const RLEPair> pairs, const MaskDimensions source_dimensions,
                                          const MaskDimensions target_dimensions, const RgbLetterbox& letterbox, MaskResizeScratch* scratch,
                                          RowMajorMaskBounds* source_bounds) {
    if (scratch == nullptr || letterbox.resized_width == 0U || letterbox.resized_height == 0U ||
        letterbox.offset_x + letterbox.resized_width > target_dimensions.width ||
        letterbox.offset_y + letterbox.resized_height > target_dimensions.height) {
    ...
    for (std::uint32_t y = 0U; y < letterbox.resized_height; ++y) {
        ...
        std::uint8_t* target =
            scratch->target_mask.data() + static_cast<std::size_t>(letterbox.offset_y + y) * target_dimensions.width + letterbox.offset_x;
        for (std::uint32_t x = 0U; x < letterbox.resized_width; ++x) {
            target[x] = source[scratch->source_x[x]];
        }
    }
