...
[[nodiscard]] std::size_t checked_align_up(const std::size_t value, const std::size_t alignment, const char* context) {
    if (alignment == 0U || (alignment & (alignment - 1U)) != 0U) {
        throw std::invalid_argument("benchmark alignment must be a nonzero power of two");
    }
...
[[nodiscard]] std::vector<ImageEntry> build_index(const PreparedBenchmarkSplit& split, const std::uint64_t pixel_offset,
                                                  const std::uint64_t image_stride) {
    ...
    for (std::size_t image_index = 0U; image_index < split.images.size(); ++image_index) {
        ...
        if (image_offset > std::numeric_limits<std::uint64_t>::max() - pixel_offset) {
            throw std::overflow_error("benchmark pixel index offset overflow");
        }
        destination.pixel_offset = pixel_offset + image_offset;
        const std::size_t label_offset =
            checked_size_multiply(expected_first_label, sizeof(PackedInstance), "benchmark label offset overflow");
        ...
        destination.label_bytes = common_math::checked_cast<std::uint32_t>(
            static_cast<std::size_t>(source.label_count) * sizeof(PackedInstance), "benchmark label size overflow");
...
[[nodiscard]] FileHeader make_header(const PreparedBenchmarkSplit& split, const std::uint32_t resolution, const std::size_t image_stride,
    ...
    for (std::size_t class_index = 0U; class_index < split.class_names.size(); ++class_index) {
        ...
        if (name.empty() || name.size() >= header.class_names[class_index].size()) {
            throw std::runtime_error("benchmark class name is empty or too long");
        }
...
void decode_images(const BenchmarkWriteRequest& request, const common_io::FileHandle& output, const std::size_t pixel_offset,
                   const std::size_t image_stride, const std::size_t pixel_bytes) {
    const PreparedBenchmarkSplit& split = request.split;
    WritablePixelRange output_pixels(output.get(), pixel_offset, pixel_bytes);
    const int worker_count =
        std::max(1, std::min(request.num_workers, common_math::checked_cast<int>(split.images.size(), "worker count overflow")));
                ...
                for (std::uint32_t image_index = begin; image_index < end; ++image_index) {
                    ...
                    try {
                        ...
                        if (image_descriptor < 0) {
                            throw common_io::errno_error("cannot open cached benchmark image", relative_path.data());
                        }
                    ...
                    const RgbLetterbox letterbox =
                        compute_rgb_letterbox(image.source_width, image.source_height, request.resolution, request.resolution);
                    ...
                    if (letterbox.resized_width == request.resolution && letterbox.resized_height == request.resolution &&
                        ...
                        letterboxed_rgb_hwc_u8_to_nchw_f32(source_pixels, destination, letterbox.resized_width, letterbox.resized_height,
                                                           request.resolution, request.resolution, letterbox.offset_x, letterbox.offset_y);
                    }
...
PackedInstance benchmark_letterbox_box(const std::uint8_t class_id, const float x1, const float y1, const float x2, const float y2,
                                       const RgbLetterbox& letterbox) {
    if (letterbox.resized_width == 0U || letterbox.resized_height == 0U) {
        throw std::runtime_error("benchmark box requires a valid letterbox");
    }
    const auto scaled_coordinate = [](const float value, const std::uint32_t extent, const std::uint32_t offset, const bool round_up,
        ...
        return common_math::checked_cast<std::int16_t>(
            static_cast<std::int32_t>(rounded) + common_math::checked_cast<std::int32_t>(offset, "benchmark box offset overflow"),
            overflow_message);
    };
...
void write_benchmark_split(const BenchmarkWriteRequest& request) {
    ...
    const std::size_t pixel_offset = checked_align_up(checked_size_add(index_offset, index_bytes, "benchmark index end overflow"),
                                                      HUGE_PAGE_SIZE, "benchmark pixel alignment overflow");
    ...
    const std::size_t label_bytes =
        checked_size_multiply(request.split.labels.size(), sizeof(PackedInstance), "benchmark label block overflow");
    ...
    const FileHeader header =
        make_header(request.split, request.resolution, image_stride, index_offset, pixel_offset, label_offset, rle_offset, total_size);
    ...
    validate_compiled_rle_pairs(request.split.labels, request.split.rle_pairs,
                                static_cast<std::size_t>(request.resolution) * request.resolution, request.cancel_requested);

    const std::filesystem::path output_parent =
        request.output_path.parent_path().empty() ? std::filesystem::path{"."} : request.output_path.parent_path();
    ...
    if (persisted_rle_bytes != rle_bytes) {
        throw std::runtime_error("persisted benchmark labels do not reference the complete mask block");
    }
