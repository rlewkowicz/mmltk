...
struct Image {
    ...
    double at(unsigned x,unsigned y,unsigned channel) const {
        ...
        if (layout.format==RgbPixelFormat::PlanarUnitSrgbF32)
            return reinterpret_cast<const float*>(bytes+channel*layout.plane_stride_bytes)[x];
    ...
    void set(unsigned x,unsigned y,unsigned channel,double value) {
        ...
        else bytes[x*(layout.format==RgbPixelFormat::RGB8 ? 3:4)+channel]=static_cast<std::uint8_t>(std::clamp(value,0.0,1.0)*255.0+0.5);
    }
    void fill(unsigned pattern) {
        const unsigned channels=layout.format==RgbPixelFormat::RGBA8 ? 4:3;
        for (unsigned y=0;y<layout.height;++y) for (unsigned x=0;x<layout.width;++x) for (unsigned k=0;k<channels;++k) {
            double value=0;
            switch(pattern) {
                ...
                case 6: {
                    ...
                    value=transition[(x+y+k)%5];break;
                }
...
inline Image reference(const Image& source,unsigned width,unsigned height,std::size_t padding=0) {
        ...
        for (unsigned sy=static_cast<unsigned>(std::floor(y0));sy<std::ceil(y1);++sy)
            for (unsigned sx=static_cast<unsigned>(std::floor(x0));sx<std::ceil(x1);++sx) {
                const double weight=(std::min(x1,double(sx+1))-std::max(x0,double(sx)))*
                                    (std::min(y1,double(sy+1))-std::max(y0,double(sy)))/area;
                const auto color=ycc(source,sx,sy);
                for (unsigned k=0;k<3;++k) {mean[i][k]+=color[k]*weight;second[i][k]+=color[k]*color[k]*weight;}
    ...
    for (unsigned y=0;y<height;++y) for (unsigned x=0;x<width;++x) {
        ...
        for (unsigned k=0;k<3;++k) {
            ...
            for (unsigned dy=0;dy<2;++dy) for (unsigned dx=0;dx<2;++dx) {
                ...
                m+=mean[j][k]/4;lo+=mean[j][k]*mean[j][k]/4;hi+=second[j][k]/4;
            }
            patch_mean[i][k]=m;
    ...
    for (unsigned y=0;y<height;++y) for (unsigned x=0;x<width;++x) {
        ...
        for (unsigned k=0;k<3;++k) for (unsigned dy=0;dy<2;++dy) for (unsigned dx=0;dx<2;++dx) {
            const auto j=std::size_t(y>=dy ? y-dy:0)*width+(x>=dx ? x-dx:0);
            out[k]+=(patch_mean[j][k]+ratio[j][k]*(mean[i][k]-patch_mean[j][k]))/4;
        }
        const std::array<double,3> rgb{out[0]+1.402*(out[2]-0.5),
            out[0]-0.344136*(out[1]-0.5)-0.714136*(out[2]-0.5),out[0]+1.772*(out[1]-0.5)};
        for (unsigned k=0;k<3;++k) {
...
inline double maximum_error(const Image& a,const Image& b) {
    ...
    for (unsigned y=0;y<a.layout.height;++y) for (unsigned x=0;x<a.layout.width;++x) for (unsigned k=0;k<channels;++k) {
...
inline Image threshold_source(unsigned width,unsigned height,double delta) {
    ...
    for(unsigned y=0;y<height;++y) for(unsigned x=0;x<width;++x) for(unsigned k=0;k<3;++k)
        source.set(x,y,k,oetf(0.3+((x/2)%2 ? delta:0)+(x%2 ? 0.01:-0.01)));
    return source;
}
inline constexpr std::array<std::array<unsigned,4>,16> geometries{{
    {17,13,9,7},{15,9,5,3},{16,12,8,6},{23,11,23,4},{13,19,3,19},{1,23,1,7},
    {19,1,5,1},{1,1,1,1},{17,13,1,1},{35,17,17,8},{129,127,2,3},{7,5,7,5},
    {34,6,17,3},{32,6,16,3},{65537,1,2,1},{8193,3,17,1}
}};
inline constexpr std::array<RgbPixelFormat,3> formats{
    RgbPixelFormat::RGB8,RgbPixelFormat::RGBA8,RgbPixelFormat::PlanarUnitSrgbF32};
}
