...
std::expected<CompiledDataset, std::error_code> CompiledDataset::open_source(const std::filesystem::path& path,
                                                                             const std::size_t image_limit) {
    auto mapping = mmltk::common::io::MappedFile::try_open_readonly(path.string());
    if (!mapping) { return std::unexpected{mapping.error()}; }
    return open_mapped(path, std::move(*mapping), image_limit, AccessPattern::Random);
}

CompiledDataset CompiledDataset::open_mapped(const std::filesystem::path& path, mmltk::common::io::MappedFile mapping,
                                             const std::size_t image_limit, const AccessPattern access) {
    CompiledDataset store;
    store.path_ = path;
    store.mapping_ = std::move(mapping);
    if (store.mapping_.size() < sizeof(mmltk::backend::data::FileHeader)) {
        throw std::runtime_error("compiled dataset is smaller than its v7 header");
    }
    ...
    const mmltk::backend::data::CompiledFileSections sections =
        mmltk::backend::data::validate_compiled_file_sections(store.header_, store.mapping_.size());
    ...
    const std::size_t used_rle_bytes =
        mmltk::backend::data::validate_compiled_label_entries(store.labels_, store.header_, store.rle_pairs_.size_bytes());
    if (used_rle_bytes != store.rle_pairs_.size_bytes()) {
        throw std::runtime_error("compiled label metadata does not reference the complete RLE block");
    }
    if (store.header_.image_height != 0U &&
        store.header_.image_width > std::numeric_limits<std::size_t>::max() / store.header_.image_height) {
...
std::span<const mmltk::backend::data::RLEPair> CompiledDataset::instance_rle(
...
mmltk::backend::data::RgbLetterbox CompiledDataset::letterbox(const std::uint32_t compiled_index) const {
    const mmltk::backend::data::ImageEntry& entry = image_entry(compiled_index);
    return mmltk::backend::data::compute_rgb_letterbox(entry.original_width, entry.original_height, header_.image_width,
...
bool CompiledDataset::read_images(const std::span<const CompiledImageRead> reads, const std::span<std::byte> destination,
                                  const std::atomic<bool>& cancelled, const bool prefault) const {
...
bool CompiledDataset::read_images_to(const std::span<const CompiledImageRead> reads, const ImageDestination destination,
                                     const std::atomic<bool>& cancelled, const bool prefault) const {
    ...
    for (const auto& read : reads) {
        if (read.index >= image_entries_.size() || read.destination_offset > destination.capacity ||
            stride > destination.capacity - read.destination_offset)
