    ...
    iced::stream::channel(0, async move |mut output| {
        ...
        loop {
            ...
            let (socket, mut incoming) = match Socket::open(&config.websocket_url) {
                Ok(value) => value,
                Err(error) => {
                    if output.send(TransportEvent::Disconnected(error)).await.is_err() { return; }
                    gloo_timers::future::TimeoutFuture::new(250).await;
                    continue;
                }
            };
            if output.send(TransportEvent::Connected(connection.clone())).await.is_err() { return; }
            let mut pending_events = std::collections::VecDeque::with_capacity(64);
            let reason = loop {
                let has_pending = !pending_events.is_empty();
                let event = {
                    let writable = async {
                        if !has_pending { iced::futures::future::pending::<()>().await; }
                        iced::futures::future::poll_fn(|context| std::pin::Pin::new(&mut output).poll_ready(context)).await
                    }.fuse();
                ...
                if connection.is_closed() { break "browser connection closed".into(); }
                match event {
                    Some(SocketEvent::Bytes(bytes)) => {
                        ...
                        if let Err(error) = connection.observe(&record) { break error; }
                        ...
                        if let Some(event) = application_event(record) {
                            if let Err(error) = retain_event(&mut pending_events, event) { break error; }
                        }
                    }
                    Some(SocketEvent::Deliver) => {
                        if let Some(event) = pending_events.pop_front() {
                            if output.try_send(event).is_err() { break "application consumer closed".into(); }
                        }
                    }
                    Some(SocketEvent::Writable) => {},
                ...
                if socket.socket.ready_state() == web_sys::WebSocket::OPEN {
                    if let Err(error) = connection.flush(|bytes| socket.send(bytes)) { break error; }
                }
            };
            connection.close();
            drop(socket);
            ...
            while let Some(event) = pending_events.pop_front() {
                if output.send(event).await.is_err() { return; }
            }
            if output
...
fn parse_inbound(encoded: &[u8]) -> TransportEvent {
    match protocol::decode_server(encoded) {
        Ok(record) => application_event(record).unwrap_or_else(|| TransportEvent::ProtocolError("unexpected progress fixture".into())),
        Err(error) => TransportEvent::ProtocolError(format!("invalid server CBOR record: {error}")),
    }
}

#[cfg(any(target_arch = "wasm32", test))]
fn retain_event(pending: &mut std::collections::VecDeque<TransportEvent>, event: TransportEvent) -> Result<(), String> {
    if let TransportEvent::SystemEvent(next) = &event {
        if next.delivery == crate::generated::EventDelivery::LatestState {
            if let Some((index, revision)) = pending.iter().enumerate().find_map(|(index, prior)| match prior {
                TransportEvent::SystemEvent(prior) if prior.delivery == crate::generated::EventDelivery::LatestState
                    && std::mem::discriminant(&prior.event) == std::mem::discriminant(&next.event) => Some((index, prior.state_revision)),
                _ => None,
            }) {
                if next.state_revision <= revision { return Ok(()); }
    ...
    if pending.len() == 64 { return Err("essential application event continuity capacity exceeded".into()); }
...
#[cfg(any(target_arch = "wasm32", test))]
#[derive(Debug)]
enum SocketEvent { Writable, Deliver, Bytes(Vec<u8>), Closed(String) }
...
#[cfg(any(target_arch = "wasm32", test))]
impl SocketEvents {
    fn notify(state: &mut SocketIngress) { if let Some(waker) = state.waker.take() { waker.wake(); } }
    ...
    fn bytes(&self, length: usize, copy: impl FnOnce() -> Vec<u8>) -> bool {
        let mut state = self.0.lock().expect("socket ingress");
        if state.terminal.is_some() { return false; }
        if length == 0 || length > crate::generated::MAX_RECORD_WIRE_BYTES.saturating_sub(state.byte_count) || state.bytes.len() == 64 {
...
impl iced::futures::Stream for SocketEvents {
    ...
    fn poll_next(self: std::pin::Pin<&mut Self>, context: &mut std::task::Context<'_>) -> std::task::Poll<Option<Self::Item>> {
        let mut state = self.0.lock().expect("socket ingress");
        if state.terminal.is_none() && state.writable { state.writable = false; return std::task::Poll::Ready(Some(SocketEvent::Writable)); }
        if let Some(bytes) = state.bytes.pop_front() {
            state.byte_count -= bytes.len();
            return std::task::Poll::Ready(Some(SocketEvent::Bytes(bytes)));
        }
        if let Some(reason) = &state.terminal { return std::task::Poll::Ready(Some(SocketEvent::Closed(reason.clone()))); }
...
impl Socket {
    fn open(url: &str) -> Result<(Self, SocketEvents), String> {
        ...
        let socket = web_sys::WebSocket::new(url).map_err(|error| format!("WebSocket open failed: {error:?}"))?;
        socket.set_binary_type(web_sys::BinaryType::Arraybuffer);
        let receiver = SocketEvents::default();
        let opened = receiver.clone();
        let open = Closure::wrap(Box::new(move |_: web_sys::Event| { opened.writable(); }) as Box<dyn FnMut(_)>);
        ...
        let message = Closure::wrap(Box::new(move |event: web_sys::MessageEvent| {
            let accepted = if let Ok(buffer) = event.data().dyn_into::<js_sys::ArrayBuffer>() {
                messages.bytes(buffer.byte_length() as usize, || js_sys::Uint8Array::new(&buffer).to_vec())
            } else {
                messages.close("browser host sent a non-binary WebSocket message".into());
                false
            };
            if !accepted { let _ = message_socket.close_with_code_and_reason(1002, "invalid or excess ingress"); }
        }) as Box<dyn FnMut(_)>);
        let errors = receiver.clone();
        let error = Closure::wrap(Box::new(move |_: web_sys::Event| { errors.close("WebSocket failed".into()); }) as Box<dyn FnMut(_)>);
        let closed = receiver.clone();
        let close = Closure::wrap(Box::new(move |event: web_sys::CloseEvent| { closed.close(format!("WebSocket closed: {} {}", event.code(), event.reason())); }) as Box<dyn FnMut(_)>);
        ...
        Ok((Self { socket, _open: open, _message: message, _error: error, _close: close }, receiver))
    }
    fn send(&self, bytes: &[u8]) -> Result<(), String> {
        self.socket.send_with_u8_array(bytes).map_err(|error| format!("WebSocket send failed: {error:?}"))
    }
}
#[cfg(target_arch = "wasm32")]
...
mod tests {
    ...
    fn inbound_protocol_failures_are_terminal_for_the_current_peer() {
        ...
        assert!(std::pin::Pin::new(&mut ingress).poll_next(&mut context).is_pending());
        assert!(ingress.bytes(1, || vec![0xa2]));
        ingress.writable();
        assert!(matches!(std::pin::Pin::new(&mut ingress).poll_next(&mut context), std::task::Poll::Ready(Some(SocketEvent::Writable))));
        let std::task::Poll::Ready(Some(SocketEvent::Bytes(partial))) = std::pin::Pin::new(&mut ingress).poll_next(&mut context) else { panic!("queued bytes"); };
        assert!(matches!(parse_inbound(&partial), TransportEvent::ProtocolError(_)));
        assert!(ingress.bytes(crate::generated::MAX_RECORD_WIRE_BYTES, || vec![0; crate::generated::MAX_RECORD_WIRE_BYTES]));
        ...
        ingress.writable();
        assert!(matches!(std::pin::Pin::new(&mut ingress).poll_next(&mut context), std::task::Poll::Ready(Some(SocketEvent::Bytes(bytes))) if bytes.len() == crate::generated::MAX_RECORD_WIRE_BYTES));
        assert!(matches!(std::pin::Pin::new(&mut ingress).poll_next(&mut context), std::task::Poll::Ready(Some(SocketEvent::Closed(reason))) if reason.contains("capacity")));
        assert!(!ingress.bytes(1, || panic!("terminal peer cannot accept more bytes")));
        let mut records = SocketEvents::default();
        for value in 0..64 { assert!(records.bytes(1, || vec![value])); }
        assert!(!records.bytes(1, || panic!("record overflow must not allocate")));
        records.writable();
        for expected in 0..64 {
            assert!(matches!(std::pin::Pin::new(&mut records).poll_next(&mut context), std::task::Poll::Ready(Some(SocketEvent::Bytes(bytes))) if bytes == vec![expected]));
        }
        assert!(matches!(std::pin::Pin::new(&mut records).poll_next(&mut context), std::task::Poll::Ready(Some(SocketEvent::Closed(reason))) if reason.contains("capacity")));
        ...
        for expected in [1, 2] {
            assert!(matches!(std::pin::Pin::new(&mut closed).poll_next(&mut context), std::task::Poll::Ready(Some(SocketEvent::Bytes(bytes))) if bytes == vec![expected]));
        }
        for _ in 0..2 {
            assert!(matches!(std::pin::Pin::new(&mut closed).poll_next(&mut context), std::task::Poll::Ready(Some(SocketEvent::Closed(reason))) if reason == "WebSocket closed: 1001 going away"));
        }

        let snapshot = crate::generated::application_snapshot_defaults().unwrap().into_iter().find_map(|fact| {
            if let crate::generated::ApplicationSnapshot::Annotation(value) = fact.value { Some(value) } else { None }
        }).unwrap();
        let state = |revision, delivery| {
            let mut snapshot = snapshot.clone(); snapshot.revision = revision;
            TransportEvent::SystemEvent(crate::protocol::SystemEvent { delivery,
                state_revision: if delivery == crate::generated::EventDelivery::LatestState { revision } else { 0 },
                event: crate::generated::ApplicationEvent::AnnotationAnnotationChanged(crate::generated::AnnotationChanged { snapshot }),
            })
        };
        let edge = |correlation| TransportEvent::IntentReply(crate::protocol::IntentReply { correlation, result: Ok(Value::Null) });
        let mut pending = std::collections::VecDeque::with_capacity(64);
        retain_event(&mut pending, state(1, crate::generated::EventDelivery::LatestState)).unwrap();
        retain_event(&mut pending, edge(1)).unwrap();
        retain_event(&mut pending, state(2, crate::generated::EventDelivery::LatestState)).unwrap();
        retain_event(&mut pending, edge(2)).unwrap();
        for revision in 3..=130 {
            retain_event(&mut pending, state(revision, crate::generated::EventDelivery::LatestState)).unwrap();
            assert_eq!(pending.len(), 3, "blocked delivery retains only the latest state and discrete edges");
        }
        for expected in [1, 2] { assert!(matches!(pending.pop_front(), Some(TransportEvent::IntentReply(reply)) if reply.correlation == expected)); }
        assert!(matches!(pending.pop_front(), Some(TransportEvent::SystemEvent(crate::protocol::SystemEvent {
            event: crate::generated::ApplicationEvent::AnnotationAnnotationChanged(changed), ..
        })) if changed.snapshot.revision == 130));
        retain_event(&mut pending, state(11, crate::generated::EventDelivery::LatestState)).unwrap();
        ...
        for revision in [10, 11, 0] {
            ...
            if let TransportEvent::SystemEvent(crate::protocol::SystemEvent {
                event: crate::generated::ApplicationEvent::AnnotationAnnotationChanged(changed), ..
            }) = &mut older { changed.snapshot.busy = true; }
            retain_event(&mut pending, older).unwrap();
            assert_eq!(pending.len(), 2);
            assert!(matches!(pending.front(), Some(TransportEvent::SystemEvent(crate::protocol::SystemEvent {
            ...
            assert!(matches!(pending.back(), Some(TransportEvent::IntentReply(reply)) if reply.correlation == 3));
        }
        retain_event(&mut pending, state(12, crate::generated::EventDelivery::LatestState)).unwrap();
        assert!(matches!(pending.pop_front(), Some(TransportEvent::IntentReply(reply)) if reply.correlation == 3));
        assert!(matches!(pending.pop_front(), Some(TransportEvent::SystemEvent(crate::protocol::SystemEvent { state_revision: 12, .. }))));
        for correlation in 0..63 { retain_event(&mut pending, edge(correlation)).unwrap(); }
        for revision in 1..=130 {
            retain_event(&mut pending, state(revision, crate::generated::EventDelivery::LatestState)).unwrap();
            assert_eq!(pending.len(), 64);
        }
        retain_event(&mut pending, state(131, crate::generated::EventDelivery::Transient)).unwrap();
        assert_eq!(pending.len(), 64, "transient overflow is dropped");
        assert!(retain_event(&mut pending, state(132, crate::generated::EventDelivery::Critical)).is_err());
        for expected in 0..63 { assert!(matches!(pending.pop_front(), Some(TransportEvent::IntentReply(reply)) if reply.correlation == expected)); }
        assert!(matches!(pending.pop_front(), Some(TransportEvent::SystemEvent(crate::protocol::SystemEvent {
            event: crate::generated::ApplicationEvent::AnnotationAnnotationChanged(changed), ..
        })) if changed.snapshot.revision == 130));
