...
pub fn disclosure<'a, Message: 'a>(
    ...
    HeightTransition { id: widget::Id::from(id), visible, animate_resize: false, reservation: None, content: content.into() }
}

pub struct HeightTransition<'a, Message, Theme = crate::fluent_theme::Theme, Renderer = iced::Renderer> {
    id: widget::Id,
    visible: bool,
...
impl State {
    fn new(id: widget::Id) -> Self {
        Self { id, motion: animation(0.0), instant: Instant::now(), last_redraw: None,
            installed: false, visible: false, interactive: false, width: 0.0, reservation: None, reserved: 0.0 }
    }

    fn measure(&mut self, size: Size, visible: bool, reservation: Option<u64>, animate_resize: bool, now: Instant) -> f32 {
        ...
        if !self.installed || width_changed
...
impl<Message, Theme, Renderer> Widget<Message, Theme, Renderer> for HeightTransition<'_, Message, Theme, Renderer>
where Renderer: renderer::Renderer {
    ...
    fn state(&self) -> widget::tree::State { widget::tree::State::new(State::new(self.id.clone())) }
    ...
    fn layout(&mut self, tree: &mut widget::Tree, renderer: &Renderer, limits: &layout::Limits) -> layout::Node {
        let child_limits = layout::Limits::new(
            Size::new(limits.min().width, 0.0), Size::new(limits.max().width, f32::INFINITY));
        let child = self.content.as_widget_mut().layout(&mut tree.children[0], renderer, &child_limits);
        let height = tree.state.downcast_mut::<State>().measure(
            Size::new(limits.max().width, child.size().height), self.visible, self.reservation,
            self.animate_resize, Instant::now());
    ...
    fn update(&mut self, tree: &mut widget::Tree, event: &Event, layout: Layout<'_>, cursor: mouse::Cursor,
        ...
        let on_screen = Rectangle { height: bounds.height.max(1.0), ..bounds }.intersection(viewport).is_some();
        ...
        let clip = bounds.intersection(viewport).filter(|clip| clip.height > 0.0 && self.visible);
        ...
        } else if clip.is_none() && matches!(event,
        ...
        if cancel_input || (self.visible && bounds.height < layout.children().next().unwrap().bounds().height) {
    ...
    fn draw(&self, tree: &widget::Tree, renderer: &mut Renderer, theme: &Theme, style: &renderer::Style,
        ...
        let Some(clip) = layout.bounds().intersection(viewport).filter(|clip| clip.height > 0.0) else { return; };
        renderer.with_layer(clip, |renderer| self.content.as_widget().draw(&tree.children[0], renderer,
            theme, style, layout.children().next().unwrap(), clipped_cursor(cursor, clip), &clip));
    }
    ...
    fn operate(&mut self, tree: &mut widget::Tree, layout: Layout<'_>, renderer: &Renderer, operation: &mut dyn widget::Operation) {
        ...
        if self.visible && layout.bounds().height > 0.0 {
            operation.traverse(&mut |operation| self.content.as_widget_mut().operate(&mut tree.children[0],
                layout.children().next().unwrap(), renderer, &mut VisibleOperation { operation, clip: layout.bounds() }));
        }
    }
    ...
    fn mouse_interaction(&self, tree: &widget::Tree, layout: Layout<'_>, cursor: mouse::Cursor,
        ...
        let Some(clip) = layout.bounds().intersection(viewport).filter(|clip| self.visible && cursor.is_over(*clip)) else {
            return mouse::Interaction::None;
        };
        self.content.as_widget().mouse_interaction(&tree.children[0], layout.children().next().unwrap(), cursor, &clip, renderer)
    }
    ...
    fn overlay<'b>(&'b mut self, tree: &'b mut widget::Tree, layout: Layout<'b>, renderer: &Renderer,
        ...
        if !self.visible || layout.bounds().height < child.bounds().height
            || layout.bounds().intersection(viewport).is_none() { return None; }
        self.content.as_widget_mut().overlay(&mut tree.children[0], child, renderer, viewport, translation)
    }
}

impl<'a, Message: 'a, Theme: 'a, Renderer: renderer::Renderer + 'a> From<HeightTransition<'a, Message, Theme, Renderer>>
    for Element<'a, Message, Theme, Renderer> {
    fn from(value: HeightTransition<'a, Message, Theme, Renderer>) -> Self { Element::new(value) }
}
...
fn clipped_cursor(cursor: mouse::Cursor, clip: Rectangle) -> mouse::Cursor {
    if cursor.is_over(clip) { cursor } else { mouse::Cursor::Unavailable }
}
...
impl widget::Operation for UnfocusClipped {
    ...
    fn focusable(&mut self, _id: Option<&widget::Id>, bounds: Rectangle, state: &mut dyn widget::operation::Focusable) {
        if !self.0.is_some_and(|clip| contains(clip, bounds)) { state.unfocus(); }
    }
}
...
struct VisibleOperation<'a> { operation: &'a mut dyn widget::Operation, clip: Rectangle }
impl widget::Operation for VisibleOperation<'_> {
    fn traverse(&mut self, operate: &mut dyn FnMut(&mut dyn widget::Operation)) {
        let clip = self.clip;
        self.operation.traverse(&mut |operation| operate(&mut VisibleOperation { operation, clip }));
    }
    fn container(&mut self, id: Option<&widget::Id>, bounds: Rectangle) {
        if let Some(bounds) = self.clip.intersection(&bounds) { self.operation.container(id, bounds); }
    }
    fn focusable(&mut self, id: Option<&widget::Id>, bounds: Rectangle, state: &mut dyn widget::operation::Focusable) {
        if contains(self.clip, bounds) { self.operation.focusable(id, bounds, state); } else { state.unfocus(); }
    }
    fn text_input(&mut self, id: Option<&widget::Id>, bounds: Rectangle, state: &mut dyn widget::operation::TextInput) {
        if contains(self.clip, bounds) { self.operation.text_input(id, bounds, state); }
    }
    fn text(&mut self, id: Option<&widget::Id>, bounds: Rectangle, text: &str) {
        if let Some(bounds) = self.clip.intersection(&bounds) { self.operation.text(id, bounds, text); }
    }
    fn scrollable(&mut self, id: Option<&widget::Id>, bounds: Rectangle, content: Rectangle, translation: Vector,
        state: &mut dyn widget::operation::Scrollable) {
        if contains(self.clip, bounds) { self.operation.scrollable(id, bounds, content, translation, state); }
    }
    fn custom(&mut self, id: Option<&widget::Id>, bounds: Rectangle, state: &mut dyn std::any::Any) {
        if contains(self.clip, bounds) { self.operation.custom(id, bounds, state); }
    }
}
