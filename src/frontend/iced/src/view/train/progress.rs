...
impl Presentation {
    fn from_snapshot(snapshot: &TrainingSnapshot) -> Option<Self> {
        ...
        let current = snapshot
            .metrics
            .as_ref()
            .filter(|_| snapshot.local.progress.sequence != 0);
        let mut result = Self {
            heading: String::new(),
            images: None,
            fraction: None,
        ...
        result.heading = format!(
            "{phase} · Epoch {} / {}",
            (p.epoch + 1).min(p.totalepochs).max(1),
            p.totalepochs
        );
        ...
        let measured_rate = observed
            && p.completedimages > 0
            && p.elapsedseconds.is_finite()
            && p.elapsedseconds > 0.0
            && p.imagespersecond.is_finite()
            && p.imagespersecond > 0.0;
        let eta = if measured_rate && p.totalimages > 0 {
            duration(p.totalimages.saturating_sub(p.completedimages) as f64 / p.imagespersecond)
        } else {
            "Unavailable".into()
        };
        result.timing = Some(format!(
            "Elapsed {} · Epoch ETA {eta}",
        ...
        if let Some(total) = p.scalars.total.filter(|_| training) {
            ...
            result.losses = Some(format!(
                "Classification {} · Box {} · Total {}",
                value(p.classloss),
                value(p.boxloss),
                value(total)
            ));
...
pub(super) fn view(model: &ApplicationModel) -> Element<'_, super::Message> {
    let Some(facts) = model
        ...
        .training
        .as_ref()
        .and_then(Presentation::from_snapshot)
    else {
        return column![].into();
    };
    let mut content = column![text(facts.heading)].spacing(crate::view::workflow::FIELD_SPACING);
    if let Some(images) = facts.images {
        content = content.push(text(images));
    }
    if let Some(fraction) = facts.fraction {
        content =
            content.push(container(progress_bar(0.0..=1.0, fraction)).id("train.progress.bar"));
    }
    for label in [facts.timing, facts.rate, facts.losses]
        .into_iter()
        .flatten()
    {
        content = content.push(text(label));
    }
    container(crate::view::shared::card("Training progress", "", content))
        .id("train.card.progress")
        .into()
}
#[cfg(test)]
mod tests {
    ...
    fn preparation_and_inactive_states_do_not_expose_old_measurements() {
        ...
        assert_eq!(
            facts.images.as_deref(),
            Some("Image progress unavailable in this phase")
        );
        for outcome in [
            ComputeOperationOutcome::Idle,
    ...
    fn accumulated_training_batches_do_not_invent_optimizer_measurements() {
        ...
        assert_eq!(
            facts.timing.as_deref(),
            Some("Elapsed 0:00:05 · Epoch ETA Unavailable")
        );
    }
    ...
    fn measured_progress_and_phase_changes_use_only_current_phase_facts() {
        ...
        assert_eq!(
            facts.timing.as_deref(),
            Some("Elapsed 0:00:05 · Epoch ETA 0:00:18")
        );
        assert_eq!(
            facts.losses.as_deref(),
            Some("Classification 0.2500 · Box 0.5000 · Total 1.0000")
        );
        for phase in [
            TrainingPhase::Validate,
            TrainingPhase::EpochComplete,
            ...
            assert_eq!(
                facts.rate.as_deref(),
                Some("Rate unavailable in this phase")
            );
            assert_eq!(
                facts.timing.as_deref(),
                Some("Elapsed 0:00:05 · Epoch ETA Unavailable")
    ...
    fn invalid_or_unknown_measurements_do_not_produce_a_rate_or_eta() {
        for (rate, elapsed) in [
            ...
            assert!(
                facts
                    ...
                    .as_ref()
                    .unwrap()
                    .ends_with("Epoch ETA Unavailable")
            );
        }
        let mut snapshot = snapshot();
        let p = &mut snapshot.metrics.as_mut().unwrap().progress;
        ...
        assert_eq!(
            facts.images.as_deref(),
            Some("4 images · image total unavailable")
        );
        ...
        assert_eq!(
            facts.losses.as_deref(),
            Some("Classification Unavailable · Box 0.5000 · Total Unavailable")
        );
