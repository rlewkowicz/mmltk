...
use crate::generated::{
    ...
    DatasetCompileActivity, DatasetCompilePhase, DatasetCompileTrack,
};
use iced::Fill;
use iced::widget::{Space, column, container, progress_bar, text};
...
pub(crate) const TRACK_IDS: [&str; 3] = [
    "train.dataset.progress.acquisition",
    "train.dataset.progress.labels",
    "train.dataset.progress.pixels",
];
...
fn size(bytes: u64) -> String {
    ...
    let rounded = (u128::from(bytes) * precision + u128::from(divisor) / 2) / u128::from(divisor);
...
fn track_caption(track: &DatasetCompileTrack, bytes: bool) -> String {
    ...
    let mut caption = format!(
        ...
        quantity(
            track.completed,
            track.totalknown.then_some(track.total),
            track.complete,
            bytes
        )
    );
    if track.invalidated != 0 {
        caption.push_str(&format!(
            " · {} invalidated by repair",
            count(track.invalidated)
        ));
    }
    caption
}
...
fn track_ratio(track: &DatasetCompileTrack) -> Option<f32> {
    (track.totalknown && track.total != 0 && track.activity != DatasetCompileActivity::Unnecessary)
        .then(|| (track.completed as f64 / track.total as f64) as f32)
}

fn track<Message: 'static>(
    id: &'static str,
    name: &'static str,
    fact: &DatasetCompileTrack,
    bytes: bool,
) -> Element<'static, Message> {
    let bar: Element<'static, Message> = match track_ratio(fact) {
        Some(ratio) => progress_bar(0.0..=1.0, ratio).girth(4).into(),
...
fn source_identity(source: BenchmarkDatasetSource) -> (&'static str, &'static str) {
    match source {
        BenchmarkDatasetSource::KCoco2017 => {
            ("COCO 2017", "train.dataset.progress.source.coco2017")
        }
...
fn source_heading(source: &BenchmarkSourceProgress) -> String {
    ...
    } else if source.activity.is_empty() {
        "Waiting"
    } else {
        "Active"
    };
...
fn source_summary(source: &BenchmarkSourceProgress) -> String {
    ...
    format!(
        ...
        quantity(
            ...
            bytes_known.then_some(source.totalbytes),
            source.complete,
            true
        ),
        quantity(
            source.completedimages,
            images_known.then_some(source.totalimages),
            source.complete,
            false
        )
    )
...
fn source_details(source: &BenchmarkSourceProgress) -> String {
    ...
    if let Some(transfer) = transfer {
        let mut current = quantity(
            transfer.completedbytes,
            (transfer.totalbytes != 0).then_some(transfer.totalbytes),
            false,
            true,
        );
        if transfer.cachehit {
            current.push_str(" reused");
        }
        if transfer.attempt != 0 && !transfer.cachehit {
            current.push_str(&format!(
                " · attempt {}",
                count(u64::from(transfer.attempt))
            ));
        }
        if transfer.resumed {
    ...
    if source.retrycount != 0 {
        details.push(format!("{} retries", count(source.retrycount)));
    }
    ...
    if source.invalidatedimages != 0 {
        details.push(format!(
            "{} images invalidated",
            count(source.invalidatedimages)
        ));
    }
    details.join(" · ")
}
...
fn work_is_bytes(phase: DatasetCompilePhase) -> bool {
    matches!(
        phase,
        DatasetCompilePhase::Downloading | DatasetCompilePhase::Extracting
    )
}
...
fn metrics(progress: &crate::generated::ArtifactProgress) -> String {
    let rate = if work_is_bytes(progress.phase) {
        size
    } else {
        count
    };
    [
        (progress.elapsedseconds != 0)
            .then(|| format!("{}s elapsed", count(progress.elapsedseconds))),
        (progress.remainingseconds != 0)
            .then(|| format!("{}s remaining", count(progress.remainingseconds))),
        (progress.throughputpersecond != 0)
            .then(|| format!("{}/s", rate(progress.throughputpersecond))),
        (progress.projectedoutputbytes != 0)
            .then(|| format!("{} projected", size(progress.projectedoutputbytes))),
        (progress.droppedinstances != 0)
            .then(|| format!("{} instances dropped", count(progress.droppedinstances))),
        (progress.quarantinedimages != 0)
            .then(|| format!("{} images quarantined", count(progress.quarantinedimages))),
    ]
    .into_iter()
    .flatten()
    .collect::<Vec<_>>()
    ...
    if let Some(state) = state {
        ...
        if show_tracks(state) {
            ...
            for source in &progress.sources {
                ...
                body = body.push(
                    container(row)
                        .id(source_identity(source.source).1)
                        .width(Fill),
                );
            }
