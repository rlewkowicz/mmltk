...
impl<Message> Action<Message> {
    fn new(page: FeatureId, active: bool, start: Option<Message>, stop: Option<Message>) -> Self {
        let capability = if page == FeatureId::Annotate {
            Capability::SaveOnly
        } else {
...
pub fn view<'a, Message: Clone + 'a>(
    ...
    let action = button(label)
        ...
        .style(if presentation.danger {
            crate::fluent_theme::button_workflow_stop
        } else {
            crate::fluent_theme::button_workflow_primary
        })
        .padding(0)
        .width(Fill)
        .height(Fill);
    let decorated = Element::new(Decorated {
        child: action.into(),
        active,
        page,
    });
...
impl<Message> Widget<Message, Theme, iced::Renderer> for Decorated<'_, Message> {
    fn tag(&self) -> widget::tree::Tag {
        widget::tree::Tag::of::<Animation>()
    }
    fn state(&self) -> widget::tree::State {
        widget::tree::State::new(Animation::default())
    }
    ...
    fn update(
        ...
        if let Event::Window(iced::window::Event::RedrawRequested(now)) = event {
            ...
            if tree
                ...
                .redraw(self.active, visible, *now)
            {
    ...
    fn draw(
        ...
        if !self.active && !reporting {
            return;
        }
        let state = tree.state.downcast_ref::<Animation>();
        let Some(iced::Background::Color(blue)) =
            crate::fluent_theme::button_primary(theme, iced::widget::button::Status::Active)
                .background
        else {
            return;
        };
        let Some(_clip) = layout
            .bounds()
            .intersection(viewport)
        ...
        if reporting {
            ...
            crate::integration_control::primary_action_draw(
                Composition::new(self.page, 0.0).stable_id(Region::PrimaryAction),
                label(self.page, self.active),
                self.active,
                theme.is_dark(),
                state.phase,
                layout.bounds(),
        ...
        let shader = iced::widget::shader(border::Border {
            phase: state.phase,
            blue,
            resources: state.resources.clone(),
        });
        <_ as Widget<Message, Theme, iced::Renderer>>::draw(
            &shader, tree, renderer, theme, style, layout, cursor, viewport,
        );
    }
}
...
mod tests {
    ...
    fn exact_labels_and_save_only_capability() {
        for (page, idle, active) in [
            ...
            assert_eq!(
                active_action.on_press,
                (page != FeatureId::Annotate).then_some(2)
            );
            assert!(
                Action::new(page, true, Some(1), None::<i32>)
                    .on_press
                    .is_none()
            );
        }
