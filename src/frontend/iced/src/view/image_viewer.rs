...
pub fn controls<'a, Message: Clone + 'a>(
    fit: Message,
    close: Message,
    fit_id: &'static str,
    close_id: &'static str,
    ...
    row![
        container(button("Fit").on_press(fit)).id(fit_id),
        text("Wheel to zoom · right-drag to pan").size(12),
        space::horizontal(),
        container(button("×").on_press(close)).id(close_id)
    ]
    .spacing(7)
    .into()
}
