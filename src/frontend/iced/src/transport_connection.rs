...
pub struct Connection {
    ...
    input: Arc<Mutex<crate::annotation_input::AnnotationInput>>,
    integration: bool,
}

impl Connection {
    pub(crate) fn new(outbound: mpsc::Sender<OutboundRecord>) -> Self {
        Self { outbound, integration: false, input: Arc::new(Mutex::new(crate::annotation_input::AnnotationInput::default())) }
    }

    #[cfg(target_arch = "wasm32")]
    pub(crate) fn enable_integration(&mut self, enabled: bool) { self.integration = enabled; }
    pub(crate) fn is_closed(&self) -> bool { self.outbound.is_closed() || self.input.lock().expect("input owner").is_closed() }
    ...
    pub fn send_annotation_pointer(
        ...
        let accepted = self.input.lock().expect("input owner").pointer(pointer, document_epoch);
        if accepted.is_err() {
            self.close();
    ...
    pub async fn writable(self) -> Result<(), OutboundSendError> {
        self.require_open()?;
        iced::futures::future::poll_fn(|context| self.input.lock().expect("input owner").poll_writable(context)).await
    }
    pub(crate) fn observe(&self, record: &crate::protocol::ServerRecord) -> Result<(), String> {
        self.input.lock().expect("input owner").observe(record)
    }
    pub(crate) fn flush(&self, send: impl FnMut(&[u8]) -> Result<(), String>) -> Result<(), String> {
        self.input.lock().expect("input owner").flush(send)
    }
    pub(crate) fn close(&self) {
        self.input.lock().expect("input owner").close();
    ...
    fn send(&mut self, record: OutboundRecord, transient: bool) -> Result<SendDisposition, OutboundSendError> {
        self.require_open()?;
        if !self.input.lock().expect("input owner").record(record).map_err(|_| OutboundSendError::Allocation)? {
