...
fn primary_action_measurement_uses_nested_scroll_coordinates_and_outer_frame() {
    ...
    for (horizontal_offset, vertical_offset, visible) in [
        ...
        find.scrollable(
            Some(&Id::from(crate::view::HORIZONTAL_SCROLL_ID)),
            horizontal,
            horizontal,
            Vector::new(horizontal_offset, 0.0),
            &mut Scroll,
        );
        find.traverse(&mut |operation| {
            operation.scrollable(
                Some(&Id::from(crate::view::PAGE_SCROLL_ID)),
                page,
                page,
                Vector::new(0.0, vertical_offset),
                &mut Scroll,
            );
            operation.traverse(&mut |operation| operation.container(Some(&id), outer));
        });
        let Outcome::Some(measured) = find.finish() else {
            panic!("measurement missing")
        };
        assert_eq!(
            measured.target,
            Rectangle {
                x: outer.x - horizontal_offset,
                y: outer.y - vertical_offset,
                ..outer
            }
        );
        ...
        let inner = Rectangle {
            x: measured.target.x + 1.0,
            y: measured.target.y + 1.0,
            width: measured.target.width - 2.0,
            height: measured.target.height - 2.0,
        };
        ...
        assert_eq!(
            measured
                .page
                .intersection(&measured.horizontal)
                .is_some_and(|viewport| contains_rectangle(viewport, inner)),
            visible
        );
