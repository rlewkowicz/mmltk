...
impl Default for State {
    fn default() -> Self {
        Self {
            presentation: super::dataset_presentation::State::default(),
        ...
        match driver.phase.clone() {
            ...
            Phase::BenchmarkChoice(index) => {
                if super::reporting_enabled() {
                    if self.benchmark_prepared != Some(index) {
                        self.benchmark_prepared = Some(index);
                        let reveal = match index {
                            2 => "train.dataset.validation.stock.description",
                            3 => "train.dataset.validation.coconut_stock.description",
            ...
            Phase::DatasetFixture(index) => {
                ...
                if index + 1 < super::dataset_presentation::FIXTURE_COUNT {
                    ...
                    self.fixtures_complete = true;
                    self.presentation.fixture = None;
                    driver.advance_to(Phase::DatasetDisclosure(0))
                }
            }
