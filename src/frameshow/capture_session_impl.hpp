...
struct CaptureSession::Impl {
  ...
  struct InferenceSlotRuntime {
    ...
    std::atomic<std::uint32_t> state{
        capture_internal::ToStateValue(capture_internal::InferenceState::kFree)};
  ...
  struct PreviewSlotRuntime {
    ...
    std::atomic<std::uint32_t> state{
        capture_internal::ToStateValue(capture_internal::PreviewState::kFree)};
    std::uint64_t frame_id = 0;
    std::uint64_t capture_ns = 0;
    std::uint64_t ready_ns = 0;
    bool short_frame = false;
 ...
 private:
  ...
  Status ScheduleInferenceCopy(HostSlotRuntime& host_slot,
