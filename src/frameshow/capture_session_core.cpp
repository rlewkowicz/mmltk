...
Status CaptureSession::Impl::start() {
  ...
  packed_region_.store(PackRegion(NormalizeRegion(config.initial_region)),
...
Status CaptureSession::Impl::stop() {
  ...
  if (!was_running && fd_ == -1 && host_slots_.empty() && inference_slots_.empty() &&
      preview_slots_.empty()) {
    return Status::Ok();
  }
  ...
  return was_running ? Status::Ok()
                     : MakeStatus(StatusCode::kNotRunning,
                                  "capture session was not running");
}
...
Status CaptureSession::Impl::mark_preview_displayed(std::uint32_t buffer_index,
                                                    cudaStream_t stream) {
...
CaptureStats CaptureSession::Impl::snapshot_stats() const {
  ...
  stats.inference_frames_published =
      inference_frames_published_.load(std::memory_order_acquire);
  ...
  stats.inference_backpressure_drops =
      inference_backpressure_drops_.load(std::memory_order_acquire);
  stats.preview_backpressure_drops =
      preview_backpressure_drops_.load(std::memory_order_acquire);
...
Status CaptureSession::Impl::ValidateConfig() const {
  if (config.width == 0U || config.height == 0U) {
    return MakeStatus(StatusCode::kInvalidArgument,
                      "width and height must be non-zero");
  }
  if (config.width > kPackedRegionFieldLimit ||
      config.height > kPackedRegionFieldLimit) {
    return MakeStatus(StatusCode::kUnsupported,
                      "dimensions above 65535 are not supported");
  }
  if (config.fps == 0U) {
  ...
  if (config.v4l2_buffer_count == 0U) {
    return MakeStatus(StatusCode::kInvalidArgument,
                      "v4l2_buffer_count must be non-zero");
  }
  return Status::Ok();
}
...
CaptureRegion CaptureSession::Impl::NormalizeRegion(CaptureRegion region) const {
  ...
  region.height =
      region.height == 0U ? max_height : std::min(region.height, max_height);
  return region;
}
...
Status CaptureSession::start() { return impl_->start(); }
Status CaptureSession::stop() { return impl_->stop(); }
...
Status CaptureSession::mark_preview_displayed(std::uint32_t buffer_index,
                                              cudaStream_t stream) {
...
CaptureStats CaptureSession::snapshot_stats() const { return impl_->snapshot_stats(); }

std::string CaptureSession::last_error() const { return impl_->last_error(); }
