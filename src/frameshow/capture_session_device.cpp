...
Status CaptureSession::Impl::ConfigureCaptureFormat() {
  ...
  if (format.fmt.pix.pixelformat != kBgr3V4l2PixelFormat) {
    return MakeStatus(StatusCode::kUnsupported,
                      "driver negotiated a different pixel format than requested BGR3");
  }

  config.width = format.fmt.pix.width;
  config.height = format.fmt.pix.height;
  const std::size_t minimum_line_bytes =
      static_cast<std::size_t>(config.width) * kBgr3BytesPerPixel;
  bytes_per_line_ = std::max<std::size_t>(format.fmt.pix.bytesperline, minimum_line_bytes);
  size_image_ = std::max<std::size_t>(
      format.fmt.pix.sizeimage, bytes_per_line_ * static_cast<std::size_t>(config.height));
  return Status::Ok();
}
...
Status CaptureSession::Impl::AllocateInferenceSlots() {
  ...
  const std::size_t frame_width_bytes =
      static_cast<std::size_t>(config.width) * kBgr3BytesPerPixel;
  for (std::uint32_t slot_index = 0; slot_index < actual_v4l2_buffer_count_; ++slot_index) {
...
void CaptureSession::Impl::DestroyInferenceSlots() {
  for (auto& slot : inference_slots_) {
...
Status CaptureSession::Impl::AllocatePreviewSlots() {
  ...
  const std::size_t preview_width_bytes =
      static_cast<std::size_t>(config.width) * kBgr3BytesPerPixel;
...
Status CaptureSession::Impl::QueueV4l2Buffer(std::uint32_t slot_index,
                                             bool record_requeue_failure) {
  v4l2_buffer buf{};
  buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
  buf.memory = V4L2_MEMORY_USERPTR;
  buf.index = slot_index;
  buf.m.userptr = reinterpret_cast<unsigned long>(host_slots_[slot_index]->capture_buffer.data);
  buf.length = host_slots_[slot_index]->capture_buffer.bytes;
