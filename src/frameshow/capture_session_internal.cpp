...
Status AllocateHostBuffer(std::size_t bytes, bool pinned, HostBuffer* out) {
    ...
    if (::posix_memalign(&memory, page_size, aligned_bytes) != 0) {
        return MakeStatus(StatusCode::kInternalError, "posix_memalign failed for capture buffer");
    }
    ...
    const cudaError_t cuda_status = cudaHostRegister(memory, aligned_bytes, cudaHostRegisterDefault);
