...
std::optional<std::uint32_t> CaptureSession::Impl::ResolveHostSlotIndex(
    const v4l2_buffer& buf) const {
...
std::optional<std::uint32_t> CaptureSession::Impl::ReserveInferenceSlot() {
  ...
  if (latest_index >= 0 && latest_index < static_cast<int>(inference_slots_.size())) {
    InferenceSlotRuntime& latest_slot =
        *inference_slots_[static_cast<std::size_t>(latest_index)];
    if (latest_slot.state.load(std::memory_order_acquire) ==
        ToStateValue(InferenceState::kPublished)) {
      ...
      if (latest_query == cudaSuccess) {
        ...
        if (latest_slot.state.compare_exchange_strong(
                expected, ToStateValue(InferenceState::kWriting),
...
std::optional<std::uint32_t> CaptureSession::Impl::ReservePreviewSlot() {
  ...
  for (std::size_t attempt = 0; attempt < preview_slots_.size(); ++attempt) {
    ...
    if (state == ToStateValue(PreviewState::kFree) ||
        (state == ToStateValue(PreviewState::kPublished) &&
         latest_index != static_cast<int>(candidate))) {
...
void CaptureSession::Impl::HandleDequeuedBuffer(const v4l2_buffer& buf) {
  ...
  const Status status =
      ScheduleInferenceCopy(host_slot, inference_slot, region, capture_ns, short_frame);
  if (!status.ok()) {
    frames_dropped_.fetch_add(1, std::memory_order_relaxed);
...
void CaptureSession::Impl::ZeroFillShortFrame(HostSlotRuntime& host_slot,
...
Status CaptureSession::Impl::ScheduleInferenceCopy(HostSlotRuntime& host_slot,
                                                   InferenceSlotRuntime& inference_slot,
  ...
  const std::size_t copy_width_bytes =
      static_cast<std::size_t>(full_region.width) * kBgr3BytesPerPixel;
  ...
  const cudaError_t ready_status =
      cudaEventRecord(inference_slot.ready_event, inference_slot.copy_stream);
  ...
  const cudaError_t callback_status = cudaLaunchHostFunc(
      inference_slot.copy_stream, RequeueCapturedBufferThunk, &host_slot.requeue_context);
  ...
  latest_inference_index_.store(static_cast<int>(inference_slot.slot_index),
                                std::memory_order_release);
  inference_frames_published_.fetch_add(1, std::memory_order_relaxed);
  return Status::Ok();
}

Status CaptureSession::Impl::TrySchedulePreviewTap(
    const InferenceSlotRuntime& inference_slot) {
  if (preview_slots_.empty()) {
    return Status::Ok();
  }
  ...
  const cudaError_t wait_status =
      cudaStreamWaitEvent(preview_slot.copy_stream, inference_slot.ready_event, 0);
  ...
  const cudaError_t ready_status =
      cudaEventRecord(preview_slot.ready_event, preview_slot.copy_stream);
  if (ready_status != cudaSuccess) {
    preview_slot.state.store(ToStateValue(PreviewState::kFree), std::memory_order_release);
    return MakeCudaStatus(ready_status, "cudaEventRecord");
  }
  ...
  latest_preview_index_.store(static_cast<int>(preview_slot.slot_index),
                              std::memory_order_release);
