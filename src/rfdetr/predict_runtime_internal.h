...
struct StagedPredictionRecord {
    ...
    EvaluationCudaTimingPool* timing_pool = nullptr;
    EvaluationCudaTimingLease timing;
};
...
inline StagedPredictionRecord make_staged_prediction_record(int64_t dataset_index, int64_t image_id,
    ...
    metadata.push_back(PredictionBatchMetadata{dataset_index, image_id, std::move(source_name)});
    staged.staged = stage_prediction_batch(std::move(metadata), postprocessed_batch_from_result(result), category_count,
                                           max_dets_per_image, std::move(lease), device_id, stream_handle);
    return staged;
}

inline StagedPredictionRecord make_staged_prediction_batch(std::vector<PredictionBatchMetadata> metadata,
                                                           PostprocessedBatch batch, std::size_t category_count,
                                                           std::size_t max_dets_per_image, PredictionBufferLease lease,
                                                           int device_id, void* stream_handle) {
    StagedPredictionRecord staged;
    staged.staged = stage_prediction_batch(std::move(metadata), std::move(batch), category_count, max_dets_per_image,
...
inline std::vector<int> load_image_ids(const std::filesystem::path& compiled_path) {
    return CocoDataset::load_from_binary(compiled_path, EvaluationMetricSet::BBox).image_ids();
}
...
inline CompiledPredictionSetup make_compiled_prediction_setup(const PredictOptions& options,
    ...
    RuntimeContext runtime{resolve_runtime_config(options.workers, runtime_lanes,
                                                  kDefaultCompiledPredictionPrefetchFactor, options.cpu_affinity)};
    auto loader = std::make_unique<DatasetLoader>(validate_detail::make_inference_loader_config(
        ...
        options.device_id, kDefaultCompiledPredictionPrefetchFactor, runtime.split().gather_threads));
...
struct PendingPredictionRecordBatch {
    PendingPredictionBatchEncoding encoding;
...
inline PendingPredictionRecordBatch enqueue_prediction_record(WorkerPool& cpu_pool, StagedPredictionRecord&& staged,
                                                              const float threshold,
                                                              CocoDataset* evaluation_dataset = nullptr) {
    return PendingPredictionRecordBatch{
        enqueue_prediction_batch_encoding(cpu_pool, std::move(staged.staged), staged.evaluation_profile,
                                          evaluation_dataset),
        threshold,
...
inline void drain_staged_prediction_records(std::deque<std::future<StagedPredictionRecord>>& lane_futures,
                                            std::deque<PendingPredictionRecordBatch>& cpu_futures, WorkerPool& cpu_pool,
                                            const float threshold, CocoDataset* evaluation_dataset = nullptr) {
    StagedPredictionRecord staged = lane_futures.front().get();
    lane_futures.pop_front();
    cpu_futures.push_back(enqueue_prediction_record(cpu_pool, std::move(staged), threshold, evaluation_dataset));
}

inline void drain_prediction_records(std::deque<PendingPredictionRecordBatch>& cpu_futures, PredictionRunResult& result,
                                     std::unique_ptr<spdmon::ProgressBar>& bar) {
    EvaluationProfileRecord* evaluation_profile = cpu_futures.front().evaluation_profile;
    EvaluationCudaTimingPool* timing_pool = cpu_futures.front().timing_pool;
    EvaluationCudaTimingLease timing = cpu_futures.front().timing;
    PendingPredictionRecordBatch pending = std::move(cpu_futures.front());
    cpu_futures.pop_front();
    std::vector<PredictionBatchItem> completed = collect_prediction_batch_encoding(std::move(pending.encoding));
    if (evaluation_profile != nullptr && timing_pool != nullptr && timing) {
        timing.timing->accumulate(*evaluation_profile);
        timing_pool->release(timing);
    }
    if (pending.evaluation_dataset != nullptr) {
        for (PredictionBatchItem& image : completed) {
            if (!image.evaluation_matches) {
                throw std::logic_error("evaluation prediction task omitted compact matches");
            }
            pending.evaluation_dataset->merge_matches(std::move(*image.evaluation_matches));
        }
    } else {
        for (PredictionBatchItem& image : completed) {
            result.records.push_back(
                make_prediction_record(image.dataset_index, image.image_id, std::move(image.source_name),
                                       filter_threshold(std::move(image.predictions), pending.threshold)));
        }
    }
    result.processed_images += completed.size();
    if (bar) {
...
inline void maybe_drain_prediction_work(std::deque<std::future<StagedPredictionRecord>>& lane_futures,
    ...
    if (static_cast<int>(lane_futures.size()) >= split.lane_threads) {
        drain_staged_prediction_records(lane_futures, cpu_futures, cpu_pool, threshold, evaluation_dataset);
    }
...
inline void run_parallel_prediction_pipeline(PredictionRunResult& result, std::unique_ptr<spdmon::ProgressBar>& bar,
    ...
    std::deque<PendingPredictionRecordBatch> cpu_futures;
    size_t item_index = 0;
    ...
    flush_prediction_work(lane_futures, cpu_futures, cpu_pool, result, bar, threshold, split, evaluation_dataset);
}
...
inline void run_sequential_prediction_pipeline(PredictionRunResult& result, std::unique_ptr<spdmon::ProgressBar>& bar,
                                               WorkerPool& cpu_pool, const size_t max_cpu_futures, float threshold,
                                               ItemGeneratorFn&& generator, ProcessFn&& process_fn,
                                               CocoDataset* evaluation_dataset = nullptr) {
    if (max_cpu_futures == 0) {
        throw std::invalid_argument("sequential prediction pipeline requires a positive CPU future limit");
    }
    std::deque<PendingPredictionRecordBatch> cpu_futures;
    size_t item_index = 0;
    while (std::forward<ItemGeneratorFn>(generator)(item_index)) {
        ...
        for (StagedPredictionRecord& staged : staged_records) {
            cpu_futures.push_back(
                enqueue_prediction_record(cpu_pool, std::move(staged), threshold, evaluation_dataset));
            if (cpu_futures.back().evaluation_profile != nullptr) {
                EvaluationProfileRecord& profile = *cpu_futures.back().evaluation_profile;
                size_t task_count = 0;
                for (const PendingPredictionRecordBatch& pending : cpu_futures) {
                    task_count += pending.encoding.images.size();
                }
                profile.peak_in_flight_tasks = std::max(profile.peak_in_flight_tasks, task_count);
                profile.peak_in_flight_slots = std::max(profile.peak_in_flight_slots, cpu_futures.size());
            }
...
inline void finalize_prediction_result(PredictionRunResult& result,
    ...
    const size_t image_count = result.processed_images > 0 ? result.processed_images : result.records.size();
    result.timing = validate_detail::elapsed_timing(started_at, image_count);
}
...
inline RawPredictionRuntimeSetup make_raw_prediction_runtime_setup(
    ...
    RawPredictionRuntimeSetup setup{
        RuntimeContext{resolve_runtime_config(options.workers, runtime_lanes, 0, options.cpu_affinity)},
        {},
        {},
    };
...
inline std::vector<BackendInferenceLane> make_backend_prediction_lanes(const ResolvedModelArtifacts& artifacts,
    ...
    return predict_internal::make_backend_inference_lanes(
        std::move(backend_lanes), device_id, slot_count, static_cast<std::size_t>(artifacts.config.num_classes), 1,
        artifacts.config.segmentation
            ? std::make_optional(std::make_pair(static_cast<uint32_t>(artifacts.config.resolution),
                                                static_cast<uint32_t>(artifacts.config.resolution)))
            : std::nullopt,
...
inline std::vector<RawImagePredictionLane> make_raw_prediction_lanes(const ResolvedModelArtifacts& artifacts,
    ...
    std::vector<BackendInferenceLane> backend_lanes =
        make_backend_prediction_lanes(artifacts, backend_name, device_id, allow_fp16, lane_count, slot_count, true);
...
inline std::vector<NativePredictionLane> make_native_prediction_lanes(
    const int device_id, const std::int64_t image_height, const std::int64_t image_width, const int lane_count,
    const std::size_t slot_count, const std::int64_t prediction_capacity, const bool include_masks) {
    c10::cuda::CUDAGuard device_guard(checked_device_index(device_id));
    ...
    for (int lane_index = 0; lane_index < lane_count; ++lane_index) {
        ...
        lanes.push_back(NativePredictionLane{
            ...
            std::make_shared<PredictionBufferSlotPool>(
                slot_count,
                PredictionBufferConfig{
                    1,
                    prediction_capacity,
                    include_masks ? std::make_optional(std::make_pair(static_cast<uint32_t>(image_height),
