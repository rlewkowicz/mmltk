...
__global__ void draw_manual_mask_runs_rgba_pitched_kernel(const draw_launch::ManualMaskRunsRgbaPitchedLaunch launch) {
    ...
    if (run_index >= launch.run_count) {
        return;
    }
    const std::uint64_t pixel_count = static_cast<std::uint64_t>(launch.overlay_region.width) *
                                      static_cast<std::uint64_t>(launch.overlay_region.height);
    const std::uint64_t start = launch.run_pairs[run_index * 2U];
    const std::uint64_t length = launch.run_pairs[run_index * 2U + 1U];
    ...
    for (std::uint64_t pixel = start + threadIdx.x; pixel < end; pixel += blockDim.x) {
        ...
        cuda_launch::store_rgba_pixel(
            launch.overlay_region.pixels, launch.overlay_region.pitch_bytes, x, y,
            cuda_launch::RgbaPixelU8{launch.color.r, launch.color.g, launch.color.b, launch.color.a});
    }
}
...
MMLTK_DRAW_CUDA_DEFINE_LAUNCHER(launch_draw_manual_mask_runs_rgba_pitched,
