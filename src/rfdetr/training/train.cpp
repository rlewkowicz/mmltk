...
std::future<TrainLaneResult> enqueue_train_lane(
    WorkerPool& lane_pool, WorkerPool* cpu_pool, DatasetLoader& loader, TrainLaneContext& lane, const Batch& batch,
    std::shared_ptr<SharedCudaEvent> params_ready, double num_boxes_value, double scaled_loss_factor,
    size_t parameter_version, const DetectionConfig& detection_config, const NativeRfDetrModel& model,
    const std::vector<std::string>& parameter_names, int device_id, int image_height, int image_width,
    const torch::Tensor& mean, torch::Tensor std_tensor, bool amp_enabled, at::ScalarType autocast_dtype) {
    return lane_pool.enqueue([cpu_pool, &loader, &lane, batch, params_ready = std::move(params_ready), num_boxes_value,
                              scaled_loss_factor, parameter_version, &detection_config, &model, &parameter_names,
                              device_id, image_height, image_width, mean, std_t = std::move(std_tensor), amp_enabled,
        ...
        std::vector<torch::Tensor> gradients;
        {
            ...
            if (!torch::isfinite(loss).item<bool>()) {
                throw std::runtime_error("non-finite RF-DETR loss encountered during native training" +
...
EvalPassResult evaluate_model(const TrainOptions& options, RuntimeContext& runtime, NativeRfDetrModel& model,
                              const std::filesystem::path& compiled_path, const DetectionConfig& detection_config,
    ...
    DatasetLoader loader(make_loader_config(compiled_path.string(), eval_batch_size, false, 1,
                                            runtime.split().gather_threads, runtime.loader_affinity_string(),
    ...
    while (loader.next_batch(batch)) {
        ...
        for (size_t image_pos = 0; image_pos < processed_count; ++image_pos) {
            ...
            lane_futures.push_back(
                lane_pool.enqueue([&lanes, lane_index, processed = std::move(image_result), processed_ready, image_id,
                                   category_count = loader.num_classes(), max_dets = options.eval_max_dets,
                    ...
                    return stage_result_to_predictions(image_id, processed, category_count, max_dets, std::move(lease),
...
TrainRunResult run_training(const TrainOptions& options) {
    ...
    auto make_loader_config_for = [&](const std::filesystem::path& compiled_path, size_t loader_batch_size,
                                      bool shuffle, int prefetch_factor, bool shard_batches) {
    ...
    DatasetLoader train_loader(
        make_loader_config_for(options.train_compiled_path, options.batch_size, true, options.prefetch_factor, true));
    ...
    if (main_process) {
        ...
        val_loader = std::make_unique<DatasetLoader>(
            make_loader_config_for(options.val_compiled_path, val_batch_size, false, 1, false));
    }
    std::unique_ptr<DatasetLoader> test_loader;
    if (main_process && !options.test_compiled_path.empty()) {
        const size_t val_batch_size = options.val_batch_size > 0 ? options.val_batch_size : options.batch_size;
        test_loader = std::make_unique<DatasetLoader>(make_loader_config(
            options.test_compiled_path.string(), val_batch_size, false, 1, train_runtime.split().gather_threads,
    ...
    if (!options.val_compiled_path.empty()) {
        model.optimize_for_inference(checked_inference_batch_size(options.batch_size), false, options.compilation_mode);
    }
    if (main_process) {
    ...
    if (main_process) {
        mmltk::logging::logger("rfdetr.train")
            ->info(
                ...
                "eval_lanes={} effective_batch_per_rank={} effective_batch_global={}",
    ...
    if (main_process) {
        ...
        write_json_file(progress_path, json{
                                           {"phase", "starting"},
                                           {"epoch", start_epoch},
    ...
    for (int epoch = start_epoch; epoch < options.epochs; ++epoch) {
        ...
        auto loss_sum_device =
            torch::zeros({}, torch::TensorOptions().dtype(torch::kFloat32).device(cuda_device(options.device_id)));
        auto class_loss_sum_device =
            torch::zeros({}, torch::TensorOptions().dtype(torch::kFloat32).device(cuda_device(options.device_id)));
        auto box_loss_sum_device =
            torch::zeros({}, torch::TensorOptions().dtype(torch::kFloat32).device(cuda_device(options.device_id)));
        ...
        auto write_progress_snapshot = [&](std::string_view phase, double val_loss, const json& val_summary,
            ...
            const double average_class_loss =
                local_micro_batches > 0 ? host_class_loss_sum / static_cast<double>(local_micro_batches) : 0.0;
            const double average_box_loss =
                local_micro_batches > 0 ? host_box_loss_sum / static_cast<double>(local_micro_batches) : 0.0;
            const double average_loss =
                local_micro_batches > 0 ? host_loss_sum / static_cast<double>(local_micro_batches) : 0.0;
        ...
        const auto run_optimizer_step = [&](const auto& post_step) {
        ...
        if (train_lane_count <= 1) {
            while (true) {
                ...
                torch::Tensor normalized;
                {
                    ...
                    normalized = normalize_batch(*batch, options.device_id, train_loader.image_height(),
                                                 train_loader.image_width(), mean, std)
                                     .contiguous();
                }
            ...
            while (local_full_batches < usable_full_batches) {
                ...
                for (int lane_index = 0; lane_index < train_lane_count; ++lane_index) {
                    ...
                    lane_futures.push_back(enqueue_train_lane(
                        *train_lane_pool, &train_runtime.cpu_pool(), train_loader, lane, *batch, params_ready,
                        ...
                        all_param_names, options.device_id, static_cast<int>(train_loader.image_height()),
                        static_cast<int>(train_loader.image_width()), mean, std, amp_enabled, autocast_dtype));
                }
                ...
                if (local_waves % options.grad_accum_steps == 0) {
                    run_optimizer_step([&] {
                        ++parameter_version;
                        params_ready =
                            record_current_stream_event(options.device_id, "parallel train parameter readiness event");
                    });
                }
                flush_progress(false);
            }
        ...
        if (main_process) {
            ...
            auto val_result = evaluate_model(options, train_runtime, model, options.val_compiled_path, detection_config,
                                             epoch, phase_progress_label("val", epoch, options.epochs));

            mmltk::logging::logger("rfdetr.train")
                ->info("epoch {} stats: optimizer={} train_loss={:.6f} val_loss={:.6f} bbox_ap={:.4f} mask_ap={:.4f}",
            ...
            if (ema.has_value()) {
                ...
                auto ema_result =
                    evaluate_model(options, train_runtime, model, options.val_compiled_path, detection_config, epoch,
    ...
    if (main_process && test_loader) {
        ...
        result.test_summary = evaluate_model(options, train_runtime, best_model, options.test_compiled_path,
                                             detection_config, std::nullopt, "test")
                                  .summary;
    }
...
void print_training_summary(const TrainOptions& options, const TrainRunResult& result) {
    ...
    mmltk::logging::logger("rfdetr.train")
        ->info(
            "rfdetr train[{}]: preset={} optimizer={} epochs={} train_loss={:.6f} val_loss={:.6f} val_bbox_ap={:.4f} "
            "val_mask_ap={:.4f} best={} checkpoint={}",
