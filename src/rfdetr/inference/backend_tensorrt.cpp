...
const char* configure_fp16_build(nvinfer1::IBuilderConfig& config, const bool allow_fp16) {
    if (!allow_fp16) {
        return "disabled";
    }
...
class TensorRtSharedState {
   ...
   private:
    ...
    void build_engine_from_onnx(const std::filesystem::path& onnx_path) {
        ...
        const char* fp16_policy = configure_fp16_build(*config, allow_fp16_);
        {
            std::ostringstream message;
            message << "[trt:build] config: fp16_request=" << (allow_fp16_ ? "on" : "off")
                    << " fp16_policy=" << fp16_policy << " workspace_bytes=" << (1ULL << 30)
                    << " opt_level=" << config->getBuilderOptimizationLevel() << " profiling_verbosity=detailed";
            emit_tensorrt_log_line(message.str());
        }
