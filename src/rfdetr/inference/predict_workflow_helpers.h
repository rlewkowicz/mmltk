...
inline void ensure_raw_image_resize_scratch(std::vector<std::uint8_t>& resize_scratch, std::size_t capacity) {
    if (resize_scratch.capacity() < capacity) {
        resize_scratch.reserve(capacity);
    }
}

inline void hwc_uint8_to_nchw_float(const std::uint8_t* src, float* dst, int height, int width) {
...
inline RawImageBatchItem decode_raw_image_into_tensor(const PredictImageInput& input, int64_t dataset_index,
                                                      int64_t default_image_id, int resolution, float* destination,
    ...
    if (raw_width != resolution || raw_height != resolution) {
        const std::size_t resize_bytes =
            static_cast<std::size_t>(resolution) * static_cast<std::size_t>(resolution) * 3U;
        ensure_raw_image_resize_scratch(resize_scratch, resize_bytes);
        resize_scratch.resize(resize_bytes);
        raw_image_resizer().resize(raw_pixels, raw_width, raw_height, resize_scratch.data(), resolution, resolution);
...
inline std::vector<RawImageBatchItem> load_raw_image_batch(
    WorkerPool& cpu_pool, const std::vector<PredictImageInput>& inputs, size_t start_index, size_t batch_count,
    int resolution, const torch::Tensor& batch_cpu, std::vector<std::vector<std::uint8_t>>& resize_scratch_slots) {
    ...
    for (size_t offset = 0; offset < batch_count; ++offset) {
        ...
        futures.push_back(
            cpu_pool.enqueue([input = std::move(input), dataset_index = static_cast<int64_t>(start_index + offset),
                              image_id = static_cast<int64_t>(start_index + offset + 1), resolution,
                              batch_slot = std::move(batch_slot), &resize_scratch]() mutable {
                return decode_raw_image_into_tensor(input, dataset_index, image_id, resolution,
                                                    batch_slot.data_ptr<float>(), resize_scratch);
            }));
    }
...
inline torch::Tensor raw_image_target_sizes(const std::vector<RawImageBatchItem>& items, int device_id,
    ...
    if (!stream.has_value()) {
        return torch::tensor(target_sizes, options).view({static_cast<int64_t>(items.size()), 2});
    }
    c10::cuda::CUDAStreamGuard stream_guard(*stream);
    return torch::tensor(target_sizes, options).view({static_cast<int64_t>(items.size()), 2});
}

inline torch::Tensor raw_image_target_sizes(const RawImageBatchItem& item, int device_id,
                                            const std::optional<c10::cuda::CUDAStream>& stream = std::nullopt) {
...
inline std::string format_prediction_summary(const PredictOptions& options, const PredictionRunResult& result) {
    const char* source_kind =
        options.source_kind == PredictSourceKind::CompiledDataset ? "compiled_dataset" : "image_files";
    std::ostringstream stream;
    stream.setf(std::ios::fixed);
    stream.precision(4);
    stream << "rfdetr predict[" << result.backend_name << "]: source=" << source_kind
           << " input=" << result.artifacts.input_path.string() << " preset=" << result.artifacts.config.preset_name
           << " records=" << result.records.size() << " threshold=" << options.threshold;
    stream.precision(2);
    stream << " img_per_s=" << result.timing.img_per_s << " output=" << options.output_path.string();
    return stream.str();
}

inline std::string format_evaluation_summary(const EvaluateOptions&, const EvaluationRunResult& result) {
    std::ostringstream stream;
    stream.setf(std::ios::fixed);
    stream.precision(6);
    stream << "rfdetr evaluate[" << result.backend_name << "]: input=" << result.artifacts.input_path.string()
           << " preset=" << result.artifacts.config.preset_name << " images=" << result.image_count
           << " bbox_ap=" << result.result.summary.bbox.ap << " bbox_ap50=" << result.result.summary.bbox.ap50
           << " bbox_ap75=" << result.result.summary.bbox.ap75 << " mask_ap=";
    if (result.result.summary.mask.has_value()) {
        stream << result.result.summary.mask->ap << " mask_ap50=" << result.result.summary.mask->ap50
               << " mask_ap75=" << result.result.summary.mask->ap75;
    } else {
        stream << "null mask_ap50=null mask_ap75=null";
    }
    stream.precision(2);
...
inline json prediction_record_to_json(const PredictionRecord& record, const std::vector<std::string>& class_names) {
...
inline void append_raw_prediction_records(std::vector<PredictionRecord>& records, std::vector<RawImageBatchItem>& items,
                                          const std::vector<TensorMap>& outputs, std::size_t category_count,
                                          std::size_t max_dets_per_image, float threshold) {
    for (size_t item_index = 0; item_index < items.size(); ++item_index) {
        auto detections =
            filter_threshold(result_to_predictions(static_cast<int>(items[item_index].image_id), outputs[item_index],
                                                   category_count, max_dets_per_image),
                             threshold);
        records.push_back(make_prediction_record(items[item_index].dataset_index, items[item_index].image_id,
                                                 std::move(items[item_index].source_name), std::move(detections)));
    }
}
