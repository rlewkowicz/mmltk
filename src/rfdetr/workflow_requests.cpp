...
[[nodiscard]] std::vector<std::string> validation_backend_order(const ValidateRequest& request) {
    const auto available = [&](const std::string_view name) {
        return (name == "weights" && !request.weights_path.empty()) || (name == "onnx" && !request.onnx_path.empty()) ||
               (name == "tensorrt" && !request.tensorrt_path.empty());
    };
    std::vector<std::string> order;
    const auto append = [&](const std::string_view name) {
        if (available(name) && std::find(order.begin(), order.end(), name) == order.end()) {
            order.emplace_back(name);
        }
    };
    std::size_t start = 0U;
    while (start <= request.eval_order.size()) {
        const std::size_t comma = request.eval_order.find(',', start);
        const std::string_view name(request.eval_order.data() + start,
                                    (comma == std::string::npos ? request.eval_order.size() : comma) - start);
        if (!name.empty() && name != "weights" && name != "onnx" && name != "tensorrt") {
            throw std::runtime_error("validation eval_order contains unsupported backend: " + std::string(name));
        }
        append(name);
...
[[nodiscard]] std::string join_validation_backend_order(const std::vector<std::string>& order) {
    std::string joined;
...
void validate_build_engine_request(const BuildEngineRequest& request) {
    if (request.onnx_path.empty() || !request.weights_path.empty() || !request.tensorrt_path.empty() ||
...
void validate_export_onnx_request(const ExportOnnxRequest& request) {
    if (request.weights_path.empty() || !request.onnx_path.empty() || !request.tensorrt_path.empty() ||
        request.output_path.empty()) {
...
PredictOptions to_predict_options(const PredictRequest& request, std::string_view preset_name) {
    ...
    options.preset_name = request.preset_name.empty() ? std::string(preset_name) : request.preset_name;
...
void validate_validate_request(const ValidateRequest& request) {
    ...
    if (request.weights_path.empty() && request.onnx_path.empty() && request.tensorrt_path.empty()) {
        throw std::runtime_error("rfdetr validate requires at least one of --weights, --onnx, or --tensorrt");
    }
...
void finalize_validate_request(ValidateRequest& request) {
    ...
    request.eval_order = join_validation_backend_order(validation_backend_order(request));
...
ValidationOptions to_validate_options(const ValidateRequest& request) {
    ...
    options.weights_path = request.weights_path;
    options.preset_name = request.preset_name;
    options.compiled_path = request.compiled_path;
...
void validate_train_request(const TrainRequest& request) {
    ...
    if (!valid_gpu_augmentation_config(request.gpu_augmentation)) {
...
TrainOptions to_train_options(const TrainRequest& request) {
    ...
    options.validation_loss = request.validation_loss;
    options.validation_profile = request.validation_profile;
    ...
    options.gpu_augmentation = request.gpu_augmentation;
...
TrainRequest train_request_from_options(const TrainOptions& options,
    ...
    request.validation_loss = options.validation_loss;
    request.validation_profile = options.validation_profile;
    ...
    request.gpu_augmentation = options.gpu_augmentation;
...
void finalize_train_request(TrainRequest& request, std::string_view fallback_preset_name) {
    ...
    request.preset_name = recipe_preset_name;
