...
inline void add_model_input_options(CLI::App* command,
...
inline void add_build_engine_request_options(CLI::App* io_options,
                                             CLI::App* execution_options,
    ...
    execution_options->add_flag("--fp16,!--no-fp16", request.allow_fp16,
                                "Enable FP16 TensorRT kernels");
}

inline void add_export_onnx_request_options(CLI::App* io_options,
                                            CLI::App* execution_options,
    ...
    execution_options->add_option("--device-id", request.device_id, "CUDA device id")->type_name("INT");
    execution_options->add_option("--opset-version", request.opset_version,
...
inline PredictCommandSpec add_predict_command_options(CLI::App* command) {
    PredictCommandSpec spec;
    ...
    execution->add_flag("--progress,!--no-progress", spec.request.progress_bar,
                        "Render interactive progress output");
    add_compile_mode_option(execution, spec.compile_mode);
    return spec;
}

inline ValidateCommandSpec add_validate_command_options(CLI::App* command,
                                                        const bool hide_compile_tuning = false) {
    ValidateCommandSpec spec;
    auto* dataset = command->add_option_group("Dataset");
    add_path_option(dataset, "--compiled", spec.request.compiled_path, "Compiled dataset split (.bin)");
    add_path_option(dataset, "--source", spec.request.source_dir,
    ...
    dataset->add_option("--resolution", spec.request.resolution, "Square resolution for recompiles")
        ->type_name("INT");
    dataset->add_flag("--recompile", spec.request.recompile,
                      "Recompile the source dataset before validation");
    spec.compile_workers =
        dataset->add_option("--compile-workers",
                            spec.request.compile_workers,
                            "Total CPU worker budget for recompiles; 0 or negative selects all available CPUs")
            ->type_name("INT");
    spec.compile_cuda_mask_batch_size =
        dataset->add_option("--compile-cuda-mask-batch-size",
                            spec.request.compile_cuda_mask_batch_size,
                            "Batched CUDA mask-resize task count for recompiles; 0 disables GPU mask resizing")
            ->type_name("INT");
    spec.compile_cuda_device_id =
        dataset->add_option("--compile-cuda-device-id",
                            spec.request.compile_cuda_device_id,
    ...
    add_path_option(model, "--tensorrt", spec.request.tensorrt_path,
    ...
    add_path_option(reports, "--report-json", spec.request.report_json_path,
                    "Validation report JSON path");
    reports->add_option("--eval-order", spec.request.eval_order,
    ...
    execution->add_option("--limit-images", spec.request.limit_images, "Limit the number of validated images")
        ->type_name("INT");
    execution->add_option("--alignment-images", spec.request.alignment_images,
                          "Number of images used for backend alignment probes")
        ->type_name("INT");
    execution->add_option("--eval-max-dets", spec.request.eval_max_dets,
    ...
    execution->add_option("--cpu-affinity", spec.request.cpu_affinity,
                          "Linux CPU list or range string for workers");
    execution->add_flag("--profile", spec.request.profile,
...
inline TrainCommandSpec add_train_command_options(CLI::App* command,
                                                  const bool hide_gui_unsupported_options = false) {
    TrainCommandSpec spec;
    auto* dataset = command->add_option_group("Dataset");
    add_path_option(dataset, "--train-compiled", spec.request.train_compiled_path,
                    "Compiled training split (.bin)");
    add_path_option(dataset, "--val-compiled", spec.request.val_compiled_path,
                    "Compiled validation split (.bin)");
    add_path_option(dataset, "--test-compiled", spec.request.test_compiled_path,
                    "Optional compiled test split (.bin)");
    auto* checkpoint = command->add_option_group("Checkpoint");
    add_path_option(checkpoint, "--output-dir", spec.request.output_dir,
                    "Output directory for checkpoints and logs");
    add_path_option(checkpoint, "--weights", spec.request.weights_path,
                    "Source checkpoint used for initialization");
    add_path_option(checkpoint, "--resume", spec.request.resume_path,
                    "Existing native checkpoint to resume");
    auto* optimization = command->add_option_group("Optimization");
    optimization->add_option("--batch-size", spec.request.batch_size, "Per-rank training batch size")
        ->type_name("INT");
    optimization->add_option("--val-batch-size", spec.request.val_batch_size,
    ...
    optimization->add_option("--grad-accum-steps", spec.request.grad_accum_steps,
    ...
    optimization->add_flag("--freeze-encoder,!--no-freeze-encoder",
                           spec.request.freeze_encoder,
                           "Freeze encoder/backbone parameters");
    spec.lr_component_decay =
        optimization->add_option("--lr-component-decay",
                                 spec.request.lr_component_decay,
                                 "Component-wise learning rate decay");
    spec.lr_component_decay->type_name("FLOAT");
    spec.encoder_layer_decay =
        optimization->add_option("--encoder-layer-decay",
                                 spec.request.encoder_layer_decay,
                                 "Per-layer encoder learning rate decay");
    spec.encoder_layer_decay->type_name("FLOAT");
    spec.weight_decay =
        optimization->add_option("--weight-decay", spec.request.weight_decay, "Optimizer weight decay");
    ...
    spec.lr_scheduler =
        optimization->add_option("--lr-scheduler", spec.request.lr_scheduler,
                                 "Learning rate scheduler: step or cosine");
    spec.lr_min_factor =
        optimization->add_option("--lr-min-factor", spec.request.lr_min_factor,
    ...
    spec.warmup_momentum =
        optimization->add_option("--warmup-momentum", spec.request.warmup_momentum,
    ...
    optimization->add_option("--clip-max-norm", spec.request.clip_max_norm, "Gradient clipping max norm")
        ->type_name("FLOAT");
    spec.fused_optimizer = optimization->add_flag("--fused-optimizer,!--no-fused-optimizer",
                                                  spec.request.fused_optimizer,
                                                  "Use the native fused AdamW backend when available (AdamW only)");
    optimization->add_flag("--use-ema,!--no-ema", spec.request.use_ema,
    ...
    optimization->add_option("--eval-max-dets", spec.request.eval_max_dets,
    ...
    spec.device_ids =
        execution->add_option("--device-ids",
                              spec.device_ids_text,
    ...
    spec.dist_worker = distributed->add_flag("--dist-worker",
                                             spec.request.distributed_worker,
                                             "Internal flag for forked distributed workers");
    spec.dist_rank =
        distributed->add_option("--dist-rank", spec.request.distributed_rank, "Internal worker rank");
    spec.dist_rank->type_name("INT");
    spec.dist_world_size =
        distributed->add_option("--dist-world-size", spec.request.distributed_world_size,
                                "Internal worker world size");
    spec.dist_world_size->type_name("INT");
    spec.dist_store_file =
        add_path_option(distributed, "--dist-store-file", spec.request.distributed_store_path,
...
inline void require_exactly_one_model_input(const std::filesystem::path& onnx_path,
    ...
    if (selected_input_count != 1U) {
        throw std::runtime_error(
            std::string("rfdetr ") + command + " requires exactly one of --onnx or --tensorrt");
    }
}
...
inline void append_unsupported_gui_flag(std::vector<std::string>& unsupported_flags,
...
inline void reject_unsupported_gui_flags(const char* command,
                                         const char* reason,
    ...
    throw std::runtime_error(
        std::string("GUI seeding for `mmltk rfdetr ") + command + "` does not support " + reason +
...
inline void finalize_validate_command(ValidateCommandSpec& spec,
                                      const ValidateUnsupportedOptionMode unsupported_mode =
    ...
    if (unsupported_mode == ValidateUnsupportedOptionMode::RejectGuiUnsupported) {
        ...
        append_unsupported_gui_flag(unsupported_flags,
                                    spec.compile_cuda_mask_batch_size,
        ...
        reject_unsupported_gui_flags(
            "validate",
            "compile-worker tuning flags because the GUI has no matching fields",
            unsupported_flags);
    }
    mmltk::rfdetr::finalize_validate_request(spec.request);
}

inline void finalize_train_command(TrainCommandSpec& spec,
                                   const std::string_view fallback_preset_name = {},
                                   const TrainUnsupportedOptionMode unsupported_mode =
                                       TrainUnsupportedOptionMode::AllowAll) {
    spec.request.compilation_mode = parse_compilation_mode(spec.compile_mode);
    ...
    if (unsupported_mode == TrainUnsupportedOptionMode::RejectGuiUnsupported) {
        ...
        reject_unsupported_gui_flags("train",
                                     "these flags because the GUI has no matching fields",
                                     unsupported_flags);
    }
...
inline void append_bool_flag(std::vector<std::string>& args,
                             const bool value,
...
inline void append_option(std::vector<std::string>& args,
