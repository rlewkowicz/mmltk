...
inline PredictCommandSpec add_predict_command_options(CLI::App* command) {
    ...
    execution->add_flag("--fp16,!--no-fp16", spec.request.allow_fp16, "Enable FP16 backend execution when supported");
...
inline ValidateCommandSpec add_validate_command_options(CLI::App* command, const bool hide_compile_tuning = false) {
    ...
    execution->add_flag("--fp16,!--no-fp16", spec.request.allow_fp16, "Enable FP16 TensorRT execution when supported");
    return spec;
}

inline TrainCommandSpec add_train_command_options(CLI::App* command, const bool hide_gui_unsupported_options = false) {
