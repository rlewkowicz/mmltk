...
class ScopedSdpBackendGuard final {
   public:
    ScopedSdpBackendGuard(const bool flash, const bool memory_efficient, const bool math, const bool cudnn)
        ...
        context.setSDPUseMemEfficient(memory_efficient);
        context.setSDPUseMath(math);
        context.setSDPUseCuDNN(cudnn);
    }

    ~ScopedSdpBackendGuard() {
        auto& context = at::globalContext();
    ...
    ScopedSdpBackendGuard(const ScopedSdpBackendGuard&) = delete;
    ScopedSdpBackendGuard& operator=(const ScopedSdpBackendGuard&) = delete;

   private:
    std::unique_lock<std::mutex> lock_;
    bool flash_ = false;
    bool memory_efficient_ = false;
    bool math_ = false;
    bool cudnn_ = false;
};
...
ResolvedModelArtifacts resolve_weight_artifacts_impl(const std::filesystem::path& weights_path,
                                                     const std::string_view declared_preset_name) {
    ...
    } else if (!declared_preset_name.empty()) {
        const auto* declared = find_model_preset(declared_preset_name);
        if (declared == nullptr) {
            throw std::runtime_error("unknown RF-DETR preset override: " + std::string(declared_preset_name));
        }
        artifacts.config = make_config_from_preset(*declared);
    } else {
...
ResolvedModelArtifacts resolve_model_artifacts(const ModelArtifactRequest& request) {
    ...
    if (!request.weights_path.empty()) {
        ResolvedModelArtifacts artifacts = resolve_weight_artifacts_impl(request.weights_path, request.preset_name);
        if (!request.preset_name.empty() && artifacts.config.preset_name != request.preset_name) {
            const auto* preset = find_model_preset(request.preset_name);
            if (preset == nullptr) {
                throw std::runtime_error("unknown RF-DETR preset override: " + request.preset_name);
            }
            NativeRfDetrConfig declared = make_config_from_preset(*preset);
            declared.num_classes = artifacts.config.num_classes;
            artifacts.config = std::move(declared);
        }
        if (request.resolution > 0) {
            artifacts.config.resolution = request.resolution;
        }
        return artifacts;
    }
    ...
    auto resolve_non_weight_config = [&](const std::filesystem::path& input_path) {
        if (!request.preset_name.empty()) {
            if (const auto* preset = find_model_preset(request.preset_name)) {
                NativeRfDetrConfig config = make_config_from_preset(*preset);
                if (request.resolution > 0) {
...
ResolvedModelArtifacts resolve_upstream_weight_artifacts(const std::filesystem::path& weights_path) {
    return resolve_weight_artifacts_impl(weights_path, {});
}
...
void NativeRfDetrModel::export_onnx(const std::filesystem::path& output_path, int opset_version, int batch_size,
    ...
    auto dummy_mask = torch::zeros({batch_size, config_.resolution, config_.resolution},
                                   torch::TensorOptions().dtype(torch::kBool).device(device));
    auto trace_res = [&]() {
        ScopedSdpBackendGuard sdp_guard(false, false, true, false);
...
void export_weights_to_onnx(const ModelArtifactRequest& input, const std::filesystem::path& output_path, int device_id,
                            int opset_version, bool simplify) {
    auto artifacts = resolve_model_artifacts(input);
    auto device = torch::Device(torch::kCUDA, static_cast<c10::DeviceIndex>(device_id));
    NativeRfDetrModel model(artifacts.config);
    model.to(device);
    model.load_weights(artifacts.weights_path);
