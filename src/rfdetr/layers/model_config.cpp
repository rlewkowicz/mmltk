...
const std::vector<ModelPresetConfig>& preset_table() {
    ...
    static const bool kCatalogValidated = [&] {
        if (kPresets.size() != kPresetCatalog.size()) {
            throw std::logic_error("RF-DETR runtime preset count does not match the canonical catalog");
        }
        for (std::size_t index = 0U; index < kPresets.size(); ++index) {
            const ModelPresetConfig& preset = kPresets[index];
            const PresetCatalogEntry& metadata = kPresetCatalog[index];
            if (metadata.preset_name != preset.preset_name ||
                metadata.canonical_weight_filename != preset.canonical_weight_filename ||
