...
inline std::optional<std::string> read_optional_string(torch::serialize::InputArchive& archive, const char* key) {
    ...
    if (!value.isString()) throw std::runtime_error(std::format("archive key is not a string: {}", key));
...
inline std::optional<double> read_optional_double(torch::serialize::InputArchive& archive, const char* key) {
    c10::IValue value;
    if (!archive.try_read(key, value)) return std::nullopt;
    if (!value.isDouble() && !value.isInt()) throw std::runtime_error(std::format("archive key is not numeric: {}", key));
...
inline std::optional<bool> read_optional_bool(torch::serialize::InputArchive& archive, const char* key) {
    c10::IValue value;
    if (!archive.try_read(key, value)) return std::nullopt;
    if (!value.isBool()) throw std::runtime_error(std::format("archive key is not a bool: {}", key));
    return value.toBool();
}
