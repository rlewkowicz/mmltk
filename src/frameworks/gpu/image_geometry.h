...
struct ImageGeometry final {
    ...
    [[nodiscard]] constexpr bool valid() const noexcept {
        return backing_width != 0U && backing_height != 0U && original_content.width != 0U && original_content.height != 0U &&
               scale_x.valid() && scale_y.valid() &&
               static_cast<std::uint64_t>(original_content.width) * scale_x.backing_pixels ==
                   static_cast<std::uint64_t>(backing_width) * scale_x.original_pixels &&
               static_cast<std::uint64_t>(original_content.height) * scale_y.backing_pixels ==
                   static_cast<std::uint64_t>(backing_height) * scale_y.original_pixels;
    }
