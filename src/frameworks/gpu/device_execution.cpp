...
DeviceExecution resolve_device_execution(int ordinal, const mmltk::common::system::NumaTopology& topology, int numa_node,
    ...
    std::ranges::transform(result.pci_identity, result.pci_identity.begin(),
    ...
    try {
        ...
        throw std::invalid_argument("CUDA device " + std::to_string(ordinal) + " (PCI " + result.pci_identity +
                                    ") placement: " + error.what() + "; select --numa-node <GPU-local permitted node>");
    }
    return result;
}
}  // namespace mmltk::frameworks::gpu
