...
template <typename State, typename Releaser>
    requires std::default_initializable<State> && std::movable<State> && std::default_initializable<Releaser> && std::invocable<Releaser&, State&>
class UniqueResource {
   ...
   public:
    ...
    UniqueResource(UniqueResource&& other) noexcept(std::is_nothrow_move_constructible_v<State>) : state_(std::exchange(other.state_, State{})) {}
