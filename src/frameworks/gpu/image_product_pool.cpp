...
void ImageProductPool::Select(const Product& product) {
 if (!product.slot_ || product.slot_->admission != admission_ || !product.valid()) throw std::invalid_argument("completed image product is invalid or foreign");
 {
  std::scoped_lock lock(admission_->mutex);
  if (!product.slot_->Readable() || product.slot_->facts.revision != product.revision_) throw std::invalid_argument("completed image product is unavailable");
