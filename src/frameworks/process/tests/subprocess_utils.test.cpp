...
TEST_CASE("captured_child_runner_collects_output", "[frameworks][process]") {
    const auto result = process::run_captured_child_process(
        "test child", "failed to read test child output: ", [](const int output_fd, const int setup_fd) {
            process::prepare_captured_output_child(output_fd, setup_fd);
...
TEST_CASE("captured_child_runner_reports_setup_failure", "[frameworks][process]") {
    const auto result =
        process::run_captured_child_process("test child", "failed to read test child output: ", [](const int, const int setup_fd) {
            errno = ENOENT;
