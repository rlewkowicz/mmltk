...
bool allocation_path_names_match(const DecodeAllocationLimit& limit, const wire::AllocationRequest& request) noexcept {
    if ((limit.recursive_dynamic && limit.path.size() > request.path.size()) ||
        (!limit.recursive_dynamic && limit.path.size() != request.path.size()))
        return false;
...
bool allocation_limit_accepts(const DecodeAllocationLimit& limit, const wire::AllocationRequest& request) noexcept {
    if ((request.kind == wire::AllocationKind::Text || request.kind == wire::AllocationKind::Bytes) && limit.max_bytes &&
        request.size > *limit.max_bytes)
        return false;
    if ((request.kind == wire::AllocationKind::Sequence || request.kind == wire::AllocationKind::Object) && limit.max_items &&
        request.size > *limit.max_items)
        return false;
    return true;
}
...
bool allocation_allowed(const void* raw_context, const wire::AllocationRequest& request) noexcept {
    ...
    for (const DecodeAllocationLimit& limit : context.reflected->entries) {
        ...
        const bool discriminator_dependent =
            std::ranges::any_of(limit.path, [](const DecodePathSelector& selector) { return !selector.object_kind.empty(); });
...
bool FixedCborEncoder::text(const std::string_view value) noexcept {
    return head(3U, value.size()) && append(std::as_bytes(std::span{value}));
}
...
wire::ErrorCode FixedCborEncoder::error() const noexcept {
    return capacity_exceeded_ ? wire::ErrorCode::LimitExceeded : wire::ErrorCode::TypeMismatch;
}
...
bool FixedCborEncoder::big_endian(const std::uint64_t value, const std::size_t bytes) noexcept {
    ...
    for (std::size_t index = bytes; index != 0U; --index)
        destination_[position_++] = static_cast<std::byte>(value >> ((index - 1U) * 8U));
    return true;
}
