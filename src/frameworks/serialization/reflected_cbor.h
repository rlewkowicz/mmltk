 ...
 [[nodiscard]] bool EncodeCompact(FixedCborEncoder& writer) const {
  ...
  d::visit_members<Record>([&]<class Declaration>(const auto&) {
   if constexpr (std::meta::reflect_constant(Declaration::pointer) == std::meta::reflect_constant(BytesMember)) {
    ...
    valid = valid && bytes_.second.empty() && bytes_.size() <= policy.maximum_bytes && bytes_.size() >= policy.minimum_bytes && writer.bytes(bytes_.first);
   } else {
    valid = valid && d::member_constraints_accept<Declaration>(record_.*Declaration::pointer) && encode_compact(writer, record_.*Declaration::pointer);
   }
  });
  return valid;
 }
 [[nodiscard]] bool DecodeCompact(wire::Reader& reader) {
  ...
  d::visit_members<Record>([&]<class Declaration>(const auto&) {
   ...
   if constexpr (std::meta::reflect_constant(Declaration::pointer) == std::meta::reflect_constant(BytesMember)) {
    ...
    valid = decode_compact_item(record_.*Declaration::pointer, reader, 1U) && d::member_constraints_accept<Declaration>(record_.*Declaration::pointer);
   }
  });
  ...
  for (std::size_t field = 0U; field < *count; ++field) {
   ...
   d::visit_members<Record>([&]<class Declaration>(const auto&) {
    ...
    if constexpr (std::meta::reflect_constant(Declaration::pointer) == std::meta::reflect_constant(BytesMember)) {
     ...
     valid = decode_compact_item(record_.*Declaration::pointer, reader, 2U) && d::member_constraints_accept<Declaration>(record_.*Declaration::pointer);
    }
   });
