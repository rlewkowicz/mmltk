...
class Reader {
   ...
   private:
    ...
    [[nodiscard]] std::expected<FlatValue, DecodeError> read_flat_scalar(ItemHead head, bool apply_allocation_policy,
                                                                         bool materialize = true);
    [[nodiscard]] std::expected<std::size_t, DecodeError> begin_container_item(std::size_t depth, std::uint8_t expected_major,
                                                                               AllocationKind allocation_kind);
    [[nodiscard]] std::expected<Value, DecodeError> read_item(std::size_t depth, bool apply_allocation_policy = true,
                                                              bool materialize = true);
    ...
    friend std::expected<void, DecodeError> validate_raw_item_structural(ByteView bytes, Limits limits,
                                                                         StructuralValidationScratch scratch);
};
...
[[nodiscard]] std::expected<void, DecodeError> validate_raw_item_structural(ByteView bytes, Limits limits,
                                                                            StructuralValidationScratch scratch);
