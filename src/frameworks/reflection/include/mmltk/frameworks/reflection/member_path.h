...
class ReflectedMemberIdentity final {
   public:
    ...
    [[nodiscard]] static consteval ReflectedMemberIdentity from_path() {
        ...
        static_assert(sizeof...(Members) != 0U && sizeof...(Members) <= kMaximumDepth,
                      "reflected member path exceeds its fixed identity capacity");
        if constexpr (sizeof...(Members) != 0U && sizeof...(Members) <= kMaximumDepth) {
            static_assert(detail::member_chain_is_applicable<CanonicalRoot, Members...>(),
                          "reflected member path cannot be applied to its root type");
        }
...
[[nodiscard]] consteval auto rebase_member_path_impl(MemberPathAccessor<Selector...>, MemberPathAccessor<Relative...>) {
    ...
    return [&]<std::size_t... Index>(std::index_sequence<Index...>) {
        if constexpr (sizeof...(Index) == 0U) {
            ...
            static_assert(
                std::same_as<std::remove_cvref_t<decltype(ParentAccessor{}(std::declval<Root&>()))>, std::remove_cvref_t<Destination>>,
                "the provider selector parent must have the relation destination type");
        }
        using Rebased = MemberPathAccessor<std::get<Index>(selector)..., Relative...>;
        static_assert(Rebased::template applicable<std::remove_cvref_t<Root>>(),
                      "the rebased provider destination must apply to its canonical root");
        return Rebased{};
   ...
   private:
    ...
    [[nodiscard]] static consteval ReflectedMemberPath from_identity(const ReflectedMemberIdentity identity) {
        ...
        for (std::size_t index = 0U; index < identity.depth_; ++index) {
            ...
            if (result.size_ + separator + member.size() > result.text_.size())
                throw "reflected member path exceeds its fixed text capacity";
            if (separator != 0U) result.text_[result.size_++] = '.';
            for (const char value : member)
                result.text_[result.size_++] = value;
        }
        return result;
    }
