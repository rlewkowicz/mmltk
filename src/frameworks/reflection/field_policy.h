...
struct FixedText : Annotation {
 ...
 [[nodiscard]] constexpr bool accepts(const std::string_view value) const noexcept {
  ...
  switch (characters) {
   case FixedTextCharacterPolicy::PrintableAscii: return std::ranges::all_of(value, [](const unsigned char item) { return item >= 0x20U && item < 0x7fU; });
  }
  return false;
 }
};
...
[[nodiscard]] constexpr bool bounded_text(const std::string_view value, const std::size_t maximum) noexcept { return value.size() <= maximum; }
[[nodiscard]] inline bool satisfies(const std::filesystem::path& value, const FieldConstraint& constraint) noexcept {
 return value.native().size() >= constraint.minimum_bytes && (constraint.maximum_bytes == 0U || value.native().size() <= constraint.maximum_bytes);
}
