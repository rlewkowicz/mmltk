...
void append_number(std::vector<std::string>& arguments, const std::string_view name, const int value) {
...
void append_number(std::vector<std::string>& arguments, const std::string_view name, const float value) {
    ...
    const auto converted =
        std::to_chars(text.data(), text.data() + text.size(), value, std::chars_format::general, std::numeric_limits<float>::max_digits10);
...
void append_flag(std::vector<std::string>& arguments, const std::string_view enabled_name, const std::string_view disabled_name,
                 const bool enabled) {
...
[[nodiscard]] std::string train_recipe_option_name(const std::string_view member_name) {
    ...
    for (const char character : member_name)
...
void append_train_recipe_value(std::vector<std::string>& arguments, const std::string_view name, const Value& value) {
    if constexpr (std::is_enum_v<Value>) {
        append_value(arguments, name,
                     require_enum_spelling(mmltk::backend::models::rfdetr::cli_enum_spelling(value),
                                           "RF-DETR train request has an invalid recipe enum"));
    } else {
...
std::vector<std::string> build_train_command_arguments(const mmltk::backend::models::rfdetr::TrainRequest& request,
    ...
    append_value(arguments, "--optimizer",
                 require_enum_spelling(mmltk::backend::models::rfdetr::cli_enum_spelling(effective.optimizer),
                                       "RF-DETR train request has an invalid optimizer"));
    ...
    append_value(arguments, "--assignment",
                 require_enum_spelling(mmltk::backend::models::rfdetr::cli_enum_spelling(supervision.assignment),
                                       "RF-DETR train request has an invalid assignment"));
    ...
    constexpr std::array transport_options{
        reflection::negative_flag<Request, &Request::h2d_dataloader>("--gdrcopy", "Use GDRCopy image loading", "Execution")};
