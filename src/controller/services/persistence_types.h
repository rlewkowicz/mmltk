...
[[nodiscard]] const mmltk::controller::contracts::GuiSettingsState* view_persistence_settings(
    const PersistenceSettingsSnapshot& snapshot) noexcept;
[[nodiscard]] bool take_persistence_settings(PersistenceSettingsSnapshot& snapshot,
                                             mmltk::controller::contracts::GuiSettingsState& destination) noexcept;
...
inline const mmltk::controller::contracts::GuiSettingsState* view_persistence_settings(
    const PersistenceSettingsSnapshot& snapshot) noexcept {
    return snapshot.size() == 1U ? &snapshot.front() : nullptr;
}

inline bool take_persistence_settings(PersistenceSettingsSnapshot& snapshot,
                                      mmltk::controller::contracts::GuiSettingsState& destination) noexcept {
