...
class RuntimeVastOperations final : public VastOperations {
   public:
    std::vector<VastOfferSummary> query(const VastQueryConfig& config,
                                        const std::vector<mmltk::controller::contracts::ProviderGpuFamily>& families,
                                        const VastBridgeInvocation& invocation) const override {
        return query_vast_offers(config, families, invocation);
    }
    VastCreateInstanceResult create(const VastBridgeConfig& config, const int offer, const std::string_view image,
                                    const VastLaunchTemplateOptions& options, const VastBridgeInvocation& invocation) const override {
...
void validate_config(const VastBridgeConfig& config) {
    if (config.api_key.empty() || config.api_key.size() > kProviderApiKeyCapacity || config.python_executable.empty() ||
        config.bridge_script_path.empty() || config.python_executable.native().size() > kProviderFieldCapacity ||
        config.bridge_script_path.native().size() > kProviderFieldCapacity || config.http_timeout <= std::chrono::milliseconds::zero()) {
...
VastProviderOwner::VastProviderOwner(VastBridgeConfig config) {
    register_owner(std::move(config), std::make_shared<RuntimeVastOperations>());
}
...
void VastProviderOwner::register_owner(VastBridgeConfig config, std::shared_ptr<const VastOperations> operations) {
    ...
    client_ = VastProviderClient{
        std::make_shared<const VastProviderState>(VastProviderState{.config = std::move(config), .operations = std::move(operations)})};
}
