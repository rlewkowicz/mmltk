...
VastReconciliation VastClient::reconcile(const VastReconciliationRequest& request, const VastCancellationToken& cancellation) const {
    ...
    VastInstanceInfo record =
        provider.operations->show(provider.config, request.instance_id, cancellation_invocation(cancellation, provider.config.http_timeout));
    validate_exact_instance(record, request.instance_id);
    std::string state =
        record.actual_status + " " + record.current_state + " " + record.intended_status + " " + record.next_state + " " + record.status_message;
