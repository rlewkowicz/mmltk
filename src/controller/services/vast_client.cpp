...
[[nodiscard]] bool valid_family(const mmltk::controller::contracts::ProviderGpuFamily family) noexcept {
    return static_cast<std::underlying_type_t<mmltk::controller::contracts::ProviderGpuFamily>>(family) <=
           static_cast<std::underlying_type_t<mmltk::controller::contracts::ProviderGpuFamily>>(
...
void validate_query(const VastQueryConfig& config, const std::vector<mmltk::controller::contracts::ProviderGpuFamily>& families) {
    ...
    for (std::size_t index = 0U; index < families.size(); ++index) {
        if (!valid_family(families[index]) || std::ranges::find(families.begin(), families.begin() + static_cast<std::ptrdiff_t>(index),
                                                                families[index]) != families.begin() + static_cast<std::ptrdiff_t>(index)) {
...
void validate_template(const VastLaunchTemplateOptions& options) {
    const std::array<const std::string*, 10U> fields{&options.user,        &options.login,        &options.label,       &options.onstart,
                                                     &options.onstart_cmd, &options.entrypoint,   &options.jupyter_dir, &options.extra,
                                                     &options.env,         &options.template_hash};
    std::size_t total = 0U;
...
void validate_instance(const VastInstanceInfo& instance) {
    const std::array<const std::string*, 12U> fields{
        &instance.actual_status, &instance.current_state,  &instance.next_state,    &instance.intended_status,
        &instance.image_uuid,    &instance.label,          &instance.gpu_name,      &instance.ssh_host,
        &instance.public_ipaddr, &instance.status_message, &instance.jupyter_token, &instance.ports,
    };
...
VastOffers bounded_offers(std::vector<VastOfferSummary> source, const std::size_t requested_limit) {
    ...
    for (std::size_t index = 0U; index < source.size(); ++index) {
        ...
        if (!valid_family(offer.family) || offer.num_gpus <= 0 || !std::isfinite(offer.gpu_ram) || !std::isfinite(offer.dph) ||
            !std::isfinite(offer.dlperf) || !std::isfinite(offer.dlperf_usd) || !std::isfinite(offer.reliability) ||
            !std::isfinite(offer.inet_down) || !std::isfinite(offer.disk_space)) {
...
[[nodiscard]] VastBridgeInvocation cancellation_invocation(const VastCancellationToken& cancellation,
...
namespace {
void validate_create(const int offer_id, const std::string_view image, const std::string_view launch_token,
                     const VastLaunchTemplateOptions& options) {
    if (offer_id <= 0 || image.empty() || image.size() > kVastImageCapacity || launch_token.empty() ||
        launch_token.size() > kVastLaunchTokenCapacity || (!options.label.empty() && options.label != launch_token)) {
...
contracts::ProviderQueryResult materialize_provider_query_result(const std::span<const VastOfferSummary> offers) {
    const auto failed = [](const std::string_view detail) {
        return contracts::ProviderQueryResult{.outcome = contracts::ProviderQueryOutcome::Failed,
                                              .detail = contracts::bounded_provider_detail(detail)};
    };
...
VastOffers VastClient::query(const mmltk::controller::contracts::ProviderPreferences& preferences,
    ...
    for (const auto family : preferences.families)
    ...
    return bounded_offers(provider.operations->query(config, families, cancellation_invocation(cancellation, config.http_timeout)),
                          config.result_limit);
}
VastCreateInstanceResult VastClient::create(const int offer_id, const mmltk::controller::contracts::ProviderPreferences& preferences,
                                            const std::string_view launch_token, VastEffectAttempt& attempt,
                                            const VastCancellationToken& cancellation) const {
    const auto& provider = ResolveProvider();
    if (!preferences.valid()) throw std::invalid_argument("invalid provider preferences");
    ...
    attempt.MarkStarted();
    VastCreateInstanceResult result = provider.operations->create(provider.config, offer_id, preferences.image, options,
                                                                  cancellation_invocation(cancellation, provider.config.http_timeout));
...
void VastClient::mutate(const mmltk::controller::contracts::ProviderMutation mutation, const int instance_id, VastEffectAttempt& attempt,
    ...
    attempt.MarkStarted();
    if (mutation == mmltk::controller::contracts::ProviderMutation::Start)
        provider.operations->start(provider.config, instance_id, invocation);
    if (mutation == mmltk::controller::contracts::ProviderMutation::Stop)
        provider.operations->stop(provider.config, instance_id, invocation);
}
VastReconciliation VastClient::reconcile(const VastReconciliationRequest& request, const VastCancellationToken& cancellation) const {
    ...
    if (request.mutation == mmltk::controller::contracts::ProviderMutation::Create) {
        VastInventory inventory = bounded_inventory(
            provider.operations->inventory(provider.config, cancellation_invocation(cancellation, provider.config.http_timeout)));
    ...
    VastInstanceInfo record = provider.operations->show(provider.config, request.instance_id,
                                                        cancellation_invocation(cancellation, provider.config.http_timeout));
    validate_exact_instance(record, request.instance_id);
    std::string state = record.actual_status + " " + record.current_state + " " + record.intended_status + " " + record.next_state + " " +
...
VastInstanceInfo VastClient::instance(const int instance_id, const VastCancellationToken& cancellation) const {
    ...
    VastInstanceInfo result =
        provider.operations->show(provider.config, instance_id, cancellation_invocation(cancellation, provider.config.http_timeout));
...
std::string VastClient::logs(const int instance_id, const std::optional<std::size_t> tail,
                             const VastCancellationToken& cancellation) const {
    const auto& provider = ResolveProvider();
    if (instance_id <= 0 || (tail && (*tail == 0U || *tail > kVastLogTailLineLimit)))
        throw std::invalid_argument("invalid Vast log request");
    if (cancellation.cancelled()) throw VastBridgeError(VastBridgeFailureKind::Cancelled, "Vast log query cancelled");
    auto output =
        provider.operations->logs(provider.config, instance_id, tail, cancellation_invocation(cancellation, provider.config.http_timeout));
