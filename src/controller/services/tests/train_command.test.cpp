...
void check_progress_flag_forwarding(const bool progress_bar, const std::string_view expected_present,
                                    const std::string_view expected_absent) {
    mmltk::backend::models::rfdetr::TrainRequest request = make_train_request({1});
...
void test_supervision_combinations_are_forwarded_with_exact_values() {
    ...
    for (const auto& [assignment, denoising, expected_assignment, expected_dn] :
         {std::tuple{TrainAssignmentKind::Hungarian, false, "hungarian", "--no-dn"},
          std::tuple{TrainAssignmentKind::Hungarian, true, "hungarian", "--dn"},
          std::tuple{TrainAssignmentKind::MatchFree, false, "match-free", "--no-dn"},
          std::tuple{TrainAssignmentKind::MatchFree, true, "match-free", "--dn"}}) {
        ...
        assert_float_flag_round_trip(args, "--match-free-correspondence-weight",
                                     request.training_supervision.match_free.correspondence_weight);
...
void test_muon_recipe_defaults_are_resolved() {
    const auto recipe = mmltk::backend::models::rfdetr::resolve_train_recipe("rf-detr-seg-medium",
                                                                             mmltk::backend::models::rfdetr::TrainOptimizerKind::Muon);
...
void test_recipe_application_respects_overrides() {
    ...
    TrainRecipeRelation::template set_override<
        mmltk::frameworks::reflection::member_path<&mmltk::backend::models::rfdetr::TrainRequest::lr>>(overrides);
    mmltk::backend::models::rfdetr::apply_train_recipe(
        options,
        mmltk::backend::models::rfdetr::resolve_train_recipe("rf-detr-medium", mmltk::backend::models::rfdetr::TrainOptimizerKind::Muon),
        overrides);
...
MMLTK_REGISTER_TEST_CASE("[gui][train_command][training_supervision]",
