...
void test_drain_nonblocking_fd_reads_available_output() {
    ...
    const std::string output =
        mmltk::controller::services::console_output::read_fd(pipe.read_descriptor(), "failed to read test output pipe: ", true);
...
void test_read_fd_to_string_reads_until_eof() {
    ...
    const std::string output =
        mmltk::controller::services::console_output::read_fd(pipe.read_descriptor(), "failed to read test blocking output pipe: ");
...
TEST_CASE("console output drains a nonblocking descriptor", "[controller][services][console-output]") {
    test_drain_nonblocking_fd_reads_available_output();
}
