...
void test_explore_preview_candidate_is_atomic_and_persists_native_modes() {
    ...
    CHECK_THROWS_AS(settings.persist_explore_filter(initial, {}), mmltk::controller::contracts::BusyError);
    const auto filtered = settings.persist_explore_filter(detail, {.filter = {.minimum_instances = 1U}, .overlay = {.show_boxes = false}});
