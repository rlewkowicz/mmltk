...
[[nodiscard]] std::string read_file(const std::filesystem::path& path) {
 ...
 stream.seekg(0);
 if (!contents.empty() && !stream.read(contents.data(), static_cast<std::streamsize>(contents.size()))) { throw std::runtime_error("cannot read test output"); }
 return contents;
}
...
TEST_CASE("bounded runtime projection escapes valid text and rejects malformed UTF8", "[gui][services]") {
 ...
 for (const std::string_view malformed : mmltk::testsupport::kMalformedUtf8) { runtime.write({.event = "text", .message = malformed}); }
 CHECK(diagnostics.counters().accepted == 1U);
 diagnostics.close();
...
TEST_CASE("file dialog selected results require an owned bounded nonempty path", "[gui][services]") {
 ...
 const FileDialogSelection oversized{.target = target, .result = FileDialogSelected{std::string(mmltk::frameworks::reflection::kMaximumPathBytes + 1U, 'x')}};
