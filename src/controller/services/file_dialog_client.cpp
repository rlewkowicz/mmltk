...
[[nodiscard]] FileDialogResult dialog_result(const FileDialogDisposition disposition, const std::string_view path = {},
...
[[nodiscard]] DialogScope make_scope(const PathText& launch_directory) {
    DialogScope scope{.container_root = canonical_path(std::filesystem::path{std::string{launch_directory.view()}}),
                      .helper_root = {},
                      .host_mounted = false};
...
[[nodiscard]] std::string_view trim(const std::string_view value) noexcept {
    ...
    std::size_t first = 0U;
    while (first < value.size() && whitespace(static_cast<unsigned char>(value[first])))
        ++first;
    std::size_t last = value.size();
    while (last > first && whitespace(static_cast<unsigned char>(value[last - 1U])))
        --last;
...
[[nodiscard]] PathText selected_path(const FixedOutput& output, const DialogScope& scope) {
    ...
    if (selected.find('\0') != std::string_view::npos) {
        throw std::runtime_error("file-dialog helper returned a path containing a null byte");
    }
...
[[nodiscard]] bool executable_program(const PathText& helper) noexcept {
    struct stat status{};
    return helper.valid() && ::stat(helper.bytes.data(), &status) == 0 && S_ISREG(status.st_mode) &&
           ::access(helper.bytes.data(), X_OK) == 0;
}
...
[[nodiscard]] PathText resolve_helper_program(const PathText& configured) {
    ...
    while (true) {
        ...
        if (candidate.valid() && append(candidate, "/") && append(candidate, configured.view()) && executable_program(candidate)) {
            return candidate;
        }
...
[[nodiscard]] bool build_arguments(const PathText& helper, const FileDialogRequest& request, const DialogScope& scope,
    ...
    switch (request.mode) {
        case mmltk::controller::contracts::FileDialogMode::OpenFile:
            break;
    ...
    if (!append(filter, request.filter.name.view()) || !append(filter, " | ") || !append(filter, request.filter.pattern.view()) ||
        !add(filter.view())) {
        return false;
    }
    return true;
...
[[nodiscard]] ChildSetupError setup_error(const mmltk::common::io::ScopedFd& descriptor) {
    ...
    if (!append(message, setup_stage_name(failure.stage)) || !append(message, " failed: ") ||
        !append(message, std::strerror(failure.error_number))) {
        message = PolicyText::From("file-dialog helper setup failed");
    }
    return {.message = message,
            .failure = failure.stage == ChildSetupStage::Exec ? FileDialogFailure::Exec : FileDialogFailure::ChildSetup};
}
...
void drain_output_to_eof(mmltk::common::io::ScopedFd& descriptor, FixedOutput& output) {
    while (drain_output_available(descriptor, output) != OutputDrainState::Closed) {
        ...
        if (ready < 0 || (event.revents & POLLNVAL) != 0) {
            throw std::runtime_error("file-dialog output drain failed: " + std::string{std::strerror(errno)});
        }
    }
}
...
bool FileDialogClient::valid() const noexcept {
    return configuration_ && configuration_->helper.valid() && configuration_->launch_directory.valid();
}
...
FileDialogResult FileDialogClient::run(const FileDialogRequest& request, FileDialogCancellationToken cancellation) const noexcept {
    ...
    try {
        ...
        if (!configuration || !request.valid()) {
            return dialog_result(FileDialogDisposition::Failed, {},
                                 configuration ? "invalid file-dialog request" : "file-dialog capability is unavailable",
        ...
        if (cancellation_descriptor.get() < 0 ||
            mmltk::common::concurrency::detail::cancellation_descriptor_requested(cancellation_descriptor.get())) {
        ...
        if (!build_arguments(helper, request, scope, arguments, argument_count)) {
            return dialog_result(FileDialogDisposition::Failed, {}, "file-dialog argument exceeded capacity",
                                 FileDialogFailure::InvalidRequest);
        }
        std::array<char*, kArgumentCapacity + 1U> arguments_view{};
        for (std::size_t index = 0U; index < argument_count; ++index) {
            arguments_view[index] = arguments[index].bytes.data();
        }
        ...
        if (child == 0) {
            ...
            if (null_descriptor < 0 || ::dup2(null_descriptor, STDERR_FILENO) < 0) {
                fail_child_setup(child_setup_write, ChildSetupStage::RedirectErrors);
            }
        ...
        while (!child_exited) {
            ...
            if ((descriptors[0].revents & (POLLIN | POLLHUP | POLLERR)) != 0) {
                static_cast<void>(drain_output_available(output_read, output));
            }
        ...
        if (child_setup_error.valid()) {
            return dialog_result(FileDialogDisposition::Failed, {}, child_setup_error.message.view(), child_setup_error.failure);
        }
        if (!WIFEXITED(status)) {
            return dialog_result(FileDialogDisposition::Failed, {}, "file-dialog helper terminated by signal",
                                 FileDialogFailure::ProcessExit);
        }
        const int exit_code = WEXITSTATUS(status);
        if (exit_code == 1) return dialog_result(FileDialogDisposition::Cancelled);
        if (exit_code != 0) {
            return dialog_result(FileDialogDisposition::Failed, {}, "file-dialog helper rejected request", FileDialogFailure::ProcessExit);
        }
        const PathText path = selected_path(output, scope);
        return path.valid()
                   ? FileDialogResult{.disposition = FileDialogDisposition::Selected, .failure = FileDialogFailure::None, .path = path}
