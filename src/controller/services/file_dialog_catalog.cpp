    ...
    mmltk::controller::contracts::ModelSelectionRelation::VisitRows(
        [&]<class Relation>(const mmltk::controller::contracts::ModelSelectionCompatibility& compatibility) {
            ...
            constexpr auto artifact_path =
                mmltk::frameworks::reflection::reflected_member_path<typename ApplicationSchema::settings_type, Relation::artifact>();
...
FileDialogCatalog FileDialogCatalog::Create(const std::span<const FileDialogDescriptor> entries) {
    ...
    for (std::size_t left = 0U; left < entries.size(); ++left) {
        ...
        if (entry.stable_id == 0U ||
            entry.stable_id != mmltk::controller::browser::application_settings_field_stable_id(entry.field_path.view()) ||
            !entry.field_path.valid() || !entry.workflows.valid() || !entry.title.valid() || !entry.filter.name.valid() ||
            !entry.filter.pattern.valid() || !mmltk::frameworks::reflection::enum_contains(entry.mode) ||
...
std::optional<mmltk::controller::contracts::SettingsValueUpdate> resolve_file_dialog_path(const std::uint64_t stable_field_id,
    ...
    const auto entry = std::ranges::find_if(
        entries, [stable_field_id](const auto& value) { return value.stable_id == stable_field_id && !value.defer_apply(); });
