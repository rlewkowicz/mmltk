...
struct DiagnosticsClient::State final {
 ...
 [[nodiscard]] DiagnosticSubmitResult admit_locked(std::unique_lock<std::mutex>& lock, std::size_t count, bool wait) noexcept;
...
void DiagnosticsClient::State::run() noexcept {
 ...
 if (::sigemptyset(&blocked_signals) != 0 || ::sigaddset(&blocked_signals, SIGPIPE) != 0 || ::pthread_sigmask(SIG_BLOCK, &blocked_signals, nullptr) != 0) {
 ...
 for (;;) {
  ...
  ssize_t written = -1;
  do { written = ::write(descriptor.get(), bytes, remaining); } while (written < 0 && errno == EINTR);
  if (written > 0) {
...
DiagnosticsProducer::Operation::Operation(std::shared_ptr<DiagnosticsClient::State> state, const bool complete) noexcept : state_(std::move(state)) {
...
bool DiagnosticsProducer::Operation::enabled() const noexcept { return state_ != nullptr && state_->enabled.load(std::memory_order_acquire); }
void DiagnosticsProducer::Operation::fail_delivery() const noexcept {
 ...
 if (state_->policy == DiagnosticsExecutionPolicy::BackgroundWriter)
  state_->signal_locked();
 else
  state_->publish_terminal_locked();
}
...
DiagnosticSubmitResult DiagnosticsProducer::Operation::try_submit(const DiagnosticRecord record) const noexcept { return submit(record, false); }
...
DiagnosticSubmitResult DiagnosticsProducer::Operation::submit(const DiagnosticRecord record, const bool wait_for_capacity, const bool validate) const noexcept {
...
DiagnosticSubmitResult DiagnosticsProducer::Operation::submit_encoded_batch(const std::size_t count, void* const context, const EncodedBatchWriter writer, const bool complete) const noexcept {
 ...
 std::unique_lock lock(state_->mutex);
 if (complete && state_->policy == DiagnosticsExecutionPolicy::CallerDriven) return DiagnosticSubmitResult::Capacity;
 ...
 for (std::size_t index = 0U; index < count; ++index) {
  ...
  if (!writer(context, index, target.bytes, encoded_size) || encoded_size == 0U || encoded_size > DiagnosticsClient::kRecordCapacity) {
   state_->dropped.fetch_add(count, std::memory_order_relaxed);
   return DiagnosticSubmitResult::RecordTooLarge;
  }
