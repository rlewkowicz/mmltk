...
[[nodiscard]] std::string_view owner_name(const RuntimeDiagnosticOwner owner) noexcept {
    switch (owner) {
        case RuntimeDiagnosticOwner::BrowserRuntime:
            return "browser_runtime";
        case RuntimeDiagnosticOwner::BrowserServer:
            return "browser_server";
        case RuntimeDiagnosticOwner::FirefoxProcess:
            return "firefox_process";
        case RuntimeDiagnosticOwner::Explore:
            return "explore";
        case RuntimeDiagnosticOwner::Annotation:
            return "annotation";
        case RuntimeDiagnosticOwner::Upscale:
            return "upscale";
        case RuntimeDiagnosticOwner::Live:
            return "live";
        case RuntimeDiagnosticOwner::Presentation:
            return "presentation";
        case RuntimeDiagnosticOwner::AnnotationResource:
...
struct RuntimeDiagnosticTarget::State final {
    explicit State(DiagnosticsProducer producer_in) noexcept : producer(std::move(producer_in)) {}
...
RuntimeDiagnostics::RuntimeDiagnostics(DiagnosticsProducer producer) noexcept
    : state_(std::make_shared<RuntimeDiagnosticTarget::State>(std::move(producer))) {}
    ...
    try {
        ...
        const std::string record =
            nlohmann::json{
                ...
                {"detail", fact.detail},
                {"device", fact.device},
                {"capacity_width", fact.context.capacity_width},
                {"capacity_height", fact.context.capacity_height},
                {"staging_bytes", fact.context.staging_bytes},
                {"surface_high", fact.context.surface_high},
                {"surface_low", fact.context.surface_low},
                {"selection_generation", fact.context.selection_generation},
                {"frame_revision", fact.context.frame_revision},
                {"condition", fact.context.condition},
                {"outcome", fact.context.outcome},
                {"message", fact.message},
                {"steady_ns", steady_ns},
            }
                .dump();
        static_cast<void>(operation.submit({.json = record}));
    } catch (...) {}
}
...
void RuntimeDiagnosticTarget::State::write_browser_event(const std::string_view event,
    ...
    static_cast<void>(operation.submit({.json = writer.view()}));
}
