...
def _make_client() -> VastAI:
    timeout_text = os.getenv("MMLTK_VAST_HTTP_TIMEOUT_SECONDS", "")
    if not timeout_text:
        raise RuntimeError("MMLTK_VAST_HTTP_TIMEOUT_SECONDS is required")
    timeout_seconds = float(timeout_text)
    if timeout_seconds <= 0:
        raise RuntimeError("MMLTK_VAST_HTTP_TIMEOUT_SECONDS must be positive")

    client = VastAI(api_key=_require_api_key(), raw=True)
...
def main() -> int:
    ...
    try:
        ...
        print(f"Vast response parse failure: {error}", file=sys.stderr)
        return 65
    except Exception as error:
        print(f"Vast API failure: {error}", file=sys.stderr)
        return 70
