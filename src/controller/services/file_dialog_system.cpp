...
namespace {
[[nodiscard]] std::string bounded_dialog_detail(const std::string_view detail) {
    return std::string{detail.substr(0, services::kFileDialogTextCapacity)};
}
}  // namespace
...
services::FileDialogSelection NativeFileDialogRuntime::Open(const services::ResolvedFileDialog& resolved, const std::stop_token stop) {
    ...
    if (result.disposition == services::FileDialogDisposition::Cancelled || result.disposition == services::FileDialogDisposition::Reaped)
        return selection;
    if (result.disposition != services::FileDialogDisposition::Selected || !result.path.valid())
        throw contracts::FailedError("file dialog operation failed");
    ...
    auto update = services::resolve_file_dialog_path(services::file_dialog_stable_id(selection.target),
                                                     std::get<services::FileDialogSelected>(selection.result).path);
...
FileDialogSystem::FileDialogSystem(RuntimeFactory factory, SystemEventSink<event_type> events)
    : factory_(std::move(factory)), events_(std::move(events)) {
...
FileDialogSnapshot FileDialogSystem::Open(const services::FileDialogOpen request) {
    ...
    run_.Start({
        ...
        .work = [this, resolved = *resolved](const std::stop_token stop) -> direct::LocalRun::Notification {
            ...
            if (failed)
                return [this, settled = std::move(settled), detail = std::move(detail)]() mutable noexcept {
                    direct::PublishLazyNoexcept(events_,
                                                [&] { return event_type{FileDialogFailed{std::move(settled), std::move(detail)}}; });
                };
        ...
        .failure = [this](std::exception_ptr) -> direct::LocalRun::Notification {
            ...
            return [this, settled = std::move(settled)]() mutable noexcept {
                direct::PublishLazyNoexcept(events_,
                                            [&] { return event_type{FileDialogFailed{std::move(settled), "file dialog worker failed"}}; });
            };
        },
    });
    return snapshot();
