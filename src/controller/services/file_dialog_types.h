...
struct BoundedText final {
    ...
    [[nodiscard]] constexpr std::string_view view() const noexcept {
        return valid() ? std::string_view{bytes.data(), size} : std::string_view{};
    }
    ...
    return std::visit(
        [](const auto& value) {
            ...
            if constexpr (std::same_as<std::remove_cvref_t<decltype(value)>, ModelArtifactTarget>) {
                return mmltk::controller::contracts::valid_feature(value.workflow) &&
                       value.input != mmltk::backend::models::catalog::ModelArtifactInputKind::None;
            }
            return true;
        },
        target.value);
