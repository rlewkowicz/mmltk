...
public:
 ...
 void reserve(const std::string_view name_space, const std::string_view symbol, const std::string_view source) { symbols_.Reserve(name_space, symbol, source); }
...
private:
  ...
  if constexpr (schema::Optional<Type>::value) {
   ...
   if constexpr (mmltk::frameworks::reflection::kOpaqueRelationStorage<Type>) {
    ...
    output_ << "#[derive(Debug, Clone, PartialEq)]\npub struct " << name << "(" << storage_type << ");\nimpl IntoApplicationValue for " << name
            ...
            << storage_type << " = FromApplicationValue::from_application_value(take_field(&mut fields, \"" << member_name << "\")?)?; if storage > " << maximum
 ...
 void EmitConstraint(const std::string& member, const mmltk::frameworks::reflection::FieldConstraint constraint) {
  ...
  if constexpr (schema::Optional<Type>::value) {
   ...
   if constexpr (std::same_as<Type, mmltk::frameworks::serialization::wire::FlatValue> || std::same_as<Type, mmltk::frameworks::serialization::wire::Value>) {
   ...
   VisitRustFields<Loading>([&]<class Field, class LoadingDeclaration>(const auto&, const std::string& member) {
    ...
    Schema::VisitApplicationSettingsLeaves([&]<class Owner, class Declaration, class Member>(const auto& fact) {
     if constexpr (std::same_as<LoadingDeclaration, Declaration>) {
      bool applies = false;
      for (std::size_t index = 0; index < fact.workflows.count; ++index) applies = applies || fact.workflows.workflows[index] == feature.value;
      if (!applies) return;
      ++matches;
 ...
 void EmitInteractionEnvelope() {
  ...
  visit_interaction_opcodes<ApplicationSystems>([&]<class Endpoint>(const auto opcode) { output_ << opcode << " => Some(" << Endpoint::stable_id << "),\n"; });
  output_ << "_ => None } }\n";
 ...
 void EmitRecordProjection(bool required, bool apply) {
  ...
  output_ << "impl From<&" << rust_type<Source>() << "> for " << rust_type<Target>() << " { fn from(source: &" << rust_type<Source>() << ") -> Self { Self {\n";
