...
void emit_application_outer_routing(Writer& writer) {
    ...
    const auto system_trait = [&writer](const std::string_view system) { return writer.identifier(system, true) + "ApplicationProjection"; };
    ...
    Schema::VisitEndpoints([&]<class Endpoint>() {
        const auto symbol = "ENDPOINT_" + writer.identifier(Endpoint::system_cell::name, true) + '_' + writer.identifier(Endpoint::name, true);
    ...
    Schema::VisitEvents([&]<class Identity, class Event>(const auto&) {
        ...
        writer.reserve("enum ApplicationEvent", name, "event " + std::string(Identity::system_cell::name) + "." + writer.template native_source<Event>());
    ...
    output << "}\npub const fn application_event_system(value: &ApplicationEvent) -> ApplicationSystem { "
              "match value {\n";
    Schema::VisitEvents([&]<class Identity, class Event>(const auto&) {
        output << "    ApplicationEvent::" << variant(Identity::system_cell::name, mmltk::frameworks::serialization::reflected_schema_type_name<Event>())
    ...
    Schema::VisitEndpoints([&]<class Endpoint>() {
        if constexpr (!Endpoint::interaction) {
            ...
            writer.reserve("enum ApplicationReply", name, "endpoint " + std::string(Endpoint::system_cell::name) + "." + std::string(Endpoint::name));
    ...
    Schema::VisitEvents([&]<class Identity, class Event>(const auto&) {
        output << "    (" << Identity::system_id << ", " << Identity::event_id
               << ") => Ok(ApplicationEvent::" << variant(Identity::system_cell::name, mmltk::frameworks::serialization::reflected_schema_type_name<Event>())
               << "(FromApplicationValue::from_application_transport_value(value)?)),\n";
    });
