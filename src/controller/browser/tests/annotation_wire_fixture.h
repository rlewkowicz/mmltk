...
inline contracts::AnnotationObject make_annotation_wire_object(const contracts::AnnotationPoint point,
                                                               const contracts::AnnotationColor color) {
    ...
    return {
        ...
        .spline_knots = std::vector<AnnotationSplineKnot>(
            kAnnotationGeometryCapacity,
            {.point = point, .in = {.point = point, .enabled = true}, .out = {.point = point, .enabled = true}}),
        .skeleton_nodes = std::vector<AnnotationSkeletonNode>(
            kAnnotationGeometryCapacity, {.key = AnnotationText::From(std::string(kAnnotationNameCapacity, 'k')), .point = point}),
