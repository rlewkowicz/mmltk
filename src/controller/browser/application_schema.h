            ...
            if constexpr (vocabulary::is_leaf_v<Member>) {
                ...
                constexpr auto dialog = [&] {
                    if constexpr (constexpr auto workflow_dialog = mmltk::controller::contracts::workflow_path_dialog<path>(); workflow_dialog)
                        return std::optional<ApplicationFileDialogFact>{
                            {workflow_dialog->title, workflow_dialog->filter, workflow_dialog->pattern, workflow_dialog->mode}};
                    else
...
[[nodiscard]] consteval bool runtime_scalar_projectable() {
    ...
    return std::same_as<Type, void> || mmltk::frameworks::reflection::kReflectedIntegerScalar<Type> || std::same_as<Type, float> ||
           std::same_as<Type, double> || std::same_as<Type, std::byte> || std::same_as<Type, std::string> || std::same_as<Type, std::filesystem::path> ||
            ...
            if constexpr (requires { Declaration::pointer; }) {
                ...
                auto provider_visitor = [&]<class Provider>() {
                    if constexpr (mmltk::frameworks::reflection::HasCatalogProviderRelation<Provider>) {
                        ...
                        static_assert(
                            std::same_as<Member,
                                         mmltk::frameworks::reflection::accessor_value_t<typename Relation::destination_type, Relation::destination_selector>>);
                        constexpr auto override_path = mmltk::frameworks::reflection::rebase_member_path<Root, typename Relation::destination_type>(
