...
[[nodiscard]] consteval bool is_endpoint_method() {
 ...
 if constexpr (annotations == 0U) {
  ...
  static_assert(endpoint_member_eligible<Member>(), "endpoint annotation requires a public, user-declared, non-static ordinary method");
...
consteval void visit_endpoint_members(Visitor& visitor) {
 template for (constexpr auto member : std::define_static_array(std::meta::members_of(^^System, std::meta::access_context::unchecked()))) {
...
[[nodiscard]] constexpr mmltk::controller::contracts::reflection::ReflectedWorkflowPolicy all_feature_workflows() noexcept {
 ...
 for (std::size_t index = 0U; index < result.count; ++index) result.workflows[index] = static_cast<mmltk::controller::contracts::FeatureId>(index);
 return result;
}
...
[[nodiscard]] consteval auto declaration_metadata() {
 ...
 Declaration::VisitAnnotations([&]<class Annotation>(const Annotation& annotation) {
  ...
  if constexpr (mmltk::controller::contracts::reflection::is_feature_scope_annotation<A>) {
   ...
   for (std::size_t index = 0U; index < policy.count; ++index) policy.workflows[index] = annotation.values[index];
...
[[nodiscard]] consteval bool declaration_has_annotation() {
 std::size_t count = 0U;
 Declaration::VisitAnnotations([&]<class Actual>(const Actual&) { count += std::same_as<std::remove_cvref_t<Actual>, Annotation> ? 1U : 0U; });
...
constexpr void visit_settings_leaves(Visitor& visitor, std::string prefix, const mmltk::controller::contracts::reflection::ReflectedWorkflowPolicy inherited) {
...
template <std::meta::info Method>
concept SupportedSnapshotMethod = requires { typename SystemMethodSignature<std::remove_cv_t<decltype(&[:Method:])>>::owner_type; } && [] consteval {
...
constexpr void visit_declaration_catalog_providers(Visitor& visitor) {
 Declaration::VisitAnnotations([&]<class Annotation>(const Annotation&) {
  using A = std::remove_cvref_t<Annotation>;
  if constexpr (mmltk::frameworks::reflection::is_catalog_provider_annotation<A>) visitor.template operator()<typename A::provider_type>();
 });
}
template <class Value, class Visitor>
...
constexpr void visit_settings_relations(Visitor& visitor) {
 ...
 if constexpr (vocabulary::is_leaf_v<Type> || mmltk::frameworks::reflection::kOpaqueRelationStorage<Type>) {
  ...
  mmltk::frameworks::reflection::visit_materialized_bases<Type>([&]<class Base>() { visit_settings_relations<Root, Base, Prefix...>(visitor); });
  mmltk::frameworks::reflection::visit_materialized_members<Type>([&]<class Declaration>(const auto&) {
   if constexpr (requires { Declaration::pointer; }) {
    ...
    if constexpr (!vocabulary::is_leaf_v<Member>) visit_settings_relations<Root, Member, Prefix..., Declaration::pointer>(visitor);
   }
  });
 }
}
...
[[nodiscard]] consteval bool audit_settings_relation(std::vector<mmltk::frameworks::reflection::ReflectedMemberIdentity>& claims) {
 ...
 if constexpr (!requires {
  ...
  constexpr auto canonical_selector = rebase_member_path<Root, typename Relation::destination_type>(Selector, Relation::destination_selector);
  if (accessor_member_identity<Root, Selector>() != accessor_member_identity<Root, canonical_selector>()) return false;
  constexpr auto override_path = rebase_member_path<Root, typename Relation::destination_type>(Selector, Relation::destination_override_state);
  ...
  Relation::VisitMembers([&]<class Entry>() {
   ...
   for (const auto& prior : destinations) valid = valid && prior != identity;
...
[[nodiscard]] consteval bool settings_relations_are_valid() {
 ...
 for (const auto& storage : opaque) {
  std::size_t count = 0U;
  for (const auto& claim : claims) count += claim == storage ? 1U : 0U;
...
public:
 ...
 void append(const std::string_view value) noexcept {
  ...
  for (const unsigned char byte : value) append_byte(byte);
 }
...
void append_value(FingerprintSink& sink, const Value& value) {
 ...
 if constexpr (std::same_as<Type, bool> || std::is_arithmetic_v<Type>) {
  ...
  for (const auto& item : value) append_value(sink, item);
 } else if constexpr (Variant<Type>::value) {
...
void append_annotation(FingerprintSink& sink, const Annotation& annotation) {
 ...
 if constexpr (mmltk::frameworks::reflection::MinimumAnnotation<A>::value || mmltk::frameworks::reflection::MaximumAnnotation<A>::value ||
  ...
  for (const auto feature : annotation.values) sink.append_number(static_cast<std::uint8_t>(feature));
 } else if constexpr (mmltk::controller::contracts::reflection::is_file_dialog_annotation<A>) {
  sink.append(A::title);
...
void append_type(FingerprintSink& sink) {
 using Type = std::remove_cvref_t<Value>;
 static_assert(valid_fixed_text_shape<Type>(), "fixed-text annotation requires matching char bytes, unsigned size, capacity, and policy");
 sink.append("type");
...
[[nodiscard]] consteval bool application_event_is_member() {
 ...
 bool found = false;
 application_schema_detail::Variant<typename Cell::type::event_type>::Visit([&]<class Candidate>() { found = found || std::same_as<Candidate, Event>; });
 return found;
}
template <class Composition, auto Member, class Event>
struct ApplicationEventDescriptor final {
 ...
 static_assert(application_schema_detail::annotation_count<^^Event, contracts::reflection::Event>() == 1U, "event requires exactly one delivery annotation");
...
struct ReflectedEndpoint final {
 ...
 static constexpr bool replaceable = [] consteval {
  if constexpr (interaction) return application_schema_detail::annotation_value<Method, contracts::reflection::direct::InteractionEndpoint>().replaceable;
  return false;
 }();
...
struct ApplicationSchema final {
 ...
 static constexpr void VisitSystems(Visitor&& visitor) {
  template for (constexpr auto cell : std::define_static_array(std::meta::nonstatic_data_members_of(^^Composition, std::meta::access_context::unchecked()))) {
   ...
   constexpr auto metadata = application_schema_detail::annotation_value<snapshot, mmltk::controller::contracts::reflection::Snapshot>();
   static_assert(metadata.byte_budget != 0U, "Snapshot annotation requires a positive byte budget");
   static_assert(application_schema_detail::SupportedSnapshotMethod<snapshot>, "Snapshot annotation requires a supported zero-argument, unqualified or const, "
 ...
 static constexpr void VisitEvents(Visitor&& visitor) {
  template for (constexpr auto cell : std::define_static_array(std::meta::nonstatic_data_members_of(^^Composition, std::meta::access_context::unchecked()))) {
   using SystemCell = ReflectedSystem<Composition, cell>;
   static_assert(application_schema_detail::ReflectedEventVariant<typename SystemCell::type>, "ordinary system event_type must be one reflected std::variant");
 ...
 static void VisitSettingsDefaults(const Settings& defaults, Visitor&& visitor) {
  application_schema_detail::visit_settings_defaults<Settings>(visitor, defaults, {}, application_schema_detail::all_feature_workflows());
 }
 template <class Settings, class Visitor>
 ...
 static void VisitCatalogRows(Visitor&& visitor) {
  ...
  Provider::VisitRows([&](const Row& row, const std::size_t index) {
   ...
   if (!row_keys.emplace(key).second) throw std::logic_error("catalog provider emitted duplicate row key `" + std::string(key) + "`");
   ...
   if (!inserted) throw std::logic_error("catalog row identity collision between `" + prior->second + "` and `" + std::string(key) + "`");
 ...
 static void VisitCatalogProviders(Visitor&& visitor) {
  ...
  auto visit_provider = [&]<class Provider>() {
   ...
   if (!inserted) {
    if (prior->second != name) throw std::logic_error("catalog provider identity collision between `" + prior->second + "` and `" + std::string(name) + "`");
    return;
   }
  ...
  const auto visit_type = [&]<class Type>() { application_schema_detail::visit_type_catalog_providers<Type>(visit_provider, seen_types); };
  static_assert(application_settings_surface_is_valid<Composition>(), "application settings and catalog APIs require exactly one valid settings surface");
...
[[nodiscard]] FingerprintSink application_schema_sink() {
 ...
 ApplicationSchema<Composition>::VisitVisualSources([&]<class Cell, std::meta::info, class Projection>() {
  ...
  Projection::relation::VisitMembers([&]<class Entry>() {
   constexpr auto source = mmltk::frameworks::reflection::reflected_member_path<typename Projection::snapshot_type, Entry::source>();
   constexpr auto destination = mmltk::frameworks::reflection::reflected_member_path<VisualSourceObservation, Entry::destination>();
 ...
 ApplicationSchema<Composition>::VisitEndpoints([&]<class Endpoint>() {
  ...
  sink.append_number(Endpoint::stable_id);
  if constexpr (Endpoint::signature::has_request) {
   ApplicationSchema<Composition>::template VisitRequestFields<Endpoint>([&]<class Owner, class Declaration>(const ApplicationRequestFieldFact& field) {
 ...
 ApplicationSchema<Composition>::VisitApplicationSettingsLeaves([&]<class Owner, class Declaration, class Member>(const ApplicationSettingsLeafFact& field) {
  ...
  for (std::size_t index = 0U; index < field.workflows.count; ++index) sink.append_number(static_cast<std::uint8_t>(field.workflows.workflows[index]));
