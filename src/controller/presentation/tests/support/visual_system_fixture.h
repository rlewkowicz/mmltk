...
public:
 [[nodiscard]] static PresentationNativeWriterFactory Factory(std::shared_ptr<FakeImageBackend> backend, std::shared_ptr<TestPresentationWriterState> state) {
...
private:
 ...
 [[nodiscard]] PresentationCapability capability() const noexcept {
  ...
  return {
   ...
   .condition = state_->allow_publication.load(std::memory_order_acquire) ? PresentationCapabilityCondition::Ready : PresentationCapabilityCondition::Admitted,
  };
 }
...
public:
 explicit LoadedSettings(SystemEventSink<SettingsSystem::event_type> events = {}) : system_(std::move(events)) { REQUIRE(system_.Load(location()).applied()); }
...
public:
 ...
 void SetSemantics(const std::uint8_t value) {
  ...
  runtime_.Publish(candidate, current.extent.width, current.extent.height, [value](const auto, const auto semantic, const auto) { Fill(semantic, value); });
