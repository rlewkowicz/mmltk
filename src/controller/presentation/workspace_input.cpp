...
void WorkspaceInput::Accept(WorkspaceMouse mouse, PresentationSourceKind source) {
    if (!mouse.valid() || mouse.source != source || mouse.peer_epoch != peer_) throw contracts::InvalidIntentError("Workspace mouse ownership is invalid");
...
bool WorkspaceInput::pressed(WorkspaceMouseButton button) const noexcept { return (buttons_ & (1U << static_cast<unsigned>(button))) != 0U; }
}  // namespace mmltk::controller
