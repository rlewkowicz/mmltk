 ...
 static raster::IntRect Bounds(const AnnotationRenderState& description, std::size_t index, raster::IntRect runs, int width, int height) {
  ...
  if (runs.x1 < runs.x2 && runs.y1 < runs.y2) {
   ...
   if (description.TransformsMask(index)) {
    ...
    const float sx = object.box.second.x > object.box.first.x ? (target.second.x - target.first.x) / (object.box.second.x - object.box.first.x) : 1.0F;
    const float sy = object.box.second.y > object.box.first.y ? (target.second.y - target.first.y) / (object.box.second.y - object.box.first.y) : 1.0F;
    visible = sx > 0 && sy > 0;
 ...
 void Render(const AnnotationRenderState& description, const mmltk::frameworks::gpu::ImagePlaneView source, const mmltk::frameworks::gpu::ImagePlaneView clean,
  ...
  for (std::size_t index = 0; index < std::max(retained.objects.size(), description.ObjectCount()); ++index) {
   ...
   if (current) {
    ...
    footprint.bounds = Bounds(description, index, footprint.runs, static_cast<int>(clean.descriptor.width), static_cast<int>(clean.descriptor.height));
   }
  ...
  for (std::size_t index = 0; index < description.ObjectCount(); ++index) {
   ...
   const raster::IntRect object_clip{std::max(clip.x1, bounds.x1), std::max(clip.y1, bounds.y1), std::min(clip.x2, bounds.x2), std::min(clip.y2, bounds.y2)};
   const auto box = DrawingBox(description, index);
   ...
   if (!object.mask.runs.empty()) {
    if (raster::raster_mask_runs_rgba(
         {.overlay = {reinterpret_cast<std::uint8_t*>(semantic.data), semantic.descriptor.pitch_bytes, static_cast<int>(semantic.descriptor.width),
          ...
          .scale_x = object.box.second.x > object.box.first.x ? (target.second.x - target.first.x) / (object.box.second.x - object.box.first.x) : 1.0F,
          .scale_y = object.box.second.y > object.box.first.y ? (target.second.y - target.first.y) / (object.box.second.y - object.box.first.y) : 1.0F}) != 0)
