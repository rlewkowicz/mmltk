...
[[nodiscard]] DocumentSaveEffect AtomicSave(Backend& backend, const std::span<const std::byte> bytes, const std::string_view destination,
    ...
    const auto [revision_end, revision_error] =
        std::to_chars(cursor, temporary.data() + temporary.size() - suffix.size() - 1U, document_revision);
    if (revision_error != std::errc{}) return DocumentSaveEffect::NotApplied;
    *revision_end = '-';
    const auto [generation_end, generation_error] =
        std::to_chars(revision_end + 1, temporary.data() + temporary.size() - suffix.size() - 1U, generation);
