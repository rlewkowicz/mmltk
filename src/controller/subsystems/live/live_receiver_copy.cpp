...
cudaError_t Copy(const mmltk::frameworks::gpu::ImagePlaneView target, const std::uintptr_t source, const std::size_t source_pitch,
    ...
    return cudaMemcpy2DAsync(reinterpret_cast<void*>(target.data), target.descriptor.pitch_bytes, reinterpret_cast<const void*>(source),
                             source_pitch, target.descriptor.row_bytes(), target.descriptor.height, cudaMemcpyDeviceToDevice,
...
cudaError_t copy_live_receiver_frame(const mmltk::frameworks::gpu::ImagePlaneView target, const std::uintptr_t source,
                                     const std::size_t source_pitch, const std::uintptr_t ready_event, const std::uintptr_t stream,
                                     const LiveReceiverCopyOperations& operations) noexcept {
    if (!target.valid() || source == 0U || source_pitch == 0U || ready_event == 0U || stream == 0U || !operations.valid())
