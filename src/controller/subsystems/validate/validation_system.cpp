   ...
   public:
    Impl(SettingsSystem& settings, DatasetSystem& dataset, ModelSystem& model, ValidationRuntimeFactory factory,
        ...
        : settings_(settings), dataset_(dataset), model_(model), factory_(std::move(factory)), events_(std::move(events)),
          execution_(std::move(execution)), samples_(visual, [this] { Changed(); }) {
    ...
    [[nodiscard]] contracts::ComputeUiState Start() {
        ...
        run_.Start({
            ...
            .work = [this, settings = settings.settings, selection](
...
ValidationSnapshot ValidationSystem::Start(contracts::ValidateWorkflowIntent) { static_cast<void>(impl_->Start()); return snapshot(); }
ValidationSnapshot ValidationSystem::Stop() noexcept { static_cast<void>(impl_->Stop()); return snapshot(); }
...
mmltk::backend::models::rfdetr::EvaluationDetailPage ValidationSystem::Details(mmltk::backend::models::rfdetr::EvaluationDetailQuery query) const {
    ...
    if (!impl_->evaluation_ || query.generation != impl_->evaluation_generation_ || query.generation != impl_->state_.generation_frontier) throw contracts::InvalidIntentError("validation metric query is stale");
