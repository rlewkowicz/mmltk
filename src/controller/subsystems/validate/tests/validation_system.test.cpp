...
TEST_CASE("composed preview retains every source after the outer draw callback", "[controller][gpu][validation]") {
    ...
    gpu::SystemImageRuntime runtime({.device = 0, .backend = backend,
    ...
    for (std::size_t index = 0U; index < regions.size(); ++index) {
        ...
        auto frame = pool.Capture(nullptr, {2U, 2U}, 0U, {}, source.annotations(), source.classes(), 0,
            source.rgb8(), source.custody(), nullptr, nullptr, {}, true);
        REQUIRE(frame);
        frames[index] = frame;
        regions[index] = {std::move(frame), {static_cast<std::uint32_t>(index % 3U) * 2U,
            static_cast<std::uint32_t>(index / 3U) * 2U, 2U, 2U}};
    }
    ...
    auto concurrent = pool.Capture(nullptr, {2U, 2U}, 0U, {}, extra.annotations(), extra.classes(), 0,
        extra.rgb8(), extra.custody(), nullptr, nullptr, {}, true);
    REQUIRE(concurrent); // Source delivery continues while the independent draw owns its set.
    CHECK_FALSE(pool.Capture(nullptr, {2U, 2U}, 0U, {}, extra.annotations(), extra.classes(), 0,
        extra.rgb8(), extra.custody(), nullptr, nullptr, {}, true));
    ...
    for (unsigned attempt = 0U; attempt < 3U; ++attempt) {
        CHECK_THROWS(pool.Capture(nullptr, {2U, 2U}, 0U, {}, extra.annotations(), extra.classes(), 0,
            extra.rgb8(), extra.custody(), nullptr, nullptr, {}, true));
        CHECK(retirement->fact().occupancy == 1U);
    }
    ...
    gpu::SystemImageRuntime replacement({.device = 0, .output_layout = gpu::ImageProductLayout::CleanAndSemantic,
...
TEST_CASE("validation retains the limited sample atlas and selects detail without a producer", "[controller][gpu][validation]") {
    ...
    await([&] { const auto snapshot = samples.snapshot(); return std::count(snapshot.sample_available.begin(), snapshot.sample_available.end(), true) == sample_count; });
    const auto atlas = samples.snapshot();
    CHECK_FALSE(atlas.detail);
    CHECK(std::count(atlas.sample_available.begin(), atlas.sample_available.end(), true) == sample_count);
...
TEST_CASE("validation composition preserves independent nonempty box and mask pixels", "[controller][gpu][validation]") {
    ...
    mask[3U * 12U + 3U] = 1U;
    const rfdetr::Prediction prediction{.class_reference = 0, .class_domain = mmltk::backend::data::catalog::ClassReferenceDomain::Foreground,
        .score = 0.75F, .bbox_xyxy = {2, 2, 4, 4}, .has_mask = true};
    ...
    gpu::DeviceContext context(0, gpu::cuda_image_copy_backend(), gpu::DeviceContextMode::Isolated,
        execution.placement.numa_node, execution);
    detail::PredictionPreviewPool pool(execution, context);
    const std::array ground_truth{truth};
    auto frame = pool.Capture(source.pixels(), source.extent(), 0U, source.detections(), source.annotations(), catalog, 2,
        nullptr, source.custody(), nullptr, nullptr, ground_truth, true);
    REQUIRE(frame);
    ...
    for (unsigned flags = 0U; flags < 16U; ++flags) {
        ...
        REQUIRE(cudaMemcpy2D(rgba.data(), 12U * 4U, reinterpret_cast<const void*>(semantic.data), semantic.descriptor.pitch_bytes,
            12U * 4U, 12U, cudaMemcpyDeviceToHost) == cudaSuccess);
        ...
        REQUIRE(cudaMemcpy2D(rgba.data(), 12U * 4U, reinterpret_cast<const void*>(clean.data), clean.descriptor.pitch_bytes,
            12U * 4U, 12U, cudaMemcpyDeviceToHost) == cudaSuccess);
...
class RefusedValidationPreview final : public ValidationRuntime {
 public:
    ...
    ValidationRuntimeResult Run(mmltk::backend::models::rfdetr::ValidateRequest, std::stop_token,
        const ComputeProgressSink& progress, const mmltk::backend::models::rfdetr::ValidationDelivery& delivery) override {
        namespace rfdetr = mmltk::backend::models::rfdetr;
        ++runs_;
...
TEST_CASE("validation semantic metrics survive optional preview refusal without reinference", "[controller][systems][validation]") {
    ...
    ValidationSystem validation(settings, dataset, model, [&] { return std::make_unique<RefusedValidationPreview>(runs); },
        [&](auto event) { if (auto* changed = std::get_if<ValidationChanged>(&event)) finished.set_value(changed->snapshot); },
