...
[[nodiscard]] std::expected<const mmltk::controller::contracts::ArtifactSplitFact*, ComputeIntentMaterializer::Refusal> current_training_split(
...
void assign_model_artifact(mmltk::backend::models::rfdetr::ModelArtifactRequest& request, const mmltk::controller::contracts::ModelSelection& model) {
    ...
    switch (model.key.input) {
        case mmltk::controller::contracts::ModelArtifactInputKind::Weights: request.weights_path = model.artifact; return;
        case mmltk::controller::contracts::ModelArtifactInputKind::Onnx: request.onnx_path = model.artifact; return;
        case mmltk::controller::contracts::ModelArtifactInputKind::TensorRt: request.tensorrt_path = model.artifact; return;
...
std::expected<ComputeIntentMaterializer::ModelInput, ComputeIntentMaterializer::Refusal> ComputeIntentMaterializer::ModelInputFor(
    ...
    if (mmltk::controller::contracts::valid_feature(workflow) && !mmltk::controller::contracts::model_selection_workflow_supported(workflow))
...
std::expected<mmltk::backend::models::rfdetr::TrainRequest, ComputeIntentMaterializer::Refusal> ComputeIntentMaterializer::LocalTrain(
    ...
    if ((*training)->class_names != (*validation)->class_names) return std::unexpected(refused("training and validation class order differs"));
    request.train_compiled_path = (*training)->path;
    if (request.resume_path.empty()) {
        request.weights_path = model.artifact;
    } else {
        if (request.resume_path != model.artifact) return std::unexpected(refused("prepared training model does not match the selected resume checkpoint"));
        request.weights_path.clear();
    }
