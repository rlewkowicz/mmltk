...
void LocalRun::Start(Job job) {
    ...
    std::jthread candidate{[this, control, policy = std::move(job.policy), work = std::move(job.work),
                            failure = std::move(job.failure)](const std::stop_token stop) mutable noexcept {
