        ...
        [this, &progress, &products, &gate, stop, operation, maximum, &current_context,
         &retirement](const mmltk::backend::ml::runtime::BorrowedCommandStream stream) mutable {
                    ...
                    .begin =
                        [&](const auto& summary) {
                            ...
                            try {
                                ...
                                class_count = summary.class_domain == mmltk::backend::data::catalog::ClassReferenceDomain::Foreground
                                                  ? static_cast<int>(summary.class_catalog->size())
                    ...
                    .completed =
                        [&](const auto& record, const auto pixels, const auto& annotations) {
                            ...
                            try {
                                ...
                                if (!impl_->preview || impl_->preview_context != context) {
                                    auto candidate = std::make_unique<detail::PredictionPreviewPool>(
                                        *impl_->config.execution, *context,
                                        detail::PredictionPreviewPool::TransferOperations{&cuMemcpyPeerAsync, &cudaEventRecord, &cudaStreamSynchronize,
                                                                                          &cuMemHostRegister},
                                        retirement);
                                ...
                                auto raw =
                                    impl_->preview->Capture(pixels.chw, {pixels.width, pixels.height}, pixels.stream, record.detections, annotations, classes,
                                                            class_count, pixels.rgb8, pixels.custody, pixels.stop_source, pixels.source_control);
                                if (raw)
                                    products(Product{.extent = {pixels.width, pixels.height},
                                                     .raw = std::move(raw),
                                                     .image_id = record.image_id,
                                                     .source_index = record.dataset_index});
            ...
            return contracts::make_compute_terminal(
                result.cancelled ? contracts::ComputeOperationOutcome::Cancelled : contracts::ComputeOperationOutcome::Succeeded, 0U, result.processed_images,
                ...
                run_.Start({
                    .work = [this, settings = settings.settings, selection, source_key = key, video = state_.video,
                        ...
                        if (!stop.stop_requested() && settings.workflows.predict.source.kind == contracts::SourceKind::CompiledDataset)
                            inspection = dataset_.Inspect({settings.workflows.predict.source.compiled_path, {}, {}}, selection.key.preset,
    ...
    detail::VisualRuntimeOwner::Notification Render(mmltk::frameworks::gpu::SystemImageRuntime& runtime, std::stop_token stop) {
        ...
        try {
            ...
            for (const auto& detection : product.raw->predictions()) {
                ...
                if (!label.box.valid() || !label.color.valid() || !std::isfinite(label.confidence) ||
                    label.name.size() > mmltk::frameworks::reflection::kMaximumNameBytes)
            ...
            detail::VisualRuntimeOwner::Notification notification = [this, frame, labels = std::move(labels), identity = product.image_id,
                ...
                PredictSnapshot changed;
                {
                    std::scoped_lock lock(mutex_);
                    const auto revision =
                        detail::PredictRevision::Frame(state_.revision, state_.operation.active,
