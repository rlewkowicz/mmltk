...
public:
 ...
 [[nodiscard]] ExploreSnapshot UpdateFilter(const ExploreFilterUpdate request) {
  ...
  std::uint64_t seed;
  {
   std::scoped_lock lock(mutex_);
   RequireReady();
   require_policy_valid(request, static_cast<std::uint32_t>(state_.dataset.class_names.size()), state_.dataset.image_count, "Explore filter is invalid");
...
private:
 ...
 [[nodiscard]] ExploreSnapshot QueueDesired(Install install, const std::int64_t navigation = 0, const bool persist = false, const bool check_settings = false,
  ...
  if (viewport_update && request->snapshot.viewport.first_row != previous_viewport.first_row)
   scroll_direction_ = request->snapshot.viewport.first_row > previous_viewport.first_row ? ExploreScrollDirection::Forward : ExploreScrollDirection::Backward;
