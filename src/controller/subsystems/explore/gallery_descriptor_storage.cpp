...
void GalleryDescriptorStorage::PrepareDescriptors(const std::size_t cards, const std::size_t annotations, const std::size_t rle, const std::size_t classes,
 ...
 ensure_gallery_buffer(storage_.buffers_.descriptors_, std::max<std::size_t>(layout.bytes, 1U), "Explore pinned descriptor staging allocation failed",
 ...
 ensure_gallery_buffer(storage_.buffers_.rle_device_, std::max<std::size_t>(rle * sizeof(data::RLEPair), 1U),
                       "Explore RLE descriptor high-water allocation failed", diagnostics, device, generation);
...
void GalleryDescriptorStorage::UploadDescriptors(const cudaStream_t stream, const bool upload_classes, const ExploreDemandCheck& demand,
 ...
 UploadDescriptor(storage_.buffers_.rle_device_, descriptor_layout_.rle.offset, descriptor_layout_.rle.count * sizeof(data::RLEPair), stream, demand,
                  generation);
 if (upload_classes)
