...
bool GalleryAtlas::Contains(const std::size_t physical, const std::shared_ptr<const GalleryTileMeaning>& meaning, const std::uint64_t semantics) const {
...
void GalleryAtlas::Stage(const std::size_t physical, std::shared_ptr<const GalleryTileMeaning> meaning, const std::uint64_t semantics, const bool placeholder) {
 writes_.push_back({physical, {std::move(meaning), semantics, true, placeholder}});
}
mmltk::frameworks::gpu::ImageWorkspaceCoverage GalleryAtlas::WorkspaceCoverage(const mmltk::frameworks::gpu::ImageWorkspaceObservation& output) {
