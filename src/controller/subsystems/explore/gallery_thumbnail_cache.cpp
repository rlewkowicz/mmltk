...
std::size_t GalleryThumbnailCache::CardCount(const std::size_t matching, const std::uint32_t rows,
...
std::size_t GalleryThumbnailCache::WindowFirst(const std::size_t matching, const ExploreViewport& viewport) noexcept {
    ...
    return std::min(first > behind ? first - behind : 0U,
                    matching - CardCount(matching, viewport.row_count, viewport.columns));
}
...
void GalleryThumbnailCache::Complete(const std::size_t position, const std::uint32_t compiled_index,
                                      ...
                                      const std::uint64_t semantic_identity, const std::uint8_t bank,
    ...
    entries_.at(Slot(position)) = {.position = position, .compiled_index = compiled_index,
                                  .bank = bank, .semantic_bank = semantic_bank,
                                  .meaning = std::move(meaning), .semantic_identity = semantic_identity};
}
...
std::size_t GalleryThumbnailCache::MeaningBytes(const GalleryThumbnailCache* other,
    ...
    struct Allocation { const void* identity; std::size_t bytes; };
    std::array<Allocation, 4U * kMaximumCards> allocations;
    std::size_t count = 0U;
    const auto meaning = [&](const auto& value) {
        if (value) allocations[count++] = {value.get(), GallerySharedBytes(value) +
