...
void GalleryStreamProbe::DiagnoseRendered(const GalleryProductState& product, const GalleryDescriptorStorage& descriptors,
 ...
 diagnostics_.Emit([&] {
  return VisualDiagnosticFact{
   .system = contracts::DiagnosticOwner::Explore, .operation = VisualDiagnosticOperation::ExploreProbeFailed, .device = device_, .generation = generation};
 });
}
 ...
 if (probe_annotation) {
  ...
  diagnostics_.Emit([&] {
   return VisualDiagnosticFact{
    ...
    .context = {
     .capacity_width = static_cast<std::uint32_t>(facts[3]), .capacity_height = static_cast<std::uint32_t>(facts[6]), .staging_bytes = transition_count}};
  });
 }
}
