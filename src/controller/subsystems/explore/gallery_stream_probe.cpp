...
void GalleryStreamProbe::DiagnoseRendered(const GalleryProductState& product, const GalleryDescriptorStorage& descriptors, const mmltk::frameworks::gpu::ImagePlaneView reference_plane,
 const GalleryThumbnailCache* incumbent, const mmltk::frameworks::gpu::ImagePlaneView clean, const mmltk::frameworks::gpu::ImagePlaneView semantic, const std::uintptr_t stream,
 const std::uint64_t generation, const std::uint64_t slot, const std::uint32_t compiled_index, const std::optional<std::size_t> card_index, const std::uint32_t x, const std::uint32_t y,
 ...
 if (card_index)
  card = load_payload<explore::ExploreRenderCardDescriptor>(
   descriptors.storage_.buffers_.descriptors_.data(), descriptors.descriptor_layout_.cards.offset + *card_index * sizeof(explore::ExploreRenderCardDescriptor));
 ...
 for (std::size_t local = 0U; card && local != card->annotation_count; ++local) {
  const auto annotation = load_payload<explore::ExploreRenderAnnotationDescriptor>(
   descriptors.storage_.buffers_.descriptors_.data(), descriptors.descriptor_layout_.annotations.offset + (card->annotation_offset + local) * sizeof(explore::ExploreRenderAnnotationDescriptor));
 ...
 ensure_gallery_buffer(storage_.buffers_.semantic_count_device_, probes_.size() * kProbeFacts * sizeof(std::uint64_t), "Explore rendered diagnostic device allocation failed", diagnostics_, device_,
  product.plan.generation);
 ensure_gallery_buffer(storage_.buffers_.semantic_count_pinned_, probes_.size() * kProbeFacts * sizeof(std::uint64_t), "Explore rendered diagnostic staging allocation failed", diagnostics_, device_,
  product.plan.generation);
 ...
 if (width != 0U || height != 0U) {
  if (width == 0U || height == 0U || x >= count_target.width || y >= count_target.height || width > count_target.width - x || height > count_target.height - y)
 ...
 if (explore::count_explore_nonzero_alpha(count_target, device_facts, stream) != explore::kExploreStorageSuccess) throw std::runtime_error("Explore semantic diagnostic count failed");
 if (explore::checksum_explore_pixels(checksum_target, device_facts + 1U, stream) != explore::kExploreStorageSuccess) throw std::runtime_error("Explore image diagnostic checksum failed");
 ...
 if (sample_card && explore::sample_explore_rendered_card(checksum_target, count_target, reference, device_facts + kCardSamplesOffset, stream) != explore::kExploreStorageSuccess)
 ...
 if (card && probe_annotation) {
  ...
  const bool padded = static_cast<std::uint64_t>(rendered_probe.content_width) * rendered_probe.content_height < static_cast<std::uint64_t>(checksum_target.width) * checksum_target.height;
  const bool identity_preview = !product.plan.augmentation.enabled || !product.plan.augmentation_config.enabled;
  const bool probe_valid = rendered_probe.reference.valid() && identity_preview && padded && rendered_probe.box_width > 2U && rendered_probe.box_height > 2U &&
                           rendered_probe.box_x < checksum_target.width && rendered_probe.box_y < checksum_target.height && rendered_probe.box_width <= checksum_target.width - rendered_probe.box_x &&
                           rendered_probe.box_height <= checksum_target.height - rendered_probe.box_y;
  if (probe_valid) {
   if (explore::probe_explore_rendered_card(checksum_target, count_target, rendered_probe, device_facts + 2U, stream) != explore::kExploreStorageSuccess)
 ...
 probe_count_ = 0U;
 diagnostics_.Emit(
  [&] { return VisualDiagnosticFact{.system = contracts::DiagnosticOwner::Explore, .operation = VisualDiagnosticOperation::ExploreProbeFailed, .device = device_, .generation = generation}; });
}
...
void GalleryStreamProbe::FlushProbes(const std::uintptr_t stream) {
 ...
 if (cudaEventRecord(probes_ready_, cuda_stream) != cudaSuccess || cudaStreamWaitEvent(diagnostic_stream_, probes_ready_, 0U) != cudaSuccess ||
     cudaMemcpyAsync(storage_.buffers_.semantic_count_pinned_.data(), storage_.buffers_.semantic_count_device_.data(), submitted_probes_ * kProbeFacts * sizeof(std::uint64_t), cudaMemcpyDeviceToHost,
...
void GalleryStreamProbe::EmitProbe(const RenderedProbe& record, const std::uint64_t* facts) const noexcept {
 ...
 if (!card) return;
 const auto packed_content = (static_cast<std::uint64_t>(probe.content_x & 0xffffU) << 48U) | (static_cast<std::uint64_t>(probe.content_y & 0xffffU) << 32U) |
 ...
 if (probe_annotation) {
  const auto flags = (facts[2] != 0U ? 1U : 0U) | (facts[3] != 0U ? 2U : 0U) | (facts[4] != 0U ? 4U : 0U) | (facts[5] != 0U ? 8U : 0U) | (facts[6] != 0U ? 16U : 0U);
  diagnostics_.Emit([&] {
...
void GalleryStreamProbe::EmitCardSamples(const RenderedProbe& record, const std::uint64_t* facts) const noexcept try {
 ...
 output << "{\"dataset_identity\":" << record.dataset_identity << ",\"augmentation_seed\":" << record.seed << ",\"image_key\":" << record.image_key << ",\"compiled_index\":" << record.compiled_index
        << ",\"slot\":" << record.slot << ",\"augmentation_enabled\":" << (record.augmented ? "true" : "false")
        << ",\"augmentation_config_enabled\":" << (record.augmentation_config_enabled ? "true" : "false") << ",\"source_width\":" << record.source_extent[0]
        << ",\"source_height\":" << record.source_extent[1] << ",\"width\":" << clean.width << ",\"height\":" << clean.height << ",\"reference_width\":" << reference.width
        << ",\"reference_height\":" << reference.height << ",\"reference_present\":" << (reference.data != nullptr ? "true" : "false") << ",\"checksum\":" << facts[1] << ",\"axis_percent\":[";
...
auto GalleryStreamProbe::ReleaseProbesChecked(decltype(&cudaStreamSynchronize) stream_wait_, cudaError_t& unsettled) noexcept -> mmltk::frameworks::gpu::SystemImageModel::Release {
 ...
 if (owned && all_released)
  diagnostics_.Emit([&] { return VisualDiagnosticFact{.system = contracts::DiagnosticOwner::Explore, .operation = VisualDiagnosticOperation::ExploreProbeResourcesReleased, .device = device_}; });
...
void GalleryStreamProbe::DiagnoseDescriptors(const GalleryDescriptorStorage& descriptors_, VisualDiagnosticSink diagnostics_, int device_, const std::uint64_t generation, const std::uint64_t slot,
 const std::size_t first, const std::size_t count, const std::size_t rle_count) {
 ...
 for (std::size_t local = 0U; local != count; ++local) {
  const auto annotation = load_payload<explore::ExploreRenderAnnotationDescriptor>(
   descriptors_.storage_.buffers_.descriptors_.data(), descriptors_.descriptor_layout_.annotations.offset + (first + local) * sizeof(explore::ExploreRenderAnnotationDescriptor));
...
void GalleryStreamProbe::DiagnosePreparedImage(const ExploreRenderPlan& plan, const GalleryReadScheduler& scheduler_, const mmltk::backend::models::rfdetr::GpuAugmentationExecutor& augmenter,
 VisualDiagnosticSink diagnostics_, const GalleryReadScheduler::Lane& lane, const std::size_t image, const std::size_t staging_slot) noexcept try {
 if (!diagnostics_.valid()) return;
 const auto payload = "{\"dataset_identity\":" + std::to_string(plan.dataset_identity) + ",\"augmentation_seed\":" + std::to_string(plan.augmentation.seed) +
                      ",\"compiled_index\":" + std::to_string(lane.compiled_index) + ",\"slot\":" + std::to_string(lane.destination_slot) + ",\"lane\":" + std::to_string(lane.index) +
