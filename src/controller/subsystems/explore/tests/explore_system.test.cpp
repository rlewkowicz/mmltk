...
public:
 ...
 ExploreOrderFacts Visible(ExploreViewport, const ExploreOrderCandidate* candidate = nullptr) const override {
  return candidate == nullptr ? ExploreOrderFacts{.matching_count = static_cast<std::uint32_t>(order_.size()), .visible_indices = order_} : candidate->order;
 }
...
TEST_CASE("Explore announces the acquired detail allocation before framework prewrites can fail") {
 ...
 public:
  ...
  void PrepareDetailOutput(mmltk::frameworks::gpu::ImageAllocation allocation) noexcept override { owner_.store(allocation.owner, std::memory_order_release); }
...
TEST_CASE("Explore settles a pending predecessor on the incumbent before reconstructing Open") {
 ...
 REQUIRE(fixture.scenario.Wait([&] { return !explore.snapshot().busy; }));
 const auto observations = fixture.Recorded();
 const auto predecessor = std::ranges::find_if(observations, [](const auto& event) { return event.snapshot.busy && event.snapshot.viewport.first_row == 0U; });
...
TEST_CASE("Explore Open commits the effective pending filter with a different catalog") {
 ...
 LoadedSettings settings{[settings_events, emitted_revision, emitted_catalog, emitted_minimum_instances, emitted_show_boxes](SettingsSystem::event_type event) {
...
TEST_CASE("queued Explore cancellation is finalized by the scheduler callback") {
 ...
 ExploreScenario scenario{settings, backend, [observed_nproc, commits, gate] { return std::make_unique<TestExploreAlgorithm>(observed_nproc, gate, commits); }};
 auto& explore = scenario.system();
 auto settle_explore = settle_explore_on_exit(explore, gate->release);
 scenario.OpenAndWait({.extent = {64U, 64U}});
 const auto committed = explore.snapshot();
