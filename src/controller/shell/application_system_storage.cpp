...
SystemEventSink<ExploreSystem::event_type> make_explore_upscale_event_sink(ApplicationSystemStorage::EventSink& sink, UpscaleSystem& upscale,
                                                                           ApplicationSystemStorage::ContinuitySink continuity,
                                                                           std::function<void(PresentationSourceIdentity)> source) {
 return [&upscale, publisher = browser::ApplicationEventPublisher<&ApplicationSystems::explore>(sink, std::move(continuity), std::move(source))](
  ...
  if (const auto* changed = std::get_if<ExploreChanged>(&event);
      ...
      (changed->snapshot.mode != ExploreMode::Gallery || std::ranges::all_of(changed->snapshot.gallery.slots, [](const bool ready) { return ready; })))
...
ApplicationSystemStorage::ApplicationSystemStorage(ApplicationSystemConfiguration configuration, EventSink events, const VisualDiagnosticSink diagnostics,
                                                   ContinuitySink continuity)
  ...
  *settings_,
  [diagnostics = std::move(configuration.dataset_diagnostics)] { return std::make_unique<ArtifactDatasetRuntime>(services::ArtifactStore{}, diagnostics); },
  browser::ApplicationEventPublisher<&ApplicationSystems::dataset>(events_, continuity_));
 model_ = std::make_unique<ModelSystem>(
  *settings_, [] { return std::make_unique<ArtifactModelRuntime>(); }, browser::ApplicationEventPublisher<&ApplicationSystems::model>(events_, continuity_));
 ...
 annotation_ =
  std::make_unique<AnnotationSystem>(configuration.output_visual, make_native_annotation_runtime_factory(configuration.output_visual), borrow_exact,
                                     browser::ApplicationEventPublisher<&ApplicationSystems::annotation>(events_, continuity_, source_changed), diagnostics);
 live_ = std::make_unique<LiveSystem>(configuration.base_visual, make_native_live_runtime_factory(configuration.base_visual, std::move(configuration.live)),
 ...
 presentation_ = std::make_unique<PresentationSystem>(
  configuration.output_visual, make_native_presentation_writer_factory(configuration.output_visual, std::move(configuration.presentation), diagnostics),
