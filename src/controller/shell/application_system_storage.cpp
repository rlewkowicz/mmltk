...
SystemEventSink<ExploreSystem::event_type> make_explore_upscale_event_sink(
 ...
 return [&upscale, publisher = browser::ApplicationEventPublisher<&ApplicationSystems::explore>(sink, std::move(continuity), std::move(source))](ExploreSystem::event_type event) noexcept {
...
ApplicationSystemStorage::ApplicationSystemStorage(ApplicationSystemConfiguration configuration, EventSink events, const VisualDiagnosticSink diagnostics, ContinuitySink continuity)
 ...
 upscale_ = std::make_unique<UpscaleSystem>(configuration.output_visual, make_native_upscale_runtime_factory(configuration.output_visual, std::move(upscale_checkpoint)), borrow_exact,
  browser::ApplicationEventPublisher<&ApplicationSystems::upscale>(events_, continuity_, source_changed), diagnostics);
 explore_ = make_shell_explore_system(*settings_, configuration, *compute.execution, make_explore_upscale_event_sink(events_, *upscale_, continuity_, source_changed), diagnostics);
 annotation_ = std::make_unique<AnnotationSystem>(configuration.output_visual, make_native_annotation_runtime_factory(configuration.output_visual), borrow_exact,
 ...
 presentation_ =
  std::make_unique<PresentationSystem>(configuration.output_visual, make_native_presentation_writer_factory(configuration.output_visual, std::move(configuration.presentation), diagnostics),
