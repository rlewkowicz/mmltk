...
services::FirefoxProcessStartResult ApplicationShell::start_firefox(services::FirefoxProcessConfig config) noexcept {
    ...
    firefox_process_.emplace(
        presentation_import_socket_path(), std::move(config),
...
services::FirefoxProcessLifecycle ApplicationShell::firefox_lifecycle() const noexcept {
    return firefox_process_ ? firefox_process_->lifecycle()
                            : services::FirefoxProcessLifecycle{.terminal = services::FirefoxProcessTerminal::StartupFailed};
}

const std::filesystem::path& ApplicationShell::presentation_import_socket_path() const noexcept {
    return presentation_config_.import_socket;
}
...
bool ApplicationShell::shutdown() noexcept {
    ...
    emit_shutdown_event(resources_closed ? "shutdown.complete" : "shutdown.incomplete");
...
bool ApplicationShell::join_systems() noexcept {
    ...
    systems_stopped_ = systems_->presentation().stopped() && systems_->live().stopped() && systems_->upscale().stopped() &&
                       systems_->annotation().stopped() && systems_->explore().stopped();
