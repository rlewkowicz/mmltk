...
[[nodiscard]] std::filesystem::path default_import_socket() {
 ...
 return root / ("mmltk-workspace-" + std::to_string(static_cast<long long>(::getpid())) + "-" + std::to_string(sequence.fetch_add(1U, std::memory_order_relaxed)) + ".sock");
}
...
bool ApplicationShell::start_browser_host(transport::BrowserServer::Config config) noexcept {
 ...
 return !shutdown_requested_.load(std::memory_order_acquire) && browser_host_.accepting() && browser_server_.start(std::move(config), browser_host_.callbacks());
}
...
bool ApplicationShell::InstallFirefoxProcessGroup(void* context, const pid_t group) noexcept { return context && static_cast<ApplicationShell*>(context)->install_firefox_process_group(group); }
...
void ApplicationShell::on_firefox_observation(const services::FirefoxPhysicalObservation observation) noexcept {
 if (observation.kind == services::FirefoxPhysicalObservationKind::ProcessTerminal) {
  ...
  request_shutdown(observation.process.terminal == services::FirefoxProcessTerminal::Exited && observation.process.status == 0 ? shell::ApplicationShutdownReason::WindowClose
...
void ApplicationShell::request_shutdown(const shell::ApplicationShutdownReason reason) noexcept {
 std::call_once(shutdown_request_once_, [this, reason] {
  ...
  if (diagnostics.valid())
   diagnostics.Emit([&] { return services::RuntimeDiagnosticFact{.owner = contracts::DiagnosticOwner::BrowserRuntime, .event = "shutdown.requested", .detail = static_cast<std::uint64_t>(reason)}; });
...
bool ApplicationShell::shutdown() noexcept {
 ...
 if (const auto target = runtime_diagnostics_.target(); target.valid()) {
  target.write_required({.owner = contracts::DiagnosticOwner::BrowserRuntime, .event = resources_closed ? std::string_view{"shutdown.complete"} : std::string_view{"shutdown.incomplete"}});
 }
...
bool ApplicationShell::join_systems() noexcept {
 ...
 systems_stopped_ = systems_->presentation().stopped() && systems_->live().stopped() && systems_->upscale().stopped() && systems_->annotation().stopped() && systems_->explore().stopped();
