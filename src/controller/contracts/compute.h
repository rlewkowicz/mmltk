...
[[nodiscard]] inline bool valid_compute_text(const std::string_view value, const std::size_t capacity) noexcept { return value.size() <= capacity; }
[[nodiscard]] inline std::string bounded_compute_error(const std::string_view value) { return std::string{value.substr(0U, kComputeErrorCapacity)}; }
...
struct ComputeTerminal final {
 ...
 [[nodiscard]] bool valid_worker_terminal() const noexcept {
  ...
  switch (outcome) {
   case ComputeOperationOutcome::Succeeded: return detail.empty();
   case ComputeOperationOutcome::Failed:
   case ComputeOperationOutcome::Refused: return output.empty();
   case ComputeOperationOutcome::Cancelled: return output.empty() && detail.empty();
   case ComputeOperationOutcome::Idle:
   case ComputeOperationOutcome::Running:
   case ComputeOperationOutcome::CancellationRequested: return false;
  }
  return false;
 }
...
[[nodiscard]] inline ComputeTerminal compute_failure_terminal(const std::exception_ptr failure, const std::string_view fallback) {
 ...
 } catch (const std::exception& error) { return make_compute_terminal(ComputeOperationOutcome::Failed, 0U, 0U, {}, error.what()); } catch (...) {
 }
...
inline void cancel_compute(ComputeUiState& state) {
 if (state.active) state.terminal = make_compute_terminal(ComputeOperationOutcome::CancellationRequested, state.generation_frontier);
}
