...
[[nodiscard]] std::expected<void, SettingsMutationError> assign_flat_scalar(T& destination, const Source& source) {
    ...
    if constexpr (std::same_as<U, bool>) {
        ...
        if constexpr (std::same_as<V, std::string>) {
            ...
            bool known = false;
            settings_vocabulary::for_each_enumerator<U>(
                [&](const std::string_view, const U enumerator) { known = known || candidate == enumerator; });
        ...
        if constexpr (std::same_as<V, double> || std::same_as<V, std::int64_t> || std::same_as<V, std::uint64_t>) {
            ...
            if constexpr (!std::same_as<V, double>) {
                if (static_cast<long double>(narrowed) != static_cast<long double>(source))
                    return std::unexpected(SettingsMutationError::OutOfRange);
            }
...
template <class T>
[[nodiscard]] std::expected<void, SettingsMutationError> assign_flat_leaf(T& destination,
        ...
        if constexpr (requires(const V& values) {
            ...
            if constexpr (IsStdArray<U>::value) {
                ...
                for (std::size_t index = 0U; index < source.size(); ++index) {
                    const auto result =
                        std::visit([&](const auto& scalar) { return assign_flat_scalar(candidate[index], scalar); }, source[index]);
...
[[nodiscard]] std::expected<void, SettingsMutationError> assign_flat_path(T& destination, const std::string_view path,
    ...
    if constexpr (std::is_arithmetic_v<U> || std::is_enum_v<U> || std::same_as<U, std::string> || std::same_as<U, std::filesystem::path> ||
        ...
        const bool found = settings_vocabulary::visit_mutable_path(
            destination, path, [&](const std::string_view, auto& leaf) { result = assign_flat_leaf(leaf, value); });
...
[[nodiscard]] bool valid_source(const SourceSelectionState& source) noexcept {
    return (source.crop_width == 0 ||
            (source.crop_width <= source.capture_width && source.crop_x <= source.capture_width - source.crop_width)) &&
           (source.crop_height == 0 ||
            (source.crop_height <= source.capture_height && source.crop_y <= source.capture_height - source.crop_height));
}

[[nodiscard]] bool valid_train(const TrainViewState& train) noexcept {
    const auto& request = train.request;
    const bool valid_distributed =
...
[[nodiscard]] bool selected_model_artifact_available(const ModelArtifactSelectionState& artifacts) noexcept {
    switch (artifacts.input) {
        case ModelArtifactInputKind::Weights:
            return !artifacts.weights_path.empty();
        case ModelArtifactInputKind::Onnx:
            return !artifacts.onnx_path.empty();
        case ModelArtifactInputKind::TensorRt:
            return !artifacts.tensorrt_path.empty();
...
[[nodiscard]] bool valid_model_selection_draft(const ModelArtifactSelectionState& artifacts, const bool concrete_compatible) noexcept {
    if (mmltk::backend::models::rfdetr::find_preset_catalog_entry(artifacts.preset_name) == nullptr || artifacts.resolution <= 0) {
        return false;
    }
...
[[nodiscard]] bool valid_model_selection_relations(const GuiSettingsState& state) noexcept {
    ...
    ModelSelectionRelation::VisitRows([&]<class Relation>(const ModelSelectionCompatibility& row) {
        ...
        if constexpr (std::tuple_size_v<decltype(Relation::predicate)> != 0U) {
            if (!row.required_export_build_tensorrt || std::get<0>(Relation::predicate)(state) != *row.required_export_build_tensorrt)
                return;
        }
        ...
        if (input != row.input || !model_selection_source_allowed(row, source) ||
            (source == ModelSelectionSource::Custom && artifact.empty()))
            return;
        valid[*valid_index] = true;
    });
...
[[nodiscard]] bool valid_settings(const GuiSettingsState& state) noexcept {
    return !mmltk::frameworks::reflection::validate_reflected_fields(state).has_value() && valid_train(state.workflows.train) &&
           valid_validate(state.workflows.validate) && valid_predict(state.workflows.predict) &&
           valid_source(state.workflows.annotate.source) &&
...
std::expected<void, SettingsMutationError> apply_gui_settings_values(GuiSettingsState& state,
                                                                     const std::span<const SettingsValueUpdate> updates) {
    ...
    for (const SettingsValueUpdate& update : updates) {
        ...
        if (update.path.empty() || update.path.size() > kMaxSettingsPathBytes) {
            return std::unexpected(SettingsMutationError::InvalidPath);
        }
...
ExploreSourceFact resolve_explore_source(const GuiSettingsState& state) {
    ...
    switch (explore.dataset_source) {
        case ExploreDatasetSource::Train:
            compiled_source = train.request.train_compiled_path;
            break;
        case ExploreDatasetSource::Validation:
            compiled_source = train.request.val_compiled_path;
            break;
        case ExploreDatasetSource::Test:
            compiled_source = train.request.test_compiled_path;
            break;
        case ExploreDatasetSource::Custom:
            compiled_source = explore.custom_compiled_path;
            break;
    }
