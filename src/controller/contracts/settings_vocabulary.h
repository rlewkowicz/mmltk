...
inline constexpr bool is_mutable_leaf_v = [] {
    ...
    if constexpr (is_optional<U>::value) {
        ...
        return std::is_arithmetic_v<Element> || std::is_enum_v<Element> || std::same_as<Element, std::string> ||
               std::same_as<Element, std::filesystem::path>;
    } else {
...
inline constexpr bool is_persistence_metadata_v = [] {
    ...
    Declaration::VisitAnnotations([&]<class Annotation>(const Annotation&) {
        count += std::same_as<std::remove_cvref_t<Annotation>, reflection::PersistenceMetadata> ? 1U : 0U;
    });
...
constexpr void for_each_member(T&& object, Visitor&& visitor) {
    ...
    mmltk::frameworks::reflection::visit_materialized_bases<U>([&]<class Base>() {
        for_each_member(static_cast<std::conditional_t<std::is_const_v<std::remove_reference_t<T>>, const Base, Base>&>(object), visitor);
    });
...
constexpr void for_each_member_pair(const T& left, const T& right, Visitor&& visitor) {
    ...
    mmltk::frameworks::reflection::visit_materialized_members<T>([&]<class Declaration>(const auto& fact) {
        if constexpr (requires { Declaration::pointer; })
            std::invoke(visitor, fact.member_name, left.*Declaration::pointer, right.*Declaration::pointer);
    });
}
...
constexpr void for_each_enumerator(Visitor&& visitor) {
    for (const auto entry : mmltk::frameworks::reflection::enum_entries<Enum>())
        std::invoke(visitor, entry.name, entry.value);
}
...
[[nodiscard]] constexpr std::pair<std::string_view, std::string_view> split_path(const std::string_view path) noexcept {
    ...
    return separator == std::string_view::npos ? std::pair{path, std::string_view{}}
                                               : std::pair{path.substr(0U, separator), path.substr(separator + 1U)};
}
...
[[nodiscard]] constexpr std::size_t visit_path_count(T&& object, const std::string_view path, Leaf&& leaf) {
    ...
    mmltk::frameworks::reflection::visit_materialized_bases<U>([&]<class Base>() {
        matches += visit_path_count<MutableOnly>(
            static_cast<std::conditional_t<std::is_const_v<std::remove_reference_t<T>>, const Base, Base>&>(object), path, leaf);
    });
    mmltk::frameworks::reflection::visit_materialized_members<U>([&]<class Declaration>(const auto& fact) {
        if constexpr (requires { Declaration::pointer; }) {
