...
[[nodiscard]] consteval std::optional<WorkflowPathDialog> workflow_path_dialog() {
    ...
    if constexpr (std::same_as<std::remove_cv_t<decltype(Path)>, std::remove_cv_t<decltype(member_path<
        &GuiSettingsState::workflows, &WorkflowSettingsState::validate, &ValidateViewState::request,
    ...
    if constexpr (std::same_as<std::remove_cv_t<decltype(Path)>, std::remove_cv_t<decltype(member_path<
        &GuiSettingsState::workflows, &WorkflowSettingsState::train, &TrainViewState::request,
    ...
    if constexpr (std::same_as<std::remove_cv_t<decltype(Path)>, std::remove_cv_t<decltype(member_path<
        ...
        std::same_as<std::remove_cv_t<decltype(Path)>, std::remove_cv_t<decltype(member_path<
        &GuiSettingsState::workflows, &WorkflowSettingsState::validate, &ValidateViewState::request,
        &mmltk::backend::models::rfdetr::ValidateRequest::class_layout_path>)>> ||
        std::same_as<std::remove_cv_t<decltype(Path)>, std::remove_cv_t<decltype(member_path<
        &GuiSettingsState::workflows, &WorkflowSettingsState::predict, &PredictViewState::request,
