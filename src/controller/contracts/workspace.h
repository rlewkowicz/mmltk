...
[[nodiscard]] constexpr bool workspace_text_valid(const std::string_view value) noexcept {
    return !value.empty() &&
           std::ranges::all_of(value, [](const unsigned char character) { return character >= 0x20U && character != 0x7FU; });
}
...
[[nodiscard]] constexpr bool workspace_storage_valid(const std::array<char, Capacity>& storage, const std::uint16_t size,
    ...
    return size != 0U && size <= storage.size() && revision != 0U && workspace_text_valid({storage.data(), size}) &&
           std::ranges::all_of(storage.begin() + static_cast<std::ptrdiff_t>(size), storage.end(),
