...
struct ProviderOffer final {
    ...
    [[nodiscard]] bool valid() const noexcept {
        return offer_id > 0 && gpu_count > 0 && gpu_name.size() <= 128U && location.size() <= 128U && std::isfinite(gpu_ram_gib) &&
               gpu_ram_gib >= 0.0 && std::isfinite(hourly_price) && hourly_price >= 0.0 && std::isfinite(reliability) &&
...
struct[[= mmltk::controller::contracts::reflection::feature_scope(mmltk::controller::contracts::FeatureId::Train)]]
    ProviderQueryIntent final {};
struct[[= mmltk::controller::contracts::reflection::feature_scope(mmltk::controller::contracts::FeatureId::Train)]]
    ProviderOfferIdentity final {
    int offer_id = 0;
    [[nodiscard]] bool valid() const noexcept { return offer_id > 0; }
    bool operator==(const ProviderOfferIdentity&) const = default;
};
struct[[= mmltk::controller::contracts::reflection::feature_scope(mmltk::controller::contracts::FeatureId::Train)]]
    ProviderClearIntent final {};
struct[[= mmltk::controller::contracts::reflection::feature_scope(mmltk::controller::contracts::FeatureId::Train)]]
    ProviderStartIntent final {};
struct[[= mmltk::controller::contracts::reflection::feature_scope(mmltk::controller::contracts::FeatureId::Train)]]
    ProviderStopIntent final {};
...
inline constexpr std::array kProviderQueryTerminalPresentations{
    ...
    terminal_presentation::Policy{ProviderQueryOutcome::Succeeded, terminal_presentation::Classification::Success,
    ...
    terminal_presentation::Policy{ProviderQueryOutcome::Cancelled, terminal_presentation::Classification::Cancelled,
                                  "provider.query.cancelled", "The provider query was cancelled."},
...
inline constexpr std::array kRemoteOperationTerminalPresentations{
    ...
    terminal_presentation::Policy{RemoteOperationOutcome::Applied, terminal_presentation::Classification::Success,
                                  "remote_operation.applied", ""},
    ...
    terminal_presentation::Policy{RemoteOperationOutcome::Inconclusive, terminal_presentation::Classification::Failed,
                                  "remote_operation.inconclusive", "The remote operation outcome is inconclusive."},
    terminal_presentation::Policy{RemoteOperationOutcome::Cancelled, terminal_presentation::Classification::Cancelled,
                                  "remote_operation.cancelled", "The remote operation was cancelled."},
    ...
    terminal_presentation::Policy{RemoteOperationOutcome::Refused, terminal_presentation::Classification::Refused,
                                  "remote_operation.refused", "The remote operation was refused."},
};
...
struct ProviderQueryResult final {
    ...
    [[nodiscard]] bool valid() const noexcept {
        ...
        switch (outcome) {
            ...
            case ProviderQueryOutcome::Rejected:
                return offers.empty();
            case ProviderQueryOutcome::Idle:
                return false;
            default:
                return false;
        }
...
[[nodiscard]] inline ProviderQueryResult normalize_provider_query_result(ProviderQueryResult result) {
    if (!result.valid())
        return {.outcome = ProviderQueryOutcome::Failed, .detail = bounded_provider_detail("provider returned an invalid result")};
...
struct ProviderEffectResult final {
    ...
    [[nodiscard]] bool valid() const noexcept {
        ...
        switch (disposition) {
            case ProviderReconciliationDisposition::Applied:
                return !cancelled && detail.empty();
            case ProviderReconciliationDisposition::NotApplied:
                return instance_id == 0;
            case ProviderReconciliationDisposition::Inconclusive:
                return true;
        }
        return false;
    }
...
[[nodiscard]] inline ProviderEffectResult provider_effect_not_applied(const std::string_view detail, const bool cancelled = false) {
    return {.disposition = ProviderReconciliationDisposition::NotApplied,
            .instance_id = 0,
            .cancelled = cancelled,
            .detail = bounded_provider_detail(detail)};
}
...
[[nodiscard]] inline ProviderEffectResult provider_effect_inconclusive(const std::string_view detail, const bool cancelled = false) {
    return {.disposition = ProviderReconciliationDisposition::Inconclusive,
            .instance_id = 0,
            .cancelled = cancelled,
            .detail = bounded_provider_detail(detail)};
}
