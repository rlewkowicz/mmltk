...
struct ArtifactTerminal final {
 ...
 [[nodiscard]] bool valid() const noexcept {
  ...
  switch (outcome) {
   ...
   case ArtifactTerminalOutcome::CancellationRequested: return artifact.empty() && detail.empty();
   case ArtifactTerminalOutcome::Succeeded: return !artifact.empty() && detail.empty();
   case ArtifactTerminalOutcome::Failed:
   case ArtifactTerminalOutcome::Refused: return artifact.empty();
   case ArtifactTerminalOutcome::Cancelled: return artifact.empty() && detail.empty();
  }
  return false;
 }
