...
[[nodiscard]] inline const AnnotationFrame *
annotation_frame_ptr(const std::optional<AnnotationFrame> &frame) noexcept {
  return frame.has_value() ? std::addressof(*frame) : nullptr;
}

[[nodiscard]] inline AnnotationWorkflowProjectedSceneRequest
make_annotation_workflow_projected_scene_request(
...
[[nodiscard]] inline AnnotationWorkflowProjectedSceneRequest
make_annotation_current_projected_scene_request(
    const AnnotationDocument &document, const AnnotationSession &session,
    const std::optional<AnnotationFrame> &frame) noexcept {
  return make_annotation_workflow_projected_scene_request(
      &document, &session, annotation_frame_ptr(frame));
}

[[nodiscard]] inline std::shared_ptr<const AnnotationProjectedScene>
resolve_annotation_projected_scene(
    AnnotationWorkflowRuntime &runtime,
    const AnnotationWorkflowProjectedSceneRequest &request) {
  return runtime.projected_scene_cache.resolve(
      AnnotationProjectedSceneCacheInputs{
          request.document,
          request.session,
          request.frame,
      });
...
resolve_annotation_projected_scene(
  ...
  return resolve_annotation_projected_scene(
      runtime, make_annotation_current_projected_scene_request(document,
                                                               session, frame));
}
...
[[nodiscard]] inline bool projected_scene_matches_document_generation(
  ...
  return projected_scene != nullptr &&
         projected_scene->document_generation == document_generation;
}

[[nodiscard]] inline std::shared_ptr<const AnnotationProjectedScene>
retain_projected_scene_for_document_generation(
    std::shared_ptr<const AnnotationProjectedScene> projected_scene,
    const std::uint64_t document_generation) noexcept {
  if (!projected_scene_matches_document_generation(projected_scene.get(),
                                                   document_generation)) {
    return {};
  }
...
[[nodiscard]] inline AnnotationWorkflowResolvedObjectsResult
resolve_annotation_workflow_objects(AnnotationWorkflowRuntime &runtime,
  ...
  try {
    ...
    result.resolved_objects =
        resolve_annotation_objects(*frame, categories, document.objects(),
                                   live_mode, result.projected_scene.get());
    result.ok = true;
...
[[nodiscard]] inline AnnotationWorkflowPreparedPreview
prepare_annotation_workflow_preview(
  ...
  prepared.preview = build_annotation_preview(frame, categories,
                                              document_snapshot.objects, false,
                                              projected_scene);
...
inline void store_annotation_projected_scene(
  ...
  store_annotation_projected_scene(
      runtime, std::move(scene),
      make_annotation_current_projected_scene_request(document, session,
                                                      frame));
}

[[nodiscard]] inline bool annotation_workflow_should_reset_canvas_view(
  ...
  return current_frame == nullptr || current_frame->width != next_frame.width ||
         current_frame->height != next_frame.height ||
         current_frame->view_x != next_frame.view_x ||
         current_frame->view_y != next_frame.view_y;
}

inline void
reset_annotation_workflow_runtime(AnnotationWorkflowRuntime &runtime) noexcept {
  runtime.projected_scene_cache.invalidate();
...
[[nodiscard]] inline bool annotation_workflow_preview_running(
    const AnnotationWorkflowRuntime &runtime) noexcept {
  return runtime.preview.running;
}

[[nodiscard]] inline bool
annotation_workflow_preview_pending(const AnnotationWorkflowRuntime &runtime,
                                    const AnnotationFrame *frame) noexcept {
  return frame != nullptr &&
         (runtime.preview.dirty || runtime.preview.frame_id != frame->frame_id);
}

inline void
reset_annotation_workflow_preview(AnnotationWorkflowRuntime &runtime,
...
inline void invalidate_annotation_workflow_preview(
    AnnotationWorkflowRuntime &runtime) noexcept {
  runtime.preview.dirty = true;
  runtime.preview.error_count = 0;
  ++runtime.preview.generation;
...
begin_annotation_workflow_preview(AnnotationWorkflowRuntime &runtime) noexcept {
...
inline void
end_annotation_workflow_preview(AnnotationWorkflowRuntime &runtime) noexcept {
  runtime.preview.running = false;
  runtime.preview.error_count = 0;
}

inline void
note_annotation_workflow_preview_ready(AnnotationWorkflowRuntime &runtime,
                                       const AnnotationFrame &frame) noexcept {
  runtime.preview.running = false;
  runtime.preview.error_count = 0;
  runtime.preview.dirty = false;
...
inline void note_annotation_workflow_preview_error(
    AnnotationWorkflowRuntime &runtime) noexcept {
  runtime.preview.running = false;
  ++runtime.preview.error_count;
}

[[nodiscard]] inline bool annotation_workflow_preview_generation_matches(
    const AnnotationWorkflowRuntime &runtime,
    const std::uint64_t generation) noexcept {
  return runtime.preview.generation == generation;
}

[[nodiscard]] inline bool
annotation_workflow_preview_can_retry(const AnnotationWorkflowRuntime &runtime,
