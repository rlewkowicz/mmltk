...
struct AnnotationProjectedSceneCacheKey {
...
[[nodiscard]] inline std::optional<AnnotationProjectedSceneCacheKey>
make_annotation_projected_scene_cache_key(const AnnotationProjectedSceneCacheInputs& inputs) noexcept {
    if (inputs.document == nullptr || inputs.session == nullptr || inputs.frame == nullptr) {
        return std::nullopt;
    }
    return make_annotation_projected_scene_cache_key(*inputs.document,
                                                    *inputs.session,
                                                    *inputs.frame);
}

[[nodiscard]] inline std::shared_ptr<const AnnotationProjectedScene>
refresh_annotation_projected_scene(
    ...
    if (projected_scene == nullptr ||
        projected_scene->document_generation != document.generation()) {
        return std::make_shared<AnnotationProjectedScene>(
            AnnotationRenderer::build_projected_scene(frame,
                                                     document,
                                                     selected_object_index));
    }
    if (projected_scene->selected_object_index != selected_object_index) {
        return std::make_shared<AnnotationProjectedScene>(
            AnnotationRenderer::refresh_projected_scene_selection(frame,
                                                                  document,
                                                                  *projected_scene,
                                                                  selected_object_index));
    }
...
public:
    ...
    [[nodiscard]] bool matches(const AnnotationProjectedSceneCacheInputs& inputs) const noexcept {
        const std::optional<AnnotationProjectedSceneCacheKey> key =
            make_annotation_projected_scene_cache_key(inputs);
    ...
    [[nodiscard]] std::shared_ptr<const AnnotationProjectedScene> resolve(
        ...
        if (can_refresh_selection(*key) &&
            inputs.document != nullptr &&
            inputs.frame != nullptr) {
            update(std::make_shared<AnnotationProjectedScene>(
                       AnnotationRenderer::refresh_projected_scene_selection(
                           *inputs.frame,
                           *inputs.document,
                           *state_.scene,
                           key->selected_object_index)),
                   *key);
            return state_.scene;
        }
        ...
        update(std::make_shared<AnnotationProjectedScene>(
                   AnnotationRenderer::build_projected_scene(*inputs.frame,
                                                            *inputs.document,
                                                            key->selected_object_index)),
               *key);
        return state_.scene;
