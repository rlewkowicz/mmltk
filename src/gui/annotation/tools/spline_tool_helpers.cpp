...
[[nodiscard]] AnnotationSplineEditState make_spline_edit_state(const std::size_t object_index,
...
bool apply_and_refresh_spline_state(AnnotationDocument& document, const std::size_t selected_index, ApplyFn&& apply_fn,
...
[[nodiscard]] AnnotationSplineHandleMode next_spline_handle_mode(const AnnotationSplineHandleMode mode) noexcept {
    switch (mode) {
...
bool apply_selected_spline_close(AnnotationDocument& document, AnnotationSession& session,
                                 const std::size_t selected_index, const AnnotationSplineEditState& current_state) {
    return apply_and_refresh_spline_state(
        document, selected_index, [&] { return document.apply(AnnotationCloseSplineCommand{selected_index}); },
        [&](const AnnotationSplineShape& updated) {
            set_active_spline_edit_state(session, selected_index, updated, current_state.active_knot_index,
                                         current_state.active_segment_index, false, false);
        });
}

bool apply_selected_spline_reopen(AnnotationDocument& document, AnnotationSession& session,
                                  const std::size_t selected_index, const AnnotationSplineEditState& current_state) {
    return apply_and_refresh_spline_state(
        document, selected_index, [&] { return document.apply(AnnotationReopenSplineCommand{selected_index}); },
        [&](const AnnotationSplineShape& updated) {
            set_active_spline_edit_state(session, selected_index, updated, current_state.active_knot_index,
                                         current_state.active_segment_index, false, false);
        });
...
bool apply_selected_spline_delete_active_knot(AnnotationDocument& document, AnnotationSession& session,
    ...
    return apply_and_refresh_spline_state(
        ...
        [&] {
            return document.apply(AnnotationRemoveSplineKnotCommand{selected_index, *current_state.active_knot_index});
        },
        [&](const AnnotationSplineShape& updated) {
            set_active_spline_edit_state(session, selected_index, updated, current_state.active_knot_index);
        });
...
bool apply_selected_spline_cycle_handle_mode(AnnotationDocument& document, const std::size_t selected_index,
                                             const AnnotationSplineEditState& current_state,
...
[[nodiscard]] bool begin_selected_spline_grouped_edit(const AnnotationToolContext& context,
                                                      const std::size_t selected_index) {
    return begin_grouped_tool_edit(context, AnnotationToolKind::Spline, selected_index, selected_index);
}

[[nodiscard]] AnnotationToolMutation finish_selected_spline_grouped_edit(const AnnotationToolContext& context,
                                                                         const bool changed) {
    if (changed) {
...
AnnotationToolMutation handle_spline_new_object_click(const AnnotationToolContext& context,
    ...
    if (!begin_grouped_tool_edit(context, AnnotationToolKind::Spline, std::nullopt, restore_selection)) {
        return {};
    }

    AnnotationObject object = make_spline_annotation_object(context.document.size(), event.default_category_index);
    ...
    finalize_created_spline_object(context.document, context.session, inserted_index, 0U);

    return make_spline_tool_mutation(true);
}

AnnotationToolMutation handle_spline_existing_object_click(const AnnotationToolContext& context,
                                                           const AnnotationToolCanvasClickEvent& event,
                                                           const std::size_t selected_index,
                                                           const AnnotationSplineEditState& current_state) {
    if (event.double_click || current_state.close_intent) {
        const bool changed = apply_and_refresh_spline_state(
            context.document, selected_index,
            [&] { return context.document.apply(AnnotationCloseSplineCommand{selected_index}); },
            [&](const AnnotationSplineShape& updated) {
                set_active_spline_edit_state(context.session, selected_index, updated, current_state.active_knot_index);
            });
        if (changed) {
            (void)commit_grouped_tool_edit(context, AnnotationToolKind::Spline);
        }
    ...
    if (current_state.active_segment_index.has_value()) {
        if (!begin_selected_spline_grouped_edit(context, selected_index)) {
            return {};
        }
        const bool changed = apply_and_refresh_spline_state(
            context.document, selected_index,
            [&] {
                return context.document.apply(AnnotationInsertSplineKnotCommand{
                    selected_index, *current_state.active_segment_index, static_cast<float>(event.capture_x)});
            },
            [&](const AnnotationSplineShape& updated) {
    ...
    const bool changed = apply_and_refresh_spline_state(
        context.document, selected_index,
        [&] {
            return context.document.apply(
                AnnotationAppendSplineKnotCommand{selected_index, static_cast<float>(event.capture_x)});
        },
        [&](const AnnotationSplineShape& updated) {
            set_active_spline_edit_state(context.session, selected_index, updated, updated.knots.size() - 1U);
        });
    if (!changed) {
        (void)cancel_grouped_tool_edit(context, AnnotationToolKind::Spline);
    }
...
void seed_created_spline_edit_state(AnnotationSession& session, const std::size_t object_index,
                                    const AnnotationObject& object) noexcept {
    const AnnotationSplineShape* spline = std::get_if<AnnotationSplineShape>(&object.shape);
    if (spline == nullptr) {
        return;
    }
    set_active_spline_edit_state(session, object_index, *spline, std::nullopt, std::nullopt, false, false);
}

void finalize_created_spline_object(AnnotationDocument& document, AnnotationSession& session,
    ...
    set_active_spline_edit_state(session, object_index, *spline, active_knot_index, std::nullopt, false, false);
}
...
AnnotationSplineEditState current_or_seed_spline_edit_state(const AnnotationSession& session,
    ...
    if (session.spline_edit_state().object_index == object_index) {
        ...
        return make_spline_edit_state(object_index, shape, current_state.active_knot_index,
                                      current_state.active_segment_index, current_state.close_intent,
                                      current_state.reopen_requested);
    }
    return make_spline_edit_state(
        object_index, shape,
        shape.knots.empty() ? std::optional<std::size_t>{} : std::optional<std::size_t>{shape.knots.size() - 1U},
        std::nullopt, false, false);
}

AnnotationSidebarSelectedSplineViewModel make_selected_spline_sidebar_view_model(const AnnotationSession& session,
                                                                                 const std::size_t object_index,
                                                                                 const AnnotationSplineShape& shape) {
    const AnnotationSplineEditState edit_state = current_or_seed_spline_edit_state(session, object_index, shape);
...
void set_active_spline_edit_state(AnnotationSession& session, const std::size_t object_index,
                                  const AnnotationSplineShape& shape,
                                  const std::optional<std::size_t> active_knot_index,
                                  const std::optional<std::size_t> active_segment_index, const bool close_intent,
                                  const bool reopen_requested) noexcept {
    session.set_spline_edit_state(make_spline_edit_state(object_index, shape, active_knot_index, active_segment_index,
                                                         close_intent, reopen_requested));
}
    ...
    if (selected_index.has_value()) {
        ...
        if (selected_object != nullptr) {
            if (const auto* spline = std::get_if<AnnotationSplineShape>(&selected_object->shape);
                ...
                return handle_spline_existing_object_click(context, event, *selected_index, current_state);
            }
        }
    }
...
AnnotationToolMutation handle_spline_action(const AnnotationToolContext& context,
    ...
    const AnnotationObject* selected_object = context.document.object(*selected_index);
    const AnnotationSplineShape* spline =
        selected_object != nullptr ? std::get_if<AnnotationSplineShape>(&selected_object->shape) : nullptr;
    if (spline == nullptr) {
        return {};
    }
    ...
    switch (event.action) {
        case AnnotationToolActionKind::Confirm: {
            const bool changed =
                apply_selected_spline_close(context.document, context.session, *selected_index, current_state);
            return finish_selected_spline_grouped_edit(context, changed);
        }
        ...
        case AnnotationToolActionKind::DeleteActiveElement:
            ...
            {
                const bool changed = apply_selected_spline_delete_active_knot(context.document, context.session,
                                                                              *selected_index, current_state);
                return finish_selected_spline_grouped_edit(context, changed);
            }
        case AnnotationToolActionKind::CycleHandleMode:
            ...
            {
                const bool changed =
                    apply_selected_spline_cycle_handle_mode(context.document, *selected_index, current_state, *spline);
                if (!changed) {
                    (void)cancel_grouped_tool_edit(context, AnnotationToolKind::Spline);
                    return {};
                }
        ...
        case AnnotationToolActionKind::ReopenSpline: {
            ...
            const bool changed =
                apply_selected_spline_reopen(context.document, context.session, *selected_index, current_state);
            return finish_selected_spline_grouped_edit(context, changed);
        }
...
bool insert_selected_spline_knot_at_active_segment(AnnotationDocument& document, AnnotationSession& session) {
    return apply_selected_object_edit(
        ...
        [&document, &session](const std::size_t selected_object_index, const AnnotationObject& object) {
            ...
            const AnnotationSplineEditState current_state =
                current_or_seed_spline_edit_state(session, selected_object_index, *spline);
            if (!current_state.active_segment_index.has_value()) {
                return false;
            }

            const std::optional<AnnotationPoint> insert_point =
                annotation_spline_segment_point(*spline, *current_state.active_segment_index, 0.5f);
            if (!insert_point.has_value()) {
                return false;
            }

            return apply_and_refresh_spline_state(
                document, selected_object_index,
                [&] {
                    return document.apply(AnnotationInsertSplineKnotCommand{
                        selected_object_index, *current_state.active_segment_index, *insert_point});
                },
                [&](const AnnotationSplineShape& updated) {
                    set_active_spline_edit_state(session, selected_object_index, updated,
                                                 *current_state.active_segment_index + 1U);
                });
        });
}

bool set_selected_spline_active_segment(AnnotationDocument& document, AnnotationSession& session,
                                        const std::optional<std::size_t> segment_index) {
    return apply_selected_object_edit(
        document, session,
        [&session, segment_index](const std::size_t selected_object_index, const AnnotationObject& object) {
            const AnnotationSplineShape* spline = std::get_if<AnnotationSplineShape>(&object.shape);
            if (spline == nullptr) {
                return false;
            }

            const std::size_t segment_count = spline_segment_count(*spline);
            if (segment_index.has_value() && *segment_index >= segment_count) {
                return false;
            }
            ...
            const AnnotationSplineEditState next_state{
                selected_object_index,      current_state.active_knot_index, segment_index,
                current_state.close_intent, current_state.reopen_requested,
            };
            if (current_state.object_index == next_state.object_index &&
                current_state.active_knot_index == next_state.active_knot_index &&
                current_state.active_segment_index == next_state.active_segment_index &&
                current_state.close_intent == next_state.close_intent &&
                current_state.reopen_requested == next_state.reopen_requested) {
                return false;
            }
...
bool set_selected_spline_handle_mode(AnnotationDocument& document, const AnnotationSession& session,
                                     const AnnotationSplineHandleMode mode) {
    return apply_selected_object_edit(
        document, session,
        [&document, &session, mode](const std::size_t selected_object_index, const AnnotationObject& object) {
            const AnnotationSplineShape* spline = std::get_if<AnnotationSplineShape>(&object.shape);
            if (spline == nullptr || spline->knots.empty()) {
                return false;
            }

            std::optional<std::size_t> knot_index = session.spline_edit_state().object_index == selected_object_index
                                                        ? session.spline_edit_state().active_knot_index
                                                        : std::nullopt;
            if (!knot_index.has_value() || *knot_index >= spline->knots.size()) {
...
AnnotationSplineSidebarMutationResult apply_annotation_spline_sidebar_edit(
    ...
    const AnnotationCategories& categories, const AnnotationSplineSidebarEditRequest& request) {
    if (!request.request_insert_active_spline_knot && !request.update_spline_active_segment &&
        !request.spline_handle_mode.has_value() && !request.spline_action.has_value()) {
        return {};
    }

    AnnotationSplineSidebarMutationResult result;
    {
        const bool has_direct_sidebar_edit = request.request_insert_active_spline_knot ||
                                             request.update_spline_active_segment ||
                                             request.spline_handle_mode.has_value();
        AnnotationEditorTransactionScope transaction(document, has_direct_sidebar_edit);

        if (request.update_spline_active_segment &&
            set_selected_spline_active_segment(document, session, request.spline_active_segment_index)) {
    ...
    if (request.spline_action.has_value() && frame != nullptr) {
        const AnnotationToolMutation mutation = handle_spline_action(
