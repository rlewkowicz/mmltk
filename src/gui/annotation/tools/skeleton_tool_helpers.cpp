...
std::string skeleton_joint_label(const AnnotationSkeletonNode& node,
...
AnnotationToolMutation handle_skeleton_new_object_click(const AnnotationToolContext& context,
    ...
    const AnnotationCategory* category =
        event.default_category_index < context.categories.items.size()
            ? &context.categories.items[event.default_category_index]
            : nullptr;
    AnnotationObject object = make_skeleton_annotation_object(context.document.size(),
...
void seed_created_skeleton_edit_state(AnnotationSession& session,
                                     const std::size_t object_index,
    ...
    set_active_skeleton_edit_state(session,
                                   object_index,
                                   *skeleton,
                                   first_hidden_skeleton_node_index(skeleton),
                                   std::nullopt,
                                   false);
}

void finalize_created_skeleton_object(AnnotationDocument& document,
    ...
    if (!session.skeleton_edit_state().active_joint_index.has_value()) {
        (void)commit_grouped_tool_edit(document,
                                       session,
                                       AnnotationToolKind::Skeleton);
    }
}

AnnotationSkeletonEditState current_or_seed_skeleton_edit_state(
    ...
    if (session.skeleton_edit_state().object_index == object_index) {
        ...
        return make_skeleton_edit_state(object_index,
                                        shape,
                                        current_state.active_joint_index,
                                        current_state.last_placed_joint_index,
                                        current_state.reseed_requested);
    }
    return make_skeleton_edit_state(object_index,
                                    shape,
                                    first_hidden_skeleton_node_index(&shape),
                                    std::nullopt,
                                    false);
}

AnnotationSidebarSelectedSkeletonViewModel
make_selected_skeleton_sidebar_view_model(const AnnotationSession& session,
                                          const std::size_t object_index,
                                          const AnnotationSkeletonShape& shape) {
    const AnnotationSkeletonEditState edit_state =
        current_or_seed_skeleton_edit_state(session, object_index, shape);
    const AnnotationGroupedEditTransactionState grouped_edit =
        session.grouped_edit_transaction();
    const std::optional<std::size_t> active_joint_index =
        edit_state.active_joint_index;
    const std::optional<std::size_t> next_joint_index =
        next_skeleton_node_index(&shape, active_joint_index);
...
void set_active_skeleton_edit_state(AnnotationSession& session,
                                    const std::size_t object_index,
                                    const AnnotationSkeletonShape& shape,
                                    const std::optional<std::size_t> active_joint_index,
                                    const std::optional<std::size_t> last_placed_joint_index,
                                    const bool reseed_requested) noexcept {
    session.set_skeleton_edit_state(make_skeleton_edit_state(object_index,
                                                             shape,
                                                             active_joint_index,
                                                             last_placed_joint_index,
                                                             reseed_requested));
}

bool update_selected_skeleton_object_metadata(AnnotationDocument& document,
                                              AnnotationSession& session,
    ...
    const std::size_t resolved_category_index =
        categories.items.empty()
            ? 0U
            : std::min(category_index, categories.items.size() - 1U);
    return apply_selected_object_edit(document,
                                      ...
                                      [&](const std::size_t selected_object_index,
                                          ...
                                          const bool category_changed =
                                              object.category_index != resolved_category_index;
                                          ...
                                          return apply_selected_object_transaction(document, [&]() {
                                              ...
                                              if (category_changed) {
                                                  if (std::holds_alternative<
                                                          AnnotationSkeletonShape>(object.shape)) {
                                                      AnnotationObject updated_object = object;
                                                      updated_object.category_index =
                                                          resolved_category_index;
                                                      const AnnotationCategory* target_category =
                                                          resolved_category_index <
                                                                  categories.items.size()
                                                              ? &categories.items
                                                                   [resolved_category_index]
                                                              : nullptr;
                                                      (void)remap_skeleton_annotation_object_to_category(
                                                          &updated_object,
                                                          target_category);
                                                      AnnotationSkeletonShape remapped_shape =
                                                          std::get<AnnotationSkeletonShape>(
                                                              std::move(updated_object.shape));
                                                      changed = document.apply(
...
AnnotationToolMutation handle_skeleton_canvas_click(const AnnotationToolContext& context,
    ...
    if (selected_index.has_value()) {
        ...
        if (selected_object != nullptr &&
            std::holds_alternative<AnnotationSkeletonShape>(selected_object->shape)) {
            const AnnotationSkeletonShape* skeleton =
                std::get_if<AnnotationSkeletonShape>(&selected_object->shape);
            if (skeleton == nullptr) {
                return {};
            }
...
bool set_selected_skeleton_active_joint(
    ...
    return apply_selected_object_edit(document,
                                      ...
                                      [&session, joint_index](
                                          ...
                                          if (joint_index.has_value() &&
                                              *joint_index >= skeleton->nodes.size()) {
                                              return false;
                                          }

                                          const AnnotationSkeletonEditState current_state =
                                              current_or_seed_skeleton_edit_state(session,
                                                                                  selected_object_index,
                                                                                  *skeleton);
                                          ...
                                          if (current_state.object_index ==
                                                  next_state.object_index &&
                                              current_state.active_joint_index ==
                                                  next_state.active_joint_index &&
                                              current_state.last_placed_joint_index ==
                                                  next_state.last_placed_joint_index &&
                                              current_state.reseed_requested ==
                                                  next_state.reseed_requested) {
                                              return false;
                                          }
...
AnnotationSkeletonSidebarMutationResult apply_annotation_skeleton_sidebar_edit(
    ...
    const AnnotationCategories& categories,
    const AnnotationSkeletonSidebarEditRequest& request) {
    if (!request.update_skeleton_active_joint &&
        !request.skeleton_action.has_value()) {
        return {};
    }
    ...
    if (request.update_skeleton_active_joint &&
        set_selected_skeleton_active_joint(document,
                                           session,
    ...
    if (request.skeleton_action.has_value() && frame != nullptr) {
        const AnnotationToolMutation mutation =
            handle_skeleton_action(AnnotationToolContext{
                                       document,
                                       session,
                                       *frame,
                                       categories,
