...
std::size_t clamp_category_index(const AnnotationCategories& categories,
...
std::size_t selected_category_index(const AnnotationDocument& document,
...
bool AnnotationController::set_active_tool(const AnnotationToolKind kind,
...
bool AnnotationController::handle_canvas_click(AnnotationDocument& document,
                                               AnnotationSession& session,
    ...
    const AnnotationToolMutation mutation = tool->handle_canvas_click(
        make_tool_context(document, session, frame, categories),
...
bool AnnotationController::handle_box_commit(AnnotationDocument& document,
                                             AnnotationSession& session,
                                             const AnnotationFrame& frame,
                                             const AnnotationCategories& categories,
                                             const RectDragKind drag_kind,
    ...
    const AnnotationToolMutation mutation = tool->handle_box_commit(
        make_tool_context(document, session, frame, categories),
...
bool AnnotationController::handle_brush_sample(AnnotationDocument& document,
                                               AnnotationSession& session,
                                               const AnnotationFrame& frame,
                                               const AnnotationCategories& categories,
                                               const int capture_x,
    ...
    return tool->handle_brush_sample(make_tool_context(document, session, frame, categories),
                                     AnnotationToolBrushEvent{
                                         capture_x,
                                         capture_y,
                                         radius,
                                     });
...
bool AnnotationController::handle_handle_drag(AnnotationDocument& document,
                                              AnnotationSession& session,
                                              const AnnotationFrame& frame,
                                              const AnnotationCategories& categories,
...
bool AnnotationController::handle_fill(AnnotationDocument& document,
                                       AnnotationSession& session,
                                       const AnnotationFrame& frame,
                                       const AnnotationCategories& categories,
                                       const int capture_x,
    ...
    return tool->handle_fill(make_tool_context(document, session, frame, categories),
                             AnnotationToolFillEvent{
                                 capture_x,
                                 capture_y,
                             });
}

bool AnnotationController::handle_tool_action(AnnotationDocument& document,
                                              AnnotationSession& session,
                                              const AnnotationFrame& frame,
                                              const AnnotationCategories& categories,
                                              const AnnotationToolActionKind action) {
    return handle_tool_action_for_kind(session.active_tool(),
                                       document,
                                       session,
                                       frame,
                                       categories,
...
bool AnnotationController::handle_tool_action_for_kind(const AnnotationToolKind kind,
                                                       AnnotationDocument& document,
                                                       AnnotationSession& session,
                                                       const AnnotationFrame& frame,
                                                       const AnnotationCategories& categories,
    ...
    const AnnotationToolMutation mutation = tool->handle_action(
        make_tool_context(document, session, frame, categories),
        AnnotationToolActionEvent{action});
    return apply_tool_mutation(mutation, document, session);
}

std::optional<std::size_t> AnnotationController::create_object(AnnotationToolKind kind,
                                                               AnnotationDocument& document,
                                                               AnnotationSession& session,
                                                               const AnnotationFrame& frame,
                                                               const AnnotationCategories& categories,
    ...
    const std::size_t normalized_category_index =
        clamp_category_index(categories, category_index);
    const AnnotationPoint initial_point = annotation_frame_capture_center(frame);
    const AnnotationCategory* skeleton_category =
        normalized_category_index < categories.items.size()
            ? &categories.items[normalized_category_index]
            : nullptr;
    ...
    const AnnotationEditTransactionKind grouped_edit_kind =
        tool_detail::grouped_edit_kind_for_tool(kind);
    const bool grouped_edit = grouped_edit_kind != AnnotationEditTransactionKind::None;
    if (grouped_edit &&
        !tool_detail::begin_grouped_tool_edit(document,
                                              session,
                                              kind,
                                              std::nullopt,
                                              normalize_selected_object_index(document,
                                                                              session))) {
        return std::nullopt;
    }
    if (!object.has_value() ||
        !document.apply(AnnotationInsertObjectCommand{std::move(*object), std::nullopt})) {
        if (grouped_edit) {
            (void)tool_detail::cancel_active_grouped_tool_edit(document, session);
        }
        return std::nullopt;
...
AnnotationToolContext AnnotationController::make_tool_context(AnnotationDocument& document,
                                                              AnnotationSession& session,
                                                              const AnnotationFrame& frame,
                                                              const AnnotationCategories& categories) {
...
bool AnnotationController::apply_tool_mutation(const AnnotationToolMutation& mutation,
                                               AnnotationDocument& document,
                                               AnnotationSession& session) {
