...
AnnotationEditorTransactionScope::AnnotationEditorTransactionScope(
    ...
    : document_(&document),
      owns_transaction_(begin_if_needed && !document.transaction_active()) {
...
bool cancel_grouped_edit_if_kind_matches(AnnotationDocument& document,
...
std::optional<std::size_t> normalize_selected_object_index(
...
const AnnotationObject* selected_object(const AnnotationDocument& document,
                                        const AnnotationSession& session) noexcept {
    const auto& selected_object_index =
        normalize_selected_object_index(document, session);
    return selected_object_index.has_value()
               ? document.object(*selected_object_index)
               : nullptr;
}

void select_object(AnnotationSession& session,
                   AnnotationDocument& document,
                   std::optional<std::size_t> index) noexcept {
    if (index.has_value() && *index >= document.size()) {
        index.reset();
    }
    if (cancel_grouped_edit_for_selection(document, session, index) &&
        index.has_value() && *index >= document.size()) {
        index.reset();
    }
    session.select_object(index);
}

std::optional<AnnotationSelectedObjectContext> selected_object_context(
    const AnnotationDocument& document,
...
std::string selected_object_id_for_history(const AnnotationDocument& document,
                                           const AnnotationSession& session) {
    const auto& selected_object_index =
        normalize_selected_object_index(document, session);
    if (!selected_object_index.has_value()) {
...
void repair_selection_after_history_jump(AnnotationDocument& document,
                                         AnnotationSession& session,
...
bool begin_grouped_edit(AnnotationDocument& document,
                        AnnotationSession& session,
                        const AnnotationEditTransactionKind kind,
    ...
    const AnnotationGroupedEditTransactionState current_state =
        session.grouped_edit_transaction();
    if (current_state.kind == kind &&
        current_state.object_index == object_index &&
...
bool commit_grouped_edit(AnnotationDocument& document,
                         AnnotationSession& session,
                         const AnnotationEditTransactionKind kind) {
    const AnnotationGroupedEditTransactionState current_state =
        session.grouped_edit_transaction();
    ...
    const bool changed =
        document.transaction_active() ? document.commit_transaction() : false;
    session.clear_grouped_edit_transaction();
    return changed;
}

bool cancel_grouped_edit(AnnotationDocument& document,
                         AnnotationSession& session,
                         const AnnotationEditTransactionKind kind,
                         std::optional<std::size_t> selection_after_cancel) {
    return cancel_grouped_edit_if_kind_matches(document,
                                               session,
                                               kind,
                                               selection_after_cancel);
}

bool cancel_active_grouped_edit(AnnotationDocument& document,
                                AnnotationSession& session,
                                std::optional<std::size_t> selection_after_cancel) {
...
bool cancel_grouped_edit_for_object_index(AnnotationDocument& document,
                                          AnnotationSession& session,
    ...
    if (grouped_edit.kind == AnnotationEditTransactionKind::None ||
        !grouped_edit.object_index.has_value() ||
...
bool cancel_grouped_edit_for_selection(AnnotationDocument& document,
                                       AnnotationSession& session,
                                       const std::optional<std::size_t> selection_after_cancel) {
    const AnnotationGroupedEditTransactionState& grouped_edit =
        session.grouped_edit_transaction();
