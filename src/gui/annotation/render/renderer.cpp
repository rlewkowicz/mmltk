...
mmltk::live::ManualOverlayPoint interaction_point_to_capture(const AnnotationFrame& frame,
                                                             const PreviewInteractionOverlayPoint& point) {
    return mmltk::live::ManualOverlayPoint{
        point.x + static_cast<int>(frame.view_x),
        point.y + static_cast<int>(frame.view_y),
    };
}

mmltk::live::ManualOverlayStyle interaction_style(const std::uint8_t r, const std::uint8_t g, const std::uint8_t b,
                                                  const std::uint8_t alpha, const int line_thickness,
                                                  const int point_radius, const bool draw_handles = false,
                                                  const int handle_radius = 4) {
    return mmltk::live::ManualOverlayStyle{
        r,
        g,
        b,
        alpha,
        std::max(1, line_thickness),
        std::max(1, point_radius),
        draw_handles,
        std::max(1, handle_radius),
    };
}

void append_interaction_instances(const AnnotationFrame& frame, const PreviewInteractionOverlaySnapshot& interaction,
                                  mmltk::live::ManualOverlayDocumentSnapshot* snapshot) {
    if (snapshot == nullptr) {
        return;
    }
    snapshot->interaction_instances.reserve(interaction.boxes.size() + interaction.polylines.size() +
                                            interaction.marker_sets.size() + interaction.skeletons.size());
    std::size_t primitive_index = 0U;
    for (const PreviewInteractionOverlayBox& box : interaction.boxes) {
        mmltk::live::ManualOverlayInstance instance;
        instance.instance_id = "interaction/box/" + std::to_string(primitive_index++);
        instance.box = mmltk::live::ManualOverlayBox{
            box.box.x1 + static_cast<int>(frame.view_x),
            box.box.y1 + static_cast<int>(frame.view_y),
            box.box.x2 + static_cast<int>(frame.view_x),
            box.box.y2 + static_cast<int>(frame.view_y),
        };
        instance.style = interaction_style(box.r, box.g, box.b, 255U, box.thickness, box.handle_radius,
                                           box.draw_handles, box.handle_radius);
        snapshot->interaction_instances.push_back(std::move(instance));
    }
    for (const PreviewInteractionOverlayPolyline& polyline : interaction.polylines) {
        mmltk::live::ManualOverlayInstance instance;
        instance.instance_id = "interaction/polyline/" + std::to_string(primitive_index++);
        instance.polyline_points.reserve(polyline.points.size());
        for (const PreviewInteractionOverlayPoint& point : polyline.points) {
            instance.polyline_points.push_back(interaction_point_to_capture(frame, point));
        }
        instance.polyline_closed = polyline.closed;
        instance.style = interaction_style(polyline.r, polyline.g, polyline.b, 255U, polyline.thickness, 3);
        snapshot->interaction_instances.push_back(std::move(instance));
    }
    for (const PreviewInteractionOverlayMarkerSet& markers : interaction.marker_sets) {
        mmltk::live::ManualOverlayInstance instance;
        instance.instance_id = "interaction/markers/" + std::to_string(primitive_index++);
        instance.points.reserve(markers.points.size());
        for (const PreviewInteractionOverlayPoint& point : markers.points) {
            instance.points.push_back(interaction_point_to_capture(frame, point));
        }
        instance.style = interaction_style(markers.r, markers.g, markers.b, markers.alpha, 1, markers.radius);
        snapshot->interaction_instances.push_back(std::move(instance));
    }
    for (const PreviewInteractionOverlaySkeleton& skeleton : interaction.skeletons) {
        mmltk::live::ManualOverlayInstance instance;
        instance.instance_id = "interaction/skeleton/" + std::to_string(primitive_index++);
        instance.points.reserve(skeleton.points.size());
        for (const PreviewInteractionOverlayPoint& point : skeleton.points) {
            instance.points.push_back(interaction_point_to_capture(frame, point));
        }
        instance.skeleton_edges.reserve(skeleton.edges.size());
        for (const PreviewInteractionOverlayEdge& edge : skeleton.edges) {
            instance.skeleton_edges.push_back(mmltk::live::ManualOverlayEdge{edge.source_index, edge.target_index});
        }
        instance.style = interaction_style(skeleton.r, skeleton.g, skeleton.b, 255U, skeleton.thickness, 3);
        snapshot->interaction_instances.push_back(std::move(instance));
    }
}
...
mmltk::live::ManualOverlayDocumentSnapshot AnnotationRenderer::build_manual_overlay_snapshot(
    ...
    const std::optional<AnnotationBox> crop_capture_box) {
    const AnnotationProjectedScene scene = build_projected_scene(frame, document, session.selected_object_index());
    return build_manual_overlay_snapshot(frame, document, session, scene, crop_capture_box);
}

mmltk::live::ManualOverlayDocumentSnapshot AnnotationRenderer::build_manual_overlay_snapshot(
    const AnnotationFrame& frame, const AnnotationDocument& document, const AnnotationSession& session,
    const AnnotationProjectedScene& scene, const std::optional<AnnotationBox> crop_capture_box) {
    mmltk::live::ManualOverlayDocumentSnapshot snapshot;
    ...
    const auto projected_draft_box = [&frame](const PreviewRectDragSession& drag) -> std::optional<AnnotationBox> {
        if (!drag.active || !annotation_box_has_area(drag.draft_box)) {
            return std::nullopt;
        }
        return AnnotationRenderer::project_capture_box(frame, drag.draft_box).frame_box;
    };
    const std::optional<AnnotationBox> crop_frame_box =
        session.crop_drag_session().active
            ? std::optional<AnnotationBox>{session.crop_drag_session().draft_box}
            : (crop_capture_box.has_value()
                   ? std::optional<AnnotationBox>{AnnotationRenderer::project_capture_box(frame, *crop_capture_box)
                                                      .frame_box}
                   : std::nullopt);
    const std::optional<AnnotationHandleDragState>& handle_drag = session.handle_drag();
    const std::optional<AnnotationHandleId> active_handle = handle_drag.has_value() && handle_drag->active
                                                                ? std::optional<AnnotationHandleId>{handle_drag->handle}
                                                                : std::nullopt;
    const AnnotationInteractionOverlayRequest interaction_request =
        AnnotationRenderer::build_interaction_overlay_request(
            frame, 0, scene, session.hovered_object_index(), session.direct_drag_index(), crop_frame_box, 4,
            projected_draft_box(session.direct_drag_session()), projected_draft_box(session.create_drag_session()),
            session.hovered_handle(), active_handle);
    append_interaction_instances(frame, AnnotationRenderer::build_interaction_overlay_snapshot(interaction_request),
                                 &snapshot);
    return snapshot;
}
