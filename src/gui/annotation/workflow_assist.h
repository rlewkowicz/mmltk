...
[[nodiscard]] inline AnnotationWorkflowAssistOutcome
run_annotation_workflow_assist(
...
[[nodiscard]] inline AnnotationWorkflowAssistLaunchDecision
decide_annotation_workflow_assist_launch(
...
[[nodiscard]] inline AnnotationWorkflowAssistDispatchPlan
plan_annotation_workflow_assist_dispatch(
    const AnnotationWorkflowAssistLaunchRequest &request,
    const bool annotation_live_running) {
  AnnotationWorkflowAssistLaunchDecision decision =
      decide_annotation_workflow_assist_launch(request);
...
make_annotation_workflow_assist_execution_request(
    const AnnotationFrame &frame_snapshot, const AnnotateViewState &annotate,
    std::string preset_name, std::string *error_message = nullptr) {
  try {
    AnnotationWorkflowAssistExecutionRequest request;
    request.frame =
        annotate.source.kind == SourceKind::VideoStream
            ? extract_annotation_frame_region(
                  frame_snapshot, resolved_video_crop_box(annotate.source))
            : frame_snapshot;
...
[[nodiscard]] inline AnnotationWorkflowAssistOutcome
make_annotation_workflow_assist_outcome(
    const AnnotationFrame &assist_frame,
    const mmltk::rfdetr::PredictionRunResult &result) {
  if (result.records.size() != 1U) {
    throw std::runtime_error(
        "annotation assist expected exactly one prediction record");
  }
  ...
  for (const mmltk::rfdetr::Prediction &prediction :
       result.records.front().detections) {
    ...
    const auto class_index =
        static_cast<std::size_t>(prediction.category_id - 1);
    ...
    const AnnotationBox local_box = rfdetr_workflows::prediction_to_annotation_box(
        prediction, assist_frame.width, assist_frame.height);
    seed.box = annotation_box_from_frame(assist_frame, local_box);
    if (prediction.has_mask) {
      seed.mask = decode_annotation_prediction_mask(
          prediction.mask, assist_frame.width, assist_frame.height);
...
[[nodiscard]] inline std::vector<AnnotationObject>
make_annotation_workflow_assist_objects(
    AnnotationCategories &categories,
    const std::vector<AnnotationWorkflowAssistSeed> &seeds,
  ...
  for (std::size_t index = 0; index < seeds.size(); ++index) {
    ...
    object.object_id =
        "assist-" + std::to_string(first_object_index + index + 1U);
    object.category_index =
        ensure_annotation_category(categories, seed.class_name);
    if (seed.has_mask) {
      object.shape = AnnotationMaskShape{
          seed.box,           seed.mask_region,        seed.mask,
          seed.seed_frame_id, seed.seed_live_frame_id,
      };
    } else {
...
[[nodiscard]] inline bool annotation_workflow_assist_outcome_matches_frame(
    ...
    const AnnotationWorkflowAssistOutcome &outcome) noexcept {
  return frame != nullptr &&
         annotation_frame_matches_saved_identity(*frame, outcome.frame_id,
                                                 outcome.live_frame_id);
}

[[nodiscard]] inline AnnotationWorkflowAssistApplyResult
apply_annotation_workflow_assist_outcome(
    AnnotationDocument &document, AnnotationSession &session,
    AnnotationCategories &categories,
  ...
  std::vector<AnnotationObject> objects = make_annotation_workflow_assist_objects(
      categories, outcome.seeds, *result.first_inserted_index);
  for (AnnotationObject &object : objects) {
    result.changed = AnnotationEditor::insert_object(document, session,
                                                     std::move(object)) ||
                     result.changed;
  }
  if (!result.changed) {
...
template <typename EnsureLiveFramePixelsFn, typename MakeExecutionRequestFn,
          typename OnSuccessFn, typename OnErrorFn>
[[nodiscard]] inline bool launch_annotation_workflow_assist(
    const AnnotationWorkflowAssistLaunchRequest &request,
    const bool annotation_live_running,
    AnnotationWorkflowAssistLaunchState state,
    ...
    MakeExecutionRequestFn &&make_execution_request,
    OnSuccessFn &&on_success, OnErrorFn &&on_error) {
  const AnnotationWorkflowAssistDispatchPlan plan =
      plan_annotation_workflow_assist_dispatch(request,
  ...
  if (plan.refresh_live_frame) {
    ...
    if (!std::forward<EnsureLiveFramePixelsFn>(ensure_live_frame_pixels)(
            &frame_error)) {
  ...
  mmltk::runtime::submit_background_task(
      ...
      [state, on_success = std::forward<OnSuccessFn>(on_success)](
          AnnotationWorkflowAssistOutcome outcome) mutable {
        if (state.running != nullptr) {
          *state.running = false;
        }
        if (state.error != nullptr) {
      ...
      [state, on_error = std::forward<OnErrorFn>(on_error)](
...
run_annotation_workflow_assist(
  ...
  const std::filesystem::path temp_path =
      ...
      ("mmltk-annotate-" + std::to_string(request.frame.frame_id) + ".png");
  try {
    ...
    const mmltk::rfdetr::PredictOptions options =
        rfdetr_workflows::build_annotate_predict_options(
            request.annotate, request.preset_name, std::move(input));
