...
public:
    AnnotationEditorTransactionScope(AnnotationDocument& document,
                                     bool begin_if_needed) noexcept;
...
[[nodiscard]] std::optional<std::size_t> normalize_selected_object_index(
...
[[nodiscard]] std::optional<AnnotationSelectedObjectContext> selected_object_context(
    const AnnotationDocument& document,
...
bool apply_selected_object_edit(AnnotationDocument& document,
                                const AnnotationSession& session,
                                Fn&& fn) {
    const std::optional<AnnotationSelectedObjectContext> context =
        selected_object_context(document, session);
...
[[nodiscard]] std::optional<std::size_t> clamp_selection_index(
    const AnnotationDocument& document,
    std::optional<std::size_t> index) noexcept;
...
[[nodiscard]] bool begin_grouped_edit(AnnotationDocument& document,
                                      AnnotationSession& session,
                                      AnnotationEditTransactionKind kind,
                                      std::optional<std::size_t> object_index,
                                      std::optional<std::size_t> restore_selection_index);
[[nodiscard]] bool commit_grouped_edit(AnnotationDocument& document,
                                       AnnotationSession& session,
                                       AnnotationEditTransactionKind kind);
[[nodiscard]] bool cancel_grouped_edit(AnnotationDocument& document,
                                       AnnotationSession& session,
                                       AnnotationEditTransactionKind kind,
                                       std::optional<std::size_t> selection_after_cancel =
                                           std::nullopt);
[[nodiscard]] bool cancel_active_grouped_edit(AnnotationDocument& document,
                                              AnnotationSession& session,
                                              std::optional<std::size_t> selection_after_cancel =
                                                  std::nullopt);
...
[[nodiscard]] bool cancel_grouped_edit_for_selection(
    AnnotationDocument& document,
    AnnotationSession& session,
    std::optional<std::size_t> selection_after_cancel);
[[nodiscard]] bool should_group_transaction_for_change_count(
    std::size_t change_count) noexcept;
