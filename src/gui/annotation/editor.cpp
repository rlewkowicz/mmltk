...
bool AnnotationEditor::insert_object(AnnotationDocument& document, AnnotationSession& session, AnnotationObject object,
                                     const std::optional<std::size_t> index) {
    const std::optional<std::size_t> previous_selection = normalize_selected_object_index(document, session);
    const std::size_t insert_index = std::min(index.value_or(document.size()), document.size());
...
bool AnnotationEditor::replace_object(AnnotationDocument& document, const std::size_t index, AnnotationObject object) {
...
bool AnnotationEditor::remove_object(AnnotationDocument& document, AnnotationSession& session,
    ...
    if (*previous_selection == index || *previous_selection >= document.size()) {
        select_object(session, document, std::min(index, document.size() - 1U));
...
bool AnnotationEditor::remove_selected_object(AnnotationDocument& document, AnnotationSession& session) {
...
bool AnnotationEditor::undo(AnnotationDocument& document, AnnotationSession& session) {
    const std::optional<std::size_t> previous_index = normalize_selected_object_index(document, session);
    const std::string previous_object_id = selected_object_id_for_history(document, session);
    if (!document.undo()) {
        return false;
    }
    repair_selection_after_history_jump(document, session, previous_index, previous_object_id);
    return true;
}

bool AnnotationEditor::redo(AnnotationDocument& document, AnnotationSession& session) {
    const std::optional<std::size_t> previous_index = normalize_selected_object_index(document, session);
    const std::string previous_object_id = selected_object_id_for_history(document, session);
    if (!document.redo()) {
        return false;
...
bool AnnotationEditor::ensure_default_category(AnnotationCategories& categories, const std::string_view fallback_name) {
    if (!categories.items.empty()) {
        return false;
    }
    const std::string name = fallback_name.empty() ? std::string("object") : std::string(fallback_name);
    (void)ensure_annotation_category(categories, name);
    return true;
}

bool AnnotationEditor::add_category(AnnotationCategories& categories, const std::string_view name) {
    if (name.empty()) {
        return false;
    }
...
bool AnnotationEditor::reset_selected_object_box(AnnotationDocument& document, const AnnotationSession& session,
                                                 const AnnotationFrame& frame) {
    return apply_selected_object_edit(document, session,
...
bool AnnotationEditor::cleanup_selected_mask(AnnotationDocument& document, const AnnotationSession& session,
                                             const AnnotationFrame& frame, const AnnotationMaskCleanupOp op,
                                             const int radius) {
    return apply_selected_object_edit(document, session,
...
bool AnnotationEditor::update_selected_object_color_ranges(AnnotationDocument& document,
    ...
    return apply_selected_object_edit(document, session,
                                      [&](const std::size_t selected_object_index, const AnnotationObject&) {
                                          return apply_selected_object_transaction(document, [&]() {
                                              return document.apply(AnnotationSetObjectColorRangesCommand{
                                                  selected_object_index,
                                                  sup,
                                                  nosup,
                                              });
