...
AnnotationWorkspaceViewModel AnnotationWorkspaceModelBuilder::build(
    ...
    if (projected_scene == nullptr) {
        projected_scene = refresh_annotation_projected_scene(frame,
                                                             document,
                                                             {},
                                                             std::nullopt);
    }
    ...
    if (model.selection.selected_object_index.has_value()) {
        ...
        } else if (const AnnotationObject* object =
                       document.object(*model.selection.selected_object_index);
                   object != nullptr) {
            const std::optional<AnnotationBox> selected_box =
                annotation_object_display_box(*object);
            if (selected_box.has_value()) {
                const AnnotationBox normalized_box =
                    normalize_annotation_box(*selected_box,
                ...
                if (annotation_box_has_area(normalized_box)) {
                    ...
                    model.selection.selected_box_fully_visible =
                        projected.fully_visible;
                }
            }
        }
    }
...
AnnotationSidebarViewModel AnnotationSidebarModelBuilder::build(
    const AnnotationDocument& document,
    ...
    model.selected_object_index =
        normalize_selected_object_index(document, session);
    if (model.selected_object_index.has_value()) {
        if (const AnnotationObject* selected_object =
                document.object(*model.selected_object_index);
            selected_object != nullptr) {
            model.preferred_new_object_category_index =
                selected_object->category_index;
        }
    }
    if (!categories.items.empty()) {
        model.preferred_new_object_category_index =
            std::min(model.preferred_new_object_category_index,
                     categories.items.size() - 1U);
    } else {
    ...
    for (std::size_t index = 0; index < document.size(); ++index) {
        ...
        const std::string class_name =
            object->category_index < categories.items.size()
        ...
        model.objects.push_back(AnnotationSidebarObjectViewModel{
            ...
            "#" + std::to_string(index + 1U) + " " + class_name + " [" +
                annotation_object_shape_label(*object) + "]",
            model.selected_object_index.has_value() &&
                *model.selected_object_index == index,
        });
    }
    ...
    if (model.selected_object_index.has_value()) {
        if (const AnnotationObject* selected_object =
            ...
            selected_model.display_box =
                annotation_object_display_box(*selected_object);
            selected_model.supports_mask_editing =
    ...
    const bool box_creation_armed =
        session.active_tool() == AnnotationToolKind::Box ||
    ...
    model.can_edit_selected_mask =
        model.selected_object.has_value() &&
        model.selected_object->supports_mask_editing;
    model.can_redraw_selected_box =
