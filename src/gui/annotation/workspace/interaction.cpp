...
bool selected_object_supports_mask_editing(const AnnotationDocument& document,
...
void rasterize_line_samples(const int x0,
                            const int y0,
                            const int x1,
                            const int y1,
                            SampleFn&& sample_fn) {
    ...
    for (int step = 0; step <= steps; ++step) {
        ...
        const int x =
            static_cast<int>(std::round(static_cast<float>(x0) +
                                        static_cast<float>(x1 - x0) * t));
        const int y =
            static_cast<int>(std::round(static_cast<float>(y0) +
                                        static_cast<float>(y1 - y0) * t));
        sample_fn(x, y);
    }
}

std::optional<AnnotationEditableHandle> hit_test_editable_handle(
    ...
    const float hit_radius =
        std::clamp(canvas_layout.state.scale * 4.0f,
                   kMinHandleHitRadiusPx,
                   kMaxHandleHitRadiusPx);
    ...
    for (const AnnotationEditableHandle& handle : handles) {
        const float handle_screen_x =
            canvas_layout.image_screen_x + handle.frame_point.x * canvas_layout.state.scale;
        const float handle_screen_y =
            canvas_layout.image_screen_y + handle.frame_point.y * canvas_layout.state.scale;
    ...
    return best_handle != nullptr ? std::optional<AnnotationEditableHandle>{*best_handle}
                                  : std::nullopt;
}
...
AnnotationWorkspaceInteractionResult process_annotation_workspace_interaction(
    ...
    std::shared_ptr<const AnnotationProjectedScene> interaction_projected_scene =
        workspace_view.projected_scene;
    const auto ensure_projected_scene_up_to_date =
        [&](const std::optional<std::size_t> selected_object_index)
            -> const AnnotationProjectedScene* {
            interaction_projected_scene =
                refresh_annotation_projected_scene(frame,
                                                  document,
                                                  interaction_projected_scene,
                                                  selected_object_index);
            return interaction_projected_scene.get();
        };
    const std::optional<std::size_t> initial_selected_object_index =
        normalize_selected_object_index(document, session);
    ...
    const std::vector<AnnotationVisibleObject>& workspace_visible_objects =
        overlay_projected_scene != nullptr ? overlay_projected_scene->visible_objects
                                           : kEmptyVisibleObjects;
    const std::vector<AnnotationEditableHandle>& workspace_editable_handles =
        overlay_projected_scene != nullptr ? overlay_projected_scene->editable_handles
                                           : kEmptyEditableHandles;

    const std::optional<AnnotationVisibleObjectHit> hovered_box_hit =
        config.overlay_hovered && !direct_drag_session.active
            ? AnnotationRenderer::hit_test_visible_objects(workspace_visible_objects,
                                                           canvas_layout.viewport,
                                                           canvas_pointer,
                                                           config.image_x,
                                                           config.image_y,
    ...
    const std::optional<AnnotationVisibleObject> hovered_box =
        hovered_box_hit.has_value()
            ? std::optional<AnnotationVisibleObject>{hovered_box_hit->object}
            : std::nullopt;
    const RectDragKind hovered_box_drag_kind =
        hovered_box_hit.has_value()
            ? hovered_box_hit->drag_kind
            : RectDragKind::None;

    const bool handle_drag_enabled =
        session.active_tool() == AnnotationToolKind::Direct &&
        !create_drag_session.active &&
        !crop_drag_session.active &&
        !direct_drag_session.active &&
    ...
    const std::optional<AnnotationEditableHandle> hovered_handle =
        handle_drag_enabled && config.overlay_hovered
            ? hit_test_editable_handle(workspace_editable_handles,
                                       canvas_layout,
                                       canvas_pointer)
            : std::nullopt;
    session.set_hovered_handle(hovered_handle.has_value()
                                   ? std::optional<AnnotationHandleId>{hovered_handle->id}
                                   : std::nullopt);
    ...
    const AnnotationBox active_crop_box =
        crop_drag_session.active ? crop_drag_session.draft_box
                                 : crop_box.value_or(AnnotationBox{});
    const bool crop_drag_enabled =
        crop_box.has_value() &&
        session.active_tool() == AnnotationToolKind::Direct &&
        !create_drag_session.active &&
        !direct_drag_session.active &&
    ...
    const RectDragKind hovered_crop_kind =
        crop_drag_enabled && config.overlay_hovered && !hovered_box.has_value()
            ? rectangle_hover_kind_with_options(canvas_pointer,
                                                canvas_layout.viewport,
                                                active_crop_box,
                                                true,
                                                config.crop_edge_hit_half_width,
                                                config.crop_corner_hit_size)
            : RectDragKind::None;

    if (!config.color_sampled &&
        config.overlay_hovered &&
        config.overlay_left_clicked &&
    ...
    if (config.overlay_left_clicked) {
        if (hovered_handle.has_value()) {
        ...
        } else if (session.active_tool() == AnnotationToolKind::Direct &&
            hovered_box.has_value() &&
            hovered_box_drag_kind != RectDragKind::None &&
            hovered_crop_kind == RectDragKind::None) {
            select_object(session, document, hovered_box->index);
            session.set_direct_drag_index(hovered_box->index);
            start_preview_rect_drag(direct_drag_session,
                                    hovered_box_drag_kind,
                                    canvas_pointer.screen_x,
                                    canvas_pointer.screen_y,
                                    hovered_box->frame_box,
                                    1);
        } else if (session.active_tool() == AnnotationToolKind::Box &&
                   !create_drag_session.active) {
            start_preview_rect_drag(create_drag_session,
                                    RectDragKind::Create,
                                    canvas_pointer.screen_x,
                                    canvas_pointer.screen_y,
                                    AnnotationBox{
            ...
            start_preview_rect_drag(crop_drag_session,
                                    hovered_crop_kind,
                                    canvas_pointer.screen_x,
                                    canvas_pointer.screen_y,
                                    active_crop_box,
                                    kCropCommitMinExtent);
        }
    }

    if (config.overlay_left_clicked &&
        hovered_crop_kind == RectDragKind::None &&
        !session.handle_drag().has_value() &&
        !direct_drag_session.active &&
        !create_drag_session.active &&
        controller.handle_canvas_click(document,
                                       session,
                                       frame,
                                       categories,
                                       config.capture_x,
    ...
    if (crop_drag_session.active) {
        crop_drag_frame =
            update_preview_rect_drag(crop_drag_session,
                                     config.overlay_left_down,
                                     canvas_pointer.screen_x,
                                     canvas_pointer.screen_y,
                                     canvas_layout.viewport,
        ...
        if (crop_drag_frame.commit) {
            persist_crop_box_to_source(annotation_box_from_frame(frame, crop_drag_frame.box),
                                       &source);
            result.preview_invalidated = true;
        }
    }
    ...
    if (direct_drag_session.active && direct_drag_target_index.has_value()) {
        ...
        direct_drag_frame =
            update_preview_rect_drag(direct_drag_session,
                                     config.overlay_left_down,
                                     canvas_pointer.screen_x,
                                     canvas_pointer.screen_y,
                                     canvas_layout.viewport,
        ...
        if (direct_drag_frame.commit) {
            const AnnotationBox updated_capture =
                annotation_box_from_frame(frame, direct_drag_frame.box);
            if (controller.handle_box_commit(document,
                                             session,
                                             frame,
                                             categories,
                                             direct_drag_kind,
    ...
    if (create_drag_session.active) {
        create_drag_frame =
            update_preview_rect_drag(create_drag_session,
                                     config.overlay_left_down,
                                     canvas_pointer.screen_x,
                                     canvas_pointer.screen_y,
                                     canvas_layout.viewport,
        ...
        if (create_drag_frame.commit) {
            ...
            std::size_t category_index =
                selected_object != nullptr ? selected_object->category_index : 0U;
            if (!categories.items.empty()) {
                category_index = std::min(category_index, categories.items.size() - 1U);
            }
            const AnnotationBox capture_box =
                annotation_box_from_frame(frame, create_drag_frame.box);
            if (controller.handle_box_commit(document,
                                             session,
                                             frame,
                                             categories,
    ...
    } else if (handle_drag.has_value() &&
               handle_drag->active &&
               controller.handle_handle_drag(document,
                                             session,
                                             frame,
                                             categories,
                                             handle_drag->handle,
    ...
    update_brush_preview(session,
                         selected_object_supports_mask_editing(document, session),
                         config);
    ...
    if (paint_mode_active && selected_index.has_value()) {
        if (!config.overlay_left_down) {
        ...
        } else if (config.overlay_hovered &&
                   !config.color_sampled &&
            ...
            rasterize_line_samples(session.paint_stroke().last_capture_x,
                                   session.paint_stroke().last_capture_y,
                                   config.capture_x,
                                   config.capture_y,
                                   [&](const int sample_x, const int sample_y) {
                                       changed = controller.handle_brush_sample(document,
                                                                                session,
                                                                                frame,
                                                                                categories,
                                                                                sample_x,
                                                                                sample_y,
                                                                                config.brush_radius) || changed;
                                   });
    ...
    if (!config.color_sampled &&
        session.active_tool() == AnnotationToolKind::MaskFill &&
        selected_object_supports_mask_editing(document, session) &&
        selected_index.has_value() &&
        config.overlay_hovered &&
        config.overlay_left_clicked &&
        controller.handle_fill(document,
                               session,
                               frame,
                               categories,
                               config.capture_x,
    ...
    const bool handle_drag_active =
        current_handle_drag.has_value() && current_handle_drag->active;
    session.set_pointer_captured(session.create_drag_session().active ||
                                 session.crop_drag_session().active ||
                                 session.direct_drag_session().active ||
                                 handle_drag_active ||
                                 session.paint_stroke().active);
    ...
    const std::optional<std::size_t> selected_object_index =
        normalize_selected_object_index(document, session);
    overlay_projected_scene =
        ensure_projected_scene_up_to_date(selected_object_index);

    const std::optional<std::size_t> replaced_box_index =
        session.direct_drag_session().active ? session.direct_drag_index()
    ...
    if (replaced_box_index.has_value()) {
        drag_box = session.direct_drag_session().active
                   ...
                   : direct_drag_frame.commit
                       ? std::optional<AnnotationBox>{direct_drag_frame.box}
                       : std::nullopt;
    }
    const std::optional<AnnotationBox> create_box =
        session.create_drag_session().active
            ? std::optional<AnnotationBox>{session.create_drag_session().draft_box}
        : create_drag_frame.commit
            ? std::optional<AnnotationBox>{create_drag_frame.box}
            : std::nullopt;
    const std::optional<AnnotationBox> render_crop_box =
        session.crop_drag_session().active
            ? std::optional<AnnotationBox>{session.crop_drag_session().draft_box}
        : crop_drag_frame.commit && config.live_video && config.full_frame
            ? AnnotationRenderer::project_capture_box(frame,
                                                      resolved_video_crop_box(source))
                  .frame_box
            : workspace_view.crop_frame_box;
    const std::optional<AnnotationHandleId> dragged_handle =
        handle_drag_active
            ? std::optional<AnnotationHandleId>{current_handle_drag->handle}
            : std::nullopt;
    result.overlay_request = AnnotationRenderer::build_interaction_overlay_request(
        frame,
        config.cuda_device_index,
        *overlay_projected_scene,
        result.hovered_object_index,
        replaced_box_index,
        render_crop_box,
        std::max(1, config.crop_handle_radius),
        drag_box,
        create_box,
        session.hovered_handle(),
        dragged_handle);
