...
inline void reset_annotation_workflow_save_progress(
...
inline void mark_annotation_hold_save_overflow(
    AnnotationWorkflowRuntime &runtime) noexcept {
  runtime.save_controller.hold_save = false;
  runtime.save_controller.hold_save_blocked = true;
}

inline void clear_annotation_workflow_save_queue(
    AnnotationWorkflowRuntime &runtime) noexcept {
  clear_annotation_save_queue(&runtime.save_controller);
}

inline void
note_annotation_workflow_save_started(AnnotationWorkflowRuntime &runtime,
                                      const AnnotationFrame &frame) noexcept {
  note_annotation_save_started(&runtime.save_controller, frame);
}

inline void queue_annotation_workflow_save(
    AnnotationWorkflowRuntime &runtime, AnnotationFrame frame,
    std::vector<AnnotationObject> objects,
    std::shared_ptr<const AnnotationProjectedScene> projected_scene = {}) {
  queue_annotation_save(&runtime.save_controller, std::move(frame),
                        std::move(objects), std::move(projected_scene));
}

[[nodiscard]] inline std::optional<AnnotationQueuedSave>
take_annotation_workflow_queued_save(AnnotationWorkflowRuntime &runtime) {
  return take_queued_annotation_save(&runtime.save_controller);
}

[[nodiscard]] inline AnnotationSaveTabUiCommitResult
apply_annotation_workflow_save_tab_ui_actions(
    AnnotationWorkflowRuntime &runtime, const bool live_video,
    const AnnotationSaveTabUiActions &actions) noexcept {
  return apply_annotation_save_tab_ui_actions(&runtime.save_controller,
                                              live_video, actions);
}

[[nodiscard]] inline AnnotationSavePipelinePlan
plan_annotation_workflow_save_pipeline(const AnnotationWorkflowRuntime &runtime,
...
[[nodiscard]] inline AnnotationWorkflowSaveRequest
make_annotation_workflow_save_request(const AnnotationWorkflowRuntime *runtime,
                                      const SourceKind source_kind,
                                      const bool annotation_live_running,
                                      const bool save_running,
...
[[nodiscard]] inline AnnotationWorkflowSaveRequest
make_annotation_workflow_save_request(const AnnotationWorkflowRuntime *runtime,
                                      const AnnotateViewState &annotate,
                                      const bool annotation_live_running,
                                      const bool save_running,
                                      const AnnotationFrame *frame,
                                      const bool has_resolved_instances,
  ...
  return make_annotation_workflow_save_request(
      runtime, annotate.source.kind, annotation_live_running, save_running,
...
[[nodiscard]] inline AnnotationWorkflowSaveInputs
make_annotation_workflow_save_inputs(
  ...
  const AnnotationSaveControllerState *save_controller =
      request.runtime != nullptr
          ? std::addressof(request.runtime->save_controller)
          : nullptr;
...
[[nodiscard]] inline AnnotationWorkflowPollPlan plan_annotation_workflow_poll(
    const AnnotationWorkflowSaveRequest &request) noexcept {
  const AnnotationWorkflowSaveInputs save_inputs =
      make_annotation_workflow_save_inputs(request);
  const AnnotationSaveTabPlan save_tab_plan =
      make_annotation_save_tab_plan(save_inputs);
...
[[nodiscard]] inline std::optional<AnnotationBackgroundSaveRequest>
make_annotation_background_save_request(
    AnnotationBackgroundSaveRequestInputs inputs,
    std::string *error_message = nullptr);

[[nodiscard]] inline AnnotationBackgroundSaveOutcome
run_annotation_background_save(AnnotationBackgroundSaveRequest request);

[[nodiscard]] inline AnnotationWorkflowSaveNowDecision
decide_annotation_save_now(const AnnotationSaveTabPlan &plan) noexcept {
...
template <typename PrepareSaveSnapshotFn, typename LaunchSaveFn,
          typename AdvanceAfterSaveFn>
[[nodiscard]] inline bool dispatch_annotation_workflow_save_now(
  ...
  const AnnotationWorkflowSaveNowDecision decision =
      decide_annotation_save_now(plan);
  if (!decision.allowed) {
    return false;
  }

  std::optional<AnnotationSaveSnapshot> save_snapshot =
      std::forward<PrepareSaveSnapshotFn>(prepare_save_snapshot)(
          decision.refresh_live_frame);
  if (!save_snapshot.has_value()) {
    return false;
  }
  if (!std::forward<LaunchSaveFn>(launch_save)(std::move(*save_snapshot),
                                               decision.live_mode)) {
    return false;
  }
...
[[nodiscard]] inline bool launch_annotation_workflow_save(
    AnnotationWorkflowRuntime &runtime, AnnotateViewState &annotate,
    AnnotationCategories &categories, AnnotationSaveSnapshot save_snapshot,
    const bool live_mode, AnnotationWorkflowSaveLaunchState state,
    mmltk::runtime::BackgroundExecutor &background_executor,
    mmltk::runtime::UiCallbackQueue &ui_callbacks, LogErrorFn &&log_error) {
  ...
  std::optional<AnnotationBackgroundSaveRequest> save_request =
      make_annotation_background_save_request(
  ...
  mmltk::runtime::submit_background_task(
      ...
      [state, categories_ptr = std::addressof(categories), annotate_output_dir =
                                          annotate.output_dir](
          AnnotationBackgroundSaveOutcome outcome) mutable {
      ...
      [state, log_error = std::forward<LogErrorFn>(log_error)](
...
struct AnnotationWorkflowSaveUiRequest {
...
[[nodiscard]] inline AnnotationWorkflowSaveUiRequest
make_annotation_workflow_save_ui_request(
    AnnotateViewState *annotate, const AnnotationCategories *categories,
    const AnnotationWorkflowRuntime *runtime, const bool live_video,
    ImFont *compact_font) noexcept {
  return AnnotationWorkflowSaveUiRequest{
      annotate, categories, runtime, live_video, compact_font,
  };
}

[[nodiscard]] inline AnnotationSaveTabUiState make_annotation_save_tab_ui_state(
    const AnnotationWorkflowSaveUiRequest &inputs) noexcept {
  return AnnotationSaveTabUiState{
      inputs.annotate != nullptr ? std::addressof(inputs.annotate->output_dir)
                                 : nullptr,
      inputs.annotate != nullptr ? std::addressof(inputs.annotate->split)
                                 : nullptr,
      inputs.categories != nullptr ? inputs.categories->items.size() : 0U,
      inputs.live_video,
      inputs.runtime != nullptr ? inputs.runtime->save_controller.hold_save
                                : false,
      inputs.runtime != nullptr
          ? inputs.runtime->save_controller.hold_save_blocked
          : false,
      inputs.compact_font,
  };
}

struct AnnotationWorkflowSaveTabView {
  AnnotationSaveTabPlan plan;
  AnnotationSaveTabUiState ui_state;
};

[[nodiscard]] inline AnnotationWorkflowSaveTabView
make_annotation_workflow_save_tab_view(
    const AnnotationWorkflowSaveRequest &save_request,
    const AnnotationWorkflowSaveUiRequest &save_ui_request) noexcept {
  const AnnotationWorkflowSaveInputs save_inputs =
      make_annotation_workflow_save_inputs(save_request);
  return AnnotationWorkflowSaveTabView{
      make_annotation_save_tab_plan(save_inputs),
      make_annotation_save_tab_ui_state(save_ui_request),
  };
}

struct AnnotationWorkflowSaveSnapshotRequest {
  bool refresh_live_frame = false;
  const AnnotationFrame *frame = nullptr;
...
[[nodiscard]] inline AnnotationWorkflowSaveSnapshotRequest
make_annotation_workflow_save_snapshot_request(
    const bool refresh_live_frame, const AnnotationFrame *frame,
    const std::uint64_t document_generation,
    const std::vector<AnnotationObject> *document_objects,
    const bool has_resolved_instances, std::string *error_message = nullptr,
    const char *resolve_error_context =
        "annotate save resolve error") noexcept {
  return AnnotationWorkflowSaveSnapshotRequest{
      refresh_live_frame,     frame,
      document_generation,    document_objects,
      has_resolved_instances, error_message,
...
[[nodiscard]] inline std::optional<AnnotationSaveSnapshot>
make_annotation_workflow_current_save_snapshot(
    const AnnotationWorkflowCurrentSaveSnapshotRequest &request,
    EnsureLiveFramePixelsFn &&ensure_live_frame_pixels,
    LogResolveErrorFn &&log_resolve_error) {
  if (request.runtime == nullptr || request.document == nullptr ||
      request.session == nullptr || request.frame == nullptr ||
      request.categories == nullptr || request.resolved_objects == nullptr) {
    return std::nullopt;
  }
  ...
  return make_annotation_workflow_save_snapshot(
      make_annotation_workflow_save_snapshot_request(
          request.refresh_live_frame, current_frame,
          document_snapshot.generation, &document_snapshot.objects,
          !request.resolved_objects->empty(), request.error_message,
          request.resolve_error_context),
      std::forward<EnsureLiveFramePixelsFn>(ensure_live_frame_pixels),
      [&request, &log_resolve_error](
          const bool live_mode, const char *error_context,
          const bool clear_resolved_instances_on_error,
          std::shared_ptr<const AnnotationProjectedScene>
        ...
        AnnotationWorkflowResolvedObjectsResult resolved =
            resolve_annotation_workflow_objects(
                *request.runtime, *request.document, *request.session,
                *request.frame, *request.categories, live_mode);
        if (!resolved.ok) {
          if (clear_resolved_instances_on_error ||
              request.clear_resolved_objects_on_error) {
      ...
      [&request, document_generation = document_snapshot.generation]() {
        return resolve_annotation_projected_scene_for_document_generation(
            *request.runtime, *request.document, *request.session,
            *request.frame, document_generation);
      });
}

template <typename EnsureLiveFramePixelsFn, typename ResolveCurrentObjectsFn,
          typename CurrentProjectedSceneFn>
[[nodiscard]] inline std::optional<AnnotationSaveSnapshot>
make_annotation_workflow_save_snapshot(
    const AnnotationWorkflowSaveSnapshotRequest &request,
    EnsureLiveFramePixelsFn &&ensure_live_frame_pixels,
    ResolveCurrentObjectsFn &&resolve_current_annotation_objects,
    CurrentProjectedSceneFn &&current_projected_scene) {
  if (request.frame == nullptr || request.document_objects == nullptr ||
      !request.has_resolved_instances) {
    return std::nullopt;
  }
...
make_annotation_background_save_request(
  ...
  return std::make_optional<AnnotationBackgroundSaveRequest>(
      AnnotationBackgroundSaveRequest{
          AnnotationSaveConfig{
              std::move(inputs.output_dir),
              inputs.split.empty() ? std::string("train")
                                   : std::move(inputs.split),
          },
...
run_annotation_background_save(AnnotationBackgroundSaveRequest request) {
  ...
  outcome.save = save_annotation_scene(
      request.config, request.snapshot.frame, outcome.categories,
      request.snapshot.objects, request.live_mode,
      request.snapshot.projected_scene.get());
  outcome.summary =
