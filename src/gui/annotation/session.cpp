...
void AnnotationSession::set_hovered_object_index(std::optional<std::size_t> index) {
    if (hovered_object_index_ == index) {
        return;
    }
    hovered_object_index_ = index;
    mark_dirty();
    mark_overlay_dirty();
}

void AnnotationSession::touch_overlay() noexcept {
    mark_overlay_dirty();
}
...
bool AnnotationSession::has_active_transient_state() const {
    return hovered_handle_.has_value() || hovered_object_index_.has_value() || pointer_captured_ ||
           brush_preview_.visible || has_active_interaction_state() ||
