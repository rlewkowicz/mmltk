...
SeedSnapshotState load_seed_snapshot(const fs::path& settings_path) {
    ...
    apply_default_gui_state(state.train, state.validate, state.predict, state.annotate, state.export_state);
...
PredictSeedCliState parse_predict_seed(const std::vector<std::string>& args) {
    ...
    PredictSeedCliState state;
    mmltk::rfdetr::cli_shared::add_predict_command_options(&app, state);
...
ValidateSeedCliState parse_validate_seed(const std::vector<std::string>& args) {
    ...
    app.option_defaults()->always_capture_default();
    ValidateSeedCliState state;
    mmltk::rfdetr::cli_shared::add_validate_command_options(&app, state, true);
...
TrainSeedCliState parse_train_seed(const std::vector<std::string>& args, const std::string& fallback_preset_name) {
    ...
    app.option_defaults()->always_capture_default();
    TrainSeedCliState state;
    mmltk::rfdetr::cli_shared::add_train_command_options(&app, state, true);
...
void apply_predict_seed(SeedSnapshotState& snapshot, const PredictSeedCliState& state) {
    ...
    snapshot.predict.preset_name =
        choose_seed_preset(snapshot.predict.preset_name,
...
void apply_validate_seed(SeedSnapshotState& snapshot, const ValidateSeedCliState& state) {
    ...
    snapshot.validate.preset_name =
        choose_seed_preset(snapshot.validate.preset_name, {state.request.onnx_path, state.request.tensorrt_path});
...
void apply_train_seed(SeedSnapshotState& snapshot, const TrainSeedCliState& state) {
    ...
    if (!inferred_preset.empty()) {
        snapshot.train.preset_name = inferred_preset;
    } else if (snapshot.train.preset_name.empty()) {
        snapshot.train.preset_name = kDefaultGuiPresetName;
    }
...
void apply_build_engine_seed(SeedSnapshotState& snapshot, const mmltk::rfdetr::BuildEngineRequest& request) {
    ...
    snapshot.export_state.preset_name = choose_seed_preset(snapshot.export_state.preset_name, {request.onnx_path});
...
void apply_export_onnx_seed(SeedSnapshotState& snapshot, const mmltk::rfdetr::ExportOnnxRequest& request) {
    snapshot.current_view = View::Export;
    snapshot.export_state.preset_name = choose_seed_preset(snapshot.export_state.preset_name, {request.weights_path});
...
void apply_gui_cli_seed_file(const fs::path& settings_path, const std::vector<std::string>& cli_args) {
    ...
    if (subcommand == "train") {
        apply_train_seed(snapshot, parse_train_seed(command_args, snapshot.train.preset_name));
