...
bool preview_rect_box_meets_min_extent(const AnnotationBox& box, const int min_extent) {
    ...
    return (box.x2 - box.x1) >= required &&
           (box.y2 - box.y1) >= required;
}

void start_preview_rect_drag(PreviewRectDragSession& session,
                             const RectDragKind kind,
                             const float mouse_x,
                             const float mouse_y,
...
PreviewRectDragResult update_preview_rect_drag(PreviewRectDragSession& session,
    ...
    if (left_down) {
        ...
        result.changed = !(result.box.x1 == session.drag.start_box.x1 &&
                           result.box.y1 == session.drag.start_box.y1 &&
                           result.box.x2 == session.drag.start_box.x2 &&
                           result.box.y2 == session.drag.start_box.y2);
    ...
    result.changed = !(session.draft_box.x1 == session.drag.start_box.x1 &&
                       session.draft_box.y1 == session.drag.start_box.y1 &&
                       session.draft_box.x2 == session.drag.start_box.x2 &&
                       session.draft_box.y2 == session.drag.start_box.y2);
    if (session.drag.kind == RectDragKind::Create) {
        ...
        result.commit = result.changed && preview_rect_box_meets_min_extent(session.draft_box, session.commit_min_extent);
    }
    session = {};
    return result;
}
