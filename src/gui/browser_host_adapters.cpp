...
[[nodiscard]] nlohmann::json make_workflow_intent_settings_patch(const std::string_view workflow_key,
    ...
    for (auto it = options.begin(); it != options.end(); ++it) {
        if (!it.key().empty() && (it.key() == "current_view" || it.key() == "ui" || it.key() == "workflows")) {
            continue;
        }
...
AnnotationDocumentState annotation_document_state_from_snapshot(
    ...
    state.objects.reserve(snapshot.instances.size());
    for (std::size_t object_index = 0U; object_index < snapshot.instances.size(); ++object_index) {
        const mmltk::live::ManualOverlayInstance& instance = snapshot.instances[object_index];
        AnnotationDocumentState::Object object;
        object.object_index = static_cast<std::uint32_t>(object_index);
        object.x1 = instance.box.x1;
        object.y1 = instance.box.y1;
        object.x2 = instance.box.x2;
        object.y2 = instance.box.y2;
        const auto append_points = [&](const std::vector<mmltk::live::ManualOverlayPoint>& points) {
            object.points.reserve(object.points.size() + points.size());
            for (const mmltk::live::ManualOverlayPoint& point : points) {
                object.points.push_back(AnnotationDocumentState::Point{point.x, point.y});
            }
        };
        append_points(instance.polyline_points);
        const std::uint32_t skeleton_offset = static_cast<std::uint32_t>(object.points.size());
        append_points(instance.points);
        object.edges.reserve(instance.skeleton_edges.size());
        for (const mmltk::live::ManualOverlayEdge& edge : instance.skeleton_edges) {
            object.edges.push_back(AnnotationDocumentState::Edge{
                skeleton_offset + edge.source_index,
                skeleton_offset + edge.target_index,
            });
        }
...
std::optional<AnnotationDocumentState> annotation_document_state_from_editor(
    ...
    const mmltk::gui::AnnotationProjectedScene scene =
        mmltk::gui::AnnotationRenderer::build_projected_scene(*frame, *document, session->selected_object_index());
    AnnotationDocumentState state = annotation_document_state_from_snapshot(
        mmltk::gui::AnnotationRenderer::build_manual_overlay_snapshot(*frame, *document, *session, scene));
    const auto protocol_coordinate = [](const float coordinate) noexcept {
        const long rounded = std::lround(coordinate);
        return static_cast<std::int32_t>(std::clamp(rounded,
                                                    static_cast<long>(std::numeric_limits<std::int32_t>::min()),
                                                    static_cast<long>(std::numeric_limits<std::int32_t>::max())));
    };
    state.handles.reserve(scene.editable_handles.size());
    for (const mmltk::gui::AnnotationEditableHandle& handle : scene.editable_handles) {
        AnnotateHandleRole role = AnnotateHandleRole::Point;
        switch (handle.id.role) {
            case mmltk::gui::AnnotationHandleRole::Point:
                role = AnnotateHandleRole::Point;
                break;
            case mmltk::gui::AnnotationHandleRole::SplineKnot:
                role = AnnotateHandleRole::SplineKnot;
                break;
            case mmltk::gui::AnnotationHandleRole::SplineInHandle:
                role = AnnotateHandleRole::SplineInHandle;
                break;
            case mmltk::gui::AnnotationHandleRole::SplineOutHandle:
                role = AnnotateHandleRole::SplineOutHandle;
                break;
            case mmltk::gui::AnnotationHandleRole::SkeletonNode:
                role = AnnotateHandleRole::SkeletonNode;
                break;
            case mmltk::gui::AnnotationHandleRole::None:
                continue;
        }
        state.handles.push_back(AnnotationDocumentState::Handle{
            static_cast<std::uint32_t>(handle.id.object_index),
            static_cast<std::uint32_t>(handle.id.element_index),
            role,
            protocol_coordinate(handle.capture_point.x),
            protocol_coordinate(handle.capture_point.y),
        });
    }
    return state;
}
        ...
        {"preview",
         {
             ...
             {"last_failure_cuda_device", state.preview.last_failure_cuda_device.has_value()
        ...
        {"compositor",
         {
             ...
             {"source_acquire_waits", state.compositor.source_acquire_waits},
             {"source_leases_acquired", state.compositor.source_leases_acquired},
             {"source_leases_released", state.compositor.source_leases_released},
             {"source_stale_releases", state.compositor.source_stale_releases},
             {"source_skipped_stale_frames", state.compositor.source_skipped_stale_frames},
             {"source_slot_pressure", state.compositor.source_slot_pressure},
             {"source_release_latency_ns", state.compositor.source_release_latency_ns},
...
nlohmann::json make_predict_start_settings_patch(const PredictStartIntent& intent) {
    return make_workflow_intent_settings_patch("predict", "predict", intent.options);
}

nlohmann::json make_annotate_save_settings_patch(const AnnotateSaveIntent& intent) {
    return make_workflow_intent_settings_patch("annotate", "annotate", intent.options);
}
    ...
    switch (binding) {
        ...
        case BrowserFileDialogBinding::TrainTestCompiledPath:
            return access.train != nullptr ? BrowserFileDialogTarget{&access.train->test_compiled_path}
                                           : BrowserFileDialogTarget{};
        case BrowserFileDialogBinding::TrainDatasetSourceDir:
            return access.train != nullptr ? BrowserFileDialogTarget{&access.train->dataset_source_dir}
                                           : BrowserFileDialogTarget{};
        case BrowserFileDialogBinding::TrainCompiledDatasetDir:
            return access.train != nullptr ? BrowserFileDialogTarget{&access.train->compiled_dataset_dir}
                                           : BrowserFileDialogTarget{};
        ...
        case BrowserFileDialogBinding::ValidateSourceRoot:
            return access.validate != nullptr ? BrowserFileDialogTarget{&access.validate->source_dir}
                                              : BrowserFileDialogTarget{};
        case BrowserFileDialogBinding::ValidateWeights:
            return access.validate != nullptr ? BrowserFileDialogTarget{&access.validate->weights_path}
                                              : BrowserFileDialogTarget{};
...
StateSnapshot make_state_snapshot(const GuiStateSnapshotInputs& inputs) {
    ...
    if (inputs.gui != nullptr) {
        ...
        snapshot.workflow_state = nlohmann::json::object();
...
bool apply_annotate_workspace_box_drag(const AnnotateWorkspaceBoxDragIntent& intent,
    ...
    const auto ensure_no_conflicting_interaction = [&]() {
        ...
        if (context.session->crop_drag_session().active) {
            throw std::runtime_error("browser annotate.workspace.box_drag requires no active crop drag");
        }
    };
    ...
    if (intent.phase == AnnotateWorkspacePhase::Begin) {
        ...
        if (!intent.object_index.has_value()) {
            if (context.source == nullptr || context.source->crop_width <= 0 || context.source->crop_height <= 0) {
                throw std::runtime_error(
                    "browser annotate.workspace.box_drag crop edit requires an active source crop");
            }
            const mmltk::gui::AnnotationBox crop_box = mmltk::gui::normalize_annotation_box(
                mmltk::gui::AnnotationBox{
                    context.source->crop_x,
                    context.source->crop_y,
                    context.source->crop_x + context.source->crop_width,
                    context.source->crop_y + context.source->crop_height,
                },
                mmltk::gui::annotation_frame_capture_width(*context.frame),
                mmltk::gui::annotation_frame_capture_height(*context.frame));
            mmltk::gui::start_preview_rect_drag(
                context.session->crop_drag_session(), annotation_box_drag_kind_from_browser_kind(*intent.drag_kind),
                static_cast<float>(intent.capture_x), static_cast<float>(intent.capture_y), crop_box, 32);
            refresh_annotation_pointer_capture(*context.session);
            return false;
        }
    ...
    if (context.session->crop_drag_session().active) {
        const std::optional<AnnotateBoxDragKind> active_drag_kind =
            browser_box_drag_kind_from_session(context.session->crop_drag_session());
        if (!active_drag_kind.has_value() || *active_drag_kind == AnnotateBoxDragKind::Create ||
            (intent.drag_kind.has_value() && *intent.drag_kind != *active_drag_kind) ||
            intent.object_index.has_value()) {
            throw std::runtime_error("browser annotate.workspace.box_drag payload does not match the active crop drag");
        }
        if (context.source == nullptr) {
            throw std::runtime_error("browser annotate.workspace.box_drag crop source is unavailable");
        }
        const mmltk::gui::PreviewRectDragResult drag = update_drag_session(context.session->crop_drag_session());
        if (intent.phase == AnnotateWorkspacePhase::End && drag.commit) {
            mmltk::gui::persist_crop_box_to_source(drag.box, context.source);
        }
        refresh_annotation_pointer_capture(*context.session);
        return drag.changed;
    }
...
bool apply_annotate_workspace_pointer(const AnnotateWorkspacePointerIntent& intent,
    ...
    const bool pointer_inside = intent.capture_x >= 0 && intent.capture_y >= 0;

    if (intent.phase == AnnotateWorkspacePhase::Cancel || !pointer_inside) {
        context.session->set_hovered_object_index(std::nullopt);
        context.session->set_hovered_handle(std::nullopt);
    } else {
        context.session->set_hovered_object_index(
            intent.object_index.has_value() ? std::optional<std::size_t>{static_cast<std::size_t>(*intent.object_index)}
                                            : std::nullopt);
        context.session->set_hovered_handle(
            intent.handle.has_value()
                ? std::optional<mmltk::gui::AnnotationHandleId>{annotation_handle_id_from_browser_ref(*intent.handle)}
                : std::nullopt);
    }

    if (intent.phase == AnnotateWorkspacePhase::Hover) {
        const mmltk::gui::AnnotationToolKind tool = annotation_tool_kind_from_name_impl(intent.tool);
        if (pointer_inside && mmltk::gui::annotation_tool_kind_uses_brush(tool) &&
            selected_mask_editable_object(*context.document, *context.session) != nullptr) {
            set_annotation_brush_preview(*context.session, intent.capture_x, intent.capture_y, intent.brush_radius,
                                         tool == mmltk::gui::AnnotationToolKind::MaskErase);
        } else {
            context.session->set_brush_preview(mmltk::gui::AnnotationBrushPreview{});
        }
        return false;
    ...
    if (intent.tool == "select") {
        ...
        if (intent.phase == AnnotateWorkspacePhase::End) {
            mmltk::gui::select_object(*context.session, *context.document,
                                      intent.object_index.has_value()
                                          ? std::optional<std::size_t>{static_cast<std::size_t>(*intent.object_index)}
                                          : std::nullopt);
        }
    ...
    if (tool == mmltk::gui::AnnotationToolKind::MaskFill) {
        return apply_annotate_workspace_fill(AnnotateWorkspaceFillIntent{intent.capture_x, intent.capture_y}, context);
    }
    if (tool == mmltk::gui::AnnotationToolKind::Point || tool == mmltk::gui::AnnotationToolKind::Spline ||
        ...
        return apply_annotate_workspace_click(AnnotateWorkspaceClickIntent{intent.capture_x, intent.capture_y, false},
                                              context);
    }
    return false;
}
...
struct BrowserHostIntentApplyHandlers {
    ...
    void operator()(const RoutedIntent&, const UiLogIntent&) const {}
...
void apply_routed_intent(const RoutedIntent& intent, const BrowserHostIntentContext& context) {
    const BrowserAnnotateWorkspaceContext annotate_workspace_context{
        context.annotation_document,
        context.annotation_session,
        context.annotation_controller,
        context.annotation_frame,
        context.annotation_categories,
        context.annotate != nullptr ? &context.annotate->source : nullptr,
        context.ensure_annotation_frame_pixels,
    };
