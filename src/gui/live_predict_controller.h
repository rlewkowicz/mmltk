...
class LivePredictController {
   public:
    ...
    void launch(mmltk::runtime::BackgroundExecutor& background_executor, mmltk::runtime::UiCallbackQueue& ui_callbacks,
                const PredictViewState& state, const std::string& preset_name, std::function<void(int)> on_started,
    ...
    [[nodiscard]] std::shared_ptr<mmltk::live::LiveSessionController> controller_handle() const noexcept;
   ...
   private:
    struct StartOutcome {
        std::shared_ptr<mmltk::live::LiveSessionController> controller;
        mmltk::live::LiveSessionStatus status;
        int device_id = 0;
    };

    std::shared_ptr<mmltk::live::LiveSessionController> controller_;
