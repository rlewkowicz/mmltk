...
[[nodiscard]] float quantize_hundredths(const float value) noexcept {
    return std::round(value * 100.0F) / 100.0F;
}

[[nodiscard]] double json_hundredths(const float value) noexcept {
...
void normalize_model_artifacts(ModelArtifactSelectionState& state, const ModelArtifactsShape& shape) {
    const mmltk::rfdetr::PresetCatalogEntry* preset = mmltk::rfdetr::find_preset_catalog_entry(state.preset_name);
    const bool unsupported_input = (state.input == ModelInputMode::Weights && !shape.weights) ||
                                   (state.input == ModelInputMode::Onnx && !shape.onnx) ||
                                   (state.input == ModelInputMode::TensorRt && !shape.tensorrt);
    if (preset == nullptr || unsupported_input) {
        state = ModelArtifactSelectionState{};
        return;
    }
    if (state.resolution <= 0) {
        state.resolution = static_cast<int>(preset->resolution);
    }
    if (state.input == ModelInputMode::None) {
    ...
    if (state.source == ModelSelectionSource::Canonical) {
        state.input = ModelInputMode::Weights;
...
nlohmann::json snapshot_dataset_paths(const DatasetPathState& state) {
    return nlohmann::json{
        ...
        {"compiled_directory", state.compiled_directory},
        {"use_compiled_directory_defaults", state.use_compiled_directory_defaults},
        ...
        {"compile_dimensions", state.compile_dimensions},
    };
}
...
void apply_dataset_paths_json(const nlohmann::json& json, DatasetPathState& state) {
    ...
    get_optional(json, "compiled_directory", state.compiled_directory);
    get_optional(json, "use_compiled_directory_defaults", state.use_compiled_directory_defaults);
    ...
    get_optional(json, "overwrite", state.overwrite);
    get_optional(json, "compile_dimensions", state.compile_dimensions);
}
...
nlohmann::json snapshot_train_workflow_state(const TrainViewState& state, const TrainPaneState& train_pane) {
    return nlohmann::json{
        ...
        {"validation_profile", state.validation_profile},
        {"amp", state.amp},
...
nlohmann::json snapshot_augmentation_group(const mmltk::rfdetr::AugmentationGroupConfig& group) {
    return nlohmann::json{
        {"probability", json_hundredths(group.probability)},
        {"min_strength", json_hundredths(group.min_strength)},
        {"max_strength", json_hundredths(group.max_strength)},
    };
}

nlohmann::json snapshot_gpu_augmentation(const mmltk::rfdetr::GpuAugmentationConfig& config) {
    return nlohmann::json{
        {"enabled", config.enabled},
        {"geometry", snapshot_augmentation_group(config.geometry)},
        {"resize", snapshot_augmentation_group(config.resize)},
        {"color", snapshot_augmentation_group(config.color)},
        {"noise", snapshot_augmentation_group(config.noise)},
        {"blur", snapshot_augmentation_group(config.blur)},
        {"occlusion", snapshot_augmentation_group(config.occlusion)},
        {"copy_paste_probability", json_hundredths(config.copy_paste_probability)},
    };
}

void apply_augmentation_group_json(const nlohmann::json& json, mmltk::rfdetr::AugmentationGroupConfig& group) {
    get_optional(json, "probability", group.probability);
    get_optional(json, "min_strength", group.min_strength);
    get_optional(json, "max_strength", group.max_strength);
    group.probability = quantize_hundredths(group.probability);
    group.min_strength = quantize_hundredths(group.min_strength);
    group.max_strength = quantize_hundredths(group.max_strength);
}

void apply_gpu_augmentation_json(const nlohmann::json& json, mmltk::rfdetr::GpuAugmentationConfig& config) {
    get_optional(json, "enabled", config.enabled);
    const auto apply_group = [&json](const char* key, mmltk::rfdetr::AugmentationGroupConfig& group) {
        if (const auto value = json.find(key); value != json.end() && value->is_object()) {
            apply_augmentation_group_json(*value, group);
        }
    };
    apply_group("geometry", config.geometry);
    apply_group("resize", config.resize);
    apply_group("color", config.color);
    apply_group("noise", config.noise);
    apply_group("blur", config.blur);
    apply_group("occlusion", config.occlusion);
    get_optional(json, "copy_paste_probability", config.copy_paste_probability);
    config.copy_paste_probability = quantize_hundredths(config.copy_paste_probability);
}
...
nlohmann::json snapshot_model_artifacts(const ModelArtifactSelectionState& state, const ModelArtifactsShape& shape) {
    nlohmann::json json = {
        ...
        {"input", static_cast<int>(state.input)},
    };
    if (shape.weights) {
...
void add_model_selection_json(nlohmann::json& json, const WorkflowModelSelectionState& state) {
    json.update(nlohmann::json{
        {"preset_name", state.preset_name},
        {"model_resolution", state.model_resolution},
        {"model_source", static_cast<int>(state.model_source)},
        {"model_input", static_cast<int>(state.model_input)},
    });
}

void apply_model_selection_json(const nlohmann::json& json, WorkflowModelSelectionState& state) {
    get_optional(json, "preset_name", state.preset_name);
    get_optional(json, "model_resolution", state.model_resolution);
    int source = static_cast<int>(state.model_source);
    get_optional(json, "model_source", source);
    state.model_source = model_selection_source_from_index(source, state.model_source);
    int input = static_cast<int>(state.model_input);
    get_optional(json, "model_input", input);
    state.model_input = model_input_from_index(input, state.model_input);
}
...
void apply_model_artifacts_json(const nlohmann::json& json, ModelArtifactSelectionState& state,
    ...
    get_optional(json, "preset_name", state.preset_name);
    get_optional(json, "resolution", state.resolution);
    int source = static_cast<int>(state.source);
    get_optional(json, "source", source);
    state.source = model_selection_source_from_index(source, state.source);
    int input = static_cast<int>(state.input);
    get_optional(json, "input", input);
    state.input = model_input_from_index(input, state.input);
    if (shape.weights) {
...
nlohmann::json normalize_train_workflow_v2_to_v3(const nlohmann::json& legacy) {
    ...
    normalized[kDatasetPathsKey] = snapshot_dataset_paths(DatasetPathState{
        ...
        .source_dir = {},
        .compiled_directory = {},
        .use_compiled_directory_defaults = true,
...
nlohmann::json normalize_gui_settings_document_impl(const nlohmann::json& j) {
    ...
    if (schema_version == 3U) {
        ...
        std::string preset_name = get_value_or<std::string>(j, "selected_preset", std::string(kDefaultModelPresetName));
        ...
        if (preset == nullptr) {
            ...
            preset = mmltk::rfdetr::find_preset_catalog_entry(preset_name);
        }
        auto& workflows = normalized["workflows"];
        if (workflows.is_object()) {
            for (const char* workflow : {"train", "validate", "predict", "annotate", "export"}) {
                auto workflow_it = workflows.find(workflow);
                if (workflow_it == workflows.end() || !workflow_it->is_object()) {
                    continue;
                }
                nlohmann::json& artifacts = (*workflow_it)[kModelArtifactsKey];
                if (!artifacts.is_object()) {
                    artifacts = nlohmann::json::object();
                }
                artifacts["preset_name"] = preset_name;
                int resolution = preset != nullptr ? static_cast<int>(preset->resolution) : kDefaultModelResolution;
                if (workflow_it->contains(kValidationKey) && (*workflow_it)[kValidationKey].is_object()) {
                    resolution = get_value_or<int>((*workflow_it)[kValidationKey], "resolution", resolution);
                }
                artifacts["resolution"] = resolution;
                const bool has_weights = !get_value_or<std::string>(artifacts, "weights_path", {}).empty();
                const bool has_onnx = !get_value_or<std::string>(artifacts, "onnx_path", {}).empty();
                const bool has_tensorrt = !get_value_or<std::string>(artifacts, "tensorrt_path", {}).empty();
                const std::filesystem::path weights_path = get_value_or<std::string>(artifacts, "weights_path", {});
                const bool canonical_weights = has_weights && preset != nullptr &&
                                               weights_path.filename().string() == preset->canonical_weight_filename;
                artifacts["source"] = static_cast<int>(has_onnx || has_tensorrt || (has_weights && !canonical_weights)
                                                           ? ModelSelectionSource::Custom
                                                           : ModelSelectionSource::Canonical);
                artifacts["input"] = static_cast<int>(has_weights    ? ModelInputMode::Weights
                                                      : has_onnx     ? ModelInputMode::Onnx
                                                      : has_tensorrt ? ModelInputMode::TensorRt
                                                                     : ModelInputMode::Weights);
            }
        }
        normalized.erase("selected_preset");
        return normalized;
    }
    if (schema_version == 2U) {
        return normalize_gui_settings_document_impl(normalize_gui_settings_v2_to_v3(j));
    }
...
void to_json(nlohmann::json& j, const ModelArtifactSelectionState& s) {
    j = nlohmann::json{
        {"weights_path", s.weights_path},     {"onnx_path", s.onnx_path},   {"tensorrt_path", s.tensorrt_path},
        {"preset_name", s.preset_name},       {"resolution", s.resolution}, {"source", static_cast<int>(s.source)},
        {"input", static_cast<int>(s.input)},
    };
}
...
void from_json(const nlohmann::json& j, ModelArtifactSelectionState& s) {
    ...
    get_optional(j, "preset_name", s.preset_name);
    get_optional(j, "resolution", s.resolution);
    int source = static_cast<int>(s.source);
    get_optional(j, "source", source);
    s.source = model_selection_source_from_index(source, s.source);
    int input = static_cast<int>(s.input);
    get_optional(j, "input", input);
    s.input = model_input_from_index(input, s.input);
    normalize_model_artifacts(s, ModelArtifactsShape{});
}
...
void to_json(nlohmann::json& j, const DatasetPathState& s) {
    j = nlohmann::json{
        ...
        {"compile_dimensions", s.compile_dimensions},
    };
}
...
void from_json(const nlohmann::json& j, DatasetPathState& s) {
    ...
    get_optional(j, "overwrite", s.overwrite);
    get_optional(j, "compile_dimensions", s.compile_dimensions);
}
...
void to_json(nlohmann::json& j, const TrainViewState& s) {
    ...
    j["gpu_augmentation"] = snapshot_gpu_augmentation(s.gpu_augmentation);
...
void from_json(const nlohmann::json& j, TrainViewState& s) {
    ...
    get_optional(j, "validation_loss", s.validation_loss);
    get_optional(j, "validation_profile", s.validation_profile);
    ...
    if (const auto augmentation = j.find("gpu_augmentation"); augmentation != j.end() && augmentation->is_object()) {
        apply_gpu_augmentation_json(*augmentation, s.gpu_augmentation);
    }
...
void to_json(nlohmann::json& j, const UiSettingsState& s) {
    j = nlohmann::json{
        ...
        {"text_input_font_size", s.text_input_font_size},
        ...
        {"workspace_aspect_ratio", s.workspace_aspect_ratio},
        {"annotation_brush_radius", s.annotation_brush_radius},
        {"mask_cleanup_radius", s.mask_cleanup_radius},
    };
}
...
void from_json(const nlohmann::json& j, UiSettingsState& s) {
    ...
    get_optional(j, "accent_color", s.accent_color);
    ...
    get_optional(j, "text_input_font_size", s.text_input_font_size);
    ...
    s.density = static_cast<UiDensity>(std::clamp(density, 0, 2));
    get_optional(j, "workspace_aspect_ratio", s.workspace_aspect_ratio);
    get_optional(j, "annotation_brush_radius", s.annotation_brush_radius);
    get_optional(j, "mask_cleanup_radius", s.mask_cleanup_radius);
    s.ui_scale = std::isfinite(s.ui_scale) ? std::clamp(s.ui_scale, 0.85F, 1.75F) : 1.0F;
    s.font_size = std::isfinite(s.font_size) ? std::clamp(s.font_size, 10.0F, 32.0F) : 14.0F;
    s.secondary_font_size =
        std::isfinite(s.secondary_font_size) ? std::clamp(s.secondary_font_size, 9.0F, 28.0F) : 12.0F;
    s.mono_font_size = std::isfinite(s.mono_font_size) ? std::clamp(s.mono_font_size, 9.0F, 28.0F) : 12.0F;
    s.text_input_font_size =
        std::isfinite(s.text_input_font_size) ? std::clamp(s.text_input_font_size, 9.0F, 31.0F) : 13.0F;
    s.workspace_aspect_ratio = std::clamp(s.workspace_aspect_ratio, 0, 4);
    s.annotation_brush_radius = std::clamp(s.annotation_brush_radius, 1, 128);
    s.mask_cleanup_radius = std::clamp(s.mask_cleanup_radius, 1, 32);
}
...
nlohmann::json snapshot_workflows(const WorkflowSettingsSnapshot& workflows) {
    ...
    if (workflows.train != nullptr) {
        ...
        train_json[kAugmentationKey] = snapshot_gpu_augmentation(s.gpu_augmentation);
    ...
    if (workflows.train != nullptr) {
        ...
        if (train_it != workflows_it->end() && train_it->is_object()) {
            ...
            if (const auto training = train_it->find(kTrainingKey);
                ...
                get_optional(*training, "validation_loss", s.validation_loss);
                get_optional(*training, "validation_profile", s.validation_profile);
            ...
            if (const auto augmentation = train_it->find(kAugmentationKey);
                augmentation != train_it->end() && augmentation->is_object()) {
                apply_gpu_augmentation_json(*augmentation, s.gpu_augmentation);
            }
            apply_train_pane_state(s, train_pane);
        }
