...
ModelInputMode model_input_mode_from_request(const Request& request) {
...
template <typename Request>
void apply_explicit_model_selection(WorkflowModelSelectionState& state, const Request& request) {
    state.model_input = model_input_mode_from_request(request);
    state.model_source = ModelSelectionSource::Custom;
    if (!request.preset_name.empty()) {
        state.preset_name = request.preset_name;
    } else {
        const std::filesystem::path& input_path = !request.weights_path.empty() ? request.weights_path
                                                  : !request.onnx_path.empty()  ? request.onnx_path
                                                                                : request.tensorrt_path;
        if (const mmltk::rfdetr::ModelPresetConfig* inferred =
                mmltk::rfdetr::infer_model_preset_from_path(input_path)) {
            state.preset_name = inferred->preset_name;
        }
    }
    if (request.resolution > 0) {
        const std::int64_t resolution = static_cast<std::int64_t>(request.resolution);
        if (resolution > std::numeric_limits<int>::max()) {
            throw std::runtime_error("RF-DETR model resolution exceeds the supported integer range");
        }
        state.model_resolution = static_cast<int>(resolution);
    } else if (const mmltk::rfdetr::ModelPresetConfig* preset = mmltk::rfdetr::find_model_preset(state.preset_name)) {
        state.model_resolution = preset->resolution;
    }
}
...
void apply_predict_runtime_selection(ModelInputMode model_input, const ModelArtifactSelectionState& artifacts,
    ...
    options.preset_name = artifacts.preset_name;
    options.resolution = require_positive_int(artifacts.resolution, "model resolution");
...
mmltk::rfdetr::ExportOnnxRequest build_export_onnx_request(const ExportViewState& state,
    ...
    return build_export_onnx_request(
        state.weights_path, output_path_override.empty() ? state.onnx_path : output_path_override.string(),
        state.device_id, state.opset_version, state.simplify, state.preset_name, state.model_resolution);
}
...
mmltk::rfdetr::BuildEngineRequest build_build_engine_request(const mmltk::rfdetr::ExportOnnxRequest& export_request,
    ...
    request.preset_name = export_request.preset_name;
    request.resolution = export_request.resolution;
...
mmltk::rfdetr::ExportOnnxRequest build_export_onnx_request(const std::string& weights_path,
                                                           ...
                                                           int opset_version, bool simplify, std::string preset_name,
...
void apply_build_engine_request(ExportViewState& state, const mmltk::rfdetr::BuildEngineRequest& request) {
    ...
    apply_explicit_model_selection(state, request);
...
mmltk::rfdetr::ValidateRequest build_validate_request(const ValidateViewState& state) {
    ...
    request.resolution =
        static_cast<std::uint32_t>(require_positive_size(state.model_resolution, "validate resolution"));
...
std::string summarize_validation_result(const mmltk::rfdetr::ValidationRunResult& result) {
    ...
    for (const std::string& backend_name : result.eval_order) {
        ...
        stream << " " << backend_name << "_bbox_ap=" << found->second.summary.bbox.ap << " " << backend_name
               << "_mask_ap=";
        const auto& mask = found->second.summary.mask;
        if (mask.has_value()) {
            stream << mask->ap;
        } else {
            stream << "null";
