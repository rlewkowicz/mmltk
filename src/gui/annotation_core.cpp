...
void write_annotation_png(const std::filesystem::path& path, const int width, const int height, const int channels,
                          const void* pixels, const int stride_bytes) {
...
AnnotationHsv sample_annotation_hsv(const AnnotationFrame& frame, const int x, const int y) {
    ...
    if (frame.pixels_bgr.size() < static_cast<std::size_t>(frame.width) * static_cast<std::size_t>(frame.height) * 3U) {
    ...
    return annotation_bgr_to_hsv(frame.pixels_bgr[byte_offset + 0], frame.pixels_bgr[byte_offset + 1],
...
bool annotation_range_active(const AnnotationColorRange& range) {
    return range.tolerance.hue_minus_pct > 0.0f || range.tolerance.hue_plus_pct > 0.0f ||
           range.tolerance.saturation_minus_pct > 0.0f || range.tolerance.saturation_plus_pct > 0.0f ||
...
AnnotationBox annotation_box_to_frame(const AnnotationFrame& frame, const AnnotationBox& capture_box) {
    ...
    const AnnotationBox normalized = normalize_annotation_box(capture_box, annotation_frame_capture_width(frame),
...
AnnotationFrame extract_annotation_frame_region(const AnnotationFrame& frame, const AnnotationBox& capture_box) {
    const AnnotationBox requested = normalize_annotation_box(capture_box, annotation_frame_capture_width(frame),
                                                             annotation_frame_capture_height(frame));
    const AnnotationBox view_box = annotation_frame_view_box(frame);
    const AnnotationBox overlap = annotation_intersect_boxes(requested, view_box);
    if (!annotation_box_has_area(requested) || overlap.x1 != requested.x1 || overlap.y1 != requested.y1 ||
        overlap.x2 != requested.x2 || overlap.y2 != requested.y2) {
    ...
    extracted.pixels_bgr.assign(
        static_cast<std::size_t>(extracted.width) * static_cast<std::size_t>(extracted.height) * 3U, 0U);

    const AnnotationBox local = annotation_box_to_frame(frame, requested);
    for (std::uint32_t row = 0; row < extracted.height; ++row) {
        const std::size_t src_offset = (static_cast<std::size_t>(local.y1) + static_cast<std::size_t>(row)) *
...
std::optional<AnnotationBox> annotation_bbox_from_mask(const std::vector<std::uint8_t>& mask, const std::uint32_t width,
    ...
    if (mask.size() != static_cast<std::size_t>(width) * static_cast<std::size_t>(height) || width == 0U ||
        height == 0U) {
        return std::nullopt;
    }
...
std::vector<std::uint8_t> decode_annotation_prediction_mask(const mmltk::rfdetr::EncodedMask& mask,
    ...
    for (const auto& run : mask.runs) {
        ...
        std::fill(dense.begin() + static_cast<std::ptrdiff_t>(start), dense.begin() + static_cast<std::ptrdiff_t>(end),
                  1U);
    }
    return dense;
}
...
std::vector<std::uint8_t> decode_annotation_mask_rle(const std::string_view encoded_mask, const std::uint32_t width,
    ...
    while (cursor < encoded_mask.size()) {
        ...
        std::fill(dense.begin() + static_cast<std::ptrdiff_t>(bounded_start),
                  dense.begin() + static_cast<std::ptrdiff_t>(bounded_end), 1U);
    }
