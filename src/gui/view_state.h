...
inline constexpr std::string_view kDefaultModelPresetName = "rf-detr-nano";
inline constexpr int kDefaultModelResolution = 384;

struct WorkflowModelSelectionState {
    std::string preset_name{std::string(kDefaultModelPresetName)};
    int model_resolution = kDefaultModelResolution;
...
struct UiSettingsState {
    ...
    float ui_scale = 1.0f;
    std::string accent_color = "#2ac3de";
    float font_size = 14.0f;
    float secondary_font_size = 12.0f;
    float mono_font_size = 12.0f;
    float text_input_font_size = 13.0f;
    ...
    int workspace_aspect_ratio = 0;
    int annotation_brush_radius = 12;
    int mask_cleanup_radius = 2;
...
struct TrainViewState : WorkflowModelSelectionState,
                        mmltk::rfdetr::TrainOptimizerTuningConfig,
                        TrainViewRuntimeConfig,
                        TrainExecutionPaneState {
    TrainViewState() {
        batch_size = 2;
        epochs = 12;
        workers = 8;
    ...
    std::string output_dir = "./gui-train-output";
    std::string dataset_source_dir = "./dataset";
    std::string compiled_dataset_dir = "./compiled";
    bool use_compiled_directory_defaults = true;
...
struct ValidateViewState : WorkflowModelSelectionState,
                           mmltk::rfdetr::ValidationSharedConfig,
...
struct PredictViewState : WorkflowModelSelectionState {
...
inline DatasetPathState dataset_paths(const TrainViewState& s) {
    return DatasetPathState{
        .compiled_path = {},
        .source_dir = s.dataset_source_dir,
        .compiled_directory = s.compiled_dataset_dir,
        .use_compiled_directory_defaults = s.use_compiled_directory_defaults,
        .train_compiled_path = s.train_compiled_path,
        .val_compiled_path = s.val_compiled_path,
        .test_compiled_path = s.test_compiled_path,
        .overwrite = s.overwrite_compiled_dataset,
...
inline void apply_dataset_paths(TrainViewState& s, const DatasetPathState& dataset_state) {
    s.dataset_source_dir = dataset_state.source_dir;
    s.compiled_dataset_dir = dataset_state.compiled_directory;
    s.use_compiled_directory_defaults = dataset_state.use_compiled_directory_defaults;
    ...
    s.overwrite_compiled_dataset = dataset_state.overwrite;
    s.compile_dimensions = dataset_state.compile_dimensions;
}

inline ModelArtifactSelectionState model_artifacts(const TrainViewState& s) {
    return ModelArtifactSelectionState{s.weights_path,     std::string{},  std::string{}, s.preset_name,
                                       s.model_resolution, s.model_source, s.model_input};
}
...
inline mmltk::rfdetr::TrainOptions train_options(const TrainViewState& s, const std::vector<int>& device_ids = {}) {
    ...
    options.preset_name = s.preset_name;
    ...
    options.validation_loss = s.validation_loss;
    options.validation_profile = s.validation_profile;
    options.amp = s.amp;
    ...
    options.gpu_augmentation = s.gpu_augmentation;
    ...
    options.resolution = s.model_resolution;
    return options;
}
...
inline void apply_train_options(TrainViewState& s, const mmltk::rfdetr::TrainOptions& options) {
    ...
    artifact_state.preset_name = options.preset_name.empty() ? artifact_state.preset_name : options.preset_name;
    artifact_state.weights_path = options.weights_path.string();
    artifact_state.resolution = options.resolution > 0 ? options.resolution : artifact_state.resolution;
    artifact_state.source = ModelSelectionSource::Custom;
    artifact_state.input = ModelInputMode::Weights;
    ...
    s.validation_loss = options.validation_loss;
    s.validation_profile = options.validation_profile;
    s.amp = options.amp;
    s.freeze_encoder = options.freeze_encoder;
    s.gpu_augmentation = options.gpu_augmentation;
...
inline ModelArtifactSelectionState model_artifacts(const ValidateViewState& s) {
    return ModelArtifactSelectionState{s.weights_path,     s.onnx_path,    s.tensorrt_path, s.preset_name,
                                       s.model_resolution, s.model_source, s.model_input};
}
...
inline ModelArtifactSelectionState model_artifacts(const ExportViewState& s) {
    return ModelArtifactSelectionState{s.weights_path,     s.onnx_path,    std::string{}, s.preset_name,
                                       s.model_resolution, s.model_source, s.model_input};
}
