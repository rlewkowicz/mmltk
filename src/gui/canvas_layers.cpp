...
AnnotationBox apply_rectangle_drag(const RectDragState& state,
...
RectDragKind rectangle_hover_kind_with_options(const CanvasPointerState& pointer,
    ...
    const bool on_edge = !near_top_left && !near_top_right &&
                         ...
                         ((near_left && in_y_range) || (near_right && in_y_range) ||
                          (near_top && in_x_range) || (near_bottom && in_x_range));
    if (near_top_left) {
...
RectLayerFrameResult update_rect_layers(RectLayerState& state,
    ...
    if (state.active_layer_id != 0) {
        ...
        result.box = apply_rectangle_drag(state.drag,
                                          pointer,
                                          viewport,
                                          static_cast<int>(active_layer->bounds_width),
                                          static_cast<int>(active_layer->bounds_height),
                                          active_layer->min_size);
...
ResolvedVideoCrop resolve_video_crop(const SourceSelectionState& state) {
    ...
    if (crop.x == 0 &&
        ...
        crop.width > 0 &&
        crop.height > 0 &&
        (crop.width < state.capture_width || crop.height < state.capture_height)) {
...
AnnotationBox resolved_video_crop_box(const SourceSelectionState& state) {
    ...
    return clamp_box(AnnotationBox{
                         crop.x,
                         crop.y,
                         crop.x + crop.width,
                         crop.y + crop.height,
