...
mmltk::live::LiveSessionConfig make_predict_live_session_config(const mmltk::rfdetr::LivePredictOptions& options) {
    ...
    config.workspace_drm_modifier = mmltk::live::require_workspace_drm_modifier(options.device_id);
    return config;
}
#endif
...
void LivePredictController::launch(mmltk::runtime::BackgroundExecutor& background_executor,
                                   ...
                                   const std::string& preset_name, std::function<void(int)> on_started,
...
#else
            outcome.controller =
                std::make_shared<mmltk::live::LiveSessionController>(make_predict_live_session_config(options));
...
std::shared_ptr<mmltk::live::LiveSessionController> LivePredictController::controller_handle() const noexcept {
    return controller_;
}
