...
inline void append_masked_websocket_frame(std::string& frame, const unsigned char opcode, const std::span<const std::byte> payload) {
    ...
    for (const unsigned char byte : kWebSocketTestMask)
        frame.push_back(static_cast<char>(byte));
    for (std::size_t index = 0U; index < payload.size(); ++index) {
        frame.push_back(
            static_cast<char>(std::to_integer<unsigned char>(payload[index]) ^ kWebSocketTestMask[index % kWebSocketTestMask.size()]));
    }
}
...
[[nodiscard]] inline std::vector<std::byte> length_prefixed_frame(const std::span<const std::byte> payload) {
    if (payload.size() > std::numeric_limits<std::uint32_t>::max())
        throw std::length_error("test transport payload exceeds its 32-bit frame prefix");
