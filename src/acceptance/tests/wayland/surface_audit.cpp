...
auto SurfaceAudit::admit(const std::string& id) -> bool {
 ...
 if (existing == surfaces.end()) {
  ...
  if (state.source_steps.size() < kAcceptanceRecordLimit && state.publication_steps.size() < kAcceptanceRecordLimit && state.reads.size() < kAcceptanceRecordLimit &&
      state.transfers.size() < kAcceptanceRecordLimit && state.receipts.size() < kAcceptanceRecordLimit && state.ended_spans.size() < kAcceptanceRecordLimit &&
      state.samples.size() < kAcceptanceRecordLimit && state.failed_source_operations.size() < kAcceptanceRecordLimit && state.failed_publications.size() < kAcceptanceRecordLimit &&
...
auto SurfaceAudit::native_identity(const nlohmann::json& record, const bool workspace) -> std::string {
 ...
 for (const auto& [offset, field] : {std::pair{0U, "surface_high"}, std::pair{16U, "surface_low"}}) {
  ...
  const auto converted = std::to_chars(digits.data(), digits.data() + digits.size(), scalar(record, workspace ? (offset == 0U ? "workspace_source_high" : "workspace_source_low") : field), 16);
...
auto SurfaceAudit::valid_identity(const std::string_view id) -> bool {
 return id.size() == 32U && id != std::string(32U, '0') && std::ranges::all_of(id, [](const char value) { return (value >= '0' && value <= '9') || (value >= 'a' && value <= 'f'); });
}
auto SurfaceAudit::metadata_receipt(const nlohmann::json& record) -> std::optional<MetadataReceipt> {
 ...
 if (bytes == 0U || fingerprint.size() != 16U || !std::ranges::all_of(fingerprint, [](const char digit) { return (digit >= '0' && digit <= '9') || (digit >= 'a' && digit <= 'f'); })) {
...
auto SurfaceAudit::valid_transfer_timeline(const nlohmann::json& record, const std::uint64_t transfer) -> bool {
 return transfer != 0U && transfer <= std::numeric_limits<std::uint64_t>::max() / 2U && scalar(record, "timeline_ready") == transfer * 2U - 1U && scalar(record, "timeline_release") == transfer * 2U;
}
auto SurfaceAudit::native(const nlohmann::json& record) -> void {
 ...
 if (event.starts_with("presentation.source.admission.") || event == "presentation.source.ready" || event == "presentation.source.withdrawal" || event == "presentation.source.retirement") {
  ...
  if (generation == 0U || width == 0U || height == 0U || allocation == 0U || native_identity(record, true) != id ||
      (source.generation != 0U &&
       (source.generation != generation || source.width != width || source.height != height || source.allocation != allocation || source.direct_sampling != record.value("direct_sampling", false))))
  ...
  if (width > pitch / 4U || height == 0U || bytes / height < pitch || scalar(record, "workspace_width") != width || scalar(record, "workspace_height") != height ||
      (source.bytes != 0U && (source.bytes != bytes || source.pitch != pitch)))
 ...
 const auto width = scalar(record, "capacity_width");
 const auto height = scalar(record, "capacity_height");
 if (generation == 0U || width == 0U || height == 0U || scalar(record, "selection_generation") == 0U || scalar(record, "frame_revision") == 0U || !record.contains("condition") ||
     !record.contains("outcome") || (state.generation != 0U && (state.generation != generation || state.width != width || state.height != height)))
  reject("native surface provenance is missing or inconsistent");
 state.generation = generation;
 state.width = width;
 state.height = height;
 const bool copying = event == "presentation.source_borrow.started" || event == "presentation.source_borrow.completed" || event == "presentation.source.read_submitted";
 ...
 const bool transferred = event == "presentation.ready_sync.started" || event == "presentation.ready_sync.completed" || event == "presentation.frame.edge" || source_release;
 ...
 if (event == "presentation.arena.advertised")
  ...
  const bool span_end = event == "presentation.source_borrow.completed" || event == "presentation.ready_sync.completed" || event == "presentation.release_wait.completed";
  if (span_end) {
  ...
  if (event == "presentation.source_borrow.started") {
   ...
   if (read == state.reads.end() || !state.transfers.emplace(publication_key, TransferReceipt{read->second}).second) reject("physical publication lacks a unique source read");
  } else if (event == "presentation.ready_sync.completed") {
   auto& publication = state.publication_steps[publication_key];
  ...
  if (transferred) {
   ...
   if (receipt == state.transfers.end() || receipt->second.read != observed)
    ...
    if (!native_shutdown || !browser_exited || scalar(record, "outcome") != 1U || receipt->second.terminal != TerminalRead::None || receipt->second.released ||
        state.publication_steps.at(publication_key) != 3U || !sources.contains(observed.source) || record.value("direct_sampling", false) != sources.at(observed.source).direct_sampling) {
...
auto SurfaceAudit::browser(const nlohmann::json& record) -> void {
 ...
 if (!record.contains("surface")) {
  if (event.starts_with("iced.surface.") || event == "firefox.workspace.admitted" || event == "firefox.workspace.claim_outcome" || event == "firefox.workspace.registry_inserted" ||
      event == "firefox.workspace.import_ready_emitted" || event == "firefox.workspace.ready" || event == "firefox.workspace.withdrawal" || event == "firefox.workspace.retired")
 ...
 if (record.contains("width")) {
  ...
  if (width == 0U || height == 0U || (state.browser_width != 0U && (state.browser_width != width || state.browser_height != height))) reject("browser dimensions are missing or inconsistent");
 ...
 if (event == "firefox.workspace.admitted") {
  ...
  if (state.firefox_stage != 2U || state.firefox_import_failed || state.firefox_withdrawn || state.firefox_retired) reject("Firefox import failure lacks its unique claimed surface");
 ...
 if (event == "firefox.workspace.release_only_submitted" || event == "firefox.workspace.release_only_completed") {
  ...
  if (publication == 0U || session == 0U || frame == 0U || !valid_transfer_timeline(record, transfer) || record.contains("layer") || record.contains("slot") || source == sources.end() ||
  ...
  if (event == "firefox.workspace.release_only_submitted") {
   ...
   receipt = {.source = source_id, .transfer = transfer, .session = session, .frame = frame, .stage = 2U, .direct_sampling = source->second.browser_direct_sampling, .release_only = true};
  } else if (!receipt.release_only || receipt.stage != 2U || receipt.source != source_id || receipt.transfer != transfer || receipt.session != session || receipt.frame != frame) {
 ...
 if (event == "firefox.workspace.frame_forwarded" || event == "firefox.workspace.frame_dispatched" || event == "firefox.workspace.copy_completed" || event == "firefox.workspace.read_settled") {
  ...
  if (event == "firefox.workspace.frame_forwarded") {
   ...
   if (receipt.stage != 0U || observed.transfer == 0U || source == sources.end() || !source->second.browser.live() || source->second.arena != id || observed.width > source->second.browser_width ||
       observed.direct_sampling != source->second.browser_direct_sampling || observed.height > source->second.browser_height || !valid_transfer_timeline(record, observed.transfer))
   ...
   if (receipt.release_only || receipt.stage != required || receipt.layer != observed.layer || receipt.slot != observed.slot || receipt.session != observed.session ||
       receipt.frame != observed.frame || receipt.width != observed.width || receipt.height != observed.height || receipt.source != observed.source || receipt.transfer != observed.transfer ||
 ...
 if (event == "iced.surface.draw_encoded" || event == "iced.frame.draw_settled" || event == "iced.frame.draw_abandoned" || event == "iced.frame.sample_released") {
  const auto publication = scalar(record, "presentation_revision");
  const auto receipt = state.receipts.find(publication);
  if (receipt == state.receipts.end() || receipt->second.release_only || receipt->second.stage < 2U || scalar(record, "frame_revision") != receipt->second.frame ||
      scalar(record, "content_session") != receipt->second.session || !record.contains("slot") || scalar(record, "slot") != receipt->second.slot || !record.contains("layer") ||
      scalar(record, "layer") != receipt->second.layer || scalar(record, "content_width") != receipt->second.width || scalar(record, "content_height") != receipt->second.height) {
  ...
  if (custody.released || state.retired != 0U) {
   ...
   if (!custody.acquired || custody.encoded >= custody.selected)
    ...
    if (draws.empty() || draws.back().selected != id || draws.back().requested != record.value("requested_surface", "") || draws.back().publication != publication || draws.back().encoded != 0U ||
        identity == 0U || !state.draw_indices.emplace(identity, draws.size() - 1U).second)
 ...
 if (event == "iced.surface.draw_submitted") {
  ...
  if (!valid_identity(requested) || sample == state.samples.end() || sample->second != scalar(record, "frame_revision") || draw == nullptr || draw->publication != publication ||
      draw->requested != requested || draw->submitted != 0U || draw->settled != 0U || draw->encoded == 0U || draw->encoded >= browser_ordinal || state.retired != 0U ||
      custody == state.custody.end() || custody->second.released || custody->second.settled + custody->second.abandoned >= custody->second.encoded)
  ...
  if (event == "iced.surface.source_texture_create") {
   ...
   for (const auto& [publication, custody] : state.custody) {
    const auto receipt = state.receipts.find(publication);
    if (receipt != state.receipts.end() && receipt->second.source == source && ((custody.acquired && !custody.released) || custody.encoded != custody.settled + custody.abandoned))
  ...
  if (event == "iced.surface.sample_acquired") {
   ...
   const auto receipt = state.receipts.find(publication);
   if (receipt == state.receipts.end() || receipt->second.release_only || receipt->second.stage < 2U || receipt->second.source != record.value("source", "") ||
  ...
  if (state.retired != 0U)
   reject("Iced surface used after texture retirement");
  else if (!valid_identity(requested) || requested == id || state.acquired == 0U || retained == state.custody.end() || !retained->second.acquired || retained->second.released)
   ...
   if (pending.reconstruction || preparation == 0U || retained->second.acquired_at >= browser_ordinal || pending.acquired != 0U || pending.discarded != 0U || pending.retired != 0U)
  ...
  if (std::ranges::any_of(state.source_textures, [](const auto& source) { return source.second != 0U; })) reject("logical texture retirement preceded physical source wrappers");
...
auto SurfaceAudit::source_joined(const SourceState& source) const -> bool {
 ...
 return !source.failed && (initialized || cancelled) && source.generation != 0U && source.width == source.browser_width && source.height == source.browser_height && source.allocation != 0U &&
        source.allocation == source.browser_allocation && source.direct_sampling == source.browser_direct_sampling && valid_identity(source.arena);
}
auto SurfaceAudit::receipts_joined(const std::string& id, const SurfaceState& state, const bool permit_held) const -> bool {
 ...
 const auto retained_read = [&](const std::uint64_t publication, const ReceiverReceipt& receipt) {
  const auto custody = state.custody.find(publication);
  return permit_held && !receipt.release_only && receipt.direct_sampling && receipt.stage == 2U && custody != state.custody.end() && custody->second.acquired && !custody->second.released;
 };
 for (const auto& [publication, receipt] : state.receipts) {
  ...
  if ((!held && !terminal && receipt.stage != 3U) || receipt.stage < 2U || transfer == state.transfers.end() || (!held && !terminal && (!transfer->second.releasing || !transfer->second.released)) ||
      source == sources.end() || !source_joined(source->second) || source->second.arena != id)
   return false;
 ...
 for (const auto& [publication, frame] : state.samples) {
  ...
  if (receipt == state.receipts.end() || receipt->second.release_only ||
      (receipt->second.stage != 3U && !terminal && !(permit_held && receipt->second.direct_sampling && receipt->second.stage == 2U)) || receipt->second.frame != frame)
   return false;
  const auto custody = state.custody.find(publication);
  if (transfer == state.transfers.end() || custody == state.custody.end() || !transfer->second.metadata || !custody->second.metadata || transfer->second.metadata != custody->second.metadata)
   return false;
 }
 return true;
}
...
auto SurfaceAudit::joined_surface_failure(const std::string& id, const SurfaceState& state) const -> std::string {
 ...
 if (state.created == 0U && state.samples.empty()) {
  ...
  if ((!unclaimed_shutdown && (state.native_stage != 4U || state.firefox_stage != 5U)) || state.import_failed || state.firefox_import_failed || state.width != state.browser_width ||
  ...
  if ((!released_without_sampling && (!state.publications.empty() || !state.reads.empty() || !state.transfers.empty() || !state.receipts.empty())) || !state.custody.empty() ||
      !state.source_textures.empty())
 ...
 if (state.firefox_import_failed) {
  ...
  if (state.firefox_stage != 2U || state.created == 0U || state.acquired != 0U || state.discarded <= state.created || state.retired <= state.discarded || !state.samples.empty() ||
      state.firefox_withdrawn || state.firefox_retired)
 ...
 if (!state.withdrawn && !state.firefox_withdrawn && !state.import_failed && !native_shutdown) return "surface " + id + " lacks native or receiver withdrawal before physical retirement";
 ...
 if (state.discarded != 0U && (state.retired == 0U || (!state.withdrawn && !state.firefox_withdrawn))) return "discarded pending surface " + id + " lacks withdrawal and texture retirement";
 return {};
}
...
auto SurfaceAudit::settled_draw(const Draw& draw) const -> const SampleCustody* {
 ...
 if (draw.ordinal <= scenario_started || custody == surface.custody.end() || !custody->second.acquired || custody->second.acquired_at >= draw.ordinal || draw.submitted <= draw.encoded ||
     draw.encoded <= draw.ordinal || draw.settled <= draw.submitted || draw.abandoned)
  return nullptr;
...
auto SurfaceAudit::pending_fallback(const std::string& candidate) const -> const Draw* {
 ...
 if (pending.admitted == 0U || pending.acquired != 0U || !pending.publications.empty() || !pending.reconstruction || pending.reconstruction->requested != candidate ||
     pending.reconstruction->ordinal <= pending.admitted)
  return nullptr;
 ...
 if (active.generation >= pending.generation || retained == active.custody.end() || retained->second.acquired_at == 0U || retained->second.acquired_at != latest_prior_acquisition ||
     !receipts_joined(active_identity, active, true))
  return nullptr;
 const auto fallback = std::ranges::find_if(draws, [&](const Draw& draw) {
  return draw.selected == active_identity && draw.requested == (pending.created == 0U ? active_identity : candidate) && draw.publication == pending.reconstruction->publication &&
         draw.ordinal > pending.reconstruction->ordinal && settled_draw(draw) != nullptr;
 });
...
auto SurfaceAudit::pending_supersession_completed() const -> bool {
 ...
 for (const auto& [b, pending] : surfaces) {
  ...
  if (!pending.candidate_withdrawn || preparation == 0U || pending.acquired != 0U || !pending.publications.empty() || discarded <= preparation || !retired || !pending.reconstruction ||
      pending.reconstruction->requested != b || pending.reconstruction->ordinal <= preparation || (pending.created != 0U && pending.reconstruction->ordinal >= discarded) || !pending.firefox_retired ||
  ...
  for (const auto& draw : draws) {
   ...
   if (latest.generation > pending.generation && latest.created > preparation && latest.acquired > discarded && latest.acquired > pending.reconstruction->ordinal && draw.ordinal > fallback->ordinal &&
       draw.requested == draw.selected && custody != nullptr && custody->acquired_at == latest.acquired && receipts_joined(draw.selected, latest, true))
    return true;
  }
 }
 return false;
