...
auto SurfaceAudit::source_transition(const std::string& id, const std::string_view event, const bool browser, const std::uint64_t code) -> void {
 ...
 if (!browser && next == 5U && std::ranges::any_of(surfaces, [&](const auto& surface) {
      return std::ranges::any_of(surface.second.transfers,
                                 [&](const auto& transfer) { return transfer.second.read.source == id && transfer.second.terminal == TerminalRead::Retained; });
     })) {
...
auto SurfaceAudit::native(const nlohmann::json& record) -> void {
 ...
 if (event.starts_with("presentation.source.admission.") || event == "presentation.source.ready" || event == "presentation.source.withdrawal" ||
  ...
  if (generation == 0U || width == 0U || height == 0U || allocation == 0U || native_identity(record, true) != id ||
      (source.generation != 0U && (source.generation != generation || source.width != width || source.height != height || source.allocation != allocation ||
...
auto SurfaceAudit::browser(const nlohmann::json& record) -> void {
 ...
 if (event == "firefox.workspace.release_only_submitted" || event == "firefox.workspace.release_only_completed") {
  ...
  if (publication == 0U || session == 0U || frame == 0U || !valid_transfer_timeline(record, transfer) || record.contains("layer") || record.contains("slot") ||
      source == sources.end() || source->second.arena != id) {
 ...
 if (event == "firefox.workspace.frame_forwarded" || event == "firefox.workspace.frame_dispatched" || event == "firefox.workspace.copy_completed" ||
  ...
  if (event == "firefox.workspace.frame_forwarded") {
   ...
   if (receipt.release_only || receipt.stage != required || receipt.layer != observed.layer || receipt.slot != observed.slot ||
       receipt.session != observed.session || receipt.frame != observed.frame || receipt.width != observed.width || receipt.height != observed.height ||
       receipt.source != observed.source || receipt.transfer != observed.transfer || receipt.direct_sampling != observed.direct_sampling ||
       (event == "firefox.workspace.copy_completed" && receipt.direct_sampling) || (event == "firefox.workspace.read_settled" && !receipt.direct_sampling))
 ...
 if (event == "iced.surface.draw_encoded" || event == "iced.frame.draw_settled" || event == "iced.frame.draw_abandoned" ||
  ...
  if (receipt == state.receipts.end() || receipt->second.release_only || receipt->second.stage < 2U ||
      scalar(record, "frame_revision") != receipt->second.frame || scalar(record, "content_session") != receipt->second.session || !record.contains("slot") ||
      scalar(record, "slot") != receipt->second.slot || !record.contains("layer") || scalar(record, "layer") != receipt->second.layer ||
      scalar(record, "content_width") != receipt->second.width || scalar(record, "content_height") != receipt->second.height) {
 ...
 if (event == "iced.surface.draw_submitted") {
  ...
  if (state.retired != 0U)
   ...
   if (pending.reconstruction || preparation == 0U || retained->second.acquired_at >= browser_ordinal || pending.acquired != 0U || pending.discarded != 0U ||
       pending.retired != 0U)
...
auto SurfaceAudit::receipts_joined(const std::string& id, const SurfaceState& state, const bool permit_held) const -> bool {
 ...
 const auto retained_read = [&](const std::uint64_t publication, const ReceiverReceipt& receipt) {
  const auto custody = state.custody.find(publication);
  return permit_held && !receipt.release_only && receipt.direct_sampling && receipt.stage == 2U && custody != state.custody.end() && custody->second.acquired &&
         !custody->second.released;
 };
...
auto SurfaceAudit::pending_supersession_completed() const -> bool {
 ...
 for (const auto& [b, pending] : surfaces) {
  ...
  if (!pending.candidate_withdrawn || preparation == 0U || pending.acquired != 0U || !pending.publications.empty() || discarded <= preparation || !retired ||
      !pending.reconstruction || pending.reconstruction->requested != b || pending.reconstruction->ordinal <= preparation ||
  ...
  for (const auto& draw : draws) {
   ...
   if (latest.generation > pending.generation && latest.created > preparation && latest.acquired > discarded &&
       latest.acquired > pending.reconstruction->ordinal && draw.ordinal > fallback->ordinal && draw.requested == draw.selected && custody != nullptr &&
