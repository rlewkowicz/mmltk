...
struct BrowserAudit final {
 ...
 std::map<std::string, nlohmann::json, std::less<>> dataset_native_facts;
 std::set<std::uint64_t> dataset_native_generations;
 bool dataset_native_valid = true;
 std::map<std::string, std::pair<std::array<double, 4>, std::string>, std::less<>> dataset_draw_rows;
 std::map<std::uint64_t, double> dataset_fixture_heights;
 std::map<std::uint64_t, std::vector<double>> dataset_fixture_frames;
 ...
 struct DatasetPaint {
  ...
  std::optional<std::array<double, 4>> reference;
  unsigned fields = 0U;
 };
 std::optional<std::array<double, 4>> dataset_viewport;
 std::map<std::string, DatasetPaint, std::less<>> dataset_paints;
 ...
 std::uint64_t cancelled_compile_generation = 0U;
 std::uint64_t active_compile_generation = 0U;
 [[nodiscard]] bool dataset_presentation_complete() const;
