...
auto AtlasDrawAudit::empty_draw_submitted(const SurfaceAudit& physical) const -> bool {
 ...
 return std::ranges::any_of(acquisitions, [&](const auto& entry) {
  ...
  if (surface == physical.surfaces.end() || surface->second.width != scalar(record, "width") || surface->second.height != scalar(record, "height"))
   return false;
...
auto AtlasDrawAudit::check(const bool passed, const std::string_view reason, const nlohmann::json& record, const nlohmann::json* prior,
                           const std::string_view field, const nlohmann::json* expected) -> bool {
 if (!passed)
...
auto AtlasDrawAudit::check_fields(const nlohmann::json& record, const nlohmann::json& prior, const std::initializer_list<const char*> fields,
                                  const std::string_view reason) -> bool {
...
auto AtlasDrawAudit::allocation_key(const nlohmann::json& record) -> AllocationKey {
...
auto AtlasDrawAudit::differing_field(const nlohmann::json& left, const nlohmann::json& right, const std::initializer_list<const char*> fields) -> const char* {
 const auto found =
  std::ranges::find_if(fields, [&](const auto* field) { return !left.contains(field) || !right.contains(field) || left[field] != right[field]; });
...
auto AtlasDrawAudit::ready_pixels_complete(const nlohmann::json& record) -> bool {
 ...
 if (!check(samples != ready_cell_samples.end(), "stage_ready_pixel_samples_missing", record) ||
     ...
     !check(record.contains("ready_slots") && record["ready_slots"].is_array() && record.contains("visible_indices") && record["visible_indices"].is_array(),
 ...
 for (std::size_t slot = 0U; slot < record["visible_indices"].size(); ++slot) {
  ...
  const double width =
   std::min(left + .6 * side, record["clip"][0].get<double>() + record["clip"][2].get<double>()) - std::max(left, record["clip"][0].get<double>());
  const double height =
   std::min(top + .6 * side, record["clip"][1].get<double>() + record["clip"][3].get<double>()) - std::max(top, record["clip"][1].get<double>());
...
auto AtlasDrawAudit::stage(const nlohmann::json& record) -> void {
 ...
 if (name.starts_with("resize-")) {
  ...
  bool matching =
   check(resize_stages.size() < names.size() && name == names[resize_stages.size()], "resize_stage_out_of_order", record) && source_stage_matches_draw(record);
  if (matching) {
   ...
   if (!resize_stages.empty()) {
    ...
    matching = matching &&
               check(portrait ? scalar(record, "rows") > scalar(previous, "rows") : scalar(record, "rows") < scalar(previous, "rows"),
 ...
 if (name.starts_with("held-")) {
  bool matching = check(held_stages.size() < held_names.size() && name == held_names[held_stages.size()], "held_stage_out_of_order", record) &&
  ...
  if (matching && held_stages.contains("held-visible")) {
   ...
   if (name == "held-aligned" || name == "held-extra" || name == "held-restored")
    matching = matching && check(return_baseline.has_value(), "held_stage_return_baseline_missing", record) &&
               check(scalar(record, "rows") == scalar(*return_baseline, "rows") + (name == "held-extra" ? 1U : 0U), "held_stage_row_count_mismatch", record,
                     &*return_baseline);
  }
  valid = valid && matching;
 ...
 if (name.starts_with("return-")) {
  bool matching =
   check(return_stage < return_names.size() && name == return_names[return_stage], "return_stage_out_of_order", record) && source_stage_matches_draw(record);
  if (matching && return_baseline) {
   matching = check_fields(record, *return_baseline, {"columns", "card_extent", "dataset_identity"}, "return_stage_differs_from_baseline") &&
              ...
              check(scalar(record, "rows") == scalar(*return_baseline, "rows") + (return_stage == 2U ? 1U : 0U), "return_stage_row_count_mismatch", record,
                    &*return_baseline);
  }
  valid = valid && matching;
  if (matching) {
 ...
 if (matching) {
  ...
  if (name == "end") {
   ...
   matching =
    check(columns != 0U && row + scalar(record, "rows") == (total + columns - 1U) / columns, "end_stage_row_mismatch", record) &&
...
auto AtlasDrawAudit::consume(const nlohmann::json& record) -> void {
 ...
 if (event == "integration.atlas_ready_cell") {
  ...
  matching = matching && check(record.contains("cell_sample_x") && record.contains("cell_sample_y") && record.contains("cell_sample_rgba"),
                               "ready_cell_sample_point_missing", record);
  if (matching) {
   if (pixel_meaning &&
       !same_fields(record, *pixel_meaning,
                    {"dataset_identity", "card_extent", "augmentation_enabled", "augmentation_seed", "overlay_boxes", "overlay_masks", "overlay_labels"}))
 ...
 if (event == "iced.surface.sample_acquired") {
  ...
  if (acquisitions.size() >= kAcceptanceRecordLimit || acquisitions.contains(key)) {
   ...
   valid = check(false, acquisitions.size() >= kAcceptanceRecordLimit ? "sample_acquisition_capacity_exhausted" : "sample_acquisition_repeated", record,
                 prior != acquisitions.end() ? &prior->second : nullptr);
   return;
  }
 ...
 bool matching =
  check(source != sources.end(), "draw_gallery_source_missing", record) && check(capture != acquisitions.end(), "draw_sample_acquisition_missing", record) &&
 ...
 if (matching) {
  ...
  matching = matching &&
             check(std::abs(image[2].get<double>() / static_cast<double>(columns) - image[3].get<double>() / static_cast<double>(rows)) < 0.01,
 ...
 if (matching && overlap_draw && !same_fields(record, *overlap_draw, {"dataset_identity", "card_extent", "augmentation_enabled", "augmentation_seed"}))
 ...
 if (matching) {
  for (std::size_t slot = 0U; slot < record["visible_indices"].size(); ++slot) {
   ...
   if (return_stage > 0U && !away_return && stages.empty() && !held_stages.contains("held-complete") && found != retained_ready.end() && found->second.ready &&
       !ready) {
...
auto BrowserAudit::held_placeholder_motion(const std::uint64_t index, const std::uint64_t generation) const -> bool {
 ...
 if (!atlas_draws.valid || held == atlas_draws.held_stages.end() || pending_hover_index != index || pending_read_generation != generation ||
     held_visible_ordinal == 0U || pending_hover_ordinal <= held_visible_ordinal)
  return false;
 ...
 return std::ranges::any_of(shared_explore_motion, [&](const auto& motion) {
  return motion.ordinal > held_visible_ordinal && motion.ordinal < pending_hover_ordinal && motion.x >= static_cast<double>((slot % columns) * side) &&
         motion.x < static_cast<double>((slot % columns + 1U) * side) && motion.y >= static_cast<double>((slot / columns) * side) &&
...
auto BrowserAudit::consume(const nlohmann::json& record) -> void {
 ...
 if (event == "integration.phase_progress") {
  ...
  if (record.value("detail", "") == "shared-primary" && numeric(record, "d") == 1.0 &&
   ...
   shared_primary_colors.insert_or_assign(record.value("control", ""),
  ...
  benchmark_purple = benchmark_purple || (record.value("control", "") == BENCHMARK_OVERRIDE && record.value("detail", "") == "benchmark-purple" &&
  ...
  bounds_valid = bounds_valid && !control.empty() && key != 0U && numeric(record, "b") > 0.0 && numeric(record, "c") > 0.0 && scale > 0.0 &&
                 rendered_style_keys.contains(control) && rendered_style_keys.at(control) == key &&
  ...
  if (!explore_seed_control.empty() && record.value("control", "") == explore_seed_control && scalar(record, "detail") != explore_seed_target &&
  ...
  if (explore_paste_baseline && record.value("control", "") == explore_seed_control && scalar(record, "detail") == explore_seed_target &&
      scalar(record, "a") == scalar(*explore_paste_baseline, "a") && scalar(record, "b") > scalar(record, "a") && scalar(record, "c") == 1U &&
      scalar(record, "d") == 1U)
  ...
  if (record.value("detail", "") == "detail-layout" && numeric(record, "b") > 0.0 && numeric(record, "c") > 0.0 && scalar(record, "d") == 1U)
   detail_resize_measurements.insert(scalar(record, "a"));
 } else if (event == "integration.atlas_resize") {
  if (numeric(record, "a") > 0.0 && numeric(record, "b") > 0.0 && scalar(record, "c") > 0U && scalar(record, "d") >= scalar(record, "c"))
  ...
  show_fps_round_trip = record.value("control", "") == "settings.show_fps" && record.value("detail", "") == "round-trip" && numeric(record, "a") == 1.0 &&
  ...
  workspace_fps_text |= record.value("control", "") == kExploreGalleryControl && valid_text && numeric(record, "a") > 0.0 && numeric(record, "b") >= 0.5 &&
                        std::abs(displayed - numeric(record, "a") / numeric(record, "b")) <= 0.5 && std::abs(numeric(record, "c") - 6.0) < 0.01 &&
                        std::abs(numeric(record, "d") - 6.0) < 0.01;
 } else if (event == "integration.workspace_fps_pixels") {
  workspace_fps_pixels |= record.value("control", "") == kExploreGalleryControl && record.value("detail", "") == "visible-counter" &&
  ...
  benchmark_inactive = record.value("control", "") == DATASET_BROWSE && record.value("detail", "") == "unchanged" && numeric(record, "a") == 7.0 &&
                       numeric(record, "b") == 1.0 && numeric(record, "c") > 0.0;
  ...
  if (record.value("detail", "") == "native-settled")
   benchmark_choices.insert_or_assign(scalar(record, "a"), std::pair{record.value("control", ""), std::array{numeric(record, "a"), numeric(record, "b"),
  ...
  dataset_configured = record.value("detail", "") == "typed-settings" && record.value("control", "") == COMPILE_RESOLUTION && scalar(record, "a") != 0U &&
                       scalar(record, "b") == 1U;
  ...
  progress = record.value("control", "") == COMPILE_PROGRESS && scalar(record, "a") != 0U && !record.value("detail", "").empty() && compile_total != 0U &&
             compile_completed <= compile_total;
  ...
  if (matched) {
   ...
   if (event == "integration.annotation_pixel" && scale > 0.0 && scale < 1.0 && std::ranges::max(expected) == 255.0 && std::ranges::min(expected) == 0.0 &&
       peak - low >= 127.5) {
    minimum = low;
    gain = 255.0 / (peak - low);
    tolerance = 48.0;
   ...
   for (std::size_t channel = 0U; channel < 3U; ++channel) {
    matched = matched && std::isfinite(expected[channel]) && std::isfinite(observed[channel]) && expected[channel] >= 0.0 && expected[channel] <= 255.0 &&
              observed[channel] >= 0.0 && observed[channel] <= 255.0;
   ...
   if (matched && event == "integration.annotation_pixel" && scale < 1.0 && std::ranges::max(expected) != std::ranges::min(expected)) {
    ...
    for (std::size_t channel = 0U; channel < 3U; ++channel)
     blended = blended && std::abs(observed[channel] - std::lerp(background[channel], expected[channel], coverage)) <= 3.0;
   }
  ...
  initial_atlas_complete = record.value("detail", "") == "no-input-canvas-pixels" && scalar(record, "a") != 0U && scalar(record, "b") != 0U &&
                           atlas_canvas_pixels.contains({scalar(record, "c"), scalar(record, "d")});
  ...
  if (found != viewer_label_colors.end() && scalar(record, "b") != 0U) {
   ...
   for (std::size_t channel = 0U; channel != expected.size(); ++channel)
    viewer_class_colors = viewer_class_colors && std::abs(found->second[channel] - expected[channel] / 255.0) <= 1.0 / 255.0;
  ...
  original_detail_drawn =
   original_detail_drawn || (scalar(record, "a") == 512U && scalar(record, "b") == 256U && std::abs(numeric(record, "c") / numeric(record, "d") - 2.0) < 0.01);
 } else if (event == "integration.explore_reopened") {
  reopened = record.value("detail", "") == "usable-after-reopen" && scalar(record, "a") != 0U && scalar(record, "b") != 0U && scalar(record, "c") != 0U &&
             scalar(record, "d") != 0U;
 } else if (event == "integration.ui_scale_drag") {
  if (record.value("detail", "") == "second-position")
   ui_scale_drag =
    numeric(record, "a") == numeric(record, "c") && numeric(record, "b") > 0.85 && numeric(record, "a") != numeric(record, "b") && numeric(record, "d") == 2.0;
  ...
  if (record.value("control", "") == "settings.ui_scale" && numeric(record, "b") == (pressed ? 1.0 : 0.0) && numeric(record, "c") > 0.0 &&
  ...
  ui_scale_restored = record.value("control", "") == "settings.ui_scale" && record.value("detail", "") == "baseline" &&
                      numeric(record, "a") == numeric(record, "b") && numeric(record, "b") == numeric(record, "c") && numeric(record, "d") != 0.0;
 } else if (event == "integration.error_modal") {
  error_modal_usable =
   record.value("detail", "") == "copy-and-dismiss" && numeric(record, "a") == 1.0 && numeric(record, "b") == 1.0 && numeric(record, "c") == 1.0;
 } else if (event == "integration.explore_exact_grid") {
  const auto columns = scalar(record, "c");
  const auto rows = scalar(record, "d");
  exact_grid = record.value("detail", "") == "oversized-logical-fill" && columns != 0U && rows != 0U && scalar(record, "a") % columns == 0U &&
               scalar(record, "b") % rows == 0U && scalar(record, "a") / columns == scalar(record, "b") / rows;
  ...
  newest_placeholder = record.value("detail", "") == "newest-without-wait-all" && scalar(record, "a") != 0U && scalar(record, "b") != 0U &&
  ...
  const bool dispatched = record.value("detail", "") == "real-canvas-pointer" && pending_pointer_frame_revision != 0U &&
                          ...
                          surface_scales.contains(std::pair{scalar(record, "b"), dispatched_frame_revision}) && numeric(record, "c") > 0.0 &&
                          numeric(record, "d") > 0.0;
  if (dispatched) {
  ...
  const bool selected = record.value("detail", "") == "selected-from-dispatched-pointer" && scalar(record, "a") == pointer_revision &&
                        scalar(record, "b") == pointer_slot && scalar(record, "c") == pointer_compiled_index && scalar(record, "c") == scalar(record, "d") &&
  ...
  const bool complete_growth = record.value("detail", "").ends_with("-four-times") && scalar(record, "c") == scalar(record, "a") * 4U &&
  ...
  const bool complete_presentation = record.value("detail", "") == "complete-four-times-exported-frame" && scalar(record, "a") == scalar(record, "c") &&
                                     scalar(record, "b") == scalar(record, "d") && scalar(record, "a") > 1500U && scalar(record, "b") > 0U;
  ...
  if (record.value("detail", "") == "exact-completed-blue" && scalar(record, "a") > 0U && scalar(record, "b") > 0U && scalar(record, "c") > 0U &&
      scalar(record, "d") >= 32U)
   upscale_completed_pixels.insert(record.value("control", ""));
 } else if (event == "integration.upscale_same_method") {
  if (record.value("detail", "") == "same-completed-result" && scalar(record, "a") > 0U && scalar(record, "b") > 0U && scalar(record, "c") > 0U &&
      scalar(record, "d") == 1U)
   upscale_same_method.insert(record.value("control", ""));
 } else if (event == "integration.upscale_later_frame") {
  upscale_later_frame = record.value("detail", "") == "distinct-imported-frame" && scalar(record, "b") > scalar(record, "a") && scalar(record, "c") != 0U &&
                        scalar(record, "d") != 0U;
  ...
  annotation_tool =
   record.value("detail", "") == "typed-tool" && record.value("control", "").starts_with("annotation.tool.") && scalar(record, "b") > scalar(record, "a");
  ...
  complete = record.value("detail", "") == "typed-mvc-wayland" && annotation_revision != 0U && exported_receipt != 0U && exported_receipt == frame_receipt &&
             frame_receipt == browser_receipt;
...
auto BrowserAudit::readiness_blocker() const -> std::string_view {
 ...
 const bool every_region = std::ranges::all_of(pages, [this, &page_prefix, &primary_action](const std::string_view page) {
  ...
  return std::ranges::all_of(regions,
                             [this, page](const std::string_view region) { return page_bounds.contains(std::string{page} + ":" + std::string{region}); }) &&
 ...
 const bool model_composition = [&] {
  ...
  return std::ranges::all_of(model_parts, [this](const Bounds& part) { return model_card.contains(part); }) && model_card.contains(model_progress) &&
         selector.contains(presets) && selector.contains(divider) && selector.contains(custom) && std::abs(custom.x - (selector.x + 2.0)) < 1.0 &&
         std::abs(custom.width - (selector.width - 4.0)) < 1.0 && divider.y >= presets.y + presets.height - 1.0 &&
         custom.y >= divider.y + divider.height - 1.0 && status.y >= selector.y + selector.height - 1.0 && model_progress.y >= status.y + status.height - 1.0 &&
         action.y >= model_progress.y + model_progress.height - 1.0;
 }();
 const double page_width = train_setup.width + train_center.width + train_diagnostics.width;
 const bool reference_columns = train_setup.valid() && train_center.valid() && train_diagnostics.valid() && train_setup.x < train_center.x &&
 ...
 const bool explore_composition = explore_dataset.valid() && explore_gallery.valid() && explore_details.valid() && explore_dataset.x < explore_gallery.x &&
                                  explore_gallery.x < explore_details.x && std::abs(explore_dataset.width - 280.0) < 1.0 &&
                                  std::abs(explore_details.width - 300.0) < 1.0;
 ...
 const bool annotation_composition =
  annotation_workspace != nullptr && annotation_diagnostics != nullptr && annotation_setup != nullptr && annotation_center != nullptr &&
  annotation_advanced != nullptr && annotation_save != nullptr && annotation_sidebar.valid() && annotation_timeline.valid() && annotation_operation.valid() &&
  annotation_stop.valid() && annotation_brush.valid() && annotation_tool_control.valid() && annotation_diagnostics->contains_horizontally(annotation_sidebar) &&
  annotation_diagnostics->contains_horizontally(annotation_operation) && annotation_diagnostics->contains_horizontally(annotation_stop) &&
  annotation_diagnostics->contains_horizontally(annotation_brush) && annotation_diagnostics->contains_horizontally(annotation_tool_control) &&
 ...
 const bool settings_composition = settings_modal.valid() && std::abs(settings_modal.width - 520.0) < 0.01 && settings_footer.valid() &&
 ...
 const bool settings_numeric_alignment = [&] {
  ...
  for (std::size_t index = 0; index < SETTINGS_NUMERIC_CONTROLS.size(); ++index) {
   ...
   if (!control.valid() || !label.valid() || !value.valid() || std::abs(label.x - reference_label.x) >= 1.0 ||
       ...
       std::abs(value.width - reference_value.width) >= 1.0 || label.x < control.x - 1.0 || value.x + value.width > control.x + control.width + 1.0) {
    return false;
   }
  }
  return true;
 ...
 const bool advanced_composition = aligned_grid(advanced_general) && aligned_grid(advanced_optimizer) && aligned_grid(advanced_match_free) &&
 ...
 const bool error_composition = error_modal.valid() && error_copy.valid() && error_dismiss.valid() && error_copy.x < error_dismiss.x &&
                                error_copy.y >= error_modal.y - 1.0 && error_dismiss.y >= error_modal.y - 1.0 &&
                                error_modal.contains_horizontally(error_copy) && error_modal.contains_horizontally(error_dismiss);
 ...
 const bool complete_pointer_drag =
  std::ranges::all_of(expected_drag_stages, [this](const std::string_view stage) { return ui_scale_pointer_stages.contains(stage); });
 ...
 return first_failed_check(
  ...
  "floating spinner suppression", advanced_integer_persisted, "Advanced integer persistence", advanced_floating_persisted, "Advanced floating persistence",
  compile_progress_placement, "Dataset progress placement", model_progress_placement, "Model progress containment", model_composition && model_copy,
  "Model card composition", benchmark_override.valid() && benchmark_round_trip && benchmark_choices_complete(), "benchmark override interaction",
  ...
  "Dataset lifecycle", explore_ready, "ready snapshot", sweep, "viewport sweep", scrolled, "gallery scroll", detail, "detail selection", bounded_exact_grid,
  "bounded exact grid", newest_placeholder, "newest placeholder", pointer_inverse, "pointer inverse", pointer_dispatched, "pointer dispatch", pointer_selected,
  "pointer selection", gallery_shader_fill, "gallery shader fill", pointer_render_chain, "pointer render chain", atlas_identities, "atlas identities",
  augmentation_enabled, "augmentation enabled", augmentation_rerolled, "augmentation rerolled", reshuffle_order_only, "reshuffle order only", detail_source,
  "detail source", detail_fit, "detail fit", detail_containers, "detail containers", padded_and_original_detail, "padded and original detail", upscale_growth,
  "upscale growth", upscale_presentation, "upscale presentation", upscale_modes == expected_upscale_modes, "upscale modes",
  upscale_presentations == expected_upscale_presentations, "upscale presentations", upscale_completed_pixels == expected_upscale_presentations,
  "upscale completed blue pixels", upscale_same_method == expected_upscale_presentations, "upscale exact re-click", upscale_later_frame, "upscale later frame",
  std::ranges::all_of(
...
auto BrowserAudit::rendered_frame_for_slots(const std::map<std::uint64_t, std::uint64_t>& native_slots, const std::uint64_t source_revision,
                                            const std::size_t minimum_redraws) const noexcept -> bool {
 for (const auto& [snapshot_revision, slots] : explore_slots) {
  ...
  for (const auto& geometry : surface_geometries) {
   ...
   if (geometry.source_revision != source || !surface_scales.contains(key) || !surface_draws.contains(geometry.presentation_revision) ||
       (minimum_redraws != 0U &&
        (!surface_redraw_counts.contains(geometry.presentation_revision) || surface_redraw_counts.at(geometry.presentation_revision) < minimum_redraws)))
    continue;
   const double scale = surface_scales.at(key);
   const bool normalized = geometry.width > 0.0 && geometry.height > 0.0 && scale > 0.0 &&
                           (!explore_gallery.valid() || (std::abs(geometry.width / scale - explore_gallery.width) < 1.0 && square_atlas_frames.contains(key) &&
...
auto BrowserAudit::observed_frame_revision_for_slots(const std::map<std::uint64_t, std::uint64_t>& native_slots,
                                                     const std::uint64_t source_revision) const noexcept -> bool {
...
auto BrowserAudit::final_cursor_slots() const noexcept -> const std::map<std::uint64_t, std::uint64_t>* {
 ...
 if (final_cursor_revision == 0U || final_cursor_frame_revision == 0U || final_cursor_generation == 0U || final_cursor_slot_count == 0U ||
     final_cursor_slot_count > mmltk::controller::kExploreVisibleItemCapacity || slots == explore_slots.end() ||
...
bool BrowserAudit::benchmark_choices_complete() const {
 ...
 if ((baseline[0] != 0.0 && baseline[0] != 1.0) || (baseline[1] != 0.0 && baseline[1] != 1.0) ||
     (baseline[2] != 0.0 && baseline[2] != 1.0 && baseline[2] != 2.0) || baseline[3] <= 0.0)
  return false;
 constexpr std::array benchmark_controls{"train.dataset.benchmark.custom", "train.dataset.benchmark.coconut", "train.dataset.validation.coconut",
 ...
 for (std::size_t index = 0; index != 12U; ++index) {
  ...
  if (control != expected_control || values[1] != dataset || values[2] != validation || values[3] < previous_revision ||
      (changed && values[3] == previous_revision))
   return false;
  ...
  for (std::size_t choice = 0; choice != benchmark_controls.size(); ++choice) {
   ...
   if (visible == benchmark_visibility.end() || visible->second[1] != expected || visible->second[2] != expected || visible->second[3] != values[3])
    return false;
  }
