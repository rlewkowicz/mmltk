...
static void transfer_fd(int original, Fd& received) {
 ...
 int sockets[2];
 if (socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0, sockets)) throw std::runtime_error("socketpair");
 ...
 if (sendmsg(sender.value, &message, MSG_NOSIGNAL) != 1) throw std::runtime_error("sendmsg");
 control.fill(0);
 if (recvmsg(receiver.value, &message, MSG_CMSG_CLOEXEC) != 1) throw std::runtime_error("recvmsg");
 header = CMSG_FIRSTHDR(&message);
 if (!header || header->cmsg_level != SOL_SOCKET || header->cmsg_type != SCM_RIGHTS || header->cmsg_len != CMSG_LEN(sizeof(int)) || message.msg_flags & (MSG_CTRUNC | MSG_TRUNC))
...
struct Device {
 ...
 void initialize_cuda(unsigned mode) {
  ...
  if (isolated_context)
   CHECK(cuCtxCreate(&context, nullptr, CU_CTX_SCHED_AUTO, cuda_device));
  else
   CHECK(cuDevicePrimaryCtxRetain(&context, cuda_device));
  ...
  std::printf("cuda_pid=%ld cuda_context=%s\n", static_cast<long>(getpid()), mode == 2 ? "separate_producer_and_semaphore" : isolated_context ? "isolated" : "primary");
 }
 void initialize_vulkan(bool validation) {
  ...
  debug.pfnUserCallback = [](VkDebugUtilsMessageSeverityFlagBitsEXT severity, VkDebugUtilsMessageTypeFlagsEXT, const VkDebugUtilsMessengerCallbackDataEXT* data, void* user) -> VkBool32 {
   if (severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) static_cast<std::atomic<unsigned>*>(user)->fetch_add(1, std::memory_order_relaxed);
  ...
  for (auto candidate : devices) {
   ...
   if (std::memcmp(uuid.bytes, ids.deviceUUID, VK_UUID_SIZE) == 0) {
    physical = candidate;
    std::printf("vulkan_pid=%ld device=%s driver=%u uuid=", static_cast<long>(getpid()), properties.properties.deviceName, properties.properties.driverVersion);
  ...
  std::vector<VkPhysicalDeviceGroupProperties> groups(count, vk_structure<VkPhysicalDeviceGroupProperties>(VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_GROUP_PROPERTIES));
  CHECK(vkEnumeratePhysicalDeviceGroups(instance, &count, groups.data()));
  for (const auto& group : groups)
   for (uint32_t i = 0; i < group.physicalDeviceCount; ++i)
    if (group.physicalDevices[i] == physical && group.physicalDeviceCount != 1) throw std::runtime_error("CUDA interop requires a single-device Vulkan group");
  ...
  if (!(semaphore_properties.externalSemaphoreFeatures & VK_EXTERNAL_SEMAPHORE_FEATURE_EXPORTABLE_BIT)) throw std::runtime_error("timeline opaque FD export unavailable");
  float priority = 1;
...
struct Image {
 ...
 Image(Device& owner, uint32_t w, uint32_t h, MemoryOwner memory) : device(owner), width(w), height(h), memory_owner(memory) {}
 ...
 void release_native() {
  ...
  if (cuda_semaphore) {
   CHECK(cudaDestroyExternalSemaphore(cuda_semaphore));
  ...
  if (address) {
   CHECK(cuMemFree(address));
   address = 0;
  }
  if (external_memory) {
   CHECK(cuDestroyExternalMemory(external_memory));
   external_memory = nullptr;
  }
  if (native) {
   CHECK(cuMemFreeHost(native));
   native = nullptr;
  }
  if (backing.value >= 0) {
 ...
 void barrier(uint32_t from, uint32_t to, VkImageLayout old_layout, VkAccessFlags source, VkAccessFlags destination) {
  ...
  vkCmdPipelineBarrier(device.command, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier);
 }
 void initialize(bool vulkan_only = false) {
  ...
  if ((format_properties.linearTilingFeatures & sampled) != sampled) throw std::runtime_error("linear RGBA filtered sampling unavailable");
  ...
  const auto required_feature = memory_owner == MemoryOwner::Cuda ? VK_EXTERNAL_MEMORY_FEATURE_IMPORTABLE_BIT : VK_EXTERNAL_MEMORY_FEATURE_EXPORTABLE_BIT;
  if (!(features & required_feature) || !(external_properties.externalMemoryProperties.compatibleHandleTypes & external.handleType))
   throw std::runtime_error("requested linear RGBA opaque FD sharing direction unavailable");
  if (width > properties.imageFormatProperties.maxExtent.width || height > properties.imageFormatProperties.maxExtent.height ||
  ...
  needs_dedicated = dedicated.requiresDedicatedAllocation || (features & VK_EXTERNAL_MEMORY_FEATURE_DEDICATED_ONLY_BIT) || memory_owner == MemoryOwner::VulkanDedicated;
  ...
  allocate.memoryTypeIndex = device.memory_type(requirements.memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
  ...
  if (memory_owner == MemoryOwner::Cuda)
   ...
   device.submit(semaphore, 0);  // Vulkan signals 1 before the first CUDA access.
  ...
  allocate.memoryTypeIndex = device.memory_type(buffer_requirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
 ...
 void compare(const void* pixels, uint32_t seed, const char* owner) const {
  ...
  for (uint32_t y = 0; y < height; ++y)
   for (uint32_t x = 0; x < width; ++x) bad += values[size_t(y) * width + x] != cuda_vulkan_pixel(seed, x, y);
 ...
 uint64_t produce(uint32_t expected) {
  ...
  CHECK(cuCtxSetCurrent(device.producer_context ? device.producer_context : device.context));
  const auto writing_stream = device.producer_context ? device.producer_stream : device.stream;
  CHECK(cuda_vulkan_fill(reinterpret_cast<void*>(address + layout.offset), layout.rowPitch, width, height, expected, writing_stream));
  if (device.producer_context) {
 ...
 void consume(uint64_t ready, uint32_t expected) {
  ...
  vkCmdPipelineBarrier(device.command, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 1, &host_barrier, 0, nullptr, 0, nullptr);
  device.submit(semaphore, ready);
  compare(host, expected, "Vulkan");
 }
 void transfer(uint32_t expected) { consume(produce(expected), expected); }
};
...
static Packet receive_packet(int socket, Operation expected, Fd* memory = nullptr, Fd* semaphore = nullptr) {
 ...
 if (received != sizeof(packet) || message.msg_flags & (MSG_TRUNC | MSG_CTRUNC) || !valid || count != (memory ? 2U : 0U) || packet.operation != expected)
...
public:
 Exporter() {
  int sockets[2];
  if (socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0, sockets)) throw std::runtime_error("peer socketpair");
  ...
  if (status) {
   pid_ = -1;
   throw std::runtime_error("fresh Vulkan peer spawn: " + std::to_string(status));
  }
 }
 ...
 void settle_exit() {
  ...
  pid_ = -1;
  if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) throw std::runtime_error("Vulkan exporter failed before settled exit");
 }
};
static uint32_t pixel_seed(unsigned pair, unsigned slot, unsigned iteration) { return (pair << 16U) | (slot << 15U) | iteration; }
static int vulkan_peer() {
 ...
 for (unsigned pair = 0; pair < settings.pairs; ++pair) {
  ...
  for (unsigned iteration = 0; iteration < settings.transfers; ++iteration) {
   for (unsigned slot = 0; slot < images.size(); ++slot) {
    ...
    if (write.pair != pair || write.slot != slot || write.iteration != iteration || write.seed != pixel_seed(pair, slot, iteration) || write.ready != 2ULL * (iteration + 1))
 ...
 if (device.validation_errors.load(std::memory_order_relaxed)) throw std::runtime_error("Vulkan peer validation errors");
 return 0;
}
...
static NativePair run_peer(Exporter& peer, Device& device, Packet settings) {
 ...
 for (unsigned pair = 0; pair < settings.pairs; ++pair) {
  for (unsigned slot = 0; slot < images.size(); ++slot) {
   ...
   images[slot] = std::make_unique<Image>(device, settings.width, settings.height, static_cast<MemoryOwner>(settings.owner));
   auto& image = *images[slot];
   const auto allocation = receive_packet(peer.socket(), Operation::Allocation, &image.backing, &image.exported_semaphore);
   if (allocation.pair != pair || allocation.slot != slot || allocation.width != settings.width || allocation.height != settings.height ||
       std::memcmp(&allocation.uuid, &device.uuid, sizeof(CUuuid)) || allocation.dedicated > 1 || (settings.owner == 2 && !allocation.dedicated) || allocation.pitch < uint64_t(settings.width) * 4 ||
       allocation.offset > allocation.bytes || allocation.pitch > (allocation.bytes - allocation.offset) / settings.height)
  ...
  for (unsigned iteration = 0; iteration < settings.transfers; ++iteration) {
   for (unsigned slot = 0; slot < images.size(); ++slot) {
    ...
    if (compared.pair != pair || compared.slot != slot || compared.iteration != iteration || compared.seed != write.seed || compared.ready != write.ready)
 ...
 peer.settle_exit();
 for (unsigned slot = 0; slot < images.size(); ++slot) images[slot]->native_read(pixel_seed(settings.pairs - 1, slot, settings.transfers - 1));
...
static void cross_process(Packet settings, unsigned context_mode) {
 ...
 for (unsigned slot = 0; slot < retained.size(); ++slot) retained[slot]->native_read(pixel_seed(settings.pairs - 1, slot, settings.transfers - 1));
...
int main(int argc, char** argv) {
 ...
 try {
  ...
  if (argc == 2 && std::strcmp(argv[1], "--help") == 0) {
   std::puts(
    "Usage: ./mmltk --test cuda-vulkan -- [width height allocation-pairs transfers validation context-mode memory-owner "
    "process-mode]");
   ...
   std::puts(
    "process-mode: 0=same process (all owners), 1=separate Vulkan/CUDA processes (owners 1/2), with exporter-exit retention "
    "and replacement.");
   return 0;
  }
  ...
  if (!width || !height || width > 4096 || height > 4096 || !pairs || pairs > 256 || !transfers || transfers > 256 || validation > 1 || context_mode > 2 || memory_owner > 2 || process_mode > 1 ||
      (process_mode && !memory_owner))
  ...
  if (device.validation_errors.load(std::memory_order_relaxed)) throw std::runtime_error("Vulkan validation reported errors");
