...
namespace {
__global__ void fill_image(void* destination, std::size_t pitch,
                           std::uint32_t width, std::uint32_t height, std::uint32_t value) {
...
cudaError_t cuda_vulkan_fill(void* destination, std::size_t pitch,
    ...
    fill_image<<<dim3((width + 15U) / 16U, (height + 15U) / 16U), dim3(16, 16), 0, stream>>>(
        destination, pitch, width, height, value);
