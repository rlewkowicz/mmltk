...
bool environment_profile_enabled() noexcept {
    ...
    if (enabled && enabled[0] && std::strcmp(enabled, "0") != 0) {
        return true;
    }
...
void profile_enable() noexcept {
    explicit_profile_enabled.store(true, std::memory_order_relaxed);
}

ScopedProfile::ScopedProfile(const char* name) : name_(name), start_ns_(0), active_(profile_enabled()) {
    if (active_) {
        start_ns_ = now_ns();
    }
}
