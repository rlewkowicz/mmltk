...
#ifndef SPDLOG_DISABLE_DEFAULT_LOGGER
SPDLOG_INLINE std::shared_ptr<logger> make_default_logger() {
#ifdef _WIN32
    auto color_sink = std::make_shared<sinks::wincolor_stdout_sink_mt>();
#else
    auto color_sink = std::make_shared<sinks::ansicolor_stdout_sink_mt>();
#endif
    return std::make_shared<spdlog::logger>("", std::move(color_sink));
}

SPDLOG_INLINE registry::registry() : formatter_(new pattern_formatter()), default_logger_(make_default_logger()) {
...
SPDLOG_INLINE void registry::apply_logger_env_levels(const std::shared_ptr<logger>& new_logger) {
    std::lock_guard<std::mutex> lock(logger_map_mutex_);
    auto it = log_levels_.find(new_logger->name());
    auto new_level = it != log_levels_.end() ? it->second : global_log_level_;
    new_logger->set_level(new_level);
