...
SPDLOG_INLINE spdlog::level::level_enum from_str(const std::string& name) SPDLOG_NOEXCEPT {
    ...
    auto it = std::find_if(std::begin(level_string_views), std::end(level_string_views),
                           [&name_view](const string_view_t& level_name) { return iequals(name_view, level_name); });
...
namespace details {
SPDLOG_INLINE std::string format_errno_message(const std::string& msg, int last_errno) {
#ifdef SPDLOG_USE_STD_FORMAT
    return std::system_error(std::error_code(last_errno, std::generic_category()), msg).what();
#else
    memory_buf_t outbuf;
...
SPDLOG_INLINE spdlog_ex::spdlog_ex(const std::string& msg, int last_errno)
    : msg_(details::format_errno_message(msg, last_errno)) {}
