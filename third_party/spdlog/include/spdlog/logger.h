...
class SPDLOG_API logger {
   public:
...
#ifdef SPDLOG_WCHAR_TO_UTF8_SUPPORT
    template <typename... Args>
    void log(const source_loc& loc, level::level_enum lvl, wformat_string_t<Args...> fmt, Args&&... args) {
#if defined(SPDLOG_USE_STD_FORMAT) && __cpp_lib_format < 202207L
        log_(loc, lvl, fmt, std::forward<Args>(args)...);
#else
        log_(loc, lvl, fmt.get(), std::forward<Args>(args)...);
    ...
    void log(log_clock::time_point log_time, const source_loc& loc, level::level_enum lvl, wstring_view_t msg) {
        bool log_enabled = should_log(lvl);
        bool traceback_enabled = tracer_.enabled();
        if (!log_enabled && !traceback_enabled) {
            return;
    ...
    void log(const source_loc& loc, level::level_enum lvl, wstring_view_t msg) {
        bool log_enabled = should_log(lvl);
        bool traceback_enabled = tracer_.enabled();
        if (!log_enabled && !traceback_enabled) {
            return;
...
#ifdef SPDLOG_WCHAR_TO_UTF8_SUPPORT
    template <typename... Args>
    void log_(const source_loc& loc, level::level_enum lvl, wstring_view_t fmt, Args&&... args) {
        bool log_enabled = should_log(lvl);
        bool traceback_enabled = tracer_.enabled();
        if (!log_enabled && !traceback_enabled) {
            return;
