...
FMT_CONSTEXPR auto lossless_integral_conversion(const From from, int& ec) -> To {
    ...
    static_assert(from_limits::is_integer, "From must be integral");
    static_assert(to_limits::is_integer, "To must be integral");

    if (detail::const_check(from_limits::is_signed && !to_limits::is_signed)) {
        if (fmt::detail::is_negative(from)) {
            ec = 1;
            return {};
        }
        if (detail::const_check(from_limits::digits > to_limits::digits) &&
...
template <typename Char>
FMT_CONSTEXPR auto parse_tm_formatter(parse_context<Char>& ctx, formatter<std::tm, Char>& base, bool localized,
                                      bool& use_tm_formatter) -> const Char* {
    auto it = ctx.begin(), end = ctx.end();
    if (localized && it != end && *it == 'L') {
        ++it;
        base.set_localized();
    }
    use_tm_formatter = it != end && *it != '}';
    return use_tm_formatter ? base.parse(ctx) : it;
}
...
#define FMT_CHRONO_PARSE_TM_FORMATTER(localized)                                                         \
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* {                                  \
        return detail::parse_tm_formatter(ctx, static_cast<formatter<std::tm, Char>&>(*this), localized, \
                                          use_tm_formatter_);                                            \
    }
...
struct formatter<weekday, Char> : private formatter<std::tm, Char> {
   ...
   public:
    FMT_CHRONO_PARSE_TM_FORMATTER(true)
...
struct formatter<day, Char> : private formatter<std::tm, Char> {
   ...
   public:
    FMT_CHRONO_PARSE_TM_FORMATTER(false)
