...
#else
#define FMT_ASSERT(condition, message) ((condition) ? (void)0 : ::fmt::assert_fail(__FILE__, __LINE__, (message)))
#endif
...
auto has_formatter_impl(T* p, buffered_context<Char>* ctx = nullptr) -> decltype(formatter<U, Char>().format(*p, *ctx),
                                                                                 std::true_type());
template <typename Char>
