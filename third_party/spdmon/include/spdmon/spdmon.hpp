...
[[nodiscard]] inline std::string format_progress_postfix(
    const std::string_view label, const size_t active_workers,
    const size_t pulse) {
    static constexpr std::array<const char*, 4> kSpinnerFrames{"|", "/", "-", "\\"};
    const char* const spinner = kSpinnerFrames[pulse % kSpinnerFrames.size()];

    std::string postfix{label};
    if (active_workers > 0) {
        postfix += " ";
        postfix += std::to_string(active_workers);
        postfix += " active";
    }
    postfix += " ";
    postfix += spinner;
...
class ProgressBar {
   public:
    ...
    void restart(size_t current, size_t total, std::string unit) {
        current_ = current;
        total_ = total;
        unit_ = std::move(unit);
        started_ = current_time();
   ...
   private:
    ...
    void render(bool force) {
        ...
        if (line.size() > terminal_width) {
            line = build_fitted_label_line(label, include_postfix, bar_width, metrics, terminal_width);
        }
    ...
    // Ellipsizes the label so the rendered line fits the terminal, then
    // rebuilds the line with the shortened label (updated in place).
    std::string build_fitted_label_line(std::string& label, bool include_postfix, int bar_width,
                                        const RenderMetrics& metrics, size_t terminal_width) {
        ...
        return build_line(label, include_postfix, bar_width, metrics);
    }
