...
#include "mozilla/hal_ipc/PHalParent.h"
...
RefPtr<ContentParent::LaunchPromise> ContentParent::WaitForLaunchAsync(
  ...
  return mSubprocess->WhenProcessHandleReady()->Then(
      ...
      [self = std::move(self), aPriority](
        ...
        if (aValue.IsResolve() &&
            self->LaunchSubprocessResolve(aPriority)) {
          MOZ_LOG(ContentParent::GetLog(), LogLevel::Debug,
...
bool ContentParent::WaitForLaunchSync(ProcessPriority aPriority) {
  ...
  if (launchSuccess && LaunchSubprocessResolve(aPriority)) {
    return true;
  }
...
struct PrincipalValidationFailureInfo {
  nsAutoCString principalType;
  nsAutoCString scheme;
};

static PrincipalValidationFailureInfo DescribeAndLogPrincipalValidationFailure(
    nsIPrincipal* aPrincipal, const char* aMethod) {
  PrincipalValidationFailureInfo info;

  if (!aPrincipal) {
    info.principalType.AssignLiteral("NullPtr");
  } else if (aPrincipal->IsSystemPrincipal()) {
    info.principalType.AssignLiteral("SystemPrincipal");
  } else if (aPrincipal->GetIsExpandedPrincipal()) {
    info.principalType.AssignLiteral("ExpandedPrincipal");
  } else if (aPrincipal->GetIsContentPrincipal()) {
    info.principalType.AssignLiteral("ContentPrincipal");

    nsAutoCString origin;
    MOZ_ALWAYS_SUCCEEDS(aPrincipal->GetOriginNoSuffix(origin));
    MOZ_ALWAYS_SUCCEEDS(net_ExtractURLScheme(origin, info.scheme));
  } else {
    info.principalType.AssignLiteral("Unknown");
  }
...
void ContentParent::LogAndAssertFailedPrincipalValidationInfo(
    nsIPrincipal* aPrincipal, const char* aMethod) {
  DescribeAndLogPrincipalValidationFailure(aPrincipal, aMethod);
...
mozilla::ipc::IPCResult ContentParent::PrincipalValidationIpcFail(
    ...
    const char* aMethod) {
  auto info = DescribeAndLogPrincipalValidationFailure(aPrincipal, aMethod);
  ...
  return IPC_FAIL_UNSAFE_PRINTF(
      ...
      info.principalType.get(), info.scheme.IsEmpty() ? "" : " ",
      info.scheme.get(), aMethod,
...
bool ContentParent::LaunchSubprocessResolve(ProcessPriority aPriority) {
...
hal_ipc::PHalParent* ContentParent::AllocPHalParent() {
  return hal_ipc::CreateHalParent();
}

bool ContentParent::DeallocPHalParent(hal_ipc::PHalParent* aHal) {
  delete aHal;
  return true;
}
