...
// The host serves its own page over loopback and nothing else is served at all,
// so the origin's host is the whole authorization question for the reserved
// texture label.
bool IsLoopbackDocument(nsIGlobalObject* aGlobal) {
  nsIPrincipal* const principal = aGlobal ? aGlobal->PrincipalOrNull() : nullptr;
  if (!principal) {
    return false;
  }
  nsAutoCString host;
  if (NS_FAILED(principal->GetAsciiHost(host))) {
    return false;
  }
...
already_AddRefed<Buffer> Device::CreateBuffer(
 ...
 dom::GPUTextureDescriptor Device::SwapChainTextureDescriptor(
...
already_AddRefed<Texture> Device::CreateTexture(
  ...
  // A page names an allocation the host admitted by labelling an ordinary
  // texture with the reserved prefix; stock WebGPU offers no other way to name
  ...
  if (StringBeginsWith(aDesc.mLabel, kWorkspaceLabelPrefix) &&
    ...
    disarmedLabel.AssignLiteral(u"unprivileged/");
    disarmedLabel.Append(aDesc.mLabel);
  }
  webgpu::StringHelper label(disarmedLabel.IsEmpty() ? aDesc.mLabel
                                                     : disarmedLabel);
...
void Device::BindWorkspace() {
  if (!IsLoopbackDocument(GetParentObject())) return;
  GetChild()->FlushQueuedMessages();
  static_cast<void>(GetChild()->SendWorkspaceBind(GetId()));
}

void Device::UnbindWorkspace() {
  if (!IsLoopbackDocument(GetParentObject())) return;
  GetChild()->FlushQueuedMessages();
  static_cast<void>(GetChild()->SendWorkspaceUnbind(GetId()));
}
...
already_AddRefed<BindGroup> Device::CreateBindGroup(
  ...
  for (const auto& entry : aDesc.mEntries) {
    ...
    e.binding = entry.mBinding;
    auto setTextureViewBinding = [&e,
                                  &canvasContexts](const TextureView& view) {
      e.texture_view = view.GetId();
      auto context = view.GetTargetContext();
      if (context) {
