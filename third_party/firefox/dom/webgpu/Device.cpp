...
already_AddRefed<Texture> Device::CreateTexture(
  ...
  constexpr auto kWorkspaceLabelPrefix = u"mmltk-workspace-v1/"_ns;
  nsAutoString authenticatedLabel;
  const bool workspaceLabel =
      StringBeginsWith(aDesc.mLabel, kWorkspaceLabelPrefix);
  if (workspaceLabel) {
    const char* expectedOrigin = std::getenv("MMLTK_WORKSPACE_BROKER_ORIGIN");
    const char* token = std::getenv("MMLTK_WORKSPACE_BROKER_TOKEN");
    nsAutoCString principalOrigin;
    nsIPrincipal* principal = GetParentObject()->PrincipalOrNull();
    const bool tokenValid =
        token && std::strlen(token) == 64 &&
        std::all_of(token, token + 64, [](const char value) {
          return (value >= '0' && value <= '9') ||
                 (value >= 'a' && value <= 'f') ||
                 (value >= 'A' && value <= 'F');
        });
    nsAutoCString expected(expectedOrigin ? expectedOrigin : "");
    if (!expected.IsEmpty() && expected.Last() == '/') {
      expected.Truncate(expected.Length() - 1);
    }
    const bool authorized =
        principal && NS_SUCCEEDED(principal->GetOrigin(principalOrigin)) &&
        !expected.IsEmpty() && principalOrigin.Equals(expected) && tokenValid;
    if (authorized) {
      authenticatedLabel.Assign(kWorkspaceLabelPrefix);
      authenticatedLabel.AppendASCII(token, 64);
      authenticatedLabel.Append(u'/');
      authenticatedLabel.Append(
          Substring(aDesc.mLabel, kWorkspaceLabelPrefix.Length()));
    }
  }
  webgpu::StringHelper label(authenticatedLabel.IsEmpty() ? aDesc.mLabel
                                                          : authenticatedLabel);
