...
#  include "mozilla/webgpu/MmltkWorkspaceBroker.h"
...
#if defined(XP_LINUX) && !0
extern "C" bool wgpu_server_mmltk_workspace_configure_adapter(
    WGPUWebGPUParentPtr aParent, uint32_t aRenderMajor,
    uint32_t aRenderMinor, const uint8_t* aDeviceUuid,
    const uint64_t* aModifiers, size_t aModifierCount,
    bool aTimelineSemaphore) {
  auto* parent = static_cast<WebGPUParent*>(aParent);
  return parent->ConfigureMmltkWorkspaceAdapter(
      aRenderMajor, aRenderMinor, aDeviceUuid, aModifiers, aModifierCount,
      aTimelineSemaphore);
}

extern "C" bool wgpu_server_mmltk_workspace_take_slot(
    WGPUWebGPUParentPtr aParent, uint64_t aGeneration, uint32_t aSlot,
    WGPUMmltkWorkspaceSlot* aDescriptor, int32_t* aDmaBufFd) {
  auto* parent = static_cast<WebGPUParent*>(aParent);
  static_assert(sizeof(WGPUMmltkWorkspaceSlot) == sizeof(MmltkWorkspaceSlot));
  static_assert(offsetof(WGPUMmltkWorkspaceSlot, width) ==
                offsetof(MmltkWorkspaceSlot, width));
  static_assert(offsetof(WGPUMmltkWorkspaceSlot, height) ==
                offsetof(MmltkWorkspaceSlot, height));
  static_assert(offsetof(WGPUMmltkWorkspaceSlot, stride) ==
                offsetof(MmltkWorkspaceSlot, stride));
  static_assert(offsetof(WGPUMmltkWorkspaceSlot, offset) ==
                offsetof(MmltkWorkspaceSlot, offset));
  static_assert(offsetof(WGPUMmltkWorkspaceSlot, allocation_size) ==
                offsetof(MmltkWorkspaceSlot, allocationSize));
  static_assert(offsetof(WGPUMmltkWorkspaceSlot, drm_modifier) ==
                offsetof(MmltkWorkspaceSlot, drmModifier));
  static_assert(offsetof(WGPUMmltkWorkspaceSlot, drm_format) ==
                offsetof(MmltkWorkspaceSlot, drmFormat));
  static_assert(offsetof(WGPUMmltkWorkspaceSlot, slot_count) ==
                offsetof(MmltkWorkspaceSlot, slotCount));
  return parent->TakeMmltkWorkspaceSlot(
      aGeneration, aSlot, reinterpret_cast<MmltkWorkspaceSlot*>(aDescriptor),
      aDmaBufFd);
}

extern "C" bool wgpu_server_mmltk_workspace_send_slot_ready(
    WGPUWebGPUParentPtr aParent, uint64_t aGeneration, uint32_t aSlot,
    int32_t aSemaphoreFd) {
  auto* parent = static_cast<WebGPUParent*>(aParent);
  return parent->SendMmltkWorkspaceSlotReady(aGeneration, aSlot,
                                             aSemaphoreFd);
}

extern "C" void wgpu_server_mmltk_workspace_release_slot(
    WGPUWebGPUParentPtr aParent, uint64_t aGeneration, uint32_t aSlot) {
  auto* parent = static_cast<WebGPUParent*>(aParent);
  parent->ReleaseMmltkWorkspaceSlot(aGeneration, aSlot);
}

extern "C" uint64_t wgpu_server_mmltk_workspace_present_revision(
    WGPUWebGPUParentPtr aParent, uint64_t aGeneration, uint32_t aSlot) {
  auto* parent = static_cast<WebGPUParent*>(aParent);
  return parent->MmltkWorkspacePresentRevision(aGeneration, aSlot);
}
...
#if defined(XP_LINUX) && !0
bool WebGPUParent::ConfigureMmltkWorkspaceAdapter(
    uint32_t aRenderMajor, uint32_t aRenderMinor, const uint8_t* aDeviceUuid,
    const uint64_t* aModifiers, size_t aModifierCount,
    bool aTimelineSemaphore) {
  if (!mMmltkWorkspaceBroker) {
    mMmltkWorkspaceBroker = std::make_unique<MmltkWorkspaceBroker>();
  }
  return mMmltkWorkspaceBroker->ConfigureAdapter(
      aRenderMajor, aRenderMinor, aDeviceUuid, aModifiers, aModifierCount,
      aTimelineSemaphore);
}

bool WebGPUParent::TakeMmltkWorkspaceSlot(
    uint64_t aGeneration, uint32_t aSlot,
    MmltkWorkspaceSlot* aDescriptor, int* aDmaBufFd) {
  if (!mMmltkWorkspaceBroker) {
    mMmltkWorkspaceBroker = std::make_unique<MmltkWorkspaceBroker>();
  }
  return mMmltkWorkspaceBroker->TakeSlot(aGeneration, aSlot, aDescriptor,
                                         aDmaBufFd);
}

bool WebGPUParent::SendMmltkWorkspaceSlotReady(uint64_t aGeneration,
                                               uint32_t aSlot,
                                               int aSemaphoreFd) {
  return mMmltkWorkspaceBroker &&
         mMmltkWorkspaceBroker->SendSlotReady(aGeneration, aSlot,
                                              aSemaphoreFd);
}

void WebGPUParent::ReleaseMmltkWorkspaceSlot(uint64_t aGeneration,
                                             uint32_t aSlot) {
  if (mMmltkWorkspaceBroker) {
    mMmltkWorkspaceBroker->ReleaseSlot(aGeneration, aSlot);
  }
}

uint64_t WebGPUParent::MmltkWorkspacePresentRevision(uint64_t aGeneration,
                                                     uint32_t aSlot) {
  return mMmltkWorkspaceBroker
             ? mMmltkWorkspaceBroker->PresentRevision(aGeneration, aSlot)
             : 0;
}
#endif
