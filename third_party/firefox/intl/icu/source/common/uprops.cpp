...
UCPTrie *gInpcTrie = nullptr;  
UCPTrie *gInscTrie = nullptr;  
UCPTrie *gVoTrie = nullptr;  
...
static UBool isModifierCombiningMark(const BinaryProperty &, UChar32 c, UProperty ) {
    for (int32_t i = 0; i < UPRV_LENGTHOF(MODIFIER_COMBINING_MARK); i += 2) {
        if (c < MODIFIER_COMBINING_MARK[i]) { return false; }  
        if (c < MODIFIER_COMBINING_MARK[i + 1]) { return true; }  
    }
    return false;
}
...
static const BinaryProperty binProps[UCHAR_BINARY_LIMIT]={
    ...
    { UPROPS_SRC_MCM, 0 , isModifierCombiningMark }, 
};
...
static int32_t getBlock(const IntProperty &, UChar32 c, UProperty ) {
    return static_cast<int32_t>(ublock_getCode(c));
}

static int32_t blockGetMaxValue(const IntProperty &, UProperty ) {
