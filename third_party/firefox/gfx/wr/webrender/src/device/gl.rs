...
impl ProgramCache {
    ...
    pub fn report_memory(&self, op: VoidPtrToSizeFn) -> usize {
        self.entries.borrow().values()
            .map(|e| unsafe {
                op(e.binary.bytes.as_ptr() as *const std::ffi::c_void)
            })
            .sum()
    }
}
...
impl Device {
    ...
    pub fn begin_frame(&mut self) -> GpuFrameId {
        ...
        let being_profiled = render_stats::thread_is_being_profiled();
            ...
            && !using_wrapper
        {
            fn note(name: &str, duration: Duration) {
                render_stats::add_text_marker("OpenGL Calls", name, duration);
            }
