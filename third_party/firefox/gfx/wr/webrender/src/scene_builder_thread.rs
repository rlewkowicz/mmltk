...
impl SceneBuilderThread {
    ...
    fn process_transaction(&mut self, mut txn: TransactionMsg) -> Box<BuiltTransaction> {
            ...
            match message {
                ...
                SceneMsg::SetDisplayList {
                    ...
                    let (builder_start_time_ns, builder_end_time_ns, send_time_ns) =
                      display_list.times();
                    let content_send_time = render_stats::ns_to_ms(zeitstempel::now() - send_time_ns);
                    let dl_build_time = render_stats::ns_to_ms(builder_end_time_ns - builder_start_time_ns);
                    profile.set(render_stats::CONTENT_SEND_TIME, content_send_time);
                    profile.set(render_stats::DISPLAY_LIST_BUILD_TIME, dl_build_time);
                    profile.set(render_stats::DISPLAY_LIST_MEM, render_stats::bytes_to_mb(display_list.size_in_bytes()));
        ...
        let scene_build_time_ms =
            render_stats::ns_to_ms(zeitstempel::now() - scene_build_start);
        profile.set(render_stats::SCENE_BUILD_TIME, scene_build_time_ms);

        frame_stats.scene_build_time += scene_build_time_ms;

        if !txn.blob_requests.is_empty() {
            profile.start_time(render_stats::BLOB_RASTERIZATION_TIME);

            let is_low_priority = false;
            rasterize_blobs(&mut txn, is_low_priority, &mut self.tile_pool);

            profile.end_time(render_stats::BLOB_RASTERIZATION_TIME);
        }
...
impl LowPrioritySceneBuilderThread {
    ...
    fn process_transaction(&mut self, mut txn: Box<TransactionMsg>) -> Box<TransactionMsg> {
        ...
        txn.profile.start_time(render_stats::BLOB_RASTERIZATION_TIME);
        rasterize_blobs(&mut txn, is_low_priority, &mut self.tile_pool);
        txn.profile.end_time(render_stats::BLOB_RASTERIZATION_TIME);
