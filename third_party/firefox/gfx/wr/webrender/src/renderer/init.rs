...
use api::{BlobImageHandler, ColorF, DocumentId, IdNamespace};
...
use crate::render_stats::{self, Profiler, TransactionProfile};
...
pub fn create_webrender_instance(
        ...
        .unwrap_or_else(|| {
            let worker = ThreadPoolBuilder::new()
                ...
                .start_handler(move |idx| {
                    render_stats::register_thread(&format!("WRWorker#{}", idx));
                })
                .exit_handler(move |_idx| {
                    render_stats::unregister_thread();
                })
                .build();
    ...
    thread::Builder::new().name(scene_thread_name.clone()).spawn(move || {
        render_stats::register_thread(&scene_thread_name);
    ...
    let low_priority_scene_tx = if options.support_low_priority_transactions {
        ...
        thread::Builder::new().name(lp_scene_thread_name.clone()).spawn(move || {
            render_stats::register_thread(&lp_scene_thread_name);
    ...
    thread::Builder::new().name(rb_thread_name.clone()).spawn(move || {
        ...
        render_stats::register_thread(&rb_thread_name);
