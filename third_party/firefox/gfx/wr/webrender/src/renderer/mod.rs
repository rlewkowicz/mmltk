...
use crate::render_stats::{self, RenderCommandLog, GpuProfileTag, TransactionProfile};
use crate::render_stats::{Profiler, add_event_marker, add_text_marker, thread_is_being_profiled};
...
impl TextureResolver {
    ...
    fn update_profile(&self, profile: &mut TransactionProfile) {
        ...
        profile.set(render_stats::EXTERNAL_IMAGE_BYTES, render_stats::bytes_to_mb(external_image_bytes));
    }
...
impl Renderer {
    ...
    fn render_impl(
        ...
        self.profile.end_time_if_started(render_stats::FRAME_SEND_TIME);
        self.profile.start_time(render_stats::RENDERER_TIME);
        ...
        if thread_is_being_profiled() {
            ...
            if let Some(n) = self.profile.get(render_stats::RENDERED_PICTURE_TILES) {
        ...
        self.profile.end_time(render_stats::RENDERER_TIME);
        self.profile.end_time_if_started(render_stats::TOTAL_FRAME_CPU_TIME);

        let current_time = zeitstempel::now();
        if device_size.is_some() {
            let time = render_stats::ns_to_ms(current_time - self.last_time);
            self.profile.set(render_stats::FRAME_TIME, time);
        }
        ...
        if self.max_recorded_profiles > 0 {
            ...
            let cpu_profile = CpuProfile::new(
                cpu_frame_id,
                (self.profile.get_or(render_stats::FRAME_BUILDING_TIME, 0.0) * 1000000.0) as u64,
                (self.profile.get_or(render_stats::RENDERER_TIME, 0.0) * 1000000.0) as u64,
                self.profile.get_or(render_stats::DRAW_CALLS, 0.0) as usize,
            );
        ...
        if thread_is_being_profiled() {
            let duration = Duration::new(0,0);
            let message = (self.profile.get_or(render_stats::DRAW_CALLS, 0.0) as usize).to_string();
            add_text_marker("NumDrawCalls", &message, duration);
        }

        let report = self.texture_resolver.report_memory();
        self.profile.set(render_stats::RENDER_TARGET_MEM, render_stats::bytes_to_mb(report.render_target_textures));
        self.profile.set(render_stats::PICTURE_TILES_MEM, render_stats::bytes_to_mb(report.picture_tile_textures));
        self.profile.set(render_stats::ATLAS_TEXTURES_MEM, render_stats::bytes_to_mb(report.atlas_textures));
        self.profile.set(render_stats::STANDALONE_TEXTURES_MEM, render_stats::bytes_to_mb(report.standalone_textures));

        self.profile.set(render_stats::DEPTH_TARGETS_MEM, render_stats::bytes_to_mb(self.device.depth_targets_memory()));

        self.profile.set(render_stats::TEXTURES_CREATED, self.device.textures_created);
        self.profile.set(render_stats::TEXTURES_DELETED, self.device.textures_deleted);

        results.stats.texture_upload_mb = self.profile.get_or(render_stats::TEXTURE_UPLOADS_MEM, 0.0);
        ...
        for i in 0..RenderReasons::NUM_BITS {
            let counter = render_stats::RENDER_REASON_FIRST + i as usize;
            let mut val = 0.0;
    ...
    fn update_gpu_profile(&mut self, device_size: DeviceIntSize) {
        ...
        if !samplers.is_empty() {
            ...
            self.profile.set(render_stats::ALPHA_TARGETS_SAMPLERS, alpha_targets);
            self.profile.set(render_stats::TRANSPARENT_PASS_SAMPLERS, transparent_pass);
            self.profile.set(render_stats::OPAQUE_PASS_SAMPLERS, opaque_pass);
            self.profile.set(render_stats::TOTAL_SAMPLERS, alpha_targets + transparent_pass + opaque_pass);
        }
    }
    ...
    fn update_texture_cache(&mut self) {
        ...
        self.profile.start_time(render_stats::TEXTURE_CACHE_UPDATE_TIME);
        ...
        if create_cache_texture_time > 0 {
            self.profile.set(
                render_stats::CREATE_CACHE_TEXTURE_TIME,
                render_stats::ns_to_ms(create_cache_texture_time)
            );
        }
        if delete_cache_texture_time > 0 {
            self.profile.set(
                render_stats::DELETE_CACHE_TEXTURE_TIME,
                render_stats::ns_to_ms(delete_cache_texture_time)
            )
        }

        let t = self.profile.end_time(render_stats::TEXTURE_CACHE_UPDATE_TIME);
    ...
    fn draw_instanced_batch<T: Clone>(
        ...
        for chunk in data.chunks(chunk_size) {
            ...
            self.profile.inc(render_stats::DRAW_CALLS);
            stats.total_draw_calls += 1;
        }

        self.profile.add(render_stats::VERTICES, 6 * data.len());
    }
    ...
    fn draw_picture_cache_target(
        ...
        self.profile.inc(render_stats::RENDERED_PICTURE_TILES);
    ...
    fn draw_render_target(
        ...
        let counter = match target.target_kind {
            RenderTargetKind::Color => render_stats::COLOR_PASSES,
            RenderTargetKind::Alpha => render_stats::ALPHA_PASSES,
        };
    ...
    fn draw_frame(
        ...
        self.profile.set(render_stats::GPU_BUFFER_MEM, gpu_buffer_mb);
        ...
        for (_pass_index, pass) in frame.passes.iter_mut().enumerate() {
            ...
            if !frame.has_been_rendered {
                ...
                if !pass.picture_cache.is_empty() {
                    self.profile.inc(render_stats::COLOR_PASSES);
                }
