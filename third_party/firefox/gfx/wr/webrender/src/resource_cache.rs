...
use crate::render_stats::{self, TransactionProfile, bytes_to_mb};
...
pub fn max_texture_size(&self) -> i32 {
            ...
            match update {
                ResourceUpdate::AddImage(img) => {
                    if let ImageData::Raw(ref bytes) = img.data {
                        ...
                        profile.set(render_stats::IMAGE_TEMPLATES_MEM, bytes_to_mb(self.image_templates_memory));
                    }
                    ...
                    profile.set(render_stats::IMAGE_TEMPLATES, self.resources.image_templates.images.len());
                }
                ...
                ResourceUpdate::DeleteFont(font) => {
                    if let Some(shared_key) = self.resources.fonts.font_keys.delete_key(&font) {
                        ...
                        profile.set(render_stats::FONT_TEMPLATES, self.resources.fonts.templates.len());
                        profile.set(render_stats::FONT_TEMPLATES_MEM, bytes_to_mb(self.font_templates_memory));
                    }
                }
    ...
    pub fn add_rasterized_blob_images(
        ...
        for (request, result) in images {
            ...
            profile.add(render_stats::RASTERIZED_BLOBS_PX, data.rasterized_rect.area());
