...
RefPtr<WebRenderAPI::CreatePromise> WebRenderAPI::Create(
  ...
  return InvokeAsync(
      ...
      [aBridge, aWidget, aWindowId, aSize, aWindowKind]() {
        ...
        auto* const gl = compositor->gl();
        RenderThread* const renderThread = RenderThread::Get();
        auto* const progCache = renderThread->GetProgramCache()
                                    ? renderThread->GetProgramCache()->Raw()
                                    : nullptr;
        auto* const shaders = renderThread->GetShaders()
        ...
        if (!wr_window_new(
                ...
                gfx::gfxVars::UseWebRenderScissoredCacheClears(), gl,
