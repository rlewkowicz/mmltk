...
bool VideoBridgeChild::DispatchAllocShmemInternal(size_t aSize,
  ...
  GetThread()->Dispatch(NS_NewRunnableFunction(
      "VideoBridgeChild::ProxyAllocShmemNow",
      [self = RefPtr<VideoBridgeChild>(this), &task, aSize, aShmem, aUnsafe,
       &success]() {
...
bool VideoBridgeChild::DeallocShmem(ipc::Shmem& aShmem) {
  ...
  GetThread()->Dispatch(NS_NewRunnableFunction(
      "VideoBridgeChild::ProxyDeallocShmemNow",
      [self = RefPtr<VideoBridgeChild>(this), &task, &aShmem, &result]() {
