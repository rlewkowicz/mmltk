...
bool ProcessWOFF2(ots::FontFile* header, ots::OTSStream* output,
  ...
  size_t limit =
      std::min(size_t(OTS_MAX_DECOMPRESSED_FILE_SIZE), output->size());
  std::string decompressed;
  woff2::WOFF2StringOut decompressor(&decompressed);
  decompressor.SetMaxSize(limit);
  if (!woff2::ConvertWOFF2ToTTF(data, length, &decompressor) ||
      decompressed.empty() || decompressed.size() > limit) {
    return false;
  }
  ...
  if (data[4] == 't' && data[5] == 't' && data[6] == 'c' && data[7] == 'f') {
    return ProcessTTC(header, output, bytes, decompressed.size(), index);
  }
  ots::Font font(header);
  return ProcessTTF(header, &font, output, bytes, decompressed.size(), 0);
}
...
#if defined(OTS_SYNTHESIZE_MISSING_GVAR)
