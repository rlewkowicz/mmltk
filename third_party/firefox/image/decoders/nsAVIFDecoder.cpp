...
class Dav1dDecoder final : AVIFDecoderInterface {
 public:
  ...
  DecodeResult Decode(const Mp4parseAvifInfo& aAVIFInfo,
    ...
    Dav1dResult r =
        GetPicture(*mColorContext, *aSamples.mColorImage, colorPic.get());
    if (r != 0) {
      return AsVariant(r);
    }
...
class AOMDecoder final : AVIFDecoderInterface {
 public:
  ...
  DecodeResult Decode(const Mp4parseAvifInfo& aAVIFInfo,
    ...
    DecodeResult r =
        GetImage(*mColorContext, *aSamples.mColorImage, &aomImg);
...
nsAVIFDecoder::DecodeResult nsAVIFDecoder::DoDecodeInternal(
  ...
  MOZ_ASSERT(mDecoder);
  r = mDecoder->Decode(parsedInfo, parsedImage);
  MOZ_LOG(sAVIFLog, LogLevel::Debug,
