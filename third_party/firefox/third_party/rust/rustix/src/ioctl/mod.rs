...
#[cfg(all(not(linux_raw), any(target_env = "gnu", target_env = "uclibc")))]
type _Opcode = c::c_ulong;

#[cfg(all(not(linux_raw), not(target_env = "gnu"), not(target_env = "uclibc")))]
type _Opcode = c::c_int;


#[cfg(any(bsd, target_os = "redox", target_os = "horizon", target_os = "hurd", target_os = "vita"))]
type _Opcode = c::c_ulong;
