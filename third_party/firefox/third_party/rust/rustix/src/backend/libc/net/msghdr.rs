...
#[cfg(all(not(any(target_os = "espidf", target_os = "wasi")), any(target_os = "redox", all(not(target_env = "musl"), not(all(target_env = "uclibc", any(target_arch = "arm", target_arch = "mips")))))))]
#[inline]
...
/// Convert the value to the `msg_iovlen` field of a `msghdr` struct.
#[cfg(all(not(any(target_os = "espidf", target_os = "vita", target_os = "wasi")), not(any(target_os = "redox", all(not(target_env = "musl"), not(all(target_env = "uclibc", any(target_arch = "arm", target_arch = "mips"))))))))]
#[inline]
