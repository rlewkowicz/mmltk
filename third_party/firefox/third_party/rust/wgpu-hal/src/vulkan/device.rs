...
impl super::Device {
    ...
    /// # Safety
    ///
    /// - The logical device must have `VK_KHR_external_memory_fd`,
...
pub unsafe fn texture_from_dmabuf_fd(
        ...
        if self.shared.extension_fns.external_memory_fd.is_none()
            || !self
                .shared
                .enabled_extensions
                .contains(&ext::external_memory_dma_buf::NAME)
            || !self
                .shared
                .enabled_extensions
        ...
        {
            log::error!(
                "Vulkan logical device is missing external DMA-BUF image extensions"
            );
