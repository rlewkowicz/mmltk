...
struct DeviceShared {
    ...
    queue_index: u32,
    raw_queue: vk::Queue,
    // Vulkan queue handles are externally synchronized. This gate is shared
    // with the shell's generic external-memory bridge, so every queue submit,
    // present, and idle operation has one owner.
    queue_operation_gate: Arc<std::sync::Mutex<()>>,
    instance: Arc<InstanceShared>,
...
impl ExternalTimelineSubmittedFrame {
    pub fn ready(&self) -> u64 {
        self.ready
    }

    pub fn copied_slot(&self) -> Option<usize> {
        self.copied_slot
    }
}
...
fn plan_external_timeline_edge(
    ...
    destination_attached: bool,
    observed_timeline: Option<u64>,
) -> Result<(u64, Option<(u64, Option<usize>)>), vk::Result> {
    if submitted_release & 1 != 0 {
        return Err(vk::Result::ERROR_UNKNOWN);
    }
    let (remaining_credits, unconsumed_edges) =
        reconcile_external_timeline_edges(preconsumed_edges, edges);
    if unconsumed_edges == 0 {
        return Ok((remaining_credits, None));
    }
    // One externally-owned image cannot publish a second frame until this
    // submission signals the first frame's even release.
    if unconsumed_edges != 1 {
        return Err(vk::Result::ERROR_UNKNOWN);
    }
    let ready = submitted_release
        .checked_add(1)
        .ok_or(vk::Result::ERROR_UNKNOWN)?;
    let copy = destination_attached
        .then_some(copy_slot)
        .flatten()
        .filter(|_| published_ready == Some(ready));
    let release_authorized = published_ready == Some(ready);
    if copy.is_none() && !release_authorized {
        let observed = observed_timeline.ok_or(vk::Result::ERROR_UNKNOWN)?;
        if observed <= submitted_release {
            return Ok((remaining_credits, None));
        }
        if observed != ready {
            return Err(vk::Result::ERROR_UNKNOWN);
    ...
    Ok((remaining_credits, Some((ready, copy))))
}

impl ExternalTimelineQueueSubmission {
    fn timeline_value(&self) -> Result<u64, vk::Result> {
        Ok(match self.device.extension_fns.timeline_semaphore {
            Some(ExtensionFn::Extension(ref extension)) => unsafe {
                extension.get_semaphore_counter_value(self.timeline)
            },
            Some(ExtensionFn::Promoted) => unsafe {
                self.device.raw.get_semaphore_counter_value(self.timeline)
            },
    ...
    fn wait_for_submitted_release(&self, timeout_ns: u64) -> Result<(), vk::Result> {
        ...
        let wait = vk::SemaphoreWaitInfo::default()
            ...
            .values(&values);
        match self.device.extension_fns.timeline_semaphore {
            Some(ExtensionFn::Extension(ref extension)) => unsafe {
                extension.wait_semaphores(&wait, timeout_ns)
            },
            Some(ExtensionFn::Promoted) => unsafe {
                self.device.raw.wait_semaphores(&wait, timeout_ns)
            },
            None => Err(vk::Result::ERROR_FEATURE_NOT_PRESENT),
        }
    }
...
mod external_timeline_submission_tests {
    ...
    impl FrameQueue {
        ...
        fn submit_edges(
            ...
            let (remaining_credits, submission) = plan_external_timeline_edge(
                self.submitted_release,
                self.preconsumed_edges,
                edges,
                published_ready,
                copy_slot,
                self.destination_attached,
                observed_timeline,
            )?;
            self.preconsumed_edges = remaining_credits;
            if let Some((ready, copy_slot)) = submission {
                self.submitted_release = ready + 1;
                if copy_slot.is_some() {
                    self.visible_ready = Some(ready);
                }
            }
            Ok(())
        }
    ...
    #[test]
    fn retirement_credit_consumes_only_its_later_edge() {
        assert_eq!(reconcile_external_timeline_edges(1, 1), (0, 0));
        assert_eq!(reconcile_external_timeline_edges(0, 1), (0, 1));
        assert_eq!(reconcile_external_timeline_edges(2, 1), (1, 0));
    }

    #[test]
    fn exact_r1_and_r2_identities_replace_the_visible_destination() {
        let mut queue = FrameQueue::attached();
        queue.submit_edges(1, Some(1), Some(0), None).unwrap();
        assert_eq!(queue.visible_ready, Some(1));
        queue.submit_edges(1, Some(3), Some(1), None).unwrap();
        assert_eq!(queue.visible_ready, Some(3));
        assert_eq!(queue.submitted_release, 4);
    }

    #[test]
    fn invalid_and_stale_edges_release_without_changing_r1() {
        let mut queue = FrameQueue::attached();
        queue.submit_edges(1, Some(1), Some(0), None).unwrap();

        // An ordinary page submission has no input to this state machine and
        // therefore cannot advance external progress between publication and
        // dispatcher admission.
        assert_eq!(queue.visible_ready, Some(1));
        assert_eq!(queue.submitted_release, 2);

        queue.submit_edges(1, Some(1), Some(1), Some(2)).unwrap();
        assert_eq!(queue.visible_ready, Some(1));
        assert_eq!(queue.submitted_release, 2);

        queue.submit_edges(1, None, None, Some(3)).unwrap();
        assert_eq!(queue.visible_ready, Some(1));
        assert_eq!(queue.submitted_release, 4);

        queue.submit_edges(1, Some(3), Some(1), Some(5)).unwrap();
        assert_eq!(queue.visible_ready, Some(1));
        assert_eq!(queue.submitted_release, 6);
    }

    #[test]
    fn detach_and_reconnect_cleanup_never_restore_visible_copy_authority() {
        let mut queue = FrameQueue::attached();
        queue.submit_edges(1, Some(1), Some(0), None).unwrap();
        queue.destination_attached = false;
        queue.submit_edges(1, Some(3), Some(1), None).unwrap();
        assert_eq!(queue.visible_ready, Some(1));

        // Detachment observed and released ready=5 before its callback edge
        // became readable.
        queue.submitted_release = 6;
        queue.preconsumed_edges = 1;
        queue.submit_edges(1, Some(5), Some(0), None).unwrap();
        assert_eq!(queue.visible_ready, Some(1));
        assert_eq!(queue.submitted_release, 6);

        queue.submit_edges(1, Some(7), Some(1), None).unwrap();
        assert_eq!(queue.visible_ready, Some(1));
        assert_eq!(queue.submitted_release, 8);

        let mut replacement = FrameQueue::attached();
        replacement.submit_edges(1, Some(1), Some(0), None).unwrap();
        assert_eq!(replacement.visible_ready, Some(1));
        assert_eq!(queue.visible_ready, Some(1));
    }
}
...
impl DeviceShared {
    fn begin_queue_operation(
        &self,
    ) -> Result<std::sync::MutexGuard<'_, ()>, vk::Result> {
        Ok(self
...
impl crate::Queue for Queue {
    ...
    unsafe fn present(
        ...
        // Configure and unconfigure already hold the swapchain lock while they
        // enter queue-gated release. Use the same order here so present cannot
        // invert those two ownership locks.
        let mut swapchain = surface.swapchain.write();
        let _queue_operation = self
            .device
            .begin_queue_operation()
            .map_err(|error| match error {
                vk::Result::ERROR_DEVICE_LOST => crate::SurfaceError::Lost,
                _ => crate::SurfaceError::Other("external queue submission failed"),
            })?;
