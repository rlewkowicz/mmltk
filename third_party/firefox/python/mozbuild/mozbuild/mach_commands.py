...
def prepend_debugger_args(*args, **kwargs):
    from mozdebug import prepend_debugger_args as implementation

    return implementation(*args, **kwargs)


def load_source(*args, **kwargs):
    from mozfile import load_source as implementation
...
_MMLTK_RUNTIME_OBJDIR_INPUTS = (
    "config.status",
    "config/autoconf.mk",
    "browser/installer/package-manifest",
    "browser/installer/locale-manifest.in",
)
_MMLTK_HASH_BUFFER_SIZE = 4 * 1024 * 1024


def _mmltk_hash_file(digest, path, buffer):
    path = Path(path)
    stat = path.lstat()
    digest.update(str(path).encode())
    digest.update(stat.st_mode.to_bytes(4, "little"))
...
def _mmltk_hash_tree(digest, root, buffer):
    root = Path(root)
    pending = [(root, "")]
    while pending:
        ...
        for entry in entries:
            ...
            if entry.is_dir(follow_symlinks=False):
                pending.append((Path(entry.path), entry_relative))
                continue
            digest.update(entry_relative.encode())
...
def _mmltk_runtime_fingerprint(command_context):
    ...
    buffer = memoryview(bytearray(_MMLTK_HASH_BUFFER_SIZE))
    _mmltk_hash_tree(digest, dist_bin, buffer)
    for relative in _MMLTK_RUNTIME_FINGERPRINT_INPUTS:
        path = Path(command_context.topsrcdir) / relative
        if path.is_file():
            _mmltk_hash_file(digest, path, buffer)
    for relative in _MMLTK_RUNTIME_OBJDIR_INPUTS:
        path = Path(command_context.topobjdir) / relative
        if path.is_file():
            _mmltk_hash_file(digest, path, buffer)
...
def _mmltk_runtime_complete(path):
    ...
    return all(
        ...
        for executable in ("firefox", "firefox-bin")
    )


def _mmltk_stage_event(event, **fields):
    output = os.environ.get("MMLTK_FIREFOX_STAGE_LOG_FILE")
    if not output:
        return
    record = {
        "timestamp_ns": time.time_ns(),
        "pid": os.getpid(),
        "phase": "firefox_runtime_stage",
        "event": event,
        **fields,
    }
...
def _mmltk_exchange_paths(left, right):
    import ctypes
...
def _mmltk_replace_runtime(next_runtime, runtime):
    next_runtime = Path(next_runtime)
    runtime = Path(runtime)
    if not runtime.exists():
        os.replace(next_runtime, runtime)
        return
    _mmltk_exchange_paths(runtime, next_runtime)
    shutil.rmtree(next_runtime, ignore_errors=True)
...
def mmltk_stage_runtime(command_context):
    ...
    runtime = dist / "firefox"
    next_runtime = dist / "firefox.next"
    ...
    try:
        ...
        _mmltk_stage_event("cache_miss", fingerprint=fingerprint)
        _mmltk_stage_event("start", fingerprint=fingerprint)
        shutil.rmtree(next_runtime, ignore_errors=True)
        result = command_context._run_make(
            directory=".",
            target="stage-package",
            append_env={"MOZ_PKG_DIR": next_runtime.name},
        ...
        fingerprint_next = fingerprint_path.with_name(
            f".{fingerprint_path.name}.next"
        )
        fingerprint_next.write_text(f"{fingerprint}\n", encoding="utf-8")
        os.replace(fingerprint_next, fingerprint_path)
        _mmltk_stage_event("finish", fingerprint=fingerprint, result="success")
        return 0
    except Exception as error:
        shutil.rmtree(next_runtime, ignore_errors=True)
        _mmltk_stage_event(
            "failure", fingerprint=fingerprint, error=str(error), result="failure"
        )
        command_context.log(
            logging.ERROR,
            "mmltk-stage-runtime",
            {"error": str(error)},
            "Failed to stage owned Firefox runtime: {error}",
        )
        return 1
