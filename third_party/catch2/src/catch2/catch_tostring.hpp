...
template <typename T>
static constexpr bool IsStreamInsertable_v<T, decltype(void(std::declval<std::ostream&>() << std::declval<T>()))> =
    true;
...
struct StringMaker {
    ...
    static std::enable_if_t<::Catch::Detail::IsStreamInsertable_v<Fake>, std::string> convert(const Fake& value) {
...
void PrintTuple(const Tuple& tuple, std::ostream& os, std::index_sequence<Is...>) {
    char a[1 + sizeof...(Is)] = {
        ((os << (Is ? ", " : " ") << ::Catch::Detail::stringify(std::get<Is>(tuple))), '\0')...};
    (void)a;
}
...
struct StringMaker<std::tuple<Types...>> {
    static std::string convert(const std::tuple<Types...>& tuple) {
        ReusableStringStream rss;
        rss << '{';
        Detail::PrintTuple(tuple, rss.get(), std::make_index_sequence<sizeof...(Types)>{});
        rss << " }";
        return rss.str();
    }
};
...
struct StringMaker<std::chrono::time_point<std::chrono::system_clock, Duration>> {
    static std::string convert(std::chrono::time_point<std::chrono::system_clock, Duration> const& time_point) {
        const auto systemish = std::chrono::time_point_cast<std::chrono::system_clock::duration>(time_point);
