...
void JsonReporter::testCasePartialStarting(TestCaseInfo const&, uint64_t index) {
    startObject();
    m_objectWriters.top().write("run-idx"_sr).write(index);
    startArray("path"_sr);
}

void JsonReporter::testCasePartialEnded(TestCaseStats const& tcStats, uint64_t) {
    endArray();
...
void JsonReporter::sectionEnded(SectionStats const&) {
    endArray();
