...
#define INTERNAL_CATCH_MAKE_TYPE_LIST2(...) \
    decltype(get_wrapper<INTERNAL_CATCH_REMOVE_PARENS_GEN(__VA_ARGS__)>(Catch::Detail::priority_tag<1>{}))
...
#define INTERNAL_CATCH_TYPES_REGISTER(TestFunc)                                                               \
    ...
    void reg_test(TypeList<Ts...>, Catch::NameAndTags nameAndTags) {                                          \
        Catch::AutoReg(Catch::makeTestInvoker(&TestFunc<Ts...>), CATCH_INTERNAL_LINEINFO, Catch::StringRef(), \
                       nameAndTags);                                                                          \
    }
