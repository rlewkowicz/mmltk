...
enum class ParseResultType : std::uint8_t {
    Matched,
    NoMatch,
    ShortCircuitAll,
    ShortCircuitSame
...
template <typename F>
static constexpr bool is_unary_function_v<F, Catch::Detail::void_t<decltype(std::declval<F>()(fake_arg()))>> = true;
...
class ResultValueBase : public ResultBase {
   ...
   protected:
    ...
    ResultValueBase(ResultValueBase&& other) noexcept(std::is_nothrow_move_constructible<T>::value)
        : ResultBase(CATCH_MOVE(other)) {
        if (m_type == ResultType::Ok)
    ...
    ResultValueBase& operator=(ResultValueBase const& other) {
        if (m_type == ResultType::Ok)
            m_value.~T();
        ResultBase::operator=(CATCH_MOVE(other));
        if (m_type == ResultType::Ok)
            new (&m_value) T(other.m_value);
        return *this;
    }
    ResultValueBase& operator=(ResultValueBase&& other) noexcept(std::is_nothrow_move_constructible<T>::value) {
        if (m_type == ResultType::Ok)
            m_value.~T();
    ...
    ~ResultValueBase() override {  // NOLINT(portability-template-virtual-member-function)
        if (m_type == ResultType::Ok)
            m_value.~T();
    }
...
class BasicResult : public ResultValueBase<T> {
   ...
   protected:
    void enforceOk() const override {  // NOLINT(portability-template-virtual-member-function)
...
enum class Optionality : std::uint8_t {
    Optional,
    Required
};
...
class ComposableParserImpl : public ParserBase {  // NOLINT(bugprone-crtp-constructor-accessibility)
   public:
    template <typename T>
...
template <typename DerivedT>
class ParserRefImpl : public ComposableParserImpl<DerivedT> {  // NOLINT(bugprone-crtp-constructor-accessibility)
   protected:
    ...
    explicit ParserRefImpl(std::shared_ptr<BoundRef> const& ref)  // NOLINT(bugprone-crtp-constructor-accessibility)
        : m_ref(ref) {}
