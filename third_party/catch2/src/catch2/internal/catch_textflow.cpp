...
// Shared across Column and Columns iterators/streaming.
template <typename Iterator>
Iterator postIncrementImpl(Iterator& it) {
    Iterator prev(it);
    ++it;
    return prev;
}
...
std::ostream& printLines(std::ostream& os, Lines const& lines) {
    bool first = true;
    for (auto line : lines) {
        if (first) {
            first = false;
...
Column::const_iterator Column::const_iterator::operator++(int) {
    return postIncrementImpl(*this);
}

std::ostream& operator<<(std::ostream& os, Column const& col) {
    return printLines(os, col);
}
