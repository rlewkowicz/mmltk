...
class float4 {
   public:
    ...
    float4(const float4& s) : value(s.value) {}

    float4(const __m128 s) : value(s) {}

    float4(const int s) : value(_mm_set1_ps((float)s)) {}

    float4(const float s) : value(_mm_set1_ps(s)) {}

    float4(const double s) : value(_mm_set1_ps((float)s)) {}
