    ...
    Theme: Catalog,
{
    ...
    fn update(
        ...
        // The button owns primary activation. Its content may still be
        // selectable or otherwise event-aware, but it cannot consume the
        // release that completes an activation already admitted here.
        let activation_captured = match event {
        ...
        if !activation_captured && shell.is_event_captured() {
            if matches!(
                event,
                Event::Mouse(
