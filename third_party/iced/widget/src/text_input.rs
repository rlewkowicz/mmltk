        ...
        match &event {
            ...
            | Event::Touch(touch::Event::FingerPressed { .. }) => {
                ...
                if let Some(cursor_position) = click_position {
                    if !was_focused && let Some(on_focus) = self.on_focus.clone() {
            ...
            Event::Mouse(mouse::Event::ButtonPressed(mouse::Button::Right)) => {
                ...
                let target = cursor_position.x - text_bounds.x - alignment_offset;
                let value = if self.is_secure {
                    self.value.secure()
                } else {
                    self.value.clone()
                ...
                let position = (target > 0.0)
                    .then(|| find_cursor_position(text_bounds, &value, state, target))
                    .flatten()
                    .unwrap_or(0);
                        ...
                        Some('c') if state.keyboard_modifiers.command() && !self.is_secure => {
                            if let Some((start, end)) = state.cursor.selection(&self.value) {
                                shell.write_clipboard_for(
                                    ...
                                    clipboard_target(self.id.as_ref(), "text_input"),
                                );
                            }
            ...
            Event::Clipboard(clipboard::Event::Read {
                result: Ok(content),
                ..
            }) => {
                let Some(on_input) = &self.on_input else {
            ...
            Event::Clipboard(clipboard::Event::Read { result: Err(_), .. }) => {
                let state = state::<Renderer>(tree);
                if state.is_focused.is_some() && matches!(state.is_pasting, Some(Paste::Reading)) {
                    state.is_pasting = None;
                    shell.capture_event();
                }
            }
...
fn clipboard_target(id: Option<&widget::Id>, fallback: &'static str) -> String {
    id.map_or_else(|| fallback.to_owned(), ToString::to_string)
}
...
impl<P: text::Paragraph> operation::TextInput for State<P> {
    ...
    fn selected_text(&self) -> Option<String> {
        let value = Value::new(self.value.content());
        let (start, end) = self.cursor.selection(&value)?;
        Some(value.select(start, end).to_string())
    }

    fn paste_text(&mut self, content: &str) -> String {
        let mut value = Value::new(self.value.content());
        let mut editor = Editor::new(&mut value, &mut self.cursor);
        editor.paste(Value::new(content));
        editor.contents()
    }

    fn cut_text(&mut self) -> Option<operation::text_input::TextCut> {
        let mut value = Value::new(self.value.content());
        let (start, end) = self.cursor.selection(&value)?;
        let selection = value.select(start, end).to_string();
        let mut editor = Editor::new(&mut value, &mut self.cursor);
        editor.delete();
        Some(operation::text_input::TextCut {
            selection,
