...
impl core::text::Paragraph for Paragraph {
    ...
    fn hit_test_strict(&self, point: Point) -> Option<Hit> {
        ...
        let x = point.x * internal.hint_factor;
        let y = point.y * internal.hint_factor;
        let directly_over_glyph = internal.buffer.layout_runs().any(|run| {
            run.line_top <= y
                && y < run.line_top + run.line_height
                && run
                    .glyphs
                    .iter()
                    .any(|glyph| glyph.x <= x && x <= glyph.x + glyph.w)
        });
        if !directly_over_glyph {
            return None;
        }
        let cursor = internal.buffer.hit(x, y)?;
        Some(Hit::CharOffset(cursor.index))
    }
    ...
    fn selection_bounds(
        ...
        for run in internal
            .buffer
            .layout_runs()
            .filter(|run| start_line <= run.line_i && run.line_i <= end_line)
        {
            let mut range: Option<(f32, f32)> = None;
            let flush = |range: &mut Option<(f32, f32)>, bounds: &mut Vec<Rectangle>| {
                if let Some((left, right)) = range.take()
                    && right > left
                {
                    bounds.push(
                        Rectangle::new(
            ...
            for glyph in run.glyphs {
                ...
                for (offset, grapheme) in cluster.grapheme_indices(true) {
                    ...
                    let selected = (run.line_i != start_line || grapheme_end > start_index)
                        && (run.line_i != end_line || grapheme_start < end_index);
                    if selected {
                        range = Some(match range {
                            Some((left, right)) => (left.min(x), right.max(x + grapheme_width)),
                            None => (x, x + grapheme_width),
                        });
                    } else {
                        flush(&mut range, bounds);
                    }
                    x += grapheme_width;
                }
            }
