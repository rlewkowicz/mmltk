...
#[wasm_bindgen]
extern "C" {
    #[wasm_bindgen(js_namespace = window, js_name = mmltkRecoverWebGpu)]
    fn request_webgpu_recovery(reason: &str);
}
            ...
            Event::EventLoopAwakened(event) => {
                match event {
                    ...
                    event::Event::WindowEvent {
                        ...
                        #[cfg(target_arch = "wasm32")]
                        if let Some(new_size) = window.pending_resize.take() {
                            let resized = winit::event::WindowEvent::Resized(new_size);

                            window.state.update(&program, &window.raw, &resized);
                        ...
                        match current_compositor.present(
                            ...
                            Err(error) => match error {
                                compositor::SurfaceError::OutOfMemory => {
                                    ...
                                    #[cfg(target_arch = "wasm32")]
                                    request_webgpu_recovery("Iced WebGPU surface out of memory");
                    ...
                    event::Event::WindowEvent {
                        ...
                        if let winit::event::WindowEvent::Resized(new_size) = &window_event {
                            window.pending_resize = Some(*new_size);
                            continue;
                        }
...
fn run_clipboard<Message: Send>(
    ...
    for request in requests.reads {
        let proxy = proxy.clone();
        let target = request.target;

        clipboard.read(request.kind, move |result| {
            proxy.send_action(Action::Event {
                window,
                event: core::Event::Clipboard(core::clipboard::Event::Read {
                    target,
    ...
    if let Some(request) = requests.write {
        let proxy = proxy.clone();
        let byte_count = clipboard_content_bytes(&request.content);
        let target = request.target;

        clipboard.write(request.content, move |result| {
            proxy.send_action(Action::Event {
                window,
                event: core::Event::Clipboard(core::clipboard::Event::Written {
                    target,
                    byte_count,
                    result,
                }),
...
fn clipboard_content_bytes(content: &core::clipboard::Content) -> usize {
    match content {
        core::clipboard::Content::Text(text) | core::clipboard::Content::Html(text) => text.len(),
        core::clipboard::Content::Files(_) => 0,
        #[cfg(feature = "image")]
        core::clipboard::Content::Image(image) => image.rgba.len(),
        _ => 0,
    }
}
