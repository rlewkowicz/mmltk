...
class HuffmanCodingSymbolsPerLength
   ...
   protected:
    template <typename InitializeTable>
    [[nodiscard]] constexpr Error initializeFromLengthsWithTable(const VectorView<BitCount>& codeLengths,
                                                                 InitializeTable&& initializeTable) {
        if (const auto errorCode = BaseType::initializeMinMaxCodeLengths(codeLengths); errorCode != Error::NONE) {
            return errorCode;
        }
        ...
        initializeTable(codeLengths, bitLengthFrequencies);
        return Error::NONE;
    }

    constexpr void initializeSymbolsPerLength(const VectorView<BitCount>& codeLengths,
   ...
   public:
    [[nodiscard]] constexpr Error initializeFromLengths(const VectorView<BitCount>& codeLengths) {
        return initializeFromLengthsWithTable(codeLengths, [this](const auto& lengths, const auto& frequencies) {
            initializeSymbolsPerLength(lengths, frequencies);
        });
    }

    template <typename BitReader>
    [[nodiscard]] forceinline constexpr std::optional<Symbol> decode(BitReader& bitReader) const {
        HuffmanCode code = 0;
