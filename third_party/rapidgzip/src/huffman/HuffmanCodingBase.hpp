...
namespace rapidgzip {
template <typename T_HuffmanCode, uint8_t T_MAX_CODE_LENGTH, typename T_Symbol, size_t T_MAX_SYMBOL_COUNT,
...
class HuffmanCodingBase {
   ...
   protected:
    constexpr Error initializeMinMaxCodeLengths(const VectorView<BitCount>& codeLengths) {
    ...
    constexpr Error checkCodeLengthFrequencies(const CodeLengthFrequencies& bitLengthFrequencies,
    ...
    constexpr void initializeMinimumCodeValues(CodeLengthFrequencies& bitLengthFrequencies) {
   ...
   public:
    constexpr Error initializeFromLengths(const VectorView<BitCount>& codeLengths) {
        if (const auto errorCode = initializeMinMaxCodeLengths(codeLengths); errorCode != Error::NONE) {
            return errorCode;
        }
    ...
    [[nodiscard]] constexpr auto const& minimumCodeValuesPerLevel() const noexcept {
...
[[nodiscard]] constexpr bool checkHuffmanCodeLengths(const CodeLengths& codeLengths) {
