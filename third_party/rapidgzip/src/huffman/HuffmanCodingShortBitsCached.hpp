...
template <typename HuffmanCode, uint8_t MAX_CODE_LENGTH, typename Symbol, size_t MAX_SYMBOL_COUNT,
          uint8_t LUT_BITS_COUNT, bool REVERSE_BITS, bool CHECK_OPTIMALITY = true>
class HuffmanCodingShortBitsCached
   ...
   public:
    using BaseType =
        HuffmanCodingSymbolsPerLength<HuffmanCode, MAX_CODE_LENGTH, Symbol, MAX_SYMBOL_COUNT, CHECK_OPTIMALITY>;
   ...
   public:
    [[nodiscard]] constexpr Error initializeFromLengths(const VectorView<BitCount>& codeLengths) {
        ...
        for (size_t symbol = 0; symbol < codeLengths.size(); ++symbol) {
            ...
            if constexpr (REVERSE_BITS) {
                ...
                const auto maximumPaddedCode =
                    static_cast<HuffmanCode>((code << fillerBitCount) | nLowestBitsSet<HuffmanCode>(fillerBitCount));
                assert(maximumPaddedCode < m_codeCache.size());
    ...
    [[nodiscard]] forceinline std::optional<Symbol> decode(BitReader& bitReader) const {
        try {
   ...
   private:
    template <typename BitReader>
    [[nodiscard]] forceinline constexpr std::optional<Symbol> decodeLong(BitReader& bitReader) const {
        HuffmanCode code = 0;
