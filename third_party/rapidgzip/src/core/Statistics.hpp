...
struct Statistics {
    ...
    [[nodiscard]] std::string formatAverageWithUncertainty(bool includeBounds = false,
        ...
        const auto roundToUncertainty = [magnitude](auto value) {
            return std::round(static_cast<double>(value) / std::pow(10, magnitude)) * std::pow(10, magnitude);
        };
...
class Histogram {
   public:
    template <typename Container>
    explicit Histogram(const Container& container, uint16_t binCount, std::string unit = {})
    ...
    Histogram(T min, T max, uint16_t binCount, std::string unit = {})
        : m_statistics(std::array<T, 2>{min, max}), m_bins(binCount, 0), m_unit(std::move(unit)) {
        if constexpr (std::is_floating_point_v<T>) {
            if (!std::isfinite(m_statistics.min) || !std::isfinite(m_statistics.max)) {
                return;
            }
    ...
    constexpr bool merge(T value) {
        ...
        const auto unitValue =
            static_cast<double>(value - m_statistics.min) / static_cast<double>(m_statistics.max - m_statistics.min);
        const auto index =
            value == m_statistics.max ? m_bins.size() - 1U : static_cast<size_t>(std::floor(unitValue * m_bins.size()));
    ...
    [[nodiscard]] constexpr const auto& bins() const noexcept {
        return m_bins;
    }
    ...
    [[nodiscard]] std::string plot() const {
        ...
        const auto maxLabelLength =
            std::max_element(binLabels.begin(), binLabels.end(), [](const auto& a, const auto& b) {
        ...
        for (size_t i = 0; i < m_bins.size(); ++i) {
            ...
            const auto binVisualSize =
                *maxBin == 0 ? size_t(0)
                             : static_cast<size_t>(static_cast<double>(bin) / static_cast<double>(*maxBin) *
