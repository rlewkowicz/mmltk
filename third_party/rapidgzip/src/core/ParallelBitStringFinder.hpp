...
class ParallelBitStringFinder : public BitStringFinder<bitStringSize> {
   ...
   public:
    ParallelBitStringFinder(UniqueFileReader fileReader, uint64_t bitStringToFind,
                            size_t parallelization = std::max(1U, availableCores() / 8U), size_t requestedBytes = 0,
                            size_t fileBufferSizeBytes = 1_Mi)
        : BaseType(std::move(fileReader), bitStringToFind,
                   chunkSize(fileBufferSizeBytes, requestedBytes, parallelization)),
          m_threadPool(parallelization) {}

    ParallelBitStringFinder(const char* buffer, size_t size, uint64_t bitStringToFind, size_t parallelization)
        : BaseType(UniqueFileReader(), bitStringToFind), m_threadPool(parallelization) {
   ...
   private:
    [[nodiscard]] static constexpr size_t chunkSize(size_t const fileBufferSizeBytes, size_t const requestedBytes,
                                                    size_t const parallelization) {
        const auto result =
            std::max(fileBufferSizeBytes, static_cast<size_t>(ceilDiv(bitStringSize, 8)) * parallelization);
        return std::max(result, requestedBytes);
    }

    static void workerMain(char const* const buffer, size_t const bufferSizeInBytes, uint8_t const firstBitsToIgnore,
                           uint64_t const bitStringToFind, size_t const bitOffsetToAdd, ThreadResults* const result) {
...
size_t ParallelBitStringFinder<bitStringSize>::find() {
    ...
    while (!BaseType::eof() || !m_threadResults.empty()) {
        while (!m_threadResults.empty()) {
            ...
            while (!result.foundOffsets.empty() || result.future.valid()) {
                ...
                result.changed.wait(lock, [&result]() {
                    return !result.foundOffsets.empty() || (result.future.wait_for(0s) == std::future_status::ready);
                });
        ...
        for (; !this->bufferEof(); this->m_bufferBitsRead += subChunkStrideInBytes * CHAR_BIT) {
            ...
            auto const subChunkSizeInBits =
                this->m_bufferBitsRead - bufferOffsetInBits + subChunkStrideInBytes * CHAR_BIT;
            auto const subChunkSizeInBytes =
                std::min(ceilDiv(subChunkSizeInBits, CHAR_BIT), this->m_buffer.size() - bufferOffsetInBytes);

            auto& result = m_threadResults.emplace_back();
            result.future = m_threadPool.submit([=, &result]() {
                workerMain(this->m_buffer.data() + bufferOffsetInBytes, subChunkSizeInBytes, subChunkOffsetInBits,
                           this->m_bitStringToFind, (this->m_nTotalBytesRead + bufferOffsetInBytes) * CHAR_BIT,
                           &result);
            });
        }
    }
