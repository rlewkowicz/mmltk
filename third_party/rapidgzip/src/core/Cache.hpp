...
class CacheStrategy {
   public:
    ...
    [[nodiscard]] virtual std::optional<Index> nextNthEviction(size_t countToEmplaceHypothetically) const = 0;

    virtual std::optional<Index> evict(std::optional<Index> indexToEvict = {}) = 0;
};
...
class LeastRecentlyUsed : public CacheStrategy<Index> {
   ...
   public:
    ...
    [[nodiscard]] std::optional<Index> nextNthEviction(size_t countToEmplaceHypothetically) const override {
...
template <typename Key, typename Value, typename CacheStrategy = CacheStrategy::LeastRecentlyUsed<Key> >
class Cache {
   public:
    struct Statistics {
        size_t hits{0};
   ...
   public:
    ...
    [[nodiscard]] std::optional<Key> nextEviction(const std::optional<Key>& key = std::nullopt) const {
    ...
    [[nodiscard]] std::optional<Key> nextNthEviction(size_t countToBeInserted) const {
        const auto freeCapacity = capacity() - m_cache.size();
        return countToBeInserted <= freeCapacity ? std::nullopt
    ...
    [[nodiscard]] const auto& contents() const noexcept {
        return m_cache;
    }
