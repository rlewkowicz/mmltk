...
namespace rapidgzip {
template <typename T_BlockFinder, typename T_BlockData, typename FetchingStrategy>
class BlockFetcher {
   public:
       ...
       public:
        ...
        [[nodiscard]] std::string print() const {
            ...
            const auto decodeDuration =
                decodeBlockStartTime && decodeBlockEndTime ? duration(*decodeBlockStartTime, *decodeBlockEndTime) : 0.0;
            ...
            out << "\n    Parallelization                   : " << parallelization << "\n    Cache"
                ...
                << "\n    Time spent in:" << "\n        decodeBlock                   : " << decodeBlockTotalTime
   ...
   public:
    ...
    [[nodiscard]] bool statisticsEnabled() const {
    ...
    [[nodiscard]] bool test(const size_t blockOffset) const {
        return (m_prefetching.find(blockOffset) != m_prefetching.end()) || m_cache.test(blockOffset) ||
               m_prefetchCache.test(blockOffset);
    }

    [[nodiscard]] std::shared_ptr<BlockData> get(const size_t blockOffset,
   ...
   private:
    void insertIntoCache(const size_t blockOffset, std::shared_ptr<BlockData> blockData) {
    ...
    [[nodiscard]] std::pair<std::optional<std::shared_ptr<BlockData> >, std::future<BlockData> > getFromCaches(
    ...
    [[nodiscard]] std::future<BlockData> takeFromPrefetchQueue(size_t blockOffset) {
    ...
    void prefetchNewBlocks(const GetPartitionOffset& getPartitionOffsetFromOffset,
        ...
        const auto threadPoolSaturated = [&]() { return m_prefetching.size() + 1 >= m_threadPool.capacity(); };
        ...
        for (auto blockIndexToPrefetch : blockIndexesToPrefetch) {
            ...
            if (!prefetchBlockOffset.has_value() || (prefetchGetReturnCode == GetReturnCode::FAILURE) ||
                !nextPrefetchBlockOffset.has_value() || isInCacheOrQueue(*prefetchBlockOffset) ||
    ...
    [[nodiscard]] std::future<BlockData> submitOnDemandTask(const size_t blockOffset,
        ...
        auto resultFuture = m_threadPool.submit(
            [this, blockOffset, nextBlockOffset]() {
                return decodeAndMeasureBlock(blockOffset,
                                             nextBlockOffset ? *nextBlockOffset : std::numeric_limits<size_t>::max());
            },
            0);
        assert(resultFuture.valid());
        return resultFuture;
   ...
   protected:
    [[nodiscard]] virtual BlockData decodeBlock(size_t blockOffset, size_t nextBlockOffset) const = 0;
    ...
    std::future<decltype(std::declval<T_Functor>()())> submitTaskWithHighPriority(T_Functor task) {
        return m_threadPool.submit(std::move(task), -1);
    }

    [[nodiscard]] const auto& cache() const noexcept {
        return m_cache;
    }
   ...
   private:
    [[nodiscard]] BlockData decodeAndMeasureBlock(size_t blockOffset, size_t nextBlockOffset) const {
        ...
        if (m_statisticsEnabled) {
            ...
            m_statistics.decodeBlockStartTime.emplace(minStartTime ? std::min(*minStartTime, tDecodeStart)
                                                                   : tDecodeStart);

            const auto& maxEndTime = m_statistics.decodeBlockEndTime;
            m_statistics.decodeBlockEndTime.emplace(maxEndTime ? std::max(*maxEndTime, tDecodeEnd) : tDecodeEnd);
