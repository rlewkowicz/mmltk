...
class FetchingStrategy {
   public:
    ...
    virtual void fetch(size_t index) {
        m_lastFetched = index;
    }
    ...
    [[nodiscard]] virtual bool isSequential() const noexcept {
        return false;
    }

    [[nodiscard]] virtual std::vector<size_t> prefetch(size_t maxAmountToPrefetch) const = 0;
...
class FetchNextFixed : public FetchingStrategy {
   public:
    [[nodiscard]] std::vector<size_t> prefetch(size_t maxAmountToPrefetch) const override {
        if (!lastFetched()) {
            return {};
        }
...
class FetchNextAdaptive : public FetchingStrategy {
   public:
    explicit FetchNextAdaptive(size_t memorySize = 3) : m_memorySize(memorySize) {}
    ...
    [[nodiscard]] static std::vector<size_t> extrapolateForward(const size_t highestValue,
    ...
    [[nodiscard]] static std::vector<size_t> extrapolate(const Iterator rangeBegin, const Iterator rangeEnd,
...
class FetchMultiStream : public FetchNextAdaptive {
   public:
    explicit FetchMultiStream(size_t memorySize = 3, size_t maxStreamCount = 16U)
        : FetchNextAdaptive(maxStreamCount * memorySize), m_memorySizePerStream(memorySize) {}
    ...
    [[nodiscard]] std::vector<size_t> prefetch(size_t maxAmountToPrefetch) const override {
        ...
        const auto extrapolateSubsequence = [&, this](const auto begin, const auto end) {
            ...
            for (auto currentHighestValue = std::reverse_iterator(end);
                 currentHighestValue != std::reverse_iterator(begin); ++currentHighestValue) {
            ...
            const auto saturationCount =
                !memoryFull() && (consecutiveValues > 0) ? consecutiveValues : m_memorySizePerStream;
        ...
        const auto newEnd = std::remove_if(result.begin(), result.end(), [this](auto value) {
            return std::find(m_previousIndexes.begin(), m_previousIndexes.end(), value) != m_previousIndexes.end();
        });
        result.resize(std::min(static_cast<size_t>(std::distance(result.begin(), newEnd)), maxAmountToPrefetch));
        return result;
    }
