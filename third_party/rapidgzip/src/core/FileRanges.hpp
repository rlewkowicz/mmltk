...
struct FileRange {
    ...
    [[nodiscard]] bool operator==(const FileRange& other) const noexcept {
        return (offset == other.offset) && (size == other.size) && (offsetIsLine == other.offsetIsLine) &&
...
[[nodiscard]] inline const char* skipWhitespaces(const char* const first, const char* const last) {
...
[[nodiscard]] inline const char* readNumber(const char* const first, const char* const last, size_t& value,
                                            bool& valueIsLine) {
    ...
    for (const auto suffix : {std::string_view("B"), std::string_view("L"), std::string_view("")}) {
        for (const auto& [prefix, factor] : PREFIXES) {
            if ((prefix.size() + suffix.size() > longestMatch) && startsWith(unitString, prefix) &&
                startsWith(std::string_view(unitString.data() + prefix.size(), unitString.size() - prefix.size()),
                           suffix)) {
...
[[nodiscard]] inline std::vector<FileRange> parseFileRanges(const std::string_view expression) {
    ...
    for (; current != expressionEnd; current = skipWhitespaces(current, expressionEnd)) {
        switch (state) {
            case State::TUPLE_END:
                ...
                range.size = 0;
                if (startsWith(std::string_view(current, static_cast<size_t>(std::distance(current, expressionEnd))),
                               INFINITY_STRING)) {
    ...
    if ((state != State::TUPLE_END) && (state != State::OFFSET_END)) {
        throw std::invalid_argument("Incomplete size@offset tuple at end of expression: " + std::string(expression));
    }
