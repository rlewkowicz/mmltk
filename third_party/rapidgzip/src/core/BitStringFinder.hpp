...
class BitStringFinder {
   ...
   public:
    ...
    BitStringFinder(UniqueFileReader fileReader, uint64_t bitStringToFind, size_t fileBufferSizeBytes = 1_Mi)
        : m_bitStringToFind(bitStringToFind & nLowestBitsSet<uint64_t>(bitStringSize)),
          ...
          m_fileChunksInBytes(std::max(fileBufferSizeBytes, static_cast<size_t>(ceilDiv(bitStringSize, CHAR_BIT)))) {
   ...
   public:
    [[nodiscard]] static std::vector<size_t> findBitStrings(std::string_view const& buffer, uint64_t bitString);
...
template <uint8_t bitStringSize>
std::vector<size_t> BitStringFinder<bitStringSize>::findBitStrings(std::string_view const& buffer, uint64_t bitString) {
    ...
    const auto findStrings = [](std::string_view const& data, std::string_view const& stringToFind) {
        std::vector<size_t> blockOffsets;
        for (auto position = data.find(stringToFind, 0); position != std::string_view::npos;
    ...
    const auto msbToString = [](uint64_t const bitStringToConvert, uint8_t const bitStringtoConvertSize) {
...
template <uint8_t bitStringSize>
size_t BitStringFinder<bitStringSize>::find() {
    if (bitStringSize == 0) {
    ...
    while (!eof()) {
        ...
        m_offsetsInBuffer.erase(std::remove_if(m_offsetsInBuffer.begin(), m_offsetsInBuffer.end(),
