...
class BlockMap {
   public:
    struct BlockInfo {
       public:
        ...
        friend std::ostream& operator<<(std::ostream& out, const BlockInfo& blockInfo) {
   ...
   public:
    ...
    size_t push(size_t encodedBlockOffset, size_t encodedSize, size_t decodedSize) {
        ...
        const auto match = std::lower_bound(m_blockToDataOffsets.begin(), m_blockToDataOffsets.end(),
    ...
    [[nodiscard]] std::optional<BlockInfo> getEncodedOffset(size_t encodedOffsetInBits) const {
        const std::scoped_lock lock(m_mutex);
   ...
   private:
    [[nodiscard]] BlockInfo get(const typename BlockOffsets::const_reverse_iterator& blockOffset) const {
        BlockInfo result;
        if (blockOffset == m_blockToDataOffsets.rend()) {
            return result;
        }
