...
template <typename S, typename T>
[[nodiscard]] constexpr bool startsWith(const S& fullString, const T& prefix, bool caseSensitive = true) noexcept {
...
template <typename S, typename T>
[[nodiscard]] constexpr bool endsWith(const S& fullString, const T& suffix, bool caseSensitive = true) noexcept {
...
[[nodiscard]] inline std::vector<std::string_view> split(const std::string_view toSplit, const char separator) {
    ...
    for (auto it = toSplit.begin(); it != toSplit.end(); ++it) {  // NOLINT(readability-qualified-auto)
        if (*it == separator) {
            result.emplace_back(toSplit.data() + std::distance(toSplit.begin(), start), std::distance(start, it));
            start = it;
            ++start;
        }
    }
...
[[nodiscard]] inline std::chrono::time_point<std::chrono::high_resolution_clock> now() noexcept {
...
class ThreadSafeOutput {
   public:
    ThreadSafeOutput() {
        ...
        m_out << "[" << std::put_time(std::localtime(&timePoint), "%H:%M:%S") << "." << subseconds << "]" << "[0x"
              << std::hex << std::this_thread::get_id() << std::dec << "]";
    }

    template <typename T>
    ThreadSafeOutput& operator<<(const T& value) {
        m_out << " " << value;
        return *this;
    }
...
inline std::ostream& operator<<(std::ostream& out, const ThreadSafeOutput& output) {
    out << output.str();
    return out;
}
...
class FinallyNotify {
   public:
    explicit FinallyNotify(std::condition_variable& toNotify) : m_toNotify(toNotify) {}
...
class Finally {
   public:
    explicit Finally(std::function<void()> cleanup) : m_cleanup(std::move(cleanup)) {}
...
template <typename T>
[[nodiscard]] constexpr typename T::value_type getMinPositive(const T& container) {
    if (container.empty()) {
...
template <typename Iterator, typename T = const decltype(*std::declval<Iterator>())&>
[[nodiscard]] constexpr size_t countAdjacentIf(
    const Iterator rangeBegin, const Iterator rangeEnd,
    const std::function<bool(const decltype(*std::declval<Iterator>())&, const decltype(*std::declval<Iterator>())&)>&
        equal) {
    size_t result{0};
...
[[nodiscard]] inline bool testFlags(const uint64_t value, const uint64_t flags) {
...
[[nodiscard]] size_t countNewlines(const std::string_view& view) {
...
struct FindNthNewlineResult {
    ...
    [[nodiscard]] bool operator==(const FindNthNewlineResult& other) const noexcept {
...
inline std::ostream& operator<<(std::ostream& out, const FindNthNewlineResult& result) {
    out << "( position: " << result.position << ", remaining line count: " << result.remainingLineCount << ")";
    return out;
}

[[nodiscard]] constexpr FindNthNewlineResult findNthNewline(const std::string_view& view, const uint64_t lineCount,
...
[[nodiscard]] constexpr uint64_t operator"" _Ki(unsigned long long int value) noexcept {
    return value * 1024ULL;
}

[[nodiscard]] constexpr uint64_t operator"" _Mi(unsigned long long int value) noexcept {
    return value * 1024ULL * 1024ULL;
}

[[nodiscard]] constexpr uint64_t operator"" _Gi(unsigned long long int value) noexcept {
    return value * 1024ULL * 1024ULL * 1024ULL;
}

template <typename Pair, typename Value>
[[nodiscard]] constexpr bool rangeContains(const Pair& range, const Value& value) noexcept {
    return (range.first <= value) && (value <= range.second);
}

template <typename PairA, typename PairB>
[[nodiscard]] constexpr bool rangesIntersect(const PairA& rangeA, const PairB& rangeB) noexcept {
    return rangeContains(rangeA, rangeB.first) || rangeContains(rangeA, rangeB.second) ||
           rangeContains(rangeB, rangeA.first) || rangeContains(rangeB, rangeA.second);
}
