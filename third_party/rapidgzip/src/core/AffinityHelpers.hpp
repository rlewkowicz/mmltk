...
inline void pinThreadToLogicalCore(int logicalCoreId) {
    ...
    if (result != 0) {
        std::stringstream msg;
        msg << "When trying to pin current thread running on logical core " << sched_getcpu() << " to " << logicalCoreId
            << ", sched_setaffinity returned " << result << " and errno " << errno << " (" << std::strerror(errno)
...
[[nodiscard]] inline unsigned int availableCores() {
    ...
    if (result != 0) {
        std::stringstream msg;
        msg << "Failed to get affinity, sched_getaffinity returned " << result << " and errno " << errno << " ("
            << std::strerror(errno) << "). " << "A bitmask sized " << nCpusForSufficientMask << " was allocated.";
        throw std::runtime_error(std::move(msg).str());
    }
