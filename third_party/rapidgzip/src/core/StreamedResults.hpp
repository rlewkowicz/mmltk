...
class StreamedResults {
   public:
    ...
    class ResultsView {
       public:
        ResultsView(const Values* results, std::mutex* mutex) : m_results(results), m_lock(*mutex) {
            if (m_results == nullptr) {
                throw std::invalid_argument("Arguments may not be nullptr!");
            }
        }

        [[nodiscard]] const Values& results() const {
            return *m_results;
        }
   ...
   public:
    ...
    [[nodiscard]] std::pair<std::optional<size_t>, GetReturnCode> get(
        size_t position, double timeoutInSeconds = std::numeric_limits<double>::infinity()) const {
