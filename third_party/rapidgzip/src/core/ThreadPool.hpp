...
class ThreadPool {
    ...
    class PackagedTaskWrapper {
       private:
        ...
        struct SpecializedFunctor : BaseFunctor {
            explicit SpecializedFunctor(T_Functor&& functor) : m_functor(std::move(functor)) {}
   ...
   public:
    ...
    template <class T_Functor, std::enable_if_t<std::is_invocable_v<T_Functor>, void>* = nullptr>
    std::future<decltype(std::declval<T_Functor>()())> submit(T_Functor&& task, int priority = 0) {
        const std::lock_guard lock(m_mutex);
    ...
    [[nodiscard]] size_t unprocessedTasksCount(const std::optional<int> priority = {}) const {
        const std::lock_guard lock(m_mutex);
        if (priority) {
   ...
   private:
    [[nodiscard]] bool hasUnprocessedTasks() const {
        return std::any_of(m_tasks.begin(), m_tasks.end(), [](const auto& tasks) { return !tasks.second.empty(); });
    }
    ...
    void workerMain(size_t threadIndex) {
        ...
        while (m_threadPoolRunning) {
            ...
            const auto nonEmptyTasks =
                std::find_if(m_tasks.begin(), m_tasks.end(), [](const auto& tasks) { return !tasks.second.empty(); });
