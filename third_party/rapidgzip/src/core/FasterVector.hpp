...
class RpmallocAllocator {
   ...
   public:
    [[nodiscard]] constexpr ElementType* allocate(std::size_t nElementsToAllocate) {
    ...
    constexpr void deallocate(ElementType* allocatedPointer, [[maybe_unused]] std::size_t nElementsAllocated) {
...
class FasterVector {
   ...
   public:
    ...
    explicit FasterVector(size_t size, const std::optional<T>& initialValue = {}) {
    ...
    template <typename InputIt, typename = RequireInputIterator<InputIt> >
    void insert(const T* position, const InputIt& inputBegin, const InputIt& inputEnd) {
        ...
        if (newCapacity == 0) {
            ...
            if (m_data == nullptr) {
                ...
                m_data = static_cast<T*>(
                    rpaligned_realloc(m_data, ALIGNMENT, newCapacity * sizeof(T), m_capacity * sizeof(T), 0));
            }
#else
            if (m_data == nullptr) {
...
[[nodiscard]] bool operator==(const FasterVector<T>& lhs, const std::vector<T, Alloc>& rhs) {
    return std::equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
}

template <class T, class Alloc>
[[nodiscard]] bool operator==(const std::vector<T, Alloc>& lhs, const FasterVector<T>& rhs) {
    return std::equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
}
