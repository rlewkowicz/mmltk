...
[[nodiscard]] inline bool stdoutIsDevNull() {
    struct stat devNull{};
    struct stat stdOut{};
    return (fstat(STDOUT_FILENO, &stdOut) == 0) && (stat("/dev/null", &devNull) == 0) &&
...
inline void fileSeek(std::FILE* file, long long int offset, int origin) {
    ...
    if (returnCode != 0) {
        ...
        message << "Seeking to " << offset << " from origin " << originToString(origin)
                << " failed with code: " << returnCode << ", " << std::strerror(errno) << "!";
...
struct unique_file_descriptor {
    ...
    unique_file_descriptor(unique_file_descriptor&& other) noexcept : m_fd(other.m_fd) {
        other.m_fd = -1;
    }
...
inline unique_file_ptr make_unique_file_ptr(char const* const filePath, char const* const mode) {
...
inline unique_file_ptr make_unique_file_ptr(int fileDescriptor, char const* mode) {
    return make_unique_file_ptr(fdopen(fileDescriptor, mode));
}

inline unique_file_ptr throwingOpen(const std::string& filePath, const char* mode) {
    if (mode == nullptr) {
...
[[nodiscard]] Container readFile(const std::string& fileName) {
    ...
    (defined(MAC_OS_X_VERSION_MIN_REQUIRED) && MAC_OS_X_VERSION_MIN_REQUIRED >= MAC_OS_X_VERSION_10_15)
[[nodiscard]] inline std::string findParentFolderContaining(const std::string& folder,
...
inline unique_file_ptr throwingOpen(const std::filesystem::path& filePath, const char* mode) {
...
/**
 * Shared tail handling for vectorized writes: skips over fully written iovec
 * entries, flushes a partially written entry via writeRemainder(data, size),
 * and advances the index @p i accordingly.
 * @return the error code of the flush, or 0 when nothing remained / on success.
 */
template <typename WriteRemainder>
[[nodiscard]] inline int advancePartiallyWrittenIovec(const std::vector<::iovec>& dataToWrite, size_t& i,
                                                      size_t nBytesWritten, WriteRemainder&& writeRemainder) {
    for (; (i < dataToWrite.size()) && (dataToWrite[i].iov_len <= nBytesWritten); ++i) {
    ...
    if ((i < dataToWrite.size()) && (nBytesWritten > 0)) {
        ...
        assert(iovBuffer.iov_len > nBytesWritten);
        ...
        const auto errorCode = writeRemainder(remainingData, remainingSize);
        if (errorCode != 0) {
            return errorCode;
        }
        ++i;
...
[[nodiscard]] inline int writeAllSpliceUnsafe([[maybe_unused]] const int outputFileDescriptor,
    ...
    while (dataToSplice.iov_len > 0) {
        const auto nBytesWritten = ::vmsplice(outputFileDescriptor, &dataToSplice, 1, 0);
        if (nBytesWritten < 0) {
            return errno;
        }
...
[[nodiscard]] inline int writeAllSpliceUnsafe([[maybe_unused]] const int outputFileDescriptor,
    ...
    for (size_t i = 0; i < dataToWrite.size();) {
        ...
        auto nBytesWritten = ::vmsplice(outputFileDescriptor, &dataToWrite[i], segmentCount, 0);
        ...
        const auto errorCode = advancePartiallyWrittenIovec(
            dataToWrite, i, static_cast<size_t>(nBytesWritten), [outputFileDescriptor](const void* data, size_t size) {
                return writeAllSpliceUnsafe(outputFileDescriptor, data, size);
            });
        if (errorCode != 0) {
            return errorCode;
        }
...
class SpliceVault {
   ...
   public:
    [[nodiscard]] static std::pair<SpliceVault*, VaultLock> getInstance(int fileDescriptor) {
        ...
        if (vault == vaults.end()) {
            vault = vaults.emplace(fileDescriptor, std::unique_ptr<SpliceVault>(new SpliceVault(fileDescriptor))).first;
        }
        return std::make_pair(vault->second.get(), vault->second->lock());
    }

    template <typename T>
    [[nodiscard]] int splice(const void* const dataToWrite, size_t const dataToWriteSize,
    ...
    template <typename T>
    [[nodiscard]] int splice(const std::vector<::iovec>& buffersToWrite, const std::shared_ptr<T>& splicedData) {
        if (m_pipeBufferSize < 0) {
            return -1;
        }
        ...
        const auto dataToWriteSize =
            std::accumulate(buffersToWrite.begin(), buffersToWrite.end(), size_t(0),
   ...
   private:
    template <typename T>
    void account(const std::shared_ptr<T>& splicedData, size_t const dataToWriteSize) {
        ...
        while (!m_splicedData.empty() &&
               (m_totalSplicedBytes - std::get<2>(m_splicedData.front()) >= static_cast<size_t>(m_pipeBufferSize))) {
    ...
    explicit SpliceVault(int fileDescriptor)
        : m_fileDescriptor(fileDescriptor), m_pipeBufferSize(fcntl(fileDescriptor, F_GETPIPE_SZ)) {}

    [[nodiscard]] VaultLock lock() {
...
[[nodiscard]] inline int writeAllToFd(const int outputFileDescriptor, const void* const dataToWrite,
    ...
    for (uint64_t nTotalWritten = 0; nTotalWritten < dataToWriteSize;) {
        ...
        const auto nBytesToWritePerCall = static_cast<unsigned int>(
            std::min(static_cast<uint64_t>(std::numeric_limits<unsigned int>::max()), dataToWriteSize - nTotalWritten));
...
#ifdef HAVE_IOVEC
inline void pwriteAllToFd(const int outputFileDescriptor, const void* const dataToWrite, const uint64_t dataToWriteSize,
                          const uint64_t fileOffset) {
    for (uint64_t nTotalWritten = 0; nTotalWritten < dataToWriteSize;) {
        const auto* const currentBufferPosition = reinterpret_cast<const uint8_t*>(dataToWrite) + nTotalWritten;
        const auto nBytesWritten =
            ::pwrite(outputFileDescriptor, currentBufferPosition, dataToWriteSize - nTotalWritten,
...
[[nodiscard]] inline int writeAllToFdVector(const int outputFileDescriptor, const std::vector<::iovec>& dataToWrite) {
    for (size_t i = 0; i < dataToWrite.size();) {
        const auto segmentCount = std::min(static_cast<size_t>(IOV_MAX), dataToWrite.size() - i);
        ...
        const auto errorCode = advancePartiallyWrittenIovec(dataToWrite, i, static_cast<size_t>(nBytesWritten),
                                                            [outputFileDescriptor](const void* data, size_t size) {
                                                                return writeAllToFd(outputFileDescriptor, data, size);
                                                            });
        if (errorCode != 0) {
            return errorCode;
        }
...
[[nodiscard]] inline int writeAll(const int outputFileDescriptor, void* const outputBuffer,
                                  const void* const dataToWrite, const uint64_t dataToWriteSize) {
...
class OutputFile {
   public:
    explicit OutputFile(const std::string& filePath) : m_writingToStdout(filePath.empty()) {
        if (filePath.empty()) {
#ifdef _MSC_VER
    ...
    void truncate(size_t size)  // NOLINT
    {
        ...
        if ((m_fileDescriptor != -1) && (size < m_oldOutputFileSize)) {
            if (::ftruncate(m_fileDescriptor, size) == -1) {  // NOLINT
                std::cerr << "[Error] Failed to truncate file because of: " << strerror(errno) << " (" << errno
                          << ")\n";
            }
        }
#endif
