...
class BufferedFileReader : public FileReader {
   ...
   public:
    explicit BufferedFileReader(UniqueFileReader fileReader, size_t bufferSize = 128_Ki)
        : m_maxBufferSize(bufferSize), m_file(std::move(fileReader)) {}

    explicit BufferedFileReader(const std::vector<char>& inMemoryFileContents, size_t bufferSize = 128_Ki)
        : m_maxBufferSize(bufferSize), m_buffer(inMemoryFileContents.begin(), inMemoryFileContents.end()) {}

    explicit BufferedFileReader(const VectorView<char>& inMemoryFileContents, size_t bufferSize = 128_Ki)
        : m_maxBufferSize(bufferSize), m_buffer(inMemoryFileContents.begin(), inMemoryFileContents.end()) {}

    explicit BufferedFileReader(AlignedBuffer inMemoryFileContents, size_t bufferSize = 128_Ki)
    ...
    [[nodiscard]] size_t read(char* buffer, size_t nMaxBytesToRead) override {
        ...
        if (nMaxBytesToRead - nBytesRead >= m_maxBufferSize) {
            const auto nBytesReadFromFile =
                m_file->read(buffer == nullptr ? buffer : buffer + nBytesRead, nMaxBytesToRead - nBytesRead);
