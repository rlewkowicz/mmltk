...
class SharedFileReader final : public FileReader {
   private:
    explicit SharedFileReader(FileReader* file)
        : m_statistics(dynamic_cast<SharedFileReader*>(file) == nullptr
          ...
          m_mutex(dynamic_cast<SharedFileReader*>(file) == nullptr ? std::make_shared<std::mutex>()
   ...
   public:
    explicit SharedFileReader(UniqueFileReader file) : SharedFileReader(file.release()) {}
    ...
    ~SharedFileReader() override {
        if (m_statistics && m_statistics->showProfileOnDestruction && (m_statistics.use_count() == 1)) {
            const auto nTimesRead =
                m_fileSizeBytes ? m_statistics->read.sum / static_cast<double>(*m_fileSizeBytes) : 0;
            ...
            std::cerr << (ThreadSafeOutput()
                          ...
                          << "   reads         : (" << m_statistics->read.formatAverageWithUncertainty(true) << " ) B ("
                          << m_statistics->read.count << "calls )\n"
                          << "   locks         :" << m_statistics->locks << "\n"
                          << "   read in total" << static_cast<uint64_t>(m_statistics->read.sum) << "B out of"
                          << size().value_or(0) << "B," << "i.e., read the file" << nTimesRead << "times\n"
   ...
   public:
    ...
    [[nodiscard]] std::pair<std::unique_ptr<FileLock>, FileReader*> underlyingFile() {
        return {getUniqueLock(), m_sharedFile.get()};
    }

    void setUsePread(bool use) {
        m_usePread = use;
    }

    [[nodiscard]] bool usePread() const noexcept {
        return m_usePread;
    }
...
[[nodiscard]] inline std::unique_ptr<SharedFileReader> ensureSharedFileReader(UniqueFileReader&& fileReader) {
    if (!fileReader) {
