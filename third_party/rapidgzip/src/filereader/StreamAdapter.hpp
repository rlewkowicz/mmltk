...
class FileReaderStreamBuffer : public std::streambuf {
   public:
    ...
    explicit FileReaderStreamBuffer(UniqueFileReader file) : m_file(std::move(file)) {
        if (!m_file) {
    ...
    virtual ~FileReaderStreamBuffer() {
        sync();
    }
...
class FileReaderStream : public FileReaderStreamBuffer, public std::istream {
   public:
    explicit FileReaderStream(UniqueFileReader file)
        : FileReaderStreamBuffer(std::move(file)), std::istream(static_cast<std::streambuf*>(this)) {}
