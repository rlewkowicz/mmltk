...
[[nodiscard]] PyObject* toPyObject(Value value) {
...
[[nodiscard]] PyObject* toPyObject(PyObject* value) {
    if (value == nullptr) {
...
[[nodiscard]] Result fromPyObject(PyObject* pythonObject);

template <>
[[nodiscard]] long long int fromPyObject<long long int>(PyObject* pythonObject) {
    return PyLong_AsLongLong(pythonObject);
}

template <>
[[nodiscard]] size_t fromPyObject<size_t>(PyObject* pythonObject) {
    return PyLong_AsSize_t(pythonObject);
}

template <>
[[nodiscard]] bool fromPyObject<bool>(PyObject* pythonObject) {
    return pythonObject == Py_True;
}

template <>
[[nodiscard]] PyObject* fromPyObject<PyObject*>(PyObject* pythonObject) {
    return pythonObject;
}

template <typename Result, typename... Args>
Result callPyObject(PyObject* pythonObject, Args... args) {
    ...
    if constexpr (std::is_same_v<Result, void>) {
        ...
        if (result == nullptr) {
            ...
            message << "Cannot convert nullptr Python object to the requested result type (" << typeid(Result).name()
                    << ")!";
...
class PythonFileReader : public FileReader {
   public:
    explicit PythonFileReader(PyObject* pythonObject)
    ...
    [[nodiscard]] UniqueFileReader cloneRaw() const override {
        throw std::invalid_argument(
            "Cloning file path reader not allowed because the internal file position "
    ...
    [[nodiscard]] size_t read(char* buffer, size_t nMaxBytesToRead) override {
        ...
        if (nBytesRead < 0) {
            std::stringstream message;
            message << "[PythonFileReader] Read call failed (" << nBytesRead << " B read)!\n"
                    << "  Buffer: " << (void*)buffer << "\n"
    ...
    [[nodiscard]] size_t write(const char* buffer, size_t nBytesToWrite) {
        if (m_pythonObject == nullptr) {
        ...
        if ((nBytesWritten < 0) || (static_cast<size_t>(nBytesWritten) < nBytesToWrite)) {
            std::stringstream message;
            message << "[PythonFileReader] Write call failed (" << nBytesWritten << " B written)!\n"
                    << "  Buffer: " << (void*)buffer << "\n"
                    << "  tell: " << tell() << "\n"
                    << "\n";
            std::cerr << message.str();
   ...
   private:
    [[nodiscard]] static PyObject* checkNullptr(PyObject* pythonObject) {
        if (pythonObject == nullptr) {
    ...
    [[nodiscard]] static PyObject* getAttribute(PyObject* pythonObject, const char* name) {
