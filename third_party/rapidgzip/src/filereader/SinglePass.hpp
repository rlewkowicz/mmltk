...
class SinglePassFileReader final : public FileReader {
   ...
   public:
    explicit SinglePassFileReader(UniqueFileReader fileReader)
        : m_file(std::move(fileReader)), m_fileno(m_file ? m_file->fileno() : -1) {}
   ...
   private:
        ...
        while (!m_cancelReaderThread && !m_underlyingFileEOF) {
            if (m_numberOfBytesRead >= saturatingAddition(m_bufferUntilOffset.load(), 64 * CHUNK_SIZE)) {
                ...
                m_notifyReaderThread.wait(lock, [this]() {
                    return m_cancelReaderThread ||
                           (m_numberOfBytesRead < saturatingAddition(m_bufferUntilOffset.load(), 64 * CHUNK_SIZE));
                });
                continue;
            }
            ...
            while (nBytesBuffered < chunk.size()) {
                const auto nBytesRead =
                    m_file->read(reinterpret_cast<char*>(chunk.data()) + nBytesBuffered, chunk.size() - nBytesBuffered);
                if (nBytesRead == 0) {
                    break;
                }
        ...
        if (offset < m_numberOfBytesRead) {
            if (startChunk >= m_buffer.size()) {
                throw std::logic_error(
                    "[SinglePassFileReader] Current position is inside file but failed to find "
                    "chunk!");
            }
    ...
    [[nodiscard]] const Chunk& getChunk(size_t index) const {
        ...
        if ((index + 1 < m_buffer.size()) && (chunk.size() != CHUNK_SIZE)) {
            std::stringstream message;
            message << "[SinglePassFileReader] All but the last chunk must be of equal size! Chunk " << index
                    << " out of " << m_buffer.size() << " has size " << formatBytes(chunk.size())
   ...
   protected:
    ...
    std::unique_ptr<JoiningThread> m_readerThread{std::make_unique<JoiningThread>([this]() { readerThreadMain(); })};
};
