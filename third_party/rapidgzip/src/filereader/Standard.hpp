...
class StandardFileReader : public FileReader {
   public:
    ...
    explicit StandardFileReader(const std::filesystem::path& filePath) : StandardFileReader(filePath.string()) {}

    explicit StandardFileReader(const char* filePath) : StandardFileReader(std::string(filePath)) {}
#endif
    ...
    [[nodiscard]] size_t read(char* buffer, size_t nMaxBytesToRead) override {
        ...
        if (buffer == nullptr) {
            if (seekable()) {
                ...
                while (nBytesRead < nMaxBytesToRead) {
                    const auto nBytesReadPerCall = std::fread(tmpBuffer.data(), 1, tmpBuffer.size(), m_file.get());
        ...
        if (nBytesRead == 0) {
            ...
            message << "[StandardFileReader] Read call failed (" << nBytesRead << " B read)!\n"
                    << "  Buffer: " << (void*)buffer << "\n"
...
[[nodiscard]] inline UniqueFileReader openFileOrStdin(const std::string& inputFilePath) {
