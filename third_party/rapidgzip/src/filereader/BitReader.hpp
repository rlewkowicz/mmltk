...
class BitReader final : public FileReader {
   ...
   public:
    explicit BitReader(UniqueFileReader fileReader, const size_t bufferRefillSize = DEFAULT_BUFFER_REFILL_SIZE)
        : m_file(ensureSharedFileReader(std::move(fileReader))), m_bufferRefillSize(bufferRefillSize) {
    ...
    BitBuffer read2(bit_count_t bitsWanted) {
        ...
        if constexpr (MOST_SIGNIFICANT_BITS_FIRST) {
            ...
            bits = bitBufferSize() == 0
                       ? BitBuffer(0)
                       : (m_bitBuffer >> m_bitBufferFree) & N_LOWEST_BITS_SET_LUT<BitBuffer>[bitBufferSize()];
        }
        ...
        try {
            ...
            refillInputAndBitBuffers(bitsNeeded);
        }
        ...
        if (UNLIKELY(bitsWanted > bitBufferSize())) [[unlikely]] {
            if constexpr (!MOST_SIGNIFICANT_BITS_FIRST && (ENDIAN != Endian::UNKNOWN)) {
                if (LIKELY(m_inputBufferPosition + sizeof(BitBuffer) < m_inputBuffer.size())) [[likely]] {
                    ...
                    m_bitBuffer = (m_bitBuffer >> bitsToLoad) | (bytesToAppend << (MAX_BIT_BUFFER_SIZE - bitsToLoad));
            ...
            try {
                ...
                refillInputAndBitBuffers(bitsWanted);
            }
        }
   ...
   public:
    ...
    [[nodiscard]] constexpr Statistics statistics() const {
        return m_statistics;
    }
   ...
   private:
    ...
    void refillInputAndBitBuffers(const bit_count_t bitsWanted) {
        refillBuffer();
        try {
            refillBitBuffer();
        } catch (const BufferNeedsToBeRefilled&) {
    ...
    void fillBitBuffer() {
        struct ShiftBackOnReturn {
            ShiftBackOnReturn(BitBuffer& bitBuffer, const uint8_t& originalBitBufferSize) noexcept
                : m_bitBuffer(bitBuffer), m_originalBitBufferSize(originalBitBufferSize) {}
        ...
        for (; m_originalBitBufferSize + CHAR_BIT <= MAX_BIT_BUFFER_SIZE;
            ...
            if constexpr (MOST_SIGNIFICANT_BITS_FIRST) {
                ...
                m_bitBuffer |=
                    (static_cast<BitBuffer>(m_inputBuffer[m_inputBufferPosition++]) << m_originalBitBufferSize);
            }
        }
    ...
    [[nodiscard]] forceinline BitBuffer peekUnsafe(bit_count_t bitsWanted) const {
        ...
        if constexpr (MOST_SIGNIFICANT_BITS_FIRST) {
            return (m_bitBuffer >> (bitBufferSize() - bitsWanted)) & N_LOWEST_BITS_SET_LUT<BitBuffer>[bitsWanted];
        } else {
            assert(bitBufferSize() > 0);
            // NOLINTNEXTLINE(clang-analyzer-core.UndefinedBinaryOperatorResult)
            return (m_bitBuffer >> m_bitBufferFree) & N_LOWEST_BITS_SET_LUT<BitBuffer>[bitsWanted];
        }
    }
...
template <bool MOST_SIGNIFICANT_BITS_FIRST, typename BitBuffer>
inline size_t BitReader<MOST_SIGNIFICANT_BITS_FIRST, BitBuffer>::seek(long long int offsetBits, int origin) {
    if (origin == SEEK_END) {
...
template <bool MOST_SIGNIFICANT_BITS_FIRST, typename BitBuffer>
inline size_t BitReader<MOST_SIGNIFICANT_BITS_FIRST, BitBuffer>::fullSeek(size_t offsetBits) {
    if (!m_file) {
        throw std::logic_error("File has already been closed!");
    }
    ...
    if (seekable()) {
        ...
        if ((m_file->eof() && (!m_file->seekable() || (m_file->tell() > m_file->size()))) || m_file->fail()) {
            ...
            msg << "[BitReader] Could not seek to specified byte " << bytesToSeek << " subbit "
                << static_cast<int>(subBitsToSeek)
                ...
                << ", tell: " << m_file->tell() << ", size: " << m_file->size().value_or(0)
                << ", feof: " << m_file->eof() << ", ferror: " << m_file->fail() << ", newPosition: " << newPosition;
