...
#else
        code =
            load_native_u32((uint8_t*)matches_next) | ((uint64_t)load_native_u32((uint8_t*)(matches_next + 1)) << 32);
#endif
...
typedef uint64_t (*generate_icf_map_fn)(struct isal_zstream*, struct deflate_icf*, uint64_t);

static void icf_body_fillgreedy(struct isal_zstream* stream, generate_icf_map_fn generate_map) {
    ...
    while (matches_next_icf >= matches_end_icf) {
        ...
        processed = generate_map(stream, matches_icf_lookup, input_size);
...
void icf_body_hash1_fillgreedy_lazy(struct isal_zstream* stream) {
    icf_body_fillgreedy(stream, gen_icf_map_h1_base);
}

void icf_body_lazyhash1_fillgreedy_greedy(struct isal_zstream* stream) {
    icf_body_fillgreedy(stream, gen_icf_map_lh1);
}
