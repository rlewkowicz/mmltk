...
[[nodiscard]] constexpr const char* toString(const CompressionType compressionType) {
...
[[nodiscard]] Container compress(const VectorView<typename Container::value_type> toCompress,
    ...
    switch (compressionType) {
        ...
        case CompressionType::ZLIB:
            return rapidgzip::compressWithIsal<Container>(toCompress, {}, IsalContainerFormat::Zlib);
...
class CompressedVector {
   ...
   public:
    ...
    explicit CompressedVector(Container&& toCompress, const CompressionType compressionType)
    ...
    explicit CompressedVector(const VectorView<typename Container::value_type> toCompress,
                              const CompressionType compressionType)
        : m_compressionType(compressionType),
          m_decompressedSize(toCompress.size()),
          m_data(std::make_shared<Container>(compress<Container>(toCompress, compressionType))) {}

    explicit CompressedVector(Container&& compressedData, const size_t decompressedSize,
                              const CompressionType compressionType)
        : m_compressionType(compressionType),
    ...
    [[nodiscard]] std::shared_ptr<const Container> compressedData() const noexcept {
    ...
    [[nodiscard]] std::shared_ptr<const Container> decompress() const {
        ...
        const auto decompressWithWrapper = [this](FileType fileType) {
            return std::make_shared<Container>(
                inflateWithWrapper<InflateWrapper>(*m_data, m_decompressedSize, {}, fileType));
        };
