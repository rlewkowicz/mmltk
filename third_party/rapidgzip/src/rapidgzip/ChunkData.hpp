...
struct ChunkData : public deflate::DecodedData {
    ...
    struct Subchunk {
        [[nodiscard]] bool operator==(const Subchunk& other) const {
            return (encodedOffset == other.encodedOffset) && (decodedOffset == other.decodedOffset) &&
                   (encodedSize == other.encodedSize) && (decodedSize == other.decodedSize) &&
                   (newlineCount == other.newlineCount) &&
                   (static_cast<bool>(window) == static_cast<bool>(other.window)) &&
                   (!static_cast<bool>(window) || !static_cast<bool>(other.window) || (*window == *other.window));
        }

        [[nodiscard]] bool hasBeenPostProcessed(const bool requireNewlineCount) const {
   ...
   public:
    explicit ChunkData(const Configuration& configurationToUse) : configuration(configurationToUse) {
    ...
    [[nodiscard]] CompressionType windowCompressionType() const {
        ...
        return configuration.windowSparsity || (decodedSizeInBytes * 8 > 2 * encodedSizeInBits) ? CompressionType::ZLIB
    ...
    void applyWindow(WindowView const& window, CompressionType const windowCompressionType) {
        ...
        if (crc32s.front().enabled() && (alreadyProcessedSize < BaseType::dataSize())) {
            ...
            prependComputedCRC32(toProcessSize);
        ...
        for (auto& subchunk : m_subchunks) {
            ...
            if (configuration.newlineCharacter && !subchunk.newlineCount) {
                ...
                for (auto it = DecodedData::Iterator(*this, subchunk.decodedOffset, subchunk.decodedSize);
                    ...
                    newlineCount +=
                        std::count(reinterpret_cast<const char*>(buffer), reinterpret_cast<const char*>(buffer) + size,
    ...
    /** Computes the CRC32 over the first @p toProcessSize decoded bytes and prepends it to the first stream CRC. */
    void prependComputedCRC32(const size_t toProcessSize) {
    ...
    bool appendDeflateBlockBoundary(const size_t encodedOffset, const size_t decodedOffset) {
        if (blockBoundaries.empty() || (blockBoundaries.back().encodedOffset != encodedOffset) ||
    ...
    [[nodiscard]] deflate::DecodedVector getWindowAt(WindowView const& previousWindow, size_t skipBytes) const {
...
inline void ChunkData::setEncodedOffset(size_t offset) {
    ...
    if (!m_subchunks.empty()) {
        ...
        const auto nextOffset =
            nextSubchunk == m_subchunks.end() ? encodedEndOffsetInBits : nextSubchunk->encodedOffset;
        m_subchunks.front().encodedOffset = offset;
        m_subchunks.front().encodedSize = nextOffset - offset;
    }
}

[[nodiscard]] inline std::vector<ChunkData::Subchunk> ChunkData::split([[maybe_unused]] const size_t spacing) const {
    if (encodedEndOffsetInBits == std::numeric_limits<size_t>::max()) {
    ...
    const auto nBlocks =
        static_cast<size_t>(std::round(static_cast<double>(decodedSizeInBytes) / static_cast<double>(spacing)));
    ...
    for (size_t iSubchunk = 1; iSubchunk < nBlocks; ++iSubchunk) {
        ...
        const auto isCloser = [perfectDecompressedOffset](const auto& b1, const auto& b2) {
    ...
    if ((lastBoundary.decodedOffset < decodedSizeInBytes) || result.empty()) {
        ...
        subchunk.decodedSize = decodedSizeInBytes - lastBoundary.decodedOffset, result.emplace_back(subchunk);
    ...
    if (encodedEndOffsetInBits - encodedOffsetInBits != encodedSizeInBits) {
        std::stringstream message;
        message << "The offset: " << encodedOffsetInBits << ", size: " << encodedSizeInBits
                << ", and end offset: " << encodedEndOffsetInBits << " are inconsistent!";
        throw std::logic_error(std::move(message).str());
    }

    const auto subchunkEncodedSizeSum = std::accumulate(
        result.begin(), result.end(), size_t(0), [](size_t sum, const auto& block) { return sum + block.encodedSize; });
    const auto subchunkDecodedSizeSum = std::accumulate(
        result.begin(), result.end(), size_t(0), [](size_t sum, const auto& block) { return sum + block.decodedSize; });
...
[[nodiscard]] inline int writeAllSplice([[maybe_unused]] const int outputFileDescriptor,
...
[[nodiscard]] inline int writeAll(const std::shared_ptr<ChunkData>& chunkData, const int outputFileDescriptor,
                                  const size_t offsetInBlock, const size_t dataToWriteSize) {
    ...
    for (auto it = DecodedData::Iterator(*chunkData, offsetInBlock, dataToWriteSize); static_cast<bool>(it); ++it) {
        const auto& [buffer, size] = *it;
...
struct ChunkDataCounter final : public ChunkData {
    explicit ChunkDataCounter(const Configuration& configurationToUse) : ChunkData(configurationToUse) {
    ...
    [[nodiscard]] std::vector<Subchunk> split([[maybe_unused]] const size_t spacing) const {
