...
class GzipBlockFinder final : public BlockFinderInterface {
   ...
   public:
    explicit GzipBlockFinder(UniqueFileReader fileReader, size_t spacing)
        : m_file(std::move(fileReader)),
          m_fileSizeInBits(m_file->size() ? std::make_optional(*m_file->size() * CHAR_BIT) : std::nullopt),
    ...
    [[nodiscard]] std::pair<std::optional<size_t>, GetReturnCode> get(
        size_t blockIndex, [[maybe_unused]] double timeoutInSeconds) override {
        const std::scoped_lock lock(m_mutex);
    ...
    [[nodiscard]] size_t find(size_t encodedBlockOffsetInBits) const override {
        ...
        if ((encodedBlockOffsetInBits > m_blockOffsets.back()) && (encodedBlockOffsetInBits % m_spacingInBits == 0)) {
            const auto blockIndex =
                m_blockOffsets.size() + (encodedBlockOffsetInBits / m_spacingInBits - firstPartitionIndex());
    ...
    [[nodiscard]] size_t partitionOffsetContainingOffset(size_t blockOffset) const {
   ...
   private:
    ...
    [[nodiscard]] std::pair<std::optional<size_t>, GetReturnCode> getBgzfBlock(size_t blockIndex) {
