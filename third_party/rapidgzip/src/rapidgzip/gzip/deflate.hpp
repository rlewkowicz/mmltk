...
using LiteralOrLengthHuffmanCoding =
    HuffmanCodingShortBitsCached<uint16_t, MAX_CODE_LENGTH, uint16_t, MAX_LITERAL_HUFFMAN_CODE_COUNT, 11, true, true>;
#endif
...
using PrecodeHuffmanCoding =
    HuffmanCodingReversedBitsCachedCompressed<uint8_t, MAX_PRECODE_LENGTH, uint8_t, MAX_PRECODE_COUNT>;
...
[[nodiscard]] constexpr FixedHuffmanCoding createFixedHC() {
    ...
    const auto error = result.initializeFromLengths({encodedFixedHuffmanTree.data(), encodedFixedHuffmanTree.size()});
    if (error != Error::NONE) {
...
[[nodiscard]] forceinline Error readDistanceAndLiteralCodeLengths(
    LiteralAndDistanceCLBuffer& literalCL, gzip::BitReader& bitReader, const PrecodeHuffmanCoding& precodeCoding,
    const size_t literalCLSize,
    const std::function<uint8_t(uint8_t)>& translateSymbol = [](uint8_t symbol) { return symbol; }) {
    size_t i = 0;
    for (; i < literalCLSize;) {
...
class Block :  // NOLINT(cppcoreguidelines-pro-type-member-init)
   ...
   public:
    [[nodiscard]] bool eob() const noexcept {
    ...
    [[nodiscard]] constexpr uint8_t padding() const noexcept {
        return m_padding;
    }

    [[nodiscard]] std::pair<DecodedDataView, Error> read(gzip::BitReader& bitReader,
    ...
    [[nodiscard]] const std::vector<Backreference>& backreferences() const noexcept {
   ...
   private:
    template <bool WRAP_POSITION, typename Window>
    forceinline void appendToWindowImpl(Window& window, typename Window::value_type decodedSymbol);

    template <typename Window>
    forceinline void appendToWindow(Window& window, typename Window::value_type decodedSymbol);

    template <typename Window>
    forceinline void appendToWindowUnsafe(Window& window, typename Window::value_type decodedSymbol);

    template <typename Window>
    forceinline void resolveBackreference(Window& window, uint16_t distance, uint16_t length, size_t nBytesRead);

    template <typename Window>
    [[nodiscard]] forceinline Error resolveBackreferenceCode(gzip::BitReader& bitReader, Window& window,
                                                             uint16_t length, size_t& nBytesRead);

    template <typename Window>
    [[nodiscard]] std::pair<size_t, Error> readInternal(gzip::BitReader& bitReader, size_t nMaxToDecode,
                                                        Window& window);

    template <typename Window>
    [[nodiscard]] std::pair<size_t, Error> readInternalUncompressed(gzip::BitReader& bitReader, Window& window);

    template <typename Window, typename HuffmanCoding>
    [[nodiscard]] std::pair<size_t, Error> readInternalCompressed(gzip::BitReader& bitReader, size_t nMaxToDecode,
                                                                  Window& window, const HuffmanCoding& coding);

#ifdef LIBRAPIDARCHIVE_WITH_ISAL
    template <typename Window>
    [[nodiscard]] std::pair<size_t, Error> readInternalCompressedMultiCached(
        gzip::BitReader& bitReader, size_t nMaxToDecode, Window& window, const LiteralOrLengthHuffmanCoding& coding);

#endif

    [[nodiscard]] forceinline std::pair<uint16_t, Error> getDistance(gzip::BitReader& bitReader) const;

    template <typename Window, typename Symbol = typename Window::value_type, typename View = VectorView<Symbol> >
    [[nodiscard]] static std::array<View, 2> lastBuffers(const Window& window, size_t position, size_t size) {
        if (size > window.size()) {
   ...
   private:
    ...
    static_assert(std::min(PreDecodedBuffer().size(), PreDecodedBuffer().size() * sizeof(uint16_t) / sizeof(uint8_t)) >=
...
template <bool ENABLE_STATISTICS>
Error Block<ENABLE_STATISTICS>::readDynamicHuffmanCoding(gzip::BitReader& bitReader) {
    ...
    auto precodeApplyError =
        readDistanceAndLiteralCodeLengths(m_literalCL, bitReader, m_precodeHC, literalCodeCount + distanceCodeCount);
...
template <bool ENABLE_STATISTICS>
forceinline std::pair<uint16_t, Error> Block<ENABLE_STATISTICS>::getDistance(gzip::BitReader& bitReader) const {
    return rapidgzip::deflate::getDistance(m_compressionType, m_distanceHC, bitReader);
}

template <bool ENABLE_STATISTICS>
std::pair<DecodedDataView, Error> Block<ENABLE_STATISTICS>::read(gzip::BitReader& bitReader, size_t nMaxToDecode) {
    if (eob()) {
        return {{}, Error::NONE};
    }
    ...
    if (m_compressionType == CompressionType::UNCOMPRESSED) {
        ...
        if (m_uncompressedSize >= MAX_WINDOW_SIZE) {
            ...
            nBytesRead =
                bitReader.read(reinterpret_cast<char*>(window.data() + remainingData.size()), m_uncompressedSize);
        } else if (!m_containsMarkerBytes) {
            m_windowPosition = (m_windowPosition + m_uncompressedSize) % window.size();
            size_t totalBytesRead{0};
            auto buffers =
                lastBuffers<DecodedBuffer, uint8_t, WeakVector<uint8_t> >(window, m_windowPosition, m_uncompressedSize);
    ...
    if (m_containsMarkerBytes) {
        ...
        if ((m_distanceToLastMarkerByte >= m_window16.size()) ||
            ((m_distanceToLastMarkerByte >= MAX_WINDOW_SIZE) && (m_distanceToLastMarkerByte == m_decodedBytes))) {
            setInitialWindow();
            result.data = lastBuffers(window, m_windowPosition, nBytesRead);
        } else {
...
template <bool ENABLE_STATISTICS>
template <bool WRAP_POSITION, typename Window>
inline void Block<ENABLE_STATISTICS>::appendToWindowImpl(Window& window, typename Window::value_type decodedSymbol) {
...
template <bool ENABLE_STATISTICS>
template <typename Window>
inline void Block<ENABLE_STATISTICS>::appendToWindow(Window& window, typename Window::value_type decodedSymbol) {
    appendToWindowImpl<true>(window, decodedSymbol);
}

template <bool ENABLE_STATISTICS>
template <typename Window>
inline void Block<ENABLE_STATISTICS>::appendToWindowUnsafe(Window& window, typename Window::value_type decodedSymbol) {
    appendToWindowImpl<false>(window, decodedSymbol);
}

template <bool ENABLE_STATISTICS>
template <typename Window>
inline void Block<ENABLE_STATISTICS>::resolveBackreference(Window& window, const uint16_t distance,
                                                           const uint16_t length, const size_t nBytesRead) {
    if (m_trackBackreferences) {
        ...
        if (distance > decodedBytesInBlock) {
            m_backreferences.emplace_back(
                Backreference{static_cast<uint16_t>(distance - decodedBytesInBlock), std::min(length, distance)});
        }
    }
    ...
    if (LIKELY(m_windowPosition + length < window.size())) [[likely]] {
        if (LIKELY((length <= distance) && (distance <= m_windowPosition))) [[likely]] {
            ...
            if constexpr (containsMarkerBytes) {
                ...
                for (; distanceToLastMarkerByte < length; ++distanceToLastMarkerByte) {
                    if (window[m_windowPosition - 1 - distanceToLastMarkerByte] > std::numeric_limits<uint8_t>::max()) {
        ...
        for (size_t nCopied = 0; nCopied < length;) {
            for (size_t position = offset; (position < offset + nToCopyPerRepeat) && (nCopied < length);
                 ++position, ++nCopied) {
...
template <bool ENABLE_STATISTICS>
template <typename Window>
Error Block<ENABLE_STATISTICS>::resolveBackreferenceCode(gzip::BitReader& bitReader, Window& window,
                                                         const uint16_t length, size_t& nBytesRead) {
    if (length == 0) {
        return Error::NONE;
    }
    ...
    if constexpr (!containsMarkerBytes) {
        if (distance > m_decodedBytes + nBytesRead) {
            return Error::EXCEEDED_WINDOW_RANGE;
        }
    }
...
template <bool ENABLE_STATISTICS>
template <typename Window>
std::pair<size_t, Error> Block<ENABLE_STATISTICS>::readInternal(gzip::BitReader& bitReader, size_t nMaxToDecode,
                                                                Window& window) {
    if (m_compressionType == CompressionType::UNCOMPRESSED) {
...
template <bool ENABLE_STATISTICS>
template <typename Window>
std::pair<size_t, Error> Block<ENABLE_STATISTICS>::readInternalUncompressed(gzip::BitReader& bitReader,
                                                                            Window& window) {
...
// Shared prologue of the compressed-block readers: validates the Huffman
// coding and clamps the decode count so that appends cannot overflow the
// window slack reserved for the longest run.
template <typename HuffmanCoding, typename Window>
[[nodiscard]] inline size_t clampedMaxDecode(const HuffmanCoding& coding, const size_t nMaxToDecode,
                                             const Window& window) {
    if (!coding.isValid()) {
        throw std::invalid_argument("No Huffman coding loaded! Call readHeader first!");
    }

    return std::min(nMaxToDecode, window.size() - MAX_RUN_LENGTH);
}

template <bool ENABLE_STATISTICS>
template <typename Window, typename HuffmanCoding>
std::pair<size_t, Error> Block<ENABLE_STATISTICS>::readInternalCompressed(gzip::BitReader& bitReader,
                                                                          size_t nMaxToDecode, Window& window,
                                                                          const HuffmanCoding& coding) {
    nMaxToDecode = clampedMaxDecode(coding, nMaxToDecode, window);
    ...
    for (nBytesRead = 0; nBytesRead < nMaxToDecode;) {
        ...
        if (const auto error = resolveBackreferenceCode(bitReader, window, length, nBytesRead); error != Error::NONE) {
...
template <bool ENABLE_STATISTICS>
template <typename Window>
std::pair<size_t, Error> Block<ENABLE_STATISTICS>::readInternalCompressedMultiCached(
    gzip::BitReader& bitReader, size_t nMaxToDecode, Window& window, const LiteralOrLengthHuffmanCoding& coding) {
    nMaxToDecode = clampedMaxDecode(coding, nMaxToDecode, window);
    ...
    for (nBytesRead = 0; nBytesRead < nMaxToDecode;) {
        ...
        for (; symbolCount > 0; symbolCount--, symbol >>= 8U) {
            ...
            const auto length = static_cast<uint16_t>(symbol - 254U);
            if (const auto error = resolveBackreferenceCode(bitReader, window, length, nBytesRead);
                error != Error::NONE) {
                return {nBytesRead, error};
            }
        }
...
template <bool ENABLE_STATISTICS>
void Block<ENABLE_STATISTICS>::setInitialWindow(VectorView<uint8_t> const& initialWindow) {
    if (!m_containsMarkerBytes) {
        return;
    }
    ...
    std::memcpy(window.data() + (window.size() - conflatedBuffer.size()), conflatedBuffer.data(),
...
[[nodiscard]] inline bool readBlockHeader(Block<false>& block, gzip::BitReader& bitReader) {
...
/**
 * Shared driver for the sparse-window helpers: reads deflate blocks until
 * MAX_WINDOW_SIZE bytes were decoded or the stream ends. For every decoded
 * view, @p processView(view, nBytesRead, nBytesReadFromBlock) is called and
 * may return false to abort the whole loop. After each block,
 * @p afterBlock(nBytesRead, nBytesReadFromBlock) is called.
 * @return the total number of bytes read, or std::nullopt when aborted.
 */
template <typename ProcessView, typename AfterBlock>
[[nodiscard]] inline std::optional<size_t> forEachDeflateBlock(Block<false>& block, gzip::BitReader& bitReader,
                                                               ProcessView&& processView, AfterBlock&& afterBlock) {
    size_t nBytesRead{0};
    while (nBytesRead < MAX_WINDOW_SIZE) {
        if (!readBlockHeader(block, bitReader)) {
            break;
        }
        ...
        while ((nBytesRead + nBytesReadFromBlock < MAX_WINDOW_SIZE) && !block.eob()) {
            ...
            if (!processView(view, nBytesRead, nBytesReadFromBlock)) {
                return std::nullopt;
            }
        }

        afterBlock(nBytesRead, nBytesReadFromBlock);
...
[[nodiscard]] inline bool verifySparseWindow(gzip::BitReader& bitReader, const std::vector<bool>& windowByteIsRequired,
    ...
    const auto processView = [&expectedOutput](const auto& view, const size_t nBytesRead, size_t& nBytesReadFromBlock) {
        ...
        for (const auto& buffer : view.data) {
            const auto sizeToCompare =
                std::min(expectedOutput.size() - (nBytesRead + nBytesReadFromBlock), buffer.size());
    ...
    return forEachDeflateBlock(block, bitReader, processView, [](size_t, size_t) {}).has_value();
}

[[nodiscard]] inline std::vector<bool> getUsedWindowSymbols(gzip::BitReader& bitReader) {
...

    {
        ...
        const auto processView = [&decompressed](const auto& view, const size_t nBytesReadSoFar,
                                                 size_t& nBytesReadFromBlock) {
            if constexpr (CHECK_CORRECTNESS) {
                if (view.dataWithMarkersSize() > 0) {
                    throw std::logic_error("Result should not contain markers because we have set a window!");
                }
                for (const auto& buffer : view.data) {
                    const auto sizeToCopy =
                        std::min(decompressed.size() - (nBytesReadSoFar + nBytesReadFromBlock), buffer.size());
                    if (sizeToCopy == 0) {
                        continue;
                    }
                    std::memcpy(decompressed.data() + nBytesReadSoFar + nBytesReadFromBlock, buffer.data(), sizeToCopy);
        ...
        const auto markBackreferencedBytes = [&block, &window](const size_t nBytesReadSoFar, size_t) {
            const auto& backreferences = block.backreferences();
            for (const auto& reference : backreferences) {
                if (reference.distance < nBytesReadSoFar) {
                    continue;
                }

                const auto distanceFromEnd = reference.distance - nBytesReadSoFar;
        ...
        nBytesRead = forEachDeflateBlock(block, bitReader, processView, markBackreferencedBytes).value();
    }
...
[[nodiscard]] std::vector<uint8_t> getSparseWindow(gzip::BitReader& bitReader, const Container& window) {
