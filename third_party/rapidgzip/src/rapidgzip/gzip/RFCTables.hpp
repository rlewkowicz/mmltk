...
[[nodiscard]] constexpr uint16_t calculateDistance(uint16_t distance, uint8_t extraBitsCount,
                                                   uint16_t extraBits) noexcept {
    assert(distance >= 4);
    return 1U + (1U << (extraBitsCount + 1U)) + ((distance % 2U) << extraBitsCount) + extraBits;
};

[[nodiscard]] constexpr uint16_t calculateDistanceExtraBits(uint16_t distance) noexcept {
...
[[nodiscard]] inline uint16_t getLength(uint16_t code, gzip::BitReader& bitReader) {
    if (code <= 264) {
        return code - 257U + 3U;
    }
    if (code < 285) {
...
[[nodiscard]] inline uint8_t getLengthMinus3(uint16_t code, gzip::BitReader& bitReader) {
    return static_cast<uint8_t>(getLength(code, bitReader) - 3U);
}

template <typename DistanceHuffmanCoding>
inline std::pair<uint16_t, Error> getDistance(CompressionType compressionType, const DistanceHuffmanCoding& distanceHC,
