...
[[nodiscard]] inline std::string getExtraFlagsDescription(uint8_t code) noexcept {
    switch (code) {
        case 0:
            return "none";
        case 2:
...
inline std::pair<Header, Error> readHeader(gzip::BitReader& bitReader) {
    ...
    try {
        ...
        const auto readZeroTerminatedString = [&bitReader]() -> std::pair<std::string, Error> {
            std::string result;
...
[[nodiscard]] constexpr const char* toString(CompressionLevel compressionLevel) {
...
inline std::pair<Header, Error> readHeader(const std::function<uint64_t()>& readByte) {
    Header header;
