...
[[nodiscard]] uint32_t crc32SliceByN(uint32_t crc, const char* data, size_t size) {
...
#pragma GCC unroll 8
    for (size_t i = 0; i + SLICE_SIZE <= size; i += SLICE_SIZE) {
        ...
        std::memcpy(chunk.data() + sizeof(uint32_t), data + i + sizeof(uint32_t), SLICE_SIZE - sizeof(uint32_t));
...
[[nodiscard]] uint32_t updateCRC32(const uint32_t crc, const char* const buffer, const size_t size) {
#ifdef LIBRAPIDARCHIVE_WITH_ISAL
...
[[nodiscard]] constexpr uint32_t polynomialMultiplyModulo(const uint32_t a, uint32_t b, const uint32_t p) {
    uint32_t result = 0;
...
[[nodiscard]] constexpr uint32_t combineCRC32(uint32_t crc1, uint32_t crc2, uint64_t crc32ByteStreamLength) {
...
class CRC32Calculator {
   public:
    ...
    [[nodiscard]] uint32_t crc32() const noexcept {
        return ~m_crc32;
    }
...
[[nodiscard]] inline uint32_t crc32(const void* const buffer, const size_t size) {
