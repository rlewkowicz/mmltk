...
class IsalInflateWrapper {
   ...
   public:
    explicit IsalInflateWrapper(gzip::BitReader&& bitReader,
    ...
    [[nodiscard]] std::pair<size_t, std::optional<Footer> > readStream(uint8_t* output, size_t outputSize);
    ...
    [[nodiscard]] bool isFinalBlock() const {
    ...
    void setStartWithHeader(bool enable) {
   ...
   private:
    ...
    std::array<std::byte, SIZE> readBytes();
...
[[nodiscard]] inline std::pair<size_t, std::optional<Footer> > IsalInflateWrapper::readStream(uint8_t* const output,
    ...
    while ((decodedSize + m_stream.total_out < outputSize) && (m_stream.avail_out > 0)) {
        ...
        if (errorCode < 0) {
            ...
            message << "[IsalInflateWrapper][Thread " << std::this_thread::get_id() << "] "
                    ...
                    << "! Already decoded " << m_stream.total_out << " B. " << "Read "
                    << formatBits(oldUnusedBits - getUnusedBits()) << " during the failing isal_inflate "
        ...
        if (m_stream.block_state == ISAL_BLOCK_FINISH) {
            ...
            if ((m_stream.points_to_stop_at & ISAL_STOPPING_POINT_END_OF_STREAM) != 0) {
                ...
                const auto headerSuccess = readHeader();
                if (headerSuccess && ((m_stream.points_to_stop_at & ISAL_STOPPING_POINT_END_OF_STREAM_HEADER) != 0)) {
                    m_stream.stopped_at = ISAL_STOPPING_POINT_END_OF_STREAM_HEADER;
                }
            }
    ...
    switch (m_fileType) {
        ...
        case FileType::ZLIB: {
            const auto& [header, error] =
                zlib::readHeader([this]() { return static_cast<uint64_t>(readBytes<1U>().front()); });
...
template <typename Header, typename GetHeader>
inline bool IsalInflateWrapper::readIsalHeader(Header* const header, const GetHeader& getHeader) {
...
[[nodiscard]] inline std::string_view IsalInflateWrapper::getErrorString(int errorCode) noexcept {
    switch (errorCode) {
        case ISAL_DECOMP_OK:
...
enum class IsalContainerFormat : uint16_t {
    Deflate = IGZIP_DEFLATE,
    Gzip = IGZIP_GZIP,
    Zlib = IGZIP_ZLIB,
};

template <typename ResultContainer = std::vector<uint8_t> >
[[nodiscard]] ResultContainer compressWithIsal(const VectorView<uint8_t> toCompress,
                                               const VectorView<uint8_t> dictionary = {},
                                               const IsalContainerFormat containerFormat = IsalContainerFormat::Gzip) {
    ...
    stream.gzip_flag = static_cast<uint16_t>(containerFormat);
...
[[nodiscard]] Container inflateWithIsal(const Container& toDecompress, const size_t decompressedSize,
    ...
    if (result != ISAL_DECOMP_OK) {
        ...
        message << "Decompression of " << toDecompress.size()
                << "B sized vector failed with error code: " << IsalInflateWrapper::getErrorString(result) << " ("
