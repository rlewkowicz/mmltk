...
[[nodiscard]] inline std::pair<size_t, size_t> seekToNonFinalUncompressedDeflateBlock(
    gzip::BitReader& bitReader, size_t const untilOffset = std::numeric_limits<size_t>::max()) {
    ...
    try {
        auto untilOffsetSizeMember = untilOffset >= std::numeric_limits<size_t>::max() - MAX_PRECEDING_BYTES
        ...
        const auto startOffsetByte = std::max(static_cast<size_t>(BYTE_SIZE),
