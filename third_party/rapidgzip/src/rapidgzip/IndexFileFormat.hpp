...
struct Checkpoint {
    ...
    [[nodiscard]] constexpr bool operator==(const Checkpoint& other) const noexcept {
        return (compressedOffsetInBits == other.compressedOffsetInBits) &&
               (uncompressedOffsetInBytes == other.uncompressedOffsetInBytes) && (lineOffset == other.lineOffset);
    }
};
...
inline std::ostream& operator<<(std::ostream& out, NewlineFormat newlineFormat) {
...
struct GzipIndex {
   ...
   public:
    ...
    [[nodiscard]] constexpr bool operator==(const GzipIndex& other) const noexcept {
        return (compressedSizeInBytes == other.compressedSizeInBytes) &&
               (uncompressedSizeInBytes == other.uncompressedSizeInBytes) &&
               (checkpointSpacing == other.checkpointSpacing) && (windowSizeInBytes == other.windowSizeInBytes) &&
               (checkpoints == other.checkpoints) && (hasLineOffsets == other.hasLineOffsets) &&
...
inline void checkedRead(FileReader* const indexFile, void* buffer, size_t size) {
    ...
    if (nBytesRead != size) {
        throw std::runtime_error("Premature end of index file! Got only " + std::to_string(nBytesRead) + " out of " +
                                 std::to_string(size) + " requested bytes.");
    }
}

/**
 * Shared header handling of the index importers: validates the reader state,
 * completes the header buffer up to @p headerBufferSize bytes, and verifies
 * that it starts with @p magicBytes, throwing @p magicMismatchMessage else.
 */
[[nodiscard]] inline std::vector<char> readIndexHeader(const UniqueFileReader& indexFile,
                                                       const std::vector<char>& alreadyReadBytes,
                                                       const size_t headerBufferSize, const std::string_view magicBytes,
                                                       const char* const magicMismatchMessage) {
    if (!indexFile) {
        throw std::invalid_argument("Index file reader must be valid!");
    }
    ...
    if (alreadyReadBytes.size() > headerBufferSize) {
    ...
    if (headerBytes.size() < headerBufferSize) {
        const auto oldSize = headerBytes.size();
        headerBytes.resize(headerBufferSize);
        checkedRead(indexFile.get(), headerBytes.data() + oldSize, headerBytes.size() - oldSize);
    }

    if (!std::equal(magicBytes.begin(), magicBytes.end(), headerBytes.begin())) {
        throw std::invalid_argument(magicMismatchMessage);
    }
    return headerBytes;
}

template <typename T>
[[nodiscard]] T readValue(FileReader* const file) {
    T value;
...
inline void validateWindowSizes(const GzipIndex& index, const std::vector<Checkpoint>& checkpoints,
                                const uint32_t windowSizeInBytes) {
    const auto hasValidWindow = [&index, windowSizeInBytes](const auto& checkpoint) {
...
namespace bgzip {
[[nodiscard]] inline size_t countDecompressedBytes(gzip::BitReader&& bitReader,
...
[[nodiscard]] inline GzipIndex readGzipIndex(UniqueFileReader indexFile, UniqueFileReader archiveFile = {},
                                             const std::vector<char>& alreadyReadBytes = {}) {
    if (!indexFile) {
        throw std::invalid_argument("Index file reader must be valid!");
    }
    ...
    checkedRead(indexFile.get(), reinterpret_cast<char*>(&numberOfEntries) + alreadyReadBytes.size(),
    ...
    for (uint64_t i = 1; i < numberOfEntries; ++i) {
        ...
        if (checkpoint.compressedOffsetInBits > index.compressedSizeInBytes * 8U) {
            ...
            message << "Compressed bit offset (" << checkpoint.compressedOffsetInBits
                    << ") should be smaller or equal than the file size (" << index.compressedSizeInBytes * 8U << ")!";
            throw std::invalid_argument(std::move(message).str());
        }
        ...
        if (checkpoint.compressedOffsetInBits <= lastCheckPoint.compressedOffsetInBits) {
            std::stringstream message;
            message << "Compressed bit offset (" << checkpoint.compressedOffsetInBits
                    << ") should be greater than predecessor (" << lastCheckPoint.compressedOffsetInBits << ")!";
            throw std::invalid_argument(std::move(message).str());
        }
        ...
        if (checkpoint.uncompressedOffsetInBytes < lastCheckPoint.uncompressedOffsetInBytes) {
            ...
            message << "Uncompressed offset (" << checkpoint.uncompressedOffsetInBytes
                    << ") should be greater or equal than predecessor (" << lastCheckPoint.uncompressedOffsetInBytes
                    << ")!";
            throw std::invalid_argument(std::move(message).str());
        }
...
[[nodiscard]] inline GzipIndex readGzipIndex(UniqueFileReader indexFile,
    ...
    static constexpr size_t HEADER_BUFFER_SIZE =
        MAGIC_BYTES.size() + 1U + 1U + 2 * sizeof(uint64_t) + 2 * sizeof(uint32_t);
    const auto headerBytes = readIndexHeader(indexFile, alreadyReadBytes, HEADER_BUFFER_SIZE, MAGIC_BYTES,
                                             "Magic bytes do not match! Expected 'GZIDX'.");
    ...
    if (index.windowSizeInBytes != 32_Ki) {
        throw std::invalid_argument(
            "Only a window size of 32 KiB makes sense because indexed_gzip supports "
    ...
    for (uint32_t i = 0; i < checkpointCount; ++i) {
        ...
        if (i >= 1) {
            ...
            compressionRatio =
                ...
                8 / static_cast<double>(checkpoint.compressedOffsetInBits - previousCheckpoint.compressedOffsetInBits);
        }
...
inline void writeGzipIndex(const GzipIndex& index,
    ...
    validateWindowSizes(index, checkpoints, windowSizeInBytes);

    checkedWrite(MAGIC_BYTES.data(), MAGIC_BYTES.size());
    checkedWrite("\x01", 1);
    checkedWrite("\x00", 1);  // NOLINT(bugprone-string-literal-with-embedded-nul)
    ...
    if (!checkpoints.empty() && (checkpointSpacing < windowSizeInBytes)) {
        ...
        const auto minSpacing = std::accumulate(uncompressedOffsets.begin() + 1, uncompressedOffsets.end(), uint64_t(0),
    ...
    for (const auto& checkpoint : checkpoints) {
        ...
        if (!result && !isLastWindow) {
            throw std::logic_error("Did not find window to offset " + formatBits(checkpoint.compressedOffsetInBits));
        }
...
[[nodiscard]] inline GzipIndex readGzipIndex(UniqueFileReader indexFile, const std::optional<size_t> archiveSize = {},
                                             const std::vector<char>& alreadyReadBytes = {}) {
    static constexpr size_t HEADER_BUFFER_SIZE = MAGIC_BYTES.size() + 1U;
    const auto headerBytes =
        readIndexHeader(indexFile, alreadyReadBytes, HEADER_BUFFER_SIZE, MAGIC_BYTES, "Magic bytes do not match!");
    ...
    for (uint32_t i = 0; i < checkpointCount; ++i) {
        ...
        if (compressedWindowSize == 0) {
            ...
            const auto [decompressedWindowSize, footer] =
                inflateWrapper.readStream(decompressedWindow.data(), decompressedWindow.size());
            if (!footer) {
            ...
            index.windows->emplaceShared(
                ...
                std::make_shared<WindowMap::Window>(std::move(compressedWindow), decompressedWindowSize,
        ...
        if (index.hasLineOffsets) {
            if (index.checkpoints.empty() ||
                ...
                throw std::domain_error(
                    "The last checkpoint at the end of the compressed stream does not match "
...
inline void writeGzipIndex(const GzipIndex& index,
    ...
    for (const auto& checkpoint : checkpoints) {
        ...
        if (result->empty()) {
            ...
            const auto recompressed = compressWithIsal(window, {}, IsalContainerFormat::Zlib);
...
[[nodiscard]] inline GzipIndex readGzipIndex(UniqueFileReader indexFile, UniqueFileReader archiveFile = {},
    ...
    if (const auto commonSize = std::min(formatId.size(), gztool::MAGIC_BYTES.size());
        std::string_view(formatId.data(), commonSize) == std::string_view(gztool::MAGIC_BYTES.data(), commonSize)) {
