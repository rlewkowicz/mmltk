...
struct DecodedData {
   public:
    ...
    class Iterator {
       public:
        explicit Iterator(const DecodedData& decodedData, const size_t offsetInChunk = 0,
   ...
   public:
    ...
    [[nodiscard]] DecodedVector getLastWindow(WindowView const& previousWindow) const;

    [[nodiscard]] DecodedVector getWindowAt(WindowView const& previousWindow, size_t skipBytes) const;
    ...
    [[nodiscard]] const std::vector<MarkerVector>& getDataWithMarkers() const noexcept {
        return dataWithMarkers;
    }

    [[nodiscard]] const std::vector<VectorView<uint8_t>>& getData() const noexcept {
        return data;
    }
#endif
...
inline void DecodedData::append(DecodedDataView const& buffers) {
    ...
    const auto& appendToEquallySizedChunks = [](auto& targetChunks, const auto& buffer) {
    ...
    if (buffers.dataWithMarkersSize() > 0) {
        if (!data.empty()) {
            throw std::invalid_argument(
                "It is not allowed to append data with markers when fully decoded data "
...
[[nodiscard]] inline size_t DecodedData::countMarkerSymbols() const {
    ...
    for (const auto& chunk : dataWithMarkers) {
        result +=
            std::count_if(chunk.begin(), chunk.end(), [](const uint16_t symbol) { return (symbol & 0xFF00U) != 0; });
    }
    return result;
}
...
[[nodiscard]] inline DecodedVector DecodedData::getLastWindow(WindowView const& previousWindow) const {
    return getWindowAt(previousWindow, size());
}

[[nodiscard]] inline DecodedVector DecodedData::getWindowAt(WindowView const& previousWindow,
    ...
    // Copies the chunk contents into the window, transforming each symbol,
    // while skipping the first `offset` bytes and stopping at the window end.
    const auto copyChunksIntoWindow = [&offset, &prefilled, &window](const auto& chunks, const auto& transformSymbol) {
        for (const auto& chunk : chunks) {
            ...
            for (size_t i = offset; (i < chunk.size()) && (prefilled < window.size()); ++i, ++prefilled) {
                window[prefilled] = transformSymbol(chunk[i]);
            }
            offset = 0;
        }
    };
    ...
    if (previousWindow.size() >= MAX_WINDOW_SIZE) {
        copyChunksIntoWindow(dataWithMarkers, MapMarkers<true>(previousWindow));
    } else {
        copyChunksIntoWindow(dataWithMarkers, MapMarkers<false>(previousWindow));
    }

    copyChunksIntoWindow(data, [](const auto symbol) { return symbol; });
...
inline void DecodedData::cleanUnmarkedData() {
    while (!dataWithMarkers.empty()) {
        ...
        const auto marker = std::find_if(toDowncast.rbegin(), toDowncast.rend(),
...
#ifdef HAVE_IOVEC
[[nodiscard]] inline std::vector<::iovec> toIoVec(const DecodedData& decodedData, const size_t offsetInBlock,
