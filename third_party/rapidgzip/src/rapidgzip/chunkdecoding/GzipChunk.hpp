...
class GzipChunk final {
   public:
    ...
    static void startNewSubchunk(std::vector<Subchunk>& subchunks, const size_t encodedOffset) {
        const auto nextDecodedOffset =
            subchunks.empty() ? 0 : subchunks.back().decodedOffset + subchunks.back().decodedSize;
    ...
    static void determineUsedWindowSymbolsForLastSubchunk(std::vector<Subchunk>& subchunks,
        ...
        try {
            const Finally seekBack{[&bitReader, oldOffset = bitReader.tell()]() { bitReader.seekTo(oldOffset); }};
    ...
    static void finalizeWindowForLastSubchunk(ChunkData& chunk, std::vector<Subchunk>& subchunks,
                                              gzip::BitReader& bitReader) {
        if (subchunks.empty()) {
            return;
        }
    ...
    static void finalizeChunk(ChunkData& chunk, std::vector<Subchunk>&& subchunks, gzip::BitReader& bitReader,
                              const size_t nextBlockOffset) {
        subchunks.back().encodedSize = nextBlockOffset - subchunks.back().encodedOffset;
        if ((subchunks.size() >= 2) && (subchunks.back().decodedSize < chunk.minimumSplitChunkSize())) {
    ...
    static void appendDeflateBlockBoundary(ChunkData& chunk, std::vector<Subchunk>& subchunks,
                                           gzip::BitReader& bitReader, const size_t encodedOffset,
    ...
    [[nodiscard]] static ChunkData decodeChunkWithInflateWrapper(UniqueFileReader&& sharedFileReader,
        ...
        while (true) {
            ...
            for (; (nBytesRead < subchunk.size()) && !footer; nBytesRead += nBytesReadPerCall) {
                std::tie(nBytesReadPerCall, footer) =
                    inflateWrapper.readStream(subchunk.data() + nBytesRead, subchunk.size() - nBytesRead);
    ...
    [[nodiscard]] static ChunkData finishDecodeChunkWithInexactOffset(
        gzip::BitReader* const bitReader, size_t const untilOffset, WindowView const initialWindow,
        size_t const maxDecompressedChunkSize, ChunkData&& result, std::vector<Subchunk>&& subchunks) {
        ...
        inflateWrapper.setStoppingPoints(static_cast<StoppingPoint>(
            StoppingPoint::END_OF_BLOCK | StoppingPoint::END_OF_BLOCK_HEADER | StoppingPoint::END_OF_STREAM_HEADER));
        ...
        while (!stoppingPointReached) {
            ...
            while ((nBytesRead < buffer.size()) && !footer && !stoppingPointReached) {
                std::tie(nBytesReadPerCall, footer) =
                    inflateWrapper.readStream(buffer.data() + nBytesRead, buffer.size() - nBytesRead);
                ...
                switch (inflateWrapper.stoppedAt()) {
                    ...
                    case StoppingPoint::END_OF_BLOCK_HEADER:
                        if (((nextBlockOffset >= untilOffset) && !inflateWrapper.isFinalBlock() &&
    ...
    [[nodiscard]] static ChunkData decodeChunkWithRapidgzip(gzip::BitReader* const bitReader, size_t const untilOffset,
        ...
        if (initialWindow) {
            return finishDecodeChunkWithInexactOffset<IsalInflateWrapper>(bitReader, untilOffset, *initialWindow,
        ...
        while (true) {
            if (isAtStreamEnd) {
                ...
                return finishDecodeChunkWithInexactOffset<IsalInflateWrapper>(
                    bitReader, untilOffset, {}, maxDecompressedChunkSize, std::move(result), std::move(subchunks));
#endif
            ...
            if (auto error = block->readHeader(*bitReader); error != Error::NONE) {
                ...
                message << "Failed to read deflate block header at offset " << formatBits(result.encodedOffsetInBits)
                        << " (position after trying: " << formatBits(bitReader->tell()) << ": " << toString(error);
                throw std::domain_error(std::move(message).str());
            }
            ...
            while (!block->eob()) {
                ...
                if (blockBytesRead > 256_Mi) {
                    throw std::runtime_error(
                        "A single deflate block that decompresses to more than 256 MiB was "
    ...
    [[nodiscard]] static ChunkData decodeChunk(UniqueFileReader&& sharedFileReader, size_t const blockOffset,
        ...
        if (initialWindow && untilOffsetIsExact) {
            ...
            auto result = decodeChunkWithInflateWrapper<InflateWrapper>(
                ...
                fileSize ? std::min(untilOffset, *fileSize * BYTE_SIZE) : untilOffset, *window, decodedSize,
        ...
        const auto tryToDecode = [&](const std::pair<size_t, size_t>& offset) -> std::optional<ChunkData> {
            try {
                bitReader.seekTo(offset.second);
                auto result = decodeChunkWithRapidgzip(&bitReader, untilOffset, std::nullopt, chunkDataConfiguration);
        ...
        const auto findNextUncompressed = [&](size_t beginOffset, size_t endOffset) {
            if (beginOffset >= endOffset) {
