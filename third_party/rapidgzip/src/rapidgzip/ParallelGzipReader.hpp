...
[[nodiscard]] static UniqueFileReader wrapFileReader(UniqueFileReader&& fileReader, IOReadMethod ioReadMethod) {
    switch (ioReadMethod) {
...
class ParallelGzipReader final : public FileReader {
   ...
   public:
    explicit ParallelGzipReader(UniqueFileReader fileReader, size_t parallelization = 0,
        ...
        : m_chunkSizeInBytes(std::max(8_Ki, chunkSizeInBytes)),
          ...
          m_startBlockFinder([this]() {
              return std::make_unique<BlockFinder>(UniqueFileReader(m_sharedFileReader->clone()), m_chunkSizeInBytes);
          }) {
        ...
        if (fileSize && (m_chunkSizeInBytes * 2U * parallelization > *fileSize)) {
            m_chunkSizeInBytes = std::max(512_Ki, ceilDiv(ceilDiv(*fileSize, 3U * parallelization), 512_Ki) * 512_Ki);
        }
    ...
    explicit ParallelGzipReader(int fileDescriptor, size_t parallelization, uint64_t chunkSizeInBytes,
    ...
    explicit ParallelGzipReader(const std::string& filePath, size_t parallelization, uint64_t chunkSizeInBytes,
                                IOReadMethod ioReadMethod)
        : ParallelGzipReader(wrapFileReader(std::make_unique<StandardFileReader>(filePath), ioReadMethod),
                             parallelization, chunkSizeInBytes) {}

    explicit ParallelGzipReader(PyObject* pythonObject, size_t parallelization, uint64_t chunkSizeInBytes,
                                IOReadMethod ioReadMethod)
    ...
    ~ParallelGzipReader() override {
        if (m_showProfileOnDestruction && m_statisticsEnabled) {
            ...
            out << "    Window compression : "
                << (m_windowCompressionType ? toString(*m_windowCompressionType) : std::string("Default")) << "\n";
        ...
        if (m_atEndOfFile) {
            ...
            if (!fileSize) {
                throw std::logic_error(
                    "When the file end has been reached, the block map should have been finalized "
    ...
    size_t read(const int outputFileDescriptor = -1, char* const outputBuffer = nullptr,
                const size_t nBytesToRead = std::numeric_limits<size_t>::max()) {
        const auto writeFunctor = [nBytesDecoded = uint64_t(0), outputFileDescriptor, outputBuffer](
                                      const std::shared_ptr<ChunkData>& chunkData, size_t const offsetInBlock,
            ...
            if (errorCode != 0) {
                ...
                message << "Failed to write all bytes because of: " << strerror(errorCode) << " (" << errorCode << ")";
            ...
            if (outputBuffer != nullptr) {
                ...
                for (auto it = DecodedData::Iterator(*chunkData, offsetInBlock, dataToWriteSize); static_cast<bool>(it);
                     ++it) {
    ...
    size_t read(const WriteFunctor& writeFunctor, const size_t nBytesToRead = std::numeric_limits<size_t>::max()) {
        ...
        while ((nBytesDecoded < nBytesToRead) && !eof()) {
            ...
            if (offsetInBlock >= blockSize) {
                ...
                message << "[ParallelGzipReader] Block does not contain the requested offset! "
                        ...
                        << formatBytes(m_currentPosition) << ")" << ", decoded offset: " << decodedOffsetInBytes << " ("
            ...
            auto* const singlePassFileReader = dynamic_cast<SinglePassFileReader*>(file);
            if (singlePassFileReader != nullptr) {
                singlePassFileReader->releaseUpTo(chunkData->encodedOffsetInBits / CHAR_BIT);
            }
            ...
            if (index.hasLineOffsets) {
                ...
                if (lineOffset == m_newlineOffsets.end()) {
                    ...
                    message << "Failed to find line offset for uncompressed offset: "
                            ...
                            << ", number of line offsets to stored: " << m_newlineOffsets.size();
                    throw std::runtime_error(std::move(message).str());
                }
    ...
    [[nodiscard]] const std::vector<NewlineOffset>& newlineOffsets() const noexcept {
   ...
   public:
    ...
    void importIndex(UniqueFileReader indexFile) {
        const auto t0 = now();
        setBlockOffsets(readGzipIndex(std::move(indexFile), m_sharedFileReader->clone(), m_fetcherParallelization));
        if (m_showProfileOnDestruction) {
            std::cerr << "[ParallelGzipReader::importIndex] Took " << duration(t0) << " s\n";
        }
    }

    void exportIndex(const std::function<void(const void* buffer, size_t size)>& checkedWrite,
    ...
    void exportIndex(PyObject* pythonObject, const IndexFormat indexFormat = IndexFormat::INDEXED_GZIP) {
    ...
    void gatherLineOffsets() {
        ...
        const auto collectLineOffsets = [this, &processedLines, &newlineOffsets, &processedBytes,
            ...
            using rapidgzip::deflate::DecodedData;
            for (auto it = DecodedData::Iterator(*chunkData, offsetInChunk, dataToWriteSize); static_cast<bool>(it);
                 ++it) {
                const auto& [buffer, size] = *it;

                const std::string_view view{reinterpret_cast<const char*>(buffer), size};
                for (auto position = view.find(newlineCharacter, 0); position != std::string_view::npos;
            ...
            while (it != newlineOffsets.end()) {
                ...
                if (m_newlineOffsets.empty() || (m_newlineOffsets.back().uncompressedOffsetInBytes != *it)) {
                    ...
                    newlineOffset.lineOffset =
                        static_cast<uint64_t>(std::distance(newlineOffsets.begin(), it)) + processedLines;
                    ...
                    if (!m_newlineOffsets.empty()) {
                        if (m_newlineOffsets.back().uncompressedOffsetInBytes >= *it) {
                            ...
                            message << "Got earlier or equal chunk offset than the last processed one! "
                                    << "Last newline byte offset: " << m_newlineOffsets.back().uncompressedOffsetInBytes
   ...
   private:
    ...
    ChunkFetcher& chunkFetcher() {
        ...
        m_chunkFetcher =
            std::make_unique<ChunkFetcher>(ensureSharedFileReader(m_sharedFileReader->clone()), m_blockFinder,
    ...
    void gatherLineOffsets(const std::shared_ptr<const ChunkData>& chunk) {
        ...
        for (const auto& subchunk : chunk->subchunks()) {
            ...
            m_newlineOffsets.emplace_back(NewlineOffset{lastLineCount.lineOffset + subchunk.newlineCount.value(),
                                                        blockInfo->decodedOffsetInBytes + subchunk.decodedSize});
        }
    }

    void setBlockFinderOffsets(const std::map<size_t, size_t>& offsets)  // NOLINT(misc-no-recursion)
    {
        if (offsets.empty()) {
    ...
    void processCRC32(const std::shared_ptr<ChunkData>& chunkData, [[maybe_unused]] size_t const offsetInBlock,
        ...
        if (!m_crc32.enabled() || (m_nextCRC32ChunkOffset != chunkData->encodedOffsetInBits) ||
        ...
        const auto totalCRC32StreamSize =
            std::accumulate(chunkData->crc32s.begin(), chunkData->crc32s.end(), size_t(0),
