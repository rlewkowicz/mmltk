...
class WindowMap {
   ...
   public:
    ...
    explicit WindowMap(const WindowMap& other) : m_windows(*other.data().second) {}

    void emplace(const size_t encodedBlockOffset, WindowView window, CompressionType compressionType) {
        emplaceShared(encodedBlockOffset, std::make_shared<Window>(window, compressionType));
    }

    void emplaceShared(const size_t encodedBlockOffset, SharedWindow sharedWindow) {
        if (!sharedWindow) {
            return;
        }
    ...
    [[nodiscard]] std::pair<std::unique_lock<std::mutex>, const Windows*> data() const {
        return {std::unique_lock(m_mutex), &m_windows};
    }
