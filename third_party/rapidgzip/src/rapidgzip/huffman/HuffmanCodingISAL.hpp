...
class HuffmanCodingISAL  // NOLINT(cppcoreguidelines-pro-type-member-init)
{
   ...
   public:
    [[nodiscard]] Error initializeFromLengths(const VectorView<uint8_t>& codeLengths) {
        ...
        if (set_and_expand_lit_len_huffcode(lit_and_dist_huff.data(), LIT_LEN, lit_count.data(),
                                            lit_expand_count.data(), code_list.data()) != ISAL_DECOMP_OK) {
            m_error = Error::INVALID_HUFFMAN_CODE;
            return m_error;
        }

        make_inflate_huff_code_lit_len(&m_huffmanCode, lit_and_dist_huff.data(), LIT_LEN_ELEMS, lit_count.data(),
                                       code_list.data(), 0);
   ...
   private:
    static void write_huff_code(huff_code* const huff_code, uint32_t const code, uint32_t const length) {
        huff_code->code_and_length = code | (length << 24U);  // NOLINT(cppcoreguidelines-pro-type-union-access)
    }

   public:
    [[nodiscard]] forceinline std::pair<uint32_t, uint32_t> decode(gzip::BitReader& bitReader) const {
        ...
        const auto next12Bits = nextBits & N_LOWEST_BITS_SET_LUT<uint64_t>[ISAL_DECODE_LONG_BITS];
        ...
        if (LIKELY((next_sym & LARGE_FLAG_BIT) == 0)) [[likely]] {
            ...
            return {next_sym & LARGE_SHORT_SYM_MASK, (next_sym >> LARGE_SYM_COUNT_OFFSET) & LARGE_SYM_COUNT_MASK};
        }
        ...
        next_sym =
            m_huffmanCode.long_code_lookup[(next_sym & LARGE_SHORT_SYM_MASK) + (nextBits >> ISAL_DECODE_LONG_BITS)];
