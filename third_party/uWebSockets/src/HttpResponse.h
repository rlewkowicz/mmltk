...
struct HttpResponse : public AsyncSocket<SSL> {
   ...
   public:
    ...
    void upgrade(UserData&& userData, std::string_view secWebSocketKey, std::string_view secWebSocketProtocol,
        ...
        us_socket_t* sourceSocket = (us_socket_t*)this;

        static_assert(std::is_nothrow_move_constructible_v<HttpResponseData<SSL>>,
                      "HTTP response adoption requires allocation-free rollback");
        HttpResponseData<SSL> savedHttpResponseData(std::move(*getHttpResponseData()));
        ...
        WebSocket<SSL, true, UserData>* webSocket = (WebSocket<SSL, true, UserData>*)us_socket_context_adopt_socket(
            SSL, (us_socket_context_t*)webSocketContext, sourceSocket,
            sizeof(WebSocketData) + sizeof(UserData) + sizeof(WebSocketData::ConstructionState));
        if (!webSocket) {
            new (us_socket_ext(SSL, sourceSocket)) HttpResponseData<SSL>(std::move(savedHttpResponseData));
            throw std::bad_alloc{};
        }
        *webSocket->getConstructionState() = WebSocketData::ConstructionState::Raw;
        if (httpContextData->isParsingHttp) {
            // Publish the relocated socket before any WebSocket/UserData construction can throw.
            // HttpContext's dispatch guard then closes the correct allocation and consults the
            // construction marker instead of touching the stale pre-realloc pointer.
            httpContextData->upgradedWebSocket = webSocket;
        }

        BackPressure backpressure(std::move(savedHttpResponseData.buffer));
        ...
        *webSocket->getConstructionState() = WebSocketData::ConstructionState::UserDataConstructed;
