...
#ifndef UWEBSOCKETS_ANY_INVOKABLE_H_
#define UWEBSOCKETS_ANY_INVOKABLE_H_
...
inline constexpr bool is_small_object_v =
    sizeof(T) <= sizeof(buffer) && alignof(buffer) % alignof(T) == 0 && std::is_nothrow_move_constructible_v<T>;
...
struct handler_traits {
    ...
    struct small_handler : handler_base<small_handler<T>> {
        ...
        static R call(storage& s, ArgTypes... args) {
            return std::invoke(*static_cast<T*>(static_cast<void*>(&s.buf_)), std::forward<ArgTypes>(args)...);
        }
    };
...
using can_convert =
    std::conjunction<std::negation<std::is_same<remove_cvref_t<F>, AI>>,
    ...
    class any_invocable<R(ArgTypes...) cv ref noexcept(noex)>                                                          \
       ...
       public:                                                                                                         \
        ...
        template <class F, class = std::enable_if_t<                                                                   \
                               any_detail::can_convert<any_invocable, F, noex, R, F inv_quals, ArgTypes...>::value>>   \
        any_invocable(F&& f) {                                                                                         \
            base_type::template create<std::decay_t<F>>(std::forward<F>(f));                                           \
        }                                                                                                              \
                                                                                                                       \
        template <class T, class... Args, class VT = std::decay_t<T>,                                                  \
                  class = std::enable_if_t<std::is_move_constructible_v<VT> && std::is_constructible_v<VT, Args...> && \
        ...
        std::enable_if_t<!std::is_same_v<FDec, any_invocable> && std::is_move_constructible_v<FDec>, any_invocable&>   \
        operator=(F&& f) {                                                                                             \
...
__OFATS_ANY_INVOCABLE(, , false, &)
__OFATS_ANY_INVOCABLE(, , true, &)
__OFATS_ANY_INVOCABLE(, &, false, &)
__OFATS_ANY_INVOCABLE(, &, true, &)
__OFATS_ANY_INVOCABLE(, &&, false, &&)
__OFATS_ANY_INVOCABLE(, &&, true, &&)
