...
struct HttpContext {
   ...
   private:
    ...
    HttpContext<SSL>* init() {
        static_assert(std::is_nothrow_default_constructible_v<HttpResponseData<SSL>>,
                      "HTTP socket dispatch requires allocation-free response construction");
        us_socket_context_on_open(SSL, getSocketContext(), [](us_socket_t* s, int, char* ip, int ip_length) noexcept {
            ...
            try {
                ...
                httpContextData->cDispatchFailureHandler.notify();
                us_socket_close(SSL, s, 0, nullptr);
                return (us_socket_t*)nullptr;
            }
        });

        us_socket_context_on_close(SSL, getSocketContext(), [](us_socket_t* s, int, void*) noexcept {
        ...
        us_socket_context_on_data(SSL, getSocketContext(), [](us_socket_t* s, char* data, int length) noexcept {
            HttpContextData<SSL>* httpContextData = getSocketContextDataS(s);
            try {
                ...
                if (returnedSocket == FULLPTR) {
                    us_socket_write(SSL, s, httpErrorResponses[err].data(), (int)httpErrorResponses[err].length(),
                                    false);
                ...
                httpContextData->isParsingHttp = false;
                AsyncSocket<SSL>* failedSocket = httpContextData->upgradedWebSocket
                                                     ? (AsyncSocket<SSL>*)httpContextData->upgradedWebSocket
                                                     : (AsyncSocket<SSL>*)s;
                httpContextData->upgradedWebSocket = nullptr;
                failedSocket->uncorkWithoutSending();
                httpContextData->cDispatchFailureHandler.notify();
                if (!us_socket_is_closed(SSL, (us_socket_t*)failedSocket)) {
                    us_socket_close(SSL, (us_socket_t*)failedSocket, 0, nullptr);
                }
                return (us_socket_t*)nullptr;
            }
        });

        us_socket_context_on_writable(SSL, getSocketContext(), [](us_socket_t* s) noexcept {
        ...
        us_socket_context_on_end(SSL, getSocketContext(), [](us_socket_t* s) noexcept {
            AsyncSocket<SSL>* asyncSocket = (AsyncSocket<SSL>*)s;
            return asyncSocket->close();
        });

        us_socket_context_on_timeout(SSL, getSocketContext(), [](us_socket_t* s) noexcept {
            AsyncSocket<SSL>* asyncSocket = (AsyncSocket<SSL>*)s;
            return asyncSocket->close();
        });
   ...
   public:
    static HttpContext* create(Loop* loop, us_socket_context_options_t options = {}) {
        ...
        try {
            ...
            us_socket_context_free(SSL, (us_socket_context_t*)httpContext);
            throw;
        }
