...
struct WebSocket : AsyncSocket<SSL> {
   ...
   private:
    ...
    WebSocketData* getWebSocketData() {
        return (WebSocketData*)us_socket_ext(SSL, (us_socket_t*)this);
    }

    WebSocketData::ConstructionState* getConstructionState() {
        return reinterpret_cast<WebSocketData::ConstructionState*>(
            static_cast<unsigned char*>(us_socket_ext(SSL, (us_socket_t*)this)) + sizeof(WebSocketData) +
            sizeof(USERDATA));
    }
    ...
    void* init(bool perMessageDeflate, CompressOptions compressOptions, BackPressure&& backpressure) {
        ...
        *getConstructionState() = WebSocketData::ConstructionState::WebSocketDataConstructed;
        return this;
    }

   public:
    USERDATA* getUserData() {
        WebSocketData* webSocketData = getWebSocketData();
    ...
    using Super::pause;
    using Super::pauseRead;
    using Super::resume;
    using Super::resumeRead;
    ...
    SendStatus send(std::string_view message, OpCode opCode = OpCode::BINARY, int compress = false, bool fin = true) {
        try {
            ...
            if (message.length() >= 16 * 1024 && !compress && !SSL && !webSocketData->subscriber &&
                    ...
                    if (message.length() && opCode < 3 && webSocketData->compressionStatus == WebSocketData::ENABLED) {
                        if (compress != CompressFlags::ALREADY_COMPRESSED) {
                            ...
                            if (webSocketData->deflationStream) {
                                message =
                                    webSocketData->deflationStream->deflate(loopData->zlibContext, message, false);
                            } else {
                ...
                protocol::formatMessage<isServer>(sendBuffer, message.data(), message.length(), opCode,
    ...
    void end(int code = 0, std::string_view message = {}) {
        ...
        SendStatus sendStatus = DROPPED;
        bool sendFailed = false;
        try {
            sendStatus = send(std::string_view(closePayload, closePayloadLength), OpCode::CLOSE);
        } catch (...) {
            sendFailed = true;
            webSocketContextData->cDispatchFailureHandler.notify();
            Super::uncorkWithoutSending();
        ...
        if (!sendFailed && !this->isCorked() && sendStatus == SUCCESS) {
            this->shutdown();
        }
        ...
        *getConstructionState() = WebSocketData::ConstructionState::WebSocketDataConstructed;
        if (sendFailed && !us_socket_is_closed(SSL, (us_socket_t*)this)) {
