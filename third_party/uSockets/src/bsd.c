...
#ifndef _WIN32
static int bsd_checked_syscall_result(ssize_t result) {
    if (result > INT_MAX) {
        errno = EOVERFLOW;
...
#if defined(_WIN32) || defined(__APPLE__)
    struct us_internal_udp_packet_buffer* packet_buffer = (struct us_internal_udp_packet_buffer*)msgvec;
    return packet_buffer->len[index] <= INT_MAX ? (int)packet_buffer->len[index] : LIBUS_SOCKET_ERROR;
#else
    unsigned int length = ((struct mmsghdr*)msgvec)[index].msg_len;
    return length <= INT_MAX ? (int)length : LIBUS_SOCKET_ERROR;
#endif
}
...
void* bsd_create_udp_packet_buffer() {
    ...
    struct us_internal_udp_packet_buffer* b =
        malloc(sizeof(struct us_internal_udp_packet_buffer) + (size_t)LIBUS_UDP_MAX_SIZE * (size_t)LIBUS_UDP_MAX_NUM);
    if (!b) {
        return NULL;
    }

    for (int i = 0; i < LIBUS_UDP_MAX_NUM; i++) {
        b->buf[i] = ((char*)b) + sizeof(struct us_internal_udp_packet_buffer) + (size_t)LIBUS_UDP_MAX_SIZE * (size_t)i;
    }
    ...
    for (int n = 0; n < LIBUS_UDP_MAX_NUM; ++n) {
        b->iov[n].iov_base = &((char*)(b + 1))[(size_t)n * (size_t)LIBUS_UDP_MAX_SIZE];
...
int bsd_recv(LIBUS_SOCKET_DESCRIPTOR fd, void* buf, int length, int flags) {
    return bsd_checked_syscall_result(recv(fd, buf, (size_t)length, flags));
}
...
int bsd_write2(LIBUS_SOCKET_DESCRIPTOR fd, const char* header, int header_length, const char* payload,
    ...
    return bsd_checked_syscall_result(writev(fd, chunks, 2));
}
#else
int bsd_write2(LIBUS_SOCKET_DESCRIPTOR fd, const char* header, int header_length, const char* payload,
               int payload_length) {
...
#define MSG_NOSIGNAL 0
    ...
    return bsd_checked_syscall_result(send(fd, buf, (size_t)length, ((msg_more != 0) * MSG_MORE) | MSG_NOSIGNAL));

#else

    return bsd_checked_syscall_result(send(fd, buf, (size_t)length, MSG_NOSIGNAL));
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_listen_socket_unix(const char* path, int options) {
    ...
    size_t path_length = path ? strlen(path) : sizeof(server_address.sun_path);
    ...
    int size = (int)(offsetof(struct sockaddr_un, sun_path) + path_length);
#ifdef _WIN32
    _unlink(path);
#else
    unlink(path);
...
#define IPV6_RECVPKTINFO IPV6_PKTINFO
    ...
    if (setsockopt(listenFd, IPPROTO_IPV6, IPV6_RECVPKTINFO, (void*)&enabled, sizeof(enabled)) == -1) {
        if (errno == 92) {
#ifdef IP_PKTINFO
            (void)setsockopt(listenFd, IPPROTO_IP, IP_PKTINFO, (void*)&enabled, sizeof(enabled));
#elif IP_RECVDSTADDR
            (void)setsockopt(listenFd, IPPROTO_IP, IP_RECVDSTADDR, (void*)&enabled, sizeof(enabled));
#endif
        }
    }
    ...
    if (setsockopt(listenFd, IPPROTO_IPV6, IPV6_RECVTCLASS, (void*)&enabled, sizeof(enabled)) == -1) {
        if (errno == 92) {
            (void)setsockopt(listenFd, IPPROTO_IP, IP_RECVTOS, (void*)&enabled, sizeof(enabled));
        }
    }
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_connect_socket(const char* host, int port, const char* source_host, int options) {
    ...
    int connect_result = connect(fd, result->ai_addr, (socklen_t)result->ai_addrlen);
    freeaddrinfo(result);

    if (connect_result == LIBUS_SOCKET_ERROR && errno != EINPROGRESS) {
        bsd_close_socket(fd);
        return LIBUS_SOCKET_ERROR;
    }
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_connect_socket_unix(const char* server_path, int options) {
    ...
    memset(&server_address, 0, sizeof(server_address));
    server_address.sun_family = AF_UNIX;
    size_t path_length = server_path ? strlen(server_path) : sizeof(server_address.sun_path);
    if (path_length >= sizeof(server_address.sun_path)) {
        errno = ENAMETOOLONG;
        return LIBUS_SOCKET_ERROR;
    }
    memcpy(server_address.sun_path, server_path, path_length + 1);
    int size = (int)(offsetof(struct sockaddr_un, sun_path) + path_length);
    ...
    if (connect(fd, (struct sockaddr*)&server_address, (socklen_t)size) == LIBUS_SOCKET_ERROR && errno != EINPROGRESS) {
        bsd_close_socket(fd);
        return LIBUS_SOCKET_ERROR;
    }
