...
static struct us_listen_socket_t* us_internal_socket_context_adopt_listen_socket(struct us_socket_context_t* context,
                                                                                 ...
                                                                                 int socket_ext_size) {
    struct us_poll_t* p =
        us_create_poll(context->loop, 0, sizeof(struct us_listen_socket_t) - sizeof(struct us_poll_t));
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        return us_internal_ssl_socket_context_find_server_name_userdata(
            (struct us_internal_ssl_socket_context_t*)context, hostname_pattern);
    }
#endif
    return NULL;
}
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        us_internal_ssl_socket_context_add_server_name((struct us_internal_ssl_socket_context_t*)context,
                                                       hostname_pattern, options, user);
    }
#endif
}
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        us_internal_ssl_socket_context_remove_server_name((struct us_internal_ssl_socket_context_t*)context,
                                                          hostname_pattern);
    }
#endif
}
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        us_internal_ssl_socket_context_on_server_name(
            (struct us_internal_ssl_socket_context_t*)context,
            (void (*)(struct us_internal_ssl_socket_context_t*, const char* hostname))cb);
    }
#endif
}
...
struct us_listen_socket_t* us_socket_context_listen(int ssl, struct us_socket_context_t* context, const char* host,
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        return us_internal_ssl_socket_context_listen((struct us_internal_ssl_socket_context_t*)context, host, port,
                                                     options, socket_ext_size);
    }
#endif
    ...
    return us_internal_socket_context_adopt_listen_socket(context, listen_socket_fd, socket_ext_size);
}
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        return us_internal_ssl_socket_context_listen_unix((struct us_internal_ssl_socket_context_t*)context, path,
                                                          options, socket_ext_size);
    }
#endif
...
struct us_socket_t* us_socket_context_connect(int ssl, struct us_socket_context_t* context, const char* host, int port,
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        return (struct us_socket_t*)us_internal_ssl_socket_context_connect(
            (struct us_internal_ssl_socket_context_t*)context, host, port, source_host, options, socket_ext_size);
    }
#endif
    ...
    return us_internal_socket_context_adopt_connect_socket(context, connect_socket_fd, socket_ext_size);
}
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        return (struct us_socket_t*)us_internal_ssl_socket_context_connect_unix(
            (struct us_internal_ssl_socket_context_t*)context, server_path, options, socket_ext_size);
    }
#endif
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        return (struct us_socket_context_t*)us_internal_create_child_ssl_socket_context(
            (struct us_internal_ssl_socket_context_t*)context, context_ext_size);
    }
#endif
...
struct us_socket_t* us_socket_context_adopt_socket(int ssl, struct us_socket_context_t* context, struct us_socket_t* s,
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        return (struct us_socket_t*)us_internal_ssl_socket_context_adopt_socket(
            (struct us_internal_ssl_socket_context_t*)context, (struct us_internal_ssl_socket_t*)s, ext_size);
    }
#endif
    ...
    struct us_socket_t* new_s =
        (struct us_socket_t*)us_poll_resize(&s->p, s->context->loop, sizeof(struct us_socket_t) + ext_size);
    new_s->timeout = 255;
    new_s->long_timeout = 255;

    if (new_s->low_prio_state == 1) {
        if (!new_s->prev)
            new_s->context->loop->data.low_prio_head = new_s;
        else
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        us_internal_ssl_socket_context_on_open(
            (struct us_internal_ssl_socket_context_t*)context,
            (struct us_internal_ssl_socket_t * (*)(struct us_internal_ssl_socket_t*, int, char*, int)) on_open);
        return;
    }
#endif
...
void us_socket_context_on_close(int ssl, struct us_socket_context_t* context,
    ...
    if (ssl) {
        us_internal_ssl_socket_context_on_close(
            (struct us_internal_ssl_socket_context_t*)context,
            (struct us_internal_ssl_socket_t * (*)(struct us_internal_ssl_socket_t*, int code, void* reason)) on_close);
        return;
    }
#endif
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        us_internal_ssl_socket_context_on_data(
            (struct us_internal_ssl_socket_context_t*)context,
            (struct us_internal_ssl_socket_t * (*)(struct us_internal_ssl_socket_t*, char*, int)) on_data);
        return;
    }
#endif
...
void us_socket_context_on_writable(int ssl, struct us_socket_context_t* context,
    ...
    if (ssl) {
        us_internal_ssl_socket_context_on_writable(
            (struct us_internal_ssl_socket_context_t*)context,
            (struct us_internal_ssl_socket_t * (*)(struct us_internal_ssl_socket_t*)) on_writable);
        return;
    }
#endif
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        us_internal_ssl_socket_context_on_long_timeout(
            (struct us_internal_ssl_socket_context_t*)context,
            (struct us_internal_ssl_socket_t * (*)(struct us_internal_ssl_socket_t*)) on_long_timeout);
        return;
    }
#endif
...
void us_socket_context_on_timeout(int ssl, struct us_socket_context_t* context,
    ...
    if (ssl) {
        us_internal_ssl_socket_context_on_timeout(
            (struct us_internal_ssl_socket_context_t*)context,
            (struct us_internal_ssl_socket_t * (*)(struct us_internal_ssl_socket_t*)) on_timeout);
        return;
    }
#endif
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        us_internal_ssl_socket_context_on_end((struct us_internal_ssl_socket_context_t*)context,
                                              (struct us_internal_ssl_socket_t * (*)(struct us_internal_ssl_socket_t*))
                                                  on_end);
        return;
    }
#endif
...
#ifndef LIBUS_NO_SSL
    if (ssl) {
        us_internal_ssl_socket_context_on_connect_error(
            (struct us_internal_ssl_socket_context_t*)context,
            (struct us_internal_ssl_socket_t * (*)(struct us_internal_ssl_socket_t*, int)) on_connect_error);
        return;
    }
#endif
