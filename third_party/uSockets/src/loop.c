...
int us_internal_loop_data_init(struct us_loop_t* loop, void (*wakeup_cb)(struct us_loop_t* loop),
    ...
    loop->data.sweep_timer = NULL;
    loop->data.wakeup_async = NULL;
    loop->data.recv_buf = NULL;
    loop->data.head = 0;
    ...
    if (us_internal_async_set(loop->data.wakeup_async, (void (*)(struct us_internal_async*))wakeup_cb) != 0) {
        return -1;
    }
    return 0;
}
...
void us_internal_handle_low_priority_sockets(struct us_loop_t* loop) {
    ...
    for (s = loop_data->low_prio_head; s && loop_data->low_prio_budget > 0;
        ...
        us_poll_change(&s->p, us_socket_context(0, s)->loop,
                       (int)((unsigned int)us_poll_events(&s->p) | LIBUS_SOCKET_READABLE));
...
struct us_poll_t* us_internal_adopt_poll(struct us_loop_t* loop, LIBUS_SOCKET_DESCRIPTOR fd, unsigned int poll_ext_size,
    ...
    struct us_poll_t* poll = us_create_poll(loop, 0, poll_ext_size);
    if (!poll) {
    ...
    us_poll_init(poll, fd, poll_type);
    if (us_poll_start(poll, loop, events) != 0) {
        const int saved_errno = errno;
...
struct us_socket_t* us_adopt_accepted_socket(int ssl, struct us_socket_context_t* context,
    ...
    if (!context || accepted_fd == LIBUS_SOCKET_ERROR) {
    ...
    struct us_poll_t* accepted_p = us_internal_adopt_poll(
        context->loop, accepted_fd, sizeof(struct us_socket_t) - sizeof(struct us_poll_t) + socket_ext_size,
        POLL_TYPE_SOCKET, LIBUS_SOCKET_READABLE);
    if (!accepted_p) {
        return NULL;
    }
...
void us_internal_dispatch_ready_poll(struct us_poll_t* p, int error, int events, unsigned int raw_events) {
    switch (us_internal_poll_type(p)) {
        case POLL_TYPE_CALLBACK: {
            ...
            if (cb->cb_expects_the_loop == CALLBACK_KIND_FD_WATCHER) {
                struct us_internal_fd_watcher_t* watcher = (struct us_internal_fd_watcher_t*)cb;
                unsigned int ready = 0;
                if (raw_events & EPOLLIN) {
                    ready |= LIBUS_FD_READABLE;
                }
                if (raw_events & EPOLLOUT) {
                    ready |= LIBUS_FD_WRITABLE;
                }
                if (raw_events & EPOLLERR) {
                    ready |= LIBUS_FD_ERROR;
                }
                if (raw_events & (EPOLLHUP | EPOLLRDHUP)) {
                    ready |= LIBUS_FD_HANGUP;
                }
                watcher->ready_cb((struct us_fd_watcher_t*)watcher, ready);
                break;
            }
            if (!cb->leave_poll_ready) {
                us_internal_accept_poll_event(p);
            }
            cb->cb(cb->cb_expects_the_loop == CALLBACK_KIND_ASYNC ? (struct us_internal_callback_t*)cb->loop
        ...
        case POLL_TYPE_SOCKET: {
            ...
            if (error) {
                (void)us_socket_close(0, s, 0, NULL);
                return;
            }
            ...
            if (events & LIBUS_SOCKET_READABLE) {
                if (s->context->is_low_prio(s)) {
                    if (s->low_prio_state == 2) {
                        ...
                        us_poll_change(&s->p, us_socket_context(0, s)->loop,
                                       (int)((unsigned int)us_poll_events(&s->p) & LIBUS_SOCKET_WRITABLE));
...
int us_loop_integrate(struct us_loop_t* loop) {
    return us_timer_set(loop->data.sweep_timer, (void (*)(struct us_timer_t*))sweep_timer_cb,
