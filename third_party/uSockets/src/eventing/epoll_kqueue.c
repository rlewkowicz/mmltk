...
void us_loop_run(struct us_loop_t* loop) {
    ...
    while (loop->num_polls) {
        ...
        for (loop->current_ready_poll = 0; loop->current_ready_poll < loop->num_ready_polls;
...
void us_internal_loop_update_pending_ready_polls(struct us_loop_t* loop, struct us_poll_t* old_poll,
                                                 struct us_poll_t* new_poll, int old_events, int new_events) {
#ifdef LIBUS_USE_EPOLL
...
int kqueue_change(int kqfd, int fd, int old_events, int new_events, void* user_data) {
    ...
    if ((new_events & LIBUS_SOCKET_READABLE) != (old_events & LIBUS_SOCKET_READABLE)) {
        EV_SET(&change_list[change_length++], fd, EVFILT_READ,
               (new_events & LIBUS_SOCKET_READABLE) ? EV_ADD : EV_DELETE, 0, 0, user_data);
    }

    if ((new_events & LIBUS_SOCKET_WRITABLE) != (old_events & LIBUS_SOCKET_WRITABLE)) {
        EV_SET(&change_list[change_length++], fd, EVFILT_WRITE,
               (new_events & LIBUS_SOCKET_WRITABLE) ? EV_ADD : EV_DELETE, 0, 0, user_data);
    }
...
struct us_timer_t* us_create_timer(struct us_loop_t* loop, int fallthrough, unsigned int ext_size) {
    struct us_poll_t* p =
        us_create_poll(loop, fallthrough, sizeof(struct us_internal_callback_t) - sizeof(struct us_poll_t) + ext_size);
