...
inline struct boost_block_poll_t* valid_boost_block(const std::weak_ptr<boost_block_poll_t>& weakBoostBlock,
                                                    unsigned char nr, boost::system::error_code ec) {
    if (ec == boost::asio::error::operation_aborted) {
        return nullptr;
    }
    if (auto observe = weakBoostBlock.lock()) {
        struct boost_block_poll_t* boost_block = observe.get();
        return nr == boost_block->nr ? boost_block : nullptr;
    }
    return nullptr;
}
...
void poll_for_error(struct boost_block_poll_t* boost_block) {
    ...
    polls++;
    boost_block->async_wait(
        boost::asio::posix::descriptor::wait_type::wait_error,
        [nr = boost_block->nr,
         weakBoostBlock = std::weak_ptr<boost_block_poll_t>(boost_block->isValid)](boost::system::error_code ec) {
            polls--;

            if (auto boost_block = valid_boost_block(weakBoostBlock, nr, ec)) {
...
inline void handle_read(const std::weak_ptr<boost_block_poll_t>& weakBoostBlock, unsigned char nr,
...
void poll_for_read(struct boost_block_poll_t* boost_block) {
    polls++;
#ifndef LIBUS_USE_OLD_ASIO
    boost_block->async_wait(boost::asio::posix::descriptor::wait_type::wait_read,
                            [nr = boost_block->nr, weakBoostBlock = std::weak_ptr<boost_block_poll_t>(
                                                       boost_block->isValid)](boost::system::error_code ec) {
                                polls--;
                                handle_read(weakBoostBlock, nr, ec);
                            });
#else
    boost_block->async_read_some(
        boost::asio::null_buffers(),
        [nr = boost_block->nr, weakBoostBlock = std::weak_ptr<boost_block_poll_t>(boost_block->isValid)](
...
inline void handle_write(const std::weak_ptr<boost_block_poll_t>& weakBoostBlock, unsigned char nr,
                         boost::system::error_code ec) {
    if (auto boost_block = valid_boost_block(weakBoostBlock, nr, ec)) {
...
void poll_for_write(struct boost_block_poll_t* boost_block) {
    polls++;
#ifndef LIBUS_USE_OLD_ASIO
    boost_block->async_wait(boost::asio::posix::descriptor::wait_type::wait_write,
                            [nr = boost_block->nr, weakBoostBlock = std::weak_ptr<boost_block_poll_t>(
                                                       boost_block->isValid)](boost::system::error_code ec) {
                                polls--;
...
struct us_timer_t* us_create_timer(struct us_loop_t* loop, int fallthrough, unsigned int ext_size) {
    ...
    cb->p.poll_type = POLL_TYPE_CALLBACK;
...
void poll_for_timeout(struct boost_timer* b_timer, int repeat_ms) {
    b_timer->timer.async_wait(
        [nr = b_timer->nr, repeat_ms,
         weakBoostBlock = std::weak_ptr<boost_timer>(b_timer->isValid)](const boost::system::error_code& ec) {
            if (ec != boost::asio::error::operation_aborted) {
