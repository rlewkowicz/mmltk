...
struct us_loop_t;
struct us_poll_t;
struct us_fd_watcher_t;
struct us_udp_socket_t;
struct us_udp_packet_buffer_t;

enum {
    LIBUS_FD_READABLE = 1U << 0U,
    LIBUS_FD_WRITABLE = 1U << 1U,
    LIBUS_FD_ERROR = 1U << 2U,
    LIBUS_FD_HANGUP = 1U << 3U
};
...
int us_timer_set(struct us_timer_t* timer, void (*cb)(struct us_timer_t* t), int ms, int repeat_ms);
...
/* Returns NULL with errno set when the context or its extension cannot be allocated. */
...
/* Returns NULL with errno set if the socket cannot be resized. On failure the original
 * socket remains owned by and linked to its source context. */
...
/* Returns NULL with errno set if the loop or any mandatory loop-owned resource cannot be created. */
...
/* Runs one already-integrated loop iteration. timeout_ms follows epoll_wait semantics:
 * -1 blocks until readiness, while 0 performs a nonblocking turn. Returns the number of
 * ready records dispatched, 0 on timeout/interruption, or -1 on an invalid argument or
 * unrecoverable epoll error. */
int us_loop_run_once(struct us_loop_t* loop, int timeout_ms);

void us_wakeup_loop(struct us_loop_t* loop);

int us_loop_integrate(struct us_loop_t* loop);

long long us_loop_iteration_number(struct us_loop_t* loop);

/* Registers an already-open descriptor with the loop. The descriptor remains owned by the caller.
 * Error and hangup readiness are always reported in addition to the requested readable/writable
 * mask. It is valid to close the watcher from its readiness callback. The callback must not
 * unwind through this C boundary. */
...
/* Returns 0 when the new interest mask is installed, otherwise -1 with errno set. */
...
/* Returns NULL with errno set on allocation failure and leaves p valid and unchanged. */
