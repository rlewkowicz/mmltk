...
const MIN_HEAD_CONTENT_HEIGHT: f32 = 20.0;

const HEAD_SEPARATOR_WIDTH: f32 = 1.0;

const OUTLINE_WIDTH: f32 = 1.0;

const OUTLINE_COLOR: Color = Color::from_rgba8(0xd3, 0xd3, 0xd3, 0.5);
    ...
    Theme: Catalog,
{
    ...
    fn layout(&mut self, tree: &mut Tree, renderer: &Renderer, limits: &Limits) -> Node {
        let limits = limits
            .width(Length::Fill.max(self.max_width))
            .height(Length::Fill.max(self.max_height));
    ...
    fn draw(
        ...
        if bounds.intersects(viewport) {
            renderer.fill_quad(
                renderer::Quad {
                    bounds: Rectangle {
                        x: bounds.x - OUTLINE_WIDTH,
                        y: bounds.y - OUTLINE_WIDTH,
                        width: bounds.width + OUTLINE_WIDTH * 2.0,
                        height: bounds.height + OUTLINE_WIDTH * 2.0,
                    },
                    border: Border {
                        radius: (style_sheet.border_radius + OUTLINE_WIDTH).into(),
                        width: OUTLINE_WIDTH,
                        color: OUTLINE_COLOR,
                    },
                    shadow: style_sheet.shadow,
                    snap: false,
                },
                Color::TRANSPARENT,
            );
    ...
    Renderer: renderer::Renderer + iced_core::text::Renderer<Font = iced_core::Font>,
{
    ...
    size.height = size.height.max(MIN_HEAD_CONTENT_HEIGHT);

    head = head
        .align(Alignment::Start, Alignment::Center, size)
        .translate(Vector::new(padding.left, padding.top));
    ...
    Theme: Catalog,
{
    ...
    let separator_bounds = Rectangle {
        x: bounds.x,
        y: bounds.y + bounds.height - HEAD_SEPARATOR_WIDTH,
        width: bounds.width,
        height: HEAD_SEPARATOR_WIDTH,
    };
    if separator_bounds.intersects(viewport) {
        let mut separator_color = style.head_text_color;
        separator_color.a *= 0.32;
        renderer.fill_quad(
            renderer::Quad {
