...
include("${CMAKE_CURRENT_LIST_DIR}/mmltk_cef_distribution.cmake")
...
set(MMLTK_INSTALL_CEF_RUNTIMEDIR "${CMAKE_INSTALL_LIBDIR}/cef" CACHE STRING
    "Install-relative directory for packaged Chromium Embedded Framework runtime files")
...
option(MMLTK_WARNINGS_AS_ERRORS
    "Treat host-side compiler warnings as build errors"
    ON)
...
option(MMLTK_DEV_ENABLE_UBSAN
    ...
    OFF)
option(MMLTK_USE_MOLD
    "Use the mold linker for host-side link steps when supported"
    ON)
option(MMLTK_USE_CCACHE
    "Use ccache as the default compiler launcher when it is available"
    ON)
...
if(MMLTK_USE_CCACHE)
    find_program(MMLTK_CCACHE_PROGRAM ccache)
    if(MMLTK_CCACHE_PROGRAM)
        foreach(MMLTK_COMPILER_LAUNCHER_LANGUAGE IN ITEMS C CXX CUDA)
            if(NOT CMAKE_${MMLTK_COMPILER_LAUNCHER_LANGUAGE}_COMPILER_LAUNCHER)
                set(CMAKE_${MMLTK_COMPILER_LAUNCHER_LANGUAGE}_COMPILER_LAUNCHER
                    "${MMLTK_CCACHE_PROGRAM}" CACHE STRING
                    "Default ${MMLTK_COMPILER_LAUNCHER_LANGUAGE} compiler launcher for mmltk builds")
            endif()
        endforeach()
        message(STATUS "Using ccache compiler launcher: ${MMLTK_CCACHE_PROGRAM}")
    else()
        message(STATUS "ccache requested, but no ccache executable was found")
    endif()
endif()

set(MMLTK_HOST_LINKER_FLAGS "")
if(MMLTK_USE_MOLD)
    if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "Clang")
        set(MMLTK_HOST_LINKER_FLAGS "-fuse-ld=mold")
        message(STATUS "Using mold for host-side linking")
    else()
        message(STATUS
            "mold requested, but host compiler `${CMAKE_CXX_COMPILER_ID}` does not advertise -fuse-ld support")
    endif()
endif()
...
if(MMLTK_WARNINGS_AS_ERRORS)
    list(APPEND MMLTK_PROJECT_CXX_WARNING_FLAGS -Werror)
